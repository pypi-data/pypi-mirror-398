...
class YvosTest(Dataset):
    def __init__(self, img_size, data_root, split):
        ...
        all_folders = glob.glob(
            os.path.join(self.data_root, "youtube-vos-2019", split, "JPEGImages", "*")
        )
    ...
    def __getitem__(self, index):
        ...
        path = sorted(glob.glob(os.path.join(folder, "*")))[
...
if __name__ == "__main__":
    ...
    filtering_vis(
        data_root, "youtube_vos", "train", flow_data_path, threshold_dict, mse_threshold
    )

    flow_data_path = os.path.join(data_root, "flow_data/youtube_vos_test.json")
    filtering_vis(
        data_root, "youtube_vos", "test", flow_data_path, threshold_dict, mse_threshold
    )
