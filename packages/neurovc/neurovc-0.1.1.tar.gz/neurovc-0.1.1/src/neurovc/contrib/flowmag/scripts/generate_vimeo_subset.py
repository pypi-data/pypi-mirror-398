...
class VimeoTest(Dataset):
    def __init__(self, img_size, data_root, split):
        ...
        with open(
            os.path.join(
                data_root, "vimeo/vimeo_septuplet/sep_{}list.txt".format(split)
            ),
            "r",
        ) as file:
            self.folders = [
                os.path.join(
                    data_root, "vimeo/vimeo_septuplet/sequences", line.rstrip()
                )
...
if __name__ == "__main__":
    ...
    filtering_vis(
        data_root, "vimeo", "train", flow_data_path, threshold_dict, mse_threshold
    )

    flow_data_path = os.path.join(data_root, "flow_data/vimeo_test.json")
    filtering_vis(
        data_root, "vimeo", "test", flow_data_path, threshold_dict, mse_threshold
    )
