...
def get_folders(data_root, json_path):
    with open(json_path, "r") as f:
        data = json.load(f)
    return [
        os.path.join(data_root, "{}".format(item["ytid"])) for item in data["videos"]
    ]


class UVOTest(Dataset):
    def __init__(self, img_size, data_root, split, sampling=1):
        ...
        if split == "train":
            json_path = os.path.join(
                data_root, "uvo_frames/UVO_video_{}_dense.json".format(split)
            )
        elif split == "test":
            json_path = os.path.join(
                data_root, "uvo_frames/UVO_video_{}_dense.json".format(split)
        ...
        all_folders = get_folders(
            os.path.join(data_root, "uvo_frames/uvo_videos_dense"), json_path
        )
        ...
        for f in all_folders:
            if len(glob.glob(os.path.join(f, "*"))) >= nbr_frame + sampling - 1:
                folders.append(f)
                total_num += (
                    len(glob.glob(os.path.join(f, "*"))) - sampling + 1
                ) // nbr_frame
...
def get_loader(sampling, split, img_size, data_root, batch_size, shuffle, num_workers):
    dataset = UVOTest(img_size, data_root, split, sampling)
    return DataLoader(
        dataset,
...
if __name__ == "__main__":
    ...
    motion_statistics(data_root, loader, "uvo", "train_n{}".format(sampling))
    ...
    motion_statistics(data_root, loader, "uvo", "test_n{}".format(sampling))
    ...
    threshold_dict = {"max_dist": 20, "80th": 2, "0.01st": 0.1}
    mse_threshold = 10

    flow_data_path = os.path.join(data_root, "flow_data/uvo_train_n1.json")
    filtering_vis(
        data_root, "uvo", "train_n1", flow_data_path, threshold_dict, mse_threshold
    )

    flow_data_path = os.path.join(data_root, "flow_data/uvo_train_n5.json")
    filtering_vis(
        data_root, "uvo", "train_n5", flow_data_path, threshold_dict, mse_threshold
    )

    flow_data_path = os.path.join(data_root, "flow_data/uvo_test_n1.json")
    filtering_vis(
        data_root, "uvo", "test_n1", flow_data_path, threshold_dict, mse_threshold
    )

    flow_data_path = os.path.join(data_root, "flow_data/uvo_test_n5.json")
    filtering_vis(
        data_root, "uvo", "test_n5", flow_data_path, threshold_dict, mse_threshold
    )
