...
class TaoTest(Dataset):
    def __init__(self, img_size, data_root, split, sampling=1):
        ...
        all_folders = glob.glob(
            os.path.join(self.data_root, "tao/frames", split, "*", "*")
        )
...
if __name__ == "__main__":
    ...
    filtering_vis(
        data_root, "tao", "train_n1", flow_data_path, threshold_dict, mse_threshold
    )
    flow_data_path = os.path.join(data_root, "flow_data/tao_train_n5.json")
    filtering_vis(
        data_root, "tao", "train_n5", flow_data_path, threshold_dict, mse_threshold
    )

    flow_data_path = os.path.join(data_root, "flow_data/tao_val_n1.json")
    filtering_vis(
        data_root, "tao", "val_n1", flow_data_path, threshold_dict, mse_threshold
    )
    flow_data_path = os.path.join(data_root, "flow_data/tao_val_n5.json")
    filtering_vis(
        data_root, "tao", "val_n5", flow_data_path, threshold_dict, mse_threshold
    )
