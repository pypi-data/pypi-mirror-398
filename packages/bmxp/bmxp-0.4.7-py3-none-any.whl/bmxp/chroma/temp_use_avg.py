def interpolate_rts(combined, batch_num):
    ...
    """
    This fills in the missing RTs of the second batch with the first, based on consensus of matched in that region

    """
    ds1_rt_col = "RT"
    ds2_rt_col = f"RT_{batch_num}"
    combined = combined.copy()
    combined.sort_values(ds1_rt_col, inplace=True)
    combined.reset_index(inplace=True)  # We'll fix this later
    # Identify known and missing (from ds2)
    known = combined[combined[ds1_rt_col].notnull() & combined[ds2_rt_col].notnull()]
    missing = combined[combined[ds2_rt_col].isnull()]
    # For quick lookup
    known_rts = known[ds1_rt_col].to_numpy()
    known_diffs = (known[ds1_rt_col] - known[ds2_rt_col]).to_numpy()

    result_rt = combined[ds2_rt_col].to_numpy()

    # missing are those present in ds1 but not ds2
    for idx in missing.index:
        rt_val = combined.at[idx, ds1_rt_col]
        ...
        if low < high:
            ...
            if not np.isnan(avg_diff):
                result_rt[idx] = rt_val - avg_diff

    # Write back updated values
    combined[ds2_rt_col] = result_rt
    still_null = pd.isnull(combined[ds2_rt_col])
    combined.loc[still_null, ds2_rt_col] = combined[ds1_rt_col]
...
def combine_batches(batches, start=0, stop=None, to_csv=True):
    ...
    for batch_num in range(start, stop):
        ...
        try:
            ...
            current_batch["RT"] = current_batch["RT_Average"]
        except:
            continue
        ...
        scalers = a.scalers
        stds = a.stds
        a.report(filepath=COMB_DIR + f"batch_{batch_num}.pdf")
        ...
        # todo use m/z trends to interpolate mz
        ...
        # keep all the ds2 columns and rename
