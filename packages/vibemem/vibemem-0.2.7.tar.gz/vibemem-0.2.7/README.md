...
### Re-embed (repair / migrate embeddings)

If you enable an embedding service after you already have memories, or you change the embedding model, vector search can fail until existing records are re-embedded.

Re-embed in-place (updates vectors on existing records; default is only those missing vectors):

```bash
vibemem reembed --all
```

If you changed to an embedding model with a different vector dimension, you can’t re-embed into the existing collection; migrate into a new collection instead:

```bash
vibemem reembed --all --to-collection VibeMemMemory_v2
vibemem config set --weaviate-collection VibeMemMemory_v2
```
