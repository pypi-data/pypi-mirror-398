...
def test_weaviate_search_skips_invalid_records(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(ws.wfilters, "Filter", FakeFilter)

    good = SimpleNamespace(
        uuid="1",
        properties={"text": "ok", "mem_type": "finding", "scope_type": "global", "scope_id": "global"},
        metadata=SimpleNamespace(score=1.0),
    )
    bad = SimpleNamespace(
        uuid="2",
        properties={"text": "bad", "mem_type": "finding", "tags": None, "scope_type": "global", "scope_id": "global"},
        metadata=SimpleNamespace(score=0.9),
    )

    class StubQueryWithInvalid(StubQuery):
        def hybrid(self, *args: Any, **kwargs: Any) -> Any:
            self.last_kwargs = dict(kwargs)
            return SimpleNamespace(objects=[good, bad])

        def bm25(self, *args: Any, **kwargs: Any) -> Any:
            self.last_kwargs = dict(kwargs)
            return SimpleNamespace(objects=[good, bad])

    store = _make_store(StubQueryWithInvalid())
