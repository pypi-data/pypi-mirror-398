...
if sys.platform == "win32" and sys.maxsize == (2**31 - 1):
    ...
    error = """
    ====================================================================
    `hyperspec` currently doesn't support 32-bit Python windows builds. If
    this is important for your use case, please comment on this issue:

    https://github.com/jcrist/hyperspec/issues/845
    ====================================================================
    """
...
SANITIZE = os.environ.get("HYPERSPEC_SANITIZE", False)
COVERAGE = os.environ.get("HYPERSPEC_COVERAGE", False)
DEBUG = os.environ.get("HYPERSPEC_DEBUG", SANITIZE or COVERAGE)
...
ext_modules = [
    Extension(
        "hyperspec._core",
        [os.path.join("src", "hyperspec", "_core.c")],
