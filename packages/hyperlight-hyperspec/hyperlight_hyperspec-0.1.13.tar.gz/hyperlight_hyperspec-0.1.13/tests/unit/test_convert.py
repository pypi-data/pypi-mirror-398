...
import hyperspec
from hyperspec import Meta, Struct, ValidationError, convert, to_builtins
...
class TestExt:
    def test_ext(self):
        x = hyperspec.msgpack.Ext(1, b"123")
        assert convert(x, hyperspec.msgpack.Ext) is x

    def test_ext_errors(self):
        with pytest.raises(ValidationError, match="Expected `ext`, got `int`"):
            convert(1, hyperspec.msgpack.Ext)

        with pytest.raises(ValidationError, match="Expected `int`, got `ext`"):
            convert(hyperspec.msgpack.Ext(1, b"123"), int)
...
class TestEnum:
    ...
    def test_enum_missing(self):
        ...
        assert hyperspec.convert("a", Ex) is Ex.A
        assert hyperspec.convert("return-A", Ex) is Ex.A
        assert hyperspec.convert("return-B", Ex) is Ex.B
        with pytest.raises(ValidationError, match="Invalid enum value 'error'"):
            hyperspec.convert("error", Ex)
        with pytest.raises(ValidationError, match="Invalid enum value 'other'"):
            hyperspec.convert("other", Ex)
    ...
    def test_intenum_missing(self):
        ...
        assert hyperspec.convert(1, Ex) is Ex.A
        assert hyperspec.convert(3, Ex) is Ex.A
        assert hyperspec.convert(-4, Ex) is Ex.B
        with pytest.raises(ValidationError, match="Invalid enum value 5"):
            hyperspec.convert(5, Ex)
        with pytest.raises(ValidationError, match="Invalid enum value 6"):
            hyperspec.convert(6, Ex)
