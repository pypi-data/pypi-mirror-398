...
def test_module_dir():
    assert set(dir(hyperspec.toml)) == {"encode", "decode"}
...
def test_tomli_not_installed_error(monkeypatch):
    ...
    with pytest.raises(ImportError, match="conda install"):
        hyperspec.toml.decode("a = 1", type=int)
...
def test_roundtrip_any(val):
    msg = hyperspec.toml.encode({"x": val})
    res = hyperspec.toml.decode(msg)["x"]
    assert res == val
...
def test_roundtrip_typed(val, type):
    msg = hyperspec.toml.encode({"x": val})
    res = hyperspec.toml.decode(msg, type=Dict[str, type])["x"]
    assert res == val


@needs_encode
def test_encode_output_type():
    msg = hyperspec.toml.encode({"x": 1})
...
def test_encode_error():
    ...
    with pytest.raises(TypeError, match="Encoding objects of type Oops is unsupported"):
        hyperspec.toml.encode({"x": Oops()})


@needs_encode
@needs_decode
def test_encode_enc_hook():
    msg = hyperspec.toml.encode({"x": Decimal(1.5)}, enc_hook=str)
    assert hyperspec.toml.decode(msg) == {"x": "1.5"}
...
def test_encode_order(order):
    ...
    res = hyperspec.toml.encode(msg, order=order)
    if order:
...
def test_decode_str_or_bytes_like():
    assert hyperspec.toml.decode("a = 1") == {"a": 1}
    assert hyperspec.toml.decode(b"a = 1") == {"a": 1}
    assert hyperspec.toml.decode(bytearray(b"a = 1")) == {"a": 1}
    assert hyperspec.toml.decode(memoryview(b"a = 1")) == {"a": 1}
    with pytest.raises(TypeError):
        hyperspec.toml.decode(1)
...
def test_decode_strict_or_lax(strict):
    ...
    if strict:
        with pytest.raises(hyperspec.ValidationError, match="Expected `int`"):
            hyperspec.toml.decode(msg, type=typ, strict=strict)
    else:
        res = hyperspec.toml.decode(msg, type=typ, strict=strict)
        assert res == {"a": [1, 2]}
...
def test_decode_dec_hook():
    ...
    res = hyperspec.toml.decode("a = '1.5'", type=Dict[str, Decimal], dec_hook=dec_hook)
