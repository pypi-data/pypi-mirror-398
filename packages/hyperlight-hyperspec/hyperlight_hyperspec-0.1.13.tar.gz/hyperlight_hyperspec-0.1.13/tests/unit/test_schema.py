...
def test_any():
    assert hyperspec.json.schema(Any) == {}


def test_raw():
    assert hyperspec.json.schema(hyperspec.Raw) == {}


def test_msgpack_ext():
    with pytest.raises(TypeError):
        assert hyperspec.json.schema(hyperspec.msgpack.Ext)


def test_custom():
    with pytest.raises(TypeError, match="Generating JSON schema for custom types"):
        assert hyperspec.json.schema(complex)

    schema = {"type": "string", "format": "complex"}

    assert (
        hyperspec.json.schema(Annotated[complex, Meta(extra_json_schema=schema)])
        == schema
    )
...
def test_custom_schema_hook():
    ...
    assert hyperspec.json.schema(complex, schema_hook=schema_hook) == schema
    assert hyperspec.json.schema(
        Annotated[complex, Meta(extra_json_schema={"title": "A complex field"})],
        schema_hook=schema_hook,
    ) == {**schema, "title": "A complex field"}

    with pytest.raises(TypeError, match="Generating JSON schema for custom types"):
        hyperspec.json.schema(slice, schema_hook=schema_hook)


def test_none():
    assert hyperspec.json.schema(None) == {"type": "null"}


def test_bool():
    assert hyperspec.json.schema(bool) == {"type": "boolean"}


def test_int():
    assert hyperspec.json.schema(int) == {"type": "integer"}


def test_float():
    assert hyperspec.json.schema(float) == {"type": "number"}


def test_string():
    assert hyperspec.json.schema(str) == {"type": "string"}


@pytest.mark.parametrize("typ", [bytes, bytearray, memoryview])
def test_binary(typ):
    assert hyperspec.json.schema(typ) == {
        "type": "string",
...
def test_datetime(annot, extra):
    ...
    assert hyperspec.json.schema(typ) == {"type": "string", **extra}
...
def test_date():
    assert hyperspec.json.schema(datetime.date) == {
        "type": "string",
        "format": "date",
    }
...
def test_timedelta():
    assert hyperspec.json.schema(datetime.timedelta) == {
        "type": "string",
        "format": "duration",
    }
...
def test_uuid():
    assert hyperspec.json.schema(uuid.UUID) == {
        "type": "string",
        "format": "uuid",
    }
...
def test_decimal():
    assert hyperspec.json.schema(decimal.Decimal) == {
        "type": "string",
        "format": "decimal",
    }
...
def test_newtype():
    ...
    assert hyperspec.json.schema(UserId) == {"type": "string"}
    assert hyperspec.json.schema(Annotated[UserId, Meta(max_length=10)]) == {
        "type": "string",
        "maxLength": 10,
    }
...
def test_sequence_any(typ):
    assert hyperspec.json.schema(typ) == {"type": "array"}
...
def test_sequence_typed(cls):
    ...
    typ = cls[args]
    assert hyperspec.json.schema(typ) == {"type": "array", "items": {"type": "integer"}}
...
def test_dict_any(typ):
    assert hyperspec.json.schema(typ) == {"type": "object"}
...
def test_int_enum():
    ...
    assert hyperspec.json.schema(Example) == {
        "$ref": "#/$defs/Example",
...
def test_int_literal():
    assert hyperspec.json.schema(Literal[3, 1, 2]) == {"enum": [1, 2, 3]}


def test_str_literal():
    assert hyperspec.json.schema(Literal["c", "a", "b"]) == {"enum": ["a", "b", "c"]}


def test_struct_object():
    class Point(hyperspec.Struct, forbid_unknown_fields=True):
        x: int
        y: int

    class Polygon(hyperspec.Struct):
    ...
    assert hyperspec.json.schema(Polygon) == {
        "$ref": "#/$defs/Polygon",
        "$defs": {
            "Polygon": {
                "title": "Polygon",
...
def test_struct_array_like(forbid_unknown_fields):
    class Example(
        hyperspec.Struct, array_like=True, forbid_unknown_fields=forbid_unknown_fields
    ):
        """An example docstring"""
    ...
    assert hyperspec.json.schema(Example) == sol
...
def test_struct_object_tagged():
    ...
    assert hyperspec.json.schema(Point) == {
        "$ref": "#/$defs/Point",
        "$defs": {
            "Point": {
                "title": "Point",
...
def test_struct_array_tagged():
    class Point(hyperspec.Struct, tag=True, array_like=True):
        x: int
        y: int
...
def test_struct_keyword_only():
    ...
    assert hyperspec.json.schema(Test) == {
        "$ref": "#/$defs/Test",
        "$defs": {
            "Test": {
                "title": "Test",
...
def test_struct_array_keyword_only():
    class Base(hyperspec.Struct, kw_only=True, array_like=True):
        x: int = 1
        y: int
        z: int = 2
...
def test_generic_namedtuple():
    ...
    assert hyperspec.json.schema(Ex) == {
        "$ref": "#/$defs/Ex",
        "$defs": {
            "Ex": {
                "title": "Ex",
    ...
    assert hyperspec.json.schema(Ex[int]) == {
        "$ref": "#/$defs/Ex_int_",
        "$defs": {
            "Ex_int_": {
                "title": "Ex[int]",
...
def test_struct_tagged_union():
    ...
    assert hyperspec.json.schema(Union[Point, Point3D]) == {
...
def test_struct_tagged_union_mixed_types():
    ...
    assert hyperspec.json.schema(Union[Point, Point3D, int, float]) == {
        "anyOf": [
            {"type": "integer"},
            {"type": "number"},
            {
...
def test_struct_array_union():
    class Point(hyperspec.Struct, array_like=True, tag=True):
        x: int
        y: int
...
def test_struct_unset_fields():
    class Ex(hyperspec.Struct):
        x: Union[int, hyperspec.UnsetType] = hyperspec.UNSET
...
def test_generic_struct():
    class Ex(hyperspec.Struct, Generic[T]):
        """An example docstring"""
...
def test_generic_struct_tagged_union():
    class Point(hyperspec.Struct, Generic[T], tag=True):
        x: T
        y: T
    ...
    res = hyperspec.json.schema(Union[Point[int], Point3D[int]])
    assert res == sol
...
def test_numeric_metadata(field, constraint):
    ...
    assert hyperspec.json.schema(typ) == {"type": "integer", constraint: 2}
...
def test_string_metadata(field, val, constraint):
    ...
    assert hyperspec.json.schema(typ) == {"type": "string", constraint: val}
...
def test_dict_key_metadata(field, val, constraint):
    typ = Annotated[str, Meta(**{field: val})]
    assert hyperspec.json.schema(Dict[typ, int]) == {
        "type": "object",
        "additionalProperties": {"type": "integer"},
...
def test_array_metadata(typ, field, constraint):
    ...
    assert hyperspec.json.schema(typ) == {"type": "array", constraint: 2}
...
def test_object_metadata(field, constraint):
    ...
    assert hyperspec.json.schema(typ) == {"type": "object", constraint: 2}
...
def test_component_names_collide():
    s1 = """
    import hyperspec
    Ex = hyperspec.defstruct("Ex", [("x", int), ("y", int)])
    """

    s2 = """
    import hyperspec
    Ex = hyperspec.defstruct("Ex", [("a", str), ("b", str)])
    """

    with temp_module(s1) as m1, temp_module(s2) as m2:
        (r1, r2), components = hyperspec.json.schema_components([m1.Ex, m2.Ex])
...
def test_schema_components_collects_subtypes():
    ...
    (s,), components = hyperspec.json.schema_components([Dict[str, ExDataclass]])
...
def test_ref_template():
    class Ex1(hyperspec.Struct):
        a: int

    class Ex2(hyperspec.Struct):
        b: Ex1

    (s1, s2), components = hyperspec.json.schema_components(
