...
import hyperspec
from hyperspec import UNSET, Meta, Struct, UnsetType, ValidationError
...
class TestDecoder:
    ...
    def test_decode_dec_hook_errors_wrapped(self, err_cls, proto):
        ...
        with pytest.raises(hyperspec.ValidationError, match="Oh no!") as rec:
        ...
        msg = proto.encode(["some string"])
        with pytest.raises(hyperspec.ValidationError, match=r"Oh no! - at `\$\[0\]`"):
...
class TestIntEnum:
    ...
    def test_int_lookup_reused(self):
        ...
        dec = hyperspec.msgpack.Decoder(Test)  # noqa
        count = sys.getrefcount(Test.__hyperspec_cache__)
        dec2 = hyperspec.msgpack.Decoder(Test)
        count2 = sys.getrefcount(Test.__hyperspec_cache__)
        assert count2 == count + 1

        # Reference count decreases when decoder is dropped
        del dec2
        gc.collect()
        count3 = sys.getrefcount(Test.__hyperspec_cache__)
        assert count == count3
    ...
    def test_int_lookup_gc(self):
        ...
        assert gc.is_tracked(Test.__hyperspec_cache__)
    ...
    def test_int_lookup_values_out_of_range(self, values):
        ...
        with pytest.raises(NotImplementedError):
            hyperspec.msgpack.Decoder(myenum)

    def test_hyperspec_cache_overwritten(self):
        class Test(enum.IntEnum):
            A = 1

        Test.__hyperspec_cache__ = 1

        with pytest.raises(RuntimeError, match="__hyperspec_cache__"):
            hyperspec.msgpack.Decoder(Test)
    ...
    def test_compact(self, values):
        myenum = enum.IntEnum("myenum", [(f"x{i}", v) for i, v in enumerate(values)])
        dec = hyperspec.msgpack.Decoder(myenum)

        assert hasattr(myenum, "__hyperspec_cache__")

        for val in myenum:
            msg = hyperspec.msgpack.encode(val)
            val2 = dec.decode(msg)
            assert val == val2

        for bad in [-1000, min(values) - 1, max(values) + 1, 1000]:
            with pytest.raises(ValidationError):
                dec.decode(hyperspec.msgpack.encode(bad))
...
class TestEnum:
    ...
    def test_random_enum_same_lengths(self, rand, length, nitems):
        ...
        for val in myenum:
            msg = hyperspec.msgpack.encode(val.value)
            val2 = dec.decode(msg)
            assert val == val2
        ...
        for _ in range(10):
            key = unique_str()
            with pytest.raises(ValidationError):
                dec.decode(hyperspec.msgpack.encode(key))
...
class TestLiterals:
    def test_empty_errors(self):
        with pytest.raises(
            ...
            hyperspec.msgpack.Decoder(Literal[()])
    ...
    def test_invalid_values(self, typ):
        with pytest.raises(TypeError, match="not supported"):
            hyperspec.msgpack.Decoder(typ)

    def test_decode_literal_int_str_and_none_uncached_and_cached(self):
        values = (45987, "an_unlikely_string", None)
        literal = Literal[values]
        assert not hasattr(literal, "__hyperspec_cache__")
        uncached = hyperspec.msgpack.Decoder(literal)
        assert hasattr(literal, "__hyperspec_cache__")
        cached = hyperspec.msgpack.Decoder(literal)

        for val in values:
            assert uncached.decode(hyperspec.msgpack.encode(val)) == val
            assert cached.decode(hyperspec.msgpack.encode(val)) == val

    def test_cache_refcounts(self):
        literal = Literal[1, 2, "three", "four"]
        dec = hyperspec.msgpack.Decoder(literal)  # noqa
        cache = literal.__hyperspec_cache__
        count = sys.getrefcount(cache)
        dec2 = hyperspec.msgpack.Decoder(literal)
    ...
    def test_hyperspec_cache_overwritten(self, val):
        literal = Literal["a", "highly", "improbable", "set", "of", "strings"]

        literal.__hyperspec_cache__ = val
    ...
    def test_multiple_literals(self):
        ...
        dec = hyperspec.msgpack.Decoder(both)

        assert not hasattr(both, "__hyperspec_cache__")

        for val in [-1, -2, -3, "apple", "banana"]:
            assert dec.decode(hyperspec.msgpack.encode(val)) == val

        with pytest.raises(ValidationError, match="Invalid enum value 4"):
            dec.decode(hyperspec.msgpack.encode(4))

        with pytest.raises(ValidationError, match="Invalid enum value 'carrot'"):
            dec.decode(hyperspec.msgpack.encode("carrot"))
    ...
    def test_nested_literals(self):
        ...
        assert hasattr(both, "__hyperspec_cache__")
    ...
    def test_mix_int_and_int_literal(self):
        dec = hyperspec.msgpack.Decoder(Union[Literal[-1, 1], int])
        for x in [-1, 1, 10]:
            assert dec.decode(hyperspec.msgpack.encode(x)) == x

    def test_mix_str_and_str_literal(self):
        dec = hyperspec.msgpack.Decoder(Union[Literal["a", "b"], str])
        for x in ["a", "b", "c"]:
            assert dec.decode(hyperspec.msgpack.encode(x)) == x
...
class TestStructUnion:
    ...
    def test_struct_union_cached(self, array_like, proto):
        from hyperspec._core import _struct_lookup_cache as cache
...
class TestGenericStruct:
    ...
    def test_generic_struct_invalid_types_not_cached(self, proto):
        ...
        for typ in [Ex, Ex[int]]:
            ...
            assert not hasattr(typ, "__hyperspec_cache__")

    def test_hyperspec_cache_overwritten(self, proto):
        class Ex(Struct, Generic[T]):
            x: T

        typ = Ex[int]
        typ.__hyperspec_cache__ = 1
...
class TestGenericDataclassOrAttrs:
    ...
    def test_hyperspec_cache_overwritten(self, decorator, proto):
        @decorator
        class Ex(Generic[T]):
            x: T
...
class TestStructOmitDefaults:
    ...
    def test_omit_defaults_collections(self, proto, typ):
        ...
        class Test(Struct, omit_defaults=True):
            a: typ = hyperspec.field(default_factory=typ)
            b: typ = hyperspec.field(default=typ())
            c: typ = typ()
...
class TestStructDefaults:
    def test_struct_defaults(self, proto):
        class Test(Struct):
            a: int = 1
            b: list = []
            c: int = hyperspec.field(default=2)
            d: dict = hyperspec.field(default_factory=dict)
    ...
    def test_struct_default_factory_errors(self, proto):
        ...
        class Test(Struct):
            a: int = hyperspec.field(default_factory=bad)
...
class TestTypedDict:
    def test_type_cached(self, proto):
        ...
        dec = proto.Decoder(Ex)
        info = Ex.__hyperspec_cache__
        assert info is not None
        dec2 = proto.Decoder(Ex)
        assert Ex.__hyperspec_cache__ is info
    ...
    def test_subtype_error(self, proto):
        ...
        with pytest.raises(TypeError, match="may not contain more than one array-like"):
            proto.Decoder(Ex)
        assert not hasattr(Ex, "__hyperspec_cache__")
...
class TestNamedTuple:
    ...
    def test_decode_namedtuple_no_defaults(self, proto, use_typing):
        ...
        suffix = ", got 1" if proto is hyperspec.msgpack else ""
        with pytest.raises(ValidationError, match=f"length 3{suffix}"):
            dec.decode(proto.encode((1,)))

        suffix = ", got 6" if proto is hyperspec.msgpack else ""
        with pytest.raises(ValidationError, match=f"length 3{suffix}"):
...
class TestUUID:
    ...
    def test_decode_uuid_from_bytes(self):
        sol = uuid.uuid4()
        msg = hyperspec.msgpack.encode(sol.bytes)
        res = hyperspec.msgpack.decode(msg, type=uuid.UUID)
        assert res == sol

        bad_msg = hyperspec.msgpack.encode(b"x" * 8)
        with pytest.raises(hyperspec.ValidationError, match="Invalid UUID bytes"):
            hyperspec.msgpack.decode(bad_msg, type=uuid.UUID)
...
class TestNewType:
    ...
    def test_decode_annotated_newtype(self, proto):
        UserId = NewType("UserId", int)
        dec = proto.Decoder(Annotated[UserId, hyperspec.Meta(ge=0)])
    ...
    def test_decode_newtype_annotated(self, proto):
        UserId = NewType("UserId", Annotated[int, hyperspec.Meta(ge=0)])
    ...
    def test_decode_annotated_newtype_annotated(self, proto):
        UserId = Annotated[
            NewType("UserId", Annotated[int, hyperspec.Meta(ge=0)]),
            hyperspec.Meta(le=10),
        ]
        dec = proto.Decoder(UserId)
        assert dec.decode(proto.encode(1)) == 1
...
class TestTypeAlias:
    ...
    def test_typealias_with_constraints(self, proto):
        ...
        type Key = Annotated[str, hyperspec.Meta(max_length=4)]
        """
        with temp_module(src) as mod:
    ...
    def test_typealias_parametrized_generic_too_many_parameters(self):
        with temp_module("type Pair[T] = tuple[T, T]") as mod:
            with pytest.raises(TypeError):
                hyperspec.json.Decoder(mod.Pair[int, int])
...
class TestOrder:
    ...
    def test_order_object(self, kind, n, proto):
        ...
        fields.reverse()
        if kind == "struct":
            cls = hyperspec.defstruct("Test", fields)
        elif kind == "dataclass":
    ...
    def test_order_sort_implementation(self, rand, n):
        ...
        sol = hyperspec.json.encode(dict(sorted(msg.items())))
        assert res == sol
...
class TestFinal:
    ...
    def test_decode_final_annotated(self, proto):
        dec = proto.Decoder(Final[Annotated[int, hyperspec.Meta(ge=0)]])
