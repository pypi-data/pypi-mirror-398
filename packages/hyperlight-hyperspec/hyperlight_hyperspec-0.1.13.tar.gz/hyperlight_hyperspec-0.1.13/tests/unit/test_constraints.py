...
class TestMetaObject:
    ...
    def test_repr_empty(self):
        assert repr(Meta()) == "hyperspec.Meta()"
        for field in FIELDS:
            c = Meta(**{field: None})
            assert repr(c) == "hyperspec.Meta()"
    ...
    def test_repr_one_field(self, field):
        c = Meta(**{field: FIELDS[field]})
        assert repr(c) == f"hyperspec.Meta({field}={FIELDS[field]!r})"

    def test_repr_multiple_fields(self):
        c = Meta(gt=0, lt=1)
        assert repr(c) == "hyperspec.Meta(gt=0, lt=1)"
...
class TestInvalidConstraintAnnotations:
    ...
    def test_invalid_length_constraint(self, name):
        with pytest.raises(
            ...
            hyperspec.json.Decoder(Annotated[int, Meta(**{name: 1})])
    ...
    def test_invalid_tz_constraint(self):
        with pytest.raises(
            ...
            hyperspec.json.Decoder(Annotated[int, Meta(tz=True)])
    ...
    def test_invalid_integer_bounds(self, name, val):
        with pytest.raises(ValueError) as rec:
            hyperspec.json.Decoder(Annotated[int, Meta(**{name: val})])
