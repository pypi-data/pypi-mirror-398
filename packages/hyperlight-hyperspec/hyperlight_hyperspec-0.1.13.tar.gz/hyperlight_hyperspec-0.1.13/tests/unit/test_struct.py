...
import hyperspec
from hyperspec import NODEFAULT, UNSET, Struct, defstruct, field
from hyperspec.structs import StructConfig
...
def test_struct_class_attributes():
    ...
    assert Struct.__module__ == "hyperspec"
...
def test_struct_subclass_forbidden_field_names():
    ...
    with pytest.raises(
        TypeError, match="Cannot have a struct field named '__hyperspec_cached_hash__'"
    ):

        class Test3(Struct):
            __hyperspec_cached_hash__: int
...
def test_struct_subclass_forbids_non_types():
    ...
    with pytest.raises(TypeError):
        ...
        class Test(hyperspec.Struct, Foo()):
            pass
...
def test_struct_errors_nicely_if_used_in_init_subclass():
    ...
    class Test(Struct):
        def __init_subclass__(cls):
            ...
            for proto in [hyperspec.json, hyperspec.msgpack]:
...
class TestHash:
    ...
    def test_cache_hash(self):
        ...
        assert "__hyperspec_cached_hash__" in Cached.__slots__
        obj = Cached(1, Inner())
        assert not hasattr(obj, "__hyperspec_cached_hash__")
        assert hash(obj) == hash(obj)
        assert obj.__hyperspec_cached_hash__ == hash(obj)
...
class TestSetAttr:
    ...
    def test_force_setattr(self):
        ...
        res = hyperspec.structs.force_setattr(obj, "x", 2)
        assert res is None
        assert obj.x == 2

        with pytest.raises(AttributeError):
            hyperspec.structs.force_setattr(obj, "oops", 3)

        with pytest.raises(TypeError):
            hyperspec.structs.force_setattr(1, "oops", 3)
...
def replace(request):
    if request.param == "structs.replace":
        return hyperspec.structs.replace
    else:
        return copy_replace
...
class TestReplace:
    def test_replace_not_a_struct(self):
        with pytest.raises(TypeError, match="`struct` must be a `hyperspec.Struct`"):
            hyperspec.structs.replace(1, x=2)
...
class TestAsDictAndAsTuple:
    def test_asdict(self):
        x = Point(1, 2)
        assert hyperspec.structs.asdict(x) == {"x": 1, "y": 2}

    def test_astuple(self):
        x = Point(1, 2)
        assert hyperspec.structs.astuple(x) == (1, 2)

    @pytest.mark.parametrize(
        "func", [hyperspec.structs.asdict, hyperspec.structs.astuple]
    )
...
class TestInspectFields:
    def test_fields_bad_arg(self):
        ...
        for val in [1, int, Bad, Bad[int]]:
            with pytest.raises(TypeError, match="struct type or instance"):
                hyperspec.structs.fields(val)

    def test_fields_struct_meta(self):
        class CustomMeta(hyperspec.StructMeta):
            pass
        ...
        assert hyperspec.structs.fields(Model) == ()
    ...
    def test_fields_no_fields(self):
        assert hyperspec.structs.fields(hyperspec.Struct) == ()
    ...
    def test_fields(self, instance):
        ...
        class Example(hyperspec.Struct):
            x: int
            y: int = 0
            z: int = hyperspec.field(default_factory=factory)

        arg = Example(1, 2, 3) if instance else Example
        fields = hyperspec.structs.fields(arg)
    ...
    def test_fields_keyword_only(self):
        ...
        sol = (
            hyperspec.structs.FieldInfo("a", "a", int),
            hyperspec.structs.FieldInfo("b", "b", int, default=1),
            hyperspec.structs.FieldInfo("c", "c", int),
            hyperspec.structs.FieldInfo("d", "d", int, default=2),
        )
        assert hyperspec.structs.fields(Example) == sol
    ...
    def test_fields_encode_name(self):
        ...
        sol = (
            hyperspec.structs.FieldInfo("field_one", "fieldOne", int),
            hyperspec.structs.FieldInfo("field_two", "fieldTwo", int),
        )
    ...
    def test_fields_generic(self):
        ...
        class Example(hyperspec.Struct, Generic[T]):
            x: T
            y: int

        sol = (
            hyperspec.structs.FieldInfo("x", "x", T),
            hyperspec.structs.FieldInfo("y", "y", int),
        )
        assert hyperspec.structs.fields(Example) == sol
        assert hyperspec.structs.fields(Example(1, 2)) == sol

        sol = (
            hyperspec.structs.FieldInfo("x", "x", str),
            hyperspec.structs.FieldInfo("y", "y", int),
        )
        assert hyperspec.structs.fields(Example[str])
...
class TestPostInit:
    ...
    def test_post_init_not_called_on_replace(self):
        ...
        x1 = Ex()
        assert count == 1
        x2 = hyperspec.structs.replace(x1)
        assert x1 == x2
        assert count == 1
