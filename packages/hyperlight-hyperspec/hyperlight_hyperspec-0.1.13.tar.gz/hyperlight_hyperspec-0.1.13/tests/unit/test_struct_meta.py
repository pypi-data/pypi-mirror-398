...
from hyperspec.structs import asdict, astuple, force_setattr, replace


def test_struct_meta_exists():
    """Test that StructMeta is properly exposed."""
    assert hasattr(hyperspec, "StructMeta")
...
def test_struct_meta_subclass_with_encoder():
    ...
    json_bytes = hyperspec.json.encode(obj)
    decoded = hyperspec.json.decode(json_bytes, type=EncoderStruct)
    ...
    json_bytes = hyperspec.json.encode(container)
    decoded = hyperspec.json.decode(json_bytes, type=Container)
...
def test_structmeta_abcmeta_mixed_behaves_like_abc():
    ...
    # hyperspec roundtrip still works
    encoded = hyperspec.json.encode(obj)
    decoded = hyperspec.json.decode(encoded, type=ConcreteIntStruct)
    assert decoded == obj
...
def test_structmeta_abcmeta_mixed_supports_register():
    ...
    # hyperspec usage should still be fine
    encoded = hyperspec.json.encode(other)
    decoded = hyperspec.json.decode(encoded, type=OtherStruct)
...
def test_plain_struct_not_treated_as_abc():
    ...
    # Normal hyperspec behaviour works
    encoded = hyperspec.json.encode(obj)
    decoded = hyperspec.json.decode(encoded, type=Plain)
    assert decoded == obj
...
def test_struct_meta_pattern_ref_leak():
    ...
    # see https://github.com/jcrist/hyperspec/pull/899 for details
    ...
    hyperspec.Meta(pattern=pattern_string)
