...
class TestInvalidJSONTypes:
        ...
        for length in [2, 3, 4]:
            for types in itertools.combinations(types, length):
                ...
                with pytest.raises(TypeError, match="not supported"):
                    hyperspec.json.Decoder(Union[types])
    ...
    def test_invalid_dict_key_type_errors_at_runtime(self):
        ...
        with pytest.raises(
            hyperspec.ValidationError, match="Expected `array`, got `str`"
        ):
            hyperspec.json.decode(b'{"x": 1}', type=Dict[Tuple[int, int], int])


class TestEncodeFunction:
    def test_encode(self):
        assert hyperspec.json.encode(1) == b"1"

    def test_encode_error(self):
        with pytest.raises(TypeError):
            hyperspec.json.encode(object())

    def test_encode_large_object(self):
        """Check that buffer resize works"""
        data = "x" * 4097
        assert hyperspec.json.encode(data) == f'"{data}"'.encode("utf-8")
    ...
    def test_encode_enc_hook(self):
        ...
        res = hyperspec.json.encode(unsupported, enc_hook=enc_hook)
        assert hyperspec.json.encode("hello") == res
...
class TestEncoderMisc:
    ...
    def test_encode_enc_hook_recursion_error(self):
        enc = hyperspec.json.Encoder(enc_hook=lambda x: x)
    ...
    def test_encode_into(self, buf_size):
        ...
        msg = {"key": "x" * 48}
        encoded = hyperspec.json.encode(msg)
    ...
    def test_encode_lines(self, n, iterable):
        ...
        enc = hyperspec.json.Encoder(enc_hook=str)
...
class TestDecodeFunction:
    def test_decode(self):
        assert hyperspec.json.decode(b"[1, 2, 3]") == [1, 2, 3]

    def test_decode_from_str(self):
        assert hyperspec.json.decode("[1, 2, 3]") == [1, 2, 3]

        with pytest.raises(hyperspec.DecodeError, match="truncated"):
            assert hyperspec.json.decode("[1, 2, 3")

    def test_decode_type_keyword(self):
        assert hyperspec.json.decode(b"[1, 2, 3]", type=Set[int]) == {1, 2, 3}

        with pytest.raises(hyperspec.ValidationError):
            assert hyperspec.json.decode(b"[1, 2, 3]", type=Set[str])

    def test_decode_type_any(self):
        assert hyperspec.json.decode(b"[1, 2, 3]", type=Any) == [1, 2, 3]
    ...
    def test_decode_type_struct(self, array_like):
        ...
        msg = hyperspec.json.encode(Point(1, 2))

        for _ in range(2):
            assert hyperspec.json.decode(msg, type=Point) == Point(1, 2)
    ...
    def test_decode_parse_arguments_errors(self):
        ...
        with pytest.raises(TypeError, match="Extra positional arguments"):
            hyperspec.json.decode(buf, List[int])

        with pytest.raises(TypeError, match="Extra positional arguments"):
            hyperspec.json.decode(buf, 2, 3)

        with pytest.raises(TypeError, match="Extra keyword arguments"):
            hyperspec.json.decode(buf, bad=1)

        with pytest.raises(TypeError, match="Extra keyword arguments"):
            hyperspec.json.decode(buf, type=List[int], extra=1)

    def test_decode_with_trailing_characters_errors(self):
        with pytest.raises(hyperspec.DecodeError):
            hyperspec.json.decode(b'[1, 2, 3]"trailing"')
...
class TestDecoderMisc:
    ...
    def test_decoder_repr(self):
        ...
        dec = hyperspec.json.Decoder(typ)
        assert repr(dec) == f"hyperspec.json.Decoder({typ!r})"

        dec = hyperspec.json.Decoder()
        assert repr(dec) == f"hyperspec.json.Decoder({Any!r})"
    ...
    def test_decode_lines_typed(self):
        class Ex(hyperspec.Struct):
            x: int

        sol = [Ex(1), Ex(2)]
        buf = hyperspec.json.Encoder().encode_lines(sol)
        res = hyperspec.json.Decoder(Ex).decode_lines(buf)
        assert res == sol
    ...
    def test_decode_lines_malformed(self):
        buf = b'{"x": 1}\n{"x": efg'
        dec = hyperspec.json.Decoder()
        with pytest.raises(hyperspec.DecodeError, match="malformed"):
            dec.decode_lines(buf)
    ...
    def test_decoder_init_float_hook(self):
        ...
        dec = hyperspec.json.Decoder(float_hook=None)
        assert dec.float_hook is None
        ...
        with pytest.raises(TypeError):
            dec = hyperspec.json.Decoder(float_hook=1)


class TestBoolAndNone:
    def test_encode_none(self):
        assert hyperspec.json.encode(None) == b"null"

    def test_decode_none(self):
        assert hyperspec.json.decode(b"null") is None
        assert hyperspec.json.decode(b"   null   ") is None

    @pytest.mark.parametrize("s", [b"nul", b"nulll", b"nuul", b"nulp"])
    def test_decode_none_malformed(self, s):
        with pytest.raises(hyperspec.DecodeError):
            hyperspec.json.decode(s)

    def test_decode_none_typed(self):
        with pytest.raises(
            hyperspec.ValidationError, match="Expected `int | null`, got `str`"
        ):
            hyperspec.json.decode(b'"test"', type=Union[int, None])

    def test_encode_true(self):
        assert hyperspec.json.encode(True) == b"true"

    def test_decode_true(self):
        assert hyperspec.json.decode(b"true") is True
        assert hyperspec.json.decode(b"   true   ") is True
    ...
    def test_encode_false(self):
        assert hyperspec.json.encode(False) == b"false"

    def test_decode_false(self):
        assert hyperspec.json.decode(b"false") is False
        assert hyperspec.json.decode(b"   false   ") is False
    ...
    def test_decode_bool_typed(self):
        with pytest.raises(
            hyperspec.ValidationError, match="Expected `bool`, got `str`"
        ):
            hyperspec.json.decode(b'"test"', type=bool)
...
class TestStrings:
    ...
    def test_encode_str(self, decoded, encoded):
        assert hyperspec.json.encode(decoded) == encoded
    ...
    def test_encode_str_unroll_escapes(self, length, esc1, esc2, adjacent):
        ...
        for s in gen():
            ...
            res = hyperspec.json.encode(s)
            assert res == sol

    @pytest.mark.parametrize("decoded, encoded", STRINGS)
    def test_decode_str(self, decoded, encoded):
        assert hyperspec.json.decode(encoded) == decoded
    ...
    def test_decode_str_malformed_escapes(self, s, error):
        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s)

    def test_decode_str_invalid_byte(self):
        with pytest.raises(hyperspec.DecodeError, match="invalid character"):
            hyperspec.json.decode(b'"123 \x00 456"')

        with pytest.raises(hyperspec.DecodeError, match="invalid character"):
            hyperspec.json.decode(b'"123 \x01 456"')

    def test_decode_str_missing_closing_quote(self):
        with pytest.raises(hyperspec.DecodeError, match="truncated"):
            hyperspec.json.decode(b'"test')
    ...
    def test_decode_str_lengths(self, length, in_list, unicode, escape):
        ...
        buf = hyperspec.json.encode(sol)
        res = hyperspec.json.decode(buf)
        assert res == sol

        left, _, right = buf.rpartition(b'"')
        buf2 = left + b'\x01"' + right
        with pytest.raises(hyperspec.DecodeError, match="invalid character"):
            hyperspec.json.decode(buf2)

        # Test str skipping
        class Test(hyperspec.Struct):
            x: int

        buf3 = hyperspec.json.encode({"y": sol, "x": 1})
        hyperspec.json.decode(buf3, type=Test)
...
class TestBinary:
    ...
    def test_decode_binary(self, x, type):
        ...
        res = hyperspec.json.decode(s, type=type)
        assert res == bytes(x)
    ...
    def test_roundtrip_random(self, n, rand):
        for _ in range(10):
            x = rand.bytes(n)
            s = hyperspec.json.encode(x)
            x2 = hyperspec.json.decode(s, type=bytes)
            assert x == x2
    ...
    def test_malformed_base64_encoding(self, s):
        with pytest.raises(
            hyperspec.ValidationError, match="Invalid base64 encoded string"
        ):
            hyperspec.json.decode(s, type=bytes)
...
class TestDatetime:
    ...
    def test_encode_datetime_zoneinfo(self):
        ...
        sol = hyperspec.json.encode(x.isoformat())
        res = hyperspec.json.encode(x)
        assert res == sol
    ...
    def test_decode_datetime_utc(self, dt, suffix):
        ...
        s = f'"{dt}"'.encode("utf-8")
        res = hyperspec.json.decode(s, type=datetime.datetime)
        assert res == exp
    ...
    def test_decode_datetime_with_timezone(self, dt, sign, hour, minute):
        ...
        res = hyperspec.json.decode(json_s, type=datetime.datetime)
        assert res == exp
    ...
    def test_decode_timezone_cache(self):
        ...
        tz = hyperspec.json.decode(msg, type=datetime.datetime).tzinfo
        tz2 = hyperspec.json.decode(msg, type=datetime.datetime).tzinfo
        assert tz is tz2
        del tz2
    ...
    def test_decode_datetime_naive(self, s):
        ...
        res = hyperspec.json.decode(msg, type=datetime.datetime)
        assert sol == res
    ...
    def test_decode_min_datetime(self):
        res = hyperspec.json.decode(b'"0001-01-01T00:00:00Z"', type=datetime.datetime)
        exp = datetime.datetime.min.replace(tzinfo=UTC)
        assert res == exp

    def test_decode_max_datetime(self):
        res = hyperspec.json.decode(
    ...
    def test_decode_datetime_rfc3339_relaxed(self, lax, strict):
        """hyperspec supports a few relaxations of the RFC3339 format."""
        sol = datetime.datetime.fromisoformat(strict)
        msg = hyperspec.json.encode(lax)
        res = hyperspec.json.decode(msg, type=datetime.datetime)
        assert res == sol
    ...
    def test_decode_datetime_malformed(self, s):
        with pytest.raises(hyperspec.ValidationError, match="Invalid RFC3339"):
            hyperspec.json.decode(s, type=datetime.datetime)
...
class TestIntegers:
    ...
    def test_encode_int(self, ndigits):
        ...
        x = int(s)
        assert hyperspec.json.encode(x) == s
        if 0 < ndigits < 20:
            assert hyperspec.json.encode(-x) == b"-" + s

    @pytest.mark.parametrize("x", [-(2**63 + 1), -(2**63), 2**64 - 1, 2**64])
    def test_encode_big_integers(self, x):
        assert hyperspec.json.encode(x) == str(x).encode()
    ...
    def test_decode_int(self, ndigits):
        ...
        x = int(s)
        assert hyperspec.json.decode(s) == x
        if 0 < ndigits < 20:
            assert hyperspec.json.decode(b"-" + s) == -x
    ...
    def test_decode_big_int(self, x, type):
        s = str(x).encode()
        x2 = hyperspec.json.decode(s, type=type)
        assert isinstance(x2, int)
        assert x2 == x
    ...
    def test_decode_big_int_max_length(self, max_length):
        ...
        try:
            with pytest.raises(
                hyperspec.ValidationError, match="Integer value out of range"
            ):
                hyperspec.json.decode(s, type=int)
        finally:
            if cleanup:
                cleanup()
    ...
    def test_decode_int_whitespace(self, s):
        assert hyperspec.json.decode(s) == int(s)
    ...
    def test_decode_int_converts_to_float_if_requested(self):
        x = hyperspec.json.decode(b"123", type=float)
    ...
    def test_decode_int_type_error(self):
        with pytest.raises(
            hyperspec.ValidationError, match="Expected `str`, got `int`"
        ):
            hyperspec.json.decode(b"123", type=str)
...
class TestLiteral:
    ...
    def test_literal(self, values):
        ...
        dec = hyperspec.json.Decoder(literal)
        for val in values:
            assert dec.decode(hyperspec.json.encode(val)) == val

        for bad in ["bad", 1234]:
            with pytest.raises(hyperspec.ValidationError):
                dec.decode(hyperspec.json.encode(bad))

    def test_int_literal_errors(self):
        dec = hyperspec.json.Decoder(Literal[1, 2, 3])

        with pytest.raises(hyperspec.ValidationError, match="Invalid enum value 4"):
            dec.decode(b"4")

        with pytest.raises(
            hyperspec.ValidationError, match="Expected `int`, got `str`"
        ):
            dec.decode(b'"bad"')

    def test_str_literal_errors(self):
        dec = hyperspec.json.Decoder(Literal["one", "two", "three"])
        ...
        with pytest.raises(hyperspec.ValidationError, match="Invalid enum value 'bad'"):
            dec.decode(b'"bad"')
...
class TestFloat:
    ...
    def test_decode_float_long_mantissa(self, s, i):
        if i > 0:
            ...
            s = b"0." + b"0" * (-i) + s
        x = hyperspec.json.decode(s)
        x2 = hyperspec.json.decode(s, type=float)
        assert float(s) == x == x2
        with pytest.raises(
            hyperspec.ValidationError, match="Expected `int`, got `float`"
        ):
            hyperspec.json.decode(s, type=int)
    ...
    def test_decode_float_boundaries_errors(self, s):
        with pytest.raises(hyperspec.ValidationError, match="Number out of range"):
            hyperspec.json.decode(s)
    ...
    def test_decode_float_cases_from_fastfloat(self, s):
        ...
        x = hyperspec.json.decode(s.encode(), type=float)
        assert x == float(s)
    ...
    def test_decode_long_float_truncated_but_exp_brings_back_in_bounds(self, s):
        ...
        x = hyperspec.json.decode(s.encode())
        assert x == float(s)
    ...
    def test_decode_float_out_of_bounds(self, s):
        with pytest.raises(hyperspec.ValidationError, match="out of range"):
            hyperspec.json.decode(s)
    ...
    def test_float_hook_typed(self):
        ...
        dec = hyperspec.json.Decoder(Ex, float_hook=MyFloat)
    ...
    def test_float_hook_error(self):
        ...
        dec = hyperspec.json.Decoder(Ex, float_hook=float_hook)
...
class TestDecimal:
    ...
    def test_decimal_to_number_keeps_precision(self):
        enc = hyperspec.json.Encoder(decimal_format="number")
    ...
    def test_decimal_from_number_keeps_precision(self, msg):
        res = hyperspec.json.decode(msg, type=Decimal)
        sol = Decimal(msg)
        assert res == sol
    ...
    def test_decimal_from_number_still_errors_on_invalid_number(self, msg):
        ...
        Decimal(msg)
        with pytest.raises(hyperspec.DecodeError) as rec:
            hyperspec.json.decode(msg)

        # not ValidationError, a subclass
        assert type(rec.value) is hyperspec.DecodeError
    ...
    def test_decimal_from_number_priority(self):
        ...
        for msg, request_type, out_type in cases:
            out = hyperspec.json.decode(msg, type=request_type)
...
class TestSequences:
    ...
    def test_decode_typed_set(self):
        dec = hyperspec.json.Decoder(Set[int])
        assert dec.decode(b"[]") == set()
        assert dec.decode(b"[1]") == {1}
        assert dec.decode(b"[1,2]") == {1, 2}

    def test_decode_typed_frozenset(self):
        dec = hyperspec.json.Decoder(FrozenSet[int])
    ...
    def test_decode_typed_vartuple(self):
        dec = hyperspec.json.Decoder(Tuple[int, ...])
        assert dec.decode(b"[]") == ()
        assert dec.decode(b"[1]") == (1,)
    ...
    def test_decode_typed_list_wrong_element_type(self, type, bad_index):
        dec = hyperspec.json.Decoder(type)
        data = [1] * (bad_index + 1)
        data[bad_index] = "oops"
        msg = hyperspec.json.encode(data)
        err_msg = rf"Expected `int`, got `str` - at `\$\[{bad_index}\]`"
        with pytest.raises(hyperspec.ValidationError, match=err_msg):
            dec.decode(msg)
    ...
    def test_decode_sequence_malformed(self, s, error, type):
        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s, type=type)

    def test_decode_fixtuple_any(self):
        dec = hyperspec.json.Decoder(Tuple[Any, Any, Any])
        x = (1, "two", False)
        ...
        with pytest.raises(
            ...
            dec.decode(b"1")
        with pytest.raises(
            hyperspec.ValidationError, match="Expected `array` of length 3"
        ):
            dec.decode(b'[1, "two"]')

    def test_decode_fixtuple_typed(self):
        dec = hyperspec.json.Decoder(Tuple[int, str, bool])
        x = (1, "two", False)
        res = dec.decode(b'[1, "two", false]')
        assert res == x
        with pytest.raises(hyperspec.ValidationError, match="Expected `bool`"):
...
class TestNamedTuple:
    ...
    def test_decode_namedtuple_ignores_whitespace(self, s, x):
        ...
        x2 = hyperspec.json.decode(s, type=Test)
        assert x2 == Test(*x)
    ...
    def test_decode_namedtuple_malformed(self, s, error):
        ...
        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s, type=Test)
...
class TestDict:
    def test_encode_dict_raises_non_string_or_numeric_keys(self):
        with pytest.raises(
            ...
            hyperspec.json.encode({"a": 1, (1, 2): "bad"})
    ...
    def test_decode_any_dict(self):
        x = hyperspec.json.decode(b'{"a": 1, "b": "two", "c": false}')
    ...
    def test_decode_dict_wrong_element_type(self):
        dec = hyperspec.json.Decoder(Dict[str, int])
        with pytest.raises(
    ...
    def test_decode_dict_literal_key(self):
        dec = hyperspec.json.Decoder(Dict[Literal["a", "b"], int])
        assert dec.decode(b'{"a": 1, "b": 2}') == {"a": 1, "b": 2}

        with pytest.raises(hyperspec.ValidationError, match="Invalid enum value 'c'"):
            dec.decode(b'{"a": 1, "c": 2}')

    def test_decode_dict_enum_key(self):
        dec = hyperspec.json.Decoder(Dict[FruitStr, int])
        ...
        with pytest.raises(
            hyperspec.ValidationError, match="Invalid enum value 'carrot'"
        ):
            dec.decode(b'{"apple": 1, "carrot": 2}')

    def test_decode_dict_str_key_constraints(self):
        dec = hyperspec.json.Decoder(
            Dict[Annotated[str, hyperspec.Meta(min_length=3)], int]
        )
        assert dec.decode(b'{"abc": 1, "def": 2}') == {"abc": 1, "def": 2}

        with pytest.raises(
            hyperspec.ValidationError, match="Expected `str` of length >= 3"
        ):
            dec.decode(b'{"a": 1}')
    ...
    def test_decode_dict_string_cache(self, length):
        ...
        res = hyperspec.json.decode(hyperspec.json.encode(msg))
        assert msg == res
    ...
    def test_roundtrip_dict_key_types(self, key):
        msg = {key: 100}
        sol = hyperspec.json.encode(hyperspec.to_builtins(msg, str_keys=True))
        res = hyperspec.json.encode(msg)
        assert res == sol

        msg2 = hyperspec.json.decode(sol, type=Dict[type(key), int])
        assert msg == msg2

    def test_decode_dict_int_enum_key(self):
        dec = hyperspec.json.Decoder(Dict[FruitInt, int])
        assert dec.decode(b'{"-1": 10, "2": 20}') == {
            FruitInt.APPLE: 10,
            FruitInt.BANANA: 20,
        }

        with pytest.raises(hyperspec.ValidationError, match="Invalid enum value 3"):
            dec.decode(b'{"-1": 1, "3": 2}')

    @pytest.mark.parametrize("x", [-(2**63), 2**64 - 1])
    def test_encode_dict_int_key(self, x):
        msg = {-(2**63): "a", 0: "b", 2**64 - 1: "c"}
        s = hyperspec.json.encode(msg)
        assert s == b'{"-9223372036854775808":"a","0":"b","18446744073709551615":"c"}'

        for x in [-(2**63) - 1, 2**64]:
            s = hyperspec.json.encode({x: "a"})
            assert s == f'{{"{x}":"a"}}'.encode("utf-8")

    def test_decode_dict_int_key(self):
        msg = {-(2**63): "a", 0: "b", 2**64 - 1: "c"}
        buf = hyperspec.json.encode(msg)
        res = hyperspec.json.decode(buf, type=Dict[int, str])
        assert res == msg
    ...
    def test_decode_dict_int_key_malformed(self, s):
        ...
        with pytest.raises(
            ...
            hyperspec.json.decode(bad, type=Dict[int, int])

    @pytest.mark.parametrize("x", [-(2**63) - 1, 2**64, 2**65])
    def test_decode_dict_big_int(self, x):
        msg = {str(x): 1}
        buf = hyperspec.json.encode(msg)
        res = hyperspec.json.decode(buf, type=Dict[int, int])
        assert res == {x: 1}
        assert type(list(res)[0]) is int

    def test_decode_dict_int_key_constraints(self):
        dec = hyperspec.json.Decoder(Dict[Annotated[int, hyperspec.Meta(ge=3)], int])
        assert dec.decode(b'{"4": 1, "5": 2}') == {4: 1, 5: 2}

        with pytest.raises(hyperspec.ValidationError, match="Expected `int` >= 3"):
            dec.decode(b'{"2": 1}')

    def test_decode_dict_int_literal_key(self):
        dec = hyperspec.json.Decoder(Dict[Literal[-1, 2], int])
    ...
    def test_encode_dict_float_key(self):
        ...
        sol = hyperspec.json.encode({str(k): v for k, v in msg.items()})
        res = hyperspec.json.encode(msg)
        assert res == sol
    ...
    def test_decode_dict_float_key(self):
        ...
        res = hyperspec.json.decode(buf, type=Dict[float, int])
        assert res == sol

    def test_decode_dict_int_or_float_key(self):
        buf = b'{"1.5": "a", "123": "b"}'
        sol = {1.5: "a", 123: "b"}
        res = hyperspec.json.decode(buf, type=Dict[Union[int, float], str])
        assert res == sol
    ...
    def test_encode_dict_str_subclass_key(self):
        ...
        msg = hyperspec.json.encode({mystr("test"): 1})
        assert msg == b'{"test":1}'
    ...
    def test_encode_dict_custom_key(self):
        ...
        with pytest.raises(TypeError):
            hyperspec.json.encode({Custom("x"): 1}, enc_hook=enc_hook)

        with pytest.raises(RecursionError):
            hyperspec.json.encode({Custom("x"): 1}, enc_hook=lambda x: x)
    ...
    def test_decode_dict_custom_key(self):
        ...
        obj = hyperspec.json.decode(msg, type=Dict[Custom, int], dec_hook=dec_hook)
    ...
    def test_encode_dict_order_escape(self):
        ...
        res = hyperspec.json.encode(msg, order="deterministic")
...
class TestStruct:
    ...
    def test_encode_empty_struct(self, tag):
        ...
        s = hyperspec.json.encode(Test())
        if tag:
            expected = hyperspec.json.encode({"type": tag})
            assert s == expected
        else:
            assert s == b"{}"
    ...
    def test_encode_one_field_struct(self, tag):
        ...
        s = hyperspec.json.encode(Test(a=1))
        if tag:
            expected = hyperspec.json.encode({"type": tag, "a": 1})
            assert s == expected
        else:
            assert s == b'{"a":1}'
    ...
    def test_encode_two_field_struct(self, tag):
        ...
        s = hyperspec.json.encode(Test(a=1, b="two"))
        if tag:
            expected = hyperspec.json.encode({"type": tag, "a": 1, "b": "two"})
            assert s == expected
        else:
            assert s == b'{"a":1,"b":"two"}'

    def test_decode_struct(self):
        dec = hyperspec.json.Decoder(Person)
    ...
    def test_decode_struct_missing_fields(self):
        ...
        with pytest.raises(
            ...
            hyperspec.json.decode(bad, type=Person)
        ...
        with pytest.raises(
            ...
            hyperspec.json.decode(bad, type=List[Person])
    ...
    def test_decode_struct_malformed(self, s, error):
        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s, type=Person)
    ...
    def test_struct_gc_maybe_untracked_on_decode(self, array_like):
        ...
        enc = hyperspec.json.Encoder()
        dec = hyperspec.json.Decoder(List[Test])
    ...
    def test_struct_gc_false_always_untracked_on_decode(self, array_like):
        ...
        for obj in dec.decode(hyperspec.json.encode(ts)):
            assert not gc.is_tracked(obj)

    def test_struct_recursive_definition(self):
        enc = hyperspec.json.Encoder()
        dec = hyperspec.json.Decoder(Node)
    ...
    def test_decode_tagged_struct(self, tag):
        ...
        for msg in [
            ...
            res = dec.decode(hyperspec.json.encode(msg))
            assert res == Test(1, 2)

        # Tag incorrect type
        for bad in [False, 123.456]:
            with pytest.raises(hyperspec.ValidationError) as rec:
                dec.decode(hyperspec.json.encode({"type": bad}))
    ...
    def test_decode_tagged_empty_struct(self, tag):
        ...
        # Tag missing
        res = dec.decode(hyperspec.json.encode({}))
        assert res == Test()

        # Tag present
        res = dec.decode(hyperspec.json.encode({"type": tag}))
        assert res == Test()
    ...
    def test_decode_struct_tag_malformed(self, s, error):
        class Test1(hyperspec.Struct, tag=True):
            a: int
            b: int

        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s, type=Test1)
    ...
    def test_decode_tagged_struct_int_tag(self, ndigits, negative):
        ...
        t = Test(1)
        msg = hyperspec.json.encode(t)
        assert hyperspec.json.decode(msg, type=Test) == t
    ...
    def test_decode_tagged_struct_int_tag_uint64_always_invalid(self):
        ...
        with pytest.raises(hyperspec.ValidationError) as rec:
            hyperspec.json.decode(hyperspec.json.encode({"type": 2**64 - 1}), type=Test)
    ...
    def test_decode_struct_int_tag_malformed(self, s, error):
        class Test1(hyperspec.Struct, tag=123):
            a: int
            b: int
...
class TestStructUnion:
    ...
    def test_decode_struct_union_malformed(self, s, error):
        ...
        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s, type=Union[Test1, Test2])
    ...
    def test_decode_struct_union_int_tag_malformed(self, s, error):
        ...
        class Test2(hyperspec.Struct, tag=123):
            pass
    ...
    def test_decode_struct_union_ignores_whitespace(self, s):
        ...
        res = hyperspec.json.decode(s, type=Union[Test1, Test2])
...
class TestStructArray:
    ...
    def test_struct_array_like(self):
        dec = hyperspec.json.Decoder(PersonArray)

        x = PersonArray(first="harry", last="potter", age=13)
        a = hyperspec.json.encode(x)
        assert hyperspec.json.encode(("harry", "potter", 13, False)) == a
        ...
        # Wrong field type
        bad = hyperspec.json.encode(("harry", "potter", "thirteen"))
        with pytest.raises(
        ...
        # Missing fields
        bad = hyperspec.json.encode(("harry", "potter"))
        with pytest.raises(
            hyperspec.ValidationError,
        ...
        dec2 = hyperspec.json.Decoder(List[PersonArray])
        msg = hyperspec.json.encode(
            [
        ...
        # Defaults applied
        res = dec.decode(hyperspec.json.encode(("harry", "potter", 13)))
        assert res == PersonArray("harry", "potter", 13)
        assert res.prefect is False

    def test_struct_map_and_array_like_messages_cant_mix(self):
        array_msg = hyperspec.json.encode(("harry", "potter", 13))
        map_msg = hyperspec.json.encode({"first": "harry", "last": "potter", "age": 13})
        sol = Person("harry", "potter", 13)
        array_sol = PersonArray("harry", "potter", 13)

        dec = hyperspec.json.Decoder(Person)
        array_dec = hyperspec.json.Decoder(PersonArray)
    ...
    def test_decode_struct_array_like_malformed(self, s, error):
        class Point(hyperspec.Struct, array_like=True):
            x: int
            y: int
            z: int

        with pytest.raises(hyperspec.DecodeError, match=error):
            hyperspec.json.decode(s, type=Point)
    ...
    def test_decode_tagged_struct(self, tag):
        ...
        # Decode with tag
        res = dec.decode(hyperspec.json.encode([tag, 1, 2]))
        assert res == Test(1, 2)
        res = dec.decode(hyperspec.json.encode([tag, 1, 2, 3]))
        assert res == Test(1, 2, 3)

        # Trailing fields ignored
        res = dec.decode(hyperspec.json.encode([tag, 1, 2, 3, 4]))
        assert res == Test(1, 2, 3)

        # Missing required field errors
        with pytest.raises(hyperspec.ValidationError) as rec:
            dec.decode(hyperspec.json.encode([tag, 1]))
        assert "Expected `array` of at least length 3, got 2" in str(rec.value)

        # Tag missing
        with pytest.raises(hyperspec.ValidationError) as rec:
            dec.decode(hyperspec.json.encode([]))
        assert "Expected `array` of at least length 3, got 0" in str(rec.value)

        # Tag incorrect type
        with pytest.raises(hyperspec.ValidationError) as rec:
            dec.decode(hyperspec.json.encode([123.456, 2, 3]))
        assert f"Expected `{type(tag).__name__}`" in str(rec.value)
        ...
        with pytest.raises(hyperspec.ValidationError) as rec:
            dec.decode(hyperspec.json.encode([bad, 1, 2]))
        assert f"Invalid value {bad!r}" in str(rec.value)
        assert "`$[0]`" in str(rec.value)

        # Field incorrect type correct index
        with pytest.raises(hyperspec.ValidationError) as rec:
            dec.decode(hyperspec.json.encode([tag, "a", 2]))
        assert "Expected `int`, got `str`" in str(rec.value)
...
class TestStructArrayUnion:
    ...
    def test_decode_struct_array_like_union_malformed(self, s, error):
        class Test1(hyperspec.Struct, tag=True, array_like=True):
            x: int
            y: int
            z: int

        class Test2(hyperspec.Struct, tag=True, array_like=True):
            pass
    ...
    def test_decode_struct_array_like_union_int_tag_malformed(self, s, error):
        class Test1(hyperspec.Struct, tag=123, array_like=True):
            x: int
            y: int
            z: int

        class Test2(hyperspec.Struct, tag=-123, array_like=True):
            pass
...
class TestRaw:
    def test_encode_raw(self):
        b = b'{"x":1}'
        r = hyperspec.Raw(b)
        assert hyperspec.json.encode(r) == b
        assert hyperspec.json.encode({"y": r}) == b'{"y":{"x":1}}'
    ...
    def test_decode_raw_field(self):
        ...
        s = b'{"x": 1, "y": [1, 2, 3]  }'
        res = hyperspec.json.decode(s, type=Test)
        assert res.x == 1
    ...
    def test_decode_raw_is_view(self):
        s = b'   {"x": 1}   '
        r = hyperspec.json.decode(s, type=hyperspec.Raw)
    ...
    def test_decode_raw_from_str(self, wrap):
        ...
        if wrap:
            [r] = hyperspec.json.decode(msg, type=List[hyperspec.Raw])
        else:
            r = hyperspec.json.decode(msg, type=hyperspec.Raw)
        assert bytes(r) == b'{"x": 1}'
    ...
    def test_raw_in_union_works_but_doesnt_change_anything(self):
        ...
        r = hyperspec.json.decode(b'{"x": 1}', type=Test)
        assert r == Test(1)
    ...
    def test_raw_can_be_mixed_with_custom_type(self):
        ...
        res = hyperspec.json.decode(b'{"x": [1, 2]}', type=Test, dec_hook=dec_hook)
...
class TestFormat:
    ...
    def test_format(self, msg, indent):
        ...
        res = hyperspec.json.format(msg, indent=indent)

        assert res == sol

    def test_format_str(self):
        assert hyperspec.json.format("[1,     2]", indent=0) == "[1, 2]"

    def test_format_bad_calls(self):
        with pytest.raises(TypeError):
            hyperspec.json.format(1)

        with pytest.raises(TypeError):
            hyperspec.json.format(b"[]", indent=None)

        with pytest.raises(TypeError):
            hyperspec.json.format(b"[]", 2)

        with pytest.raises(TypeError):
            hyperspec.json.format(b"[]", bad=2)
    ...
    def test_format_malformed(self, msg, err):
        with pytest.raises(hyperspec.DecodeError, match=err):
            hyperspec.json.format(msg)
