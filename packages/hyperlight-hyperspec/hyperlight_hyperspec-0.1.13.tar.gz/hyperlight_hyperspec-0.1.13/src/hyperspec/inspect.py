...
import hyperspec
from hyperspec import NODEFAULT, UNSET, UnsetType as _UnsetType
...
class ExtType(Type):
    """A type corresponding to `hyperspec.msgpack.Ext`."""


class RawType(Type):
    """A type corresponding to `hyperspec.Raw`."""
...
class Field(hyperspec.Struct):
    ...
    type: Type
    required: bool = True
    default: Any = hyperspec.field(default_factory=lambda: NODEFAULT)
    default_factory: Any = hyperspec.field(default_factory=lambda: NODEFAULT)
...
class StructType(Type):
    """A type corresponding to a `hyperspec.Struct` type.
    ...
    cls: typing_Type[hyperspec.Struct]
    fields: Tuple[Field, ...]
...
def multi_type_info(types: Iterable[Any]) -> tuple[Type, ...]:
    """Get information about multiple hyperspec-compatible types.
    ...
    Examples
    --------
    >>> hyperspec.inspect.multi_type_info([int, float, list[str]])  # doctest: +NORMALIZE_WHITESPACE
...
def type_info(type: Any) -> Type:
    """Get information about a hyperspec-compatible type.
    ...
    >>> hyperspec.inspect.type_info(list[int])  # doctest: +NORMALIZE_WHITESPACE
    ...
    while True:
        ...
        if origin is not None:
            if type(t) is _AnnotatedAlias:
                metadata.extend(m for m in t.__metadata__ if type(m) is hyperspec.Meta)
                t = origin
            elif origin == Final:
                t = t.__args__[0]
...
class _Translator:
    ...
    def _translate_inner(
        ...
        if t is Any:
            ...
            return DecimalType()
        elif t is hyperspec.Raw:
            return RawType()
        elif t is hyperspec.msgpack.Ext:
            return ExtType()
        elif t is list:
            return ListType(
