...
def _import_tomllib():
    ...
    try:
        ...
        raise ImportError(
            "`hyperspec.toml.decode` requires `tomli` be installed.\n\n"
...
def _import_tomli_w():
    try:
        ...
        raise ImportError(
            "`hyperspec.toml.encode` requires `tomli_w` be installed.\n\n"
            "Please either `pip` or `conda` install it as follows:\n\n"
...
def encode(
    ...
    *,
    ...
    enc_hook : callable, optional
        A callable to call for objects that aren't supported hyperspec types.
