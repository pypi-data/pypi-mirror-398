...
class ProducerWorker:
    ...
    def __init__(
        ...
        self.output_queues = output_queues  # Sharded mp.Queues from pipeline (one per postproc worker)
        self._current_queue_index = 0  # For round-robin polling
    ...
    async def _get_task_from_queue(self) -> Optional[Dict[str, Any]]:
        """Get task from sharded output queues using round-robin.

        Polls from multiple multiprocessing.Queues (one per postproc worker)
        to eliminate lock contention. Uses non-blocking gets with round-robin
        to ensure fair consumption from all queues.

        Returns:
            Task data or None if no tasks available from any queue
        """
        loop = asyncio.get_running_loop()
        num_queues = len(self.output_queues)

        # First pass: try non-blocking get from each queue (round-robin starting point)
        for _ in range(num_queues):
            try:
                q = self.output_queues[self._current_queue_index]
                # Advance index for next call (round-robin)
                self._current_queue_index = (self._current_queue_index + 1) % num_queues

                # Non-blocking get via executor
                task_data = await loop.run_in_executor(
                    None,
                    lambda queue=q: queue.get_nowait()
                )
                return task_data

            except queue.Empty:
                # This queue is empty, try next
                continue
            except Exception:
                # Other error (e.g., queue closed), try next
                continue

        # All queues empty - do one blocking wait on first non-empty queue
        # This avoids busy-spinning when system is idle
        try:
            # Try blocking get with timeout on first queue (arbitrary choice)
            task_data = await asyncio.wait_for(
                loop.run_in_executor(
                    None,
                    lambda: self.output_queues[0].get(timeout=0.1)
                ),
                timeout=0.2
            )
            return task_data
