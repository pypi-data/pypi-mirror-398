...
function activateAnyFileViewerExt(app, _restorer) {
    var _a;
    // expose app to provider bridge
    __providerAppRef = app;
    ...
    // Register a content provider that forwards to Firefly and cancels the normal open flow
    const drive = (_a = app.serviceManager.contents) === null || _a === void 0 ? void 0 : _a.defaultDrive;
    ...
    // Create a non-default reader for FITS. Use our no-op model and associate
    // the content provider so that content fetch is intercepted before widget creation.
    const widgetFactory = new _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.ABCWidgetFactory({
        ...
        defaultFor: ['FITS'],
        readOnly: true,
        contentProviderId: 'firefly-forward'
    });
    // With content provider canceling the open during content fetch,
    // this factory's createNewWidget should seldom be called.
    // Provide a minimal invisible widget if invoked (e.g., via manual context use).
    widgetFactory.createNewWidget = (context) => {
        const content = new _lumino_widgets__WEBPACK_IMPORTED_MODULE_1__.Widget();
        const widget = new _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.DocumentWidget({ content, context, reveal: false });
...
/**
 * Content provider that forwards to Firefly and aborts the normal open flow early.
 */
class FireflyForwardContentProvider extends _jupyterlab_services__WEBPACK_IMPORTED_MODULE_2__.RestContentProvider {
    async get(localPath, options) {
        try {
            // Forward to Firefly immediately
            // app is not directly accessible here; use window.jupyterapp if available
            // but since we are in the same module closure, we capture via a static helper.
            await __forwardToFireflyFromProvider(localPath);
        }
        finally {
...
// Helper to bridge app into provider scope
let __providerAppRef = null;
async function __forwardToFireflyFromProvider(filepath) {
    if (__providerAppRef) {
        await forwardToFirefly(__providerAppRef, filepath);
    }
}
...
//# sourceMappingURL=lib_index_js.a5d5c643453713865ed5.js.map