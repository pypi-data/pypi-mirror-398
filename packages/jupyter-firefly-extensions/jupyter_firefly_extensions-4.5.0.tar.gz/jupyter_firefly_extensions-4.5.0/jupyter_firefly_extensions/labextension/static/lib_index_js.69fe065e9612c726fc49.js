...
/* harmony export */   ANYFACTORY: () => (/* binding */ ANYFACTORY),
/* harmony export */   activateAnyFileViewerExt: () => (/* binding */ activateAnyFileViewerExt)
/* harmony export */ });
...
const ANYFACTORY = 'Firefly Viewer';
 ...
 */
function activateAnyFileViewerExt(app, _restorer) {
    var _a, _b, _c, _d;
    ...
    const factory = new _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.ABCWidgetFactory({
        name: ANYFACTORY,
    ...
    // Always provide createNewWidget; even with content providers, JupyterLab will
    // call the widget factory to create a widget, so we abort immediately.
    factory.createNewWidget = (context) => {
        void forwardToFirefly(app, context.path);
        const err = new Error('Open canceled by Firefly extension');
        err.name = 'Canceled';
        throw err;
    };
    // Attempt JL >= 4.4 content provider registration to intercept content fetching early
    let providerRegistered = false;
    try {
        const jlVersion = ((_b = (_a = ((window === null || window === void 0 ? void 0 : window.lab) || window)) === null || _a === void 0 ? void 0 : _a.jupyterlab) === null || _b === void 0 ? void 0 : _b.version) || ((window === null || window === void 0 ? void 0 : window.PageConfig) && window.PageConfig.getOption && window.PageConfig.getOption('appVersion'));
        const [maj, min] = (jlVersion || '').split('.').map(n => Number(n));
        const isJL44Plus = !isNaN(maj) && !isNaN(min) && (maj > 4 || (maj === 4 && min >= 4));
        if (true) {
            // Declare the intent for this widget factory
            factory.contentProviderId = 'firefly-forward';
            // Register the provider on the default drive if the registry is available
            const drive = (((_c = app.serviceManager) === null || _c === void 0 ? void 0 : _c.contents) instanceof _jupyterlab_services__WEBPACK_IMPORTED_MODULE_1__.ContentsManager)
                ? app.serviceManager.contents.defaultDrive
                : (((_d = app.serviceManager) === null || _d === void 0 ? void 0 : _d.contents) && app.serviceManager.contents.defaultDrive);
            const registry = drive === null || drive === void 0 ? void 0 : drive.contentProviderRegistry;
            if (registry && typeof registry.register === 'function') {
                const provider = new FireflyForwardContentProvider({
                    apiEndpoint: '/api/contents',
                    serverSettings: app.serviceManager.serverSettings
                }, app);
                registry.register('firefly-forward', provider);
                providerRegistered = true;
                console.debug('Firefly: content provider registered (id=firefly-forward).');
            }
            else {
                console.debug('Firefly: content provider registry not available; falling back to cancel flow.');
            }
        }
    }
    catch (e) {
        console.debug('Firefly: content provider setup skipped due to error:', e);
    }
    // Suppress the expected 'Canceled' errors emitted during the intentional abort.
    // Tight scopes to avoid hiding real issues.
    if (!window.__fireflyCancelErrorHandlerInstalled) {
        const cancelHandler = (e) => {
            const err = e === null || e === void 0 ? void 0 : e.error;
            if (err && err.name === 'Canceled' && String(err.message || '').includes('Open canceled by Firefly extension')) {
                e.preventDefault();
                e.stopImmediatePropagation();
            }
        };
        window.addEventListener('error', cancelHandler, true);
        window.__fireflyCancelErrorHandlerInstalled = true;
    }
    if (!window.__fireflyCancelRejectionHandlerInstalled) {
        const rejectionHandler = (e) => {
            const reason = e === null || e === void 0 ? void 0 : e.reason;
            if (reason && reason.name === 'Canceled' && String(reason.message || '').includes('Open canceled by Firefly extension')) {
                e.preventDefault();
                e.stopImmediatePropagation();
            }
        };
        window.addEventListener('unhandledrejection', rejectionHandler, true);
        window.__fireflyCancelRejectionHandlerInstalled = true;
    }
 ...
 *
 * The widget is intentionally invisible and closes immediately.
 */
// Note: We abort the doc open by throwing 'Canceled' above; a dedicated
// forwarding widget is no longer required on JL 4.2.5 for this flow.
async function forwardToFirefly(app, filepath) {
    ...
    let cacheKey;
    try {
        const responseText = await (await (0,_handler__WEBPACK_IMPORTED_MODULE_3__.requestAPI)(`sendToFirefly?path=${encodeURIComponent(filepath)}`)).text();
        if (!(responseText === null || responseText === void 0 ? void 0 : responseText.startsWith('${'))) {
    ...
    try {
        const { firefly } = await (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_4__.findFirefly)();
 ...
 * This avoids base64/text decoding for large binary files.
 */
class FireflyNullDocModel extends _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.DocumentModel {
    constructor(options) {
        super(options);
...
FireflyNullModelFactory.MODEL_NAME = 'firefly-null-model';
// ---------------------------------------------------------------------
// JL >= 4.4: Content provider to intercept content fetching and abort open
// ---------------------------------------------------------------------
class FireflyForwardContentProvider extends _jupyterlab_services__WEBPACK_IMPORTED_MODULE_1__.RestContentProvider {
    ...
    async get(localPath, options) {
        try {
            // Kick off Firefly open; do not await strictly to minimize latency, but
            // still handle errors gracefully.
            await forwardToFirefly(this._app, localPath);
        }
        catch (e) {
            console.error('Firefly: forwarding via content provider failed:', e);
        }
        // Abort the normal open flow; DocumentManager should treat a Canceled error as benign.
        const err = new Error('Open canceled by Firefly extension');
        err.name = 'Canceled';
        throw err;
    }
...
async function findFirefly() {
    ...
    try {
        ...
        if (!cachedLoc) {
            const response = await (0,_handler__WEBPACK_IMPORTED_MODULE_1__.requestAPI)(ffLocEndpoint, fetchOptions);
        ...
        if (!fireflyURL)
            throw new Error(`fireflyURL couldn't be retrieved from ${ffLocEndpoint} endpoint`);
...
/* harmony import */ var _FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./FireflyCommonUtils.js */ "./lib/FireflyCommonUtils.js");
...
class FitsViewerWidget extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_1__.Widget {
    ...
    constructor(context) {
        ...
        const useModel = ((_a = window === null || window === void 0 ? void 0 : window.firefly) === null || _a === void 0 ? void 0 : _a.jlExtUseModel) || false;
    ...
    async renderModel(context, useModelFirst) {
        ...
        if (this.loaded) {
            return (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__.findFirefly)().then(({ firefly }) => firefly.action.dispatchChangeActivePlotView(this.plotId));
        }
        try {
            const { firefly, fireflyURL } = await (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__.findFirefly)();
            ...
            if (div && !this.loaded) {
                div.innerHTML = (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__.buildURLErrorHtml)('Failed to load FITS file');
            }
        }
        catch (e) {
            const div = document.getElementById(this.filename);
            if (div)
                div.innerHTML = (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__.buildURLErrorHtml)(e);
        }
    }
    dispose() {
        // Attempt to remove Firefly image view; ignore errors if firefly not available.
        (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__.findFirefly)()
...
function tellLabToLoadFileToServer(path, firefly) {
    ...
    return (0,_handler__WEBPACK_IMPORTED_MODULE_4__.requestAPI)(`sendToFirefly?path=${encodeURIComponent(path)}`)
...
async function requestAPI(endPoint = '', init = {}) {
    ...
    const requestUrl = _jupyterlab_coreutils__WEBPACK_IMPORTED_MODULE_0__.URLExt.join(settings.baseUrl, 'jupyter-firefly-extensions', // API Namespace
    endPoint);
    let response;
    try {
...
/* harmony import */ var _AnyFileViewerExt_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./AnyFileViewerExt.js */ "./lib/AnyFileViewerExt.js");
...
const anyFileViewerExt = {
    ...
    autoStart: true,
    requires: [_jupyterlab_application__WEBPACK_IMPORTED_MODULE_0__.ILayoutRestorer],
    activate: _AnyFileViewerExt_js__WEBPACK_IMPORTED_MODULE_5__.activateAnyFileViewerExt
};
// More than one extension/plugin can be exported as a list
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ([showSlateExt, fitsViewerExt, anyFileViewerExt]);
...
//# sourceMappingURL=lib_index_js.69fe065e9612c726fc49.js.map