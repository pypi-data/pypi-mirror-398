...
function activateAnyFileViewerExt(app, _restorer) {
    ...
    // Ensure Firefly viewer can be the default for CSV/TSV by removing built-in viewers' factories
    // JupyterLab's CSV/TSV viewers typically register their own factories with defaultFor.
    // When multiple factories claim the same default, the registry often prefers the first-added.
    // Removing the built-ins allows our factory to take precedence cleanly.
    ['CSV Viewer', 'TSV Viewer', 'csv', 'tsv'].forEach((factoryName) => {
        try {
            app.docRegistry.removeWidgetFactory(factoryName);
        }
        catch (_e) {
            // ignore: the factory may not exist or removal may not be supported for this name
        }
    });
...
//# sourceMappingURL=lib_index_js.0b47c2eda31733515299.js.map