...
function activateAnyFileViewerExt(app, _restorer) {
    ...
    if (!providerRegistered) {
        // Fallback for JL < 4.4 or when registry is not available: cancel widget creation immediately
        factory.createNewWidget = (context) => {
            // Forward to Firefly and abort the normal doc open flow.
            void forwardToFirefly(app, context.path);
            const err = new Error('Open canceled by Firefly extension');
            // Many JL components treat name === 'Canceled' specially (do not show UI errors)
            err.name = 'Canceled';
            throw err;
        };
        // Suppress the expected 'Canceled' error in the console generated by aborting the open flow
        // Keep this filter tightly scoped to our specific cancellation to avoid hiding real errors.
...
//# sourceMappingURL=lib_index_js.96edf0049d840c8251cb.js.map