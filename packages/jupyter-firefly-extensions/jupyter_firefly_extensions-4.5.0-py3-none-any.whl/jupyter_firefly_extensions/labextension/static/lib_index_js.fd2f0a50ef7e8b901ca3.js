...
// Factory name shown in the "Open With" menu
 ...
 *
 * Pattern follows JupyterLab "Document extensions" guidance:
 * https://jupyterlab.readthedocs.io/en/stable/extension/documents.html
 * and the extension examples:
 * https://github.com/jupyterlab/extension-examples/tree/main/documents
 *
 * Instead of throwing in createNewWidget (as our first iteration did),
 * we now return a lightweight DocumentWidget that immediately forwards
 * the file to Firefly Viewer and then closes itself on success. The model
 * uses a custom factory with fileFormat=null so JupyterLab does not fetch
 * or decode the file contents client-side.
 *
...
function activateAnyFileViewerExt(app, _restorer) {
    ...
    // Create a reader for FITS that routes to Firefly's generic show_data flow.
    // Use the custom 'firefly-null-model' so the context does not attempt to read/parse file content.
    const widgetFactory = new _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.ABCWidgetFactory({
        name: ANYFACTORY,
    ...
    // Return a lightweight DocumentWidget that forwards the file to Firefly
    // and closes itself upon success. This avoids throwing and adding a global
    // window error handler.
    widgetFactory.createNewWidget = (context) => {
        const content = new ForwarderPlaceholder();
        const widget = new _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.DocumentWidget({ content, context });
        // Kick off forwarding in the background
        void (async () => {
            const ok = await forwardToFirefly(app, context.path);
            if (ok) {
                // Close the placeholder tab once Firefly took over
                widget.title.closable = true;
                widget.close();
            }
            else {
                content.setStatus('Failed to open in Firefly. See console for details.');
            }
        })();
        return widget;
    };
...
/**
 * Forward the given file path to Firefly Viewer.
...
async function forwardToFirefly(app, filepath) {
    ...
    let cacheKey;
    try {
        const responseText = await (await (0,_handler__WEBPACK_IMPORTED_MODULE_2__.requestAPI)(`sendToFirefly?path=${encodeURIComponent(filepath)}`)).text();
        // Server returns a cache key string we can pass through to Firefly JS API
    ...
    try {
        const { firefly } = await (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_3__.findFirefly)();
...
// ---------------------------------------------------------------------
// Minimal placeholder widget used while forwarding to Firefly
// ---------------------------------------------------------------------
class ForwarderPlaceholder extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_1__.Widget {
    constructor() {
        super();
        this.addClass('jp-FireflyForwarder');
        this.setStatus('Opening in Firefly Viewer...');
    }
    setStatus(text) {
...
//# sourceMappingURL=lib_index_js.fd2f0a50ef7e8b901ca3.js.map