...
// ---------------------------------------------------------------------
// Minimal file-type registration (only when missing)
// ---------------------------------------------------------------------
// We aim to be generic: support common text and binary scientific formats.
// Only register a type when JupyterLab doesn't already know it.
const SUPPORTED_TYPES = [
    ...
    },
    {
        ...
        contentType: 'file',
        fileFormat: null,
        mimeTypes: ['application/octet-stream', 'application/x-parquet'],
        extensions: ['.parquet']
    },
    {
        name: 'CSV',
...
function activateAnyFileViewerExt(app, _restorer) {
    // Register supported file types only if missing (keeps extension generic/minimal)
    SUPPORTED_TYPES.forEach((ft) => {
        if (!app.docRegistry.getFileType(ft.name)) {
    ...
    // Create a non-default reader that routes the file open event to Firefly's show_data flow.
    ...
    const widgetFactory = new _jupyterlab_docregistry__WEBPACK_IMPORTED_MODULE_0__.ABCWidgetFactory({
        ...
        fileTypes: SUPPORTED_TYPES.map((t) => t.name),
        defaultFor: SUPPORTED_TYPES.map((t) => t.name),
        readOnly: true
    });
    ...
    // forwardToFirefly() and throw a No Op error to cancel JL's default open.
...
async function forwardToFirefly(app, filepath) {
    ...
    try {
        ...
        // Accept any non-empty response as a cache key; server defines the format.
        if (!responseText) {
            throw new Error('Empty response from Firefly server while uploading file');
        }
...
//# sourceMappingURL=lib_index_js.8ae717408f2fe6816f4d.js.map