...
async function forwardToFirefly(app, filepath) {
    ...
    const progressDialog = new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Dialog({
        title: 'Firefly Viewer',
        body: `Uploading and opening "${filename}" in Firefly…`,
        // Provide at least one button; this prevents Dialog.launch() from rejecting.
    ...
    try {
        await app.commands.execute(_SlateCommandExt_js__WEBPACK_IMPORTED_MODULE_3__.SLATE_CMD_ID);
    }
    catch (e) {
        console.error('Could not activate Firefly viewer:', e);
        // Dispose the progress dialog before showing the error dialog to avoid stacking issues
        try {
        ...
        try {
            await (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.showDialog)({
                title: 'Failed to activate Firefly Viewer',
                body: `${(_a = e === null || e === void 0 ? void 0 : e.message) !== null && _a !== void 0 ? _a : e}`,
                buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Dialog.okButton()]
            });
        }
        catch (dialogErr) {
            console.error('Error showing activation failure dialog:', dialogErr);
        }
        return;
    }
    ...
    let cacheKey;
    try {
        const responseText = await (await (0,_handler__WEBPACK_IMPORTED_MODULE_4__.requestAPI)(`sendToFirefly?path=${encodeURIComponent(filepath)}`)).text();
        if (true) {
        ...
        // Dispose progress dialog first, then show error dialog
        try {
            progressDialog.dispose();
        }
        ...
        try {
            await (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.showDialog)({
                title: 'Upload to Firefly Failed',
                body: `${(_b = e === null || e === void 0 ? void 0 : e.message) !== null && _b !== void 0 ? _b : e}`,
                buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Dialog.okButton()]
            });
        }
        catch (dialogErr) {
            console.error('Error showing upload failure dialog:', dialogErr);
        }
        return;
    }
    // 3) Tell Firefly JS API to display the uploaded file
    try {
        const { firefly } = await (0,_FireflyCommonUtils_js__WEBPACK_IMPORTED_MODULE_5__.findFirefly)();
        ...
        try {
            await (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.showDialog)({
                title: 'Open in Firefly Failed',
                body: `${(_c = e === null || e === void 0 ? void 0 : e.message) !== null && _c !== void 0 ? _c : e}`,
                buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_1__.Dialog.okButton()]
            });
        }
        catch (dialogErr) {
            console.error('Error showing open failure dialog:', dialogErr);
        }
        return;
    }
...
//# sourceMappingURL=lib_index_js.fc7f116a07422d124166.js.map