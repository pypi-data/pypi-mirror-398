...
class RenderedStatement:
    ...
    def _get_period_type(self, period: PeriodData) -> str:
        """Determine the period type based on duration."""
        if not period.is_duration or not period.start_date or not period.end_date:
            return "instant"
        ...
        if 80 <= duration_days <= 100:
            ...
            return "ytd"  # 6-month YTD
        elif 265 <= duration_days <= 285:
            return "ytd"  # 9-month YTD
        elif 355 <= duration_days <= 375:
            return "annual"
        else:
            return "other"

    def _get_fiscal_label(self, period: PeriodData) -> str:
        """Generate a fiscal label like 'Q3 2025' or '9M 2025'."""
        if not period.is_duration or not period.start_date or not period.end_date:
            return ""
        ...
        if 80 <= duration_days <= 100:
            return f"Q{((end_date.month - 1) // 3) + 1} {year}"
        elif 175 <= duration_days <= 190:
            return f"6M {year}"
        elif 265 <= duration_days <= 285:
            return f"9M {year}"
    ...
    @property
    def available_period_types(self) -> List[str]:
        """Get list of period types available in this statement."""
        period_types = set()
        for period in self.header.periods:
            period_types.add(self._get_period_type(period))
        return sorted(list(period_types))

    def to_dataframe(self, period_type: Optional[str] = None) -> Any:
        """Convert to a pandas DataFrame in long/tidy format.

        Each row represents one fact for one period, making it easy to filter
        and analyze data programmatically.

        Args:
            period_type: Filter to specific period type. Options:
                - None: Include all periods (default)
                - 'quarterly': Only 3-month periods
                - 'ytd': Only year-to-date periods (6-month, 9-month)
                - 'annual': Only 12-month periods

        Returns:
            DataFrame with columns:
                - concept: XBRL concept name
                - label: Human-readable label
                - period_type: 'quarterly', 'ytd', 'annual', or 'instant'
                - period_start: Start date (YYYY-MM-DD)
                - period_end: End date (YYYY-MM-DD)
                - fiscal_label: Label like 'Q3 2025' or '9M 2025'
                - value: The fact value
                - abstract: Whether this is an abstract/header row
                - dimension: Whether this is a dimensional breakdown
                - axis: Dimension axis if applicable
                - member: Dimension member if applicable
                - period: 'duration' or 'instant'
                - level: Hierarchy level for display
        """
        import logging
        ...
        try:
            ...
            # Filter periods based on period_type parameter
            periods_to_include = []
            for period in self.header.periods:
                ptype = self._get_period_type(period)
                if period_type is None or ptype == period_type:
                    periods_to_include.append((period, ptype))

            # Warn if no periods match the filter
            if period_type and not periods_to_include:
                available = self.available_period_types
                logger.warning(f"No {period_type} data available. Available period types: {available}")

            # Create a mapping of period index to period data
            period_index_map = {i: p for i, p in enumerate(self.header.periods)}

            # First pass: collect tree levels for normalization
            tree_levels = [row.level for row in self.rows]

            # Normalize levels using stack-based approach
            normalized_levels = []
            stack = []
            for i, tree_level in enumerate(tree_levels):
                while stack and tree_levels[stack[-1]] >= tree_level:
                    stack.pop()
                normalized_levels.append(len(stack))
                stack.append(i)

            # Create long-format rows: one row per fact per period
            for row_idx, row in enumerate(self.rows):
                normalized_level = normalized_levels[row_idx]

                for cell_idx, cell in enumerate(row.cells):
                    if cell_idx >= len(self.header.periods):
                        continue

                    period = period_index_map[cell_idx]
                    ptype = self._get_period_type(period)

                    # Skip if period doesn't match filter
                    if period_type is not None and ptype != period_type:
                        continue

                    # Skip empty values for non-abstract rows
                    if cell.value is None and not row.is_abstract:
                        continue
                    ...
                    df_row = {
                        ...
                        'label': row.label,
                        'period_type': ptype,
                        'period_start': period.start_date if period.is_duration else None,
                        'period_end': period.end_date,
                        'fiscal_label': self._get_fiscal_label(period),
                        'value': cell.value,
                        'abstract': row.is_abstract,
                        'dimension': row.is_dimension,
                        'axis': row.metadata.get('axis', ''),
                        'member': row.metadata.get('member', ''),
                        'period': 'duration' if period.is_duration else 'instant',
    ...
    for period_key, original_label in periods_to_display:
                ...
                if len(parts) > 1:
                    if show_date_range:
                        ...
                        try:
                            ...
                            # Add period indicator based on duration
                            # YTD indicator takes priority for non-quarterly periods
                            if duration_days and statement_type in ['IncomeStatement', 'CashFlowStatement']:
                                if 175 <= duration_days <= 190:  # ~6 months
                                    final_label = f"{final_label} (YTD)"
                                elif 265 <= duration_days <= 285:  # ~9 months
                                    final_label = f"{final_label} (YTD)"
                                elif q_num and 85 <= duration_days <= 95:  # Only add q_num for quarterly periods
        ...
        if final_label and show_date_range and is_duration and start_date_obj and end_date_obj:
            ...
            if "-" not in final_label:
                ...
                # Add period indicator if not already added - YTD takes priority for non-quarterly
                if duration_days and statement_type in ['IncomeStatement', 'CashFlowStatement'] and "(YTD)" not in final_label and f"({q_num})" not in final_label:
                    if 175 <= duration_days <= 190:  # ~6 months
                        final_label = f"{final_label} (YTD)"
                    elif 265 <= duration_days <= 285:  # ~9 months
                        final_label = f"{final_label} (YTD)"
        ...
        if not final_label:
            if period_key.startswith('instant_'):
                ...
                if show_date_range and start_date_obj and end_date_obj:
                    ...
                    # Add period indicator - YTD takes priority for non-quarterly
                    if duration_days and statement_type in ['IncomeStatement', 'CashFlowStatement']:
                        if 175 <= duration_days <= 190:  # ~6 months
                            final_label = f"{final_label} (YTD)"
                        elif 265 <= duration_days <= 285:  # ~9 months
