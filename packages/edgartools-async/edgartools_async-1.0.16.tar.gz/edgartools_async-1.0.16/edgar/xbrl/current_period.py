...
class CurrentPeriodView:
    ...
    def _get_appropriate_period_for_statement(self, statement_type: str) -> str:
        ...
        is_annual_filing = bool(self.xbrl.entity_info.get('annual_report'))
        is_quarterly_filing = bool(self.xbrl.entity_info.get('quarterly_report'))
        ...
        if statement_type in instant_statements:
            ...
            if current_end_date:
                ...
                # Prefer quarterly periods for 10-Qs and annual periods for 10-Ks
                matching_periods = []
                        ...
                        if len(parts) >= 3:
                            try:
                                ...
                                if end_date == current_end_date:
                                    ...
                                    if is_annual_filing:
                                        # For 10-Ks prefer annual > quarterly > other
                                        priority = 0 if period_type == 'Annual' else (
                                            1 if period_type == 'Quarterly' else 2)
                                    else:
                                        # For 10-Qs prefer quarterly > annual > other (to avoid YTD)
                                        priority = 0 if period_type == 'Quarterly' else (
                                            1 if period_type == 'Annual' else 2)

                                    duration_days = period.get('days')
                                    matching_periods.append(
                                        (priority, duration_days if duration_days is not None else float('inf'),
                                         period_key, period.get('label', period_key)))
                            except (ValueError, IndexError):
                                continue

                if matching_periods:
                    # Sort by priority then by shortest duration (so quarterly beats YTD with same end date)
                    matching_periods.sort(key=lambda x: (x[0], x[1]))
                    selected_period = matching_periods[0][2]
    ...
    def _get_statement_dataframe(self, statement_type: str, raw_concepts: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            # Pull period boundaries so we can expose them with each fact
            period_info = next((p for p in self.xbrl.reporting_periods if p.get('key') == period_filter), {})
            period_start = period_info.get('start_date')
            period_end = period_info.get('end_date') or period_info.get('date')
            period_label = period_info.get('label', self.period_label)
