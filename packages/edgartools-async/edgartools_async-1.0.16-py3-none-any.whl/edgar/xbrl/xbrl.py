...
class XBRL:
    ...
    def from_filing(cls, filing) -> Optional['XBRL']:
        ...
        if xbrl_attachments.empty:
            ...
            return None
        return cls._from_xbrl_attachments(xbrl_attachments)

    @classmethod
    def from_attachments(cls, attachments: Attachments) -> Optional['XBRL']:
        """
        Create an XBRL object from pre-resolved Attachments.

        This avoids re-downloading the full submission when attachments are already available.
        """
        xbrl_attachments = XBRLAttachments(attachments)
        if xbrl_attachments.empty:
            log.warning("No XBRL attachments found in attachments")
            return None
        return cls._from_xbrl_attachments(xbrl_attachments)

    @classmethod
    def _from_xbrl_attachments(cls, xbrl_attachments: XBRLAttachments) -> 'XBRL':
        xbrl = cls()
