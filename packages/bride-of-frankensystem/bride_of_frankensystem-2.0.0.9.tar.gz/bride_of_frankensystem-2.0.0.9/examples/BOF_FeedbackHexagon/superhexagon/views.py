...
EXPORT = [
    ...
    },
    {
        'table': 'SHTrial',
        'fields': {
            'trials_wo_key_presses': 'count(shTrialID)'
        },
        'group_by': 'sessionNumber',
        'filter': 'keyPressCount = 0'
    },
    {
        'table': 'SHReplayEvent',
        'fields': {
...
@superhexagon.route("/game_retention")
@verify_correct_page
@verify_session_valid
def superhexagon_route_after_vid():
    return render_template("superhexagon.html")
...
def sh_get_settings_dict(participantID=None):
    ...
    settings = {
        'Sessions': [
            {'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'], 'InterSessionTime': current_app.config['BREAK_TIME'], 'FeedbackType': 'None'},
            {'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'], 'InterSessionTime': current_app.config['BREAK_TIME'], 'FeedbackType': feedback_map[condition]},
            {'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'], 'InterSessionTime': current_app.config['BREAK_TIME'], 'FeedbackType': feedback_map[condition]},
            #{'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'], 'InterSessionTime': current_app.config['BREAK_TIME'], 'FeedbackType': feedback_map[condition]},
            #{'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'], 'InterSessionTime': 3, 'FeedbackType': 'None'}
        ]
    }
...
def sh_get_settings(participantID=None):
    if 'retention' in request.referrer:
        return jsonify({
            'Sessions': [{'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'], 'InterSessionTime': current_app.config['BREAK_TIME'], 'FeedbackType': 'None'}]
        })
...
def sh_post_trial():
    ...
    trial = db.SHTrial()
    sessionNumber = int_or_0(request.form["sessionNumber"])

    trial.referrer = request.referrer.split('/')[-1]
    if request.referrer.endswith('game_retention'):
        sessionNumber += 3
    ...
    trial.keyPressCount = int_or_0(request.form["keyPressCount"])
