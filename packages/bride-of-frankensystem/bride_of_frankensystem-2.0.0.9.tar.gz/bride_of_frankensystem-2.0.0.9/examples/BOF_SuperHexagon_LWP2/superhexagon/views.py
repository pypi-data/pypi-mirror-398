...
EXPORT = [
    ...
    },
    {
        'table': 'SHTrial',
        'fields': {
            'play_time_filtered': 'sum(duration)',
            'total_trials_filtered': 'count(shTrialID)',
            'average_duration_filtered': 'avg(duration)',
        },
        'group_by': 'sessionNumber',
        'filter': 'interrupted = 0 AND keyPressCount > 0'
    },
    {
        'table': 'SHTrial',
        'fields': {
...
@superhexagon.route("/game_after_vid")
@verify_correct_page
@verify_session_valid
...
def sh_get_settings_dict(participantID=None):
    ...
    if condition == 1:
        ...
        settings = {
            'Sessions': [
                {'InterTrialTime': 3, 'SessionTime': current_app.config['SESSION_TIME'],
                 'InterSessionTime': current_app.config['BREAK_TIME'], 'FeedbackType': 'None'},
            ]
        }
...
def sh_post_trial():
    ...
    if request.referrer.endswith('game_after_vid'):
        sessionNumber += 1
...
def sh_post_replay():
    ...
    for event_line in full_log.split('\n'):
        ...
        replay_event.referrer = request.referrer.split('/')[-1]
        ...
        sessionNumber = int_or_0(columns[1])

        if request.referrer.endswith('game_after_vid'):
            sessionNumber += 1

        replay_event.eventType = columns[0]
        replay_event.sessionNumber = sessionNumber
