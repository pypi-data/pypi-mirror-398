...
def locally_equivalent(matrix1: np.ndarray, matrix2: np.ndarray) -> bool:
    """Check if two 4x4 unitary matrices are locally equivalent.

    Two 4x4 unitary matrices are locally equivalent if they can
    be transformed into each other by a sequence of single-qubit
    operations before and after the interaction.

    Args:
        matrix1: The first 4x4 unitary matrix to check.
        matrix2: The second 4x4 unitary matrix to check.

    Returns:
        True if the two matrices are locally equivalent, False otherwise.
    """
    res1 = kak_decomposition(matrix1)
    res2 = kak_decomposition(matrix2)
    return np.allclose(res1.interaction_coefficients,
                       res2.interaction_coefficients)


def transform_locally_equivalent(source, target):
    """
    Find a sequence of single-qubit operations that transforms the source
    matrix into the target matrix. The source and target matrices must be
    locally equivalent.

    Args:
        source: The source matrix to transform.
        target: The target matrix to transform to.

    Returns:
        A tuple of two lists of tuples, the first list is the single-qubit
        operations before the interaction, the second list is the single-qubit
        operations after the interaction.
    """
    from qlisp import Unitary2Angles

    source_kak = kak_decomposition(source)
    target_kak = kak_decomposition(target)
    if not np.allclose(source_kak.interaction_coefficients,
                       target_kak.interaction_coefficients):
        raise ValueError("Source and target are not locally equivalent")

    b0, b1 = target_kak.single_qubit_operations_before
    a0, a1 = target_kak.single_qubit_operations_after
    c0, c1 = source_kak.single_qubit_operations_before
    d0, d1 = source_kak.single_qubit_operations_after

    b0 = c0.T.conj() @ b0
    b1 = c1.T.conj() @ b1
    a0 = a0 @ d0.T.conj()
    a1 = a1 @ d1.T.conj()
    before = [(('u3', *Unitary2Angles(b0)[:3]), 0),
              (('u3', *Unitary2Angles(b1)[:3]), 1)]
    after = [(('u3', *Unitary2Angles(a0)[:3]), 0),
             (('u3', *Unitary2Angles(a1)[:3]), 1)]
    return before, after
