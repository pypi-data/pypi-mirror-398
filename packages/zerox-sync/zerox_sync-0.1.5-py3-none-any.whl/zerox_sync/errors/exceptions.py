...
class MissingEnvironmentVariables(CustomException):
    """Exception raised when the GOOGLE_API_KEY environment variable is missing."""
...
class ModelAccessError(CustomException):
    """Exception raised when the provided model can't be accessed due to incorrect API key."""
...
class PageNumberOutOfBoundError(CustomException):
    """Exception raised when invalid page number(s) provided."""
...
class ResourceUnreachableException(CustomException):
    ...
    def __init__(
        self,
        message: str = Messages.FILE_UNREACHABLE,
        extra_info: Optional[Dict] = None,
    ):
        super().__init__(message, extra_info)
