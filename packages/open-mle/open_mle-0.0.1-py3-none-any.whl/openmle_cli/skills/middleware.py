...
Skills directory structure (builtin + per-agent + project):
Builtin: openmle_cli/builtin_skills/ (ships with package, read-only)
User-level: ~/.openmle/{AGENT_NAME}/skills/
Project-level: {PROJECT_ROOT}/.openmle/skills/

Example structure:
~/.openmle/{AGENT_NAME}/skills/
├── web-research/
...
Note: Builtin skills cannot be overridden. User/project skills are additional.
"""
...
from openmle_cli.skills.load import SkillMetadata, get_builtin_skills_dir, list_skills
...
class SkillsMiddleware(AgentMiddleware):
    ...
    Supports three skill sources:
    - Builtin: Ships with package in builtin_skills/ (read-only, cannot be overridden)
    - User: Created by user in ~/.openmle/{AGENT_NAME}/skills/ (additional skills)
    - Project: Project-specific in {PROJECT_ROOT}/.openmle/skills/ (additional skills)
    ...
    def __init__(
        ...
        *,
        ...
        self.builtin_skills_dir = get_builtin_skills_dir()
        
        # Store display paths for prompts
        self.user_skills_display = f"~/.openmle/{assistant_id}/skills"
        self.system_prompt_template = SKILLS_SYSTEM_PROMPT

    def _format_skills_locations(self) -> str:
        """Format skills locations for display in system prompt."""
        locations = [
            f"**Builtin Skills**: `{self.builtin_skills_dir}` (included with package, read-only)",
            f"**User Skills**: `{self.user_skills_display}` (your custom skills)",
        ]
        if self.project_skills_dir:
            locations.append(
                f"**Project Skills**: `{self.project_skills_dir}` (project-specific skills)"
            )
        return "\n".join(locations)
    ...
    def _format_skills_list(self, skills: list[SkillMetadata]) -> str:
        ...
        # Show builtin skills
        if builtin_skills:
            lines.append("**Builtin Skills** (included with package):")
    ...
    def before_agent(self, state: SkillsState, runtime: Runtime) -> SkillsStateUpdate | None:
        ...
        builtin, user-level, and project-level directories. Builtin skills
        are read-only and user/project skills are additional.
