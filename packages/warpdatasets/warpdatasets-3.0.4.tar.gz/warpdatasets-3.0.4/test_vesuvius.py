...
def main():
    ...
    try:
        # Get first row with wrap_refs to decode artifact references
        for batch in main_table.batches(batch_size=1, limit=1, as_format="dict", wrap_refs=True):
            print(f"    Batch keys: {list(batch.keys())}")

            # Check if image_path is wrapped
            if "image_path" in batch:
                ref = batch["image_path"][0]
                print(f"    image_path type: {type(ref).__name__}")

                # If it's a Ref object, try to read it
                if hasattr(ref, 'read'):
                    data = ref.read()
                    print(f"    Read {len(data)} bytes from image artifact!")
                ...
                elif hasattr(ref, 'as_bytes'):
                    data = ref.as_bytes()
                    print(f"    Read {len(data)} bytes via as_bytes()!")
                else:
                    print(f"    Ref value: {str(ref)[:80]}...")
            break
    except Exception as e:
        import traceback
        print(f"    ERROR: {e}")
