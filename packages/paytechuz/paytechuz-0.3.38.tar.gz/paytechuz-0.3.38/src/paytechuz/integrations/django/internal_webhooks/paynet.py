...
class PaynetWebhook(View):
    ...
    def post(self, request, **_):
        ...
        try:
            ...
            return self._error_response(rpc_id, PaynetErrors.INVALID_LOGIN_OR_PASSWORD, "Invalid login or password")
    ...
    def _check_auth(self, request):
        ...
        if not auth_header or not auth_header.startswith("Basic "):
            raise PermissionDenied("Missing or invalid authentication credentials")
        ...
        try:
            ...
            if username != self.paynet_username or password != self.paynet_password:
    ...
    def _check_transaction(self, params):
        ...
        return {
            "transactionState": transaction.state, # Need to map states?
            # Paynet states: 1=Success, 2=Cancelled?
            # Original code: Transaction.SUCCESSFUL (whatever that was)
            # Let's see original code view...
            # "transactionState": transaction.status
            # I should map `PaymentTransaction.state` to Paynet state codes.
            # Paynet: 1 = Success, 2 = Cancelled? (Need to verify constants)
            # Original code just returned `transaction.status`.
            # I will assume:
            # PaymentTransaction.SUCCESSFULLY -> 1
            # PaymentTransaction.CANCELLED -> 2
