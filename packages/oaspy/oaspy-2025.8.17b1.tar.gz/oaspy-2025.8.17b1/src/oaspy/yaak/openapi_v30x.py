...
from .yaak_v4 import validate_v4
            ...
            if data is not None:
                for item in data:
                    ...
                    if item["enabled"] is True:
                        variables[item["name"]] = {"default": str(item["value"])}

            # print("create_envs variables:", variables)
            url = list(variables.keys())
            print("create_envs url:", url)
            if len(url) > 0:
                obj["url"] = "https"
...
# TO FIX revisar los nombres
def create_tags(groups_list):
    # recorrer los grupos(folders)
    ...
    for item in groups_list:
        if item["folderId"] != "":
            item_parent = next((x["name"] for x in groups_list if x["id"] == item["folderId"]), None)
            parent_name = None
            if item_parent is not None:
                parent_name = item_parent.strip()
            if parent_name:
                # Validar si el elemento actual tiene elementos anidados
                if not any(item["id"] == parent["folderId"] for parent in groups_list):
    ...
    for item in groups_list:
        ...
        if not any(item["id"] == parent["folderId"] for parent in groups_list):
            tag_list.append({
                ...
                "description": item["description"] if "description" in item else item_name.strip(),
            })
...
# TODO
def change_type_vars(var_name):
    var_type = type(var_name).__name__
    match var_type:
        case "str":
            return "string"
        case "list":
...
def create_paths(requests_list, groups_list=None):
    ...
    try:
        ...
        for key, item in enumerate(requests_list):
            parent_id = item["folderId"] if "folderId" in item else None
            desc_name = item["name"]
            dirty_url = item["url"]
            ...
            obj_request = {
                _endpoint: {
                    _method: {
                        ...
                        "summary": desc_name,
                        "tags": [_group] if _group else ["root"],
            ...
            if _method in {"post", "put", "patch"}:
                ...
                mime_type = item["bodyType"] if "bodyType" in item else None
                ...
                if body is not None and bool(body) is True:
                    ...
                    match mime_type:
                        ...
                        case "application/x-www-form-urlencoded":
                            params = body["form"] if "form" in body else None

                            if not params:
                                print(
                                    f"skipping (missing application/x-www-form-urlencoded): {_method.upper()} -> '{desc_name}'"
                                )
                                continue
                            ...
                            for param in params:
                                enabled = False
                                obj: dict = {"type": "string", "format": "text"}
                                ...
                                if enabled is True:
                                    name = param["name"]
                                    obj_type = change_type_vars(param["value"])
                                    obj_form[name] = {"type": obj_type}
                        ...
                        case _:
                            print(f"skipping (unknown mime_type): {_method.upper()}({mime_type}) -> '{desc_name}'")
                            continue
                    ...
                    print(f"validating without body: {_method.upper()}({mime_type}) -> '{desc_name}'")
                    ...
                    match mime_type:
                        ...
                        case _:
                            # sin body ni mime_type
                            url_params = item["urlParameters"] if "urlParameters" in item else []
                            # print("url_params:", url_params)

                            if len(url_params) > 0:
                                p_list = []
                                for u_p in url_params:
                                    # print("u_p:", u_p)

                                    if u_p["enabled"] is True:
                                        p_list.append({
                                            "in": "query",
                                            "name": u_p["name"],
                                            "schema": {"type": change_type_vars(u_p["value"])},
                                        })

                                request_body = {"parameters": p_list}
                                # request_body = p_list
                                # print("request_body listo", request_body)
                            else:
                                print(f"skipping (unknown mime_type): {_method.upper()}({mime_type}) -> '{desc_name}'")
                                continue
            ...
            # print("ready to update request_paths")
            # print(_endpoint, _method)
            request_paths.update(obj_request)
            obj_request = {}

        print()
        # print("request_paths", request_paths)
...
# TO FIX
def yaak_gen_v30x(file_name, order_folder, order_request):
    """lee un archivo de yaak yaakSchema v4"""

    json_data = open_file(file_name)
    # resources = None

    if json_data is None:
        print("yaak_gen_v30x: failed to read json_data")
        return None
    ...
    envs_result = create_envs(resources.get("envs", None))
    # print("create_envs:", envs_result)
    tags_result, tag_groups_result = create_tags(resources["groups"])
    print()
    # print("tags_result:", tags_result)
    # print("tag_groups_result:", tag_groups_result)
