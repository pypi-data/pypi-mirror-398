...
class ConfigMeta(BaseModel):
    """Metadata class for use in type annotations for [`GlobalConfigOption`][tidegear.config.options.GlobalConfigOption] and its subclasses."""
...
class BaseConfigSchema:
    ...
    Danger:
        ...
        to update the version number yourself.** Tidegear deliberately does not automatically handle updating schema version numbers
        to prevent accidental data corruption. If you are not making a breaking change that requires a migration for users on the
        previous config version, you do not need to, and should not, change this value.

        In the circumstance where this value is incremented, [`run_migrations`][tidegear.config.schema.BaseConfigSchema.run_migrations]
        is not overridden or does not update the version number, and the cog utilizing the configuration schema is updated and loaded,
        any attempts to initialize the configuration schema through [`init`][tidegear.config.schema.BaseConfigSchema.init] will result
        in an exception being raised and initialization failing. This will most likely make your cog fail to load, if you initialize
        the configuration schema within your [`cog_load`][tidegear.cog.Cog.cog_load] method.
    """
    ...
    """An internal reference for a config schema's version, used for migrations. Override `version` to set your config schema's version, not this."""
    ...
    def implements_migrations(self) -> bool:
        """Returns whether or not this schema overrides the [`run_migrations`][tidegear.config.schema.BaseConfigSchema.run_migrations] method."""
        return class_overrides_attribute(child=self.__class__, parent=BaseConfigSchema, attribute=BaseConfigSchema.run_migrations.__name__)
    ...
    # not sure if this has a notable performance impact, but Config objects don't really have any risk for becoming stale, so it's harmless
    @classmethod
    @lru_cache()
    def _tidegear_get_config(
        cls, cog_name: str, identifier: int, *, force_registration: bool = True, logger: logging.RedTraceLogger | None
    ) -> red_config.Config:
        config = red_config.Config.get_conf(cog_instance=None, cog_name=cog_name, identifier=identifier, force_registration=force_registration)
    ...
    @classmethod
    def _tidegear_register_config(cls, config: red_config.Config, logger: logging.RedTraceLogger | None) -> dict[str, GlobalConfigOption]:
    ...
    async def _tidegear_run_migrations(self) -> None:
        ...
        try:
            ...
            await group.set(before)
            msg = f"Encountered an unexpected error when running migrations for `{self.cog_name}` (`{self.__class__.__qualname__}`)."
