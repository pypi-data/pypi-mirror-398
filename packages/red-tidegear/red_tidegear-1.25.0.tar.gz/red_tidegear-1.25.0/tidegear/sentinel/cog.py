...
class SentinelCog(Cog):
    ...
    async def sentinel_moderate(
        ...
        **kwargs: Any,
        ...
        if not ctx.guild:
            await send_error(ctx, content="Cannot moderate users outside of a guild!")
            return None
        ...
        try:
            ...
            await send_error(ctx, content="Internal error encountered, report this to the bot owner!")
            self._sentinel_logger.exception("Encountered exception within moderation handler %s", moderation_handler.__qualname__)
            return None
