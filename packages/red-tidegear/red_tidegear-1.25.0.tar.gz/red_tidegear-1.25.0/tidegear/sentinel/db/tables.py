...
class Table(BaseTable):
    ...
    @overload
    def json(
        self, *, columns: list[Column] | None = None, string: Literal[False] = False, encoding: str = ..., indent: bool = ..., options: int = ...
    ) -> bytes: ...

    @overload
    def json(
        self, *, columns: list[Column] | None = None, string: Literal[True], encoding: str = ..., indent: bool = ..., options: int = ...
    ) -> str: ...

    def json(
        self, *, columns: list[Column] | None = None, string: bool = False, encoding: str = "utf-8", indent: bool = False, options: int = 0
    ) -> bytes | str:
...
class PartialUser(Table, AbstractPartial):
    ...
    @overload
    async def fetch(self, fetcher: discord.Guild, *, fetch: Literal[False] = False, upsert: bool = ...) -> discord.Member | None: ...

    async def fetch(self, fetcher: Red | discord.Guild, *, fetch: bool = False, upsert: bool = True) -> discord.User | discord.Member | None:
...
class PartialChannel(Table, AbstractPartial):
    ...
    async def guild(self, /, *, cache: bool = True) -> PartialGuild:
        ...
        if isinstance(self.guild_id, PartialGuild):  # support both initialized PartialGuild objects and raw ints
            guild = self.guild_id
        elif not (guild := await PartialGuild.objects(PartialGuild.all_related()).where(PartialGuild.id == self.guild_id).first()):
...
class Change(Table):
    ...
    async def moderator(self, /, *, cache: bool = True) -> PartialUser:
        ...
        elif not (user := await PartialUser.objects(PartialUser.all_related()).where(PartialUser.id == self.moderator_id).first()):
            not_found()
...
class Moderation(Table):
    ...
    Attributes: Columns:
        ...
        target_role_id: The internal ID of the role this moderation case targets within the database.
            **This is not a Discord Role ID!**
            Use [`.target()`][tidegear.sentinel.Moderation.target] to get an actual [`PartialRole`][tidegear.sentinel.PartialChannel] object.
    ...
    async def guild(self, /, *, cache: bool = True) -> PartialGuild:
        ...
        Raises:
            NotFoundError: If the [`PartialGuild`][tidegear.sentinel.PartialGuild] tied to this moderation no longer exists in the database.
                This should be reported as a bug if it occurs, as there's a foreign key constraint that should prevent this on the table itself.
            UnsetError: If the moderation has no guild set. This indicates a bug in the moderation handler for whatever moderation type this is.
    ...
    async def moderator(self, /, *, cache: bool = True) -> PartialUser:
        ...
        Raises:
            NotFoundError: If the [`PartialUser`][tidegear.sentinel.PartialUser] tied to this moderation no longer exists in the database.
                This should be reported as a bug if it occurs, as there's a foreign key constraint that should prevent this on the table itself.
            UnsetError: If the moderation has no moderator set. This indicates a bug in the moderation handler for whatever moderation type this is.
    ...
    async def target(self, /, *, cache: bool = True) -> PartialTargetable:
        ...
        if self.target_user_id is not None:
            ...
            elif not (result := await PartialUser.objects(PartialUser.all_related()).where(PartialUser.id == self.target_user_id).first()):
        ...
        if not result and self.target_channel_id is not None:
            if isinstance(self.target_channel_id, PartialChannel):  # support both initialized PartialChannel objects and raw ints
                ...
                result := await PartialChannel.objects(PartialChannel.all_related()).where(PartialChannel.id == self.target_channel_id).first()
            ):
        ...
        if not result and self.target_role_id is not None:
            ...
            elif not (result := await PartialRole.objects(PartialRole.all_related()).where(PartialRole.id == self.target_role_id).first()):
