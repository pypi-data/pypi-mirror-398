...
class SentinelConfigSchema(BaseConfigSchema):
    ...
    When this is disabled, these permission requirements are removed for the moderator. The bot must still have the required permissions.
    """
    ...
    When this is disabled, moderators can moderate anyone whose top role is beneath the bot's top role, assuming they pass the other required checks.
    """
    ...
    """Whether or not to automatically send an ephemeral message to the moderator containing information about the moderation action.

    If the moderation command being used is not an Application Command, the message will be sent to the moderator's direct messages.
    """
    ...
    async def get_type_config_with_fallback(
        self, config: GuildConfigOption[JsonableType], *, guild: discord.Guild | int, moderation_type: "type[ModerationType] | ModerationType | str"
    ) -> JsonableType:
        ...
        if type_config.__pydantic_fields__["default"].annotation != config.__pydantic_fields__["default"].annotation | None:  # pyright: ignore[reportOptionalOperand]
            msg = f"The {type_attr_name!r} attribute exists on this configuration schema, but does not have matching annotations with {config.key!r}."
            raise TypeError(msg)
