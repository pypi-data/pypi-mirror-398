...
{
    m.doc() = "hola_graph._core: C++ bindings for HOLA (Human-like Orthogonal Layout Algorithm) from adaptagrams.";

// #ifdef VERSION_INFO
//     m.attr("__version__") = MACRO_STRINGIFY(VERSION_INFO);
//     // Parse version string to create version_info tuple
//     {
//         std::string version_str = MACRO_STRINGIFY(VERSION_INFO);
//         int major = 0, minor = 0, patch = 0;
//         sscanf(version_str.c_str(), "%d.%d.%d", &major, &minor, &patch);
//         m.attr("version_info") = py::make_tuple(major, minor, patch);
//     }
// #else
//     m.attr("__version__") = "dev";
//     m.attr("version_info") = py::make_tuple(0, 0, 0);
// #endif
    ...
    py::class_<Graph, Graph_SP>(m, "Graph", "The Graph class represents graphs consisting of nodes and edges.")
        ...
        .def("rotate_90cw", [](Graph &o) -> void { return o.rotate90cw(); }, "Rotate the layout 90 degrees clockwise.")
        ...
        .def("rotate_90acw", [](Graph &o) -> void { return o.rotate90acw(); }, "Rotate the layout 90 degrees anticlockwise.")
        ...
        .def("project", [](Graph &o, const struct ColaOptions &a0, enum vpsc::Dim const &a1) -> int { return o.project(a0, a1); }, "Project onto cola constraints.", py::arg("opts"), py::arg("dim"))
        ...
        .def("apply_proj_seq", [](Graph &o, const struct ColaOptions &a0, class ProjSeq &a1) -> bool { return o.applyProjSeq(a0, a1); }, "Attempt to apply the projections given by a ProjSeq object.", py::arg("opts"), py::arg("ps"))
        ...
        .def("update_nodes_from_rects", [](Graph &o) -> void { return o.updateNodesFromRects(); }, "Update Node positions based on internal vpsc Rectangles.")
        .def("update_nodes_from_rects", [](Graph &o, bool const &a0) -> void { return o.updateNodesFromRects(a0); }, "Update Node positions based on internal vpsc Rectangles.", py::arg("xAxis"))
        ...
        .def("update_cola_graph_rep", &Graph::updateColaGraphRep, "Refresh, as needed, the data structures necessary for applying the methods of libcola to this Graph.", py::return_value_policy::reference_internal)
        .def("build_root_cluster", &Graph::buildRootCluster, "Build a cola::RootCluster based on the node clusters specified in a ColaOptions object.", py::return_value_policy::reference_internal, py::arg("opts"))
        .def("get_cola_graph_rep", &Graph::getColaGraphRep, "Access the cola graph rep for this Graph.", py::return_value_policy::reference_internal)
        .def("get_sep_matrix", &Graph::getSepMatrix, "Access the separation matrix for this Graph.", py::return_value_policy::reference_internal)
        ...
        .def("pad_all_nodes", &Graph::padAllNodes, "Add padding to all nodes.", py::arg("dw"), py::arg("dh"))
        ...
        .def("set_routes_in_corresp_edges", [](Graph &o, class Graph &a0) -> void { return o.setRoutesInCorrespEdges(a0); }, "Update routes of Edges in a given Graph to equal those of corresponding Edges.", py::arg("H"))
        ...
        .def("__repr__", [](const Graph &g) {
            return "<hola_graph.Graph with " + std::to_string(g.getNumNodes()) + " nodes and " + std::to_string(g.getNumEdges()) + " edges>";
        })
        .def("__str__", [](const Graph &g) {
            return "Graph(" + std::to_string(g.getNumNodes()) + " nodes, " + std::to_string(g.getNumEdges()) + " edges)";
        })
        .def("__len__", &Graph::getNumNodes, "Return the number of nodes in the graph.")
        .def("get_nodes", [](Graph &g) {
            auto nodes = g.getNodeLookup();
            std::vector<Node_SP> nodeList;
            for (auto &pair : nodes) {
                nodeList.push_back(pair.second);
            }
            return nodeList;
        }, "Return a list of all nodes in the graph.")
        .def("__contains__", [](Graph &g, unsigned int id) {
            return g.hasNode(id);
        }, "Check if a node with the given ID exists in the graph.", py::arg("id"))
        .def("__iter__", [](Graph &g) {
            auto nodes = g.getNodeLookup();
            std::vector<Node_SP> nodeList;
            ...
            return py::cast(nodeList).attr("__iter__")();
        }, "Iterate over all nodes in the graph.")
        .def("add_nodes", [](Graph &g, const std::vector<Node_SP> &nodes) {
            for (auto &node : nodes) {
                g.addNode(node, true);
            }
        }, "Add multiple nodes to the graph.", py::arg("nodes"))
        .def("clear", [](Graph &g) {
            auto nodes = g.getNodeLookup();
    ...
    py::class_<Node, Node_SP>(m, "Node")
        ...
        .def("get_graph", &Node::getGraph, "Access the Graph to which the Node belongs.", py::return_value_policy::reference_internal)
        ...
        .def("lies_opposite_segment", [](Node &o, const struct LineSegment &a0) -> bool { return o.liesOppositeSegment(a0); }, "Check whether this Node lies opposite a LineSegment.", py::arg("seg"))
        ...
        .def("__repr__", [](const Node &n) {
            auto dims = n.getDimensions();
            return "<hola_graph.Node id=" + std::to_string(n.id()) + " w=" + std::to_string(dims.first) + " h=" + std::to_string(dims.second) + ">";
        })
        .def("__str__", [](const Node &n) {
            auto centre = n.getCentre();
            auto dims = n.getDimensions();
            return "Node " + std::to_string(n.id()) + " at (" +
                   std::to_string(static_cast<int>(centre.x)) + ", " +
                   std::to_string(static_cast<int>(centre.y)) + ") size " +
                   std::to_string(static_cast<int>(dims.first)) + "x" +
                   std::to_string(static_cast<int>(dims.second));
        })
        ;
    ...
    py::class_<Edge, Edge_SP>(m, "Edge")
        ...
        .def("__repr__", [](const Edge &e) {
            return "<hola_graph.Edge id=" + std::to_string(e.id()) + ">";
        })
        .def("__str__", [](const Edge &e) {
            return "Edge " + std::to_string(e.id());
        })
        ;
    ...
    py::class_<HolaOpts, std::shared_ptr<HolaOpts>>(m, "HolaOpts", "Configuration options for the HOLA layout algorithm.")
    ...
    py::class_<ACALayout, std::shared_ptr<ACALayout>>(m, "ACALayout", "Implements the Adaptive Constrained Alignment (ACA) algorithm.\n\n See\n Steve Kieffer, Tim Dwyer, Kim Marriott, and Michael Wybrow.\n \"Incremental grid-like layout using soft and hard constraints.\" In Graph\n Drawing 2013, pp. 448-459. Springer International Publishing, 2013.")
        ...
        .def("getFDLayout", (class cola::ConstrainedFDLayout * (ACALayout::*)()) &ACALayout::getFDLayout, "C++: ACALayout::getFDLayout() --> class cola::ConstrainedFDLayout *", py::return_value_policy::reference_internal)
        ...
        .def("initOrdAlign", (struct OrderedAlignment * (ACALayout::*)(int, enum ACASepFlag) const) &ACALayout::initOrdAlign, "C++: ACALayout::initOrdAlign(int, enum ACASepFlag) const --> struct OrderedAlignment *", py::return_value_policy::reference_internal, py::arg("j"), py::arg("sf"))
        .def("initOrdAlign", [](ACALayout const &o, int const & a0, int const & a1, enum ACASepFlag const & a2) -> OrderedAlignment * { return o.initOrdAlign(a0, a1, a2); }, "", py::return_value_policy::reference_internal, py::arg("s"), py::arg("t"), py::arg("sf"))
        .def("initOrdAlign", (struct OrderedAlignment * (ACALayout::*)(int, int, enum ACASepFlag, int) const) &ACALayout::initOrdAlign, "C++: ACALayout::initOrdAlign(int, int, enum ACASepFlag, int) const --> struct OrderedAlignment *", py::return_value_policy::reference_internal, py::arg("s"), py::arg("t"), py::arg("sf"), py::arg("edgeIndex"))
        .def("mostRecentOA", (struct OrderedAlignment * (ACALayout::*)()) &ACALayout::mostRecentOA, "C++: ACALayout::mostRecentOA() --> struct OrderedAlignment *", py::return_value_policy::reference_internal)
    ...
    py::class_<OrderedAlignment, std::shared_ptr<OrderedAlignment>>(m,  "OrderedAlignment", "Represents an ordered alignment constraint between nodes.")
    ...
    py::enum_<ACAFlag>(m, "ACAFlag", py::arithmetic(), "Flags for Adaptive Constrained Alignment layout direction.")
    ...
    py::enum_<ACASepFlag>(m, "ACASepFlag", py::arithmetic(), "Separation direction flags for Adaptive Constrained Alignment.")
    ...
    struct PyCallBack_dialect_SepMatrix : public dialect::SepMatrix {
        ...
        void printCreationCode(FILE * a0) const override {
            py::gil_scoped_acquire gil;
    ...
    py::class_<cola::CompoundConstraint, std::shared_ptr<cola::CompoundConstraint>>(m, "CompoundConstraint", "An abstract base class for all high-level compound constraints.\n\n A compound constraint is a conceptual, diagramming application oriented\n type of constraint, which can be translated into a set of simple\n separation constraints, possibly extra dummy variables, and perhaps\n even some extra terms for the quadratic objective function used\n in the gradient projection solver.")
        ...
        .def("printCreationCode", (void (cola::CompoundConstraint::*)(FILE *) const) &cola::CompoundConstraint::printCreationCode, "C++: cola::CompoundConstraint::printCreationCode(FILE *) const --> void", py::arg("fp"))
    ...
    py::class_<SepMatrix, std::shared_ptr<SepMatrix>, PyCallBack_dialect_SepMatrix, cola::CompoundConstraint>(m, "SepMatrix", "")
        ...
        .def("getGraph", (class Graph * (SepMatrix::*)()) &SepMatrix::getGraph, "Get the Graph.\n\nC++: SepMatrix::getGraph() --> class Graph *", py::return_value_policy::reference_internal)
    ...
    py::module_ avoid = m.def_submodule("avoid", "libavoid geometric types for obstacle avoidance routing.");
    ...
    py::class_<Avoid::Point, std::shared_ptr<Avoid::Point>>(avoid, "Point", "The Point class defines a point in the plane.\n\n Points consist of an x and y value.  They may also have an ID and vertex\n number associated with them.")
        // Note: Avoid::Point() doesn't initialize x,y - use (0,0) explicitly
        .def( py::init( [](){ return new Avoid::Point(0.0, 0.0); } ) )
