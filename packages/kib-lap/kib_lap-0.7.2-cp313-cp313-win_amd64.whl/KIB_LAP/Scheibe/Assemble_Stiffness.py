...
class Assembled_Matrices:
    ...
    def apply_BC(self):
        ...
        for index in sorted(self.Index_Vector_BC, reverse=True):
            self.K_Assemble_BC[index][index] += 1e9
    ...
    def PlotDeflectedStructure(self, _factor=1000):
        fig, ax = plt.subplots()
        factor = _factor
        id_el = 0
        for el in self.EL:
            ...
            coords_defl = [
                [
                    self.NL[node_id - 1][0]
                    + self.disp_element_matrix[idx * 2][id_el] * factor,
                    self.NL[node_id - 1][1]
                    + self.disp_element_matrix[idx * 2 + 1][id_el] * factor,
                ]
                for idx, node_id in enumerate(el)
            ]
            coords.append(coords[0])  # Add the first point again to close the polygon
            coords_defl.append(coords_defl[0])

            polygon_deflected = patches.Polygon(coords_defl, fill=None, edgecolor="r")
            ax.add_patch(polygon_deflected)

            id_el += 1

        ax.set_xlim(self.mesh.q[0, 0] - self.mesh.a, self.mesh.q[3, 0] + self.mesh.a)
        ax.set_ylim(self.mesh.q[0, 1] - self.mesh.b, self.mesh.q[3, 1] + self.mesh.b)
        plt.grid(True)
