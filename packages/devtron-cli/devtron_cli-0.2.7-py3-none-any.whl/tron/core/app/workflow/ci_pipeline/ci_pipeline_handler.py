...
class CiPipelineHandlers:
    ...
    def update_pre_post_ci_steps(current_steps: list, new_steps: list, plugin_metadata: dict, applied_plugin_metadata: list) -> list:
        ...
        try:
            ...
            for plugin in plugin_metadata.get("parentPlugins", []):
                for version in plugin.get("pluginVersions", {}).get("minimalPluginVersionData", []):
                    plugin_name = version.get("name", None)
                    plugin_version = version.get("pluginVersion", None)
                    ...
                    if existing_step:
                        ...
                        for plugin_data in applied_plugin_metadata:
                            ...
                            if pattern.match(plugin_name):
                                detailed_plugin_version_data = plugin_data.get("pluginVersions", {}).get("minimalPluginVersionData", [])
                                input_vars = plugin_data.get("pluginVersions", {}).get("detailedPluginVersionData")[0].get("inputVariables")
                                break
                        ...
                        add_plugin_result = CiPipelineHandlers.add_new_plugin(step, index, applied_plugin_metadata, detailed_plugin_version_data, input_vars)
    ...
    @staticmethod
    def add_new_plugin(step: dict, index: int, applied_plugin_metadata: list, detailed_plugin_version_data: list, input_vars):
        ...
        try:
            ...
            if input_vars:
                ...
                for input_variable in input_vars:
