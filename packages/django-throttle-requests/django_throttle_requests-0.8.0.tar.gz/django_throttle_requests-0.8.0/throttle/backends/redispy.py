...
#
# Script takes 1 key and 2 arguments: <bucket_interval>, <capacity>
TOKEN_BUCKET_SCRIPT = """
local bucket_interval = ARGV[1]
local bucket_capacity = ARGV[2]

local time = redis.call('TIME')
local now = time[1] + (time[2] / 1000000)

local filledAt = redis.call('HGET', KEYS[1], 'filledAt')
local tokens = tonumber(redis.call('HGET', KEYS[1], 'tokens'))
if filledAt == false then 
    filledAt = now
end
if tokens == nil then
...
-- Refill bucket
local tokensToAdd = math.floor((now - filledAt) * (bucket_interval / bucket_capacity))

tokens = math.min(tokens + tokensToAdd, bucket_capacity)
...
redis.call("HSET", KEYS[1], "filledAt", now, "tokens", tokens, "bucket_capacity", bucket_capacity, "tokensToAdd", tokensToAdd, "oldFilledAt", filledAt, "delta", (now - filledAt))
--redis.call("EXPIRE", KEYS[1], bucket_interval)
return bucket_capacity - tokens
"""

TOKEN_BUCKET_SCRIPT_SHA1 = hashlib.sha1(TOKEN_BUCKET_SCRIPT.encode('utf-8')).hexdigest()
...
class RedisBackend(ThrottleBackendBase):
    ...
    def __init__(self):
        ...
        self.time_function = None  # Not used, just for debugging
    ...
    def token_bucket(self, zone_name, bucket_key, bucket_interval, limit, cost=1):
        """
        Token bucket algorithm using Redis scripting
        """
        conn = redis.Redis(connection_pool=self.pool)
        ...
        try:
            try:
                return conn.evalsha(TOKEN_BUCKET_SCRIPT_SHA1, 1, bucket_cache_key, bucket_interval, limit)
            except NoScriptError:
                return conn.eval(TOKEN_BUCKET_SCRIPT, 1, bucket_cache_key, bucket_interval, limit)
