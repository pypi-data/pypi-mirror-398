...
class CacheBackend(ThrottleBackendBase):
    ...
    def token_bucket(self, zone_name, bucket_key, bucket_span, limit, cost=1):
        bucket_cache_key = "%s:%s" % (zone_name, bucket_key)
        now = self.time_function()

        # Get the current bucket value
        bucket_contents = self.cache.get(bucket_cache_key)
        if bucket_contents is None:
            filled_at = now
            tokens = bucket_span
        else:
            filled_at, tokens = bucket_contents.split(",")
            tokens = int(tokens)
            filled_at = float(filled_at)

        # Refill bucket
        tokens_to_add = (now - filled_at) * (limit / bucket_span)
        tokens = int(min(tokens + tokens_to_add, limit))
        #print(tokens, tokens_to_add, cost)

        if tokens < cost:
            return limit + 1  # Limit exceeded

        tokens -= cost
        self.cache.set(bucket_cache_key, f"{now},{tokens}", bucket_span)
        return limit - tokens
