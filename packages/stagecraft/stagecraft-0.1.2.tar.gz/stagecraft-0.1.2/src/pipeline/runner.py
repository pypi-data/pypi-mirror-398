...
from src.pipeline.definition import PipelineDefinition
from src.pipeline.memory import MemoryConfig
...
class PipelineRunner:
    ...
    def run(self, pipeline: PipelineDefinition, initial_context: Dict[str, Any] = None) -> PipelineResult:  # type: ignore
        ...
        try:
            ...
            result = PipelineResult(
                success=False, metadata=self.execution_metadata, error=e
            )
            return result

    def __build_variable_dependency_map(
    ...
    def __log_stage_start(
        self, stage_index: int, total_stages: int, stage_name: str
    ) -> None:
    ...
    def __handle_skipped_stage(
        ...
        metadata = self.__create_stage_metadata(
            stage.name, start_time, ExecutionStatus.SKIPPED
        )
    ...
    def __handle_successful_stage(
        ...
        metadata = self.__create_stage_metadata(
            stage_name, start_time, ExecutionStatus.COMPLETED
        )
        self.__record_stage_metadata(metadata)
    ...
    def __auto_clear_memory_if_enabled(
        ...
        if (
            ...
            and self.memory_config.auto_clear_enabled
            and context
        ):
            cleared = context.auto_clear_unused_variables(
                var_required_by, completed_stages
            )
            if cleared > 0:
    ...
    def __execute_single_stage(
        ...
        try:
            ...
            self.__auto_clear_memory_if_enabled(
                context, var_required_by, completed_stages
            )
    ...
    def __execute_pipeline_stages(
        self, pipeline: PipelineDefinition, context: PipelineContext
    ):
