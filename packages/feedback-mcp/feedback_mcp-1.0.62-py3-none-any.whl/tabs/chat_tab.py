...
class ChatTab(BaseTab):
    ...
    def _submit_feedback(self):
        ...
        # 获取包含图片占位符的文本内容，并解析大文本占位符
        raw_text = self._get_text_with_image_placeholders()
        # 使用 get_resolved_text 解析大文本占位符
        if hasattr(self.feedback_text, 'get_resolved_text'):
            text_content = self.feedback_text.get_resolved_text()
        else:
