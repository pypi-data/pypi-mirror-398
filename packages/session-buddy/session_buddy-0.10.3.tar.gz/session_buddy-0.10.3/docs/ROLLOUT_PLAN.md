...
- Flags now default `true`. If you prefer a staged rollout, disable selectively via environment variables (e.g., `SESSION_MGMT_USE_SCHEMA_V2=false`, `SESSION_MGMT_ENABLE_LLM_ENTITY_EXTRACTION=false`, `SESSION_MGMT_ENABLE_CONSCIOUS_AGENT=false`, `SESSION_MGMT_ENABLE_FILESYSTEM_EXTRACTION=false`).
