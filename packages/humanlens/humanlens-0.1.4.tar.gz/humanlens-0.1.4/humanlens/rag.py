...
#
# IMPORTANT FIX:
# - Use LangChain OpenAIEmbeddings (has embed_query/embed_documents)
#   instead of ragas.embeddings.OpenAIEmbeddings
# =====================================================
...
def _to_samples(json_path: str) -> List[Dict[str, Any]]:
    ...
    if not samples:
        raise ValueError("No valid samples found (need user_input, response, retrieved_contexts).")
...
def rag_analyze(
    ...
    llm = LangchainLLMWrapper(ChatOpenAI(model=eval_model, temperature=0))
