...
class PerceptronEmulator(QMainWindow):
    ...
    def initUI(self):
        ...
        # Knob size selector
        controls_layout.addWidget(QLabel("  |  Knob Size:"))
        self.knob_size_combo = QComboBox()
        self.knob_size_combo.addItems(['Small (40px)', 'Medium (60px)', 'Large (80px)', 'XLarge (100px)'])
        self.knob_size_combo.setCurrentIndex(2)  # Default to Large
        self.knob_size_combo.currentIndexChanged.connect(self.changeKnobSize)
        controls_layout.addWidget(self.knob_size_combo)
        ...
        panels_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        panels_scroll.setStyleSheet("""
            QScrollArea {
                border: none;
        ...
        panels_widget.setLayout(panel_layout)
        panels_scroll.setWidget(panels_widget)
        
        main_layout.addWidget(panels_scroll)
        
        # Training panel (also scrollable)
        training_scroll = QScrollArea()
        training_scroll.setWidgetResizable(True)
        training_scroll.setMaximumHeight(400)
        training_scroll.setStyleSheet("""
            QScrollArea {
        ...
        training_scroll.setWidget(training_panel)
        main_layout.addWidget(training_scroll)
    ...
    def createWeightGrid(self):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                # Create knob with configurable size
                knob_size = self.get_knob_size()
                knob = RotaryKnob(min_value=-1.0, max_value=1.0, size=knob_size)
                index = row * self.cols + col
    ...
    def createBiasAndMeter(self):
        ...
        knob_size = self.get_knob_size()
        self.bias_knob = RotaryKnob(min_value=-1.0, max_value=1.0, size=knob_size)
    ...
    def createTrainingPanel(self):
        ...
        self.pattern_combo.addItems(['AND', 'OR', 'NAND', 'PATTERN LIBRARY'])
        ...
        save_load_layout.setContentsMargins(0, 5, 0, 0)
        
        self.save_patterns_btn = QPushButton("Save Set")
        self.save_patterns_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        self.save_patterns_btn.clicked.connect(self.savePatternSet)
        save_load_layout.addWidget(self.save_patterns_btn)
        
        self.load_patterns_btn = QPushButton("Load Set")
        self.load_patterns_btn.setStyleSheet("""
            QPushButton {
                background-color: #4c6ef5;
                color: #fff;
                border: none;
        ...
        """)
        self.load_patterns_btn.clicked.connect(self.loadPatternSet)
        save_load_layout.addWidget(self.load_patterns_btn)
        
        save_load_container.setLayout(save_load_layout)
        custom_layout.addWidget(save_load_container)
        
        # Load preset shapes button
        self.load_preset_btn = QPushButton("📚 Load Preset Shapes")
        ...
        """)
        self.load_preset_btn.clicked.connect(self.loadPresetShapes)
        custom_layout.addWidget(self.load_preset_btn)
    ...
    def startTraining(self):
        ...
        saved_custom_patterns = self.trainer.custom_patterns.copy() if self.pattern_combo.currentText() == 'PATTERN LIBRARY' else []
        ...
        if pattern_name == 'PATTERN LIBRARY':
    ...
    def onPatternTypeChanged(self, pattern_type):
        ...
        is_custom = (pattern_type == 'PATTERN LIBRARY')
    ...
    def savePatternSet(self):
        """Save current custom patterns to file."""
        if not self.trainer.custom_patterns:
            QMessageBox.warning(self, "No Patterns", "No patterns to save!")
            return
        
        name, ok = QInputDialog.getText(
            self,
            "Save Pattern Set",
            "Enter name for this pattern set:"
        )
        if ok and name:
            self.config_manager.save_pattern_set(name, self.trainer.custom_patterns)
            QMessageBox.information(self, "Saved", f"Pattern set '{name}' saved successfully!")
    
    def loadPatternSet(self):
        """Load saved pattern set."""
        sets = self.config_manager.list_pattern_sets()
        if not sets:
            QMessageBox.information(self, "No Sets", "No saved pattern sets found.")
            return
        
        name, ok = QInputDialog.getItem(
            self,
            "Load Pattern Set",
            "Select pattern set to load:",
            sets,
            0,
            False
        )
        if ok and name:
            patterns = self.config_manager.load_pattern_set(name)
            if patterns:
                self.trainer.custom_patterns = patterns
                self.updateCustomPatternCount()
                QMessageBox.information(self, "Loaded", f"Loaded {len(patterns)} patterns from '{name}'")
    
    def loadPresetShapes(self):
        """Load preset shapes from library."""
        from .pattern_library import PresetShapeDialog
        
        dialog = PresetShapeDialog(self.rows, self.cols, self)
        if dialog.exec():
            selected_patterns = dialog.get_selected_patterns()
            if selected_patterns:
                self.trainer.custom_patterns.extend(selected_patterns)
                self.updateCustomPatternCount()
                QMessageBox.information(
                    self,
                    "Shapes Loaded",
                    f"Added {len(selected_patterns)} preset patterns!"
                )
    
    def get_knob_size(self):
        """Get current knob size from combo box."""
        sizes = {
            0: 40,   # Small
            1: 60,   # Medium
            2: 80,   # Large
        ...
        return sizes.get(self.knob_size_combo.currentIndex(), 80)
    
    def changeKnobSize(self):
        """Handle knob size change - rebuild UI."""
        self.changeGridSize(None)  # Reuse grid size change logic
