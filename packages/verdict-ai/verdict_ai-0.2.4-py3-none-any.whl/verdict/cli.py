...
@click.group()
@click.version_option(version="0.2.4", prog_name="verdict")
def cli():
...
def decide(idea: str, context: str, provider: str, model: str):
    ...
    try:
        ...
        with console.status("[bold cyan]Generating artifacts...[/bold cyan]", spinner="dots"):
            ...
            artifacts = generator.compile(
                idea, verdict, execution_plan, pipeline.provider, pipeline.model
            )
...
def version():
    ...
    console.print(
        Panel(
            "[bold cyan]Verdict[/bold cyan] v0.2.4\n\n"
