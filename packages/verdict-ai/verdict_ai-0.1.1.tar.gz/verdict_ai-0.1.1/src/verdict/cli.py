...
def decide(idea: str, context: str):
    ...
    try:
        ...
        pipeline = DecisionPipeline(api_key)
    ...
    if not api_key.startswith("sk-ant-"):
        console.print(
            Panel(
                ...
                "Anthropic API keys should start with [cyan]sk-ant-[/cyan]\n\n"
                "[bold]How to get your API key:[/bold]\n"
                "1. Visit [cyan]https://console.anthropic.com/account/keys[/cyan]\n"
                "2. Create a new API key\n"
                "3. Copy the key and run [cyan]verdict config[/cyan] again",
                title="[bold red]Error[/bold red]",
                border_style="red",
            )
        )
...
def version():
    ...
    console.print(
        Panel(
            "[bold cyan]Verdict[/bold cyan] v0.1.0\n\n"
