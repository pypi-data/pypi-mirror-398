...
async def get_kingdom_data(kidstr: str | None = None) -> int:
    """
...
async def get_castle_id(target_kingdom: int | None = None) -> int:
...
async def switch_kingdoms(target_kingdom: str | None = None, sync: bool | None = False) -> bool | dict:
    """
    ...
    switch_data = {
        "CID":get_castle_id(target_kid),
        "KID":target_kid
        }
    
    if sync:
        try:
            response = await wrapper.send_json("jca", switch_data, True)
        ...
        try:
            response = await wrapper.send_json("jca", switch_data, False)
            return True
        except Exception as e:
            logger.error(f"Error sending switch kingdom request: {e}")
            return False
