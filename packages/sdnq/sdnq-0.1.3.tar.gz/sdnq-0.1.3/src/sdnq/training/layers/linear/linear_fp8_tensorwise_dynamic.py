...
def fp8_matmul_tensorwise_dynamic_backward(
    ...
    if do_grad_input:
        grad_input = fp8_matmul_tensorwise_dynamic(grad_output, weight, svd_up=svd_up, svd_down=svd_down, output_shape=input.shape, do_input_reshape=False, use_sr=True)
    if do_grad_weight:
