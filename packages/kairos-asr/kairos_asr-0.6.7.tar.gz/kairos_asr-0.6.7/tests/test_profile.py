...
def test_profile_detailed_vad_segmentation(asr_cuda):
    """Тест профилирования VAD сегментации из profile_detailed.py"""
    file_path = str(TEST_WAV)
    
    # Разогрев
    _ = asr_cuda.transcribe(wav_file=file_path)
    
    # Тест VAD сегментации
    t1 = time.time()
    ...
    assert (t2 - t1) < 2, "VAD не должен занимать слишком много времени (макс 2 секунды)"
...
def test_profile_detailed_process_segment(asr_cuda):
    """Тест профилирования обработки одного сегмента из profile_detailed.py"""
    file_path = str(TEST_WAV)
    ...
    if segments:
        ...
        assert (t2 - t1) > 0, "Время выполнения должно быть положительным"
        assert (t2 - t1) < 30, "Обработка сегмента не должна занимать слишком много времени (макс 30 сек)"
...
def test_profile_detailed_full_transcription(asr_cuda):
    """Тест профилирования полной транскрипции из profile_detailed.py"""
    file_path = str(TEST_WAV)
    ...
    assert (t2 - t1) > 0, "Время выполнения должно быть положительным"
    assert (t2 - t1) < 120, "Полная транскрипция не должна занимать слишком много времени (макс 120 сек)"
...
def test_profile_detailed_multiple_runs(asr_cuda):
    """Тест профилирования множественных запусков из profile_detailed.py"""
    file_path = str(TEST_WAV)
    ...
    assert avg_time < 120, "Среднее время не должно быть слишком большим (макс 120 сек)"
...
def test_profile_performance_cprofile(asr_cuda):
    """Тест профилирования с cProfile из profile_performance.py"""
    file_path = str(TEST_WAV)
...
def test_profile_performance_timing(asr_cuda):
    """Тест измерения времени выполнения из profile_performance.py"""
    file_path = str(TEST_WAV)
    ...
    assert elapsed < 120, "Транскрипция не должна занимать слишком много времени (макс 120 сек)"
