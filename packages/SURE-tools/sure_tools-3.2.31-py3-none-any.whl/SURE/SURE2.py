...
class SURE2(nn.Module):
    ...
    def setup_networks(self):
        ...
        self.encoder_n = zuko.flows.NSF(features=self.code_size, context=self.latent_dim, 
                                         transforms=self.transforms, hidden_features=self.flow_hidden_layers)
    ...
    def model(self, xs, cs=None):
        ...
        with pyro.plate('data'):
            prior_loc = torch.zeros(batch_size, self.code_size, **self.options)
            prior_scale = torch.ones_like(prior_loc)
            ns = pyro.sample('n', dist.Normal(prior_loc, prior_scale).to_event(1))
            ns = self.softmax(ns)
    ...
    def guide(self, xs, cs=None):
        with pyro.plate('data'):
            ...
            ns = pyro.sample('n', ZukoToPyro(self.encoder_n(zns)))


    def _get_codebook(self, sample_size=10):
        I = torch.eye(self.code_size, **self.options)
        cb = self.codebook(I)
        cb = self.decoder_zn(cb).sample((sample_size,)).mean(axis=0)
        return cb
    ...
    def _code(self, xs):
        ...
        alpha = self.encoder_n(zns).sample((10,)).mean(axis=0)
        return alpha
    ...
    def fit(self, xs, 
        ...
        if cs is not None:
            cs = convert_to_tensor(cs, dtype=self.dtype, device=self.get_device())
...
def main():
    ...
    if args.save_model is not None:
        if args.save_model.endswith('gz'):
            SURE2.save_model(sure, args.save_model, compression=True)
        else:
            SURE2.save_model(sure, args.save_model)
