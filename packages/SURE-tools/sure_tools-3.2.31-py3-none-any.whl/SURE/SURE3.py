...
class SURE3(nn.Module):
    ...
    def setup_networks(self):
        ...
        self.decoder_zn = zuko.flows.NSF(features=self.latent_dim, context=self.code_size, 
                                         transforms=self.transforms, hidden_features=self.flow_hidden_layers)
        self.decoder_x = zuko.flows.MAF(features=self.input_size, context=self.latent_dim, 
                                         transforms=self.transforms, hidden_features=self.flow_hidden_layers)
    ...
    def model(self, xs, us=None):
        ...
        with pyro.plate('data'):
            ...
            zns = pyro.sample('zn', ZukoToPyro(self.decoder_zn(ns)))
    ...
    def _get_codebook(self, sample_size=10):
        ...
        cb = self.decoder_zn(I).sample((sample_size,)).mean(axis=0)
        return cb
...
def main():
    ...
    if args.save_model is not None:
        if args.save_model.endswith('gz'):
            SURE3.save_model(sure, args.save_model, compression=True)
        else:
            SURE3.save_model(sure, args.save_model)
