...
### `.flightlineignore` (recommended)

Flightline supports a repo-root ignore file named **`.flightlineignore`** using **`.gitignore`-style patterns**.
This is applied in addition to `.gitignore` and Flightline’s built-in defaults (e.g. `node_modules/`, `.git/`, etc).

Example: if all AI calls live in `backend/`, ignore the entire `frontend/`:

```text
frontend/
```

You can add any gitignore patterns here (globs, `**`, negation via `!`, etc.).
