...
def print_boot_sequence(command: str = "", version: str = ""):
    ...
    # Command info line
    if command:
        info_line = f"                              flightline {command.lower()}"
        if version:
            info_line += f" v{version}"
        console.print(info_line, style="hud.dim")
    console.print()
...
def print_complete(results):
    """Print the completion summary screen."""
    console.print()

    # Backwards-compatible: allow callers to pass a single string message.
    if isinstance(results, str):
        results = {"status": results}
    elif not isinstance(results, dict):
        results = {"status": str(results)}
