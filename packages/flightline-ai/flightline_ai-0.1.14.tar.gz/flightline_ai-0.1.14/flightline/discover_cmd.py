...
def _sync_to_cloud(
    ...
    compare_to_commit_sha: Optional[str],
    report_mode: str,
    ...
    api_url = api_url_override or os.getenv("FLIGHTLINE_API_URL", "https://api.flightline.ai")
    ...
    payload = {
        ...
        "compare_to_commit_sha": compare_to_commit_sha,
...
def _render_markdown_briefing(output: DiscoveryOutput) -> str:
    """Render a lightweight markdown briefing to stdout (for CI / GitHub Actions)."""
    provider_counts = {}
    risk_counts = {"critical": 0, "high": 0, "medium": 0, "low": 0}

    for node in output.nodes:
        provider_counts[node.provider.value] = provider_counts.get(node.provider.value, 0) + 1
        risk = node.interpretation.risk_tier.value if node.interpretation else "low"
        if risk in risk_counts:
            risk_counts[risk] += 1

    lines = []
    lines.append("## 🛰️ Flightline Mission Briefing")
    lines.append("")
    lines.append(f"- **AI Operations Found:** {len(output.nodes)}")
    lines.append(
        "- **Risk Breakdown:** "
    ...
    if provider_counts:
        ...
        for provider, count in sorted(provider_counts.items(), key=lambda x: (-x[1], x[0])):
            lines.append(f"- **{provider.upper()}**: {count}")
        lines.append("")
    ...
    if top_nodes:
        ...
        for node in top_nodes:
            risk = node.interpretation.risk_tier.value if node.interpretation else "low"
            loc = f"{node.location.file}:{node.location.line}"
            lines.append(f"- **[{risk.upper()}]** `{loc}`")
            if node.interpretation and node.interpretation.observations_summary:
                for obs in node.interpretation.observations_summary[:3]:
                    lines.append(f"  - {obs}")
        lines.append("")

    lines.append("---")
    lines.append("*Generated by Flightline CLI*")
    return "\n".join(lines)
...
@click.option(
    ...
    type=str,
    default=None,
    envvar="FLIGHTLINE_API_URL",
    help="Flightline API base URL (or set FLIGHTLINE_API_URL)",
)
@click.option(
    "--format",
    "output_format",
    type=click.Choice(["hud", "md", "json"], case_sensitive=False),
    default="hud",
    show_default=True,
    help=(
        "Output format for stdout (hud prints terminal UI; md prints a markdown briefing; json prints the JSON result)."
    ),
)
...
@click.option(
    "--compare-to-sha",
    "compare_to_sha",
    envvar="GITHUB_BASE_SHA",
    help="Base commit SHA to diff against in Mission Control (for per-PR head↔base diffs)",
)
@click.option(
    "--report-mode",
    type=click.Choice(["head", "base"], case_sensitive=False),
    default="head",
    show_default=True,
    envvar="FLIGHTLINE_REPORT_MODE",
    help="Upload mode: head updates PR report; base only persists snapshot (no PR update).",
)
...
def discover(
    ...
    # Backwards compat: --json forces json output format.
    if json_output:
        output_format = "json"
    ...
    if output_format == "json":
        ...
        click.echo(_render_markdown_briefing(output_data))
    elif not quiet_mode:
    ...
    if sync:
        _sync_to_cloud(
            ...
            branch,
            commit_sha,
            compare_to_commit_sha=compare_to_sha,
