...
@click.option(
    "--api-url",
    default=lambda: os.getenv("FLIGHTLINE_API_URL", "https://be.flightlinehq.com"),
...
@click.option(
    ...
    default="backend",
    show_default=True,
    help="Where to run LLM analysis for data samples. TypeScript/Pydantic are always parsed locally.",
)
@click.option(
    "--backend-url",
...
def learn(
    ...
    if file_format == FORMAT_TYPESCRIPT:
        ...
        # Data files need LLM analysis.
        if engine == "backend":
            try:
                with httpx.Client(timeout=60.0) as client:
                    resp = client.post(
                        f"{backend_url.rstrip('/')}/api/v1/learn/analyze",
                        json={
                            "content": file_path.read_text(),
    ...
    profile_id = None
    if sync:
        # If learn ran via backend engine, the profile has already been persisted server-side.
        if (
            engine == "backend"
            and file_format == FORMAT_DATA_SAMPLE
            and isinstance(profile, dict)
            and profile.get("cloud_id")
        ):
            profile_id = str(profile.get("cloud_id"))
