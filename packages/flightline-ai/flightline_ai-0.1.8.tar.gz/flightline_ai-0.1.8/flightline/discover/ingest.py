    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for filename in filenames:
            ...
            if file_path.suffix.lower() in valid_extensions:
                ...
                if not should_ignore(file_path, root, gitignore_spec, default_spec):
...
def should_ignore(
    path: Path, root: Path, gitignore_spec: Optional[pathspec.PathSpec], default_spec: pathspec.PathSpec
) -> bool:
...
def load_gitignore(root: Path) -> Optional[pathspec.PathSpec]:
    """Load and parse .gitignore from the root directory."""
