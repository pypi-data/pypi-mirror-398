...
class TimeDict(Generic[K]):
    ...
    def __init__(self, data: Mapping[K, Decimable] = dict(), strict: bool = True):
    ...
    def fill(self, start: K, end: K, value: Decimable) -> "Self":
        """
        Create a new graph with a specified range and value.
        The range is defined by start and end.
        """
        v = to_decimal(value)
        return type(self)({k: v for k in self._inclusive_range(start, end)})

    def get(self, key: K, default: Decimal = NAN) -> Decimal:
        """
        Get the value for a specific key. If the key does not exist, return the default value.
    ...
    def _binary_op(
        ...
        if isinstance(other, Decimable):
            ...
            raise TypeError(
                f"Unsupported operand type(s) for operation: 'TimeDict' and '{type(other)}'"
            )
    ...
    def to_dict(self) -> dict[K, Decimal]:
