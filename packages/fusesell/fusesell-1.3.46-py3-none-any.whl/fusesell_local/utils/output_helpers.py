...
TEAM_SETTINGS_LABELS: Dict[str, str] = {
    "gs_team_organization": "Team Organization",
    "gs_team_rep": "Sales Representatives",
    "gs_team_product": "Products",
    "gs_team_schedule_time": "Schedule Time",
    "gs_team_initial_outreach": "Initial Outreach",
    "gs_team_follow_up": "Follow Up",
    "gs_team_auto_interaction": "Auto Interaction",
    "gs_team_followup_schedule_time": "Follow-up Schedule",
    "gs_team_birthday_email": "Birthday Emails",
}

TEAM_SETTINGS_ORDER: List[str] = [
...
def _render_team_settings_rows(data: Dict[str, Any]) -> str:
    """Render a dictionary of settings into key/value rows."""
    if not isinstance(data, dict):
        return ""
    ...
    for sub_key, sub_val in data.items():
        if sub_val in (None, "", [], {}):
            continue
        if isinstance(sub_val, list) and not _is_complex_array(sub_val):
            chips = "".join(
                f"<span class='chip'>{html.escape(str(item))}</span>"
                for item in sub_val
                if item not in (None, "", [], {})
            )
            val_html = f"<div class='chips'>{chips}</div>" if chips else "<div class='muted'>No entries.</div>"
        else:
            val_html = _render_value(sub_val, depth=2, key=str(sub_key), html_render_keys=DEFAULT_HTML_RENDER_KEYS)
        rows.append(
            "<div class='kv'>"
            f"<div class='k'>{html.escape(_friendly_key(str(sub_key)))}</div>"
            f"<div class='v'>{val_html}</div>"
            "</div>"
        )
    return "".join(rows)


def _render_team_settings_section(key: str, value: Any) -> str:
    """Render a single team settings section with a consistent card layout."""
    if value in (None, "", [], {}):
        return ""

    label = TEAM_SETTINGS_LABELS.get(key, _friendly_key(key))
    cards: List[str] = []

    if isinstance(value, list):
        for idx, item in enumerate(value, start=1):
            if isinstance(item, dict):
                rows = _render_team_settings_rows(item)
                badge = f"<div class='badge'>#{idx}</div>" if len(value) > 1 else ""
                cards.append(
                    "<div class='card section-card'>"
                    f"{badge}"
                    f"{rows or '<div class=\"muted\">No details provided.</div>'}"
                    "</div>"
                )
            else:
                cards.append(
                    ...
                    f"<div class='v'><span class='text'>{html.escape(_first_non_empty(item) or str(item))}</span></div>"
                    "</div>"
                    "</div>"
                )
    elif isinstance(value, dict):
        rows = _render_team_settings_rows(value)
        cards.append(
            "<div class='card section-card'>"
            f"{rows or '<div class=\"muted\">No details provided.</div>'}"
            "</div>"
        ...
        cards.append(
            ...
            f"<div class='v'><span class='text'>{html.escape(_first_non_empty(value) or str(value))}</span></div>"
            "</div>"
            "</div>"
        )

    section_body = "".join(cards) or "<div class='muted'>No details provided.</div>"
    return f"<div class='section'><h2>{html.escape(label)}</h2>{section_body}</div>"


def _render_team_settings_html(payload: Dict[str, Any], raw_json: str) -> str:
    """Render set/get team settings outputs with readable sections."""
    ...
    for key in TEAM_SETTINGS_ORDER:
        html_section = _render_team_settings_section(key, settings.get(key))
        if html_section:
            seen_keys.add(key)
    ...
    for key, val in settings.items():
        if key in seen_keys:
            continue
        if not str(key).startswith("gs_team_"):
            continue
        extra = _render_team_settings_section(str(key), val)
        if extra:
            sections.append(extra)

    sections_html = "".join(sections) or "<div class='muted'>No team settings provided.</div>"
    ...
    style = (
        ...
        "body{font-family:'Segoe UI','Helvetica Neue',Arial,sans-serif;color:var(--text);background:var(--bg);line-height:1.6;transition:background 0.2s,color 0.2s;}"
        "*{box-sizing:border-box;}"
        ".container{max-width:1100px;margin:32px auto;padding:0 20px 32px 20px;}"
        "h1,h2{color:var(--text);margin:0 0 12px 0;}"
        ".section{margin-bottom:20px;}"
        ".card{background:var(--card-bg);border:1px solid var(--card-border);border-radius:10px;padding:14px 16px;box-shadow:0 2px 8px var(--shadow);}"
        ".section-card{margin-top:6px;}"
        ".kv{display:grid;grid-template-columns:220px minmax(0,1fr);gap:8px 14px;padding:8px 0;border-bottom:1px solid var(--card-border);}"
        ".kv:last-child{border-bottom:none;}"
        ".k{font-weight:600;color:var(--text);}"
        ".v{color:var(--text);}"
        ...
        ".badge{display:inline-block;background:var(--badge-bg);color:var(--text);padding:2px 8px;border-radius:999px;font-weight:700;font-size:11px;margin-bottom:6px;}"
        ".text{background:var(--card-border);border-radius:6px;padding:4px 8px;display:inline-block;}"
        ".muted{color:var(--muted);font-style:italic;}"
        ".pre{background:var(--pre-bg);color:var(--pre-text);padding:12px;border-radius:8px;overflow:auto;font-family:SFMono-Regular,Consolas,'Liberation Mono',Menlo,monospace;font-size:13px;line-height:1.55;white-space:pre-wrap;word-break:break-word;}"
    ...
    return (
        "<!doctype html>"
        "<html><head><meta charset='utf-8'>"
        "<title>FuseSell AI - Team Settings</title>"
        f"<style>{style}</style>"
        "</head><body>"
        "<div class='container'>"
        "<h1>FuseSell AI - Team Settings</h1>"
        f"{sections_html}"
        "<div class='section'>"
        "<h2>Raw JSON</h2>"
        "<details><summary>View Full Raw JSON</summary>"
...
def write_full_output_html(
    ...
    *,
    ...
    try:
        ...
        if flow_name in {"set_team_settings_compact", "get_team_settings_compact"} or (
            isinstance(sanitized, dict) and any(str(k).startswith("gs_team_") for k in sanitized)
        ):
            payload_for_settings = sanitized if isinstance(sanitized, dict) else {"data": sanitized}
            content = _render_team_settings_html(payload_for_settings, raw_serialized)
