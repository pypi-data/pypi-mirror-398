...
BUILD_CONFIG = SubConfigSrc(
    ...
    # ConfigOptionSrc(
    #     'group_sub_project_id', int, default=0,
    #     extra_docs="""

    #     À définir si le projet en cours appartient à un groupe GitLab regroupant différents sous-
    #     projets ayant des pages GitLab impliquant PMT :

    #     Cette valeur permet de:
    #     - Différencier les entrées du localStorage liées à un sous-projet de celles d'un autre.
    #     - Différencier les ids html des IDE des différents sous-projets construits avec l'option
    #     {{ config_link("build.ides_id_hash_mode", val="relative", code_block=False) }}

    #     !!! danger "Différents projets PMT hébergés dans le même groupe GitLab/Forge EN/GitHub"

    #         Le localStorage d'un navigateur est spécifique à un nom de domaine. Cela implique qu'il
    #         est commun à tous les projets de documentation rendus dans le même groupe GitLab.

    #         Cela peut avoir deux impacts :

    #         1. Le bouton poubelle

    #     """,
    #     yaml_desc="""
    #         Id of the project in a GitLab group, to differentiate the contents of the local storage
    #         between different projects in the same GitLab group.
    #     """,
    # ),
