...
def connect(autocommit=False, attrs_before=None):
    c = npyodbc.connect(CNXNSTR, autocommit=autocommit, attrs_before=attrs_before)
    ...
    # in UTF-8.  The npyodbc default is to send Unicode as UTF-16 and to
    # decode WCHAR via UTF-16.  Change them both to UTF-8.
    c.setdecoding(npyodbc.SQL_CHAR, encoding='utf-8')
    c.setdecoding(npyodbc.SQL_WCHAR, encoding='utf-8')
...
def test_blob(cursor: npyodbc.Cursor):
...
def test_rowcount_select(cursor: npyodbc.Cursor):
    ...
    npyodbc calls SQLRowCount after each execute and sets Cursor.rowcount.  Databases can return
