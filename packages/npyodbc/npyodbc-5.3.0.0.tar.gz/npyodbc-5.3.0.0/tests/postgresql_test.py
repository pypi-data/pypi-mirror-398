...
def test_text_many(cursor: npyodbc.Cursor):
...
def test_bytea(cursor: npyodbc.Cursor):
...
def test_bytearray(cursor: npyodbc.Cursor):
    """
...
def test_numeric(cursor: npyodbc.Cursor):
...
def test_maxwrite(cursor: npyodbc.Cursor):
    # If we write more than `maxwrite` bytes, npyodbc will switch from binding the data all at
...
def test_close_cnxn(cursor: npyodbc.Cursor):
...
def test_rowcount(cursor: npyodbc.Cursor):
    ...
    # driver is telling npyodbc the rowcount is 0.  Since we have no way of knowing when to
...
def test_lower_case(cursor: npyodbc.Cursor):
...
def test_executemany(cursor: npyodbc.Cursor):
    ...
    try:
        ...
        npyodbc.fast_executemany = False
...
def test_getinfo_string(cursor: npyodbc.Cursor):
    value = cursor.connection.getinfo(npyodbc.SQL_CATALOG_NAME_SEPARATOR)
    assert isinstance(value, str)


def test_getinfo_bool(cursor: npyodbc.Cursor):
    value = cursor.connection.getinfo(npyodbc.SQL_ACCESSIBLE_TABLES)
    assert isinstance(value, bool)


def test_getinfo_int(cursor: npyodbc.Cursor):
    value = cursor.connection.getinfo(npyodbc.SQL_DEFAULT_TXN_ISOLATION)
    assert isinstance(value, int)


def test_getinfo_smallint(cursor: npyodbc.Cursor):
    value = cursor.connection.getinfo(npyodbc.SQL_CONCAT_NULL_BEHAVIOR)
    assert isinstance(value, int)

def test_cnxn_execute_error(cursor: npyodbc.Cursor):
    """
...
def test_autocommit(cursor: npyodbc.Cursor):
...
def test_cnxn_set_attr(cursor: npyodbc.Cursor):
    # I don't have a getattr right now since I don't have a table telling me what kind of
    # value to expect.  For now just make sure it doesn't crash.
    # From the unixODBC sqlext.h header file.
...
def test_columns(cursor: npyodbc.Cursor):
    driver_version = tuple(
        int(x) for x in cursor.connection.getinfo(npyodbc.SQL_DRIVER_VER).split(".")
    )
...
def test_cursor_messages(cursor: npyodbc.Cursor):
    """
...
def test_output_conversion(cursor: npyodbc.Cursor):
    ...
    cursor.connection.add_output_converter(npyodbc.SQL_WVARCHAR, convert)
    ...
    cursor.connection.remove_output_converter(npyodbc.SQL_WVARCHAR)
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == '123.45'
    ...
    cursor.connection.add_output_converter(npyodbc.SQL_WVARCHAR, None)
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == '123.45'
