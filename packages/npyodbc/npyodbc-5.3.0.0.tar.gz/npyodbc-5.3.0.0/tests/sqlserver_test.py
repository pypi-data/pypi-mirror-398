...
CNXNSTR = os.environ.get('PYODBC_SQLSERVER', 'DSN=npyodbc-sqlserver')


def connect(autocommit=False, attrs_before=None):
    return npyodbc.connect(CNXNSTR, autocommit=autocommit, attrs_before=attrs_before)


DRIVER = connect().getinfo(npyodbc.SQL_DRIVER_NAME)
...
@pytest.fixture()
def cursor() -> Iterator[npyodbc.Cursor]:
    cnxn = connect()
    cur = cnxn.cursor()
...
def test_text(cursor: npyodbc.Cursor):
    _test_vartype(cursor, 'text')


def test_varchar(cursor: npyodbc.Cursor):
    _test_vartype(cursor, 'varchar')


def test_nvarchar(cursor: npyodbc.Cursor):
    _test_vartype(cursor, 'nvarchar')


def test_varbinary(cursor: npyodbc.Cursor):
    _test_vartype(cursor, 'varbinary')


@pytest.mark.skipif(SQLSERVER_YEAR < 2005, reason='(max) not supported until 2005')
def test_unicode_longmax(cursor: npyodbc.Cursor):
...
def test_char(cursor: npyodbc.Cursor):
    value = "testing"
...
def test_int(cursor: npyodbc.Cursor):
    _test_scalar(cursor, 'int', [None, -1, 0, 1, 12345678])


def test_bigint(cursor: npyodbc.Cursor):
    _test_scalar(cursor, 'bigint', [None, -1, 0, 1, 0x123456789, 0x7FFFFFFF, 0xFFFFFFFF,
                                    0x123456789])


def test_overflow_int(cursor: npyodbc.Cursor):
...
def test_datasources():
    p = npyodbc.dataSources()
    assert isinstance(p, dict)


def test_getinfo_string():
    cnxn = connect()
    value = cnxn.getinfo(npyodbc.SQL_CATALOG_NAME_SEPARATOR)
    assert isinstance(value, str)


def test_getinfo_bool():
    cnxn = connect()
    value = cnxn.getinfo(npyodbc.SQL_ACCESSIBLE_TABLES)
    assert isinstance(value, bool)


def test_getinfo_int():
    cnxn = connect()
    value = cnxn.getinfo(npyodbc.SQL_DEFAULT_TXN_ISOLATION)
    assert isinstance(value, int)


def test_getinfo_smallint():
    cnxn = connect()
    value = cnxn.getinfo(npyodbc.SQL_CONCAT_NULL_BEHAVIOR)
    assert isinstance(value, int)


def test_no_fetch(cursor: npyodbc.Cursor):
...
def test_decode_meta(cursor: npyodbc.Cursor):
    """
...
def test_exc_integrity(cursor: npyodbc.Cursor):
    ...
    with pytest.raises(npyodbc.IntegrityError):
        cursor.execute("insert into t1 values ('one')")


def test_multiple_bindings(cursor: npyodbc.Cursor):
...
def test_different_bindings(cursor: npyodbc.Cursor):
    cursor.execute("create table t1(n int)")
...
def _test_vartype(cursor: npyodbc.Cursor, datatype):
...
def _test_scalar(cursor: npyodbc.Cursor, datatype, values):
    """
...
def test_noscan(cursor: npyodbc.Cursor):
    assert cursor.noscan is False
    cursor.noscan = True
    assert cursor.noscan is True


def test_nonnative_uuid(cursor: npyodbc.Cursor):
    ...
    npyodbc.native_uuid = False
    result = cursor.execute("select n from t1").fetchval()
    assert isinstance(result, str)
    assert result == str(value).upper()
    npyodbc.native_uuid = True


def test_native_uuid(cursor: npyodbc.Cursor):
...
def test_nextset(cursor: npyodbc.Cursor):
...
def test_nextset_with_raiserror(cursor: npyodbc.Cursor):
...
def test_fixed_unicode(cursor: npyodbc.Cursor):
    value = "t\xebsting"
...
def test_chinese(cursor: npyodbc.Cursor):
    v = '我的'
...
def test_bit(cursor: npyodbc.Cursor):
    value = True
...
def test_decimal(cursor: npyodbc.Cursor):
...
def test_decimal_e(cursor: npyodbc.Cursor):
...
def test_subquery_params(cursor: npyodbc.Cursor):
...
def test_empty_string(cursor: npyodbc.Cursor):
...
def test_empty_string_encoding():
    cnxn = connect()
    cnxn.setdecoding(npyodbc.SQL_CHAR, encoding='shift_jis')
    value = ""
    cursor = cnxn.cursor()
    cursor.execute("create table t1(s varchar(20))")
    cursor.execute("insert into t1 values(?)", value)
...
def test_fixed_str(cursor: npyodbc.Cursor):
    value = "testing"
    cursor.execute("create table t1(s char(7))")
    cursor.execute("insert into t1 values(?)", value)
    v = cursor.execute("select * from t1").fetchone()[0]
...
def test_empty_unicode(cursor: npyodbc.Cursor):
...
def test_negative_row_index(cursor: npyodbc.Cursor):
    cursor.execute("create table t1(s varchar(20))")
    cursor.execute("insert into t1 values(?)", "1")
...
def test_time(cursor: npyodbc.Cursor):
...
def test_datetime(cursor: npyodbc.Cursor):
...
def test_datetime_fraction(cursor: npyodbc.Cursor):
...
def test_datetime_fraction_rounded(cursor: npyodbc.Cursor):
    # SQL Server supports milliseconds, but Python's datetime supports nanoseconds.  npyodbc
...
def test_datetime2(cursor: npyodbc.Cursor):
    value = datetime(2007, 1, 15, 3, 4, 5)
...
def test_sp_results(cursor: npyodbc.Cursor):
    cursor.execute(
        """
...
def test_sp_results_from_temp(cursor: npyodbc.Cursor):
...
def test_sp_results_from_vartbl(cursor: npyodbc.Cursor):
    cursor.execute(
        """
        Create procedure proc1
        AS
...
def test_sp_with_dates(cursor: npyodbc.Cursor):
...
def test_sp_with_none(cursor: npyodbc.Cursor):
...
def test_rowcount_delete(cursor: npyodbc.Cursor):
...
def test_rowcount_nodata(cursor: npyodbc.Cursor):
    """
...
def test_rowcount_select(cursor: npyodbc.Cursor):
    """
    Ensure Cursor.rowcount is set properly after a select statement.

    npyodbc calls SQLRowCount after each execute and sets Cursor.rowcount, but SQL Server 2005
...
def test_rowcount_reset(cursor: npyodbc.Cursor):
...
def test_retcursor_delete(cursor: npyodbc.Cursor):
    cursor.execute("create table t1(i int)")
    cursor.execute("insert into t1 values (1)")
...
def test_retcursor_nodata(cursor: npyodbc.Cursor):
    """
    This represents a different code path than a delete that deleted something.
...
def test_retcursor_select(cursor: npyodbc.Cursor):
    cursor.execute("create table t1(i int)")
    cursor.execute("insert into t1 values (1)")
...
def table_with_spaces(cursor: npyodbc.Cursor):
...
def test_lower_case():
    "Ensure npyodbc.lowercase forces returned column names to lowercase."
    try:
        npyodbc.lowercase = True
        cnxn = connect()
        cursor = cnxn.cursor()
        ...
        npyodbc.lowercase = False


def test_row_description(cursor: npyodbc.Cursor):
    """
...
def test_temp_select(cursor: npyodbc.Cursor):
...
def test_executemany(cursor: npyodbc.Cursor):
...
def test_executemany_one(cursor: npyodbc.Cursor):
...
def test_executemany_dae_0(cursor: npyodbc.Cursor):
    """
...
def test_executemany_failure(cursor: npyodbc.Cursor):
    """
    ...
    with pytest.raises(npyodbc.Error):
        cursor.executemany("insert into t1(a, b) value (?, ?)", params)


def test_row_slicing(cursor: npyodbc.Cursor):
...
def test_row_repr(cursor: npyodbc.Cursor):
...
def test_concatenation(cursor: npyodbc.Cursor):
    v2 = '0123456789' * 30
    v3 = '9876543210' * 30
...
def test_view_select(cursor: npyodbc.Cursor):
...
def test_sqlserver_callproc(cursor: npyodbc.Cursor):
    try:
        cursor.execute("drop procedure npyodbctest")
        cursor.commit()
    except:
        pass
    ...
    cursor.execute("""
                    create procedure npyodbctest @var1 varchar(32)
                    as
                    begin
                      select s from t1
                    return
    ...
    cursor.execute("exec npyodbctest 'hi'")


def test_skip(cursor: npyodbc.Cursor):
...
def test_sets_execute(cursor: npyodbc.Cursor):
...
def test_row_execute(cursor: npyodbc.Cursor):
...
def test_row_executemany(cursor: npyodbc.Cursor):
...
def test_description(cursor: npyodbc.Cursor):
...
def test_cursor_messages_with_print(cursor: npyodbc.Cursor):
    """
...
def test_cursor_messages_with_fast_executemany(cursor: npyodbc.Cursor):
    """
...
def test_cursor_messages_with_stored_proc(cursor: npyodbc.Cursor):
    """
...
def test_none_param(cursor: npyodbc.Cursor):
    ...
    # for later columns.
    #
    # If SQLDescribeParam doesn't work, npyodbc would use VARCHAR which almost always worked.
    ...
    except npyodbc.DataError:
        if IS_FREEDTS:
            # cnxn.getinfo(npyodbc.SQL_DESCRIBE_PARAMETER) returns False for FreeTDS, so npyodbc
            ...
            cursor.setinputsizes([(), (npyodbc.SQL_VARBINARY, None, None)])
            cursor.execute(sql, 2, None)
        else:
            raise
    row = cursor.execute("select * from t1").fetchone()
...
def test_output_conversion():
    ...
    cnxn.add_output_converter(npyodbc.SQL_VARCHAR, convert1)
    ...
    cnxn.remove_output_converter(npyodbc.SQL_VARCHAR)
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == '123.45'
    ...
    cnxn.add_output_converter(npyodbc.SQL_VARCHAR, None)
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == '123.45'
    ...
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == 'X123.45X'
    prev_converter = cnxn.get_output_converter(npyodbc.SQL_VARCHAR)
    assert prev_converter is not None
    cnxn.add_output_converter(npyodbc.SQL_VARCHAR, convert2)
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == 'Y123.45Y'
    cnxn.add_output_converter(npyodbc.SQL_VARCHAR, prev_converter)
    value = cursor.execute("select v from t1").fetchone()[0]
    assert value == 'X123.45X'
    #
...
def test_too_large(cursor: npyodbc.Cursor):
...
def test_row_equal(cursor: npyodbc.Cursor):
...
def test_row_gtlt(cursor: npyodbc.Cursor):
    cursor.execute("create table t1(n int, s varchar(20))")
...
def test_context_manager_failure(cursor: npyodbc.Cursor):
...
def test_columns(cursor: npyodbc.Cursor):
...
def test_cancel(cursor: npyodbc.Cursor):
...
def test_emoticons_as_parameter(cursor: npyodbc.Cursor):
    # https://github.com/mkleehammer/pyodbc/issues/423
    #
    # When sending a varchar parameter, npyodbc is supposed to set ColumnSize to the number
...
def test_emoticons_as_literal(cursor: npyodbc.Cursor):
...
def _test_tvp(cursor: npyodbc.Cursor, diff_schema):
    ...
    #
    # Apparently the way npyodbc knows something is a TVP is because it is in a sequence.  I'm
    # not sure I like that as it is very generic and specific to SQL Server.  It would be wiser
    # to define a wrapper npyodbc.TVP or npyodbc.Table object, similar to the DB APIs `Binary`
    # object.
...
def test_tvp(cursor: npyodbc.Cursor):
    _test_tvp(cursor, False)


@pytest.mark.skipif(IS_FREEDTS, reason='FreeTDS does not support TVP')
def test_tvp_diffschema(cursor: npyodbc.Cursor):
    _test_tvp(cursor, True)


def get_sqlserver_version(cursor: npyodbc.Cursor):
