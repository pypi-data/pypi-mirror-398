...
class SqlServerTestCase(unittest.TestCase):
    ...
    def driver_type_is(self, type_name):
        ...
        driver_name = self.cnxn.getinfo(npyodbc.SQL_DRIVER_NAME).lower()
    ...
    def setUp(self):
        self.cnxn   = npyodbc.connect(self.connection_string)
        ...
        self.cnxn.setdecoding(npyodbc.SQL_CHAR, 'latin1')
    ...
    def test_getinfo_string(self):
        value = self.cnxn.getinfo(npyodbc.SQL_CATALOG_NAME_SEPARATOR)
        self.assertTrue(isinstance(value, str))

    def test_getinfo_bool(self):
        value = self.cnxn.getinfo(npyodbc.SQL_ACCESSIBLE_TABLES)
        self.assertTrue(isinstance(value, bool))

    def test_getinfo_int(self):
        value = self.cnxn.getinfo(npyodbc.SQL_DEFAULT_TXN_ISOLATION)
        self.assertTrue(isinstance(value, (int, int)))

    def test_getinfo_smallint(self):
        value = self.cnxn.getinfo(npyodbc.SQL_CONCAT_NULL_BEHAVIOR)
    ...
    def test_nextset_with_raiserror(self):
        ...
        if self.driver_type_is('freetds'):
            ...
            return  # for now
        self.assertRaises(npyodbc.ProgrammingError, self.cursor.nextset)
    ...
    def _test_strtype(self, sqltype, value, resulttype=None, colsize=None):
        ...
        if colsize >= 2000 and (sqltype == 'nvarchar' or sqltype == 'varchar'):
            self.cursor.setinputsizes([(npyodbc.SQL_WVARCHAR, 0, 0)])
        ...
        try:
            ...
            if self.driver_type_is('freetds'):
                # FREETDS_KNOWN_ISSUE
                #
                # cnxn.getinfo(npyodbc.SQL_DESCRIBE_PARAMETER) returns False for FreeTDS, so
                # npyodbc can't call SQLDescribeParam to get the correct parameter type.
                ...
                if sqltype == 'varbinary':
                    sql_param_type = npyodbc.SQL_VARBINARY
    ...
    def test_fast_executemany_to_local_temp_table(self):
        ...
        params = [(v,)]
        self.cursor.setinputsizes([(npyodbc.SQL_WVARCHAR, 50, 0)])
    ...
    def test_close_cnxn(self):
        ...
        self.assertRaises(npyodbc.ProgrammingError, self._exec)
    ...
    def test_empty_string_encoding(self):
        self.cnxn.setdecoding(npyodbc.SQL_CHAR, encoding='shift_jis')
        value = ""
        self.cursor.execute("create table t1(s varchar(20))")
        self.cursor.execute("insert into t1 values(?)", value)
        v = self.cursor.execute("select * from t1").fetchone()[0]
    ...
    def test_version(self):
        self.assertEqual(3, len(npyodbc.version.split('.'))) # 1.3.1 etc.
    ...
    def test_datetime_fraction_rounded(self):
        # SQL Server supports milliseconds, but Python's datetime supports nanoseconds.  npyodbc rounds down to what the
        # database supports.
    ...
    def test_rowcount_select(self):
        ...
        npyodbc calls SQLRowCount after each execute and sets Cursor.rowcount, but SQL Server 2005 returns -1 after a
    ...
    def test_executemany_failure(self):
        ...
        self.assertRaises(npyodbc.Error, self.cursor.executemany, "insert into t1(a, b) value (?, ?)", params)
    ...
    def test_autocommit(self):
        ...
        othercnxn = npyodbc.connect(self.connection_string, autocommit=True)
    ...
    def test_sets_execute(self):
        ...
        self.assertRaises(npyodbc.ProgrammingError, f)
    ...
    def test_none_param(self):
        ...
        #
        # If SQLDescribeParam doesn't work, npyodbc would use VARCHAR which almost always worked.  However,
        ...
        try:
            self.cursor.setinputsizes([(), (npyodbc.SQL_VARBINARY, None, None)])
    ...
    def test_output_conversion(self):
        ...
        self.cnxn.add_output_converter(npyodbc.SQL_VARCHAR, convert)
        ...
        self.cnxn.remove_output_converter(npyodbc.SQL_VARCHAR)
        value = self.cursor.execute("select v from t1").fetchone()[0]
        self.assertEqual(value, '123.45')
        ...
        self.cnxn.add_output_converter(npyodbc.SQL_VARCHAR, None)
        value = self.cursor.execute("select v from t1").fetchone()[0]
        self.assertEqual(value, '123.45')
    ...
    def test_context_manager_success(self):
        ...
        with npyodbc.connect(self.connection_string) as cnxn:
            cursor = cnxn.cursor()
    ...
    def test_context_manager_failure(self):
        ...
        self.assertRaises(npyodbc.Error, _fail)
    ...
    def test_large_update_nodata(self):
        ...
        self.cursor.setinputsizes([(npyodbc.SQL_VARBINARY,0,0)])
...
def main():
    ...
    if args.verbose:
        cnxn = npyodbc.connect(connection_string)
