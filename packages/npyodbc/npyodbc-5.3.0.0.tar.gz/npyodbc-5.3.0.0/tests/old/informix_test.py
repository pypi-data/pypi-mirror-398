...
class InformixTestCase(unittest.TestCase):
    ...
    def test_unicode_results(self):
        ...
        othercnxn = npyodbc.connect(self.connection_string, unicode_results=True)
    ...
    def test_informix_callproc(self):
        try:
            self.cursor.execute("drop procedure npyodbctest")
            self.cnxn.commit()
        except:
            pass
        ...
        # for row in self.cursor.procedureColumns('npyodbctest'):
        #     print row.procedure_name, row.column_name, row.column_type, row.type_name

        self.cursor.execute("exec npyodbctest 'hi'")
    ...
    def test_context_manager(self):
        with npyodbc.connect(self.connection_string) as cnxn:
            cnxn.getinfo(npyodbc.SQL_DEFAULT_TXN_ISOLATION)

        # The connection should be closed now.
        def test():
            cnxn.getinfo(npyodbc.SQL_DEFAULT_TXN_ISOLATION)
        self.assertRaises(npyodbc.ProgrammingError, test)
