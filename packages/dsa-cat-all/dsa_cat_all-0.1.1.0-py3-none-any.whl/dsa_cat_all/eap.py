...
def fis(
    ...
    if len(from_system) != len(l) or len(in_system) != len(l):
        raise ValueError(get_translate("Длина from_system и in_system должна совпадать с l"))
    for fs in from_system:
        if fs < 2 or fs > len(alf):
            raise ValueError(get_translate("Некорректная from_system"))
    for ts in in_system:
        if ts < 2 or ts > len(alf2):
            raise ValueError(get_translate("Некорректная in_system"))
    decimals = []
    for num, fs in zip(l, from_system):
        value = 0
        for power, ch in enumerate(reversed(num)):
            if ch not in alf or alf.index(ch) >= fs:
                raise ValueError(get_translate("Символ не принадлежит алфавиту системы"))
