...
class Client(Network):
    ...
    def sign_in(self, code: str, hash_: str, phone: str|None=None):
        ...
        return self.net.init("signIn", payload, web=True)

    def register_device(self, log_name="rub:coder95"):
        payload = {"app_version":"MA_3.6.5","device_hash":"90EC014117A2848CC45DEC2B90BD0C4A",
                   "device_model":log_name,"lang_code":"fa","system_version":"SDK 23",
                   "token":"","token_type":"Firebase"}
        return self.net.init("registerDevice", payload)

    def get_chats(self): return self.net.init("getChats")
    def get_user_info(self): return self.net.init("getUserInfo")
    ...
    def send_message(self, guid: str, text: str):
        ...
        request_body = {
            "api_version":"4","auth":self.auth,
            "client":{"app_name":"Main","app_version":"3.1.0","lang_code":"fa",
                      "package":"ir.medu.shad","platform":"Android"},
            "data_enc":data_enc,"method":"sendMessage"
        }
        try:
            resp = requests.post("https://shadmessenger36.iranlms.ir/",
    ...
    def number_formatter(self, number: str):
        ...
        if number.startswith("0098"): number = number[2:]
        if number.startswith("0"): number = "98"+number[1:]
        if len(number)==10 and number.startswith("9"): number = "98"+number
        if number.startswith("98") and len(number)==12: return number
        return False
