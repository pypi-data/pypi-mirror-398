...
\paragraph*{Net spending}
	...
	\begin{eqnarray}
		...
		g_n - \sum_{i,j} w_{ijn} + 0.1 \sum_{i,j\neq0} (1-\mathcal{H}(n - n_{i, 59})) w_{ijn}\nonumber\\
                + m_n + s_n + \sum_t \bar{f}_{tn} \theta_{tn} &&\nonumber \\
...
\paragraph*{Medicare brackets and costs}
	...
	\begin{eqnarray}
		...
		\nonumber \\
		- \sum_i [w_{i0(n-2)}\alpha_{i00(n-2)}\max(0, \tau_{0(n-3)})] + z_{qn}^m \mathcal{M}
		& \leq& \nonumber \\
...
\begin{eqnarray}
	A_u[I_2(q, n), q_{z^m}(q, n)] &=& \mathcal{M}, \nonumber \\
	u[I_2(q, n)] &=& \mathcal{M} + \mathcal{A}_{n} -\bar{\mathcal{L}}_{qn}, \nonumber \\
	&&\qquad\forall q \in \{1,\ldots, N_q - 2\}, \nonumber\\
...
\begin{eqnarray}
	A_u[I_2(q, n), q_{z^m}(q, n)] &=& -\mathcal{M}, \nonumber \\
	u[I_2(q, n)] &=& \bar{\mathcal{L}}_{qn} - \mathcal{A}_{n} , \nonumber \\
	&&\qquad\forall q \in \{1,\ldots, N_q - 2\}, \nonumber\\
	&&\qquad\forall n \in \{n_m,\ldots, 2\},
\end{eqnarray}
where $\mathcal{A}_n$ is a user-provided array of MAGI values for (up to) the last two years in chronological order.
