...
class _IRISObject(object):
    ...
    def get(self, propertyName):
        ...
        """
        if self._closed:
            raise RuntimeError("IRISObject is closed")
    ...
    def getIRISList(self, propertyName):
        ...
        if self._closed:
            raise RuntimeError("IRISObject is closed")
        return self._iris._execute(
            intersystems_iris.IRISList, intersystems_iris.IRIS.GET_PROPERTY, self, propertyName, None
        )
    ...
    def set(self, propertyName, propertyValue):
        ...
        if self._closed:
            raise RuntimeError("IRISObject is closed")
        self._iris._execute(
            None, intersystems_iris.IRIS.SET_PROPERTY, self, propertyName, None, propertyValue, honorByReference=False
        )
        return
    ...
    def invokeIRISList(self, methodName, *args):
        ...
        if self._closed:
            raise RuntimeError("IRISObject is closed")
        return self._iris._execute(
            intersystems_iris.IRISList, intersystems_iris.IRIS.VALUE_METHOD, self, methodName, args
        )
