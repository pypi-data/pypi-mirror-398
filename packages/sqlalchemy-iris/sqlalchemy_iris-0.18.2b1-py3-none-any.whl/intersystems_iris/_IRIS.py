...
class _IRIS(object):
    ...
    def __init__(self, connection):
        if connection is None or connection.isClosed():
    ...
    def get(self, globalName, *subscripts):
        ...
        return self._execute(
            object,
            intersystems_iris.IRIS.GET_NODE,
            globalName,
            None,
            subscripts,
            mode=intersystems_iris.IRIS.MODE_GLOBAL,
        )
    ...
    def getBoolean(self, globalName, *subscripts):
        ...
        """
        return self._execute(
            bool, intersystems_iris.IRIS.GET_NODE, globalName, None, subscripts, mode=intersystems_iris.IRIS.MODE_GLOBAL
        )
    ...
    def getInteger(self, globalName, *subscripts):
        ...
        """
        return self._execute(
            int, intersystems_iris.IRIS.GET_NODE, globalName, None, subscripts, mode=intersystems_iris.IRIS.MODE_GLOBAL
        )
    ...
    def getString(self, globalName, *subscripts):
        ...
        """
        return self._execute(
            str, intersystems_iris.IRIS.GET_NODE, globalName, None, subscripts, mode=intersystems_iris.IRIS.MODE_GLOBAL
        )
    ...
    def set(self, value, globalName, *subscripts):
        ...
        self._execute(
            None,
            intersystems_iris.IRIS.SET_NODE,
            globalName,
            None,
            subscripts,
            value,
    ...
    def kill(self, globalName, *subscripts):
        ...
        self._execute(
            None,
            intersystems_iris.IRIS.KILL_NODE,
            globalName,
            None,
            subscripts,
            mode=intersystems_iris.IRIS.MODE_GLOBAL,
    ...
    def increment(self, value, globalName, *subscripts):
        ...
        return self._execute(
            object,
            intersystems_iris.IRIS.INCREMENT,
            globalName,
            None,
            subscripts,
            value,
    ...
    def isDefined(self, globalName, *subscripts):
        ...
        return self._execute(
            int,
            intersystems_iris.IRIS.DATA,
            globalName,
            None,
            subscripts,
            intersystems_iris.IRIS.FLAG_EMPTY,
            mode=intersystems_iris.IRIS.MODE_GLOBAL,
        )
    ...
    def nextSubscript(self, reversed, globalName, *subscripts):
        ...
        """
        bitflags = intersystems_iris.IRIS.FLAG_SUBSCRIPT + (
            intersystems_iris.IRIS.FLAG_REVERSE if reversed else intersystems_iris.IRIS.FLAG_EMPTY
        )
        subscript = self._execute(
            str,
            intersystems_iris.IRIS.ORDER,
            globalName,
            None,
            subscripts,
            bitflags,
    ...
    def _nextNode(self, reversed, globalName, *subscripts):
        bitflags = (
            intersystems_iris.IRIS.FLAG_SUBSCRIPT
            + intersystems_iris.IRIS.FLAG_VALUE
            + (intersystems_iris.IRIS.FLAG_REVERSE if reversed else intersystems_iris.IRIS.FLAG_EMPTY)
        )
        return self._execute(
            tuple,
    ...
    def _nextNodeWithStop(self, reversed, stop_value, globalName, *subscripts):
        bitflags = (
            ...
            + intersystems_iris.IRIS.FLAG_VALUE
            + (intersystems_iris.IRIS.FLAG_REVERSE if reversed else intersystems_iris.IRIS.FLAG_EMPTY)
            + intersystems_iris.IRIS.FLAG_STOPON
        )
        return self._execute(
            tuple,
            intersystems_iris.IRIS.ORDER,
    ...
    def _sortOrder(self, subscript1, subscript2, globalName, *subscripts):
        return self._execute(
            int,
            intersystems_iris.IRIS.VALUE_CLASSMETHOD,
            "%Net.Remote.Gateway",
            "%SubscriptSortOrder",
            [globalName, subscript1, subscript2],
        )
    ...
    def releaseAllLocks(self):
        ...
        """
        self._execute(
            None, intersystems_iris.IRIS.UNLOCK_ALL, None, None, None, mode=intersystems_iris.IRIS.MODE_GLOBAL
        )
        return
    ...
    def tRollbackOne(self):
        ...
        """
        self._execute(
            None, intersystems_iris.IRIS.TROLLBACK_ONE, None, None, None, mode=intersystems_iris.IRIS.MODE_GLOBAL
        )
        return
    ...
    def getTLevel(self):
        ...
        """
        return self._execute(
            int, intersystems_iris.IRIS.GET_TLEVEL, None, None, None, mode=intersystems_iris.IRIS.MODE_GLOBAL
        )
    ...
    def functionIRISList(self, functionName, routineName, *args):
        ...
        """
        return self._execute(
            intersystems_iris.IRISList, intersystems_iris.IRIS.VALUE_FUNCTION, functionName, routineName, args
        )
    ...
    def classMethodIRISList(self, className, methodName, *args):
        ...
        """
        return self._execute(
            intersystems_iris.IRISList, intersystems_iris.IRIS.VALUE_CLASSMETHOD, className, methodName, args
        )
        ...
        *post_values,
        ...
        with self._connection._lock:
            ...
            if return_type is None:
                ...
                returned_value = (
                    self._unmarshal_return_value(
                        str, mode, self._locale, self._is_unicode, self._connection.compact_double
                    ),
                    self._unmarshal_return_value(
                        object, mode, self._locale, self._is_unicode, self._connection.compact_double
                    ),
                )
            else:
                returned_value = self._unmarshal_return_value(
                    return_type, mode, self._locale, self._is_unicode, self._connection.compact_double
                )
        if honorByReference and args is not None:
    ...
    def _marshal_one_parameter(self, arg, *, mode, honorByReference):
        is_reference = honorByReference and isinstance(arg, intersystems_iris.IRISReference)
        if is_reference:
            arg = arg.get_value()
        ...
        if intersystems_iris._PythonGateway._PythonGateway._is_datatype(type(arg)):
        ...
        elif mode == intersystems_iris.IRIS.MODE_GLOBAL and isinstance(arg, bytearray):
            self._out_message.wire._set(arg)
        elif isinstance(arg, intersystems_iris.IRISList):
            if not self._connection.compact_double and arg.compact_double:
                raise ValueError(
                    "Cannot store an IRISList with Compact Double enabled on a server with Compact Double disabled"
                )
            ...
            if oref is None:
                oref = self._connection._map_local_object_to_oref(
                    self._in_message_secondary, self._out_message_secondary, arg
                )
            if oref is not None:
                self._out_message.wire._set(intersystems_iris._IRISOREF._IRISOREF(oref))
            else:
                raise intersystems_iris._GatewayException._GatewayException("Unable to map object: " + arg)
        if is_reference:
    ...
    def _unmarshal_return_value(self, return_type, mode, locale, is_unicode, compact_double):
        ...
        if return_type == bool:
            ...
            if value is None:
                return (
                    intersystems_iris.IRISList(None, locale, is_unicode, compact_double)
                    if mode == intersystems_iris.IRIS.MODE_GLOBAL
                    else None
                )
            return intersystems_iris.IRISList(
                intersystems_iris.IRIS._convertToBytes(value, mode, locale, is_unicode),
                locale,
                is_unicode,
                compact_double,
            )
    ...
    def _process_pass_by_reference(self, *args):
        for arg in args:
            if isinstance(arg, intersystems_iris.IRISReference):
                value = self._unmarshal_return_value(
                    object,
                    intersystems_iris.IRIS.MODE_RUNTIME,
                    self._locale,
                    self._is_unicode,
                    self._connection.compact_double,
                )
                arg._value = value
    ...
    def __parse_iris_number(value, return_type):
        ...
        if (
            value[0:3].lower() == "nan"
            or value[0:4].lower() == "-nan"
            or value[0:4].lower() == "snan"
            or value[0:5].lower() == "-snan"
        ):
    ...
    def _remove_scientific_notation(value):
        ...
        if exponent > 0:
            if "." in number:
                ...
                number = (
                    number[0:index] + number[index + 1 : index + exponent + 1] + "." + number[index + exponent + 1 :]
                )
                if number.endswith("."):
                    number = number[:-1]
            else:
    ...
    def _release_closed_iris_object(self, force=False):
        if (
            not force
            and len(self._connection._iris_object_proxy_closed)
            <= intersystems_iris.IRISConnection.CLOSED_PROXY_UPDATE_THRESHOLD
        ):
            return
        closed_iris_objects = self._connection._get_closed_iris_objects()
        closed_orefs = self._execute(
            str,
            intersystems_iris.IRIS.VALUE_CLASSMETHOD,
            "%Net.Remote.Gateway",
            "%ReleaseObjects",
