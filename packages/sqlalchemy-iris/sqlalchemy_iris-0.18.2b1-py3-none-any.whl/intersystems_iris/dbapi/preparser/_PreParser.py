...
class _PreParser(object):
    ...
    def PreParse(self, query, p_Parameters):
        new_input_params = p_Parameters._input_params
        t_query = query
        while True:
            #  First tokenize the input
            self.Tokenize(t_query)
            #  Convert WITH Clause, can be recursive
            found_with, t_query, t_params = self.With(t_query)
            if found_with and t_params and p_Parameters._input_params:
                new_input_params = [p_Parameters._input_params[i] for i in t_params]
                p_Parameters.set_input_params(new_input_params)
            if not found_with:
                break
        ...
        return self.Resolve(t_query, p_Parameters), new_input_params
        ...
        try:
            ...
            def _query(find_end_paren=True):
                ...
                while tokens.MoveNext():
                    ...
                    if token.TokenType is TOKEN.QUESTION_MARK:
                        params.append(param_counter)
                        param_counter += 1
                    ...
                    if token.TokenType is TOKEN.ID and (token.UpperEquals("FROM") or token.UpperEquals("JOIN")):
                        ...
                        if table_name_upper in with_statements:
                            new_query, new_params = with_statements[table_name_upper]
                            sub_query += new_query
                            params += new_params
                            sub_query += " AS "
            ...
            while tokens.MoveNext():
                token = tokens.Current()
                if token.TokenType is TOKEN.QUESTION_MARK:
                    new_params.append(param_counter)
                    param_counter += 1
            ...
            while True:
                ...
                assert (
                    tokens.MoveNext()
                    and tokens.Current().TokenType is TOKEN.ID
                    and tokens.Current().UpperContains("AS")
                )
            ...
            tokens.MovePrevious()
            sub_query, sub_params = _query(False)
            new_query += sub_query
            new_params += sub_params

            return found, new_query, new_params
        except Exception:
            return False, query, None
    ...
    def InsertReturning(self, query):
        ...
        tokens = self.m_Tokens.GetEnumerator()
        keywords = (
            list(self.s_StatementTable.keys())
            + list(self.s_ParsedStatements.keys())
            + list(self.s_TransactionStatements.keys())
        )
    ...
    def Tokenize(self, p_strInput):
        ...
        tokenize_switcher = {
            ...
            ParseToken.tokLESS: functools.partial(
                self.Tokenize_op, check_tokens=[ParseToken.tokEQUAL, ParseToken.tokGREAT]
            ),
    ...
    def Tokenize_default(self, token=TOKEN.UNKNOWN):
        ...
        self.m_Tokens.Append(
            intersystems_iris.dbapi.preparser._Token._Token(
                token, self.m_Scanner.EndLexeme(), self.m_Scanner.EndUpperLexeme()
            )
        )
    ...
    def Tokenize_plus(self, op_char="+"):
        ...
        if t_eToken in [TOKEN.OP, TOKEN.OPEN_PAREN, TOKEN.COMMA] and (
            self.m_Scanner.PeekNextToken == ParseToken.tokDIGIT
            or (
                self.m_Scanner.PeekNextToken() == ParseToken.tokDOT
                and self.m_Scanner.PeekNextNextToken() == ParseToken.tokDIGIT
            )
        ):
            #  Scan in number
            (t_strNumber, goodParse) = self.m_Scanner.Number()
    ...
    def Tokenize_slash(self):
        if self.m_Scanner.PeekNextToken() == ParseToken.tokASTER:
            ...
            self.m_Tokens.Append(
                intersystems_iris.dbapi.preparser._Token._Token(
                    TOKEN.UNKNOWN, self.m_Scanner.EndLexeme(), self.m_Scanner.EndUpperLexeme()
                )
            )
        else:
    ...
    def Tokenize_vbar(self):
        ...
        self.m_Tokens.Append(
            intersystems_iris.dbapi.preparser._Token._Token(
                t_eToken, self.m_Scanner.EndLexeme(), self.m_Scanner.EndUpperLexeme()
            )
        )
    ...
    def Resolve(self, p_strInput, p_Parameters):
                        ...
                        if t_str == "EXPLAIN" and pOut.p_eStmtType == StatementType.CALLWITHRESULT:
                            ...
                            while t_Enum.MoveNext():
                                if t_Enum.Current().UpperLexeme == "ALT":
                                    ...
                                    p_Parameters._clear()
                                    p_Parameters._params_list.append(
                                        intersystems_iris.dbapi._Parameter._Parameter(
                                            pQuery, ParameterMode.REPLACED_LITERAL
                                        )
                                    )
                                    p_Parameters._params_list.append(
                                        intersystems_iris.dbapi._Parameter._Parameter(
                                            pStat, ParameterMode.REPLACED_LITERAL
                                        )
                                    )
                                    p_Parameters._params_list.append(
                                        intersystems_iris.dbapi._Parameter._Parameter(
                                            pAlt, ParameterMode.REPLACED_LITERAL
                                        )
                                    )
                                    p_Parameters._params_list.append(
                                        intersystems_iris.dbapi._Parameter._Parameter(
                                            "", ParameterMode.REPLACED_LITERAL
                                        )
                                    )
        ...
        if length - self.m_ExecParamCount > 0:
            ...
            while t_Enum.MoveNext():
                if (
                    TOKEN.QUESTION_MARK == t_Enum.Current().TokenTypeGet()
                    or TOKEN.ATSIGN == t_Enum.Current().TokenTypeGet()
                ):
    ...
    def Resolve_question_mark(self, p_Parameters, t_Enum, t_Token, t_bBoundParameters):
        ...
        if not t_bBoundParameters:
            p_Parameters._params_list.append(
                intersystems_iris.dbapi._Parameter._Parameter("?", ParameterMode.INPUT, "?")
            )
        return False
    ...
    def Resolve_atsign(self, p_Parameters, t_Enum, t_Token, t_bBoundParameters):
        ...
        if (not t_bBoundParameters) or len(p_Parameters._params_list) == 0:
            p_Parameters._params_list.append(
                intersystems_iris.dbapi._Parameter._Parameter(t_Token.Lexeme, ParameterMode.UNKNOWN)
            )
        else:
            if not self.matchUpParam(p_Parameters, t_Token.Lexeme, len(p_Parameters._params_list)):
                p_Parameters._params_list.append(
                    intersystems_iris.dbapi._Parameter._Parameter(t_Token.Lexeme, ParameterMode.UNKNOWN)
                )
        return False
    ...
    def Resolve_hex(self, p_Parameters, t_Enum, t_Token, t_bBoundParameters):
        self.t_nOrdinal += 1
        cp = intersystems_iris.dbapi._Parameter._Parameter(
            bytes.fromhex(t_Token.Lexeme[2:]),
            ParameterMode.REPLACED_LITERAL,
            "?",
            type=intersystems_iris.dbapi._DBAPI.SQLType.BINARY,
        )
    ...
    def Resolve_id(self, p_Parameters, t_Enum, t_Token, t_bBoundParameters, stmtType):
                    ...
                    if self.t_nOpenParen == 0:
                        ...
                        while t_Enum.MoveNext():
                            if t_Enum.Current().TokenTypeGet() == TOKEN.CLOSE_PAREN:
                                self.t_nOpenParen -= 1
                                break
                            elif (TOKEN.ID == t_Enum.Current().TokenTypeGet()):
                                if (t_Enum.Current().UpperEquals("UNION")
                                    or t_Enum.Current().UpperEquals("LIMIT")
                                    or t_Enum.Current().UpperEquals("OFFSET")):
                                    break
        ...
        if (
            t_Token.UpperEquals("DATEPART")
            or t_Token.UpperEquals("TIMESTAMPADD")
            or t_Token.UpperEquals("TIMESTAMPDIFF")
        ):
            if t_Enum.MoveNext():
                if t_Enum.Current().TokenTypeGet() == TOKEN.OPEN_PAREN:
                    while t_Enum.MoveNext():
        ...
        if t_NewEnum.MoveNext() and t_NewEnum.Current().TokenTypeGet() == TOKEN.CLOSE_PAREN:
            ...
            if t_Enum.Current() is not None and t_NewEnum.Current().TokenTypeGet() == TOKEN.OPEN_PAREN:
                ...
                if t_Enum.Current() is not None:
                    if TOKEN.ID != t_NewEnum.Current().TokenTypeGet() or (
                        t_NewEnum.Current().UpperLexeme in _PreParser.s_replaceparm
                    ):
                        t_Enum.MoveNext()
                        return False
    ...
    def Resolve_null(self, p_Parameters, t_Enum, t_Token, t_bBoundParameters):
        ...
        if t_NewEnum.MovePrevious():
            ...
            if t_PreviousToken.TokenTypeGet() not in [
                TOKEN.NOT,
                TOKEN.IS,
                TOKEN.THEN,
                TOKEN.COMMA,
    ...
    def matchUpParam(self, param, paramName, numParam):
        match = False
        if not self.hasNamedParameters or (paramName is None or paramName == "" or paramName[0] != "@"):
            return False
        for i in range(len(param._params_list)):
            if (param._params_list[i].name.upper() == paramName.upper()) or (
                ("@" + param._params_list[i].name.upper()) == paramName.upper()
            ):
                match = True
                if i != numParam:
                    ...
                    if not cporig.parsermatched:
                        ...
                        if cporig.matchedParameterList is None:
    ...
    def Call(self, pOut, p_Parameters):
        ...
        for i in range(1):
            ...
            if t_Token.TokenTypeGet() == TOKEN.QUESTION_MARK:
                ...
                t_Enum.Current().UpperEquals("CALL")
                or t_Enum.Current().UpperEquals("EXEC")
                or t_Enum.Current().UpperEquals("EXECUTE")
            ):
                return False
            ...
            if returnParam is not None:
                ...
                if not t_bBoundParameters:
                    ...
                    if not self.matchUpParam(p_Parameters, returnParam.GetName(), t_nOrdinal - 1):
            ...
            if not t_bQuitLoop:
                ...
                while True:
                    ...
                    if not t_Enum.MoveNext():
                        break
            pOut.p_eStmtType = StatementType.CALL if (returnParam is None) else StatementType.CALLWITHRESULT
            t_bRet = True
        return t_bRet
    ...
    def Call_atsign(self, p_Parameters, t_Token, t_nOrdinal, t_bBoundParameters):
        self.hasNamedParameters = True
        if (not t_bBoundParameters) or len(p_Parameters._params_list) == 0:
            p_Parameters._params_list.add(
                intersystems_iris.dbapi._Parameter._Parameter(t_Token.Lexeme, ParameterMode.UNKNOWN)
            )
        else:
            if not self.matchUpParam(p_Parameters, t_Token.Lexeme, t_nOrdinal):
                p_Parameters._params_list.add(
                    intersystems_iris.dbapi._Parameter._Parameter(t_Token.Lexeme, ParameterMode.UNKNOWN)
                )
    ...
    def Call_hex(self, p_Parameters, t_Token, t_nOrdinal, t_bBoundParameters):
        cp = intersystems_iris.dbapi._Parameter._Parameter(
            bytes.fromhex(t_Token.Lexeme[2:]), ParameterMode.REPLACED_LITERAL, "?"
        )
        p_Parameters._params_list.append(cp)
        t_Token.TokenTypeSet(TOKEN.QUESTION_MARK)
        return (t_nOrdinal + 1, TOKEN.QUESTION_MARK)
            ...
            if t_PreviousToken.TokenTypeGet() == TOKEN.OP:
                ...
                return (
                    self.DynamicVariable(
                        t_bBoundParameters,
                        intersystems_iris.dbapi.preparser._Token._Token(
                            TOKEN.CONSTANT, t_PreviousToken.Lexeme, t_PreviousToken.UpperLexeme
                        ),
                        t_nOrdinal,
                        p_Parameters,
                    ),
    ...
    def Call_comma_paren(self, p_Parameters, t_Token, t_nOrdinal, t_bBoundParameters, t_eLastToken):
        if TOKEN.COMMA == t_eLastToken or TOKEN.OPEN_PAREN == t_eLastToken:
            t_Parameter = intersystems_iris.dbapi._Parameter._Parameter(
                mode=ParameterMode.DEFAULT_PARAMETER, paramType="d"
            )
            t_nOrdinal += 1
    ...
    def Exec(self, pOut, p_Parameters):
        ...
        for i in range(1):
                ...
                if t_Token.Lexeme[0] == "@":
                    if t_Enum.MoveNext():
                        ...
                        if (not bMoveNext) or (t_Enum.Current().Lexeme != "="):
                            if not bMoveNext:
                                t_bQuitLoop = True
                            t_Param = intersystems_iris.dbapi._Parameter._Parameter(
                                mode=ParameterMode.INPUT_OUTPUT, name=t_Token.Lexeme[1:], execParam=True
                            )
                    ...
                    t_Token = t_Enum.Current()
                    t_Enum.MoveNext()
                if t_Token.TokenTypeGet() not in [
                    TOKEN.OPEN_PAREN,
                    TOKEN.CLOSE_PAREN,
                    TOKEN.QUESTION_MARK,
                    TOKEN.UNKNOWN,
                    ...
                    if t_Token is not None:
                        if t_Token.Lexeme[0] == "-":
                            if not t_Enum.MoveNext():
                                ...
                                t_Token = t_Enum.Current()
                                t_NewToken = intersystems_iris.dbapi.preparser._Token._Token(
                                    TOKEN.CONSTANT, "-" + t_Token.Lexem, "-" + t_Token.UpperLexeme
                                )
            ...
            if t_bHasReturnType:
                ...
                t_ReturnParam = intersystems_iris.dbapi._Parameter._Parameter(
                    mode=ParameterMode.UNKNOWN, execParam=True
                )
                self.m_ExecParamCount += 1
        ...
        if t_c in ["'", '"']:
            ...
            if "e" in t_str or "E" in t_str:
                ...
                if p >= len(t_str):
                    t_str = "0"
        if p_bBoundParameters:
            p_Parameters._params_list.insert(
                p_nOrdinal - 1, intersystems_iris.dbapi._Parameter._Parameter(t_str, ParameterMode.REPLACED_LITERAL)
            )
        else:
            p_Parameters._params_list.append(
                intersystems_iris.dbapi._Parameter._Parameter(t_str, ParameterMode.REPLACED_LITERAL)
            )
        p_Token.Lexeme = "?"
        p_Token.TokenTypeSet(TOKEN.QUESTION_MARK)
        p_Token.m_replaced = True
