...
def connect(
    *args, embedded=False, hostname=None, port=None, namespace=None, username=None, password=None, **kw
) -> Union[_IRISConnection, _IRISEmbedded]:
    try:
        if not embedded:
...
class _BaseCursor:
    ...
    def close(self):
        ...
        self._fetch_done = False
        self._parameter_list_mismatch_exception = False
        if self._output_parameter_list is not None:
    ...
    def _is_alive(self):
        if self._closed:
            raise InterfaceError("Cursor is closed")
        if self._connection is None or self._connection.isClosed():
    ...
    def _preparse(self):
        ...
        try:
            pOut, self.params = parser.PreParse(self.statement, self._params)
            ...
            if self._cursor_type == CursorType.CALLABLE:
                ...
                for param in self._params._params_list:
                    if param.mode == ParameterMode.RETURN_VALUE:
                        ...
                        if len(self._params._params_list) == len(self.params) and self.params[i] is None:
...
class Cursor(_BaseCursor):
    ...
    class prepared_statement(intersystems_iris._IRISConnection.CachedSQL):
        def __init__(self, cursor):
            ...
            if cursor._columns is not None:
                self.columns = []
    ...
    def stored_results(self):
        ...
        # getResultSet()
        if self._multiple_result_sets:
            if self._rsrow is None and self._rowcount == -1:
                return None
            if self._mrs_done:
                return None
            self._get_column_info(self._in_message.wire)
    ...
    def _process_return_values(self):
        ...
        if len(return_args) > 0:
            if any(i is not None for i in return_args):
                return return_args
            else:
                return
        else:
    ...
    def fetchone_helper(self):
        ...
        if self.statementFeatureOption & Feature.optionFastSelect == Feature.optionFastSelect:
            ...
            if self._rsrow.rowItems is None:
                return
    ...
    def fetchone(self):
        ...
        if self._current_wire is None and self._cursor_ptr > self._last_row_in_warehouse_dict:
            return None
