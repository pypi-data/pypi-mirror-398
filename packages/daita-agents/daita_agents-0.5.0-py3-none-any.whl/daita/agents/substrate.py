...
class SubstrateAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initialize SubstrateAgent with smart constructor auto-configuration."""
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            async with self.trace_manager.span(
                ...
                try:
                    ...
                    if attempt > 1:
                        logger.info(f"SubstrateAgent {self.name} succeeded on attempt {attempt}")
                    ...
                    if attempt < max_attempts:
                        ...
                        if should_retry:
                            ...
                            logger.debug(f"SubstrateAgent {self.name} retrying in {delay:.2f}s")
                            await asyncio.sleep(delay)
                            continue

                    # Don't retry or no more attempts
                    logger.debug(f"SubstrateAgent {self.name} not retrying: {type(e).__name__}")
                    raise
        ...
        **kwargs
        ...
        if self.llm is None:
            ...
            raise AgentError(
                ...
                f"or pass api_key parameter to SubstrateAgent."
            )
