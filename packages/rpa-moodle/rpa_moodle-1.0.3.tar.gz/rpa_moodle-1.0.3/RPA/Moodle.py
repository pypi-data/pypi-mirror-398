...
class MoodleLibrary:
    ...
    def setup_moodle_connection(self, token_file_path: str = None):
        ...
        if 'access_token' in data and 'refresh_token' in data:
            ...
            base_url = data.get('refresh_token')
        else:
    ...
    def get_drive_file_id_from_url(self, url: str) -> Optional[str]:
        """
        Trích xuất Google Drive/Sheets file ID từ URL.
    ...
    def download_file_from_google_drive(self, file_id: str, output_path: str) -> str:
        ...
        Supports both regular files and Google Sheets (auto-export to Excel).
        ...
        for student in students:
            ...
            try:
                ...
                if "already" in str(e).lower() or "exists" in str(e).lower():
                    # User exists, still add to enrollment list
                    all_usernames.append(username)
                    self.logger.info(f"User already exists: {username}, will be enrolled")
                else:
                    failed.append({
                        'username': username,
                        'error': str(e)
