...
from ncjishaku.repl.scope import Scope
from ncjishaku.repl.walkers import KeywordTransformer
...
async def _repl_coroutine({0}):
    ...
    try:
        ...
        ncjishaku = None  # keep working even if in panic recovery mode
...
def wrap_code(code: str, args: str = '', auto_return: bool = True) -> ast.Module:
    ...
    for node in ast.walk(mod):
        node.lineno = -100_000  # pyright: ignore[reportAttributeAccessIssue]
        node.end_lineno = -100_000  # pyright: ignore[reportAttributeAccessIssue]
