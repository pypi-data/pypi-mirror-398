...
"""
ncjishaku.features.management
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The ncjishaku extension and bot control commands.
...
from ncjishaku.math import mean_stddev
from ncjishaku.modules import ExtensionConverter
from ncjishaku.repl import inspections
...
class ManagementFeature(Feature):
    ...
    async def jsk_load(self, ctx: ContextA, *extensions: ExtensionConverter):  # type: ignore
        ...
        # 'jsk reload' on its own just reloads ncjishaku
        if ctx.invoked_with == 'reload' and not extensions:
            extensions = [['ncjishaku']]
    ...
    async def jsk_sync(self, ctx: ContextA, *targets: str):
        ...
        for guild in guilds:
            if guild is None:
                ...
                slash_commands = list(filter(lambda command: guild in command.guild_ids, all_commands))
            ...
            try:
                ...
                # It behaves differently than the orignal jishaku, but it works.
                synced = data
