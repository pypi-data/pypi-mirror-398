...
from ncjishaku.types import BotT, ContextA
...
class Feature(commands.Cog):
    """
    Baseclass defining feature components of the ncjishaku cog.
    """
    ...
    class Command(typing.Generic[GenericFeature, P, T]):  # pylint: disable=too-few-public-methods
        ...
        def convert(
            ...
            return command_type(**self.kwargs)(self.callback)  # pyright: ignore[reportArgumentType]
    ...
    async def cog_check(self, ctx: ContextA):  # type: ignore  # pylint: disable=invalid-overridden-method
        ...
        if not await ctx.bot.is_owner(ctx.author):  # pyright: ignore[reportArgumentType]
    ...
    def submit(self, ctx: ContextA):
        """
        A context-manager that submits the current task to ncjishaku's task list
