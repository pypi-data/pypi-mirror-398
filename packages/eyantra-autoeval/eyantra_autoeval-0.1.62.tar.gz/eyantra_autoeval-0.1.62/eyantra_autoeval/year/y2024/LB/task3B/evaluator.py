...
class SimpleBagRecorder(Node):
    def __init__(self):
        super().__init__('simple_bag_recorder_task_3B')
        ...
        topic4_info = rosbag2_py._storage.TopicMetadata(
            name='/move_action/_action/status',
            type= 'action_msgs/msg/GoalStatusArray',
            serialization_format='cdr')
        self.writer.create_topic(topic4_info)
        ...
        self.subscription = self.create_subscription(TFMessage,'/tf',self.topic4_callback,10)
        self.create_subscription(GoalStatusArray,'/move_action/_action/status',self.topic5_callback,10)
    ...
    def topic4_callback(self, data):
        self.writer.write('/tf',serialize_message(data), self.get_clock().now().nanoseconds)
    def topic5_callback(self, data):
        self.writer.write('/move_action/_action/status',serialize_message(data), self.get_clock().now().nanoseconds)
    def topic3(self):
