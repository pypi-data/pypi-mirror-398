...
class AsyncLogWriter:
    def __init__(
        ...
        *,
        ...
        write_batch: (
            Callable[[Iterable[dict[str, Any]]], Awaitable[None]] | None
        ) = None,
    ) -> None:
        if max_queue_size < 1:
        ...
        async with self._lock:
            ...
            try:
                ...
                await asyncio.wait_for(
                    self._worker_task, timeout=max(1.0, timeout - 1.0)
                )
            except TimeoutError:
    ...
    async def _run(self) -> None:
        ...
        try:
            while True:
                ...
                wait_tasks = {
                    t for t in (get_task, sleep_task, flush_task) if t is not None
                }
                if not wait_tasks:
