...
def _parse_json_arguments(
    ...
    ) as exc:  # pragma: no cover - click handles error display
...
@connection_options
@click.option(
    "--vhost", default=None, help="Target virtual host (defaults to connection vhost)"
)
@click.option(
    "--name", required=True, help="Queue name (alphanumeric, dot, dash or underscore)"
)
@click.option(
    "--durable/--no-durable",
    default=False,
    show_default=True,
    help="Create queue as durable",
)
@click.option(
    "--exclusive/--no-exclusive",
    default=False,
    show_default=True,
    help="Create queue as exclusive",
)
@click.option(
