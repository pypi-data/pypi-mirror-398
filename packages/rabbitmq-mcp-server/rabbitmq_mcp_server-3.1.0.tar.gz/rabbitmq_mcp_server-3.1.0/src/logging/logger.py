...
_LEVEL_TO_NUM = {
    LogLevel.ERROR: logging.ERROR,  # type: ignore[attr-defined]
    LogLevel.WARN: logging.WARNING,  # type: ignore[attr-defined]
    LogLevel.INFO: logging.INFO,  # type: ignore[attr-defined]
    LogLevel.DEBUG: logging.DEBUG,  # type: ignore[attr-defined]
}
...
def get_logger(
    name: str = "rabbitmq-mcp", *, config: LogConfig | None = None
) -> StructuredLogger:
    """
...
class _AsyncWriterRunner:
    ...
    def submit(self, entry: dict[str, Any]) -> None:
        ...
        asyncio.run_coroutine_threadsafe(
            self._writer.write_log(entry), self._loop
        ).result()
    ...
    def stop(self, *, timeout: float = 30.0) -> None:
        ...
        try:
            asyncio.run_coroutine_threadsafe(
                self._writer.stop(timeout=timeout), self._loop
...
class _LoggerState:
    def __init__(self, name: str, config: LogConfig) -> None:
        ...
        fallback = (
            self.console_handler.write_batch if config.fallback_to_console else None
        )
        self.file_path = _resolve_output_path(config)
        self.file_handler = FileLogHandler(
            self.file_path, fallback=fallback, config=config
        )
...
class StructuredLogger:
    ...
    def warning(
        self, message: str, *, category: LogCategory | str, **kwargs: Any
    ) -> Any:
    ...
    def _log(
        ...
        **kwargs: Any,
        ...
        if "category" in kwargs:
            raise ValueError(
                "'category' must be provided as named argument, not inside kwargs"
            )

        category_value = (
            category.value if isinstance(category, LogCategory) else str(category)
        )
