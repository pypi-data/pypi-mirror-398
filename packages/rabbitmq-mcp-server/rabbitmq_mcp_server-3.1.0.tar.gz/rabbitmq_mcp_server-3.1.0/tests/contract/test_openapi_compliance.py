...
def test_list_operations_include_pagination(
...
def test_paginated_response_examples_validate(
    ...
    spec = _load_spec(contract)
    example = spec["paths"][path][method]["responses"]["200"]["content"][
        "application/json"
    ]["example"]
...
def test_request_examples_align_with_supported_payloads(
    ...
    spec = _load_spec(contract)
    request_body = spec["paths"][path][method]["requestBody"]["content"][
        "application/json"
    ]
