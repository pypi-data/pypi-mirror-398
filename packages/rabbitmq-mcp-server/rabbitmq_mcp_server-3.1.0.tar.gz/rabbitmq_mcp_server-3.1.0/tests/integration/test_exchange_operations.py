...
def test_exchange_create_types() -> None:
    with rabbitmq_container() as container:
        ...
        headers_options = exchanges.ExchangeOptions(
            durable=True, arguments={"format": "json"}
        )
        created_names = []
        ...
        for name, exchange_type, opts in [
            ...
            created = exchanges.create_exchange(
                "/", name, exchange_type, opts, settings=settings
            )
        ...
        assert all(
            any(item.name == name for item in listed.items) for name in created_names
        )
...
@pytest.mark.integration
def test_exchange_audit_logging_for_create_delete(
...
def test_exchange_concurrent_create_and_delete() -> None:
    with rabbitmq_container() as container:
        ...
        def create_exchange() -> None:
            try:
                exchanges.create_exchange(
                    "/", exchange_name, "direct", options, settings=settings
                )
...
def test_exchange_duplicate_create_conflict() -> None:
    with rabbitmq_container() as container:
        ...
        def attempt_create() -> None:
            try:
                exchanges.create_exchange(
                    "/", exchange_name, "topic", options, settings=settings
                )
            except ConflictError:
                pass
