...
def test_consumer_filters_by_routing_key(tmp_path: Path, mock_rabbitmq):
    ...
    routing_keys = [
        call[1]["routing_key"] for call in mock_channel.basic_publish.call_args_list
    ]
...
def test_file_and_rabbitmq_destinations_work_simultaneously(
    tmp_path: Path, mock_rabbitmq
):
    """
