...
def _write_sample_logs(
    logger: StructuredLogger, *, categories: Iterable[LogCategory]
) -> None:
...
def test_log_file_contains_valid_json_per_line(
    ...
    _write_sample_logs(
        logger, categories=[LogCategory.OPERATION, LogCategory.PERFORMANCE]
    )
    logger.shutdown()
...
def test_all_logs_include_schema_version(
    ...
    _write_sample_logs(
        logger, categories=[LogCategory.SECURITY, LogCategory.PERFORMANCE]
    )
    logger.shutdown()
...
def test_rotation_on_file_size_limit(tmp_path: Path) -> None:
    ...
    assert (
        len(log_files) >= 2
    ), f"Expected rotation to create multiple files, got {len(log_files)}"
