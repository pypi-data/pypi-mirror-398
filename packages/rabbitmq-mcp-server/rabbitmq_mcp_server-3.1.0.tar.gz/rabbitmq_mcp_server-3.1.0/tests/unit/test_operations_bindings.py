...
class FakeExecutor:
    def __init__(
        ...
        self.vhost = vhost
        self.last_requested_vhost: str | None = getattr(
            self, "last_requested_vhost", None
        )
        ...
        self.queue_lookup: dict[tuple[str, str], dict[str, Any]] = getattr(
            self, "queue_lookup", {}
        )
        ...
        self.binding_relations_response_sequence: list[list[dict[str, Any]]] | None = (
            getattr(
                self,
        ...
        self.bindings_response: list[dict[str, Any]] = getattr(
            self, "bindings_response", []
        )
        ...
        self.raise_on_get_exchange: Exception | None = getattr(
            self, "raise_on_get_exchange", None
        )
        self.raise_on_get_queue: Exception | None = getattr(
            self, "raise_on_get_queue", None
        )
...
def test_create_binding_success(
    ...
    monkeypatch.setattr(
        ...
        lambda v, e: call_sequence.append(f"vhost:{v}"),
    )
    monkeypatch.setattr(
        bindings,
        "validate_routing_key",
        lambda key, exchange_type: call_sequence.append(
            f"routing:{key}:{exchange_type}"
        ),
    )
    monkeypatch.setattr(bindings, "get_logger", lambda _: logger)
...
def test_create_binding_conflict(
    ...
    monkeypatch.setattr(bindings, "validate_vhost_exists", lambda v, e: None)
    monkeypatch.setattr(
        bindings, "validate_routing_key", lambda key, exchange_type: None
    )
...
def test_create_binding_missing_both_resources(
    ...
    with pytest.raises(NotFoundError) as exc:
        bindings.create_binding(
            "/sales", "missing.exchange", "missing.queue", None, None
        )
...
def test_create_binding_missing_exchange(
    ...
    with pytest.raises(NotFoundError) as exc:
        bindings.create_binding(
            "/sales", "missing.exchange", "orders.queue", None, None
        )
