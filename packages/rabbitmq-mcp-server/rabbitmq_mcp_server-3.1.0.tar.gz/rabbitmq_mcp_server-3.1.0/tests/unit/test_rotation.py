...
def test_rotated_file_naming_pattern(
    ...
    if rotated_files:
        ...
        for f in rotated_files:
            assert (
                "-" in f.stem or "." in f.stem
            ), "Rotated file should include date/timestamp"
...
def test_rotation_preserves_logs(temp_log_dir: Path, mock_config: LogConfig) -> None:
    ...
    assert (
        len(parsed) == num_entries
    ), f"Expected {num_entries} entries, got {len(parsed)}"
