...
class Settings(BaseSettings):
    ...
    @classmethod
    def from_yaml(
        cls, path: str = "config/config.yaml"
    ) -> RabbitMQSettings:  # noqa: F821
    ...
    def validate_timeouts(self) -> None:
        assert (
            1 <= self.crud_timeout <= 60
        ), "crud_timeout must be between 1 and 60 seconds"
        assert (
            1 <= self.list_timeout <= 60
        ), "list_timeout must be between 1 and 60 seconds"
        assert (
            0.1 <= self.search_timeout <= 60
        ), "search_timeout must be between 0.1 and 60 seconds"
