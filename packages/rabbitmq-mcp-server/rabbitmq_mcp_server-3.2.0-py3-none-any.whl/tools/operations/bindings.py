...
def list_bindings(
    ...
    *,
    ...
    logger.debug(
        "Fetching bindings for vhost %s", target_vhost if vhost is not None else "*"
    )
...
def create_binding(
    ...
    *,
    ...
    def _fetch_exchange() -> dict[str, Any]:
        return executor.get_exchange(target_vhost, exchange)  # type: ignore[no-any-return]

    def _fetch_queue() -> dict[str, Any]:
        return executor.get_queue(target_vhost, queue)  # type: ignore[no-any-return]
    ...
    try:
        existing_bindings = (
            executor.list_binding_relations(target_vhost, exchange, queue) or []
        )
    except NotFoundError:
        existing_bindings = []
