...
def _error(
    ...
    return MCPToolResult(
        success=False, error=MCPError(code=code, message=message, details=details)
    )


async def handle_call_id(
    operation_id: str, payload: dict[str, Any] | None = None
) -> MCPToolResult:
    ...
    if handler is None:
        return _error(
            "NOT_IMPLEMENTED", f"Handler não implementado para {operation_id}"
        )
    ...
    try:
        ...
        return _error(
            "INVALID_PAYLOAD", "Dados inválidos", details={"errors": str(exc)}
        )
...
async def _handle_disconnect(payload: dict[str, Any]) -> MCPToolResult:
    ...
    result = DisconnectResult(
        disconnected=True, graceful=not force, duration_ms=duration_ms
    )
    return MCPToolResult(success=True, result=result.model_dump())
