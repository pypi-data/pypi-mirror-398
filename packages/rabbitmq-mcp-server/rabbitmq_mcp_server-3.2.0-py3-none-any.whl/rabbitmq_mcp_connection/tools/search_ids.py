...
try:  # pragma: no cover - dependências opcionais
    ...
    chromadb = None  # type: ignore[assignment,unused-ignore]
...
class SearchBackend:
    def search(self, query: str, limit: int) -> list[tuple[str, float, dict[str, Any]]]:
...
class ChromaSearchBackend(SearchBackend):
    ...
    def search(self, query: str, limit: int) -> list[tuple[str, float, dict[str, Any]]]:
        ...
        metadatas: list[dict[str, Any]] = metadatas_raw[0] if metadatas_raw else []
...
def handle_search_ids(
    query: str, pagination: PaginationParams | None = None
) -> MCPToolResult:
    if not query:
