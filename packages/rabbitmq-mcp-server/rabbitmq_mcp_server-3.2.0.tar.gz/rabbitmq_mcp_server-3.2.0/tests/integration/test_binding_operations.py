...
def _publish_message(
    executor: bindings.RabbitMQExecutor,
    exchange_name: str,
    routing_key: str,
    payload: str = "data",
) -> None:
...
def test_binding_create_and_list_simple() -> None:
    with rabbitmq_container() as container:
        ...
        assert any(
            item.destination == queue_name and item.source == exchange_name
...
def test_binding_wildcard_routing_topic_exchange() -> None:
    with rabbitmq_container() as container:
        ...
        assert (
            not no_match
        ), f"Unexpected routing for key 'orders.eu.us.created': {no_match}"
        _purge_queue(executor, queue_name)
...
def test_binding_pagination_with_multiple_pages() -> None:
    with rabbitmq_container() as container:
        ...
        for index in range(120):
            ...
            queues.create_queue(
                "/", queue_name, queues.QueueOptions(), settings=settings
            )
            bindings.create_binding(
