...
@pytest.mark.parametrize(
    ...
    [
        (
            AuthorizationError,
            {"code": "UNAUTHORIZED", "message": "denied", "action": "Retry"},
        ),
        (
            NotFoundError,
            {"code": "NOT_FOUND", "message": "missing", "action": "Create resource"},
        ),
        (
            ConflictError,
            {"code": "CONFLICT", "message": "conflict", "action": "Resolve"},
        ),
        (
            ConnectionError,
...
def test_format_error_for_other_exceptions(
