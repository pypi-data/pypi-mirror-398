...
def test_queue_table_column_ordering() -> None:
    ...
    verbose_output = format_table_output(
        payload, queue_cmd.VERBOSE_COLUMNS, footer="footer"
    )

    assert _header_sequence(base_output) == [
        header for header, _ in queue_cmd.BASE_COLUMNS
    ]
    assert _header_sequence(verbose_output) == [
        header for header, _ in queue_cmd.VERBOSE_COLUMNS
    ]
...
def test_exchange_table_column_ordering() -> None:
    ...
    base_output = format_table_output(
        payload, exchange_cmd.BASE_COLUMNS, footer="footer"
    )
    verbose_output = format_table_output(
        payload, exchange_cmd.VERBOSE_COLUMNS, footer="footer"
    )

    assert _header_sequence(base_output) == [
        header for header, _ in exchange_cmd.BASE_COLUMNS
    ]
    assert _header_sequence(verbose_output) == [
...
def test_binding_table_column_ordering() -> None:
    ...
    assert _header_sequence(output) == [
        header for header, _ in binding_cmd.BASE_COLUMNS
    ]
