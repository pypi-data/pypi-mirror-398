...
class FakeExecutor:
    def __init__(
        ...
        self.raise_on_get_bindings: Exception | None = getattr(
            self, "raise_on_get_bindings", None
        )
    ...
    def create_exchange(
        self, vhost: str, name: str, payload: dict[str, Any]
    ) -> dict[str, Any]:
...
def test_create_exchange_success(
    ...
    monkeypatch.setattr(
        exchanges, "validate_name", lambda n: call_sequence.append(f"name:{n}")
    )
    monkeypatch.setattr(
    ...
    assert _patch_executor.created == [
        (
            "/sales",
            "orders.exchange",
            {**options.model_dump(exclude_none=True), "type": "topic"},
        )
    ]
...
def test_create_exchange_conflict(
    ...
    with pytest.raises(ConflictError) as exc:
        exchanges.create_exchange(
            None, "dup.exchange", "direct", exchanges.ExchangeOptions()
        )
