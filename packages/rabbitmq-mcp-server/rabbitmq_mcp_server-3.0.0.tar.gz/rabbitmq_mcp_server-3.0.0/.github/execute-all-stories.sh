...
execute_sm_task() {
    ...
    copilot --agent bmad-agent-bmm-sm --model claude-sonnet-4.5 --prompt "${task}" --allow-all-tools
    echo -e "${GREEN}[SM] Completed: ${task}${NC}\n"
}

# Function to execute DEV tasks
execute_dev_task() {
    local task="$1"
    echo -e "${YELLOW}[DEV] Executing: ${task}${NC}"
    copilot --agent bmad-agent-bmm-dev --model claude-sonnet-4.5 --prompt "${task}" --allow-all-tools
...
execute_epic() {
    ...
    execute_sm_task "*epic-tech-context epic-${epic_num}"
    execute_sm_task "*validate-epic-tech-context epic-${epic_num}. If any issues, gaps or recommendations found, fix them all using best judgement and/or most commonly option used in such scenario and app type (spotify mcp server). Assure fixes do not introduce new issues or gaps or recommendations."
...
execute_story() {
    ...
    # SM Tasks
    execute_sm_task "*create-story ${story_id}"
    execute_sm_task "*validate-create-story ${story_id}. If any issues, gaps or recommendations found, fix them all using best judgement and/or most commonly option used in such scenario and app type (spotify mcp server). Assure fixes do not introduce new issues or gaps or recommendations."
    execute_sm_task "*story-context ${story_id}"
    execute_sm_task "*validate-story-context ${story_id}. If any issues, gaps or recommendations found, fix them all using best judgement and/or most commonly option used in such scenario and app type (spotify mcp server). Assure fixes do not introduce new issues or gaps or recommendations."
    execute_sm_task "*story-ready-for-dev ${story_id}"
    
    # DEV Tasks
    execute_dev_task "*develop-story ${story_id}"
    execute_dev_task "*code-review ${story_id} If any issues, gaps or recommendations found, fix them all using best judgement and/or most commonly option used in such scenario and app type (spotify mcp server). Assure fixes do not introduce new issues or gaps or recommendations."
    execute_sm_task "*story-done ${story_id}"
    
    # Commit after story-done
    git add -A
    git commit -m "feat(story-${story_id}): complete story" || true
}
...
execute_epic_retrospective() {
    ...
    execute_sm_task "*epic-retrospective epic-${epic_num}. Use dev notes. It says it all."
