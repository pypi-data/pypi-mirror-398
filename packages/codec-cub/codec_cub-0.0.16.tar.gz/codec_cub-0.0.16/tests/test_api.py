...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `codec_cub`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in codec_cub.__all__ or not hasattr(codec_cub, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in codec_cub.__all__:
