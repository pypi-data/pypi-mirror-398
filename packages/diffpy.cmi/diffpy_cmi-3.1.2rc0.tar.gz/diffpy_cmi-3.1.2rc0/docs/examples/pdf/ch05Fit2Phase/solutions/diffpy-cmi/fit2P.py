"""Please see notes in Chapter 6 of the 'PDF to the People' book for
additional explanation of the code.

This Diffpy-CMI script will carry out a structural refinement of a
measured PDF from a sample containing two phases, silicon and nickel. It
is the same refinement as is done using PDFgui in this chapter of the
book, only this time using Diffpy-CMI.
"""
...
# the PDF (that we want to fit over) also, specify the Q_max
# and Q_min values used to reduce the PDF.
PDF_RMIN = 1.5
PDF_RMAX = 30
PDF_RSTEP = 0.01
QMAX = 25
...
# 8: We define a function 'make_recipe' to make the recipe that the
# fit will follow.
def make_recipe(cif_path1, cif_path2, dat_path):
    ...
    Parameters
    ----------
    cif_path1 : string, The full path to the structure CIF file to load,
    for the first phase.
    cif_path2 : string, The full path to the structure CIF file to load,
    for the second phase.
    dat_path :  string, The full path to the PDF data to be fit.

    Returns
    ----------
    recipe :    The initialized Fit Recipe object using the datname and
                structure path provided.
    """
...
# 21 We create a useful function 'plot_results' for writing a plot of the
# fit to disk.
def plot_results(recipe, fig_name):
    ...
    diff = g - gcalc
    diffzero = (min(g) - np.abs(max(diff))) * np.ones_like(g)
...
def main():
    ...
    # Establish the full path of the two CIF files with the structure of
    # interest.
    stru1_path = DPATH / CIF_NAME1
    stru2_path = DPATH / CIF_NAME2

    # 23: Call 'make_recipe' to create our fit recipe, this time providing
    # the name and location of BOTH structure files
    recipe = make_recipe(str(stru1_path), str(stru2_path), str(gr_path))
