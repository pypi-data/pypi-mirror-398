...
Run the example and then read through the 'make_recipe' code. You will see how
to refine a single structure to two data sets.

Extensions

- In 'make_recipe' the fit contributions are identically configured except for
  the profile. Factor out that configuration code and apply it in the
  'make_recipe' method. This will reduce the amount of code required to get the
...
def make_recipe(strufile, datname1, datname2):
