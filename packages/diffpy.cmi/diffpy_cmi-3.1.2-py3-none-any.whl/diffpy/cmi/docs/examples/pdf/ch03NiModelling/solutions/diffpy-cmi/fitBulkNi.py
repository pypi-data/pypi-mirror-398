"""Please see notes in Chapter 3 of the 'PDF to the People' book for
...
# In this case it is two directories up, in a
# folder called 'data'.
...
# 5: Specify the min, max, and step r-values of
...
# 8: We define a function 'make_recipe' to make the recipe that
# the fit will follow.This Fit Recipe object contains the PDF data,
# information on all the structure(s)
...
def make_recipe(cif_path, dat_path):
    ...
    # variable,'s1'. Using this structure is a very flexible way of adding
...
def main():
    ...
    # 24: During the optimization, fix and free parameters sequentially as
    # you would in PDFgui. This leads to more stability in the refinement.
    # This can be done with 'recipe.fix' and 'recipe.free', and we can use
    # either a single parameter name or any of the tags we assigned when
    # creating the fit recipe. We first fix all variables. The tag 'all'
    # incorporates every parameter. We then create a list of 'tags' which
    # we want free sequentially. We loop over them, freeing each during a
    # loop, and then fit using the SciPy function 'least_squares'.
    # 'least_squares' takes as its arguments the function to be optimized,
    # here 'recipe.residual', as well as initial values for the fitted
    # parameters, provided by 'recipe.values'. The 'x_scale="jac"' argument
    # is optional and provides for a bit more stability.
    recipe.fix("all")
