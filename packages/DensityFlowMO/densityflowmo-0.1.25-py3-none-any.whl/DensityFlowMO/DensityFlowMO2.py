...
class DensityFlowMO2(nn.Module):
    ...
    def setup_networks(self):
        ...
        if self.perturb_size>0:
            ...
            '''self.decoder_rna_effect_from_atac = MLP(
                    [self.latent_dim] + self.decoder_hidden_layers + [[self.latent_dim,self.latent_dim]],
                    activation=activate_fct,
                    output_activation=[None,Exp],
                    post_layer_fct=post_layer_fct,
                    post_act_fct=post_act_fct,
            ...
            self.decoder_atac_effect_from_rna = zuko.flows.NSF(features=self.latent_dim, context=self.latent_dim, 
                                                               transforms=self.transforms, hidden_features=self.flow_hidden_layers)
            '''self.decoder_atac_effect_from_rna = MLP(
                    [self.latent_dim] + self.decoder_hidden_layers + [[self.latent_dim,self.latent_dim]],
                    activation=activate_fct,
                    output_activation=[None,Exp],
                    post_layer_fct=post_layer_fct,
    ...
    def cross_model(self, zs_rna, zus_rna,zs_atac, zus_atac):
        pyro.module('DensityFlowMO2', self)

        with pyro.plate('data'):
            ###############################################################
            # RNA 
            rna_zns = pyro.sample('atac2rna_zn', ZukoToPyro(self.decoder_atac2rna(zs_atac)).to_event(1), obs=zs_rna)

            if self.perturb_size>0:
                rna_zus = pyro.sample('atac2rna_zus', ZukoToPyro(self.decoder_rna_effect_from_atac(zus_atac)).to_event(1), obs=zus_rna)
            
            ###############################################################
            # atac 
            atac_zns = pyro.sample('rna2atac_zn', ZukoToPyro(self.decoder_rna2atac(zs_rna)).to_event(1), obs=zs_atac)

            if self.perturb_size>0:
                atac_zus = pyro.sample('rna2atac_zus', ZukoToPyro(self.decoder_atac_effect_from_rna(zus_rna)).to_event(1), obs=zus_atac)
                    
    def cross_guide(self, zs_rna, zus_rna,zs_atac, zus_atac):
        pass
    ...
    def fit(self, 
        ...
        with tqdm(total=num_epochs, disable=not show_progress, desc='RNA Training', unit='epoch') as pbar:
        ...
        with tqdm(total=num_epochs, disable=not show_progress, desc='ATAC Training', unit='epoch') as pbar:
            for epoch in range(num_epochs):
                epoch_losses = [0.0] * num_losses
                for _, idx in dataloader:
        ...
        with tqdm(total=num_epochs, disable=not show_progress, desc='Cross-omics Training', unit='epoch') as pbar:
            for epoch in range(num_epochs):
                epoch_losses = [0.0] * num_losses
                for batch_x_rna, idx in dataloader:
                    batch_x_rna = batch_x_rna.to(self.get_device())
                    ...
                    # cross-omics
                    with torch.no_grad():
                        zs_rna,_ = self._get_rna_basal_embedding(batch_x_rna)
                        zus_rna = self._rna_cell_shift(zs_rna, perturb=batch_u)
                    
                        zs_atac,_ = self._get_atac_basal_embedding(batch_x_atac)
                        zus_atac = self._atac_cell_shift(zs_atac, perturb=batch_u)
                        
                        fixed_zs_rna = zs_rna.detach().clone()
                        fixed_zus_rna = zus_rna.detach().clone()
                        fixed_zs_atac = zs_atac.detach().clone()
                        fixed_zus_atac = zus_atac.detach().clone()
                    
                    # cross-omics
                    loss_id = 0
                    new_loss = losses[loss_id].step(zs_rna=fixed_zs_rna, zus_rna=fixed_zus_rna, zs_atac=fixed_zs_atac, zus_atac=fixed_zus_atac)
                    epoch_losses[loss_id] += new_loss
