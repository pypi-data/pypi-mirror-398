...
class DensityFlowMO(nn.Module):
    ...
    def model(self, xs_rna, xs_atac, us=None, cs=None):
        ...
        with pyro.plate('data'):
            ...
            ###############################################################
            # atac2rna
            loc,scale = self.decoder_atac2rna(atac_zns)
            _ = pyro.sample('rna_zns', dist.Normal(loc,scale).to_event(1), obs=rna_zns)
            if self.perturb_size>0:
                loc,scale = self.decoder_rna_effect_from_atac(atac_zus)
                _ = pyro.sample('rna_zus', dist.Normal(loc,scale).to_event(1), obs=rna_zus)
                
            ###############################################################
            # rna2atac
            loc,scale = self.decoder_rna2atac(rna_zns)
            _ = pyro.sample('atac_zns', dist.Normal(loc,scale).to_event(1), obs=atac_zns)
            if self.perturb_size>0:
                loc,scale = self.decoder_atac_effect_from_rna(rna_zus)
                _ = pyro.sample('atac_zus', dist.Normal(loc,scale).to_event(1), obs=atac_zus)
                    ...
                    for loss_id in range(num_losses):
                        if us is None:
                            ...
                            if cs is None:
                                ...
                                new_loss = losses[loss_id].step(batch_x_rna, batch_x_atac, us=batch_u, cs=batch_c)
