...
from ..ivention.icartesian_target import (
    ICartesianTarget,
...
class CartesianTargetConfiguration(ICartesianTargetConfiguration):
    """
    Configuration for a Cartesian Target asset.
    """
...
@inherit_docstrings
class CartesianTarget(ICartesianTarget):
    """
    A software representation of a Cartesian Target as defined within the scene assets pane.
    A cartesian target is defined in mm and degrees, where the angles are extrinsic Euler
    angles in XYZ order.
    """
    ...
    def __init__(
        ...
        self.__configuration = CartesianTargetConfiguration(api, uuid, name, default_position, parent_reference_frame_id)
        self._api = api
        self._assets: AssetList = []

    @property
    def _configuration(self) -> CartesianTargetConfiguration:
