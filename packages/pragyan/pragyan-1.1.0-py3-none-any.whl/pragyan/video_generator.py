...
class VideoGenerator:
    ...
    def generate_video(
        ...
        # Try to use algorithm-specific visualizer first
        ...
        if visualizer_info:
            # Use specialized visualizer - this sets self._scene_class_name
            visualizer, kwargs = visualizer_info
            scene_code, self._scene_class_name = self._generate_specialized_scene(
                question, solution, analysis, visualizer, kwargs
            )
            print(f"🎬 Using specialized visualizer: {self._scene_class_name}")
        elif self.llm_client:
            # Use LLM to generate custom animation
            scene_code = self._generate_llm_powered_scene(
                question, solution, analysis
            )
            self._scene_class_name = "DSAExplanation"
        else:
            # Fallback to generic animated scene
...
'''
    ...
    def _generate_llm_powered_scene(
        ...
        try:
            ...
            print(f"LLM animation generation failed: {e}")
            return self._generate_animated_scene(question, solution, analysis)
