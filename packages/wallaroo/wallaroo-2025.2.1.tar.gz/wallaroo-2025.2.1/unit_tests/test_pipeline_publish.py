...
class TestPipelinePublish:
    ...
    def test_wait_for_status_published(self, respx_mock):
        ...
        get_pipeline_publish_resp = self.add_pipeline_publish_resp(
            PipelinePublishStatus.PUBLISHED
        )

        respx_mock.post(
            f"{self.test_client.api_endpoint}/v1/api/pipelines/get_publish_status"
        ).mock(return_value=httpx.Response(200, json=get_pipeline_publish_resp))
    ...
    def test_wait_for_status_publishing(self, respx_mock):
        # Mock the necessary dependencies
        get_pipeline_publish_resp_1 = self.add_pipeline_publish_resp(
            PipelinePublishStatus.PENDINGPUBLISH
        )
        get_pipeline_publish_resp_2 = self.add_pipeline_publish_resp(
            PipelinePublishStatus.PUBLISHED
        )
        ...
        respx_mock.post(
            ...
            side_effect=[
                httpx.Response(200, json=get_pipeline_publish_resp_1),
