...
class ModelVersion(Object):
    ...
    def _fill(self, data: Dict[str, Any]) -> None:
        ...
        self._error_summary = value_if_present(data, "error_summary")
    ...
    @rehydrate("_error_summary")
    def error_summary(self) -> str:
        self._rehydrate()
        return cast(str, self._error_summary)
    ...
    def get_upload_logs(self, limit: Optional[int] = None) -> "ModelUploadLogs":
        """Returns the user visible logs for the given model_upload() call for a custom python model.
        These will be messages related to parse and "pip install" of requirements.txt or to static
        ...
        :return: ModelUploadLogs object.
        """
        from .wallaroo_ml_ops_api_client.api.model.get_upload_logs import (
            sync,
        )
        from .wallaroo_ml_ops_api_client.models.upload_logs_request import (
            UploadLogsRequest,
        )
        from .wallaroo_ml_ops_api_client.models.upload_logs_response import (
            UploadLogsResponse,
        )

        req = UploadLogsRequest(self.id(), limit)

        ret = sync(client=self.client.mlops(), body=req)

        if not isinstance(ret, UploadLogsResponse):
            # TODO: Better error
            raise Exception("Failed to find logs for this model upload.")

        return ModelUploadLogs(ret.logs)
...
class ModelUploadLogs(List[str]):
