...
class CLLM:
    ...
    def init(self):
        ...
        print_info("\nTo use CLLM, you need to:")
        print("1. Set your API keys using environment variables:")
        print("   - OPENAI_API_KEY for OpenAI/GPT models")
        print("   - ANTHROPIC_API_KEY for Claude models")
        print("2. Select a provider: cllm use <provider>:<model>")
        print("   Example: cllm use gpt-4 or cllm use claude:sonnet-4")
        print("3. Start interactive session: cllm interactive")
    ...
    def attach(self, filename):
        ...
        if not self.current_session:
            print_error("No active session. Use 'cllm interactive' first")
            return False
    ...
    def ask(self, prompt):
        ...
        if not provider_spec:
            print_error("No provider selected. Use 'cllm use <provider>' first")
            return None
    ...
    def interactive(self, session_name="default"):
        ...
        print_colored("🚀 CLLM Interactive Session", Fore.CYAN, Style.BRIGHT)
        print_colored("=" * 60, Fore.CYAN, Style.BRIGHT)

        provider_spec = self.config.get_provider()
        if provider_spec:
            provider, model = provider_spec.split(":", 1)
            print_colored(f"📡 Provider: {provider} ({model})", Fore.GREEN)
        else:
            print_warning("No provider selected. Use '/use <provider>' to set one")

        print_colored(f"💾 Session: {session_name}", Fore.GREEN)
        print_colored("\nCommands:", Fore.YELLOW)
        print("  /help        - Show commands")
        print("  /use <prov>  - Switch provider")
        print("  /attach <f>  - Attach file")
        print("  /detach <f>  - Detach file")
        print("  /files       - List attached files")
        print("  /clear       - Clear conversation")
        print("  /stats       - Show costs & stats")
        ...
        while True:
            try:
                # Get user input
                print_colored("\n💬 You: ", Fore.YELLOW, Style.BRIGHT, end="")
    ...
    def handle_command(self, command):
        ...
        if cmd == "/help":
            print_colored("\n📖 Available Commands:", Fore.CYAN, Style.BRIGHT)
            print("  /help              - Show this help")
            print("  /use <provider>    - Switch provider (e.g., /use gpt-4)")
            print("  /attach <file>     - Attach a file as context")
            print("  /detach <file>     - Remove attached file")
            print("  /files             - List all attached files")
            print("  /clear             - Clear conversation history")
            print("  /clearfiles        - Clear all attached files")
            print("  /history           - Show conversation history")
            print("  /stats             - Show session statistics and costs")
            ...
            print_colored("\n📜 Conversation History:", Fore.CYAN, Style.BRIGHT)
            for msg in messages:
                role = msg["role"]
                content = msg["content"]
                ...
                if role == "user":
                    print_colored(f"\n💬 You ({timestamp}):", Fore.YELLOW)
                    print_colored(content, Fore.WHITE)
                else:
                    print_colored(f"\n🤖 Assistant ({timestamp}):", Fore.MAGENTA)
                    print_colored(content, Fore.WHITE)
            ...
            print_colored("\n📊 Session Statistics:", Fore.CYAN, Style.BRIGHT)
            print_colored(f"   Session: {self.current_session.name}", Fore.WHITE)
            print_colored(f"   Messages: {stats['message_count']} exchanges", Fore.WHITE)
            print_colored(f"   Total tokens: {stats['total_tokens']:,}", Fore.YELLOW)
            print_colored(f"   Total cost: ${stats['total_cost']:.6f}", Fore.GREEN, Style.BRIGHT)

            # Show attached files
            files = self.current_session.list_files()
            if files:
                print_colored(f"   Attached files: {len(files)}", Fore.WHITE)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CLLM - Command Line LLM Interface",
    ...
    # Init command
    subparsers.add_parser("init", help="Initialize CLLM configuration")
