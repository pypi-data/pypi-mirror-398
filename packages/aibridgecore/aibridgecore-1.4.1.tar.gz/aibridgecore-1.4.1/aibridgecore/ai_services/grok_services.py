...
from aibridgecore.constant.constant import GROK_BASE_URL


class GrokService(AIInterface):
    """
    Base class for GROK Services
    """
    ...
    def generate_stream(
        ...
        **kwargs,
        ...
        try:
            if not prompts:
                raise GrokException("No prompts provided for streaming")
            ...
            raise GrokException(f"Grok Streaming failed: {e}")
    ...
    def execute_prompt_function_calling(
        ...
        print("Inside Grok execute_prompt_function_calling")
        print("model : ", model)
        client = OpenAI(api_key=api_key, base_url=GROK_BASE_URL)
        print("client : ", client)
        if model.startswith("g"):
    ...
    def get_response(
        ...
        try:
            ...
            if not prompts:
                raise GrokException("No prompts provided")
            api_key = api_key if api_key else parse_api_key("")
            message_data = self.get_prompt_context(context, model=model)
            print(message_data)
            model_output = []
