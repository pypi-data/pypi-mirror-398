...
class AI21labsText(AIInterface):
    ...
    def generate_stream(
        ...
        **kwargs,
        ...
        try:
            if not prompts:
                raise Ai21Exception("No prompts provided for streaming")

            api_key = api_key if api_key else parse_api_key("ai21_api")
            client = AI21Client(api_key=api_key)

            messages = self.get_prompt_context(context)
            messages.append(ChatMessage(role="user", content=prompts[0]))

            stream = client.chat.completions.create(
                model=model,
                messages=messages,
                n=variation_count,
                max_tokens=max_tokens,  # max_tokens is a required argument for ai21 api
            ...
            raise Ai21Exception(f"AI21 Streaming failed: {e}")
    ...
    def get_prompt_context(self, context):
        ...
        if context:
            for _context in context:
                ...
                context_data.append(
                    ChatMessage(role=_context["role"], content=_context["context"])
                )
    ...
    def get_response(
        ...
        try:
            ...
            for index, prompt in enumerate(prompts):
                context_data.append(ChatMessage(role="user", content=prompt))
                print("model : ", model)
                # model = "jamba-large"
                if max_tokens:
                ...
                for res in response.choices:
                    ...
                    try:
                        ...
                        if "```json" in content:
                            try:
                                json_content = content.split("```json")[1].split("```")[
                                    0
                                ]
