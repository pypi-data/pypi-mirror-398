...
def UserAgentTik(params=None):
    ...
    agent = (
        f"com.zhiliaoapp.musically.go/{version_code} "
...
class Ladon:
    ...
    def encrypt(x_khronos: int, lc_id: int, aid: int) -> str:
        return ladon_encrypt(x_khronos, aid, lc_id)
...
class Gorgon:
    ...
    def build(self, params=None, cookies=None, data=None, payload=None, url=None):
        ...
        return AHMED
    def Encoder(self, params=None, cookies=None, data=None, payload=None, url=None, unix=None):
        ...
        return {"x-gorgon": final, "x-ss-req-ticket": str(int(unix*1000)), "x-khronos": str(int(unix))}
        ...
        return Argus.encrypt(
            {
                ...
                7: params_dict.get("version_name", [None])[0] or params_dict.get("ab_version", [""])[0],  # appVersion
...
def AID(params, default=1233):
    if not params:
        return default

    try:
        if isinstance(params, dict):
            return int(params.get("aid", default))
        if isinstance(params, str):
            q = parse_qs(params)
            return int(q.get("aid", [default])[0])
    except Exception:
        pass

    return default
    
def sign(params, payload: str = None, cookie: str = None):
    unix = int(time.time())
    payload_str = ""
    aid = AID(params)
    if payload:
    ...
    signatures = Gorgon(aid).Encoder(
        params=params,
        data=payload_str,
        cookies=cookie,
        unix=unix
    )
    signatures["x-ladon"] = Ladon.encrypt(unix, 1611921764, aid)
