...
Provide the ``python -m bitranox_template_cli_app_config_log_mail`` path mandated by the
project's packaging guidelines. The wrapper delegates to
:func:`bitranox_template_cli_app_config_log_mail.cli.main` so that module execution mirrors the
...
def _module_main() -> int:
    ...
    Implements ``python -m bitranox_template_cli_app_config_log_mail`` by delegating to the
