# Claude Code Guidelines for bitranox_template_cli_app_config_log_mail
...
├── src/
│   └── bitranox_template_cli_app_config_log_mail/  # Main Python package
...
- **Version Bumps**: update `pyproject.toml` , `CHANGELOG.md` and update the constants in `src/../__init__conf__.py` according to `pyproject.toml`  
    - Automation rewrites `src/bitranox_template_cli_app_config_log_mail/__init__conf__.py` from `pyproject.toml`, so runtime code imports generated constants instead of querying `importlib.metadata`.
