...
class SheinLib:
    ...
    def get_funds_data_lz(self):
        ...
        store_manager = self.config.shein_store_manager.get(str(self.store_username).lower())
        # 数据结构: [店铺名称, 店铺账号, 店长, 在途商品金额, 在仓商品金额, 待结算金额, 可提现金额, 不可提现金额, 保证金, 汇总, 销售出库金额, 新品上架数量, 成功转备货款, 成功率, 导出时间]
        NotifyItem = [f'{self.store_name}', self.store_username, store_manager, num298, num67, num70, num72, no_withdrawable_amount, deposit_amount_paid, '', outAmount, '', '', '', TimeUtils.current_datetime()]
        log(NotifyItem)
