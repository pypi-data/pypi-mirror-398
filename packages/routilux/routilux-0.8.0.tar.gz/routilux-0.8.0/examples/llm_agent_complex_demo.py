...
class TaskParser(Routine):
    ...
    def __init__(self):
        ...
        self.output_event = self.define_event("parsed_task", ["task_id", "task_type", "requirements", "priority"])
...
class ToolExecutor(Routine):
    ...
    def __init__(self, tool_name: str = None):
        ...
        self.output_event = self.define_event("tool_result", ["result", "tool_name", "task_id", "success"])
    ...
    def execute_tool(self, tools: List[str], task_id: str):
        ...
        try:
            ...
            if self._stats["execution_count"] % 5 == 0:
                raise ValueError(f"Tool {self.tool_name} failed on attempt {self._stats['execution_count']}")
...
class ResultValidator(Routine):
    ...
    def __init__(self):
        ...
        self.output_event = self.define_event("validated_result", ["valid", "result", "task_id", "tool_name"])
...
class ResultAggregator(Routine):
    ...
    def __init__(self):
        super().__init__()
        self.input_slot = self.define_slot("result_input", handler=self.aggregate_results, merge_strategy="append")
        self.output_event = self.define_event("final_result", ["aggregated_result", "task_id", "tool_count"])
        self._results = []
...
def create_complex_flow() -> Flow:
    ...
    flow.connect(selector_id, "selected_tools", executor_search_id, "tool_input",
                 param_mapping={"tools": "tools", "task_id": "task_id"})
    flow.connect(selector_id, "selected_tools", executor_calc_id, "tool_input",
                 param_mapping={"tools": "tools", "task_id": "task_id"})
    flow.connect(selector_id, "selected_tools", executor_trans_id, "tool_input",
                 param_mapping={"tools": "tools", "task_id": "task_id"})
    flow.connect(selector_id, "selected_tools", executor_code_id, "tool_input",
                 param_mapping={"tools": "tools", "task_id": "task_id"})
    
    # Connect executors to validator (parallel validation)
    flow.connect(executor_search_id, "tool_result", validator_id, "validation_input",
                 param_mapping={"result": "result", "tool_name": "tool_name", 
                              "task_id": "task_id", "success": "success"})
    flow.connect(executor_calc_id, "tool_result", validator_id, "validation_input",
                 param_mapping={"result": "result", "tool_name": "tool_name",
                              "task_id": "task_id", "success": "success"})
    flow.connect(executor_trans_id, "tool_result", validator_id, "validation_input",
                 param_mapping={"result": "result", "tool_name": "tool_name",
                              "task_id": "task_id", "success": "success"})
    flow.connect(executor_code_id, "tool_result", validator_id, "validation_input",
                 param_mapping={"result": "result", "tool_name": "tool_name",
                              "task_id": "task_id", "success": "success"})
    
    # Connect validator to aggregator
    flow.connect(validator_id, "validated_result", aggregator_id, "result_input",
                 param_mapping={"valid": "valid", "result": "result",
                              "task_id": "task_id", "tool_name": "tool_name"})
    
    # Connect aggregator to formatter
    flow.connect(aggregator_id, "final_result", formatter_id, "formatter_input",
                 param_mapping={"aggregated_result": "aggregated_result",
                              "task_id": "task_id", "tool_count": "tool_count"})
...
def test_execution_tracking(flow: Flow):
    ...
    if tracker:
        ...
        for routine_id, perf in list(routine_performances.items())[:5]:  # Show first 5
            print(f"  {routine_id}: {perf.get('total_time', 0):.4f}s "
...
def test_complex_scenario(flow: Flow):
    ...
    for i, task in enumerate(tasks, 1):
        ...
        results.append({
            ...
            "routines": len(job_state.routine_states)
        })
