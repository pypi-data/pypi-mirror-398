...
class DataProcessor(Routine):
    ...
    def __init__(self, processor_id: str = None):
        ...
        self.output_event = self.define_event("data_processed", ["result", "processor_id", "processing_time"])
...
class DataAggregator(Routine):
    ...
    def __init__(self):
        super().__init__()
        self.input_slot = self.define_slot("processed_data", handler=self.aggregate, merge_strategy="append")
...
def create_sequential_flow() -> Flow:
    ...
    flow = Flow(
        flow_id="sequential_data_processing",
...
def test_sequential_vs_concurrent():
    ...
    print(f"Time Saved: {sequential_time - concurrent_time:.3f} seconds ({((sequential_time - concurrent_time) / sequential_time * 100):.1f}%)")
    print()
