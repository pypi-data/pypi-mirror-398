...
public:
    ...
    /// `out_idx` / `out_dist` resized to *k* and returned in ascending‑distance order.
               ...
               std::vector<float>& out_dist) const
    {
        ...
        // max‑heap holding the *k* best so far; element = {dist, idx}
    ...
    /// Compute *exact* k‑NN for every row in the dataset (self excluded).
    /// Heavy: O(N²·d) – use sparingly (e.g. during index build).
