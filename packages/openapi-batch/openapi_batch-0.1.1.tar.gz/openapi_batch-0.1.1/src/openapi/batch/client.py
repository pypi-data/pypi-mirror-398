...
from .models import BatchItem, ItemState, JobStatus
...
class BatchClient:
    """
    DX-first BatchClient.

    Defaults:
      - async_submit=True: returns immediately
      - optional callback `on_complete(job_handle)` called when job is terminal
    """
    ...
    def map(
        ...
        *,
        ...
        Default behavior (async_submit=True):
          - returns JobHandle immediately
          - background thread executes the job
          - invokes on_complete(job_handle) when terminal (if provided)

        Set async_submit=False to block until completion (previous behavior).
        """
        ...
        def runner() -> None:
            try:
                ...
                # Make sure job doesn't remain "RUNNING" forever
                ...
                if on_complete:
                    try:
                        ...
                        log("job.on_complete.exception", job_id=job_id, error=str(cb_e))

        if async_submit:
            t = threading.Thread(target=runner, name=f"openapi-batch-{job_id}", daemon=True)
            t.start()
            log("job.submitted.async", job_id=job_id, mode=mode, provider=self.provider_name)
            return handle
    ...
    def _execute_job(
        ...
        *,
        ...
        if chosen == "native":
            ...
            if p == "openai":
                self._run_openai_native_batch(job_id=job_id, items=items, decode=decode)
                return
            if p == "gemini":
                self._run_gemini_native_batch(job_id=job_id, items=items, decode=decode)
                return
    ...
    # ----------------------------
    # OpenAI native (unchanged logic, retains logs)
    # ----------------------------
    def _run_openai_native_batch(
        ...
        *,
        ...
        for it in items:
            ...
            req_lines.append(
                {"custom_id": item_id, "method": "POST", "url": self.openai_endpoint, "body": body}
            )
        ...
        while True:
            ...
            if now - last_heartbeat >= self.native_poll_heartbeat_s:
                log(
                    "openai.native.heartbeat",
                    job_id=job_id,
                    batch_id=batch_id,
                    status=st,
                    elapsed_s=elapsed,
        ...
        log(
            "openai.native.parsed",
            job_id=job_id,
            batch_id=batch_id,
            output_lines=len(output_map),
            error_lines=len(error_map),
        )
        ...
        for it in items:
            ...
            if item_id in error_map:
                ...
                self.store.mark_err(
                    job_id,
                    item_id,
                    {"error_type": "batch_error", "message": msg, "retryable": False, "raw": line},
    ...
    # ----------------------------
    # Gemini native (assumes you already applied the normalized provider fix)
    # ----------------------------
    def _run_gemini_native_batch(
        ...
        *,
        ...
        log("gemini.native.submit.start", job_id=job_id)
        ...
        log("gemini.native.submit.done", job_id=job_id, batch_name=batch_name, model=batch_model)
        ...
        while True:
            ...
            if state != last_state:
                log("gemini.native.poll", job_id=job_id, batch_name=batch_name, state=state, elapsed_s=elapsed, batch_stats=b.get("batch_stats"))
                last_state = state

            if now - last_heartbeat >= self.native_poll_heartbeat_s:
                log("gemini.native.heartbeat", job_id=job_id, batch_name=batch_name, state=state, elapsed_s=elapsed, batch_stats=b.get("batch_stats"))
                last_heartbeat = now
        ...
        log("gemini.native.files", job_id=job_id, batch_name=batch_name, state=last_state, result_file=result_file)
        ...
        if not result_file:
            ...
            self.store.set_job_status(job_id, JobStatus.FAILED)
            log("gemini.native.done", job_id=job_id, batch_name=batch_name, ok=0, dead=len(items), total=len(items))
            return

        text = self._provider.native_file_content_text(file_id=result_file)
        out_map = GeminiProvider.parse_result_jsonl(text)

        log("gemini.native.parsed", job_id=job_id, batch_name=batch_name, lines=len(out_map))
        ...
        for it in items:
            ...
            if not line:
                self.store.mark_err(
                    job_id,
                    item_id,
                    {"error_type": "batch_missing", "message": "Missing key in output file.", "retryable": False, "raw": None},
                    state=ItemState.DEAD,
                )
                continue
            ...
            if "error" in line and line["error"]:
                self.store.mark_err(
                    job_id,
                    item_id,
                    {"error_type": "batch_error", "message": str(line["error"]), "retryable": False, "raw": line},
                    state=ItemState.DEAD,
                )
                continue
        ...
        log("gemini.native.done", job_id=job_id, batch_name=batch_name, ok=prog.ok, dead=prog.dead, total=prog.total)

    def retry_failed(self, *, job_id: str, concurrency: int = 50, retry: Optional[RetryPolicy] = None) -> JobHandle:
        retry = retry or self.default_retry
        self.store.requeue_failed(job_id, include_dead=True)

        # retry is also async-by-default; expose via map() if you want later
        asyncio.run(
            run_emulated_job(
                store=self.store,
                provider=self._provider,
        ...
        return JobHandle(job_id=job_id, store=self.store)
