...
class AudioTonalDataset(Dataset):
    def __init__(
        ...
        min_freq=5000,
        max_freq=50000,
        time_patch_frames=50,
        freq_patch_frames=50,
        time_patch_advance=None,
        freq_patch_advance=None,
        cache_wavs=True,
        ...
        mask_processing_function=None,
        post_processing_time_patch_padding=0,
        post_process_full_frequency_range=False,
    ):
        """
        ...
        wav_names = [Path(f).name for f in wav_files]
        ...
        # Note: Should remove any binary file for which the audio file does not have
        # a high enough sample rate
        ...
        try:
            anno_wav_files = [
                wav_file_dict[filename] for filename in anno_wav_filenames
            ]
        except KeyError as ex:
            raise Exception(
                f"Could not find audio file {str(ex)} corresponding to binary file."
            )

        ## SAVE DATASET VALUES ##
        freq_resolution = 1000 / frame_time_span
        self.freq_patch_length_hz = freq_resolution * freq_patch_frames
        self.freq_patch_advance_hz = (
            self.freq_patch_length_hz
            if freq_patch_advance is None
            else freq_resolution * freq_patch_advance
        )
        self.time_patch_length_ms = step_time_span * time_patch_frames
        self.time_patch_advance_ms = (
            self.time_patch_length_ms
            if time_patch_advance is None
            else step_time_span * time_patch_advance
        )
        ...
        for i, wav_file in enumerate(wavio.read(w) for w in anno_wav_files):
            ...
            num_freq_divisions = (
                floor(
                    (min(max_freq, nyquist_freq) - min_freq - self.freq_patch_length_hz)
                    / self.freq_patch_advance_hz
                )
                + 1
            )
            ...
            num_time_divisions = (
                floor(
                    (file_length_ms - self.time_patch_length_ms - frame_time_span)
                    / self.time_patch_advance_ms
                )
                + 1
            )
            ...
            if cache_annotations:
                self.file_info[i]["contours"] = tonalReader(
                    self.bin_files[i]
                ).getTimeFrequencyContours()
    ...
    def get_balanced_iterable(self, epoch_size=None):
        ...
        """
        return BalancedIterableDataset(
            self, self.get_positive_indices(), epoch_size=epoch_size
        )
    ...
    def get_positive_indices(self):
        ...
        for file_idx in range(len(self.bin_files)):
            ...
            if self.cache_annotations:
                ...
                contours = tonalReader(
                    self.bin_files[file_idx]
                ).getTimeFrequencyContours()
            ...
            for contour in contours:
                for time, freq in contour:
                    ...
                    # with the node
                    freq_overlap = np.ceil(freq_patch_range - freq_patch % 1).astype(
                        int
                    )
                    time_overlap = np.ceil(time_patch_range - time_patch % 1).astype(
                        int
                    )

                    # Add indices to positive set
                    for f_idx in range(
                        min(int(freq_patch), max_freq_patch),
                        max(int(freq_patch) - freq_overlap, -1),
                        -1,
                    ):
                        for t_idx in range(
                            int(time_patch), max(int(time_patch) - time_overlap, -1), -1
                        ):
                            idx = (
                                t_idx
                                + f_idx * num_time_divisions
                                + patches_cumsum[file_idx]
                            )
    ...
    def get_index_source(self, idx):
        ...
        if idx >= len(self):
            raise IndexError(
                f"Dataset index ({idx}) out of bounds for length ({len(self)})."
            )
        ...
        start_freq = (
            idx // num_time_divisions
        ) * self.freq_patch_advance_hz + self.min_freq
    ...
    def get_datum(self, idx, return_db=False):
        ...
        padded_start_time = max(
            start_time - self.post_processing_time_patch_padding * self.step_time_span,
            0,
        )
        padded_end_time = (
            end_time + self.post_processing_time_patch_padding * self.step_time_span
        )
        ...
        datum, actual_end_time = getSpectrogram(
            wav_data,
        ...
        right_time_padding = int(
            (actual_end_time - end_time) / self.step_time_span + leftover
        )

        lower_freq_padding = (
            (self.max_freq - end_freq) / self.freq_resolution if self.full_freq else 0
        )
        leftover = lower_freq_padding % 1
        lower_freq_padding = int(lower_freq_padding)
        upper_freq_padding = (
            int((start_freq - self.min_freq) / self.freq_resolution + leftover)
            if self.full_freq
            else 0
        )

        datum = datum[
            lower_freq_padding : datum.shape[0] - upper_freq_padding,
            left_time_padding : datum.shape[1] - right_time_padding,
        ]
        label = label[
            lower_freq_padding : label.shape[0] - upper_freq_padding,
            left_time_padding : label.shape[1] - right_time_padding,
        ]
...
class BalancedDataset(Dataset):
    def __init__(self, dataset, positive_set, positive_proportion=0.5, seed=None):
        """
        ...
        # Create a random generator for reproducible shuffling
        ...
        if positive_proportion == 1:
            ...
            len_n = int((1 - positive_proportion) * len_p / positive_proportion)
...
class Hdf5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (self.file[self.data_name][idx], self.file[self.labels_name][idx])
...
def dataset_to_hdf5(dataset, filename, transpose=False):
    ...
    h5.create_dataset(
        "data", (length,) + datum1.shape, dtype="float32", chunks=(1,) + datum1.shape
    )
    h5.create_dataset(
        "labels", (length,) + label1.shape, dtype="float32", chunks=(1,) + label1.shape
    )
...
class BalancedIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        self._rng.shuffle(self.positive_indices)
        self._rng.shuffle(self.negative_indices)
...
# Substitute postfix .bin to .wav.
def bin2wav_filename(bin_file):
    """Convert a binary annotation filename to the corresponding wav filename."""
    return Path(bin_file).stem + ".wav"
