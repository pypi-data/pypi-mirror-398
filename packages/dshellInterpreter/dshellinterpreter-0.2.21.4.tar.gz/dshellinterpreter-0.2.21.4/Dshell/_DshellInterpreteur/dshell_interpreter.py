...
async def get_params(node: ParamNode, interpreter: DshellInterpreteur) -> dict[str, Any]:
    ...
    x = DshellTokenizer(variables).start()[0]
    regrouped_variables = await regroupe_commandes(x, interpreter)
    ...
    for param_name, param_data in regrouped_variables.parameters.items():
        ...
        variables = sub(rf"--([*']?)({param_name})\s+(.*)\s+(.*)", remplacer, variables, count=1)
        already_modified.add(param_name)

    variables_post_regrouped: list[str] = variables.strip().split(' ')

    index_variable = 0
    for var in regrouped_parameters.parameters.keys():
        if var not in already_modified:

            if regrouped_parameters.get_parameter(var).type == DTT.PARAMETER:
                regrouped_parameters.set_parameter(var, variables_post_regrouped[index_variable], regrouped_parameters.get_parameter(var).type)
                index_variable += 1

            elif regrouped_parameters.get_parameter(var).type == DTT.STR_PARAMETER:
                regrouped_parameters.set_parameter(var, ' '.join(variables_post_regrouped[:index_variable]), regrouped_parameters.get_parameter(var).type)
                break

            elif regrouped_parameters.get_parameter(var).type == DTT.PARAMETERS:
                regrouped_parameters.set_parameter(var, ListNode(variables_post_regrouped[index_variable:]), regrouped_parameters.get_parameter(var).type)
                break

    return regrouped_parameters.get_dict_parameters()
...
async def regroupe_commandes(body: list[Token], interpreter: DshellInterpreteur, normalise: bool = False) -> DshellArguments:
    ...
    while index < n:
        ...
        if body[index].type == DTT.PARAMETER:
            ...
            instance_dhsell_arguments.set_parameter(body[current_index].value, value, DTT.PARAMETER)
            index += 1
            ...
            while (index + 1) < n and body[index + 1].type not in (DTT.PARAMETER, DTT.STR_PARAMETER, DTT.PARAMETERS):
                ...
                index += 1
                instance_dhsell_arguments.set_parameter(body[current_index].value, final_argument, obligatory=final_argument == '*', type_=DTT.STR_PARAMETER)
            ...
            while (index + 1) < n and body[index + 1].type not in (DTT.PARAMETER, DTT.STR_PARAMETER, DTT.PARAMETERS):
                ...
                index += 1
                instance_dhsell_arguments.set_parameter(body[current_index].value, ListNode(list_parameters), obligatory=list_parameters[0] == '*' if list_parameters else False, type_=DTT.PARAMETERS)
