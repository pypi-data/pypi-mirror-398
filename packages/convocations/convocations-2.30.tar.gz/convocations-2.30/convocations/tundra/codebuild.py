...
@task
def bundle(ctx, exclude_dirs='data,.venv'):
    """
    ...
    exclude_dirs = [ f"'/{x}/*'" for x in exclude_dirs ]
    ctx.run(f"zip -q -r {t.name}.zip . -x {' '.join(exclude_dirs)}")
    notice_end()
    latest = 'latest.zip'
...
def deploy(ctx, profile=None, session=None, **kwargs):
    ...
    _deploy(
        ctx, conf, limit, branch,
        session=session, local_python=sys.executable)
