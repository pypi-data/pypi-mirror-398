...
@task(klass=AwsTask)
def setup_local_connections(
        ctx, key=None, config_file=None,
        session=None, debug=False, **kwargs):
    """
    installs connections on your local airflow
    """
    import requests
    from waddle.utils import load_config
    filename = get_context_value(ctx, 'convocations.airflow.config')
    filename = config_file or filename
    if filename:
        notice(f'loading {filename}')
        conf = load_config(filename)
        conf = conf.airflow
        notice_end('\N{penguin}\N{vs16}')
    else:
        conf = ctx.airflow
    ...
    if debug:
        notice(f'connecting to {conf.base_url} as {conf.username} / {password}')
    else:
        notice(f'connecting to {conf.base_url} as {conf.username}')
...
@task(klass=AwsTask)
def setup_local_variables(ctx, config_file=None, session=None, **kwargs):
    """
    installs variables on your local airflow running in docker compose
    """
    from waddle import load_config
    from waddle.yaml_base import KmsWrappedSecret, MasterKeyedSecret
    import requests
    ...
    for key, value in variables.items():
        ...
        if value and isinstance(value, (KmsWrappedSecret, MasterKeyedSecret)):
            value = value.value
        if found:
