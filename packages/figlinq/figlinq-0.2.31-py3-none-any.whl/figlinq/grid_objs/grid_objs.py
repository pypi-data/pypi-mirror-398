...
from figlinq import utils, exceptions
...
class Grid(MutableSequence):
    ...
    def to_dataframe(self):
        ...
        # Attach grid metadata (fid and column uids) if available, so it can be reused on upload
        try:
            ...
            for col in self:
                try:
                    ...
                    if isinstance(col.id, str) and fid and col.id.startswith(fid + ":"):
                        col_ids[col.name] = col.id
                except Exception:
                    continue
            df.attrs["figlinq"] = {"fid": fid, "col_ids": col_ids}
        except Exception:
            pass
        return df