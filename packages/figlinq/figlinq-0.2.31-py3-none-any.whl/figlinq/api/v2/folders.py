...
def all(user=None, search=None, **query):
    """Return the folder listing for a user via the ``/v2/folders/all`` route."""

    url = build_url(RESOURCE, route="all")

    params = dict(query or {})
    if user is not None:
        params.setdefault("user", user)
    if search is not None:
        params.setdefault("s", search)
