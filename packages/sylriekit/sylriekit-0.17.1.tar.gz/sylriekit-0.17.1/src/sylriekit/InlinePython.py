...
class InlinePython:
    ...
    def render_html(cls, text: str, session_token: str = None, base_dir: str = None) -> str:
        ...
        for match in pattern.finditer(text):
            ...
            # Check req-data attribute - skip block if user doesn't have required session data
            req_data = params.get("req-data")
            if req_data:
                try:
                    from sylriekit.Website import Website
                    from flask import request, g
                    # Check pending cookies first (for session updates), then fall back to request cookies
                    session_id = None
                    ...
                    if session_id:
                        data_value = Website.get_session_data(req_data, session_id)
                        if data_value is not True:
                            # User doesn't have required data, skip this block
                            last_end = end
                            continue
                    else:
                        # No session, skip this block
                        last_end = end
                        continue
                except Exception:
                    # If we can't check, skip the block for safety
                    last_end = end
                    continue
    ...
    def get_valid_cache_functions(cls, check_req_data: bool = True) -> list:
        ...
        # Filter out functions with req-data requirements the user doesn't meet
        filtered = []
        for entry in valid:
            ...
            if req_data:
                try:
                    ...
                    if session_id:
                        ...
                        # else: skip this function - user doesn't have required data
                    # else: no session, skip this function
                except Exception:
                    # If we can't check, skip for safety
                    pass
            else:
                # No req-data requirement, include the function
                filtered.append(entry)
    ...
    def hit_cache(cls, function_name: str, function_args: dict = None):
        ...
        for entry in cls._CACHE_FUNCTIONS:
            ...
            # Check req-data attribute - if set, user must have that session data = True
            req_data = entry.get("data", {}).get("req-data")
            if req_data:
                try:
                    from sylriekit.Website import Website
                    ...
                    if session_id:
                        data_value = Website.get_session_data(req_data, session_id)
                        if data_value is not True:
                            # User doesn't have required data, act like function doesn't exist
                            return {"error": True, "message": f"Function '{function_name}' not found in cache"}
                    else:
                        # No session, act like function doesn't exist
                        return {"error": True, "message": f"Function '{function_name}' not found in cache"}
                except Exception:
...
(function() {{
    ...
    console.log('[SylrieKit] Python bridge initialized with functions:', {[f["name"] for f in functions]});
}})();
</script>'''
    ...
    def _extract_js_arg_names(cls) -> str:
        ...
        # Parameters that are auto-injected server-side and should not come from JS
