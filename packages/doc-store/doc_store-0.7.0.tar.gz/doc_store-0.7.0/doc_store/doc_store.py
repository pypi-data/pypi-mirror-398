...
class DocEvent(dict):
    ...
    def __init__(
        ...
        assert elem_type in (
            ...
            EvalLayout,
            EvalContent,
        ), f"Invalid element type {elem_type}."
...
class DocStore(DocStoreInterface):
    ...
    def add_tag(self, elem_id: str, tag: str) -> None:
        ...
        elem_data = coll.find_one_and_update(
            ...
            projection={"_id": 0, "provider": 1, "type": 1, "version": 1},
        )
        if elem_data is None:
            return
        if self._event_sink is not None and elem_type != Task:
    ...
    def insert_layout(
        ...
        self._get_coll(Page).update_one(
            {"id": page_id},
    ...
    def _check_trigger_condition(self, condition: TriggerCondition) -> None:
        ...
        for provider in (condition.page_providers or []) + (condition.layout_provider or []):
            self._check_basic_name("provider", provider)
        for version in (condition.block_versions or []) + (condition.content_version or []):
