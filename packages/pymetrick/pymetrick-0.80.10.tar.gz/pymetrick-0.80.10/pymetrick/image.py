...
# 2025-12-24 v0.80.5 PL: - Add .avif files
...
def image2avif(entrada_path, salida_path, quality=70, speed=6):
    '''
    # Uso
        image2avif("foto_producto.jpg", "foto_producto.avif")
    '''
    try:
        # Abrir la imagen original
        with Image.open(entrada_path) as img:
            # Guardar en formato AVIF
            # 'quality' va de 1 (peor) a 100 (mejor). 70 es un gran equilibrio.
            # 'speed' va de 0 (más lento, mejor compresión) a 10 (más rápido, archivo más grande). El valor 6 es el predeterminado y suele ser óptimo.
            img.save(salida_path, "AVIF", quality=quality, speed=6)
    except Exception as e:
        LOG.error('Error image.image2avif %s in line %s' % (e,format(sys.exc_info()[-1].tb_lineno)))
