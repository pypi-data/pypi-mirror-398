...
def get_urls(cur, limit: int = 30) -> Generator[str, None, None]:
    cur.execute(f"""
        SELECT id, url FROM entries e WHERE NOT EXISTS (SELECT 1 FROM stories s WHERE e.url = s.url) LIMIT {limit}
    """)
...
def get_stories() -> None:
    ...
    for entry_id, url in get_urls(cur):
