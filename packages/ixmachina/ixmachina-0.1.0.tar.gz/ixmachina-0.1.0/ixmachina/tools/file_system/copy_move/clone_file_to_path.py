"""
File system tools for cloning files to exact paths.
"""
...
def clone_file_to_path(
    source_path: str,
    destination_path: str,
    file_system_memory: Optional["FileSystemMemory"] = None,
) -> Dict[str, Any]:
    """
    Clone a file to an exact destination path.

    The source file will be copied to the exact destination path.

    Args:
        source_path: Path to the source file.
        destination_path: Exact path where the file should be copied to.
    ...
    try:
        ...
        if file_system_memory is not None:
            try:
                ...
                action = Action(
                    function_name="clone_file_to_path",
                    function=clone_file_to_path,
                    arguments={
                        "source_path": source_path,
                        "destination_path": destination_path,
                    },
        ...
        return {
            ...
            "source_path": source_path,
            "destination_path": None,
            "error": f"Error cloning file: {str(e)}",
        }
