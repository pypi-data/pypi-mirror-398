...
class ALFAbyDate:
    ...
    def get_cummunication(self, date):
        try:
            final_result = []
            for branch in self.branches_list:
                final_result += self.get_basic(f'v2api/{str(branch).strip()}/communication/index')
            message = f'Платформа: ALFA. Имя: {self.add_name}. Дата: {str(date)}. Функция: get_cummunication. Результат: ОК'
            self.common.log_func(self.bot_token, self.chat_list, message, 1)
            return final_result
        except Exception as e:
            message = f'Платформа: ALFA. Имя: {self.add_name}. Дата: {str(date)}. Функция: get_cummunication. Ошибка: {e}.'
            self.common.log_func(self.bot_token, self.chat_list, message, 3)
            return message
        ...
        try:
            ...
            for customer in customer_list:
                for branch in json.loads(customer['branch_ids']):
                    print(f'Customer: {str(int(customer["id"]))}, Branch: {str(branch)}')
    ...
    def get_bonus(self, date):
        try:
            ...
            for customer in customer_list:
                for branch in json.loads(customer['branch_ids']):
                    final_result += self.get_basic(f"v2api/{str(branch).strip()}/bonus/balance-bonus?customer_id={str(int(customer['id']))}")
