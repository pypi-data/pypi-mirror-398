        ...
        try:
            ...
            for chunk in final_list:
                ...
                if code == 200:
                    ...
                    response.raise_for_status()
                time.sleep(23)
            message = f'Платформа: WB. Имя: {self.add_name}. Дата: {str(date)}. Функция: get_voronka. Результат: ОК'
            self.common.log_func(self.bot_token, self.chat_list, message, 1)
            return self.common.spread_table(self.common.spread_table(self.common.spread_table(all_cards)))
        except Exception as e:
            message = f'Платформа: WB. Имя: {self.add_name}. Дата: {str(date)}. Функция: get_voronka. Ошибка: {e}.'
            self.common.log_func(self.bot_token, self.chat_list, message, 3)
            return message
