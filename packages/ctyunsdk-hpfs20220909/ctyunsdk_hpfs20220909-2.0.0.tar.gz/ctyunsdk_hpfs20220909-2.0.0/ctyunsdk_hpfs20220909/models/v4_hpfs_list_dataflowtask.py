...
@dataclass_json
@dataclass
class V4HpfsListDataflowtaskRequest(CtyunOpenAPIRequest):
    regionID: str  # 资源池 ID
    azName: Optional[str] = None  # 多可用区下的可用区名字，不传为查询全部
    sfsUID: Optional[str] = None  # 并行文件唯一ID
    dataflowID: Optional[str] = None  # 数据流动策略ID
    taskStatus: Optional[str] = None  # 数据流动任务状态（creating/executing/completed/canceling/fail）
    taskType: Optional[str] = None  # 数据流动任务类型（import_data/import_metadata/export_data）
...
@dataclass_json
@dataclass
class V4HpfsListDataflowtaskResponse(CtyunOpenAPIResponse):
    statusCode: Optional[int] = None  # 返回状态码（800为成功，900为失败）
    errorCode: Optional[str] = None  # 错误码，为product.module.code三段式码
    error: Optional[str] = None  # 错误码，为product.module.code三段式码
    message: Optional[str] = None  # 失败时的错误描述，一般为英文描述
    description: Optional[str] = None  # 失败时的错误描述，一般为中文描述
    returnObj: Optional['V4HpfsListDataflowtaskReturnObj'] = None  # 成功时返回的数据
...
@dataclass_json
@dataclass
class V4HpfsListDataflowtaskReturnObj:
    taskList: Optional[List['V4HpfsListDataflowtaskReturnObjTaskList']] = None  # 返回的数据流动任务列表
    totalCount: Optional[int] = None  # 指定条件下用户数据流动任务总数
    currentCount: Optional[int] = None  # 当前页码下查询回来的用户数据流动任务数
...
@dataclass_json
@dataclass
class V4HpfsListDataflowtaskReturnObjTaskList:
