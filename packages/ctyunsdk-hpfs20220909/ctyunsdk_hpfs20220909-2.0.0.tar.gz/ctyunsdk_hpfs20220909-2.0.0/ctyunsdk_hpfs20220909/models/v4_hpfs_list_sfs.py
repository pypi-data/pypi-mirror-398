...
@dataclass_json
@dataclass
class V4HpfsListSfsRequest(CtyunOpenAPIRequest):
    regionID: str  # 资源池 ID
    sfsStatus: Optional[str] = None  # 并行文件状态。包括creating（创建中）、available（可用）、unusable（冻结）。不传为查询全部
    sfsProtocol: Optional[str] = None  # 挂载协议。2 种，nfs/hpfs ，不传为查询全部
    azName: Optional[str] = None  # 多可用区下的可用区名字，不传为查询全部
    fuzzyName: Optional[str] = None  # 并行文件名称(支持模糊查询)
...
@dataclass_json
@dataclass
class V4HpfsListSfsResponse(CtyunOpenAPIResponse):
    statusCode: Optional[int] = None  # 返回状态码（800为成功，900为失败）
    errorCode: Optional[str] = None  # 错误码，为product.module.code三段式码
    error: Optional[str] = None  # 错误码，为product.module.code三段式码
    message: Optional[str] = None  # 失败时的错误描述，一般为英文描述
    description: Optional[str] = None  # 失败时的错误描述，一般为中文描述
    returnObj: Optional['V4HpfsListSfsReturnObj'] = None  # 成功时返回的数据
...
@dataclass_json
@dataclass
class V4HpfsListSfsReturnObj:
    list: Optional[List['V4HpfsListSfsReturnObjList']] = None  # 返回的文件列表
    totalCount: Optional[int] = None  # 资源池指定条件下用户并行文件总数
    currentCount: Optional[int] = None  # 当前页码下查询回来的用户并行文件数
...
@dataclass_json
@dataclass
class V4HpfsListSfsReturnObjList:
