 ...
 * The z-value of the cross product of segments | (a, b) and (a, c). Positive means c is ccw | from
 * (a, b), negative cw. Zero means its collinear.
 *
 * @param a point
 * @param b point
 * @param c point
...
struct ccw_sorter
{
    ...
    bool
    operator() (const IK::Point_3 &a, const IK::Point_3 &b)
    {
 ...
 * get angle of an edge to its projects to the x-axis, this is the helper fuction of
 * bounding_rectangle
 *
...
/**
 * @brief Calculate a grid of points inside a polygon by a) convex hull, b) its bounding box, c)
 * grid of points inside the polygon
 *
 * @param [in] polygon input polyline
 * @param [in] offset_distance negative value is inwards offset, positive outwards, 0 is ignored. If
 * the offset distance is too big, the initial polygon will be used without offset
 * @param [in] division_distance distance between points, if 0 the distance is calculated
 * automatically
