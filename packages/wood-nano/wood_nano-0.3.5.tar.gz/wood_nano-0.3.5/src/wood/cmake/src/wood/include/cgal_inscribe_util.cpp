...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Main geometry methods used in cgal_inscribe_util to be used as self-contained header and source
// file
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
CGAL::Aff_transformation_3<IK> plane_to_xy(const IK::Point_3 &origin, IK::Vector_3 &x0,
                                           IK::Vector_3 &y0, IK::Vector_3 &z0) {
    unitize(x0);
    unitize(y0);
    unitize(z0);

    // Move to origin -> T0 translates point P0 to (0,0,0)
    CGAL::Aff_transformation_3<IK> t0(CGAL::TRANSLATION,
                                      IK::Vector_3(-origin.x(), -origin.y(), -origin.z()));

    // Rotate ->
    CGAL::Aff_transformation_3<IK> f0(x0.x(), x0.y(), x0.z(), y0.x(), y0.y(), y0.z(), z0.x(),
                                      z0.y(), z0.z());
...
void get_average_normal(const CGAL_Polyline &polyline, IK::Vector_3 &average_normal) {
    size_t len =
        CGAL::squared_distance(polyline.front(), polyline.back()) < wood::GLOBALS::DISTANCE_SQUARED
            ? polyline.size() - 1
            : polyline.size();
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        average_normal = average_normal + CGAL::cross_product(polyline[i] - polyline[prev],
                                                              polyline[next] - polyline[i]);
    }
}
...
void get_fast_plane(const CGAL_Polyline &polyline, IK::Point_3 &origin, IK::Vector_3 &x_axis,
                    IK::Vector_3 &y_axis, IK::Vector_3 &z_axis) {
    // origin
    origin = polyline[0];
...
void get_average_plane_axes_oriented_to_longest_edge(const CGAL_Polyline &polyline,
                                                     IK::Point_3 &origin, IK::Vector_3 &x_axis,
                                                     IK::Vector_3 &y_axis, IK::Vector_3 &z_axis) {
    size_t len =
        CGAL::squared_distance(polyline.front(), polyline.back()) < wood::GLOBALS::DISTANCE_SQUARED
            ? polyline.size() - 1
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        auto next = ((i + 1) + len) % len;
        z_axis = z_axis +
                 CGAL::cross_product(polyline[i] - polyline[prev], polyline[next] - polyline[i]);
...
void circle_points(const IK::Vector_3 &origin, const IK::Vector_3 &x_axis,
                   const IK::Vector_3 &y_axis, const IK::Vector_3 &z_axis,
                   std::vector<IK::Point_3> &points, int number_of_chunks, double radius) {
    ...
    CGAL::Aff_transformation_3<IK> xy_to_plane =
        internal::xy_to_plane(origin, x_axis, y_axis, z_axis);
...
bool orient_polyline_to_xy_and_back(CGAL_Polyline &polyline,
                                    CGAL::Aff_transformation_3<IK> &xform_to_xy,
                                    CGAL::Aff_transformation_3<IK> &xform_to_xy_inv) {
    /////////////////////////////////////////////////////////////////////////////////////
...
CGAL::Aff_transformation_3<IK> xy_to_plane(const IK::Vector_3 &origin, const IK::Vector_3 &x_axis,
                                           const IK::Vector_3 &y_axis, const IK::Vector_3 &z_axis) {
    ...
    CGAL::Aff_transformation_3<IK> f(_x_axis.x(), _y_axis.x(), _z_axis.x(), _x_axis.y(),
                                     _y_axis.y(), _z_axis.y(), _x_axis.z(), _y_axis.z(),
                                     _z_axis.z());

    // Move to 3d -> T1 translates (0,0,0) to point P1
    CGAL::Aff_transformation_3<IK> t(CGAL::TRANSLATION,
                                     IK::Vector_3(origin.x(), origin.y(), origin.z()));
...
CGAL::Aff_transformation_3<IK> axis_rotation(const double &angle, const IK::Vector_3 &axis) {
    ...
    IK::RT c = cos(angle), s = sin(angle), ux(axis.x()), uy(axis.y()), uz(axis.z());
    ...
    return CGAL::Aff_transformation_3<IK>(matrix[0], matrix[1], matrix[2], matrix[3], matrix[4],
...
std::tuple<IK::Point_3, IK::Plane_3, double> get_polylabel(
    const std::vector<CGAL_Polyline> &polylines, double precision) {
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (auto &polyline : polylines_copy) cgal::polyline_util::transform(polyline, xform_toXY);

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Sort polylines based on bounding-box to detect which polylines are holes, this only works if
    // the polygon does not have holes within holes
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (len.size() > 0) {
        for (size_t i = 0; i < polylines_copy.size(); i++) {
            CGAL::Bbox_3 AABB =
                CGAL::bbox_3(polylines_copy[i].begin(), polylines_copy[i].end(), IK());
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Convert polylines_copy to Polylabel datastructure, be aware that the polyline considers all
    // polygons closed, so skip the last point
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    mapbox::geometry::polygon<double> polygon_with_holes(polylines_copy.size());

    for (size_t i = 0; i < polylines_copy.size(); i++) {
        for (size_t j = 0; j < polylines_copy[ids[i]].size() - 1; j++) {
            polygon_with_holes[i].emplace_back(polylines_copy[ids[i]][j].x(),
                                               polylines_copy[ids[i]][j].y());
            // std::cout << polylines_copy[ids[i]][j].x() << " " << polylines_copy[ids[i]][j].y() <<
            // std::endl;
        }
    }

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Run the Polylabel algorithm, WARNING numbers are in 2D, first two doubles are center, the
    // third is the radius
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void get_polylabel_circle_division_points(const IK::Vector_3 &division_direction_in_3d,
                                          const std::vector<CGAL_Polyline> &polylines,
                                          std::vector<IK::Point_3> &points, int division,
    ...
    IK::Vector_3 center(std::get<0>(circle).hx(), std::get<0>(circle).hy(),
                        std::get<0>(circle).hz());
    bool is_direction_valid = division_direction_in_3d.hx() != 0.0 &&
                              division_direction_in_3d.hy() != 0.0 &&
                              division_direction_in_3d.hz() != 0.0;

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Find closest edge direction to the center
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    IK::Point_3 center_point(std::get<0>(circle).hx(), std::get<0>(circle).hy(),
                             std::get<0>(circle).hz());
    ...
    if (orient_to_closest_edge) {
        for (size_t i = 0; i < polylines.size(); i++) {
            for (size_t j = 0; j < polylines[i].size() - 1; j++) {
                ...
                IK::Point_3 projection =
                    polyline_segment.supporting_line().projection(center_point);

                // closest point paraneters
                bool has_on = false;
                const IK::Vector_3 polyline_segment_vector = polyline_segment.to_vector();
                const double polyline_segment_vector_squared_len =
                    polyline_segment_vector.squared_length();
                double t = 0;

                if (polyline_segment_vector.squared_length() > 0.0) {
                    if ((projection - polyline_segment[0]).squared_length() <=
                        (projection - polyline_segment[1]).squared_length())
                        t = ((projection - polyline_segment[0]) * polyline_segment_vector) /
                            polyline_segment_vector_squared_len;
                    else
                        t = 1.0 + ((projection - polyline_segment[1]) * polyline_segment_vector) /
                                      polyline_segment_vector_squared_len;
                ...
                if (has_on) {
                    double temp_distance_from_center_to_polyline_edge =
                        CGAL::squared_distance(center_point, projection);
                    if (temp_distance_from_center_to_polyline_edge <
                        distance_from_center_to_polyline_edge) {
                        distance_from_center_to_polyline_edge =
                            temp_distance_from_center_to_polyline_edge;
                        edge_id0 = i;
                        edge_id1 = j;
                    }
                }
    ...
    IK::Vector_3 division_direction_in_3d_ =
        orient_to_closest_edge
            ? polylines[edge_id0][edge_id1 + 1] - polylines[edge_id0][edge_id1 + 0]
            : IK::Vector_3(division_direction_in_3d.hx(), division_direction_in_3d.hy(),
                           division_direction_in_3d.hz());

    // plane vectors for orienting again to 2d for circle points dvisions
    IK::Vector_3 x_axis = is_direction_valid || orient_to_closest_edge
                              ? division_direction_in_3d_
                              : std::get<1>(circle).base1();
    IK::Vector_3 y_axis = is_direction_valid || orient_to_closest_edge
                              ? CGAL::cross_product(division_direction_in_3d_,
                                                    std::get<1>(circle).orthogonal_vector())
                              : std::get<1>(circle).base2();
    IK::Vector_3 z_axis = std::get<1>(circle).orthogonal_vector();

    // run the division method which is the output -> points
    internal::circle_points(center, x_axis, y_axis, z_axis, points, division,
                            std::get<2>(circle) * scale);
}

bool inscribe_rectangle_in_convex_polygon(const std::vector<CGAL_Polyline> &polylines,
                                          CGAL_Polyline &result, std::vector<IK::Point_3> &points,
                                          IK::Segment_3 &direction, double scale, double precision,
    ...
    for (auto &polyline : polylines_copy) point_count += polyline.size();

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Compute 2D bounding box to get a division step, the division step is equal to =
    // diagonal_length/(20*precision)
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (auto &polyline : polylines_copy) {
        ...
        for (auto &p : polyline) points_2d.push_back(IK::Point_2(p.hx(), p.hy()));
        points_2d.pop_back();
    }

    CGAL::Bbox_2 bbox = CGAL::bbox_2(points_2d.begin(), points_2d.end(), IK());
    auto diagonal = {IK::Point_3(bbox.xmin(), bbox.ymin(), 0),
                     IK::Point_3(bbox.xmax(), bbox.ymax(), 0)};
    double squared_diagonal_length = CGAL::squared_distance(
        IK::Point_3(bbox.xmin(), bbox.ymin(), 0), IK::Point_3(bbox.xmax(), bbox.ymax(), 0));
    ...
    for (auto &polyline : polylines_copy)
        for (int i = 0; i < polyline.size() - 1; i++) {
            ...
            int divisions = (int)std::min(
                100.0,
                std::sqrt(CGAL::squared_distance(polyline[i], polyline[i + 1])) / step_division);
            cgal::vector_util::interpolate_points(polyline[i], polyline[i + 1], divisions,
                                                  division_points, 2);
            division_points_temp.insert(division_points_temp.end(), division_points.begin(),
                                        division_points.end());
    ...
    // Orient Back to 3D
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    for (auto &point : rect) point = point.transform(xform_toXY_Inv);
    ...
    if (rectangle_division_distance > wood::GLOBALS::DISTANCE) {
        ...
        for (int i = 0; i < rect.size() - 1; i++) {
            division_points.clear();
            int divisions =
                (int)std::min(25.0, std::sqrt(CGAL::squared_distance(rect[i], rect[i + 1])) /
                                        rectangle_division_distance);
            cgal::vector_util::interpolate_points(rect[i], rect[i + 1], divisions, division_points,
                                                  2);
        ...
        int divisions_u =
            (int)(std::min(std::ceil(edge_dist_00 / unsigned_rectangle_division_distance),
                           std::ceil(edge_dist_01 / unsigned_rectangle_division_distance)));
        int divisions_v =
            (int)(std::min(std::ceil(edge_dist_10 / unsigned_rectangle_division_distance),
                           std::ceil(edge_dist_11 / unsigned_rectangle_division_distance)));
        ...
        cgal::vector_util::interpolate_points(rect[0], rect[1], divisions_u, division_points_edge_0,
                                              1);
        cgal::vector_util::interpolate_points(rect[3], rect[2], divisions_u, division_points_edge_2,
                                              1);

        points.reserve((divisions_u + 2) * (divisions_v + 2));
        for (int i = 0; i < (divisions_u + 2); i++) {
            std::vector<IK::Point_3> temp_divisions;
            cgal::vector_util::interpolate_points(division_points_edge_0[i],
                                                  division_points_edge_2[i], divisions_v,
                                                  temp_divisions, 1);
