...
void
interpolate_points (const IK::Point_3 &from, const IK::Point_3 &to, const int &steps, std::vector<IK::Point_3> &points, int type_0_noends_1_end_2_oneend)
{
                ...
                for (auto i = 1; i < steps + 1; i++)
                    {
                        ...
                        points.emplace_back (from.hx () + t * (to.hx () - from.hx ()), from.hy () + t * (to.hy () - from.hy ()), from.hz () + t * (to.hz () - from.hz ()));
                    }
            }
            break;
        case (1):
...
IK::Vector_3
get_leveled_vector (const IK::Vector_3 &vector, const double &vertical_height)
{
