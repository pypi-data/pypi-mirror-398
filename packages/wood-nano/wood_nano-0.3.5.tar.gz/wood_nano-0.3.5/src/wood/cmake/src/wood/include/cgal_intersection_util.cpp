...
int
is_parallel_to (const IK::Vector_3 &v0, const IK::Vector_3 &v1)
{
    int rc = 0;
...
int
solve3x3 (const double (&row0)[3], const double (&row1)[3], const double (&row2)[3], double d0, double d1, double d2, double *x_addr, double *y_addr, double *z_addr, double *pivot_ratio)
{
...
double
value_at (const IK::Plane_3 &Pl, const IK::Point_3 &P)
{
...
bool
line_line_3d (const IK::Segment_3 &cutter_line, const IK::Segment_3 &segment, IK::Point_3 &output)
{
...
bool
plane_plane_plane (const IK::Plane_3 &plane_0, const IK::Plane_3 &plane_1, const IK::Plane_3 &plane_2, IK::Point_3 &output)
{
    double pr = 0.0;
    double x, y, z;
    ...
    const int rank = internal::solve3x3 (
        ...
        plane_0_, plane_1_, plane_2_, -plane_0.d (), -plane_1.d (), -plane_2.d (), &x, &y, &z, &pr);
    output = IK::Point_3 (x, y, z);
    return (rank == 3) ? true : false;
}

bool
plane_plane (const IK::Plane_3 &plane_0, const IK::Plane_3 &plane_1, IK::Line_3 &output)
{
    ...
    // infinite line by give the direction of the 3-plane intersection as the cross product of the
    // first two planes
...
bool
line_plane (const IK::Segment_3 &line, const IK::Plane_3 &plane, IK::Point_3 &output, bool is_finite)
{
    bool rc = false;
    double a, b, d, fd, t;
    ...
    output = IK::Point_3 ((line[0].x () == line[1].x ()) ? line[0].x () : s * line[0].x () + t * line[1].x (), (line[0].y () == line[1].y ()) ? line[0].y () : s * line[0].y () + t * line[1].y (),
...
void
line_plane (const IK::Line_3 &line, const IK::Plane_3 &plane, IK::Point_3 &output, bool is_finite)
{
...
bool
get_quad_from_line_topbottomplanes (const IK::Plane_3 &collision_face, const IK::Segment_3 &line, const IK::Plane_3 &plane0, const IK::Plane_3 &plane1, CGAL_Polyline &output)
{
...
bool
scale_vector_to_distance_of_2planes (const IK::Vector_3 &dir, const IK::Plane_3 &plane0, const IK::Plane_3 &plane1, IK::Vector_3 &output)
{
    // check validity
...
bool
get_orthogonal_vector_between_two_plane_pairs (const IK::Plane_3 &plane_pair0_0, const IK::Plane_3 &plane_pair1_0, const IK::Plane_3 &plane_pair1_1, IK::Vector_3 &output)
{
    IK::Line_3 l0;
    IK::Line_3 l1;
...
bool
plane_4planes (const IK::Plane_3 &main_plane, const IK::Plane_3 (&sequence_of_planes)[4], CGAL_Polyline &output)
{
    IK::Point_3 p0, p1, p2, p3;
...
bool
plane_4planes_open (const IK::Plane_3 &main_plane, const IK::Plane_3 (&sequence_of_planes)[4], CGAL_Polyline &output)
{
    IK::Point_3 p0, p1, p2, p3;
    bool result0 = plane_plane_plane_with_parallel_check (sequence_of_planes[0], sequence_of_planes[1], main_plane, p0);
    bool result1 = plane_plane_plane_with_parallel_check (sequence_of_planes[1], sequence_of_planes[2], main_plane, p1);
...
bool
plane_4lines (IK::Plane_3 &plane, IK::Segment_3 &l0, IK::Segment_3 &l1, IK::Segment_3 &l2, IK::Segment_3 &l3, CGAL_Polyline &output)
{
    IK::Point_3 p0, p1, p2, p3;
...
bool
plane_plane_plane_with_parallel_check (const IK::Plane_3 &plane0, const IK::Plane_3 &plane1, const IK::Plane_3 &plane2, IK::Point_3 &output)
{
    ...
    bool flag = internal::is_parallel_to (plane0_v, plane1_v) == 0 && internal::is_parallel_to (plane0_v, plane2_v) == 0 && internal::is_parallel_to (plane1_v, plane2_v) == 0;
...
bool
line_two_planes (IK::Segment_3 &line, const IK::Plane_3 &plane0, const IK::Plane_3 &plane1)
{
    ...
    // line = IK::Segment_3(*boost::get<IK::Point_3>(&*result0),
    // *boost::get<IK::Point_3>(&*result1));

    // if (result)
    // {
    //     // std::cout << output_point.hx() << " " << output_point.hy() << " " << output_point.hz()
    //     << std::endl; points.emplace_back(output_point); edge_ids.emplace_back(i);
    // }

    // auto result0 = CGAL::intersection(line, plane0);
    // auto result1 = CGAL::intersection(line, plane1);
    // line = IK::Segment_3(*boost::get<IK::Point_3>(&*result0),
    // *boost::get<IK::Point_3>(&*result1)); return true;
}

bool
polyline_plane_to_line (const CGAL_Polyline &polyline, const IK::Plane_3 &plane, IK::Segment_3 &alignment_segment, IK::Segment_3 &output)
{
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool
polyline_plane (const CGAL_Polyline &polyline, const IK::Plane_3 &plane, std::vector<IK::Point_3> &points, std::vector<int> &edge_ids)
{
    for (auto i = 0; i < polyline.size () - 1; i++)
        {
            ...
            if (result)
                {
                    // std::cout << output_point.hx() << " " << output_point.hy() << " " <<
                    // output_point.hz() << std::endl;
...
bool
polyline_plane_cross_joint (CGAL_Polyline &c0, CGAL_Polyline &c1, IK::Plane_3 &p0, IK::Plane_3 &p1, IK::Segment_3 &line, std::pair<int, int> &pair)
{
    bool debug = false;
