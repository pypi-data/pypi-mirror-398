// this method belongs to https://github.com/mapbox/polylabel
...
#include <polylabel/envelope.h>
#include <polylabel/point.h>
#include <polylabel/point_arithmetic.h>
...
template <class T>
T getSegDistSq(const geometry::point<T> &p, const geometry::point<T> &a,
...
template <class T>
std::array<double, 3> polylabel(const geometry::polygon<T> &polygon, T precision = 1,
                                bool debug = false) {
    ...
    const geometry::point<T> size{envelope.max.x - envelope.min.x, envelope.max.y - envelope.min.y};

    const T cellSize = std::min(size.x, size.y);
    T h = cellSize / 2;

    // a priority queue of cells in order of their "potential" (max distance to polygon)
    auto compareMax = [](const Cell<T> &a, const Cell<T> &b) { return a.max < b.max; };
    using Queue = std::priority_queue<Cell<T>, std::vector<Cell<T>>, decltype(compareMax)>;
    Queue cellQueue(compareMax);

    if (cellSize == 0) {
        return {envelope.min.x, envelope.min.y, 0};  // envelope.min;
    }
    ...
    while (!cellQueue.empty()) {
        ...
        if (cell.d > bestCell.d) {
            bestCell = cell;
            if (debug)
                std::cout << "found best " << ::round(1e4 * cell.d) / 1e4 << " after " << numProbes
                          << " probes" << std::endl;
        }
    ...
    // return bestCell.c;
    return {bestCell.c.x, bestCell.c.y, bestCell.d};
}
