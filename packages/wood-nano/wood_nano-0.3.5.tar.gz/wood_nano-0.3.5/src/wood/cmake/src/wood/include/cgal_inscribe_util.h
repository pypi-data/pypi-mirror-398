...
//
// 3RD PARTY LIBRARIES:
// Get center with radius in a polygon, the polylabel folder contains all the data structures from:
// https://github.com/mapbox/polylabel CGAL for largest empty area:
 ...
 * WARNING user must not use this method - this method is used by
 * get_polylabel_circle_division_points method
 ...
 * @param [in] polylines list of polylines that representes the input polylines for the polylabel
 * algorithm
 ...
 */
void get_polylabel_circle_division_points (const IK::Vector_3 &division_direction_in_3d, const std::vector<CGAL_Polyline> &polylines, std::vector<IK::Point_3> &points, int division = 4, double scale = 0.75,
 ...
 * @param [in] scale the rectangle scale, since the algorithm works with points, it never returns
 * the exact touching with the edges
 ...
 * cretead, if above, the rectangle's edges are subdivided
 * @return bool flag if the result is valid
 */
bool inscribe_rectangle_in_convex_polygon (const std::vector<CGAL_Polyline> &polylines, CGAL_Polyline &result, std::vector<IK::Point_3> &points, IK::Segment_3 &direction, double scale = 0, double precision = 1.0,
