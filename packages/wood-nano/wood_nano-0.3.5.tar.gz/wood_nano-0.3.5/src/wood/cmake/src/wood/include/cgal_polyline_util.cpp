...
void
average_normal (const CGAL_Polyline &p, IK::Vector_3 &average_normal)
{
...
void
shift (CGAL_Polyline &pline, const int &times)
{
    pline.pop_back ();
...
double
polyline_length (CGAL_Polyline &pline)
{
    double length = 0;
...
double
polyline_length_squared (CGAL_Polyline &pline)
{
    double length = 0;
    for (auto i = 0; i < pline.size (); i++)
...
bool
line_line_overlap (const IK::Segment_3 &line0, const IK::Segment_3 &line1, IK::Segment_3 &output)
{
...
bool
line_from_projected_points (const IK::Segment_3 &line, const CGAL_Polyline &points, IK::Segment_3 &output)
{
    // reserve memory
...
void
line_line_average (const IK::Segment_3 &line0, const IK::Segment_3 &line1, IK::Segment_3 &output)
{
    output = IK::Segment_3 (CGAL::midpoint (line0[0], line1[0]), CGAL::midpoint (line0[1], line1[1]));
}

void
line_line_overlap_average (const IK::Segment_3 &line0, const IK::Segment_3 &line1, IK::Segment_3 &output)
{
    // get two overalps
    IK::Segment_3 lineA;
...
double
closest_distance_and_point (const IK::Point_3 &point, const CGAL_Polyline &polyline, size_t &edge_id, IK::Point_3 &closest_point)
{
    edge_id = 0;
...
IK::Vector_3
center_vec (const CGAL_Polyline &polyline)
{
    double x = 0, y = 0, z = 0;
    auto n = is_closed (polyline) ? polyline.size () - 1 : polyline.size ();
...
void
transform (CGAL_Polyline &polyline, const CGAL::Aff_transformation_3<IK> &transform)
{
...
void
get_average_plane (CGAL_Polyline &polyline, IK::Vector_3 (&plane_axes)[4])
{
    // Origin
...
void
get_fast_plane (const CGAL_Polyline &polyline, IK::Point_3 &origin, IK::Plane_3 &plane)
{
    // origin
    origin = polyline[0];
...
void
get_middle_line (const IK::Segment_3 &line0, const IK::Segment_3 &line1, IK::Segment_3 &output)
{
    IK::Point_3 p0 = IK::Point_3 ((line0[0].x () + line1[0].x ()) * 0.5, (line0[0].y () + line1[0].y ()) * 0.5, (line0[0].z () + line1[0].z ()) * 0.5);

    IK::Point_3 p1 = IK::Point_3 ((line0[1].x () + line1[1].x ()) * 0.5, (line0[1].y () + line1[1].y ()) * 0.5, (line0[1].z () + line1[1].z ()) * 0.5);

    output = IK::Segment_3 (p0, p1);
}

void
extend_line (IK::Segment_3 &line, const double &distance0, const double &distance1)
{
...
void
scale_line (IK::Segment_3 &line, const double &distance)
{
    IK::Point_3 p0 = line[0];
    IK::Point_3 p1 = line[1];
    IK::Vector_3 v = p1 - p0;
...
void
extend (CGAL_Polyline &pline, int sID, double dist0, double dist1, double proportion0, double proportion1)
{
...
void
extend_equally (IK::Segment_3 &segment, double dist, double proportion)
{
...
void
move (CGAL_Polyline &polyline, const IK::Vector_3 &direction)
{
...
bool
is_clockwise (CGAL_Polyline &polyline, IK::Plane_3 plane)
{
    // copy
...
void
flip (CGAL_Polyline &polyline)
{
    std::reverse (polyline.begin (), polyline.end ());
}

void
get_convex_corners (const CGAL_Polyline &polyline, std::vector<bool> &convex_or_concave)
{
...
CGAL_Polyline
tween_two_polylines (const CGAL_Polyline &polyline0, const CGAL_Polyline &polyline1, const double &weight)
{
