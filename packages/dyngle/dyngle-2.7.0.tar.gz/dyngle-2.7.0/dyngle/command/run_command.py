...
class RunCommand(DyngleCommand):
    ...
    def execute(self):
        ...
        if return_value is not None:
            output = self._format_return_value(return_value)
            return output
    
    def _format_return_value(self, value):
        """Format return value for output.
        
        Dicts and lists are formatted as YAML.
        Other types are converted to string.
        """
        if isinstance(value, (dict, list)):
            yaml_output = safe_dump(value, default_flow_style=False)
