...
from wood_wide_ai import WoodWideAI, AsyncWoodWideAI, DefaultAioHttpClient
from wood_wide_ai._utils import is_dict
...
logging.getLogger("wood_wide_ai").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[WoodWideAI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with WoodWideAI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncWoodWideAI]:
