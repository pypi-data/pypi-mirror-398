...
class NSeries:
    ...
    def register_session(self):
        # No longer required, but put in for backwards compatibility
        pass
...
class AsyncNSeries:
    ...
    def __init__(self, host=None, timeout=None):
        super().__init__()
        self.derived_data_type_dictionary = {}
        self.connected_cip_dispatcher = AsyncEIPConnectedCIPDispatcher()
        self.host = host
        self.timeout = timeout
    ...
    async def get_instance_list_service(self, tag_request_path: bytes, data: bytes):
        """
    ...
    async def update_variable_dictionary(self):
        """
    ...
    async def variable_dictionary(self, update_contents: bool = False):
        ...
        """
        if update_contents:
            for variable in await self.variable_list():
                await self.read_variable(variable)
        return self.connected_cip_dispatcher.variables

    async def user_variable_dictionary(self, update_contents: bool = False):
        """
        ...
        """
        if update_contents:
            for variable in await self.user_variable_list():
                await self.read_variable(variable)
        return self.connected_cip_dispatcher.user_variables

    async def system_variable_dictionary(self, update_contents: bool = False):
        """
        ...
        """
        if update_contents:
            for variable in await self.system_variable_list():
               await self.read_variable(variable)
        return self.connected_cip_dispatcher.system_variables

    async def save_current_dictionary(self, file_name: str):
        with (open(file_name, "wb")) as f:
            pickle.dump(self.connected_cip_dispatcher.variables, f)

    async def load_dictionary_file(self, file_name: str):
        with (open(file_name, "rb")) as f:
            self.connected_cip_dispatcher.variables = pickle.load(f)

    async def load_dictionary_file_if_present(self, file_name: str):
        """
        ...
        try:
            ...
            await self.update_variable_dictionary()
            await self.save_current_dictionary(file_name)

    async def _structure_instance_from_variable_type_object(
        ...
        nested_variable_type_object = await self._get_variable_type_object(nesting_id)
        ...
        while member_instance_id != 0:
            variable_type_object_reply = await self._get_variable_type_object(member_instance_id)
            member_cip_datatype_instance = await self._get_member_instance(member_instance_id, variable_type_object_reply)
    ...
    async def _array_instance_from_variable_name(self, variable_name: str) -> CIPArray:
        """
        ...
        if array_attributes_all_reply.cip_data_type_of_array == CIPStructure.data_type_code():
            array_member_instance = await self._get_member_instance(instance_id)
    ...
    async def _array_instance_from_variable_type_object(self, variable_type_object: VariableTypeObjectReply) -> CIPArray:
        """
        This method builds an array from information obtained from get_attribute_all on the
        variable_request_path
        :param variable_type_object:
        ...
        if variable_type_object.cip_data_type_of_array == CIPStructure.data_type_code():
            ...
            cip_data_type_instance = await self._string_instance_from_variable_type_object(variable_type_object)
            ...
            array_member_instance = \
                self.connected_cip_dispatcher.data_type_dictionary.get(
                    variable_type_object.cip_data_type_of_array)()
        cip_array_instance.from_instance(array_member_instance,
    ...
    @staticmethod
    async def _string_instance_from_variable_type_object(
            variable_object: VariableTypeObjectReply) -> CIPString:
        cip_string = CIPString()
        cip_string.size = variable_object.size
        return cip_string

    async def _get_instance_from_variable_name(self, variable_name: str):
        """
        ...
        if data_type_code == CIPStructure.data_type_code():
            ...
            cip_data_type_instance = await self._structure_instance_from_variable_type_object(variable_type_object)
            ...
            cip_data_type_instance = await self._array_instance_from_variable_name(variable_name)
    ...
    async def _get_member_instance(
        ...
        if variable_type_object_reply.cip_data_type == CIPStructure.data_type_code():
            return await self._structure_instance_from_variable_type_object(variable_type_object_reply)
            ...
            return await self._string_instance_from_variable_type_object(variable_type_object_reply)
        elif variable_type_object_reply.cip_data_type == CIPArray.data_type_code():
            return await self._array_instance_from_variable_type_object(variable_type_object_reply)
        else:
            return self.connected_cip_dispatcher.data_type_dictionary.get(variable_type_object_reply.cip_data_type)()

    async def _array_instance_from_variable_object(
        ...
        if variable_type_instance_id == b'\x00\x00\x00\x00':
            ...
            member_instance = await self._get_member_instance(member_instance_id)
            cip_array_instance = member_instance
        return cip_array_instance

    async def read_variable(self, variable_name: str) -> CIPDataType:
        """
        ...
        if isinstance(cip_data_type_instance, (CIPString, CIPArray, CIPStructure, CIPAbbreviatedStructure)):
            ...
            if (isinstance(cip_data_type_instance, CIPArray) and
                ...
                for index in range(cip_data_type_instance.number_of_elements[0]):
                    variable_response = await self.read_variable(f'{variable_name}[{index}]')
                    list_of_strings.append(variable_response.value())
                cip_data_type_instance.from_value(list_of_strings)
                return cip_data_type_instance
            else:
                return await self._multi_message_variable_read(cip_data_type_instance)
        else:
    ...
    async def write_variable(self, variable_name: str, data):
        """
        ...
        if cip_data_type_instance is None:
            ...
            if (isinstance(cip_data_type_instance, CIPArray) and
                ...
                for index in range(cip_data_type_instance.number_of_elements[0]):
                    await self.write_variable(f'{variable_name}[{index}]', data[index])
                return list_of_strings
            else:
                await self._multi_message_variable_write(cip_data_type_instance)
        else:
            request_data = CIPCommonFormat(cip_data_type_instance.data_type_code(), data=cip_data_type_instance.data)
            await self.connected_cip_dispatcher.write_tag_service(request_path, request_data)

    async def verified_write_variable(self, variable_name: str, data, retry_count: int = 2):
        remaining_retry = retry_count
        temp_data = None
        while remaining_retry >= 0 and temp_data != data:
            await self.write_variable(variable_name, data)
            temp_data = await self.read_variable(variable_name)
            remaining_retry += -1
    ...
    async def _multi_message_variable_read(self, cip_datatype_object: CIPDataType, offset=0) -> CIPDataType:
        """
        ...
        while offset < cip_datatype_object.size:
            ...
            response = await self._simple_data_segment_read(cip_datatype_object, offset, read_size)
    ...
    async def _multi_message_variable_write(self, cip_datatype_object: CIPDataType, offset=0) -> CIPReply:
        """
        ...
        while offset < cip_datatype_object.size:
            ...
            response = await self._simple_data_segment_write(
                cip_datatype_object, offset, write_size,
                cip_datatype_object.data[offset:offset + write_size])
            offset = offset + max_write_size
        return response

    async def _simple_data_segment_read(self, cip_datatype_object: CIPDataType, offset, read_size) -> CIPReply:
        """
    ...
    async def _get_variable_type_object(self, instance_id: int) -> VariableTypeObjectReply:
        """
        ...
        variable_type_object_reply_value = await self.connected_cip_dispatcher.get_attribute_all_service(request_path)
        variable_type_object_reply = VariableTypeObjectReply(variable_type_object_reply_value.bytes)
        return variable_type_object_reply

    async def _get_number_of_derived_data_types(self) -> int:
    ...
    async def _get_variable_list(self):
        """
    ...
    async def _get_instance_list_subset(self, user_defined: bool):
        ...
        while not all_received:
            reply = await self.connected_cip_dispatcher.get_instance_list(current_instance, 100, user_defined)
    ...
    async def _get_variable_list_subset(self, user_defined: bool):
        # all_received = False
        instances = await self._get_instance_list_subset(user_defined)
        tag_list = []
        for instance in instances:
            tag_list.append(instance.tag_name())
        return tag_list

    async def _get_number_of_variables(self) -> int:
        """
