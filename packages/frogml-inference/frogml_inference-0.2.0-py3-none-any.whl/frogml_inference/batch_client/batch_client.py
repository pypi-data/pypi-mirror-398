...
from frogml_inference.batch_client.cloud_provider_client import (
    DEFAULT_ENVIRONMENT_CLOUD,
    CloudProviderClient,
)
from frogml_inference.batch_client.file_serialization import (
    SERIALIZATION_HANDLER_MAP,
    SerializationFormat,
)
from frogml_inference.batch_client.instance_validation import verify_template_id
from frogml_inference.batch_client.s3 import S3Utils
...
class BatchInferenceClient:
    ...
    def warm_up(
        ...
        if not warmup_result.success:
            ...
            raise FrogmlException(error_message)
    ...
    def run(
        ...
        try:
            ...
            raise FrogmlException(interrupted_msg)
    ...
    def __wait_for_non_intermediate_response(
        ...
        If the execution status is not success, a 'FrogmlException' is raised.
        ...
        :raises FrogmlException: If the execution status is not success in `self.get_status_response`.
        """
    ...
    def get_job_status(self, execution_id: str, timeout_seconds: int = 0) -> str:
        ...
        :raise: FrogmlException if the status is not part of the 'SUCCESS_STATES' or 'FAILURE_STATES'
        """
        try:
            response: ExecutionStatusResult = self.__wait_for_non_intermediate_response(
                execution_id, timeout_seconds
            )
        except (RetryError, FrogmlException) as e:
        ...
        raise FrogmlException(f"Status {status} is not success or failure")

    @retry(
        retry=retry_if_exception_type(FrogmlException),
