...
chrome.storage.local.get(['activeSession'], (result) => {
  ...
  // Pre-populate comments with notes from recording session
  const commentsField = document.getElementById('comments');
  if (commentsField && activeSession.notes) {
    commentsField.value = activeSession.notes;
  }
...
function selectOutcome(outcome) {
  ...
  // Show/hide "required" indicator for reject
  const requiredIndicator = document.getElementById('comments-required');
  if (requiredIndicator) {
    requiredIndicator.style.display = outcome === 'reject' ? 'inline' : 'none';
  }
