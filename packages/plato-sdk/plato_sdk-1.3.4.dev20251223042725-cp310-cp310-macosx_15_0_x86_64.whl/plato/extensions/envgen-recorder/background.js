...
async function togglePauseMarker() {
    ...
    try {
        ...
        if (isPaused) {
            // Add pause marker - recording continues but user is taking a break
            pauseMarkerCount++;
            ...
            // Hide attention indicator when paused (user is on break)
            try {
                await chrome.tabs.sendMessage(activeTabId, {
                    type: 'HIDE_ATTENTION_INDICATOR'
                });
                console.log('[EnvGen] Attention indicator hidden (paused) on tab:', activeTabId);
            } catch (e) {
                console.warn('[EnvGen] Could not hide attention indicator:', e);
            }
        } else {
            // Add resume marker - user is actively recording now
            ...
            // Show attention indicator when resumed (pay attention, recording is active!)
            try {
                await chrome.tabs.sendMessage(activeTabId, {
                    type: 'SHOW_ATTENTION_INDICATOR'
                });
                console.log('[EnvGen] Attention indicator shown (recording active) on tab:', activeTabId);
            } catch (e) {
                console.warn('[EnvGen] Could not show attention indicator:', e);
...
async function startRecording(tabId, sessionId = null) {
    ...
    try {
        ...
        // isPaused will be set to true at the end after adding initial pause marker
        pauseMarkerCount = 0;
        ...
        try {
            ...
            // This happens when activeTab permission is not granted for current page
            // (e.g., user navigated to a different subdomain after opening extension)
            console.error('[EnvGen] Tab capture failed:', captureError.message);
            if (captureError.message.includes('activeTab') || captureError.message.includes('invoked')) {
                throw new Error('Please click the extension icon (puzzle piece → EnvGen) on this page to enable recording, then try again.');
            }
            throw captureError;
        }
        ...
        isRecording = true;
        isPaused = true; // Start in paused state (yellow button) - no attention indicator
        pauseMarkerCount = 1;

        // Add initial pause marker
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        let currentUrl = '';
        try {
        ...
        allEvents.push({
            ...
            metadata: {
                marker_number: 1,
                description: 'Recording started in paused state'
            }
        });

        // Don't show attention indicator - we start paused
        // Attention indicator will show when user resumes

        console.log('[EnvGen] Recording started (paused - resume to show attention indicator)');
...
async function stopRecording() {
    ...
    try {
        ...
        // Hide attention indicator when recording stops
        try {
