...
function setupNotesField(fieldId) {
  const field = document.getElementById(fieldId);
  if (!field) return;

  // Load existing notes
  field.value = activeSession?.notes || '';

  // Save notes on input (debounced)
  field.addEventListener('input', () => {
    clearTimeout(notesTimeout);
    notesTimeout = setTimeout(async () => {
      activeSession.notes = field.value;
      await chrome.storage.local.set({ activeSession });
      console.log('[EnvGen] Notes saved');

      // Sync other notes field
      const otherId = fieldId === 'recording-notes' ? 'post-recording-notes-field' : 'recording-notes';
...
chrome.storage.local.get(['activeSession'], (result) => {
  ...
  // Notes fields - save as user types (both during and after recording)
  setupNotesField('recording-notes');
  setupNotesField('post-recording-notes-field');
...
async function startRecording() {
  ...
  // Initialize pause button state (paused - yellow) - recording starts paused
...
function updatePauseButtonState(isPaused) {
  ...
  if (isPaused) {
    ...
    pauseBtn.textContent = '🎯 Focus Off (Cmd+Shift+Space)';
    pauseBtn.style.backgroundColor = '#f59e0b'; // Yellow when unfocused
    pauseBtn.style.borderColor = '#f59e0b';
    pauseBtn.classList.remove('btn-success');
    pauseBtn.classList.add('btn-warning');
  } else {
    // Focus On state: Green - user is focused on task
    pauseBtn.textContent = '🎯 Focus On (Cmd+Shift+Space)';
    pauseBtn.style.backgroundColor = '#10b981'; // Green when focused
...
function renderRecording() {
  ...
  const postRecordingNotes = document.getElementById('post-recording-notes');

  if (!recording) {
    container.innerHTML = '';
    if (noRecordingMsg) noRecordingMsg.style.display = 'block';
    if (postRecordingNotes) postRecordingNotes.style.display = 'none';
    return;
  }

  if (noRecordingMsg) noRecordingMsg.style.display = 'none';

  // Show post-recording notes section when recording exists and not actively recording
  if (postRecordingNotes && !isRecording) {
    postRecordingNotes.style.display = 'block';
    // Sync notes value
    const postNotesField = document.getElementById('post-recording-notes-field');
    if (postNotesField) {
      postNotesField.value = activeSession.notes || '';
    }
  } else if (postRecordingNotes) {
    postRecordingNotes.style.display = 'none';
  }
