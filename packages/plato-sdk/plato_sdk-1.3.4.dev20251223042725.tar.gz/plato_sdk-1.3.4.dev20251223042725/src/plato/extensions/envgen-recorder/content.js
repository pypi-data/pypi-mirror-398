...
(function () {
    ...
    const handleEvent = (e) => {
        ...
        // Check for pause marker keyboard shortcut (Cmd+Shift+Space)
        if (e.type === 'keydown') {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.type === 'GET_EVENTS') {
            ...
            return false; // Synchronous response
        } else if (message.type === 'SHOW_ATTENTION_INDICATOR') {
            // Show attention indicator when recording is ACTIVE (not paused)
            console.log('[EnvGen] Received SHOW_ATTENTION_INDICATOR message');

            const showAttentionIndicator = () => {
                // Remove existing indicator if present
                const existing = document.getElementById('envgen-attention-indicator');
                if (existing) {
                    existing.remove();
                }

                const indicator = document.createElement('div');
                indicator.id = 'envgen-attention-indicator';
                indicator.style.cssText = `
                    ...
                    text-align: center;
                    opacity: 0.9;
                    animation: envgen-pulse 2s ease-in-out infinite;
                `;
                indicator.innerHTML = '🎯 FOCUS ON';

                // Add pulse animation
                if (!document.getElementById('envgen-attention-style')) {
                    const style = document.createElement('style');
                    style.id = 'envgen-attention-style';
                    style.textContent = `
                        @keyframes envgen-pulse {
                ...
                console.log('[EnvGen] Attention indicator shown');
            };
            ...
            sendResponse({ success: true });
            return false; // Synchronous response
        } else if (message.type === 'HIDE_ATTENTION_INDICATOR') {
            // Hide attention indicator when paused
            console.log('[EnvGen] Received HIDE_ATTENTION_INDICATOR message');
            const indicator = document.getElementById('envgen-attention-indicator');
            if (indicator) {
                indicator.remove();
                console.log('[EnvGen] Attention indicator hidden');
            }
            sendResponse({ success: true });
            return false; // Synchronous response
