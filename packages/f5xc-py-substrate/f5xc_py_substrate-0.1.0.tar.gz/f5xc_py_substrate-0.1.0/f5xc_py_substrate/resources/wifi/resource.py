...
from f5xc_py_substrate.resources.wifi.models import (
    WifiListItem,
    Wifi,
    SecurityNone,
...
class WifiResource:
    ...
    def get_config(
        ...
        Get WIFI configuration from the node
        """
        ...
        try:
            ...
            raise F5XCValidationError("wifi", "get_config", e, response) from e
    ...
    def update_config(
        ...
        """Update Config for wifi.

        Update WIFI configuration on the node
        """
        path = "/api/operate/namespaces/{namespace}/sites/{site}/vpm/wifi/{node}/config"
        path = path.replace("{namespace}", namespace)
        path = path.replace("{site}", site)
        ...
        try:
            ...
            raise F5XCValidationError("wifi", "update_config", e, response) from e
    ...
    def disconnect(
        ...
        Disconnect the node from WIFI network
        """
        ...
        try:
            ...
            raise F5XCValidationError("wifi", "disconnect", e, response) from e
    ...
    def show_info(
        ...
        try:
            ...
            raise F5XCValidationError("wifi", "show_info", e, response) from e
    ...
    def list(
        ...
        """List wifi resources in a namespace.
        ...
        try:
            return [WifiListItem(**item) for item in items]
        except ValidationError as e:
            raise F5XCValidationError("wifi", "list", e, response) from e
