...
def generate_pheno_plink_fast_non_do(
    ...
    # CRITICAL: Match Hao's strain name normalization
    # He converts "." to "/" in strain names (see R code line 70)
    ph["strain"] = ph["strain"].astype(str).str.replace(" ", "", regex=False)
    ph["strain"] = ph["strain"].str.replace(".", "/", regex=False)  # NEW LINE
    ...
    with open(ped_file, "rb") as f:
        while True:
            ...
            # CRITICAL: Also normalize PED strain names to match
            name = name.replace(".", "/")  # NEW LINE
...
# ) -> pd.DataFrame:
#     """
#     Not matching Hao's kinship exactly
#     NON-DO matches Hao:
#       - join by strain
#       - FID = strain
#       - IID = paste0(strain,"_",animal_id)
