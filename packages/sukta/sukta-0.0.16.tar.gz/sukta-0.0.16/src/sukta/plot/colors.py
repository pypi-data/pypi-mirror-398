...
class TurboColormap:
    ...
    def interpolate_or_clip(cls, x):
        if x < 0.0:
            ...
            return cls.interpolate(x)

    @classmethod
    def interpolate_arr(cls, x, vmin=0.0, vmax=1.0, clip=True):
        assert vmax > vmin, "vmax must be greater than vmin"
        colormap = cls.colormap_data
        lower = x < vmin
        upper = x > vmax
        y = (np.clip(x, vmin, vmax) - vmin) / (vmax - vmin)
        a = (y * 255.0).astype(np.int32)
        b = np.minimum(255, a + 1)
        f = y * 255.0 - a
        A = np.take(colormap, a, axis=0)
        B = np.take(colormap, b, axis=0)
        C = A + (B - A) * f[..., None]
        if clip:
            C[lower] = [0.0, 0.0, 0.0]
            C[upper] = [1.0, 1.0, 1.0]
    ...
    @classmethod
    def cmap(cls):
        return ListedColormap(cls.colormap_data)

    @classmethod
    def register_turbocmap(cls):
        try:
            mpl.colormaps.register(name=cls.name, cmap=cls.cmap())
        except ValueError:
            pass
