...
def _apply_regionalized_adjustments(crop: Crop, crop_name: str) -> Crop:
    """Apply regionalized adjustments to fit local calendars.

    This aligns key time-dependent parameters with the configured
    local growing period, optionally applying a chosen adjustment strategy and
    per-crop overrides. Supports multiple crop types including Wheat, Maize,
    and PaddyRice.
    ...
    # Supported crop types for regionalized adjustments
    # Note: This function now supports all crop types, not just Wheat
    supported_crops = {"Wheat", "Maize", "PaddyRice", "Barley", "Sorghum"}
    if crop.Name not in supported_crops:
        logger.debug(
            f"Skipping regionalized adjustments for {crop.Name} "
            f"(not in supported crops: {supported_crops})"
        )
        # Still apply custom_params if provided, even for unsupported crops
