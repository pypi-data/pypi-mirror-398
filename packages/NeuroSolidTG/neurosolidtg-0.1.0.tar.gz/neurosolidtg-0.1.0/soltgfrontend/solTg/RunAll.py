...
def init():
    global SOURCE_PATH, SANDBOX_DIR, OUTPUTDIR, TIMEOUT, APIKEY, MOD, K
...
def main_pipeline(files):
    ...
    for i, f in enumerate(sorted(files)):
        ...
        SolidityTestGen.main(f, TIMEOUT, APIKEY, MOD, K)
...
def main():
    start_time = time.time()
    init()
    global SOURCE_PATH, SANDBOX_DIR, OUTPUTDIR, RERUN, TIMEOUT, APIKEY, MOD , K
    ...
    mode_arg = ['--mode']
    kwmode = {'type': str, 'help': 'mode 0 ==> SolidTG, mode 1 ==> NeuroSolidTG'}

    k_arg = ['--k']
    kwk = {'type': str, 'help': 'For NeuroSolidTG, to specify how many of the top query sequences should be taken.'}
    ...
    if args.apiKey:
        ...
        raise ValueError("Missing required argument: --apiKey, ANTHROPIC API KEY NEEDED to run  NeuroSolidTG")
        exit()
