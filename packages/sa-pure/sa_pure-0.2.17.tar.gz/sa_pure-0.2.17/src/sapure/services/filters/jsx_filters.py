...
class Join(Query):
    ...
    def body_build(self) -> Tuple[str, dict]:
        fields_str = f"||{','.join(self._fields)}" if self._fields else ""
        return "join", {"field": f"{self._relation}{fields_str}"}
...
class Fields(Query):
    ...
    def build(self) -> str:
        return f"fields={','.join(self._fields)}" if self._fields else ""

    def body_build(self) -> Tuple[str, list]:
