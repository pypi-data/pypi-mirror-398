...
品質のよいテストを書いて初めて品質の良いコードになる。例えば保守がしやすいとか拡張しやすいとか←これも副産物。一番は「持続可能な開発」を目指す事。これを目指しているから保守がしやすかったり拡張がしやすくなる。

「検索機能は標準機能だからテストしない」というのはおかしな話。
検索という機能を大きな「単体」で見た時、それをどう使ってどう表示するかは作った側でテストしなければならない。
「A」を入れてボタンを押したのに全然違う結果が返ってきて、よく見たら入力フォームの紐づけに誤りがあったという事もある。
検索機能に値を正しく渡せているかという観点が抜けている。

# 文字の色を単体テストに入れるべきか？
NGが1つでもあったら赤になっている事」みたいなテストが必要か？
これをやるにはメッセージを渡して色判定をもらった後のデータを出力メソッドに投げるひつようがある。
つまりAPIが2個になるので設計としては良くない。

# パブリッシュ
`pipenv run hatch publish -u __token__ -a TOKEN -r test`