# Reaction Rate Field Analysis: Reaction Systems
...
class ReactionSystem:
    ...
    Returns:
        A ``ReactionSystem`` object containing the specified components,
            reactions, stoichiometry, kinetics, and inequality constraints.
    """
    ...
    def out_of_space(self, t, y):
        """
        Determine if a composition point y is outside the specified
            inequality constraints.

        Note:
            This function is used in PFR integration.
        """

        vals = np.asarray(self.inequality_status(y))
        vals2 = np.atleast_2d(vals)

        # return < 0 if point is inside space, > 0 if outside
        if vals2.shape[0] == 1:
            return np.max(vals2[0])  # scalar y
        else:
            return np.max(vals, axis=1)  # vector y
