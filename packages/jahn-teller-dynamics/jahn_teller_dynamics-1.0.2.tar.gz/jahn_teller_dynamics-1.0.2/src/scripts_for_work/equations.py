...
plt.text(1.8, 18.0, r'$\hat{H} = \lambda_{\text{theory}}\hat{L}_{z}\hat{S}_{z} + \mu_{\text{B}}\left( fB_z\hat{L}_z + 2\delta_{f}B_{z} S_{z} \right) + \mu_{\text{B}} g_{S} \left( B_x \hat{S}^{\text{eff}}_x + B_y \hat{S}^{\text{eff}}_y + B_{z}\hat{S}^{\text{eff}}_{z} \right)$', fontsize = 30)
