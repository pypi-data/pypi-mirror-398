...
fn test_insert_query_hybrid_mode() {
    let params = HNSWParams::default();
    let mut index =
        HNSWIndex::new_with_storage(3, params, DistanceFunction::L2, false, StorageMode::Hybrid)
            .unwrap();
...
fn test_error_propagation_in_hybrid_mode() {
    ...
    let result = index.insert(&[1.0, 2.0]); // Wrong dimensions
    assert!(result.is_err());
}
...
fn test_save_graph_to_disk() {
    ...
    for i in 0..100 {
        index
            .insert(&[i as f32, i as f32 + 1.0, i as f32 + 2.0])
            .unwrap();
    }
