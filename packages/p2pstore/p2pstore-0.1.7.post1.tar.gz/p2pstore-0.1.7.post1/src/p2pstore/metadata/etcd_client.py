...
class EtcdMetadataClient(MetadataClient):
    ...
    def __init__(
        self, etcd_endpoints: list[str], local_ip: str, enable_watch: bool = True
    ):
        ...
        # 本地缓存 (用于 list_files 和 unregister 回调)
        self.file_map = {}
    ...
    def _connect_etcd(self):
        ...
        try:
            ...
            # 默认 4MB -> 512MB (支持 ~200 万条文件元数据，每条 ~250 bytes)
    ...
    def _watch_files(self):
        ...
        events_iterator, cancel = self._client.watch_prefix(self.PREFIX_FILE)
            ...
            for event in events_iterator:
                ...
                try:
                    ...
                    if isinstance(event, etcd3.events.PutEvent):
                        ...
                        # 检测是否为覆盖写
                        is_update = file_key in self.file_map
                        if is_update:
                            old_host = self.file_map[file_key].get("host", "unknown")
                            self.logger.debug(
                                "[ETCD-WATCH] 文件更新: key=%s, old_host=%s, new_host=%s",
                                file_key,
                                old_host,
                                host,
                            )
                            ...
                            self.logger.debug(
                                "[ETCD-WATCH] 文件注册: key=%s, host=%s", file_key, host
                            )

                        self.file_map[file_key] = value

                    elif isinstance(event, etcd3.events.DeleteEvent):
                        # 文件删除
                        if file_key in self.file_map:
                            deleted_host = self.file_map[file_key].get(
                                "host", "unknown"
                            )
                            self.logger.info(
                                "[ETCD-WATCH] 文件删除: key=%s, host=%s, file_map_size=%d",
                                file_key,
                                deleted_host,
                                len(self.file_map) - 1,
                            )
                            ...
                            if self._release_callback:
                                self.logger.debug(
                                    "[ETCD-WATCH] 触发释放回调: key=%s", file_key
                                )
                            ...
                            self.logger.debug(
                                "[ETCD-WATCH] 收到删除事件但 key=%s 不在 file_map (可能已被主线程删除)",
                                file_key,
                            )
                except Exception as e:
    ...
    def _try_recover_lease(self) -> bool:
        ...
        with self._lease_recovery_lock:
            ...
            try:
                ...
                # 重新创建 Lease
                self.lease = self._client.lease(ttl=60)
    ...
    def register_provider(self, host: str) -> None:
        ...
        try:
            ...
            self._client.put(key, value, lease=self.lease)
            self.logger.info("Provider 注册成功 (Lease ID: %x)", self.lease_id)
    ...
    def _keep_alive_loop(self):
        ...
        while self.running:
            ...
            try:
                ...
                time.sleep(10)  # 每 10 秒刷新一次（TTL 60s）
    ...
    def register_file(self, file_key: str, host: str, metadata: dict) -> bool:
        ...
        try:
            ...
            self._client.put(key, value, lease=self.lease)
            # 更新本地缓存 (虽然 watch 会更新，但本地先更新反应更快)
            self.file_map[file_key] = data
            self.logger.debug(
                "[ETCD] 文件元数据已写入: key=%s, file_map_size=%d",
                file_key,
                len(self.file_map),
            )
            return True
    ...
    def delete_file(self, file_key: str) -> bool:
        ...
        # 记录删除前的状态
        in_file_map = file_key in self.file_map
        host = (
            self.file_map.get(file_key, {}).get("host", "unknown")
            if in_file_map
            else "unknown"
        )
        self.logger.debug(
            "[ETCD] 删除文件: key=%s, in_file_map=%s, host=%s",
            file_key,
            in_file_map,
            host,
        )
        ...
        try:
            ...
            if success:
                ...
                # 不删除 file_map，让 Watch 线程统一处理释放回调
                return True
            else:
                self.logger.warning(
                    "[ETCD] Etcd 删除返回 False: key=%s (可能已不存在)", file_key
                )
            return False
        except Exception as e:
            self.logger.error("[ETCD] 删除文件异常: key=%s, error=%s", file_key, e)
            return False
    ...
    def list_files(self) -> dict[str, dict]:
        ...
        try:
            ...
            self.logger.warning(
                "[ETCD] list_files 查询异常，返回本地缓存: %s, cache_size=%d",
                e,
                len(self.file_map),
            )
    ...
    def clear_files(self) -> dict:
        ...
        try:
            ...
            self.logger.info("[ETCD] 开始清空文件: total=%d (查询自 Etcd)", count)
            ...
            # 设计说明：此处不清空 file_map，依赖 Watch 线程逐个处理 DeleteEvent
            # - Watch 收到 DeleteEvent 时会检查 file_map，只有存在时才触发 _release_callback
            # - 这保证了 RDMA 内存能正确释放（通过 transport.release()）
            # - 如果这里提前清空 file_map，Watch 线程将无法识别需要释放的 key，最终导致 RDMA 内存泄漏。

            success_count = count - len(failed_keys)
            self.logger.info(
                "[ETCD] 批量删除请求完成: 成功 %d/%d. "
                "注意: 本地 file_map 未立即清除，将等待 Watch 线程收到 DeleteEvent 后逐个触发释放回调.",
                success_count,
                count,
            )
            return {
