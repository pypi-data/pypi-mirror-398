...
async def query_order_itinerary(
        *, page: MyTicketOrderDetailPage, logger: Logger, pre_order_id: str, timeout: float = 20.0
) -> List[Dict[str, Any]]:
    ...
    for passenger_locator in tabs_items:
        ...
        except (PlaywrightError, PlaywrightTimeoutError, RuntimeError, Exception):
            pass
        data.append(dict(
...
async def first_open_page_query_order_itinerary(
    ...
    if "交易成功" in order_state:
        ...
        return await query_order_itinerary(
            logger=logger, timeout=timeout, page=order_detail_po, pre_order_id=pre_order_id
        )
    else:
