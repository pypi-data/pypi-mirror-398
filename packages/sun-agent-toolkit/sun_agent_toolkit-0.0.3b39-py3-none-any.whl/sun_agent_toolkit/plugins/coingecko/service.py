...
class CoinGeckoService:
    ...
    async def get_trending_coins(self, parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    async def get_coin_price(self, parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    async def search_coins(self, parameters: dict[str, Any]) -> dict[str, Any]:
        """Search for coins on CoinGecko"""
        params = {"query": parameters["query"], "x_cg_demo_api_key": self.api_key}
            ...
            async with session.get(url, params=params) as response:
                ...
                if parameters["exact_match"]:
                    ...
                    exact_matches = [
                        coin
                        for coin in coins
                        if coin.get("id") == parameters["query"]
                        or coin.get("symbol", "").lower() == parameters["query"].lower()
                        or coin.get("name", "").lower() == parameters["query"].lower()
                    ]
