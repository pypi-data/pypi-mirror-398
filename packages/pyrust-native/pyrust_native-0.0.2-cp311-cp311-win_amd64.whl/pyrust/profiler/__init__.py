    ...
    def profile_path(
        ...
        *,
        ...
        - ``include_stdlib`` permite incluir o no llamadas provenientes de la stdlib en el
          ...
          cuando se activa).
        - ``include_site_packages`` permite incluir dependencias externas instaladas en
          ``site-packages``/``dist-packages`` y dentro de entornos virtuales.
    ...
    def _make_should_trace(
        ...
        *,
        ...
        def _should_trace(filename: str) -> bool:
            ...
            if is_stdlib:
                return include_stdlib
            in_vendor = any(marker in path.parts for marker in vendor_markers)
            in_venv_only = venv_root != base_prefix and venv_root in path.parents and base_prefix not in path.parents
            if in_vendor or in_venv_only:
                return include_site_packages
