...
from src.core.pypi_client import PyPIClient, VersionCandidate
...
class Resolver:
    def __init__(self) -> None:
        ...
        self.peer_collector = PeerConstraintCollector(self.pypi_client)
    ...
    async def analyze_project(
        ...
        peer_constraints = await self.peer_collector.collect_peer_constraints(
            dependencies, dev_dependencies
        )

        peer_conflicts: dict[str, list[dict[str, str]]] = {}
        ...
        # Build or load version cache
        version_cache, cache_is_fresh = await self.cache_manager.build_cache(
            pyproject_path,
            important_packages,
            python_req,
            vendor_constraints,
        ...
        for pkg_name, current_spec in important_packages.items():
            try:
                # Use cached versions if cache is fresh and package is cached
                if cache_is_fresh and pkg_name in version_cache.get("packages", {}):
                    cached_pkg = version_cache["packages"][pkg_name]
                    available = cached_pkg.get("available", [])
                    if available:
                        versions = [
                            VersionCandidate(version=v, filename="", file_type="unknown")
                            for v in available
                        ]
                        logger.debug(f"Using cached versions for {pkg_name}")
                    else:
                        versions = await self.pypi_client.get_package_versions(
                            pkg_name, python_constraint=pep440_python_constraint
                        )
                else:
                    # No cache or package not in cache - fetch from PyPI
                    versions = await self.pypi_client.get_package_versions(
                        pkg_name, python_constraint=pep440_python_constraint
                    )
                ...
                for v in versions[:10]:
                    ...
                    conflicts = self.peer_collector.check_conflicts(
                        pkg_name, v.version, peer_constraints
                    )
                    if conflicts:
                        logger.debug(
                            f"Skipping {pkg_name} {v.version}: conflicts with peer dependencies"
                        )
                        # Track the conflict for the highest version
                        if pkg_name not in peer_conflicts:
                            peer_conflicts[pkg_name] = conflicts
                        continue
        ...
        return {
            ...
            "summary": {
                ...
                "peer_conflicts": len(peer_conflicts),
            },
