...
class ReportGenerator:
    ...
    def print_report(self, result: AnalysisResult, args: ArgumentConfig) -> None:
        if args.json_output:
            ...
            if result.peer_conflicts:
                data["peer_conflicts"] = result.peer_conflicts
        ...
        if result.summary:
            ...
            if result.summary.get("peer_conflicts", 0) > 0:
                summary_items.append(
                    ("Peer conflicts", str(result.summary.get("peer_conflicts", 0)))
                )
        ...
        if result.peer_conflicts:
            console.print("[bold yellow]! Peer Dependency Conflicts:[/bold yellow]")
            console.print(
                "  [dim]The following packages could not be updated due to constraints "
                "from other dependencies:[/dim]\n"
            )
            for pkg, conflicts in result.peer_conflicts.items():
                display_pkg = pkg.replace("__dev", " [dim](dev)[/dim]")
                console.print(f"  * [cyan]{display_pkg}[/cyan]:")
                for conflict in conflicts:
                    dependent = conflict.get("dependent", "?")
                    constraint = conflict.get("constraint", "?")
                    spec = conflict.get("spec", "?")
                    console.print(
                        f"    - [yellow]{dependent}[/yellow] [dim]{spec}[/dim] "
                        f"requires [bold]{pkg} {constraint}[/bold]"
                    )
            console.print()
        ...
        if result.report and args.verbose:
            ...
            for _idx, item in enumerate(result.report[:20]):  # Show first 20 items
                ...
                if item.status == "update":
                    ...
                    console.print(f"[pkg] [cyan]{pkg_display}[/cyan]: [dim]vendor dependency[/dim]")
                elif item.status == "error":
                    console.print(
                        f"[x] [cyan]{pkg_display}[/cyan]: [red]{item.reason or 'error'}[/red]"
                    )
