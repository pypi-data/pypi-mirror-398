...
- **Peer Dependency Conflict Detection**
  - Detects when recommended updates would conflict with peer dependencies of other packages
  - Fetches `requires_dist` from PyPI to build a map of package constraints
  - When a conflict is detected, Depchk tries progressively older versions until it finds a compatible one
  - Reports conflicts in summary ("Peer conflicts: N") and dependency section
  - Ignores extra-only dependencies (e.g., `pytest~=6.0; extra == 'test'`)
  - New `PeerConstraintCollector` class in `src/analyzer/peer_constraints.py`
  - Example: `mdformat ^1.0.0` blocked because `mdformat-tables ^1.0.0` requires `mdformat <0.8.0`

## [0.9.0] - 2025-12-22

### Fixed

- **Version Cache Not Used for PyPI Lookups**
  - Fixed critical issue where cached version data was not used to skip PyPI API calls
  - Now uses cached versions when cache is fresh (within 24-hour TTL), significantly reducing analysis time
  - Added `cache_is_fresh` flag to `VersionCache.build_cache()` return value
...
- **Updated Dev Dependencies**
  ...
  - ruff: ^0.8.0 -> ^0.14.10
  - bandit: ^1.8.6 -> ^1.9.2
  - Note: mdformat/mdformat-gfm remain pinned due to mdformat-tables compatibility
