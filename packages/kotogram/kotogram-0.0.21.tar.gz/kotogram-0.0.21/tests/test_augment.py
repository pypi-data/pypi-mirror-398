...
def test_verb_politeness_rule():
    ...
    variants = {
        tuple(t if isinstance(t, str) else t.surface for t in res) for res in results
    }
    ...
    variants_ichidan = {
        tuple(t if isinstance(t, str) else t.surface for t in res)
        for res in results_ichidan
    }
    ...
    variants_polite = {
        tuple(t if isinstance(t, str) else t.get("surface", t) for t in res)
        for res in results_polite
    }
...
def test_ranuki_rule():
    ...
    v = Token(
        "食べ",
        {"pos": "verb", "conjugated_type": "ichidan", "conjugated_form": "irrealis"},
    )
...
def test_adjective_politeness_rule():
    ...
    adj = Token(
        "美しい",
        {"pos": "adj", "conjugated_type": "i-adjective", "conjugated_form": "terminal"},
    )
...
@patch("kotogram.analysis._load_style_model")
def test_augment_full_flow(
    mock_load, mock_grammars, mock_parser_cls, mock_extract, mock_split
):
    ...
    mock_extract.side_effect = [
        TokenFeatures(surface=s) for s in surfaces
    ] * 100  # ample iterator
