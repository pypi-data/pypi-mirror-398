...
def sync_endpoint(config: Config, portainer_helper: PortainerHelper, endpoint_name: str,
    ...
    endpoint_id = portainer_helper.get_endpoint_id(endpoint_name)
    ...
    for stack_name in all_stack_names:
        ...
        # If the stack exists in Portainer but not the provided config, log this but do not interact with it.
        if not config_info:
        ...
        # If Portainer had existing config attempt to update it, otherwise deploy it from scratch.
        if portainer_info:
            update_existing_stack(
                config=config,
                portainer_helper=portainer_helper,
                portainer_info=portainer_info,
...
def deploy_new_stack(
    config: Config, portainer_helper: PortainerHelper, endpoint_name: str,
    endpoint_id: int, stack_name: str, push: bool,
) -> None:
    """ Deploy a new stack on Portainer with stack information defined in config.

    Args:
        config (Config): Config model with definitions of stacks for the given endpoint.
        portainer_helper (PortainerHelper): Portainer helper instance to interact with Portainer.
        endpoint_name (str): Friendly name of the endpoint to deploy to.
        endpoint_id (int): Endpoint ID to deploy to.
        stack_name (str): Name of the stack in config.
        push (bool, optional): Push changes to Portainer if True, preview changes otherwise.
    """
    if not (compose := portainer_helper.get_git_compose_file(endpoint_name, stack_name, config)):
        logger.error("No compose provided from Git")
        return
    ...
    logger.print(f"'{stack_name}': [green]Ready to create[/]")
    display_compose_diff(base_compose="", new_compose=compose)
    ...
    try:
        portainer_helper.create_stack(
            endpoint_id=endpoint_id,
            stack_name=stack_name,
            compose=compose,
            environment=config_environment,
        )
        logger.print(f"'{stack_name}': [green]Successfully created[/]")
    except HTTPError:
        logger.exception(f"'{stack_name}': [red]Unable to create[/]")


def update_existing_stack(
    config: Config, portainer_helper: PortainerHelper, portainer_info: dict[str, Any],
    endpoint_name: str, endpoint_id: int, stack_name: str, push: bool,
) -> None:
    """ Update an existing stack on Portainer with stack information defined in config.

    Args:
        config (Config): Config model with definitions of stacks for the given endpoint.
        portainer_helper (PortainerHelper): Portainer helper instance to interact with Portainer.
        portainer_info (dict[str, Any]): Existing stack information, sourced from Portainer.
        endpoint_name (str): Friendly name of the endpoint to deploy to.
        endpoint_id (int): Endpoint ID to deploy to.
        stack_name (str): Name of the stack in config.
        push (bool, optional): Push changes to Portainer if True, preview changes otherwise.
