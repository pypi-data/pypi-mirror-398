...
class Subscribe:
    ...
    async def StartAsync(self):
        ...
        # Force own pool mode - engine pool connections are tied to a different event loop
        if self.config.use_engine_pool:
            import warnings
            warnings.warn(
                "[SUBSCRIBER] use_engine_pool=True is not supported with StartAsync() because "
                "the listener runs in a separate thread with its own event loop. "
                "Forcing use_engine_pool=False (using own connection pool).",
                UserWarning
            )
        self.config.use_engine_pool = False
        self.engine._debug("[SUBSCRIBER] Using own connection pool (async mode requires separate pool)")

        # Event to signal when listener is ready
        ready_event = threading.Event()
        error_holder = [None]  # To capture any startup error
        
        def run_listener():
            try:
                asyncio.run(self._listen_loop_async(channels, ready_event))
            except Exception as e:
                error_holder[0] = e
                ready_event.set()  # Unblock main thread on error

        # Run in background thread (keeps process alive like Start())
        self._thread = threading.Thread(
            target=run_listener,
            daemon=self.config.daemon_mode
        )
        ...
        # Wait for listener to be ready (pool created and listening)
        ready_event.wait(timeout=10.0)
        
        if error_holder[0]:
            raise error_holder[0]
        
        self.engine._debug("[SUBSCRIBER] Started listening in background thread on channels: {}", channels)
