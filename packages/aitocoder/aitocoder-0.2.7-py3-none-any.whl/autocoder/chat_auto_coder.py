...
async def run_app():
    ...
    @kb.add("enter")
    def _(event):
        # Only accept input if buffer is not empty
    ...
    def get_bottom_toolbar():
        ...
        return FormattedText([
            ("class:toolbar-label", "│ Project Dir: "),
    ...
    session = PromptSession(
        ...
        mouse_support=False,  # Disable mouse support to prevent focus events from triggering unwanted input
        ...
        try:
            ...
            if new_prompt:
                user_input = await session.prompt_async(
                    ...
                    placeholder=HTML('<style fg="ansigray">Type your command here...</style>')
                )
                new_prompt = ""
            else:
                user_input = await session.prompt_async(
                    FormattedText(prompt_message), style=style,
                    placeholder=HTML('<style fg="ansibrightblack"><i>  Enter your next command here...</i></style>')
                )
...
def main():
    ...
    configure("product_mode:lite")

    # Set default model to claude-sonnet-4.5 if no model is configured
    memory_manager = get_memory_manager()
    if not memory_manager.get_config("model", None):
        configure("model:claude-sonnet-4.5", skip_print=True)
