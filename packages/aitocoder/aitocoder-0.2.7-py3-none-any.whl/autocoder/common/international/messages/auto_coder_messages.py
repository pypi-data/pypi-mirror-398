...
AUTO_CODER_MESSAGES = {
    ...
    "index_file_too_large": {
        "en": "\u26A0 File {{ file_path }} is too large ({{ file_size }} > {{ max_length }}), splitting into chunks...",
        "zh": "\u26A0 文件 {{ file_path }} 过大 ({{ file_size }} > {{ max_length }}), 正在分块处理...",
        "ja": "\u26A0 ファイル {{ file_path }} が大きすぎます ({{ file_size }} > {{ max_length }})、チャンクに分割中...",
        "ar": "\u26A0 الملف {{ file_path }} كبير جداً ({{ file_size }} > {{ max_length }})، جاري التقسيم إلى أجزاء...",
        "ru": "\u26A0 Файл {{ file_path }} слишком большой ({{ file_size }} > {{ max_length }}), разделение на части..."
    },
    ...
    "index_source_dir_mismatch": {
        "en": "\u26A0 Source directory mismatch (file_path: {{ file_path }}, source_dir: {{ source_dir }})",
        "zh": "\u26A0 源目录不匹配 (文件路径: {{ file_path }}, 源目录: {{ source_dir }})",
        "ja": "\u26A0 ソースディレクトリの不一致 (ファイルパス: {{ file_path }}, ソースディレクトリ: {{ source_dir }})",
        "ar": "\u26A0 عدم تطابق مجلد المصدر (مسار الملف: {{ file_path }}, مجلد المصدر: {{ source_dir }})",
        "ru": "\u26A0 Несоответствие исходной директории (путь файла: {{ file_path }}, исходная директория: {{ source_dir }})"
    },
    "index_related_files_fail": {
        "en": "\u26A0 Failed to find related files for chunk {{ chunk_count }}",
        "zh": "\u26A0 无法为块 {{ chunk_count }} 找到相关文件",
        "ja": "\u26A0 チャンク {{ chunk_count }} の関連ファイルの検索に失敗しました",
        "ar": "\u26A0 فشل في العثور على الملفات ذات الصلة للجزء {{ chunk_count }}",
        "ru": "\u26A0 Не удалось найти связанные файлы для части {{ chunk_count }}"
    },
    ...
    "index_file_removed": {
        "en": "\u2717 Removed non-existent file index: {{ file_path }}",
        "zh": "\u2717 已移除不存在的文件索引：{{ file_path }}",
        "ja": "\u2717 存在しないファイルのインデックスを削除しました：{{ file_path }}",
        "ar": "\u2717 تم إزالة فهرس الملف غير الموجود: {{ file_path }}",
        "ru": "\u2717 Удален индекс несуществующего файла: {{ file_path }}"
    },
    "index_file_saved": {
    ...
    "code_generation_complete": {
        "en": "{{ model_names}}, input_tokens_count: {{ input_tokens }}, generated_tokens_count: {{ output_tokens }}, input_cost: {{ input_cost }}, output_cost: {{ output_cost }}",
        "zh": "{{ model_names}}, 输入token数: {{ input_tokens }}, 输出token数: {{ output_tokens }}, 输入成本: {{ input_cost }}, 输出成本: {{ output_cost }}",
    ...
    "generate_max_rounds_reached": {
        "en": "\u26A0 Generation stopped after reaching the maximum allowed rounds ({{ count }}/{{ max_rounds }}). Current generated content length: {{ generated_tokens }}. If the output is incomplete, consider increasing 'generate_max_rounds' in configuration.",
        "zh": "\u26A0 生成已停止，因为达到了最大允许轮数 ({{ count }}/{{ max_rounds }})。当前生成内容长度: {{ generated_tokens }} tokens。如果输出不完整，请考虑在配置中增加 'generate_max_rounds'。",
        "ja": "\u26A0 最大許可ラウンド数に達したため生成が停止しました ({{ count }}/{{ max_rounds }})。現在の生成コンテンツ長: {{ generated_tokens }} トークン。出力が不完全な場合は、設定で 'generate_max_rounds' を増やすことを検討してください。",
        "ar": "\u26A0 توقف التوليد بعد الوصول إلى أقصى عدد جولات مسموح ({{ count }}/{{ max_rounds }})। طول المحتوى المولد الحالي: {{ generated_tokens }} رمز。إذا كان الإخراج غير مكتمل، فكر في زيادة 'generate_max_rounds' في التكوين.",
        "ru": "\u26A0 Генерация остановлена после достижения максимального количества раундов ({{ count }}/{{ max_rounds }}). Текущая длина сгенерированного контента: {{ generated_tokens }} токенов. Если вывод неполный, рассмотрите увеличение 'generate_max_rounds' в конфигурации."
    },
    "code_merge_start": {
    ...
    "pylint_check_failed": {
        "en": "\u26A0 Pylint check failed: {{ error_message }}",
        "zh": "\u26A0 Pylint 检查失败: {{ error_message }}",
        "ja": "\u26A0 Pylint チェックが失敗しました: {{ error_message }}",
        "ar": "\u26A0 فشل فحص Pylint: {{ error_message }}",
        "ru": "\u26A0 Проверка Pylint не удалась: {{ error_message }}"
    },
    "pylint_error": {
    ...
    "unmerged_blocks_warning": {
        "en": "\u26A0 Found {{ num_blocks }} unmerged blocks, the changes will not be applied. Please review them manually then try again.",
        "zh": "\u26A0 发现 {{ num_blocks }} 个未合并的代码块，更改将不会被应用。请手动检查后重试。",
        "ja": "\u26A0 {{ num_blocks }} 個のマージされていないブロックが見つかりました、変更は適用されません。手動で確認してから再試行してください。",
        "ar": "\u26A0 تم العثور على {{ num_blocks }} كتل غير مدموجة، لن يتم تطبيق التغييرات. يرجى مراجعتها يدوياً ثم المحاولة مرة أخرى.",
        "ru": "\u26A0 Найдено {{ num_blocks }} неслитых блоков, изменения не будут применены. Пожалуйста, проверьте их вручную и попробуйте снова."
    },
    "pylint_file_check_failed": {
        "en": "\u26A0 Pylint check failed for {{ file_path }}. Changes not applied. Error: {{ error_message }}",
        "zh": "\u26A0 {{ file_path }} 的 Pylint 检查失败。更改未应用。错误: {{ error_message }}",
        "ja": "\u26A0 {{ file_path }} の Pylint チェックが失敗しました。変更は適用されませんでした。エラー: {{ error_message }}",
        "ar": "\u26A0 فشل فحص Pylint لـ {{ file_path }}. لم يتم تطبيق التغييرات. خطأ: {{ error_message }}",
        "ru": "\u26A0 Проверка Pylint не удалась для {{ file_path }}. Изменения не применены. Ошибка: {{ error_message }}"
    },
    "merge_success": {
    ...
    "no_changes_made": {
        "en": "\u26A0 No changes were made to any files.",
        "zh": "\u26A0 未对任何文件进行更改。这个原因可能是因为coding函数生成的文本块格式有问题，导致无法合并进项目",
        "ja": "\u26A0 どのファイルも変更されませんでした。",
        "ar": "\u26A0 لم يتم إجراء أي تغييرات على أي ملفات.",
        "ru": "\u26A0 Никаких изменений в файлах не было внесено."
    },
    "files_merged": {
    ...
    "quick_filter_too_long": {
        "en": "\u26A0 index file is too large ({{ tokens_len }}/{{ max_tokens }}). The query will be split into {{ split_size }} chunks.",
        "zh": "\u26A0 索引文件过大 ({{ tokens_len }}/{{ max_tokens }})。查询将被分成 {{ split_size }} 个部分执行。",
        "ja": "\u26A0 インデックスファイルが大きすぎます ({{ tokens_len }}/{{ max_tokens }})。クエリは {{ split_size }} 個のチャンクに分割されます。",
        "ar": "\u26A0 ملف الفهرس كبير جداً ({{ tokens_len }}/{{ max_tokens }})。سيتم تقسيم الاستعلام إلى {{ split_size }} أجزاء.",
        "ru": "\u26A0 Индексный файл слишком большой ({{ tokens_len }}/{{ max_tokens }}). Запрос будет разделен на {{ split_size }} частей."
    },
    ...
    "conversation_pruning_start": {
        "en": "\u26A0 Conversation pruning started, total tokens: {{total_tokens}}, safe zone: {{safe_zone}}",
        "zh": "\u26A0 对话长度 {{total_tokens}} tokens 超过安全阈值 {{safe_zone}}，开始修剪对话。",
        "ja": "\u26A0 会話の剪定を開始しました、総トークン数: {{total_tokens}}、安全ゾーン: {{safe_zone}}",
        "ar": "\u26A0 بدأت عملية تقليم المحادثة، إجمالي الرموز: {{total_tokens}}، المنطقة الآمنة: {{safe_zone}}",
        "ru": "\u26A0 Начата обрезка разговора, общее количество токенов: {{total_tokens}}, безопасная зона: {{safe_zone}}"
    },
    "invalid_file_number": {
        "en": "\u26A0 Invalid file number {{file_number}}, total files: {{total_files}}",
        "zh": "\u26A0 无效的文件编号 {{file_number}}，总文件数为 {{total_files}}",
        "ja": "\u26A0 無効なファイル番号 {{file_number}}、総ファイル数: {{total_files}}",
        "ar": "\u26A0 رقم الملف غير صحيح {{file_number}}، إجمالي الملفات: {{total_files}}",
        "ru": "\u26A0 Неверный номер файла {{file_number}}, общее количество файлов: {{total_files}}"
    },
    "all_merge_results_failed": {
        "en": "\u26A0 All merge attempts failed, returning first candidate",
        "zh": "\u26A0 所有合并尝试都失败，返回第一个候选",
        "ja": "\u26A0 すべてのマージ試行が失敗しました、最初の候補を返します",
        "ar": "\u26A0 فشلت جميع محاولات الدمج، إرجاع المرشح الأول",
        "ru": "\u26A0 Все попытки слияния не удались, возвращаем первого кандидата"
    },
    ...
    "context_pruning_start": {
        "en": "\u26A0 Context pruning started. Total tokens: {{ total_tokens }} (max allowed: {{ max_tokens }}). Applying strategy: {{ strategy }}.",
        "zh": "\u26A0 开始上下文剪枝。总token数: {{ total_tokens }} (最大允许: {{ max_tokens }})。正在应用策略: {{ strategy }}。",
        "ja": "\u26A0 コンテキストの剪定を開始しました。総トークン数: {{ total_tokens }} (最大許可: {{ max_tokens }})。戦略を適用中: {{ strategy }}。",
        "ar": "\u26A0 بدأت عملية تقليم السياق. إجمالي الرموز: {{ total_tokens }} (الحد الأقصى المسموح: {{ max_tokens }})。تطبيق الاستراتيجية: {{ strategy }}。",
        "ru": "\u26A0 Начата обрезка контекста. Общее количество токенов: {{ total_tokens }} (максимально разрешено: {{ max_tokens }}). Применяется стратегия: {{ strategy }}."
    },
    ...
    "super_big_filter_splitting": {
        "en": "\u26A0 Index file is extremely large ({{ tokens_len }}/{{ max_tokens }}). The query will be split into {{ split_size }} chunks for processing.",
        "zh": "\u26A0 索引文件极其庞大 ({{ tokens_len }}/{{ max_tokens }})。查询将被分成 {{ split_size }} 个部分进行处理。",
        "ja": "\u26A0 インデックスファイルが極めて大きいです ({{ tokens_len }}/{{ max_tokens }})。クエリは処理のために {{ split_size }} 個のチャンクに分割されます。",
        "ar": "\u26A0 ملف الفهرس كبير بشكل استثنائي ({{ tokens_len }}/{{ max_tokens }})。سيتم تقسيم الاستعلام إلى {{ split_size }} أجزاء للمعالجة。",
        "ru": "\u26A0 Индексный файл чрезвычайно большой ({{ tokens_len }}/{{ max_tokens }}). Запрос будет разделен на {{ split_size }} частей для обработки."
    },
...
"""
    ...
    "mcp_list_marketplace_title": {
        "en": "\u00B7 Marketplace MCP Servers:",
        "zh": "\u00B7 市场 MCP 服务器:",
        "ja": "\u00B7 マーケットプレイスMCPサーバー:",
        "ar": "\u00B7 خوادم MCP في السوق:",
        "ru": "\u00B7 MCP серверы из маркетплейса:"
    },
    ...
    "mcp_query_empty": {
        ...
        "zh": "\u26A0 查询不能为空",
        "ja": "\u26A0 クエリは空にできません",
        "ar": "\u26A0 لا يمكن أن يكون الاستعلام فارغاً",
        "ru": "\u26A0 Запрос не может быть пустым"
    },
    "mcp_error_title": {
        "en": "\u2717 MCP Error",
        "zh": "\u2717 MCP 错误",
