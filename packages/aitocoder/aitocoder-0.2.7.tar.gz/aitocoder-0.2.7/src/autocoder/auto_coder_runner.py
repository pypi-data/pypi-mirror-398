...
def code_next(query: str):
    ...
    if not predicted_tasks:
        console = Console()
        console.print(f"[{COLORS.ORANGE}]No task predictions available[/]")
        return

    console = Console()

    # Create table for all predicted tasks
    console.print(f"[bold {COLORS.BLUE}]Predicted Next Tasks[/]")
...
def gen_and_exec_shell_command(query: str):
    ...
    if Confirm.ask(
        ...
        console.print(f"[{COLORS.ORANGE}]{printer.get_message_from_key('shell_script_not_executed')}[/]")
...
def auto_command(query: str, extra_args: Dict[str, Any] = {}):
    ...
    console.print(f"[{COLORS.BLUE}]{title}[/]")
