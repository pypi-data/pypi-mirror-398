...
class MainWindow(QMainWindow):
    ...
    def _create_audio_docks(self):
        ...
        # Detection results dock
        self._detection_results_widget = DetectionResultsWidget()
        detection_dock = QDockWidget("Detections", self)
        detection_dock.setObjectName("DetectionsDock")
        detection_dock.setWidget(self._detection_results_widget)
        detection_dock.setFeatures(
            QDockWidget.DockWidgetFeature.DockWidgetMovable
            | QDockWidget.DockWidgetFeature.DockWidgetFloatable
            | QDockWidget.DockWidgetFeature.DockWidgetClosable
        )
        self.addDockWidget(Qt.DockWidgetArea.BottomDockWidgetArea, detection_dock)
        self._audio_docks.append(detection_dock)

        # Tabify detection with selections dock
        self.tabifyDockWidget(selection_dock, detection_dock)
    ...
    def _setup_menus(self):
        ...
        # Detection submenu
        detection_menu = analysis_menu.addMenu("&Detection")

        run_detection_action = QAction("&Run Detection...", self)
        run_detection_action.setShortcut(QKeySequence("Ctrl+D"))
        run_detection_action.triggered.connect(self._run_detection)
        detection_menu.addAction(run_detection_action)

        batch_detection_action = QAction("&Batch Detection...", self)
        batch_detection_action.triggered.connect(self._show_batch_detection)
        detection_menu.addAction(batch_detection_action)

        detection_menu.addSeparator()

        clear_detections_action = QAction("&Clear Detections", self)
        clear_detections_action.triggered.connect(self._clear_detections)
        detection_menu.addAction(clear_detections_action)
    ...
    def _setup_connections(self):
        ...
        # Detection results connections
        self._detection_results_widget.go_to_detection.connect(
            self._on_go_to_detection
        )
        self._detection_results_widget.convert_to_annotations.connect(
            self._on_convert_detections_to_annotations
        )
    ...
    def _run_detection(self):
        """Run detection on the currently loaded audio."""
        if self._audio is None:
        ...
        # Run detection using SingleFileDetectionWorker
        from magpy.workers import SingleFileDetectionWorker

        self._detection_worker = SingleFileDetectionWorker(
            audio_data=self._audio.data[:, self._current_channel],
            sample_rate=self._audio.sample_rate,
            config=config,
            filepath=self._audio.filepath,
        )

        self._detection_worker.progress.connect(
            lambda msg: self.statusBar().showMessage(msg)
        )
        self._detection_worker.result.connect(self._on_detection_complete)
    ...
    def _on_detection_complete(self, detections: list):
        ...
        self.statusBar().showMessage(f"Detection complete: {len(detections)} results", 5000)

        # Update detection results widget
        self._detection_results_widget.set_detections(detections)

        # Update spectrogram overlay
        label_colors = self._detection_results_widget.get_label_colors()
        self._spectrogram.set_detections(detections, label_colors)

        # Show detections dock
        for dock in self._audio_docks:
            if dock.objectName() == "DetectionsDock":
    ...
    def _on_detection_error(self, message: str):
        ...
        self.statusBar().showMessage("Detection failed", 3000)
        QMessageBox.critical(self, "Detection Error", message)

    def _show_batch_detection(self):
        """Show batch detection dialog."""
        dialog = BatchDetectionDialog(self)
        dialog.detection_complete.connect(self._on_batch_detection_complete)
        dialog.exec()

    def _on_batch_detection_complete(self, result):
        """Handle batch detection completion."""
        self._detection_results_widget.set_batch_result(result)

        # Update spectrogram overlay if we have detections for current file
        if self._audio:
            current_file = self._audio.filepath
            file_detections = [
                d for d in result.detections
                if d.filepath == current_file
            ]
            if file_detections:
                label_colors = self._detection_results_widget.get_label_colors()
                self._spectrogram.set_detections(file_detections, label_colors)
    ...
    def _clear_detections(self):
        ...
        self._detection_results_widget.clear()
        self._spectrogram.clear_detections()
        self.statusBar().showMessage("Detections cleared", 2000)

    def _on_go_to_detection(self, start_time: float, end_time: float):
        """Navigate to a detection in the spectrogram."""
        # Add padding around the detection
        padding = (end_time - start_time) * 0.5
        view_start = max(0, start_time - padding)
        view_end = end_time + padding

        self._waveform.set_view_range(view_start, view_end)
        self._spectrogram.set_view_range(view_start, view_end)

    def _on_convert_detections_to_annotations(self, detections: list):
        ...
        for det in detections:
            selection = Selection(
                begin_time=det.start_time,
                end_time=det.end_time,
                low_freq=det.frequency_low,
                high_freq=det.frequency_high,
                channel=self._current_channel + 1,
            )
            selection.annotations["Label"] = det.label
            selection.annotations["Confidence"] = f"{det.confidence:.3f}"

            self._selection_table_widget.add_selection_with_undo(selection)
            count += 1

        self._update_selection_overlays()
        self.statusBar().showMessage(f"Converted {count} detections to annotations", 3000)
