...
### Phase 2: MagPy Core Viewer (Weeks 13-20) ✅ COMPLETE

**Goal:** Build foundational GUI with audio visualization and playback.

**Status:** All Phase 2 deliverables completed as of December 2025.
...
├── screens/
│   ├── spectral_analyzer.py # Screen 1: Audio viewer (integrated in main_window)
│   ├── dataset_browser.py   # Screen 2: File management (future)
│   ├── terminal.py          # Screen 3: Embedded CLI ✅
│   ├── pipeline_editor.py   # Screen 4: Visual workflows (future)
│   ├── command_builder.py   # Screen 5: GUI command construction (future)
│   ├── services_browser.py  # Screen 6: API integrations (future)
│   └── llm_assistant.py     # Screen 7: Natural language interface (future)
├── widgets/
│   ├── waveform.py          # PyQtGraph waveform ✅
│   ├── spectrogram.py       # PyQtGraph spectrogram ✅
│   ├── selection_table.py   # Selection table widget ✅
│   ├── playback.py          # Transport controls ✅
│   └── properties_panel.py  # File/selection properties ✅
├── controllers/
│   └── audio_controller.py  # Wraps bioamla controllers ✅
└── workers/
    └── detection_worker.py  # QThread for ML inference (future)
```
...
- [x] Waveform display with zoom/pan (PyQtGraph)
- [x] Spectrogram display with configurable FFT parameters
- [x] Synchronized playhead across views (with auto-scroll)
- [x] Transport controls (play, pause, stop, loop, seek)
- [x] Selection tool for time-frequency regions (Shift+Click)
- [x] Properties panel (duration, sample rate, channels, bit depth)
- [x] Ctrl+Click to seek to position

#### 2.3 Audio Controller Integration ✅

```python
# magpy/controllers/audio_controller.py
# Implemented with bioamla 0.1.1 integration:
# - AudioFileController for file I/O
# - AudioTransformController for signal processing
# - IndicesController for acoustic indices
# - Undo/redo support for transforms
# - Fallback implementations when bioamla not installed
```
...
- [x] QPlainTextEdit-based terminal emulator
- [x] Command history (up/down arrows, persisted)
- [x] Auto-completion for bioamla commands
- [x] Real-time output streaming via QProcess
- [x] Built-in commands: `clear`, `help`, `history`
