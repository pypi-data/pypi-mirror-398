...
Features:
- Click-drag selection box for time-frequency regions
- Multiple colormap options
- Synchronized navigation with waveform
...
class SelectionBounds:
    """Bounds for a time-frequency selection."""
    ...
    def is_valid(self, min_duration: float = 0.01, min_bandwidth: float = 50.0) -> bool:
        """Check if selection meets minimum size requirements."""
        return self.duration >= min_duration and self.bandwidth >= min_bandwidth
...
class SpectrogramWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._updating_position = False  # Guard against recursion
    ...
    def _setup_ui(self):
        ...
        # Current selection rectangle (for drag preview)
        self._selection_rect = pg.LinearRegionItem(
            values=(0, 0),
            orientation='vertical',
            brush=pg.mkBrush(78, 154, 6, 50),
            pen=pg.mkPen(color="#4e9a06", width=2),
            movable=False,
        )
        self._selection_rect.setVisible(False)
        self._plot_widget.addItem(self._selection_rect)

        # Use a custom GraphicsRectItem for the 2D selection box
        self._selection_box = pg.QtWidgets.QGraphicsRectItem()
        self._selection_box.setPen(pg.mkPen(color="#4e9a06", width=2))
        self._selection_box.setBrush(pg.mkBrush(78, 154, 6, 80))
        self._selection_box.setVisible(False)
        self._plot_widget.addItem(self._selection_box)
    ...
    def _setup_interactions(self):
        ...
        # Install event filter for drag selection
    ...
    def set_spectrogram(self, result: SpectrogramResult):
        ...
        # Set axis range limits to prevent scrolling beyond data bounds
        max_time = times[-1] if len(times) > 0 else 1
        max_freq = freqs[-1] if len(freqs) > 0 else 22050
        ...
        self._plot_widget.setXRange(0, max_time, padding=0)
    ...
    def set_playback_position(self, position: float):
        ...
        # Guard against recursion from auto-scroll triggering view changes
    ...
    def set_selections(self, selections: List):
        ...
        for sel in selections:
            if sel.low_freq is not None and sel.high_freq is not None:
                ...
                # rect.setBrush(pg.mkBrush(color=(78, 154, 6, 50))) TODO: Uncomment when pyqtgraph supports it
    ...
    def eventFilter(self, obj, event):
        ...
        # Check if this is the plot widget's viewport
        try:
        ...
        if event.type() == QEvent.Type.MouseButtonPress:
            if event.button() == Qt.MouseButton.LeftButton:
                ...
                if modifiers == Qt.KeyboardModifier.ShiftModifier:
                    ...
                    self._start_drag_selection(event.position())
                    return True

        elif event.type() == QEvent.Type.MouseMove:
            if self._is_dragging and self._selection_start is not None:
                self._update_drag_selection(event.position())
                return True

        elif event.type() == QEvent.Type.MouseButtonRelease:
            if event.button() == Qt.MouseButton.LeftButton and self._is_dragging:
                self._complete_drag_selection(event.position())
                return True

        return super().eventFilter(obj, event)

    def _map_widget_to_data(self, widget_pos: QPointF) -> Optional[Tuple[float, float]]:
        """Map widget coordinates to data coordinates (time, freq)."""
        if self._spectrogram_result is None:
            return None

        # Map from widget to scene, then to view
        scene_pos = self._plot_widget.plotItem.vb.mapToView(widget_pos)
        ...
        x = max(0, min(x, max_time))
        y = max(0, min(y, max_freq))

        return (x, y)

    def _start_drag_selection(self, widget_pos: QPointF):
        """Start a drag selection at the given position."""
        coords = self._map_widget_to_data(widget_pos)
        if coords is None:
            return

        self._selection_start = coords
        self._is_dragging = True
        self._is_selecting = True

        # Initialize selection box at start point
        self._update_selection_box(coords[0], coords[1], coords[0], coords[1])
        self._selection_box.setVisible(True)

    def _update_drag_selection(self, widget_pos: QPointF):
        ...
        self._update_selection_box(start_x, start_y, end_x, end_y)

    def _update_selection_box(self, x1: float, y1: float, x2: float, y2: float):
        """Update the visual selection box rectangle."""
        # Normalize coordinates
        min_x, max_x = min(x1, x2), max(x1, x2)
        min_y, max_y = min(y1, y2), max(y1, y2)

        # Map data coordinates to scene coordinates for the graphics item
        vb = self._plot_widget.plotItem.vb
        top_left = vb.mapViewToScene(QPointF(min_x, max_y))
        bottom_right = vb.mapViewToScene(QPointF(max_x, min_y))

        from PyQt6.QtCore import QRectF
        rect = QRectF(top_left, bottom_right).normalized()
        self._selection_box.setRect(rect)

    def _complete_drag_selection(self, widget_pos: QPointF):
        ...
        # Create SelectionBounds and check validity
        ...
        if bounds.is_valid(min_duration=0.005, min_bandwidth=20.0):
    ...
    def _on_mouse_clicked(self, event):
        ...
        # Don't process if we're in drag selection mode
        if self._is_dragging or self._is_selecting:
            return
