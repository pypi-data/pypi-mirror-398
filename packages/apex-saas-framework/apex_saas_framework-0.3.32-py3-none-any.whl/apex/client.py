...
def _is_async_url(url: str) -> bool:
    ...
    # If no driver specified, check settings but warn
    settings = get_settings()
    if settings.DB_ASYNC_MODE:
        import warnings
        # Determine suggested driver based on database type
        suggested_driver = None
        if "postgresql://" in url_lower or "postgres://" in url_lower:
            suggested_driver = "postgresql+asyncpg://" if settings.DB_ASYNC_MODE else "postgresql+psycopg2://"
        elif "mysql://" in url_lower:
            suggested_driver = "mysql+aiomysql://" if settings.DB_ASYNC_MODE else "mysql+pymysql://"
        elif "sqlite://" in url_lower:
            suggested_driver = "sqlite+aiosqlite://" if settings.DB_ASYNC_MODE else "sqlite://"
        
        if suggested_driver:
            warnings.warn(
                f"Database URL '{url}' has no driver specified. "
                f"Defaulting to async mode (DB_ASYNC_MODE=True). "
                f"For explicit control, use '{suggested_driver}' in your DATABASE_URL. "
                f"Or set DB_ASYNC_MODE=False in your .env file for sync mode.",
                UserWarning,
                stacklevel=3
            )
...
class Client:
    ...
    async def init_database(self, models: Optional[List[Type[Any]]] = None) -> None:
        ...
        try:
            ...
            from apex.core.exceptions import DatabaseError
            error_msg = str(e)
            if "connection" in error_msg.lower() or "connect" in error_msg.lower():
                raise DatabaseError(
                    f"Failed to connect to database: {error_msg}",
                    details={
                        "hint": f"Check your DATABASE_URL: {self.database_url[:50]}...",
                        "original_error": error_msg
                    }
                )
            else:
                raise DatabaseError(
                    f"Failed to create database tables: {error_msg}",
