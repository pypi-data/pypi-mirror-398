...
def _run(coro):
    ...
    If called from an async context (event loop already running), returns the coroutine
    for the caller to await. Otherwise, runs the coroutine synchronously.
    """
    try:
        # Check if we're in an async context (loop is running)
        loop = asyncio.get_running_loop()
        # If loop is running, we can't block - return the coroutine for caller to await
        import warnings
        warnings.warn(
            "Cannot run coroutine synchronously in async context. "
            "The coroutine was returned and should be awaited. "
            "Consider using 'await' directly instead of calling this function.",
            RuntimeWarning,
            stacklevel=2
        )
        # Return the coroutine itself - caller must await it
        return coro
    except RuntimeError:
...
# Lifecycle / setup
def bootstrap(client: Optional[Any] = None, models: Optional[list[type]] = None):
    """
    Create tables for the provided models (or the client's registered models).
    
    In async contexts (e.g., FastAPI), this returns a coroutine that should be awaited.
    In sync contexts, this runs synchronously.
    
    Example (sync):
        bootstrap(client, models=[User, Organization])
    
    Example (async/FastAPI):
        @app.on_event("startup")
        async def startup():
            await bootstrap(client, models=[User, Organization])
    """
    c = _client(client)
    try:
        # Check if we're in an async context
        asyncio.get_running_loop()
        # In async context, return coroutine for caller to await
        return c.init_database(models=models)
    except RuntimeError:
        # Not in async context, safe to run synchronously
        try:
            return _run(c.init_database(models=models))
        except Exception as e:
            logging.error(f"Error during bootstrap: {e}", exc_info=True)
            raise DatabaseError(
                f"Failed to initialize database: {str(e)}",
                details={
                    "hint": "Check your DATABASE_URL and ensure the database is running and accessible.",
