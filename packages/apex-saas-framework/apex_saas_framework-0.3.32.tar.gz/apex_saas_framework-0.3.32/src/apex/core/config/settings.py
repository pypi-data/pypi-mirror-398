...
class Settings(BaseSettings):
    ...
    # - SQLite (async): sqlite+aiosqlite:///./mydb.db
    # - SQLite (sync): sqlite:///./mydb.db
    # - MySQL (async): mysql+aiomysql://user:pass@localhost/db
    # - MySQL (sync): mysql+pymysql://user:pass@localhost/db
    # - Any SQLAlchemy-supported database
    DATABASE_URL: str = Field(
        default="sqlite+aiosqlite:///./apex.db"
    )
    # Database mode: True for async (default), False for sync
    # If True, auto-converts URLs to async format. If False, uses URLs as-is.
    DB_ASYNC_MODE: bool = True
    DB_ECHO: bool = False
    DB_POOL_SIZE: int = 5
