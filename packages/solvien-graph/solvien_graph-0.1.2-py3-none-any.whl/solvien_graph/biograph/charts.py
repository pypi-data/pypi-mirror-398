...
def quick_heatmap(data, title="Heatmap", cmap="viridis", theme="default", 
    ...
    # Text overlay (if annotate=True or auto-detect)
    # Auto-annotate for small heatmaps or publication quality (since PDF/PNG are not interactive)
    should_annotate = annotate or publication_quality or (rows <= 20 and cols <= 15)
    
    if should_annotate:
        # Only show text on reasonably sized heatmaps (for readability)
        if rows <= 30 and cols <= 20:
            # Adjust font size based on heatmap size
...
def quick_volcano(log2fc, pvalue, title="Volcano Plot", theme="default",
    ...
    # Add statistical annotations for publication quality or static formats
    if publication_quality:
        # Count significant genes
        n_upregulated = upregulated.sum()
        n_downregulated = downregulated.sum()
        n_total = len(log2fc)
        
        # Create annotation text
        annotation_data = pd.DataFrame({
            'x': [log2fc.max() * 0.7, log2fc.min() * 0.7],
            'y': [neg_log10_p.max() * 0.95, neg_log10_p.max() * 0.95],
            'text': [
                f'Upregulated: {n_upregulated}',
                f'Downregulated: {n_downregulated}'
            ],
            'color': ['#d62728', '#1f77b4']
        })
        
        # Add annotation layer
        annotation_layer = alt.Chart(annotation_data).mark_text(
            align='center',
            baseline='top',
            fontSize=11,
            fontWeight='bold'
        ...
        threshold_labels = pd.DataFrame({
            'x': [fc_threshold + 0.2, -fc_threshold - 0.2],
            'y': [neg_log10_p.max() * 0.05, neg_log10_p.max() * 0.05],
            'text': [f'FC={fc_threshold}', f'FC={fc_threshold}']
        })
        
        threshold_text = alt.Chart(threshold_labels).mark_text(
            align='center',
            fontSize=9,
            color=theme_obj["text"],
            opacity=0.7
        ).encode(
            x='x:Q',
            y='y:Q',
        ...
        chart = (chart + annotation_layer + threshold_text)
...
def quick_violin(data_dict, title="Violin Plot", theme="default",
    ...
    if figsize:
        ...
        # Adjust width based on number of categories for better spacing
        n_categories = len(data_dict)
        width = max(600, n_categories * 200)  # 200px per category minimum
        height = style["height"]
    ...
    # Calculate box size based on number of categories to prevent overlap
    n_categories = len(data_dict)
    if n_categories <= 3:
        box_size = 120  # Reduced from 200 to prevent overlap
    elif n_categories <= 5:
        box_size = 100  # Reduced from 150
    else:
        box_size = 70   # Reduced from 100
    ...
    # Add mean value annotations for publication quality
    if publication_quality:
        # Calculate means for each category
        means = df.groupby('Category')['Value'].mean().reset_index()
        means.columns = ['Category', 'Mean']
        
        # Add mean value text with better contrast
        mean_text = alt.Chart(means).mark_text(
            align='center',
            baseline='bottom',
...
def quick_boxplot(data_dict, title="Box Plot", theme="default",
    ...
    chart = alt.Chart(df).mark_boxplot(size=box_size).encode(
        x=alt.X('Category:N' if not horizontal else 'Value:Q', title="Category" if not horizontal else "Value"),
        y=alt.Y('Value:Q' if not horizontal else 'Category:N', title="Value" if not horizontal else "Category"),
        color=alt.Color('Category:N', scale=alt.Scale(range=colors)),
        tooltip=['Category:N', alt.Tooltip('Value:Q', format='.2f')]
    ).properties(
        width=width,
    ...
    # Add median value annotations for publication quality
    if publication_quality and not horizontal:
        # Calculate medians for each category
        medians = df.groupby('Category')['Value'].median().reset_index()
        medians.columns = ['Category', 'Median']
        
        # Add median value text with better contrast
        median_text = alt.Chart(medians).mark_text(
            align='center',  # Changed from 'left' to 'center'
            baseline='bottom',  # Changed from 'middle' to 'bottom'
            fontSize=13,  # Increased from 11 to 13
            fontWeight='bold',
            dx=0,  # Changed from 15 to 0 for center alignment
            dy=-8,  # Move up slightly to avoid median line
            color='white',  # White text for better visibility on colored backgrounds
            filled=True
        ).encode(
            x=alt.X('Category:N'),
