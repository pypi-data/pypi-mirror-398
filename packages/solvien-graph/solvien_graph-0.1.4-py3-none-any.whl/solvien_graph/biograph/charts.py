...
def quick_heatmap(data, title="Heatmap", cmap="viridis", theme="default", 
    ...
    # Heatmap base - no grid or ticks for clean look
    base = alt.Chart(df).mark_rect(stroke=None).encode(
        x=alt.X('ColIdx:O', title="", 
                axis=alt.Axis(
                    labels=False if xticklabels is None else True,
                    labelAngle=-45,
                    ticks=False,
                    domain=False,
                    grid=False
        ...
        y=alt.Y('RowIdx:O', title="",
                axis=alt.Axis(
                    labels=False if yticklabels is None else True,
                    ticks=False,
                    domain=False,
                    grid=False
                )),
    ...
    # Text overlay only if explicitly requested
    # Auto-annotation removed - was causing visual clutter
    if annotate:
...
def quick_boxplot(data_dict, title="Box Plot", theme="default",
    ...
    # No text annotations on box plots - keep them clean
