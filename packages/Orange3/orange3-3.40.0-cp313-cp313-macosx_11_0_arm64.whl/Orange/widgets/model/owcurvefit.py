...
class ParametersWidget(QWidget):
    ...
    def _setup_gui(self):
        ...
        self.__labels = [QLabel(_tr.m[2205, "Name"]), QLabel(_tr.m[2206, "Initial value"]),
                         QLabel(_tr.m[2207, "Lower bound"]), QLabel(_tr.m[2208, "Upper bound"])]
...
class OWCurveFit(OWBaseLearner):
    name = _tr.m[2209, "Curve Fit"]
    description = _tr.m[2210, "Fit a function to data."]
    icon = "icons/CurveFit.svg"
    priority = 90
    keywords = _tr.m[2211, "curve fit, function"]

    class Outputs(OWBaseLearner.Outputs):
        coefficients = Output(_tr.m[2212, "Coefficients"], Table, explicit=True)

    class Warning(OWBaseLearner.Warning):
        duplicate_parameter = Msg(_tr.m[2213, "Duplicated parameter name."])
        unused_parameter = Msg((_tr.m[2214, "Unused parameter '{}' in "] + _tr.m[2215, "'Parameters' declaration."]))
        data_missing = Msg(_tr.m[2216, "Provide data on the input."])

    class Error(OWBaseLearner.Error):
        invalid_exp = Msg(_tr.m[2217, "Invalid expression."])
        no_parameter = Msg((_tr.m[2218, "Missing a fitting parameter.\n"] + _tr.m[2219, "Use 'Feature Constructor' widget instead."]))
        unknown_parameter = Msg((_tr.m[2220, "Unknown parameter '{}'.\n"] + _tr.m[2221, "Declare the parameter in 'Parameters' box"]))
        parameter_in_attrs = Msg((_tr.m[2222, "Some parameters and features have the same "] + _tr.m[2223, "name '{}'."]))
    ...
    FEATURE_PLACEHOLDER = _tr.m[2224, "Select Feature"]
    PARAM_PLACEHOLDER = _tr.m[2225, "Select Parameter"]
    FUNCTION_PLACEHOLDER = _tr.m[2226, "Select Function"]
    ...
    def add_main_layout(self):
        box = gui.vBox(self.controlArea, _tr.m[2227, "Parameters"])
        ...
        function_box = gui.vBox(self.controlArea, box=_tr.m[2228, "Expression"])
        self.__expression_edit = gui.lineEdit(
            function_box, self, "expression",
            placeholderText=_tr.m[2229, "Expression..."], callback=self.settings_changed
        )
    ...
    def get_learner_parameters(self) -> Tuple[Tuple[str, Any]]:
        return ((_tr.m[2230, "Expression"], self.expression),)
    ...
    def check_data(self):
        ...
        if self.data:
            ...
            if len(cont_attrs) == 0:
                self.Error.data_error(_tr.m[2231, "Data has no continuous features."])
