...
DEFAULT_ITEM_NAME = _tr.m[1004, "item"]
DEFAULT_VALUE_NAME = _tr.m[1005, "value"]
DEFAULT_NAME_FOR_ROW = _tr.m[1006, "row"]
...
class OWMelt(widget.OWWidget):
    name = _tr.m[1007, "Melt"]
    description = _tr.m[1008, "Convert wide data to narrow data, a list of item-value pairs"]
    category = _tr.m[1009, "Transform"]
    icon = "icons/Melt.svg"
    keywords = _tr.m[1010, "melt, shopping list, wide, narrow"]
    priority = 2230

    class Inputs:
        data = widget.Input(_tr.m[1011, "Data"], Table)

    class Outputs:
        data = widget.Output(_tr.m[1012, "Data"], Table, dynamic=False)

    class Error(widget.OWWidget.Error):
        nothing_to_melt = Msg(_tr.m[1013, "No features to melt"])

    class Information(widget.OWWidget.Information):
        no_suitable_features = Msg(
            (_tr.m[1014, "No columns with unique values\n"] + _tr.m[1015, "Only columns with unique values are useful for row identifiers."]))
    ...
    def __init__(self, *args, **kwargs):
        ...
        box = gui.widgetBox(self.controlArea, _tr.m[1016, "Unique Row Identifier"])
        self.idvar_model = itemmodels.VariableListModel(
            [None], placeholder=_tr.m[1017, "Row number"])
        self.var_cb = gui.comboBox(
            box, self, "idvar", model=self.idvar_model,
            callback=self._invalidate, minimumContentsLength=16,
            tooltip=_tr.m[1018, "A column with identifier, like customer's id"])

        box = gui.widgetBox(self.controlArea, "Filter")
        gui.checkBox(
            box, self, "only_numeric", _tr.m[1019, "Ignore non-numeric features"],
            callback=self._invalidate)
        gui.checkBox(
            box, self, "exclude_zeros", _tr.m[1020, "Exclude zero values"],
            callback=self._invalidate,
            tooltip=_tr.m[1021, "Besides missing values, also omit items with zero values"])

        form = QFormLayout()
        gui.widgetBox(
            self.controlArea, _tr.m[1022, "Names for generated features"], orientation=form)
        form.addRow(_tr.m[1023, "Item:"],
                    gui.lineEdit(
                        None, self, "item_var_name",
                        callback=self._invalidate,
                        placeholderText=DEFAULT_ITEM_NAME,
                        styleSheet="padding-left: 3px"))
        form.addRow(_tr.m[1024, "Value:"],
                    gui.lineEdit(
    ...
    def send_report(self):
        self.report_items(_tr.m[1025, "Settings"], (
            (_tr.m[1026, "Row identifier"], self.controls.idvar.currentText()),
            (_tr.m[1027, "Ignore non-numeric features"], bool_str(self.only_numeric)),
            (_tr.m[1028, "Exclude zero values"], bool_str(self.exclude_zeros))
        ))
        if self._output_desc:
            self.report_items(_tr.m[1029, "Output"], self._output_desc)

    def _store_output_desc(self, output):
        self._output_desc = {
            _tr.m[1030, "Item column"]: output.domain.attributes[1].name,
            _tr.m[1031, "Value column"]: output.domain.class_var.name,
            _tr.m[1032, "Number of items"]: len(output)
        }
