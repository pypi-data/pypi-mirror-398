...
class ColumnType(enum.Enum):
    ...
    """
    # Skip column
    Skip = _tr.m[3324, "Skip"]
    # Autodetect column type
    Auto = "Auto"
    # Numeric (real) column
    Numeric = _tr.m[3325, "Numeric"]
    # Categorical column
    Categorical = _tr.m[3326, "Categorical"]
    # Text column
    Text = _tr.m[3327, "Text"]
    # Date time column
    Time = _tr.m[3328, "Time"]
...
class CSVOptionsWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.encoding_cb = QComboBox(
            ...
            toolTip=_tr.m[3333, "Select file text encoding"],
        )
        ...
        self.delimiter_cb = QComboBox(
            ...
            toolTip=_tr.m[3334, "Select cell delimiter character."]
        )
        ...
        self.delimiter_cb.addItem(_tr.m[3335, "Other"])
        ...
        form.addRow(_tr.m[3336, "Encoding"], self.encoding_cb)
        form.addRow(QFrame(self, frameShape=QFrame.HLine))
        form.addRow(_tr.m[3337, "Cell delimiter"], delimlayout)
        form.addRow(_tr.m[3338, "Quote character"], self.quoteedit)
        self.setLayout(form)
    ...
    def __show_encodings_widget(self):
        ...
        w = encodings.SelectEncodingsWidget(
            self, Qt.Tool,
            windowTitle=_tr.m[3340, "Customize Encodings List"],
    ...
    def __set_visible_codecs(self, codecs):
        ...
        cb.addItem(_tr.m[3341, "Customize Encodings List..."], userData=...)
...
class CSVImportWidget(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.grouping_sep_edit_cb = TextEditCombo(
            ...
            toolTip=_tr.m[3342, "Thousands group separator"],
            minimumContentsLength=1,
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon
        )
        items = [
            {Qt.DisplayRole: _tr.m[3343, "None"], Qt.EditRole: "",
             Qt.ToolTipRole: _tr.m[3344, "No separator"]},
            {Qt.DisplayRole: ".", Qt.EditRole: "."},
            {Qt.DisplayRole: ",", Qt.EditRole: ","},
            {Qt.DisplayRole: _tr.m[3345, "Space"], Qt.EditRole: " "},
            {Qt.DisplayRole: "'", Qt.EditRole: "'"},
        ]
        ...
        self.decimal_sep_edit_cb = TextEditCombo(
            ...
            toolTip=_tr.m[3346, "Decimal separator"],
            minimumContentsLength=1,
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon
        )
        ...
        number_sep_layout.addWidget(QLabel(_tr.m[3347, "Grouping:"]))
        number_sep_layout.addWidget(self.grouping_sep_edit_cb)
        number_sep_layout.addWidget(QLabel(_tr.m[3348, "Decimal:"]))
        number_sep_layout.addWidget(self.decimal_sep_edit_cb)
        number_sep_layout.addStretch(10)
        form.addRow(_tr.m[3349, "Number separators:"], number_sep_layout)
        ...
        types = [
            {Qt.DisplayRole: _tr.m[3350, "Auto"],
             Qt.ToolTipRole: (_tr.m[3351, "The type will be determined automatically based "] + _tr.m[3352, "on column contents."]),
             Qt.UserRole: ColumnType.Auto},
            {Qt.DisplayRole: _tr.m[3353, "Numeric"], Qt.UserRole: ColumnType.Numeric},
            {Qt.DisplayRole: _tr.m[3354, "Categorical"],
             Qt.UserRole: ColumnType.Categorical},
            {Qt.DisplayRole: _tr.m[3355, "Text"], Qt.UserRole: ColumnType.Text},
            {Qt.DisplayRole: _tr.m[3356, "Datetime"], Qt.UserRole: ColumnType.Time},
            {Qt.AccessibleDescriptionRole: "separator"},
            {Qt.DisplayRole: _tr.m[3357, "Ignore"],
             Qt.UserRole: ColumnType.Skip,
             Qt.ToolTipRole: _tr.m[3358, "The column will not be loaded"]}
        ]
        ...
        hint = QLabel(
            _tr.m[3359, "Hint: right click on the row or column header for additional options."])
        font = hint.font()
        font.setPointSizeF(0.9 * font.pointSizeF())
        hint.setFont(font)
        cb_hint.addWidget(hint)
        form.addRow(_tr.m[3360, "Column type"], cb_hint)
    ...
    def __run_row_menu(self, pos: QPoint, rows: List[int]):
        ...
        m = QMenu(header)
        skip_action = m.addAction(_tr.m[3361, "Skip"])
        skip_action.setCheckable(True)
        skip_action.setChecked(current == TablePreview.Skipped)
        m.addSection("")
        mark_header = m.addAction(_tr.m[3362, "Header"])
...
def format_exception_csv(err):
    return _tr.m[3363, "CSV parsing error: "] + str(err)
