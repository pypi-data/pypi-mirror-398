...
class OWSelectByDataIndex(widget.OWWidget):
    name = _tr.m[1491, "Select by Data Index"]
    description = _tr.m[1492, "Match instances by index from data subset."]
    category = _tr.m[1493, "Transform"]
    icon = "icons/SelectByDataIndex.svg"
    priority = 1112

    class Inputs:
        data = Input(_tr.m[1494, "Data"], Table)
        data_subset = Input(_tr.m[1495, "Data Subset"], Table)

    class Outputs:
        matching_data = Output(_tr.m[1496, "Matching Data"], Table, replaces=["Data"], default=True)
        non_matching_data = Output(_tr.m[1497, "Unmatched Data"], Table)
        # avoiding the default annotated output name (Data), as it was used
        # for Matching Data previously
        annotated_data = Output(_tr.m[1498, "Annotated Data"], Table)
    ...
    class Warning(widget.OWWidget.Warning):
        instances_not_matching = widget.Msg(_tr.m[1499, "Input tables do not share any instances."])
...
"""].strip(), box=True)
    ...
    def send_report(self):
        def data_info_text(data):
            if data is None:
                return _tr.m[1501, "No data."]
            nvars = len(data.domain.variables) + len(data.domain.metas)
            return (f"{data.name}, " + (_tr.e(_tr.c(1502, f"{len(data)} {pl(len(data), 'instance')}, ")) + _tr.e(_tr.c(1503, f"{nvars} {pl(nvars, 'variable')}"))))

        self.report_items("",
                          [(_tr.m[1504, "Data"], data_info_text(self.data)),
                           (_tr.m[1505, "Data Subset"], data_info_text(self.data_subset))])
