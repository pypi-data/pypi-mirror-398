...
class OWDistanceMap(widget.OWWidget):
    name = _tr.m[2699, "Distance Map"]
    description = _tr.m[2700, "Visualize a distance matrix."]
    icon = "icons/DistanceMap.svg"
    priority = 1200
    keywords = _tr.m[2701, "distance map"]

    class Inputs:
        distances = Input(_tr.m[2702, "Distances"], Orange.misc.DistMatrix)

    class Outputs:
        selected_data = Output(_tr.m[2703, "Selected Data"], Orange.data.Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table)
        features = Output(_tr.m[2704, "Features"], widget.AttributeList, dynamic=False)

    class Error(widget.OWWidget.Error):
        empty_matrix = widget.Msg(_tr.m[2705, "Empty distance matrix"])
        not_symmetric = widget.Msg(_tr.m[2706, "Distance matrix is not symmetric."])
    ...
    def __init__(self):
        ...
        self.sorting_cb = gui.comboBox(
            self.controlArea, self, "sorting", box=_tr.m[2707, "Element Sorting"],
            items=[_tr.m[2708, "None"], _tr.m[2709, "Clustering"], _tr.m[2710, "Clustering with ordered leaves"]],
            callback=self._invalidate_ordering)

        box = gui.vBox(self.controlArea, _tr.m[2711, "Colors"])
        ...
        self.annot_combo = gui.comboBox(
            self.controlArea, self, "annotation_idx", box=_tr.m[2712, "Annotations"],
        ...
        self.annot_combo.model()[:] = [_tr.m[2713, "None"], _tr.m[2714, "Enumeration"]]
    ...
    def set_distances(self, matrix):
        ...
        if N > OWDistanceMap._MaxOrderedClustering:
            ...
            if self.sorting == OWDistanceMap.OrderedClustering:
                ...
                msg = (_tr.m[2715, "Cluster ordering was disabled due to the input "] + _tr.m[2716, "matrix being to big"])
        else:
            item.setFlags(item.flags() | Qt.ItemIsEnabled)
        ...
        if N > OWDistanceMap._MaxClustering:
            ...
            msg = (_tr.m[2717, "Clustering was disabled due to the input "] + _tr.m[2718, "matrix being to big"])
        else:
            item.setFlags(item.flags() | Qt.ItemIsEnabled)
    ...
    def set_items(self, items, axis=1):
        ...
        if items is None:
            model[:] = [_tr.m[2719, "None"], _tr.m[2720, "Enumeration"]]
        elif not axis:
            model[:] = [_tr.m[2721, "None"], _tr.m[2722, "Enumeration"], _tr.m[2723, "Attribute names"]]
        elif isinstance(items, Orange.data.Table):
            annot_vars = list(filter_visible(items.domain.variables)) + list(items.domain.metas)
            model[:] = [_tr.m[2724, "None"], _tr.m[2725, "Enumeration"]] + annot_vars
            self.annotation_idx = 0
            self.openContext(items.domain)
        elif isinstance(items, list) and \
                all(isinstance(item, Orange.data.Variable) for item in items):
            model[:] = [_tr.m[2726, "None"], _tr.m[2727, "Enumeration"], _tr.m[2728, "Name"]]
        else:
            model[:] = [_tr.m[2729, "None"], _tr.m[2730, "Enumeration"]]
    ...
    def _update_labels(self, ):
        ...
        elif self.annot_combo.model()[self.annotation_idx] == _tr.m[2731, "Attribute names"]:
    ...
    def send_report(self):
        ...
        if self.annotation_idx <= 1:
            annot = annot.lower()
        self.report_items((
            (_tr.m[2732, "Sorting"], self.sorting_cb.currentText().lower()),
            (_tr.m[2733, "Annotations"], annot)
        ))
        if self.matrix is not None:
            self.report_plot()
