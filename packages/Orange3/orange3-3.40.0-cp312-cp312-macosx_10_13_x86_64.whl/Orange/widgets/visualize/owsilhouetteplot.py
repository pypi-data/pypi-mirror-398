...
class NoGroupVariable(InputValidationError):
    message = _tr.m[3919, "Input does not have any suitable labels"]
...
class OWSilhouettePlot(widget.OWWidget):
    name = _tr.m[3920, "Silhouette Plot"]
    description = (_tr.m[3921, "Visually assess cluster quality and "] + _tr.m[3922, "the degree of cluster membership."])

    icon = "icons/SilhouettePlot.svg"
    priority = 300
    keywords = _tr.m[3923, "silhouette plot"]

    class Inputs:
        data = Input(_tr.m[3924, "Data"], (Orange.data.Table, Orange.misc.DistMatrix))

    class Outputs:
        selected_data = Output(_tr.m[3925, "Selected Data"], Orange.data.Table, default=True)
    ...
    Distances = [(_tr.m[3926, "Euclidean"], Orange.distance.Euclidean),
                 (_tr.m[3927, "Manhattan"], Orange.distance.Manhattan),
                 (_tr.m[3928, "Cosine"], Orange.distance.Cosine)]

    graph_name = "scene"  # QGraphicsScene

    class Error(widget.OWWidget.Error):
        need_two_clusters = Msg(_tr.m[3929, "Need at least two non-empty clusters"])
        singleton_clusters_all = Msg(_tr.m[3930, "All clusters are singletons"])
        memory_error = Msg(_tr.m[3931, "Not enough memory"])
        value_error = Msg(_tr.m[3932, "Distances could not be computed: '{}'"])
        input_validation_error = Msg("{}")
        not_symmetric = widget.Msg(_tr.m[3933, "Distance matrix is not symmetric."])

    class Warning(widget.OWWidget.Warning):
        missing_cluster_assignment = Msg(
            _tr.m[3934, "{} instance{s} omitted (missing cluster assignment)"])
        nan_distances = Msg(_tr.m[3935, "{} instance{s} omitted (undefined distances)"])
        ignoring_categorical = Msg(_tr.m[3936, "Ignoring categorical features"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[3938, "Grouping"])
        self.cluster_var_model = itemmodels.VariableListModel(
            parent=self, placeholder=_tr.m[3939, "(None)"])
        ...
        gui.checkBox(
            box, self, "group_by_cluster", _tr.m[3940, "Show in groups"],
            callback=self._replot)

        box = gui.vBox(self.controlArea, _tr.m[3941, "Bars"])
        gui.widgetLabel(box, _tr.m[3942, "Bar width:"])
        gui.hSlider(
            box, self, "bar_size", minValue=1, maxValue=10, step=1,
            callback=self._update_bar_size)
        gui.widgetLabel(box, _tr.m[3943, "Annotations:"])
        ...
        self.ann_hidden_warning = warning = gui.widgetLabel(
            ibox, _tr.m[3944, "(increase the width to show)"])
    ...
    def _set_distances(self, distances: DistMatrix):
        if not distances.is_symmetric():
            raise ValidationError(_tr.m[3945, "Distance matrix is not symmetric."])
        if isinstance(distances.row_items, Orange.data.Table) and \
                distances.axis == 1:
            data = distances.row_items
        else:
            raise ValidationError(_tr.m[3946, "Input matrix does not have associated data"])
    ...
    def commit(self):
        ...
        if self.data is not None:
            ...
            proposed = _tr.m[3947, "Silhouette ({})"].format(escape(self.cluster_var.name))
    ...
    def send_report(self):
        ...
        self.report_plot()
        caption = (_tr.m[3948, "Silhouette plot "] + (_tr.e(_tr.c(3949, f"({self.Distances[self.distance_idx][0]} distance), ")) + _tr.e(_tr.c(3950, f"clustered by '{self.cluster_var.name}'"))))
        if self.annotation_var and self._silplot.rowNamesVisible():
            caption += _tr.e(_tr.c(3951, f", annotated with '{self.annotation_var.name}'"))
