...
# Default scores.
CLS_SCORES = [
    ScoreMeta(_tr.m[1442, "Information Gain"], _tr.m[1443, "Info. gain"],
              score.InfoGain, ProblemType.CLASSIFICATION, False),
    ScoreMeta(_tr.m[1444, "Information Gain Ratio"], _tr.m[1445, "Gain ratio"],
              score.GainRatio, ProblemType.CLASSIFICATION, True),
    ScoreMeta(_tr.m[1446, "Gini Decrease"], "Gini",
...
REG_SCORES = [
    ScoreMeta(_tr.m[1447, "Univariate Regression"], _tr.m[1448, "Univar. reg."],
...
class OWRank(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1449, "Rank"]
    description = _tr.m[1450, "Rank and filter data features by their relevance."]
    icon = "icons/Rank.svg"
    priority = 1102
    keywords = _tr.m[1451, "rank, filter"]

    buttons_area_orientation = Qt.Vertical

    class Inputs:
        data = Input(_tr.m[1452, "Data"], Table)
        scorer = MultiInput(_tr.m[1453, "Scorer"], score.Scorer, filter_none=True)

    class Outputs:
        reduced_data = Output(_tr.m[1454, "Reduced Data"], Table, default=True)
        scores = Output(_tr.m[1455, "Scores"], Table, dynamic=False)
        features = Output(_tr.m[1456, "Features"], AttributeList, dynamic=False)
    ...
    class Information(OWWidget.Information):
        no_target_var = Msg(_tr.m[1457, "Data does not have a (single) target variable."])
        missings_imputed = Msg(_tr.m[1458, 'Missing values will be imputed as needed.'])

    class Error(OWWidget.Error):
        invalid_type = Msg(_tr.m[1459, "Cannot handle target variable type {}"])
        inadequate_learner = Msg(_tr.m[1460, "Scorer {} inadequate: {}"])
        no_attributes = Msg(_tr.m[1461, "Data does not have a single attribute."])

    class Warning(OWWidget.Warning):
        renamed_variables = Msg(
            _tr.m[1462, "Variables with duplicated names have been renamed."])
    ...
    def __init__(self):
        ...
        for scoring_methods in (CLS_SCORES,
                                REG_SCORES,
                                []):
            box = gui.vBox(None, _tr.m[1463, "Scoring Methods"] if scoring_methods else None)
        ...
        selMethBox = gui.vBox(self.buttonsArea, _tr.m[1464, "Select Attributes"])
        ...
        b1 = button(self.tr(_tr.m[1465, "None"]), OWRank.SelectNone)
        b2 = button(self.tr(_tr.m[1466, "All"]), OWRank.SelectAll)
        b3 = button(self.tr(_tr.m[1467, "Manual"]), OWRank.SelectManual)
        b4 = button(self.tr(_tr.m[1468, "Best ranked:"]), OWRank.SelectNBest)
    ...
    def handleNewSignals(self):
        self.setStatusMessage(_tr.m[1469, 'Running'])
        self.update_scores()
    ...
    def send_report(self):
        if not self.data:
            return
        self.report_domain(_tr.m[1470, "Input"], self.data.domain)
        self.report_table(_tr.m[1471, "Ranks"], self.ranksView, num_format="{:.3f}")
        if self.out_domain_desc is not None:
            self.report_items(_tr.m[1472, "Output"], self.out_domain_desc)
    ...
    def create_scores_table(self, labels):
        ...
        if not model_list or len(model_list[0]) == 2:  # Empty or just first two columns
            return None
        unique, renamed = get_unique_names_duplicates(labels + (_tr.m[1473, 'Feature'],),
        ...
        new_table.name = _tr.m[1474, "Feature Scores"]
        return new_table
