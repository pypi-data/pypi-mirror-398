...
class OWTreeGraph(OWTreeViewer2D):
    ...
    name = _tr.m[3952, "Tree Viewer"]
    icon = "icons/TreeViewer.svg"
    priority = 35
    keywords = _tr.m[3953, "tree viewer"]

    class Inputs:
        # Had different input names before merging from
        # Classification/Regression tree variants
        tree = Input(_tr.m[3954, "Tree"], TreeModel, replaces=["Classification Tree", "Regression Tree"])

    class Outputs:
        selected_data = Output(_tr.m[3955, "Selected Data"], Table, default=True, id="selected-data")
    ...
    COL_OPTIONS = [_tr.m[3956, "Default"], _tr.m[3957, "Number of instances"], _tr.m[3958, "Mean value"], _tr.m[3959, "Variance"]]
    ...
    def __init__(self):
        ...
        self.color_label = QLabel(_tr.m[3960, "Target class: "])
        ...
        combo = gui.comboBox(
            ...
            tooltip=_tr.m[3962, "Variable that identifies the instances in nodes."]
        )
        self.display_box.layout().addRow(_tr.m[3963, "Node labels:"], combo)

        box = gui.hBox(None)
        gui.rubber(box)
        gui.checkBox(box, self, "show_intermediate",
                     _tr.m[3964, "Show details in non-leaves"],
    ...
    def _ctree_clean(self):
        self.infolabel.setText(_tr.m[3965, 'No tree.'])
    ...
    def _ctree_setup(self, model):
        ...
        if class_var.is_discrete:
            self.color_label.setText(_tr.m[3966, "Target class: "])
            self.color_combo.addItem(_tr.m[3967, "None"])
            self.color_combo.addItems(self.domain.class_vars[0].values)
            self.color_combo.setCurrentIndex(self.target_class_index)
        else:
            self.color_label.setText(_tr.m[3968, "Color by: "])
        ...
        self.infolabel.setText(_tr.e(_tr.c(3969, f'{nodes} {pl(nodes, "node")}, {leaves} {pl(leaves, "leaf|leaves")}')))
    ...
    def node_tooltip(self, node):
        ...
        if rule:
            rule = _tr.e(_tr.c(3970, f"<p><b>Selection</b></p><p>{rule}</p>"))
        ...
        if self.domain.class_var.is_discrete:
            ...
            content = _tr.e(_tr.c(3971, f"{nbp}<b>Distribution of</b> '{name}'</p><p>")) \
                + "<table>" + "".join(
            ...
            content = f"{nbp}{class_var.name} = {mean:.3g} ± {var:.3g}<br/>" + \
                _tr.e(_tr.c(3972, f"({self.tree_adapter.num_samples(node.node_inst)} instances)</p>"))

        split = self._update_node_info_attr_name(node, "")
        if split:
            split = _tr.e(_tr.c(3973, f"{nbp}<b>Next split: </b>{split}</p>"))
    ...
    def send_report(self):
        if not self.model:
            return
        items = [(_tr.m[3974, "Tree size"], self.infolabel.text()),
                 (_tr.m[3975, "Edge widths"],
                  (_tr.m[3976, "Fixed"], _tr.m[3977, "Relative to root"], _tr.m[3978, "Relative to parent"])[
                      # pylint: disable=invalid-sequence-index
                      self.line_width_method])]
        if self.domain.class_var.is_discrete:
            items.append((_tr.m[3979, "Target class"], self.color_combo.currentText()))
        elif self.regression_colors != self.COL_DEFAULT:
            items.append((_tr.m[3980, "Color by"], self.COL_OPTIONS[self.regression_colors]))
    ...
    def node_content_reg(self, node):
        ...
        text += _tr.e(_tr.c(3981, f"{insts} instances"))
        return text
