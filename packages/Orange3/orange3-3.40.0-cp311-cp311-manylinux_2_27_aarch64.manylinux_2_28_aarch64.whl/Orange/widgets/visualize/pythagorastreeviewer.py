...
class DiscreteTreeNode(TreeNode):
    ...
    def tooltip(self):
        ...
        return '<p>' \
            + text \
            + _tr.m[4052, '{}/{} samples ({:2.3f}%)'].format(
                int(samples), total, ratio * 100) \
            + '<hr>' \
            + (_tr.m[4053, 'Split by '] + splitting_attr.name
...
class ContinuousTreeNode(TreeNode):
    ...
    COLOR_METHODS = {
        _tr.m[4054, 'None']: COLOR_NONE,
        _tr.m[4055, 'Mean']: COLOR_MEAN,
        _tr.m[4056, 'Standard deviation']: COLOR_STD,
    }
    ...
    def tooltip(self):
        ...
        return _tr.m[4057, '<p>Mean: {:2.3f}'].format(mean) \
            + _tr.m[4058, '<br>Standard deviation: {:2.3f}'].format(std) \
            + _tr.m[4059, '<br>{} samples'].format(num_samples) \
            + '<hr>' \
            + (_tr.m[4060, 'Split by '] + splitting_attr.name
               if not self.tree.is_leaf(self.label) else '') \
            + ('<br><br>' if rules_str and not self.tree.is_leaf(self.label) else '') \
            + rules_str \
            + '</p>'
