...
class OWCorrespondenceAnalysis(widget.OWWidget):
    name = _tr.m[2637, "Correspondence Analysis"]
    description = _tr.m[2638, "Correspondence analysis for categorical multivariate data."]
    icon = "icons/CorrespondenceAnalysis.svg"
    keywords = _tr.m[2639, "correspondence analysis"]

    class Inputs:
        data = Input(_tr.m[2640, "Data"], Table)

    class Outputs:
        coordinates = Output(_tr.m[2641, "Coordinates"], Table, dynamic=False)
    ...
    class Error(widget.OWWidget.Error):
        empty_data = widget.Msg(_tr.m[2642, "Empty dataset"])
        no_disc_vars = widget.Msg(_tr.m[2643, "No categorical data"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2644, "Variables"])
        ...
        axes_box = gui.vBox(self.controlArea, _tr.m[2645, "Axes"])
        ...
        self.infotext = gui.widgetLabel(
            gui.vBox(self.controlArea, _tr.m[2646, "Contribution to Inertia"]), "\n"
        )
    ...
    def commit(self):
        ...
        if self.ca is not None:
            ...
            output_table = Table(
                Domain([ContinuousVariable(_tr.e(_tr.c(2647, f"Component {i + 1}")))
                        for i in range(rf.shape[1])],
                       metas=[StringVariable(_tr.m[2648, "Variable"]),
                              StringVariable(_tr.m[2649, "Value"])]),
                rf, metas=vars_data
            )
    ...
    def _setup_plot(self):
        ...
        ax.setLabel(_tr.m[2650, "Component {} ({:.1f}%)"]
                    .format(p_axes[0] + 1, inertia[p_axes[0]]))
        ax = self.plot.getAxis("left")
        ax.setLabel(_tr.m[2651, "Component {} ({:.1f}%)"]
    ...
    def _update_info(self):
        if self.ca is None:
            ...
            fmt = (_tr.m[2652, "Axis 1: {:.2f}\n"] + _tr.m[2653, "Axis 2: {:.2f}"])
            inertia = self.ca.inertia_of_axis()
            if np.sum(inertia) == 0:
                inertia = 100 * inertia
            else:
    ...
    def send_report(self):
        ...
        items = OrderedDict()
        items[_tr.m[2654, "Data instances"]] = len(self.data)
        if len(vars) == 1:
            items[_tr.m[2655, "Selected variable"]] = vars[0]
        else:
            items[_tr.m[2656, "Selected variables"]] = _tr.m[2657, "{} and {}"].format(
