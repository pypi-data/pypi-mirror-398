...
class OWPythonScript(OWWidget):
    name = _tr.m[1387, "Python Script"]
    description = _tr.m[1388, "Write a Python script and run it on input data or models."]
    category = _tr.m[1389, "Transform"]
    icon = "icons/PythonScript.svg"
    priority = 3150
    keywords = _tr.m[1390, "program, function"]
    ...
    def __init__(self):
        ...
        self.editorBox = gui.vBox(self, box=_tr.m[1391, "Editor"], spacing=4)
        ...
        self.editor_controls = gui.vBox(self.controlArea, box=_tr.m[1392, 'Preferences'])
        ...
        gui.checkBox(
            self.vim_box, self, 'vimModeEnabled', _tr.m[1393, 'Vim mode'],
            tooltip=_tr.m[1394, "Only for the coolest."],
        ...
        self.controlBox = gui.vBox(self.controlArea, _tr.m[1395, 'Library'])
        ...
        action.setToolTip(_tr.m[1396, "Add a new script to the library"])
        action.triggered.connect(self.onAddScript)
        w.addAction(action)

        action = QAction(unicodedata.lookup("MINUS SIGN"), self)
        action.setToolTip(_tr.m[1397, "Remove script from library"])
        action.triggered.connect(self.onRemoveScript)
        w.addAction(action)

        action = QAction(_tr.m[1398, "Update"], self)
        action.setToolTip(_tr.m[1399, "Save changes in the editor to library"])
        action.setShortcut(QKeySequence(QKeySequence.Save))
        action.triggered.connect(self.commitChangesToLibrary)
        w.addAction(action)

        action = QAction(_tr.m[1400, "More"], self, toolTip=_tr.m[1401, "More actions"])

        new_from_file = QAction(_tr.m[1402, "Import Script from File"], self)
        save_to_file = QAction(_tr.m[1403, "Save Selected Script to File"], self)
        restore_saved = QAction(_tr.m[1404, "Undo Changes to Selected Script"], self)
        ...
        self.execute_button = gui.button(self.buttonsArea, self, _tr.m[1405, 'Run'], callback=self.commit)

        self.run_action = QAction(_tr.m[1406, "Run script"], self, triggered=self.commit,
                                  shortcut=QKeySequence(Qt.ControlModifier | Qt.Key_R))
        self.addAction(self.run_action)

        self.saveAction = action = QAction(_tr.m[1407, "&Save"], self.text)
        action.setToolTip(_tr.m[1408, "Save script to file"])
        action.setShortcut(QKeySequence(QKeySequence.Save))
        action.setShortcutContext(Qt.WidgetWithChildrenShortcut)
        action.triggered.connect(self.saveScript)

        self.consoleBox = gui.vBox(self.splitCanvas, _tr.m[1409, 'Console'])
    ...
    def onAddScript(self, *_):
        self.libraryList.append(Script(_tr.m[1410, "New script"], self.text.toPlainText(), 0))
        self.setSelectedScript(len(self.libraryList) - 1)

    def onAddScriptFromFile(self, *_):
        filename, _ = QFileDialog.getOpenFileName(
            self, _tr.m[1411, 'Open Python Script'],
            os.path.expanduser("~/"),
            _tr.m[1412, 'Python files (*.py)\nAll files(*.*)']
        )
        if filename:
    ...
    def saveScript(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, _tr.m[1413, 'Save Python Script'],
            filename,
            _tr.m[1414, 'Python files (*.py)\nAll files(*.*)']
        )
        ...
        for signal in self.signal_names:
            ...
            if not isinstance(out_var, signal_type) and out_var is not None:
                self.Error.add_message(signal,
                                       _tr.m[1416, "'{}' has to be an instance of '{}'."].
