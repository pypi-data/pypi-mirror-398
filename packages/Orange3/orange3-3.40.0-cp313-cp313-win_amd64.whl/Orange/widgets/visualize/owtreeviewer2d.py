...
class OWTreeViewer2D(OWWidget, openclass=True):
    ...
    def __init__(self):
        ...
        box = gui.vBox(
            self.controlArea, _tr.m[3982, 'Tree'],
            sizePolicy=QSizePolicy(QSizePolicy.Minimum, QSizePolicy.Fixed))
        self.infolabel = gui.widgetLabel(box, _tr.m[3983, 'No tree.'])

        layout = QFormLayout()
        layout.setFieldGrowthPolicy(layout.ExpandingFieldsGrow)
        box = self.display_box = gui.widgetBox(self.controlArea, _tr.m[3984, "Display"],
                                               orientation=layout)
        layout.addRow(
            _tr.m[3985, "Zoom: "],
        ...
        layout.addRow(
            ...
            gui.comboBox(box, self, 'max_tree_depth',
                         items=[_tr.m[3988, "Unlimited"]] + [
                             _tr.e(_tr.c(3989, f"{x} levels")) for x in range(2, 10)],
                         addToLayout=False, sendSelectedValue=False,
                         callback=self.toggle_tree_depth, sizePolicy=policy))
        layout.addRow(
            _tr.m[3990, "Edge width: "],
            gui.comboBox(box, self, 'line_width_method',
                         items=[_tr.m[3991, 'Fixed'], _tr.m[3992, 'Relative to root'],
                                _tr.m[3993, 'Relative to parent']],
                         addToLayout=False,
    ...
    def send_report(self):
        ...
        if self.model:
            self.reportSection(_tr.m[3994, "Tree"])
