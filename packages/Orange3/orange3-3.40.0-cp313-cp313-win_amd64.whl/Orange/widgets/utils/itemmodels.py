...
class VariableListModel(PyListModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        if self._is_index_valid(index):
            ...
            if var is None and role == Qt.DisplayRole:
                return self.placeholder or _tr.m[3245, "None"]
    ...
    def variable_labels_tooltip(self, var):
        ...
        if var.attributes:
            ...
            text += _tr.m[3246, "<br/>Variable Labels:<br/>"]
            text += "<br/>".join(labels)
        return text

    def discrete_variable_tooltip(self, var):
        text = _tr.m[3247, "<b>%s</b><br/>Categorical with %i values: "] %\
    ...
    def time_variable_toltip(self, var):
        text = _tr.m[3248, "<b>%s</b><br/>Time"] % safe_text(var.name)
        text += self.variable_labels_tooltip(var)
        return text

    def continuous_variable_toltip(self, var):
        text = _tr.m[3249, "<b>%s</b><br/>Numeric"] % safe_text(var.name)
        text += self.variable_labels_tooltip(var)
        return text

    def string_variable_tooltip(self, var):
        text = _tr.m[3250, "<b>%s</b><br/>Text"] % safe_text(var.name)
        text += self.variable_labels_tooltip(var)
        return text
