...
class VarTableModel(QAbstractTableModel):
    ...
    places = _tr.m[3224, "feature"], _tr.m[3225, "target"], _tr.m[3226, "meta"], _tr.m[3227, "skip"]
    typenames = _tr.m[3228, "categorical"], _tr.m[3229, "numeric"], _tr.m[3230, "text"], _tr.m[3231, "datetime"]
    ...
    def headerData(self, i, orientation, role=Qt.DisplayRole):
        if orientation == Qt.Horizontal and role == Qt.DisplayRole and i < 4:
            return (_tr.m[3232, "Name"], _tr.m[3233, "Type"], _tr.m[3234, "Role"], _tr.m[3235, "Values"])[i]
...
class VarTypeDelegate(ComboDelegate):
    def setEditorData(self, combo, index):
        ...
        if no_numeric:
            ...
            items = [i for i in self.items if i not in (_tr.m[3236, "numeric"], _tr.m[3237, "datetime"])]
        else:
            items = self.items
