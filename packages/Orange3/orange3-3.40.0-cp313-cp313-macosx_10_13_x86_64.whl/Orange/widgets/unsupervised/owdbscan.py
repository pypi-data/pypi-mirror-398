...
class OWDBSCAN(widget.OWWidget):
    name = "DBSCAN"
    description = _tr.m[2658, "Density-based spatial clustering."]
    icon = "icons/DBSCAN.svg"
    priority = 2150

    class Inputs:
        data = Input(_tr.m[2659, "Data"], Table)

    class Outputs:
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Error(widget.OWWidget.Error):
        not_enough_instances = Msg((_tr.m[2660, "Not enough unique data instances. "] + _tr.m[2661, "At least two rows (with any defined values) are required."]))
        no_features = Msg(_tr.m[2662, "The data does not contain any features."])


    METRICS = [
        (_tr.m[2663, "Euclidean"], "euclidean"),
        (_tr.m[2664, "Manhattan"], "cityblock"),
        (_tr.m[2665, "Cosine"], "cosine")
    ]
    ...
    def __init__(self):
        ...
        box = gui.widgetBox(self.controlArea, _tr.m[2666, "Parameters"])
        gui.spin(box, self, "min_samples", 1, 100, 1,
                 callback=self._min_samples_changed,
                 label=_tr.m[2667, "Core point neighbors"])
        gui.doubleSpin(box, self, "eps", EPS_BOTTOM_LIMIT, 1000, 0.01,
                       callback=self._eps_changed,
                       label=_tr.m[2668, "Neighborhood distance"])

        box = gui.widgetBox(self.controlArea, self.tr(_tr.m[2669, "Distance Metric"]))
        gui.comboBox(box, self, "metric_idx",
                     items=list(zip(*self.METRICS))[0],
                     callback=self._metirc_changed)
        gui.checkBox(box, self, "normalize", _tr.m[2670, "Normalize features"],
        ...
        self.plot = SliderGraph(
            x_axis_label=_tr.m[2671, "Data items sorted by score"],
            y_axis_label=_tr.m[2672, "Distance to the k-th nearest neighbour"],
    ...
    def send_data(self):
        ...
        u_clust_var = get_unique_names(names, _tr.m[2673, "Cluster"])
        clust_var = DiscreteVariable(
            u_clust_var, values=[_tr.m[2674, "C%d"] % (x + 1) for x in range(k)])

        u_in_core = get_unique_names(names + [u_clust_var], _tr.m[2675, "DBSCAN Core"])
