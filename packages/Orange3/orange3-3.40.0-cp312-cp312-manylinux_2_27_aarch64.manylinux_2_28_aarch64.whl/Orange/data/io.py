...
class CSVReader(FileFormat, DataTableMixin):
    ...
    EXTENSIONS = ('.csv',)
    DESCRIPTION = _tr.m[76, 'Comma-separated values']
    DELIMITERS = ',;:\t$ '
...
class TabReader(CSVReader):
    ...
    EXTENSIONS = ('.tab', '.tsv')
    DESCRIPTION = _tr.m[77, 'Tab-separated values']
    DELIMITERS = '\t'
    PRIORITY = 10
...
class PickleReader(FileFormat):
    ...
    DESCRIPTION = _tr.m[78, 'Pickled Orange data']
    SUPPORT_COMPRESSED = True
    ...
    def read(self):
        with self.open(self.filename, 'rb') as f:
            ...
            if not isinstance(table, Table):
                raise TypeError(_tr.m[79, "file does not contain a data table"])
            else:
...
class BasketReader(FileFormat):
    ...
    DESCRIPTION = _tr.m[80, 'Basket file']
    SUPPORT_SPARSE_DATA = True
...
class _BaseExcelReader(FileFormat, DataTableMixin):
    ...
    def read(self):
        try:
            ...
            raise IOError(_tr.m[81, "Couldn't load spreadsheet from "] + self.filename)
        return table
...
class ExcelReader(_BaseExcelReader):
    ...
    EXTENSIONS = ('.xlsx',)
    DESCRIPTION = _tr.m[82, 'Microsoft Excel spreadsheet']
...
class XlsReader(_BaseExcelReader):
    ...
    EXTENSIONS = ('.xls',)
    DESCRIPTION = _tr.m[83, 'Microsoft Excel 97-2004 spreadsheet']
