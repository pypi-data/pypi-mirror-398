...
def check_results_adequacy(results, error_group, check_nan=True):
    ...
    if results is None:
        ...
        error_group.invalid_results(
            _tr.m[2150, "Categorical target variable is required."])
    elif not results.actual.size:
        error_group.invalid_results(
            _tr.m[2151, "Empty result on input. Nothing to display."])
        ...
        error_group.invalid_results(
            _tr.m[2152, "Results contain invalid values."])
    else:
        return results
    ...
    problems = [
        msg for condition, msg in (
            (results.folds is not None and len(results.folds) > 1,
             _tr.m[2153, "each training data sample produces a different model"]),
            (results.models is None,
             (_tr.m[2154, "test results do not contain stored models - try testing "] + _tr.m[2155, "on separate data or on training data"])),
            (len(selection) != 1,
             _tr.m[2156, "select a single model - the widget can output only one"]),
            (require_binary and len(results.domain.class_var.values) != 2,
             _tr.m[2157, "cannot calibrate non-binary models"]))
        if condition]
...
class ScoreTable(OWComponent, QObject):
    ...
    def update_header(self, scorers: List[Score]):
        ...
        for i, name, long_name, id_, in ((0, "Model", "Model", "Model_"),
                                         (1, _tr.m[2158, "Train"], _tr.m[2159, "Train time [s]"], "Train_"),
                                         (2, _tr.m[2160, "Test"], _tr.m[2161, "Test time [s]"], "Test_")):
