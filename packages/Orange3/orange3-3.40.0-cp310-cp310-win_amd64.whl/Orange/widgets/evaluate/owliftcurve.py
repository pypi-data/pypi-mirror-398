...
class ParameterSetter(CommonParameterSetter):
    WIDE_LINE_LABEL = _tr.m[1900, "Line"]
    DEFAULT_LINE_LABEL = _tr.m[1901, "Default Line"]

    WIDE_LINE_WIDTH = 3
    LINE_WIDTH = 1
    DEFAULT_LINE_WIDTH = 1

    WIDE_LINE_STYLE = _tr.m[1902, "Solid line"]
    LINE_STYLE = _tr.m[1903, "Solid line"]
    DEFAULT_LINE_STYLE = _tr.m[1904, "Dash line"]
...
class OWLiftCurve(widget.OWWidget):
    name = _tr.m[1905, "Performance Curve"]
    description = (_tr.m[1906, "Construct and display a performance curve "] + _tr.m[1907, "from the evaluation of classifiers."])
    icon = "icons/LiftCurve.svg"
    priority = 1020
    keywords = _tr.m[1908, "performance curve, lift, cumulative gain, precision, recall, curve"]

    class Inputs:
        evaluation_results = Input(
            _tr.m[1909, "Evaluation Results"], Orange.evaluation.Results)

    class Outputs:
        calibrated_model = Output(_tr.m[1910, "Calibrated Model"], Model)

    class Warning(widget.OWWidget.Warning):
        undefined_curves = Msg(
            _tr.m[1911, "Some curves are undefined; check models and data"])

    class Error(widget.OWWidget.Error):
        undefined_curves = Msg(_tr.m[1912, "No defined curves; check models and data"])

    class Information(widget.OWWidget.Information):
        no_output = Msg(_tr.m[1913, "Can't output a model: {}"])
    ...
    XLabels = (_tr.m[1914, "P Rate"], _tr.m[1915, "P Rate"], _tr.m[1916, "Recall"])
    YLabels = (_tr.m[1917, "Lift"], _tr.m[1918, "TP Rate"], _tr.m[1919, "Precision"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, box=_tr.m[1920, "Curve"])
        self.target_cb = gui.comboBox(
            box, self, "target_index",
            label=_tr.m[1921, "Target: "], orientation=Qt.Horizontal,
            callback=self._on_target_changed,
            contentsLength=8, searchable=True
        )
        gui.radioButtons(
            box, self, "curve_type",
            (_tr.m[1922, "Lift Curve"], _tr.m[1923, "Cumulative Gains"], _tr.m[1924, "Precision Recall"]),
            callback=self._on_curve_type_changed
        )

        self.classifiers_list_box = gui.listBox(
            self.controlArea, self, "selected_classifiers", "classifier_names",
            box=_tr.m[1925, "Models"],
            selectionMode=QListView.MultiSelection,
            callback=self._on_classifiers_changed
        )
        self.classifiers_list_box.setMaximumHeight(100)

        box = gui.vBox(self.controlArea, box=_tr.m[1926, "Settings"])
        gui.checkBox(box, self, "show_threshold", _tr.m[1927, "Show thresholds"],
                     callback=self._on_show_threshold_changed)
        gui.checkBox(box, self, "show_points", _tr.m[1928, "Show points"],
                     callback=self._on_show_points_changed)

        gui.rubber(self.controlArea)

        box = gui.vBox(self.controlArea, box=_tr.m[1929, "Area under the curve"])
    ...
    def _plot_curve(self, target, clf_idx, curve_type):
        ...
        def tip(x, y, data):
            ...
            return (f"{xlabel}: {round(x, 3)}\n" + (f"{ylabel}: {round(y, 3)}\n" + _tr.e(_tr.c(1930, f"Threshold: {round(data, 3)}"))))
    ...
    def _set_tooltip(self):
        ...
        if len(self.plot.curve_items) > 0:
            html = ('<div style="color: #333; font-size: 12px;"' + _tr.e(_tr.c(1931, ' <span>Probability threshold(s):</span>')))
            for item in self.plot.curve_items:
    ...
    def send_report(self):
        ...
        self.report_items(((_tr.m[1932, "Target class"], self.target_cb.currentText()),))
        self.report_plot()
