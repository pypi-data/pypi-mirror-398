...
class OWDistributions(OWWidget):
    name = _tr.m[3493, "Distributions"]
    description = _tr.m[3494, "Display value distributions of a data feature in a graph."]
    icon = "icons/Distribution.svg"
    priority = 120
    keywords = _tr.m[3495, "distributions, histogram"]

    class Inputs:
        data = Input(_tr.m[3496, "Data"], Table, doc=_tr.m[3497, "Set the input dataset"])

    class Outputs:
        selected_data = Output(_tr.m[3498, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)
        histogram_data = Output(_tr.m[3499, "Histogram Data"], Table, dynamic=False)

    class Error(OWWidget.Error):
        no_defined_values_var = \
            Msg(_tr.m[3500, "Variable '{}' does not have any defined values"])
        no_defined_values_pair = \
            Msg(_tr.m[3501, "No data instances with '{}' and '{}' defined"])

    class Warning(OWWidget.Warning):
        ignored_nans = Msg(_tr.m[3502, "Data instances with missing values are ignored"])
    ...
    Fitters = (
        (_tr.m[3503, "None"], None, (), ()),
        (_tr.m[3504, "Normal"], norm, ("loc", "scale"), ("μ", "σ")),
        ("Beta", beta, ("a", "b", "loc", "scale"),
         ("α", "β", "-loc", "-scale")),
        (_tr.m[3505, "Gamma"], gamma, ("a", "loc", "scale"), ("α", "β", "-loc", "-scale")),
        ("Rayleigh", rayleigh, ("loc", "scale"), ("-loc", "σ")),
        ("Pareto", pareto, ("b", "loc", "scale"), ("α", "-loc", "-scale")),
        (_tr.m[3506, "Exponential"], expon, ("loc", "scale"), ("-loc", "λ")),
        (_tr.m[3507, "Kernel density"], AshCurve, ("a",), ("",))
    )
    ...
    def __init__(self):
        ...
        varview = gui.listView(
            self.controlArea, self, "var", box=_tr.m[3508, "Variable"],
        ...
        gui.checkBox(
            varview.box, self, "sort_by_freq", _tr.m[3509, "Sort categories by frequency"],
            callback=self._on_sort_by_freq, stateWhenDisabled=False)

        box = self.continuous_box = gui.vBox(self.controlArea, _tr.m[3510, "Distribution"])
        gui.comboBox(
            box, self, "fitted_distribution", label=_tr.m[3511, "Fitted distribution"],
        ...
        slider = gui.hSlider(
            ...
            label=_tr.m[3512, "Bin width"], orientation=Qt.Horizontal,
        ...
        self.smoothing_box = gui.hSlider(
            ...
            label=_tr.m[3513, "Smoothing"], orientation=Qt.Horizontal,
            minValue=2, maxValue=20, callback=self.replot, disabled=True)
        gui.checkBox(
            box, self, "hide_bars", _tr.m[3514, "Hide bars"], stateWhenDisabled=False,
            callback=self._on_hide_bars_changed,
            disabled=not self.fitted_distribution)

        box = gui.vBox(self.controlArea, _tr.m[3515, "Columns"])
        gui.comboBox(
            box, self, "cvar", label=_tr.m[3516, "Split by"], orientation=Qt.Horizontal,
            searchable=True,
            model=DomainModel(placeholder=_tr.m[3517, "(None)"],
                              valid_types=(DiscreteVariable), ),
            callback=self._on_cvar_changed, contentsLength=18)
        gui.checkBox(
            box, self, "stacked_columns", _tr.m[3518, "Stack columns"],
            callback=self.replot)
        gui.checkBox(
            box, self, "show_probs", _tr.m[3519, "Show probabilities"],
            callback=self._on_show_probabilities_changed)
        gui.checkBox(
            box, self, "cumulative_distr", _tr.m[3520, "Show cumulative distribution"],
    ...
    def _on_show_probabilities_changed(self):
        ...
        if self.show_probs:
            label.setText(_tr.m[3521, "Fitted probability"])
            label.setToolTip(
                _tr.m[3522, "Chosen distribution is used to compute Bayesian probabilities"])
        else:
            label.setText(_tr.m[3523, "Fitted distribution"])
            label.setToolTip("")
        self.replot()
    ...
    def _set_axis_names(self):
        ...
        if self.show_probs and self.cvar:
            leftaxis.setLabel(
                _tr.e(_tr.c(3524, f"Probability of '{self.cvar.name}' at given '{self.var.name}'")))
        else:
            leftaxis.setLabel(_tr.m[3525, "Frequency"])
        ...
        if self.is_valid:
            ...
            if self.var.is_continuous:  # annotate with bins
                ...
                annotated_data = create_groups_table(
                    annotated_data, hist_indices, var_name=_tr.m[3526, "Bin"], values=hist_values)
    ...
    def _get_histogram_table(self):
        ...
        var_bin = DiscreteVariable(_tr.m[3527, "Bin"], [bar.desc for bar in self.bar_items])
        var_freq = ContinuousVariable(_tr.m[3528, "Count"])
        X = []
        if self.cvar:
    ...
    def send_report(self):
        ...
        self.report_plot()
        if self.cumulative_distr:
            text = _tr.e(_tr.c(3529, f"Cummulative distribution of '{self.var.name}'"))
        else:
            text = _tr.e(_tr.c(3530, f"Distribution of '{self.var.name}'"))
        if self.cvar:
            text += _tr.e(_tr.c(3531, f" with columns split by '{self.cvar.name}'"))
