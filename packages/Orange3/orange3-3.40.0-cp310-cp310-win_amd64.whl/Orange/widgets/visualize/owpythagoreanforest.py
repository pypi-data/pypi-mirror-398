...
class OWPythagoreanForest(OWWidget):
    name = _tr.m[3791, "Pythagorean Forest"]
    description = _tr.m[3792, 'Pythagorean forest for visualising random forests.']
    icon = 'icons/PythagoreanForest.svg'
    settings_version = 2
    keywords = _tr.m[3793, "pythagorean forest, fractal"]

    priority = 1001

    class Inputs:
        random_forest = Input(_tr.m[3794, "Random Forest"], RandomForestModel, replaces=[_tr.m[3795, "Random forest"]])

    class Outputs:
        tree = Output(_tr.m[3796, "Tree"], TreeModel)
    ...
    SIZE_CALCULATION = [
        (_tr.m[3797, 'Normal'], lambda x: x),
        (_tr.m[3798, 'Square root'], lambda x: sqrt(x)),
        (_tr.m[3799, 'Logarithmic'], lambda x: log(x + 1)),
    ]
    ...
    def __init__(self):
        ...
        # CONTROL AREA
        # Tree info area
        box_info = gui.widgetBox(self.controlArea, _tr.m[3800, 'Forest'])
        self.ui_info = gui.widgetLabel(box_info)

        # Display controls area
        box_display = gui.widgetBox(self.controlArea, _tr.m[3801, 'Display'])
        ...
        self.ui_depth_slider = gui.hSlider(
            box_display, self, 'depth_limit', label=_tr.m[3802, 'Depth'], ticks=False,
            maxValue=900
        )  # type: QSlider
        self.ui_target_class_combo = gui.comboBox(
            box_display, self, 'target_class_index', label=_tr.m[3803, 'Target class'],
            orientation=Qt.Horizontal, items=[], contentsLength=8,
            searchable=True
        )
        self.ui_size_calc_combo = gui.comboBox(
            box_display, self, 'size_calc_idx', label=_tr.m[3804, 'Size'],
            orientation=Qt.Horizontal,
            items=list(zip(*self.SIZE_CALCULATION))[0], contentsLength=8,
        )
        self.ui_zoom_slider = gui.hSlider(
            box_display, self, 'zoom', label=_tr.m[3805, 'Zoom'], ticks=False, minValue=100,
    ...
    def _update_info_box(self):
        self.ui_info.setText(_tr.m[3806, 'Trees: {}'].format(len(self.forest.trees)))
    ...
    def _update_target_class_combo(self):
        ...
        if self.instances.domain.has_discrete_class:
            label_text = _tr.m[3807, 'Target class']
            values = [c.title() for c in self.instances.domain.class_vars[0].values]
            values.insert(0, _tr.m[3808, 'None'])
        else:
            label_text = _tr.m[3809, 'Node color']
    ...
    def _clear_info_box(self):
        self.ui_info.setText(_tr.m[3810, 'No forest on input.'])
