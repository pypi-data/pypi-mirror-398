...
class ColorGradientSelection(QWidget):
    ...
    def __init__(self, *args, thresholds=(0.0, 1.0), center=None, **kwargs):
        ...
        slider = self.slider = IntervalSlider(
            ...
            toolTip=self.tr(_tr.m[3195, "Low gradient threshold"]),
            whatsThis=self.tr((_tr.m[3196, "Applying a low threshold will squeeze the "] + _tr.m[3197, "gradient from the lower end"]))
        )
        form.addRow(self.tr(_tr.m[3198, "Range:"]), slider)
    ...
    def __update_center_visibility(self):
        ...
        if visible:
            ...
            self.layout().insertRow(1, _tr.m[3199, "Center at:"], self.center_edit)
