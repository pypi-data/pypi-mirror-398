...
class CustomRuleClassifier(_RuleClassifier):
    ...
    def __init__(self, domain, rule_list, params):
        ...
        self.rule_ordering = params[_tr.m[2411, "Rule ordering"]]
        self.covering_algorithm = params[_tr.m[2412, "Covering algorithm"]]
        self.params = params

    def predict(self, X):
        if (self.rule_ordering == _tr.m[2413, "ordered"] and
                self.covering_algorithm == _tr.m[2414, "exclusive"]):
            return self.ordered_predict(X)

        if (self.rule_ordering == _tr.m[2415, "unordered"] or
                self.covering_algorithm == _tr.m[2416, "weighted"]):
...
class CustomRuleLearner(_RuleLearner):
    ...
    def __init__(self, preprocessors, base_rules, params):
        ...
        self.rule_ordering = params[_tr.m[2417, "Rule ordering"]]

        # top-level control procedure (covering algorithm)
        self.covering_algorithm = params[_tr.m[2418, "Covering algorithm"]]
        if self.covering_algorithm == _tr.m[2419, "exclusive"]:
            self.cover_and_remove = self.exclusive_cover_and_remove
        elif self.covering_algorithm == _tr.m[2420, "weighted"]:
            self.gamma = params[_tr.m[2421, "Gamma"]]
            self.cover_and_remove = self.weighted_cover_and_remove

        # bottom-level search procedure (search algorithm)
        self.rule_finder.search_algorithm.beam_width = params[_tr.m[2422, "Beam width"]]

        # bottom-level search procedure (search strategy)
        self.rule_finder.search_strategy.constrain_continuous = True
        self.rule_finder.search_strategy.restrict_equality = params[_tr.m[2423, "Restrict to equality"]]

        # bottom-level search procedure (search heuristics)
        evaluation_measure = params[_tr.m[2424, "Evaluation measure"]]
        if evaluation_measure == _tr.m[2425, "entropy"]:
            evaluator = EntropyEvaluator()
        elif evaluation_measure == _tr.m[2426, "laplace"]:
            evaluator = LaplaceAccuracyEvaluator()
        elif evaluation_measure == _tr.m[2427, "wracc"]:
        ...
        min_rule_cov = params[_tr.m[2428, "Minimum rule coverage"]]
        max_rule_length = params[_tr.m[2429, "Maximum rule length"]]
        ...
        # bottom-level search procedure (over-fitting avoidance heuristics)
        default_alpha = params[_tr.m[2430, "Default alpha"]]
        parent_alpha = params[_tr.m[2431, "Parent alpha"]]
    ...
    def fit_storage(self, data):
        ...
        if self.rule_ordering == _tr.m[2432, "ordered"]:
            ...
            if (not rule_list or rule_list and rule_list[-1].length > 0 or
                    self.covering_algorithm == _tr.m[2433, "weighted"]):
                rule_list.append(
                    self.generate_default_rule(X, Y, W, data.domain))

        elif self.rule_ordering == _tr.m[2434, "unordered"]:
...
class OWRuleLearner(OWBaseLearner):
    name = _tr.m[2435, "CN2 Rule Induction"]
    description = _tr.m[2436, "Induce rules from data using CN2 algorithm."]
    icon = "icons/CN2RuleInduction.svg"
    replaces = [
        "Orange.widgets.classify.owrules.OWRuleLearner",
    ]
    priority = 19
    keywords = _tr.m[2437, "cn2 rule induction"]

    LEARNER = CustomRuleLearner
    supports_sparse = False

    storage_orders = [_tr.m[2438, "ordered"], _tr.m[2439, "unordered"]]
    storage_covers = [_tr.m[2440, "exclusive"], _tr.m[2441, "weighted"]]
    storage_measures = [_tr.m[2442, "entropy"], _tr.m[2443, "laplace"], _tr.m[2444, "wracc"]]
    ...
    def add_main_layout(self):
        ...
        middle_box = gui.vBox(widget=self.controlArea, box=_tr.m[2451, "Rule search"])

        gui.comboBox(
            widget=middle_box, master=self, value="evaluation_measure",
            label=_tr.m[2452, "Evaluation measure:"], orientation=Qt.Horizontal,
            items=(_tr.m[2453, "Entropy"], _tr.m[2454, "Laplace accuracy"], _tr.m[2455, "WRAcc"]),
            callback=self.settings_changed, contentsLength=3)

        gui.spin(
            widget=middle_box, master=self, value="beam_width", minv=1,
            maxv=100, step=1, label=_tr.m[2456, "Beam width:"], orientation=Qt.Horizontal,
            callback=self.settings_changed, alignment=Qt.AlignRight,
            controlWidth=80)

        # bottom-level search procedure (over-fitting avoidance bias)
        bottom_box = gui.vBox(widget=self.controlArea, box=_tr.m[2457, "Rule filtering"])

        gui.spin(
            widget=bottom_box, master=self, value="min_covered_examples", minv=1,
            maxv=10000, step=1, label=_tr.m[2458, "Minimum rule coverage:"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80)

        gui.spin(
            widget=bottom_box, master=self, value="max_rule_length",
            minv=1, maxv=100, step=1, label=_tr.m[2459, "Maximum rule length:"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80)

        gui.doubleSpin(
            widget=bottom_box, master=self, value="default_alpha", minv=0.0,
            maxv=1.0, step=0.01, label=_tr.m[2460, "Statistical significance (default α):"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80,
        ...
        gui.doubleSpin(
            ...
            maxv=1.0, step=0.01, label=_tr.m[2461, "Relative significance (parent α):"],
            orientation=Qt.Horizontal, callback=self.settings_changed,
            alignment=Qt.AlignRight, controlWidth=80,
        ...
        gui.checkBox(
            ...
            label=_tr.m[2462, "Restrict operator for categorical values to equality"],
            callback=self.settings_changed,
        )
    ...
    def get_learner_parameters(self):
        return OrderedDict([
            (_tr.m[2463, "Rule ordering"], self.storage_orders[self.rule_ordering]),
            (_tr.m[2464, "Covering algorithm"], self.storage_covers[self.covering_algorithm]),
            (_tr.m[2465, "Gamma"], self.gamma),
            (_tr.m[2466, "Evaluation measure"], self.storage_measures[self.evaluation_measure]),
            (_tr.m[2467, "Restrict to equality"], self.restrict_equality),
            (_tr.m[2468, "Beam width"], self.beam_width),
            (_tr.m[2469, "Minimum rule coverage"], self.min_covered_examples),
            (_tr.m[2470, "Maximum rule length"], self.max_rule_length),
            (_tr.m[2471, "Default alpha"], (1.0 if not self.checked_default_alpha
                               else self.default_alpha)),
            (_tr.m[2472, "Parent alpha"], (1.0 if not self.checked_parent_alpha
