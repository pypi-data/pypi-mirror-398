...
FileFormats = [
    FileFormat("text/csv", _tr.m[485, "Text - comma separated"], ("*.csv", "*")),
    FileFormat("text/tab-separated-values", _tr.m[486, "Text - tab separated"], ("*.tsv", "*")),
    FileFormat("text/plain", _tr.m[487, "Text - all files"], ("*.txt", "*")),
]
...
def default_options_for_mime_type(
    ...
    delimiters = None
    try_encodings = ["utf-8", "utf-16", _tr.m[488, "iso8859-1"]]
...
class OWCSVFileImport(OWUrlDropBase):
    name = _tr.m[489, "CSV File Import"]
    description = _tr.m[490, "Import a data table from a CSV formatted file."]
    icon = "icons/CSVFile.svg"
    priority = 11
    category = _tr.m[491, "Data"]
    keywords = _tr.m[492, "csv file import, file, load, read, open, csv"]

    class Outputs:
        data = widget.Output(
            name=_tr.m[493, "Data"],
            type=Orange.data.Table,
            doc=_tr.m[494, "Loaded data set."],
            dynamic=False,
        )
        data_frame = widget.Output(
            name=_tr.m[495, "Data Frame"],
            type=pd.DataFrame,
            doc="",
            auto_summary=False,
            dynamic=False,
    ...
    class Error(widget.OWWidget.Error):
        error = widget.Msg(
            _tr.m[496, "Unexpected error"]
        )
        encoding_error = widget.Msg(
            (_tr.m[497, "Encoding error\n"] + (_tr.m[498, "The file might be encoded in an unsupported encoding or it "] + _tr.m[499, "might be binary"]))
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        grid = QGridLayout()
        grid.addWidget(QLabel(_tr.m[500, "File:"], self), 0, 0, 1, 1)
        ...
        self.recent_combo = ItemStyledComboBox(
            self, objectName=_tr.m[501, "recent-combo"], toolTip=_tr.m[502, "Recent files."],
            sizeAdjustPolicy=QComboBox.AdjustToMinimumContentsLengthWithIcon,
            minimumContentsLength=16, placeholderText=_tr.m[503, "Recent files…"]
        )
        ...
        self.browse_button = QPushButton(
            ...
            toolTip=_tr.m[504, "Browse filesystem"], autoDefault=False,
        )
        ...
        ac = menu.addAction(_tr.m[505, "Import any file…"])
        ac.triggered.connect(self.browse)

        ac = menu.addAction(_tr.m[506, "Import relative to workflow file…"])
        ac.setToolTip(_tr.m[507, "Import a file within the workflow file directory"])
        ...
        self.import_options_button = QPushButton(
            _tr.m[509, "Import Options…"], enabled=False, autoDefault=False,
    ...
    def _browse_dialog(self):
        dlg = FileDialog(
            self, windowTitle=self.tr(_tr.m[510, "Open Data File"]),
    ...
    def _might_be_binary_mb(self, path) -> QMessageBox:
        mb = QMessageBox(
            ...
            text=self.tr((_tr.m[511, "The '{basename}' may be a binary file.\n"] + _tr.m[512, "Are you sure you want to continue?"])).format(
    ...
    def _path_must_be_relative_mb(self, prefix: str) -> QMessageBox:
        mb = QMessageBox(
            parent=self, windowTitle=self.tr(_tr.m[513, "Invalid path"]),
            icon=QMessageBox.Warning,
            text=self.tr(_tr.m[514, "Selected path is not within '{prefix}'"]).format(
                prefix=prefix
            ),
        )
        ...
        if status == QFileDialog.Accepted:
            ...
            if idx is not None:
                ...
                if options_ is not None:
                    options = options_
            dlg = CSVImportDialog(
                self, windowTitle=_tr.m[515, "Import Options"], sizeGripEnabled=True)
    ...
    def _activate_import_dialog_for_item(
        ...
        dlg = CSVImportDialog(
            self, windowTitle=_tr.m[516, "Import Options"], sizeGripEnabled=True,
        )
        dlg.setWindowModality(Qt.WindowModal)
        dlg.setAttribute(Qt.WA_DeleteOnClose)
    ...
    def cancel(self):
        ...
        if self.__watcher is not None:
            ...
            self.setStatusMessage(_tr.m[517, "Cancelled"])
            self.summary_text.setText(
                _tr.m[518, "<div>Cancelled<br/><small>Press 'Reload' to try again</small></div>"]
            )

    def __set_running_state(self):
        self.progressBarInit()
        self.setBlocking(True)
        self.setStatusMessage(_tr.m[519, "Running"])
        self.cancel_button.setEnabled(True)
        self.load_button.setText(_tr.m[520, "Restart"])
        path = self.current_item().path()
        self.Error.clear()
        self.summary_text.setText(
            _tr.m[521, "<div>Loading: <i>{}</i><br/>"].format(prettyfypath(path))
        )
    ...
    def __clear_running_state(self, ):
        ...
        self.load_button.setText(_tr.m[522, "Reload"])
    ...
    def __set_error_state(self, err):
        ...
        basename = os.path.basename(path)
        if isinstance(err, UnicodeDecodeError):
            text = (_tr.m[523, "<div><i>{basename}</i> was not loaded due to a text encoding "] + (_tr.m[524, "error. The file might be saved in an unknown or invalid "] + _tr.m[525, "encoding, or it might be a binary file.</div>"])).format(
                basename=escape(basename)
            )
        else:
            text = (_tr.m[526, "<div><i>{basename}</i> was not loaded due to an error:"] + "<p style='white-space: pre;'>{err}</p>").format(
                basename=escape(basename),
    ...
    def _update_status_messages(self, data):
        ...
        self.summary_text.setText((_tr.e(_tr.c(527, f"{n_instances} {pl(n_instances, 'row')}, ")) + (_tr.e(_tr.c(528, f"{n_features} {pl(n_features, 'feature')}, ")) + _tr.e(_tr.c(529, f"{n_meta} {pl(n_meta, 'meta')}")))))
...
def _open(path, mode, encoding=None):
    ...
    if ext == ".gz":
        ...
        if len(filelist) == 1:
            ...
            raise ValueError(_tr.m[530, "Expected a single file in the archive."])
    else:
