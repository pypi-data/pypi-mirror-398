...
class CorrelationType(IntEnum):
    ...
    def items():
        ...
        """
        return [_tr.m[398, "Pearson correlation"], _tr.m[399, "Spearman correlation"]]
...
class CorrelationRank(VizRankDialogAttrPair):
    ...
    def row_for_state(self, score, state):
        ...
        if np.isnan(score[1]):
            correlation_item = QStandardItem(_tr.m[400, "N/A"])
        else:
...
class OWCorrelations(OWWidget):
    name = _tr.m[401, "Correlations"]
    description = _tr.m[402, "Compute all pairwise attribute correlations."]
    icon = "icons/Correlations.svg"
    priority = 1106
    category = _tr.m[403, "Unsupervised"]

    class Inputs:
        data = Input(_tr.m[404, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[405, "Data"], Table)
        features = Output(_tr.m[406, "Features"], AttributeList)
        correlations = Output(_tr.m[407, "Correlations"], Table, dynamic=False)
    ...
    class Information(OWWidget.Information):
        removed_cons_feat = Msg(_tr.m[408, "Constant features have been removed."])

    class Error(OWWidget.Error):
        not_enough_vars = Msg(_tr.m[409, "At least two numeric features are needed."])
        not_enough_inst = Msg(_tr.m[410, "At least two instances are needed."])
    ...
    def __init__(self):
        ...
        self.feature_model = DomainModel(
            ...
            placeholder=_tr.m[411, "(All combinations)"], valid_types=ContinuousVariable)
        gui.comboBox(
        ...
        gui.checkBox(
            box, self, "impute_missing", _tr.m[412, "Impute missing values"],
            toolTip=(_tr.m[413, "Replace missing values with means;\n"] + (_tr.m[414, "if disabled, rows with missing values for the corre"] + _tr.m[415, "sponding variables are ignored"])),
    ...
    def commit(self):
        ...
        attrs = [ContinuousVariable(_tr.m[416, "Correlation"]),
                 ContinuousVariable(_tr.m[417, "uncorrected p"]),
                 ContinuousVariable(_tr.m[418, "FDR"])]
        metas = [StringVariable(_tr.m[419, "Feature 1"]), StringVariable(_tr.m[420, "Feature 2"])]
        ...
        corr_table.name = _tr.m[421, "Correlations"]
