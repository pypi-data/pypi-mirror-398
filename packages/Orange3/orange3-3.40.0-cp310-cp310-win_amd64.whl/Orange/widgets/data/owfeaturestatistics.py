...
def format_time_diff(start, end, round_up_after=2):
    ...
    if years >= round_up_after:
        return _tr.e(_tr.c(863, f'~{years} years'))
    elif months >= round_up_after:
        return _tr.e(_tr.c(864, f'~{months} months'))
    elif weeks >= round_up_after:
        return _tr.e(_tr.c(865, f'~{weeks} weeks'))
    elif days >= round_up_after:
        return _tr.e(_tr.c(866, f'~{days} days'))
    elif hours >= round_up_after:
        return _tr.e(_tr.c(867, f'~{hours} hours'))
    elif minutes >= round_up_after:
        return _tr.e(_tr.c(868, f'~{minutes} minutes'))
    else:
        return _tr.e(_tr.c(869, f'{seconds} seconds'))
...
class FeatureStatisticsTableModel(AbstractSortTableModel):
    ...
    class Columns(IntEnum):
        ...
        def name(self):
            return {self.ICON: '',
                    self.NAME: _tr.m[870, 'Column'],
                    self.DISTRIBUTION: _tr.m[871, 'Distribution'],
                    self.CENTER: _tr.m[872, 'Mean'],
                    self.MODE: _tr.m[873, 'Mode'],
                    self.MEDIAN: _tr.m[874, 'Median'],
                    self.DISPERSION: _tr.m[875, 'Dispersion'],
                    self.MIN: _tr.m[876, 'Min.'],
                    self.MAX: _tr.m[877, 'Max.'],
                    self.MISSING: _tr.m[878, 'Missing'],
                    }[self.value]
    ...
    def get_statistics_table(self):
        ...
        if any(contivars):
            ...
            attrs = [ContinuousVariable(name)
                     for name in (_tr.m[879, "Entropy"], self.Columns.MISSING.name)]
        ...
        meta_attrs = [StringVariable(_tr.m[880, 'Column']), StringVariable(_tr.m[881, 'Mode'])]

        domain = Domain(attributes=attrs, metas=meta_attrs)
        statistics = Table.from_numpy(domain, x, metas=metas)
        statistics.name = _tr.e(_tr.c(882, f'{self.table.name} (Column Statistics)'))
        return statistics
...
class OWFeatureStatistics(widget.OWWidget):
    name = _tr.m[883, 'Column Statistics']
    description = _tr.m[884, 'Show basic statistics for columns.']
    icon = 'icons/FeatureStatistics.svg'
    keywords = _tr.m[885, 'feature, variable']

    class Inputs:
        data = Input(_tr.m[886, 'Data'], Table, default=True)

    class Outputs:
        reduced_data = Output(_tr.m[887, 'Reduced Data'], Table, default=True)
        statistics = Output(_tr.m[888, 'Statistics'], Table)
