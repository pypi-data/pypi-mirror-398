...
""".lstrip()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nameedit = QLineEdit(
            placeholderText=_tr.m[817, "Name..."],
            sizePolicy=QSizePolicy(QSizePolicy.Minimum,
                                   QSizePolicy.Fixed)
        )

        self.metaattributecb = QCheckBox(_tr.m[818, "Meta attribute"])

        self.expressionedit = QLineEdit(
            placeholderText=_tr.m[819, "Expression..."],
            toolTip=self.ExpressionTooltip)

        self.attrs_model = itemmodels.VariableListModel(
            [_tr.m[820, "Select Column"]], parent=self)
        ...
        self.funcs_model = itemmodels.PyListModelTooltip(
            chain([_tr.m[821, "Select Function"]], sorted_funcs),
    ...
    def setEditorData(self, data, domain):
        ...
        self.attrs_model[:] = [_tr.m[822, "Select Feature"]]
...
class ContinuousFeatureEditor(FeatureEditor):
    ExpressionTooltip = _tr.m[823, "A numeric expression\n\n"] \
...
class DateTimeFeatureEditor(FeatureEditor):
    ExpressionTooltip = FeatureEditor.ExpressionTooltip + \
        (_tr.m[824, "Result must be a string in ISO-8601 format "] + (_tr.m[825, "(e.g. 2019-07-30T15:37:27 or a part thereof),\n"] + _tr.m[826, "or a number of seconds since Jan 1, 1970."]))
...
class DiscreteFeatureEditor(FeatureEditor):
    ExpressionTooltip = FeatureEditor.ExpressionTooltip + \
        (_tr.m[827, "Result must be a string, if values are not explicitly given\n"] + _tr.m[828, "or a zero-based integer indices into a list of values given below."])

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        tooltip = \
            (_tr.m[829, "If values are given, above expression must return zero-based "] + _tr.m[830, "integer indices into that list."])
        self.valuesedit = QLineEdit(placeholderText="A, B ...", toolTip=tooltip)
        self.valuesedit.textChanged.connect(self._invalidate)

        layout = self.layout()
        label = QLabel(self.tr(_tr.m[831, "Values (optional)"]))
...
class StringFeatureEditor(FeatureEditor):
    ExpressionTooltip = _tr.m[832, "A string expression\n\n"] \
                        + FeatureEditor.ExpressionTooltip
...
class OWFeatureConstructor(OWWidget, ConcurrentWidgetMixin):
    name = "Formula"
    description = (_tr.m[833, "Construct new features (data columns) from a set of "] + _tr.m[834, "existing features in the input dataset."])
    category = _tr.m[835, "Transform"]
    icon = "icons/FeatureConstructor.svg"
    keywords = _tr.m[836, "feature constructor, function, lambda, calculation"]
    priority = 2240

    class Inputs:
        data = Input(_tr.m[837, "Data"], Orange.data.Table)

    class Outputs:
        data = Output(_tr.m[838, "Data"], Orange.data.Table)
    ...
    class Error(OWWidget.Error):
        more_values_needed = Msg(_tr.m[839, "Categorical feature {} needs more values."])
        invalid_expressions = Msg(_tr.m[840, "Invalid expressions: {}."])
        transform_error = Msg("{}")

    class Information(OWWidget.Information):
        replaces_existing = Msg(
            (_tr.m[841, "A new variable that is named the same as an existing one will "] + _tr.m[842, "replace it on the output."])
        )
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[843, "Variable Definitions"])
        ...
        self.addbutton = QPushButton(
            _tr.m[844, "New"], toolTip=_tr.m[845, "Create a new variable"],
            minimumWidth=120,
        ...
        menu.addSeparator()
        self.duplicateaction = menu.addAction(_tr.m[850, "Duplicate Selected Variable"])
        self.duplicateaction.triggered.connect(self.duplicateFeature)
        self.duplicateaction.setEnabled(False)
        self.addbutton.setMenu(menu)

        self.removebutton = QPushButton(
            _tr.m[851, "Remove"], toolTip=_tr.m[852, "Remove selected variable"],
            minimumWidth=120,
        ...
        self.fix_button = gui.button(
            self.buttonsArea, self, _tr.m[853, "Upgrade Expressions"],
            callback=self.fix_expressions)
        self.fix_button.setHidden(True)
        gui.button(self.buttonsArea, self, _tr.m[854, "Send"], callback=self.apply, default=True)
    ...
    def send_report(self):
        ...
        for feature in self.featuremodel:
            if isinstance(feature, DiscreteDescriptor):
                desc = _tr.m[855, "categorical"]
                if feature.values:
                    desc += _tr.m[856, " with values "] \
                            + ", ".join(f"'{val}'" for val in feature.values)
                if feature.ordered:
                    desc += _tr.m[857, "; ordered"]
            elif isinstance(feature, ContinuousDescriptor):
                desc = _tr.m[858, "numeric"]
            elif isinstance(feature, DateTimeDescriptor):
                desc = _tr.m[859, "date/time"]
            else:
                desc = _tr.m[860, "text"]
            items[feature.name] = f"{feature.expression} ({desc})"
        self.report_items(_tr.e(_tr.c(861, f"Constructed {pl(len(items), 'feature')}")), items)

    def fix_expressions(self):
        dlg = QMessageBox(
            QMessageBox.Question,
            _tr.m[862, "Fix Expressions"],
