...
def _check_column_combinations(
    ...
    if fit_regression and not column.is_continuous:
        raise ValueError(
            ("Intercept and coefficient are only allowed for continuous " + "variables"))
...
class ColumnLearner(Learner):
    def __init__(self,
        ...
        self.name = _tr.e(_tr.c(138, f"column '{column.name}'"))
...
class ColumnModel(Model):
    def __init__(self,
        ...
        self.name = _tr.e(_tr.c(139, f"column '{column.name}'{pars}"))
        ...
        if self.column.is_discrete:
            ...
            if self.coefficient is None:
                if not valid_prob_range(vals):
                    raise ValueError(("Column values must be in [0, 1] range " + "unless logistic function is applied"))
