...
class OWSave(OWSaveBase):
    name = _tr.m[1475, "Save Data"]
    description = _tr.m[1476, "Save data to an output file."]
    icon = "icons/Save.svg"
    category = _tr.m[1477, "Data"]
    keywords = _tr.m[1478, "save data, export"]

    settings_version = 3

    class Inputs:
        data = Input(_tr.m[1479, "Data"], Table)

    class Error(OWSaveBase.Error):
        unsupported_sparse = widget.Msg(_tr.m[1480, "Use Pickle format for sparse data."])
    ...
    def __init__(self):
        ...
        self.grid.addWidget(
            gui.checkBox(
                ...
                _tr.m[1481, "Add type annotations to header"],
                tooltip=
                (_tr.m[1482, "Some formats (Tab-delimited, Comma-separated) can include \n"] + _tr.m[1483, "additional information about variables types in header rows."]),
    ...
    def get_filters(cls):
        ...
        return {
            **{f"{w.DESCRIPTION} (*{w.EXTENSIONS[0]})": w
               for w in writers},
            **{_tr.e(_tr.c(1484, f"Compressed {w.DESCRIPTION} (*{w.EXTENSIONS[0]}.gz)")): w
    ...
    def send_report(self):
        ...
        writer = self.writer
        noyes = [_tr.m[1485, "No"], _tr.m[1486, "Yes"]]
        self.report_items((
            (_tr.m[1487, "File name"], self.filename or _tr.m[1488, "not set"]),
            (_tr.m[1489, "Format"], writer and writer.DESCRIPTION),
            (_tr.m[1490, "Type annotations"],
