...
class OWROCAnalysis(widget.OWWidget):
    name = _tr.m[2045, "ROC Analysis"]
    description = (_tr.m[2046, "Display the Receiver Operating Characteristics curve "] + _tr.m[2047, "based on the evaluation of classifiers."])
    icon = "icons/ROCAnalysis.svg"
    priority = 1010
    keywords = _tr.m[2048, "roc analysis, analyse"]

    class Inputs:
        evaluation_results = Input(_tr.m[2049, "Evaluation Results"], Orange.evaluation.Results)

    class Outputs:
        calibrated_model = Output(_tr.m[2050, "Calibrated Model"], Model)

    class Information(widget.OWWidget.Information):
        no_output = Msg(_tr.m[2051, "Can't output a model: {}"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2052, "Plot"])
        self.target_cb = gui.comboBox(
            box, self, "target_index",
            label=_tr.m[2053, "Target"], orientation=Qt.Horizontal,
            callback=self._on_target_changed,
            contentsLength=8, searchable=True)

        gui.widgetLabel(box, _tr.m[2054, "Classifiers"])
        ...
        abox = gui.vBox(self.controlArea, _tr.m[2055, "Curves"])
        gui.comboBox(abox, self, "roc_averaging",
                     items=[_tr.m[2056, "Merge Predictions from Folds"], _tr.m[2057, "Mean TP Rate"],
                            _tr.m[2058, "Mean TP and FP at Threshold"], _tr.m[2059, "Show Individual Curves"]],
                     callback=self._replot)

        gui.checkBox(abox, self, "display_convex_curve",
                     _tr.m[2060, "Show convex ROC curves"], callback=self._replot)
        gui.checkBox(abox, self, "display_convex_hull",
                     _tr.m[2061, "Show ROC convex hull"], callback=self._replot)

        box = gui.vBox(self.controlArea, _tr.m[2062, "Analysis"])

        gui.checkBox(box, self, "display_def_threshold",
                     _tr.m[2063, "Default threshold (0.5) point"],
                     callback=self._on_display_def_threshold_changed)

        gui.checkBox(box, self, "display_perf_line", _tr.m[2064, "Show performance line"],
        ...
        sp = gui.spin(box, self, "fp_cost", 1, 1000, 10,
                      ...
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(_tr.m[2065, "FP Cost:"]), 0, 0)
        grid.addWidget(sp, 0, 1)

        sp = gui.spin(box, self, "fn_cost", 1, 1000, 10,
                      alignment=Qt.AlignRight,
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(_tr.m[2066, "FN Cost:"]))
        ...
        grid.addWidget(QLabel(_tr.m[2067, "Prior probability:"]))
        ...
        axis.setLabel(_tr.m[2068, "FP Rate (1-Specificity)"])
        axis.setGrid(16)

        axis = self.plot.getAxis("left")
        axis.setTickFont(tickfont)
        axis.setLabel(_tr.m[2069, "TP Rate (Sensitivity)"])
        axis.setGrid(16)
    ...
    def _setup_plot(self):
        ...
        if not all(c.is_valid for c in hull_curves):
            if any(c.is_valid for c in hull_curves):
                warning = _tr.m[2070, "Some ROC curves are undefined"]
            else:
                warning = _tr.m[2071, "All ROC curves are undefined"]
    ...
    def send_report(self):
        if self.results is None:
            return
        items = OrderedDict()
        items[_tr.m[2073, "Target class"]] = self.target_cb.currentText()
        if self.display_perf_line:
            items[_tr.m[2074, "Costs"]] = \
                _tr.m[2075, "FP = {}, FN = {}"].format(self.fp_cost, self.fn_cost)
            items[_tr.m[2076, "Target probability"]] = "{} %".format(self.target_prior)
