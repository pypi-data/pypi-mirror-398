...
time_units = [_tr.m[654, "year"], _tr.m[655, "month"], _tr.m[656, "day"], _tr.m[657, "week"], _tr.m[658, "hour"], _tr.m[659, "minute"], _tr.m[660, "second"]]
INVALID_WIDTH = _tr.m[661, "invalid width"]
TOO_MANY_INTERVALS = _tr.m[662, "too many intervals"]
...
def _mdl_discretization(
    ...
    if not data.domain.has_discrete_class:
        return _tr.m[663, "no discrete class"]
...
Options: Dict[Methods, MethodDesc] = {
    ...
    for method in (
        MethodDesc(Methods.Default,
                   _tr.m[665, "Use default setting"], _tr.m[666, "default"],
                   _tr.m[667, "Treat the variable as defined in 'default setting'"],
                   None,
                   ()),
        MethodDesc(Methods.Keep,
                   _tr.m[668, "Keep numeric"], _tr.m[669, "keep"],
                   _tr.m[670, "Keep the variable as is"],
                   lambda data, var: var,
                   ()),
        MethodDesc(Methods.MDL,
                   _tr.m[671, "Entropy vs. MDL"], _tr.m[672, "entropy"],
                   (_tr.m[673, "Split values until MDL exceeds the entropy (Fayyad-Irani)\n"] + _tr.m[674, "(requires discrete class variable)"]),
                   _mdl_discretization,
                   ()),
        MethodDesc(Methods.EqualFreq,
                   _tr.m[675, "Equal frequency, intervals: "], _tr.m[676, "equal freq, k={}"],
                   _tr.m[677, "Create bins with same number of instances"],
                   lambda data, var, k: disc.EqualFreq(k)(data, var),
                   ("freq_spin", )),
        MethodDesc(Methods.EqualWidth,
                   _tr.m[678, "Equal width, intervals: "], _tr.m[679, "equal width, k={}"],
                   _tr.m[680, "Create bins of the same width"],
                   lambda data, var, k: disc.EqualWidth(k)(data, var),
                   ("width_spin", )),
        MethodDesc(Methods.Remove,
                   _tr.m[681, "Remove"], _tr.m[682, "remove"],
                   _tr.m[683, "Remove variable"],
                   lambda *_: None,
                   ()),
        MethodDesc(Methods.Binning,
                   _tr.m[684, "Natural binning, desired bins: "], _tr.m[685, "binning, desired={}"],
                   (_tr.m[686, "Create bins with nice thresholds; "] + _tr.m[687, "try matching desired number of bins"]),
                   lambda data, var, nbins: disc.Binning(nbins)(data, var),
                   ("binning_spin", )),
        MethodDesc(Methods.FixedWidth,
                   _tr.m[688, "Fixed width: "], _tr.m[689, "fixed width {}"],
                   _tr.m[690, "Create bins with the given width (not for time variables)"],
                   _fixed_width_discretization,
                   ("width_line", )),
        MethodDesc(Methods.FixedWidthTime,
                   _tr.m[691, "Time interval: "], _tr.m[692, "time interval, {} {}"],
                   _tr.m[693, "Create bins with the give width (for time variables)"],
                   _fixed_time_width_discretization,
                   ("width_time_line", "width_time_unit")),
        MethodDesc(Methods.Custom,
                   _tr.m[694, "Custom: "], _tr.m[695, "custom: {}"],
                   _tr.m[696, "Use manually specified thresholds"],
...
def format_desc(hint: VarHint) -> str:
    ...
    if hint.method_id == Methods.FixedWidthTime:
        ...
        try:
            ...
            unit = _tr.e(_tr.c(697, f"{time_units[unit]}(s)"))
        else:
            unit = _tr.e(_tr.c(698, f"{pl(width, time_units[unit])}"))
...
class DefaultDiscModel(QAbstractListModel):
    ...
    def data(self, _, role=Qt.DisplayRole):
        if role == Qt.DisplayRole:
            return _tr.m[699, "Default setting: "] + format_desc(self.hint)
        elif role == Qt.DecorationRole:
            return DefaultDiscModel.icon
        elif role == Qt.ToolTipRole:
            return _tr.m[700, "Default setting for variables without specific setings"]
        return None
...
globals().update(dict(
    ...
    Remove=namedtuple(_tr.m[701, "Remove"], []),
...
class OWDiscretize(widget.OWWidget):
    ...
    name = _tr.m[702, "Discretize"]
    description = _tr.m[703, "Discretize numeric variables"]
    category = _tr.m[704, "Transform"]
    icon = "icons/Discretize.svg"
    keywords = _tr.m[705, "discretize, bin, categorical, nominal, ordinal"]
    priority = 2130

    class Inputs:
        data = Input(_tr.m[706, "Data"], Table, doc=_tr.m[707, "Input data table"])

    class Outputs:
        data = Output(_tr.m[708, "Data"], Table, doc=_tr.m[709, "Table with categorical features"])
    ...
    def _create_buttons(self, box):
        ...
        def manual_cut_editline(text="", enabled=True) -> QLineEdit:
            edit = QLineEdit(
                text=text,
                placeholderText=_tr.m[710, "e.g. 0.0, 0.5, 1.0"],
                toolTip='<p style="white-space:pre">' +
                        (_tr.m[711, 'Enter cut points as a comma-separate list of \n'] + _tr.m[712, 'strictly increasing numbers e.g. 0.0, 0.5, 1.0).</p>']),
                enabled=enabled,
            )
        ...
        u.addItems([_tr.e(_tr.c(713, f"{unit}(s)")) for unit in time_units])
        ...
        self.copy_to_custom = FixedSizeButton(
            text=_tr.m[714, "CC"], toolTip=_tr.m[715, "Copy the current cut points to manual mode"])
    ...
    def _discretize_var(self, var: ContinuousVariable, hint: VarHint) \
        ...
        if isinstance(var, TimeVariable):
            if hint.method_id in (Methods.FixedWidth, Methods.Custom):
                return _tr.m[716, ": <keep, time var>"], var
        else:
            if hint.method_id == Methods.FixedWidthTime:
                return _tr.m[717, ": <keep, not time>"], var
    ...
    def send_report(self):
        ...
        self.report_items(_tr.m[719, "Variables"], reported)
