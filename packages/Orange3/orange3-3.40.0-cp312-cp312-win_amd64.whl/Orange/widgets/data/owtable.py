...
class OWTable(OWWidget):
    name = _tr.m[1707, "Data Table"]
    description = _tr.m[1708, "View the dataset in a spreadsheet."]
    icon = "icons/Table.svg"
    priority = 50
    keywords = _tr.m[1709, "data table, view"]

    class Inputs:
        data = Input(_tr.m[1710, "Data"], Table, default=True)
        data_subset = Input(_tr.m[1711, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[1712, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    class Warning(OWWidget.Warning):
        missing_sort_columns = Msg(
            (_tr.m[1713, "Cannot restore sorting.\n"] + _tr.m[1714, "Missing columns in input table: {}"])
        )
        non_sortable_input = Msg(
            (_tr.m[1715, "Cannot restore sorting.\n"] + _tr.m[1716, "Input table cannot be sorted due to implementation constraints."])
        )
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1717, "Variables"])
        self.c_show_attribute_labels = gui.checkBox(
            box, self, "show_attribute_labels",
            _tr.m[1718, "Show variable labels (if present)"],
            callback=self._update_variable_labels)

        gui.checkBox(box, self, "show_distributions",
                     _tr.m[1719, 'Visualize numeric values'],
                     callback=self._on_distribution_color_changed)
        gui.checkBox(box, self, "color_by_class", _tr.m[1720, 'Color by instance classes'],
                     callback=self._on_distribution_color_changed)

        box = gui.vBox(self.controlArea, _tr.m[1721, "Selection"])
        self.clear_button = gui.button(
            box, self, _tr.m[1722, "Clear Selection"], callback=self.clear_selection,
            autoDefault=False, enabled=False)
        gui.checkBox(box, self, "select_rows", _tr.m[1723, "Select full rows"],
                     callback=self._on_select_rows_changed)

        gui.rubber(self.controlArea)

        self.restore_button = gui.button(
            self.buttonsArea, self, _tr.m[1724, "Restore Original Order"],
            callback=self.restore_order,
            tooltip=_tr.m[1725, "Show rows in the original order"],
            autoDefault=False, enabled=False,
