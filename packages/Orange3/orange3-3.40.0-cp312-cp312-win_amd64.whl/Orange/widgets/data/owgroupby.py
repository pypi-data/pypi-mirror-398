...
AGGREGATIONS = {
    _tr.m[956, "Mean"]: Aggregation(
        "mean",
        {ContinuousVariable, TimeVariable},
        False),
    _tr.m[957, "Median"]: Aggregation(
        "median",
        {ContinuousVariable, TimeVariable},
        True),
    "Q1": Aggregation(
    ...
    "Q3": Aggregation(
        ...
        {ContinuousVariable, TimeVariable},
        True),
    _tr.m[958, "Min. value"]: Aggregation(
        "min",
        {ContinuousVariable, TimeVariable},
        True),
    _tr.m[959, "Max. value"]: Aggregation(
        "max",
        {ContinuousVariable, TimeVariable},
        True),
    _tr.m[960, "Mode"]: Aggregation(
        lambda x: pd.Series.mode(x).get(0, nan),
        {ContinuousVariable, DiscreteVariable, TimeVariable},
        True
    ),
    _tr.m[961, "Standard deviation"]: Aggregation(
        std,
        {ContinuousVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[962, "Variance"]: Aggregation(
        var,
        {ContinuousVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[963, "Sum"]: Aggregation(
        "sum",
        {ContinuousVariable},
        True),
    _tr.m[964, "Concatenate"]: Aggregation(
        concatenate,
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        StringVariable
    ),
    _tr.m[965, "Span"]: Aggregation(
        span,
        {ContinuousVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[966, "First value"]: Aggregation(
        "first",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        True
    ),
    _tr.m[967, "Last value"]: Aggregation(
        "last",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        True
    ),
    _tr.m[968, "Random value"]: Aggregation(
        lambda x: x.sample(1, random_state=0),
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        True
    ),
    _tr.m[969, "Count defined"]: Aggregation(
        "count",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[970, "Count"]: Aggregation(
        "size",
        {ContinuousVariable, DiscreteVariable, StringVariable, TimeVariable},
        ContinuousVariable
    ),
    _tr.m[971, "Proportion defined"]: Aggregation(
...
def _run(
    ...
    state.set_status(_tr.m[972, "Aggregating"])
    # group table rows
...
TABLE_COLUMN_NAMES = [_tr.m[973, "Attributes"], _tr.m[974, "Aggregations"]]
    ...
    def data(self, index, role=Qt.DisplayRole) -> Any:
        ...
        if role in (Qt.DisplayRole, Qt.EditRole):
            if col == TabColumn.attribute:
                ...
                n_more = "" if len(aggs) <= 3 else _tr.e(_tr.c(975, f" and {len(aggs) - 3} more"))
...
class OWGroupBy(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[976, "Group by"]
    description = ""
    category = _tr.m[977, "Transform"]
    icon = "icons/GroupBy.svg"
    keywords = _tr.m[978, "aggregate, group by"]
    priority = 1210

    class Inputs:
        data = Input(_tr.m[979, "Data"], Table, doc=_tr.m[980, "Input data table"])

    class Outputs:
        data = Output(_tr.m[981, "Data"], Table, doc=_tr.m[982, "Aggregated data"])
    ...
    def __init_main_area(self) -> None:
        ...
        gui.widgetBox(self.mainArea, orientation=grid_layout, box=_tr.m[984, "Aggregations"])
