...
class OWSelectRows(widget.OWWidget):
    name = _tr.m[1533, "Select Rows"]
    description = _tr.m[1534, "Select rows from the data based on values of variables."]
    icon = "icons/SelectRows.svg"
    priority = 100
    category = _tr.m[1535, "Transform"]
    keywords = _tr.m[1536, "select rows, filter"]

    class Inputs:
        data = Input(_tr.m[1537, "Data"], Table)

    class Outputs:
        matching_data = Output(_tr.m[1538, "Matching Data"], Table, default=True)
        unmatched_data = Output(_tr.m[1539, "Unmatched Data"], Table)
    ...
    Operators = {
        ContinuousVariable: [
            (FilterContinuous.Equal, _tr.m[1540, "equals"], _tr.m[1541, "equal"]),
            (FilterContinuous.NotEqual, _tr.m[1542, "is not"], _tr.m[1543, "are not"]),
            (FilterContinuous.Less, _tr.m[1544, "is below"], _tr.m[1545, "are below"]),
            (FilterContinuous.LessEqual, _tr.m[1546, "is at most"], _tr.m[1547, "are at most"]),
            (FilterContinuous.Greater, _tr.m[1548, "is greater than"], _tr.m[1549, "are greater than"]),
            (FilterContinuous.GreaterEqual, _tr.m[1550, "is at least"], _tr.m[1551, "are at least"]),
            (FilterContinuous.Between, _tr.m[1552, "is between"], _tr.m[1553, "are between"]),
            (FilterContinuous.Outside, _tr.m[1554, "is outside"], _tr.m[1555, "are outside"]),
            (FilterContinuous.IsDefined, _tr.m[1556, "is defined"], _tr.m[1557, "are defined"]),
        ],
        DiscreteVariable: [
            (FilterDiscreteType.Equal, _tr.m[1558, "is"]),
            (FilterDiscreteType.NotEqual, _tr.m[1559, "is not"]),
            (FilterDiscreteType.In, _tr.m[1560, "is one of"]),
            (FilterDiscreteType.IsDefined, _tr.m[1561, "is defined"])
        ],
        StringVariable: [
            (FilterString.Equal, _tr.m[1562, "equals"], _tr.m[1563, "equal"]),
            (FilterString.NotEqual, _tr.m[1564, "is not"], _tr.m[1565, "are not"]),
            (FilterString.Less, _tr.m[1566, "is before"], _tr.m[1567, "are before"]),
            (FilterString.LessEqual, _tr.m[1568, "is equal or before"], _tr.m[1569, "are equal or before"]),
            (FilterString.Greater, _tr.m[1570, "is after"], _tr.m[1571, "are after"]),
            (FilterString.GreaterEqual, _tr.m[1572, "is equal or after"], _tr.m[1573, "are equal or after"]),
            (FilterString.Between, _tr.m[1574, "is between"], _tr.m[1575, "are between"]),
            (FilterString.Outside, _tr.m[1576, "is outside"], _tr.m[1577, "are outside"]),
            (FilterString.Contains, _tr.m[1578, "contains"], _tr.m[1579, "contain"]),
            (FilterString.NotContain, _tr.m[1580, "does not contain"], _tr.m[1581, "do not contain"]),
            (FilterString.StartsWith, _tr.m[1582, "begins with"], _tr.m[1583, "begin with"]),
            (FilterString.NotStartsWith, _tr.m[1584, "does not begin with"], _tr.m[1585, "do not begin with"]),
            (FilterString.EndsWith, _tr.m[1586, "ends with"], _tr.m[1587, "end with"]),
            (FilterString.NotEndsWith, _tr.m[1588, "does not end with"], _tr.m[1589, "do not end with"]),
            (FilterString.IsDefined, _tr.m[1590, "is defined"], _tr.m[1591, "are defined"]),
            (FilterString.NotIsDefined, _tr.m[1592, "is not defined"], _tr.m[1593, "are not defined"]),
        ]
    }
    ...
    for _all_name, _all_type, _all_ops in (
            (_tr.m[1594, "All variables"], 0,
             [(None, _tr.m[1595, "are defined"])]),
            (_tr.m[1596, "All numeric variables"], 2,
             [(v, t) for v, _, t in Operators[ContinuousVariable]]),
            (_tr.m[1597, "All string variables"], 3,
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[1598, 'Conditions'], stretch=100)
        ...
        self.add_button = gui.button(
            box2, self, _tr.m[1599, "Add Condition"], callback=self.add_row)
        self.add_all_button = gui.button(
            box2, self, _tr.m[1600, "Add All Variables"], callback=self.add_all)
        self.remove_all_button = gui.button(
            box2, self, _tr.m[1601, "Remove All"], callback=self.remove_all)
        gui.rubber(box2)

        box_setting = gui.vBox(self.buttonsArea)
        self.cb_pa = gui.checkBox(
            box_setting, self, "purge_attributes",
            _tr.m[1602, "Remove unused values and constant features"],
            callback=self.conditions_changed)
        self.cb_pc = gui.checkBox(
            box_setting, self, "purge_classes", _tr.m[1603, "Remove unused classes"],
            callback=self.conditions_changed)
    ...
    def add_all(self):
        if self.cond_list.rowCount():
            Mb = QMessageBox
            if Mb.question(
                    self, _tr.m[1604, "Remove existing filters"],
                    (_tr.m[1605, "This will replace the existing filters with "] + _tr.m[1606, "filters for all variables."]), Mb.Ok | Mb.Cancel) != Mb.Ok:
                return
            self.remove_all()
    ...
    def set_new_values(self, oper_combo, adding_all, selected_values=None):
        ...
        if _tr.m[1607, "defined"] in oper_combo.currentText():
            ...
            if vtype == 2:  # continuous:
                ...
                if oper > 5:
                    gui.widgetLabel(box, _tr.m[1609, " and "])
                    box.controls.append(add_numeric(lc[1]))
            elif vtype == 3:  # string:
                box.controls = [add_textual(lc[0])]
                if oper in [6, 7]:
                    gui.widgetLabel(box, _tr.m[1610, " and "])
                ...
                if oper > 5:
                    gui.widgetLabel(box, _tr.m[1611, " and "])
    ...
    def _values_to_floats(attr, values):
        ...
        try:
            ...
            if not all(ok):
                raise ValueError((_tr.m[1612, 'Some values could not be parsed as floats'] + _tr.e(_tr.c(1613, f' in the current locale: {values}'))))
        except TypeError:
    ...
    def send_report(self):
        if not self.data:
            self.report_paragraph(_tr.m[1614, "No data."])
            return

        pdesc = None
        describe_domain = False
        for d in (self.data_desc, self.match_desc, self.nonmatch_desc):
            if not d or not d[_tr.m[1615, "Data instances"]]:
                continue
            ndesc = d.copy()
            del ndesc[_tr.m[1616, "Data instances"]]
            ...
            if oper == len(names) - 1:
                ...
                if name == _tr.m[1617, "is one of"]:
                    ...
                    if len(valnames) == 1:
                        ...
                        valstr = _tr.e(_tr.c(1618, f"{', '.join(valnames[:-1])} or {valnames[-1]}"))
                    conditions.append(_tr.e(_tr.c(1619, f"{attr} is {valstr}")))
                elif values and values[0]:
                    value = values[0] - 1
                    conditions.append(f"{attr} {name} {attr.values[value]}")
            elif var_type == 3:  # string variable
                conditions.append(
                    _tr.e(_tr.c(1620, f"{attr} {name} {' and '.join(map(repr, values))}")))
            elif var_type == 4:  # time
                values = (value.toString(format=Qt.ISODate) for value in values)
                conditions.append(_tr.e(_tr.c(1621, f"{attr} {name} {' and '.join(values)}")))
            elif all(x for x in values):  # numeric variable
                conditions.append(_tr.e(_tr.c(1622, f"{attr} {name} {' and '.join(values)}")))
        items = OrderedDict()
        if describe_domain:
            items.update(self.data_desc)
        else:
            items[_tr.m[1623, "Instances"]] = self.data_desc[_tr.m[1624, "Data instances"]]
        items[_tr.m[1625, "Condition"]] = _tr.m[1626, " AND "].join(conditions) or _tr.m[1627, "no conditions"]
        self.report_items(_tr.m[1628, "Data"], items)
        if describe_domain:
            self.report_items(_tr.m[1629, "Matching data"], self.match_desc)
            self.report_items(_tr.m[1630, "Non-matching data"], self.nonmatch_desc)
        else:
            match_inst = \
                bool(self.match_desc) and \
                self.match_desc[_tr.m[1631, "Data instances"]]
            nonmatch_inst = \
                bool(self.nonmatch_desc) and \
                self.nonmatch_desc[_tr.m[1632, "Data instances"]]
            self.report_items(
                _tr.m[1633, "Output"],
                ((_tr.m[1634, "Matching data"],
                  _tr.e(_tr.c(1635, f"{match_inst} {pl(match_inst, 'instance')}")) if match_inst else _tr.m[1636, "None"]),
                 (_tr.m[1637, "Non-matching data"],
                  nonmatch_inst > 0 and _tr.e(_tr.c(1638, f"{nonmatch_inst} {pl(nonmatch_inst, 'instance')}")))))
