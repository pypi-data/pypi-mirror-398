...
Metrics = [MetricDefinition(*args) for args in (
    (_tr.m[1791, "Calibration curve"], None, (), ""),
    (_tr.m[1792, "Classification accuracy"], (Curves.ca, ), (), ""),
    ("F1", (Curves.f1, ), (), ""),
    (_tr.m[1793, "Sensitivity and specificity"],
     (Curves.sensitivity, Curves.specificity),
     (_tr.m[1794, "sens"], _tr.m[1795, "spec"]),
     (_tr.m[1796, "<p><b>Sensitivity</b> (falling) is the proportion of correctly "] + (_tr.m[1797, "detected positive instances (TP&nbsp;/&nbsp;P).</p>"] + (_tr.m[1798, "<p><b>Specificity</b> (rising) is the proportion of detected "] + _tr.m[1799, "negative instances (TN&nbsp;/&nbsp;N).</p>"])))),
    (_tr.m[1800, "Precision and recall"],
     (Curves.precision, Curves.recall),
     (_tr.m[1801, "prec"], _tr.m[1802, "recall"]),
     (_tr.m[1803, "<p><b>Precision</b> (rising) is the fraction of retrieved instances "] + (_tr.m[1804, "that are relevant, TP&nbsp;/&nbsp;(TP&nbsp;+&nbsp;FP).</p>"] + (_tr.m[1805, "<p><b>Recall</b> (falling) is the proportion of discovered relevant "] + _tr.m[1806, "instances, TP&nbsp;/&nbsp;P.</p>"])))),
    (_tr.m[1807, "Pos and neg predictive value"],
     (Curves.ppv, Curves.npv),
     ("PPV", "TPV"),
     (_tr.m[1808, "<p><b>Positive predictive value</b> (rising) is the proportion of "] + (_tr.m[1809, "correct positives, TP&nbsp;/&nbsp;(TP&nbsp;+&nbsp;FP).</p>"] + (_tr.m[1810, "<p><b>Negative predictive value</b> is the proportion of correct "] + _tr.m[1811, "negatives, TN&nbsp;/&nbsp;(TN&nbsp;+&nbsp;FN).</p>"])))),
    (_tr.m[1812, "True and false positive rate"],
     (Curves.tpr, Curves.fpr),
     ("TPR", "FPR"),
     (_tr.m[1813, "<p><b>True and false positive rate</b> are proportions of detected "] + _tr.m[1814, "and omitted positive instances</p>"])),
)]
...
class OWCalibrationPlot(widget.OWWidget):
    name = _tr.m[1815, "Calibration Plot"]
    description = _tr.m[1816, "Calibration plot based on evaluation of classifiers."]
    icon = "icons/CalibrationPlot.svg"
    priority = 1030
    keywords = _tr.m[1817, "calibration plot"]

    class Inputs:
        evaluation_results = Input(_tr.m[1818, "Evaluation Results"], Results)

    class Outputs:
        calibrated_model = Output(_tr.m[1819, "Calibrated Model"], Model)

    class Error(widget.OWWidget.Error):
        non_discrete_target = Msg((_tr.m[1820, "Calibration plot requires a categorical "] + _tr.m[1821, "target variable."]))
        empty_input = widget.Msg(_tr.m[1822, "Empty result on input. Nothing to display."])
        nan_classes = \
            widget.Msg(_tr.m[1823, "Remove test data instances with unknown classes."])
        all_target_class = widget.Msg(
            _tr.m[1824, "All data instances belong to target class."])
        no_target_class = widget.Msg(
            _tr.m[1825, "No data instances belong to target class."])

    class Warning(widget.OWWidget.Warning):
        omitted_folds = widget.Msg(
            _tr.m[1826, "Test folds where all data belongs to (non)-target are not shown."])
        omitted_nan_prob_points = widget.Msg(
            (_tr.m[1827, "Instance for which the model couldn't compute probabilities are"] + _tr.m[1828, "skipped."]))
        no_valid_data = widget.Msg(_tr.m[1829, "No valid data for model(s) {}"])

    class Information(widget.OWWidget.Information):
        no_output = Msg(_tr.m[1830, "Can't output a model: {}"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, box=_tr.m[1831, "Settings"])
        self.target_cb = gui.comboBox(
            box, self, "target_index", label=_tr.m[1832, "Target:"],
            orientation=Qt.Horizontal, callback=self.target_index_changed,
            contentsLength=8, searchable=True)
        gui.checkBox(
            box, self, "display_rug", _tr.m[1833, "Show rug"],
            callback=self._on_display_rug_changed)
        gui.checkBox(
            box, self, "fold_curves", _tr.m[1834, "Curves for individual folds"],
            callback=self._replot)

        self.classifiers_list_box = gui.listBox(
            self.controlArea, self, "selected_classifiers", "classifier_names",
            box=_tr.m[1835, "Classifier"], selectionMode=QListWidget.ExtendedSelection,
            callback=self._on_selection_changed)
        self.classifiers_list_box.setMaximumHeight(100)

        box = gui.vBox(self.controlArea, _tr.m[1836, "Metrics"])
        combo = gui.comboBox(
        ...
        gui.radioButtons(
            ...
            btnLabels=(_tr.m[1837, "Sigmoid calibration"], _tr.m[1838, "Isotonic calibration"]),
            label=_tr.m[1839, "Output model calibration"], callback=self.commit.deferred)

        self.info_box = gui.widgetBox(self.controlArea, _tr.m[1840, "Info"])
    ...
    def _set_explanation(self):
        ...
        axis.setLabel(_tr.m[1841, "Predicted probability"] if self.score == 0
                      else _tr.m[1842, "Threshold probability to classify as positive"])
    ...
    def send_report(self):
        if self.results is None:
            return
        self.report_items((
            (_tr.m[1845, "Target class"], self.target_cb.currentText()),
            (_tr.m[1846, "Output model calibration"],
             self.score == 0
             and (_tr.m[1847, "Sigmoid calibration"],
                  _tr.m[1848, "Isotonic calibration"])[self.output_calibration])
        ))
