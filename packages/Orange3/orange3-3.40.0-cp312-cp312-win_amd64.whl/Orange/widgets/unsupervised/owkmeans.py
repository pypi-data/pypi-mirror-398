...
class ClusterTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            return "{:.3f}".format(score) if valid else _tr.m[2881, "NA"]
...
class OWKMeans(widget.OWWidget):
    name = _tr.m[2882, "k-Means"]
    description = (_tr.m[2883, "k-Means clustering algorithm with silhouette-based "] + _tr.m[2884, "quality estimation."])
    icon = "icons/KMeans.svg"
    priority = 2100
    keywords = _tr.m[2885, "k-means, kmeans, clustering"]

    class Inputs:
        data = Input(_tr.m[2886, "Data"], Table)

    class Outputs:
        annotated_data = Output(
            ANNOTATED_DATA_SIGNAL_NAME, Table, default=True,
            replaces=[_tr.m[2887, "Annotated Data"]]
        )
        centroids = Output(_tr.m[2888, "Centroids"], Table, dynamic=False)

    class Error(widget.OWWidget.Error):
        failed = widget.Msg(_tr.m[2889, "Clustering failed\nError: {}"])
        not_enough_data = widget.Msg(
            _tr.m[2890, "Too few ({}) unique data instances for {} clusters"]
        )
        no_attributes = widget.Msg(_tr.m[2891, "Data is missing features."])

    class Warning(widget.OWWidget.Warning):
        no_silhouettes = widget.Msg(
            _tr.m[2892, "Silhouette scores are not computed for >{} samples"].format(
                SILHOUETTE_MAX_SAMPLES)
        )
        not_enough_data = widget.Msg(
            _tr.m[2893, "Too few ({}) unique data instances for {} clusters"]
        )
        no_sparse_normalization = widget.Msg(_tr.m[2894, "Sparse data cannot be normalized"])

    INIT_METHODS = ((_tr.m[2895, "Initialize with KMeans++"], "k-means++"),
                    (_tr.m[2896, "Random initialization"], "random"))
    ...
    def __init__(self):
        ...
        bg = gui.radioButtonsInBox(
            ...
            box=_tr.m[2897, "Number of Clusters"], callback=self.update_method,
        )

        layout.addWidget(
            gui.appendRadioButton(bg, _tr.m[2898, "Fixed:"], addToLayout=False), 1, 1)
        ...
        layout.addWidget(
            gui.appendRadioButton(bg, _tr.m[2899, "From"], addToLayout=False), 2, 1)
        ...
        gui.widgetLabel(ftobox, _tr.m[2900, "to"])
        gui.spin(
        ...
        box = gui.vBox(self.controlArea, _tr.m[2901, "Preprocessing"])
        gui.checkBox(box, self, "normalize", _tr.m[2902, "Normalize columns"],
                     callback=self.invalidate)

        box = gui.vBox(self.controlArea, _tr.m[2903, "Initialization"])
        gui.comboBox(
        ...
        layout.addWidget(gui.widgetLabel(None, _tr.m[2904, "Re-runs: "]), 0, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        layout.addWidget(
            gui.widgetLabel(None, _tr.m[2905, "Maximum iterations: "]), 1, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        box = gui.vBox(self.mainArea, box=_tr.m[2906, "Silhouette Scores"])
        if self.optimize_k:
    ...
    def send_data(self):
        ...
        cluster_var = DiscreteVariable(
            get_unique_names(domain, _tr.m[2907, "Cluster"]),
            values=[_tr.m[2908, "C%d"] % (x + 1) for x in range(km.k)],
            compute_value=km
        )
        silhouette_var = ContinuousVariable(
            get_unique_names(domain, _tr.m[2909, "Silhouette"]))
        ...
        if self.data.name == Table.name:
            centroids.name = _tr.m[2910, "centroids"]
        else:
            centroids.name = _tr.e(_tr.c(2911, f"{self.data.name} centroids"))
    ...
    def send_report(self):
        ...
        self.report_items((
            (_tr.m[2912, "Number of clusters"], k_clusters),
            (_tr.m[2913, "Optimization"], _tr.m[2914, "{}, {} re-runs limited to {} steps"].format(
                init_method, self.n_init, self.max_iterations))))
        if self.data is not None:
            self.report_data(_tr.m[2915, "Data"], self.data)
            if self.optimize_k:
                self.report_table(
                    _tr.m[2916, "Silhouette scores for different numbers of clusters"],
                    self.table_view)
