...
class OWBaseSql(OWWidget, openclass=True):
    ...
    """
    class Outputs:
        data = Output(_tr.m[3253, "Data"], Table)
    ...
    def _setup_gui(self):
        ...
        vbox = gui.vBox(self.controlArea, _tr.m[3254, "Server"])
        self.serverbox = gui.vBox(vbox)
        self.servertext = QLineEdit(self.serverbox)
        self.servertext.setPlaceholderText(_tr.m[3255, "Server"])
        self.servertext.setToolTip(_tr.m[3256, "Server"])
        ...
        self.databasetext.setPlaceholderText(_tr.m[3257, "Database[/Schema]"])
        self.databasetext.setToolTip(_tr.m[3258, "Database or optionally Database/Schema"])
        if self.database:
        ...
        self.usernametext.setPlaceholderText(_tr.m[3259, "Username"])
        self.usernametext.setToolTip(_tr.m[3260, "Username"])

        self.serverbox.layout().addWidget(self.usernametext)
        self.passwordtext = QLineEdit(self.serverbox)
        self.passwordtext.setPlaceholderText(_tr.m[3261, "Password"])
        self.passwordtext.setToolTip(_tr.m[3262, "Password"])
        ...
        self.connectbutton = gui.button(self.serverbox, self, _tr.m[3263, "Connect"],
    ...
    def on_connection_success(self):
        ...
        self.database_desc = OrderedDict((
            (_tr.m[3264, "Host"], self.host), (_tr.m[3265, "Port"], self.port),
            (_tr.m[3266, "Database"], self.database), (_tr.m[3267, "User name"], self.username)
        ))
    ...
    def send_report(self):
        if not self.database_desc:
            self.report_paragraph(_tr.m[3268, "No database connection."])
            return
        self.report_items(_tr.m[3269, "Database"], self.database_desc)
        if self.data_desc_table:
            self.report_items(_tr.m[3270, "Data"],
