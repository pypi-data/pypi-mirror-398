...
def instance_tooltip(domain, row, skip_attrs=()):
    def show_part(_point_data, singular, plural, max_shown, _vars):
        ...
        n_vars = len(_vars)
        if n_vars > max_shown:
            cols[-1] = _tr.e(_tr.c(3181, f'... and {n_vars - max_shown + 1} others'))
        return \
            "<b>{}</b>:<br/>".format(singular if n_vars < 2 else plural) \
            + "<br/>".join(cols)

    parts = ((_tr.m[3182, "Class"], _tr.m[3183, "Classes"], 4, domain.class_vars),
             (_tr.m[3184, "Meta"], _tr.m[3185, "Metas"], 4, domain.metas),
             (_tr.m[3186, "Feature"], _tr.m[3187, "Features"], 10, domain.attributes))
