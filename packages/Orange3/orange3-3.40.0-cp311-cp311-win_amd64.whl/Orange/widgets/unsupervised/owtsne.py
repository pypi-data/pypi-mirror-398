...
INITIALIZATIONS = [("PCA", "pca"), (_tr.m[3134, "Spectral"], "spectral")]
DISTANCE_METRICS = [(_tr.m[3135, "Euclidean"], "l2"), (_tr.m[3136, "Manhattan"], "l1"), (_tr.m[3137, "Cosine"], "cosine")]
...
class TSNERunner:
    ...
    def compute_tsne_preprocessing(task: Task, state: TaskState, **_) -> None:
        state.set_status(_tr.m[3138, "Preprocessing data..."])
    ...
    def compute_normalization(task: Task, state: TaskState, **_) -> None:
        state.set_status(_tr.m[3139, "Normalizing data..."])
    ...
    def compute_pca(task: Task, state: TaskState, **_) -> None:
        ...
        state.set_status(_tr.m[3140, "Computing PCA..."])
    ...
    def compute_initialization(task: Task, state: TaskState, **_) -> None:
        ...
        state.set_status(_tr.m[3141, "Preparing initialization..."])
    ...
    def compute_affinities(task: Task, state: TaskState, **_) -> None:
        state.set_status(_tr.m[3142, "Finding nearest neighbors..."])
    ...
    def compute_tsne(task: Task, state: TaskState, progress_callback=None) -> None:
        ...
        state.set_status(_tr.m[3143, "Running optimization..."])
...
class OWtSNE(OWDataProjectionWidget, ConcurrentWidgetMixin):
    name = "t-SNE"
    description = _tr.m[3144, "Two-dimensional data projection with t-SNE."]
    icon = "icons/TSNE.svg"
    priority = 920
    keywords = _tr.m[3145, "t-sne, tsne"]
    ...
    class Inputs(OWDataProjectionWidget.Inputs):
        distances = Input(_tr.m[3146, "Distances"], DistMatrix)

    class Information(OWDataProjectionWidget.Information):
        modified = Msg((_tr.m[3147, "The parameter settings have been changed. Press "] + _tr.m[3148, "\"Start\" to rerun with the new settings."]))

    class Warning(OWDataProjectionWidget.Warning):
        consider_using_pca_preprocessing = Msg(
            (_tr.m[3149, "The input data contains a large number of features, which may slow"] + _tr.m[3150, " down t-SNE computation. Consider enabling PCA preprocessing."])
        )

    class Error(OWDataProjectionWidget.Error):
        not_enough_rows = Msg(_tr.m[3151, "Input data needs at least 2 rows"])
        not_enough_cols = Msg(_tr.m[3152, "Input data needs at least 2 attributes"])
        constant_data = Msg(_tr.m[3153, "Input data is constant"])
        no_valid_data = Msg(_tr.m[3154, "No projection due to no valid data"])

        distance_matrix_not_symmetric = Msg(_tr.m[3155, "Distance matrix is not symmetric"])
        distance_matrix_too_small = Msg(_tr.m[3156, "Input matrix must be at least 2x2"])

        dimension_mismatch = Msg(_tr.m[3157, "Data and distance dimensions do not match"])
    ...
    def _add_controls_start_box(self):
        self.preprocessing_box = gui.vBox(self.controlArea, box=_tr.m[3158, "Preprocessing"])
        self.normalize_cbx = gui.checkBox(
            self.preprocessing_box, self, "normalize", _tr.m[3159, "Normalize data"],
            callback=self._normalize_data_changed, stateWhenDisabled=False,
        )
        self.pca_preprocessing_cbx = gui.checkBox(
            self.preprocessing_box, self, "use_pca_preprocessing", _tr.m[3160, "Apply PCA preprocessing"],
            callback=self._pca_preprocessing_changed, stateWhenDisabled=False,
        )
        self.pca_component_slider = gui.hSlider(
            self.preprocessing_box, self, "pca_components", label=_tr.m[3161, "PCA Components:"],
            minValue=2, maxValue=_MAX_PCA_COMPONENTS, step=1,
            callback=self._pca_slider_changed,
        )

        self.parameter_box = gui.vBox(self.controlArea, box=_tr.m[3162, "Parameters"])
        form = QFormLayout(
        ...
        form.addRow(_tr.m[3163, "Initialization:"], self.initialization_combo)
        ...
        form.addRow(_tr.m[3164, "Distance metric:"], self.distance_metric_combo)
        ...
        form.addRow(_tr.m[3165, "Perplexity:"], self.perplexity_spin)

        form.addRow(gui.checkBox(
            self.controlArea, self, "multiscale", label=_tr.m[3166, "Preserve global structure"],
        ...
        form.addRow(_tr.m[3167, "Exaggeration:"], sbe)

        self.parameter_box.layout().addLayout(form)

        self.run_button = gui.button(
            self.parameter_box, self, _tr.m[3168, "Start"], callback=self._toggle_run
        )
    ...
    def _stop_running_task(self):
        self.cancel()
        self.run_button.setText(_tr.m[3169, "Start"])
    ...
    def _toggle_run(self):
        ...
        if self.task is not None:
            self.cancel()
            self.run_button.setText(_tr.m[3170, "Resume"])
    ...
    def enable_controls(self):
        ...
        if has_distance_matrix:
            ...
            self.normalize_cbx.setToolTip(
                _tr.m[3171, "Precomputed distances provided. Preprocessing is unnecessary!"]
            )

            self.pca_preprocessing_cbx.setDisabled(True)
            self.pca_preprocessing_cbx.setToolTip(
                _tr.m[3172, "Precomputed distances provided. Preprocessing is unnecessary!"]
            )

            # Only spectral init is valid with a precomputed distance matrix
            spectral_init_idx = self.initialization_combo.findText(_tr.m[3173, "Spectral"])
            self.initialization_combo.setCurrentIndex(spectral_init_idx)
            self.initialization_combo.setDisabled(True)
            self.initialization_combo.setToolTip(
                (_tr.m[3174, "Only spectral intialization is supported with precomputed "] + _tr.m[3175, "distance matrices."])
            )
            ...
            self.distance_metric_combo.setToolTip(
                _tr.m[3176, "Precomputed distances provided."]
            )
        ...
        if not has_distance_matrix and has_data and self.data.is_sparse():
            self.normalize_cbx.setDisabled(True)
            self.normalize_cbx.setToolTip(
                _tr.m[3177, "Data normalization is not supported on sparse matrices."]
            )
    ...
    def run(self):
        ...
        if self.data is not None:
            self.run_button.setText(_tr.m[3178, "Stop"])
    ...
    def on_done(self, task):
        # type: (Task) -> None
        self.run_button.setText(_tr.m[3179, "Start"])
    ...
    def cancel(self):
        self.run_button.setText(_tr.m[3180, "Start"])
        super().cancel()
