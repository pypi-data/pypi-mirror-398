...
class OWPLS(OWBaseLearner):
    name = 'PLS'
    description = _tr.m[2366, "Partial Least Squares Regression widget for multivariate data analysis"]
    icon = "icons/PLS.svg"
    priority = 85
    keywords = [_tr.m[2367, "partial least squares"]]

    LEARNER = PLSRegressionLearner

    class Outputs(OWBaseLearner.Outputs):
        coefsdata = Output(_tr.m[2368, "Coefficients and Loadings"], Table, explicit=True)
        data = Output(_tr.m[2369, "Data with Scores"], Table)
        components = Output(_tr.m[2370, "Components"], Table)

    class Warning(OWBaseLearner.Warning):
        sparse_data = Msg(
            _tr.m[2371, 'Sparse input data: default preprocessing is to scale it.'])
    ...
    def _create_output_coeffs_loadings(self) -> Table:
        ...
        names = [_tr.e(_tr.c(2376, f"coef ({v.name})")) for v in self.model.domain.class_vars]
        names += [_tr.e(_tr.c(2377, f"coef * X_sd ({v.name})")) for v in self.model.domain.class_vars]
        names += [f"w*c {i + 1}" for i in range(n_components)]
        domain = Domain(
            [ContinuousVariable(n) for n in names],
            metas=[StringVariable(_tr.m[2378, "Variable name"]),
                   DiscreteVariable(_tr.m[2379, "Variable role"], (_tr.m[2380, "Feature"], _tr.m[2381, "Target"]))]
        )
        ...
        M = np.array([[v.name for v in variables] + [_tr.m[2382, "intercept"]],
                      [0] * n_features + [1] * n_targets + [np.nan]],
                     dtype=object).T

        table = Table.from_numpy(domain, X=X, metas=M)
        table.name = _tr.m[2383, "Coefficients and Loadings"]
        return table
    ...
    def set_data(self, data):
        ...
        if data is not None and data.domain.class_var is None and not data.domain.class_vars:
            self.Error.data_error(
                (_tr.m[2384, "Data has no target variable.\n"] + _tr.m[2385, "Select one with the Select Columns widget."]))
            self.data = None
