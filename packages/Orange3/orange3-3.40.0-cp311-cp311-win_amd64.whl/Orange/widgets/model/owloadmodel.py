...
class OWLoadModel(OWUrlDropBase, RecentPathsWComboMixin):
    name = _tr.m[2307, "Load Model"]
    description = _tr.m[2308, "Load a model from an input file."]
    priority = 3050
    replaces = ["Orange.widgets.classify.owloadclassifier.OWLoadClassifier"]
    icon = "icons/LoadModel.svg"
    keywords = _tr.m[2309, "load model, file, open, model"]

    class Outputs:
        model = Output("Model", Model)

    class Error(widget.OWWidget.Error):
        load_error = Msg(_tr.m[2310, "An error occured while reading '{}'"])
    ...
    def __init__(self):
        ...
        vbox = gui.vBox(self.controlArea, _tr.m[2311, "File"])
        box = gui.hBox(vbox)
        ...
        button = gui.button(
            box, self, _tr.m[2312, "Reload"], callback=self.reload, default=True)
    ...
    def browse_file(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, _tr.m[2313, 'Open Model File'], start_file, self.FILTER)
        if not filename:
            return
