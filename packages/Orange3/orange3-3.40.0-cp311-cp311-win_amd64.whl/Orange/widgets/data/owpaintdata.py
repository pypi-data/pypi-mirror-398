...
class OWPaintData(OWWidget):
    TOOLS = [
        (_tr.m[1161, "Brush"], _tr.m[1162, "Create multiple instances"], AirBrushTool, _icon("brush.svg")),
        (_tr.m[1163, "Put"], _tr.m[1164, "Put individual instances"], PutInstanceTool, _icon("put.svg")),
        (_tr.m[1165, "Select"], _tr.m[1166, "Select and move instances"], SelectTool,
         _icon("select-transparent_42px.png")),
        (_tr.m[1167, "Jitter"], _tr.m[1168, "Jitter instances"], JitterTool, _icon("jitter.svg")),
        (_tr.m[1169, "Magnet"], _tr.m[1170, "Attract multiple instances"], MagnetTool, _icon("magnet.svg")),
        (_tr.m[1171, "Clear"], _tr.m[1172, "Clear the plot"], ClearTool, _icon("../../../icons/Dlg_clear.png"))
    ]

    name = _tr.m[1173, "Paint Data"]
    description = _tr.m[1174, "Create data by painting data points on a plane."]
    icon = "icons/PaintData.svg"
    priority = 60
    keywords = _tr.m[1175, "paint data, create, draw"]

    class Inputs:
        data = Input(_tr.m[1176, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[1177, "Data"], Table, dynamic=False)

    autocommit = Setting(True)
    table_name = Setting(_tr.m[1178, "Painted data"])
    attr1 = Setting("x")
    attr2 = Setting("y")
    ...
    class Warning(OWWidget.Warning):
        no_input_variables = Msg(_tr.m[1179, "Input data has no variables"])
        continuous_target = Msg(_tr.m[1180, "Numeric target value can not be used."])
        sparse_not_supported = Msg(_tr.m[1181, "Sparse data is ignored."])
        renamed_vars = Msg((_tr.m[1182, "Some variables have been renamed "] + _tr.m[1183, "to avoid duplicates.\n{}"]))

    class Information(OWWidget.Information):
        use_first_two = \
            Msg(_tr.m[1184, "Paint Data uses data from the first two attributes."])
    ...
    def _init_ui(self):
        namesBox = gui.vBox(self.controlArea, _tr.m[1185, "Names"])

        hbox = gui.hBox(namesBox, margin=0, spacing=0)
        gui.lineEdit(hbox, self, "attr1", _tr.m[1186, "Variable X: "],
                     controlWidth=80, orientation=Qt.Horizontal,
                     callback=self._attr_name_changed)
        gui.separator(hbox, 21)
        hbox = gui.hBox(namesBox, margin=0, spacing=0)
        attr2 = gui.lineEdit(hbox, self, "attr2", _tr.m[1187, "Variable Y: "],
                             controlWidth=80, orientation=Qt.Horizontal,
                             callback=self._attr_name_changed)
        gui.separator(hbox)
        ...
        gui.widgetLabel(namesBox, _tr.m[1188, "Labels"])
        ...
        tBox = gui.vBox(self.buttonsArea, _tr.m[1191, "Tools"])
        ...
        form.addRow(_tr.m[1192, "Radius:"], slider)
        ...
        form.addRow(_tr.m[1193, "Intensity:"], slider)
        ...
        form.addRow(_tr.m[1194, "Symbol:"], slider)

        self.btResetToInput = gui.button(
            tBox, self, _tr.m[1195, "Reset to Input Data"], self.reset_to_input)
    ...
    def remove_selected_class_label(self):
        ...
        self.undo_stack.beginMacro(_tr.m[1196, "Delete class label"])
...
#             self.undo_stack.push(command)
    ...
    def _add_command(self, cmd):
        ...
        if isinstance(cmd, Append):
            ...
            if indices is not None and indices.size:
                self.undo_stack.push(
                    UndoCommand(DeleteIndices(indices), self, text=_tr.m[1198, "Delete"])
                )
        elif isinstance(cmd, DeleteAll):
            indices = range(0, len(self.__buffer))
            self.undo_stack.push(
                UndoCommand(DeleteIndices(indices), self, text=_tr.m[1199, "Clear All"])
            )
            ...
            if indices is not None and indices.size:
                self.undo_stack.push(
                    UndoCommand(
                        ...
                        self, text=_tr.m[1200, "Move"])
                )
        elif isinstance(cmd, DeleteIndices):
            self.undo_stack.push(UndoCommand(cmd, self, text=_tr.m[1201, "Delete"]))
    ...
    def commit(self):
        ...
        if len(np.unique(Y)) >= 2:
            proposed.append(_tr.m[1202, "Class"])
    ...
    def send_report(self):
        ...
        if self.attr1 != "x" or self.attr2 != "y":
            settings += [(_tr.m[1203, "Axis x"], self.attr1), (_tr.m[1204, "Axis y"], self.attr2)]
        settings += [(_tr.m[1205, "Number of points"], len(self.data))]
        self.report_items(_tr.m[1206, "Painted data"], settings)
        self.report_plot()
