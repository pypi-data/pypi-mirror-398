...
class OWImpute(OWWidget):
    name = _tr.m[986, "Impute"]
    description = _tr.m[987, "Impute missing values in the data table."]
    icon = "icons/Impute.svg"
    priority = 2110
    keywords = _tr.m[988, "impute, substitute, missing"]
    category = _tr.m[989, "Transform"]

    class Inputs:
        data = Input(_tr.m[990, "Data"], Orange.data.Table)
        learner = Input(_tr.m[991, "Learner"], Learner)

    class Outputs:
        data = Output(_tr.m[992, "Data"], Orange.data.Table)

    class Error(OWWidget.Error):
        imputation_failed = Msg(_tr.m[993, "Imputation failed for '{}'"])
        model_based_imputer_sparse = \
            Msg(_tr.m[994, "Model based imputer does not work for sparse data"])

    class Warning(OWWidget.Warning):
        cant_handle_var = Msg(_tr.m[995, "Default method can not handle '{}'"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[996, "Default Method"])
        ...
        button = QRadioButton(_tr.m[997, "Fixed values; numeric variables:"])
        ...
        hlayout.addWidget(QLabel(_tr.m[998, ", time:"]))
        ...
        box = gui.hBox(self.controlArea, self.tr(_tr.m[999, "Individual Attribute Settings"]),
                       flat=False)
        ...
        self.reset_button = QPushButton(
            _tr.m[1000, "Restore All to Default"], enabled=False, default=False,
    ...
    def send_report(self):
        ...
        if specific:
            self.report_items((
                (_tr.m[1001, "Default method"], default.name),
                (_tr.m[1002, "Specific imputers"], ", ".join(specific))
            ))
        else:
            self.report_items(((_tr.m[1003, "Method"], default.name),))
