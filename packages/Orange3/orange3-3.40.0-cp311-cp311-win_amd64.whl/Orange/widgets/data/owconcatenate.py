...
class OWConcatenate(widget.OWWidget):
    name = _tr.m[286, "Concatenate"]
    description = _tr.m[287, "Concatenate (append) two or more datasets."]
    category = _tr.m[288, "Transform"]
    priority = 1111
    icon = "icons/Concatenate.svg"
    keywords = _tr.m[289, "concatenate, append, join, extend"]

    class Inputs:
        primary_data = Input(_tr.m[290, "Primary Data"], Orange.data.Table)
        additional_data = MultiInput(
            _tr.m[291, "Additional Data"], Orange.data.Table, default=True
        )

    class Outputs:
        data = Output(_tr.m[292, "Data"], Orange.data.Table)

    class Error(widget.OWWidget.Error):
        bow_concatenation = Msg(_tr.m[293, "Inputs must be of the same type."])
        incompatible_domains = \
            Msg(_tr.m[294, "Ignoring column names requires matching column types"])

    class Warning(widget.OWWidget.Warning):
        renamed_variables = Msg(
            _tr.m[295, "Variables with duplicated names have been renamed."])
        unmergeable_attributes = Msg(
            (_tr.m[296, "Some variables may not be concatenated correctly due "] + _tr.m[297, "to attributes difference ({})."]))
    ...
    source_attr_name = settings.Setting(_tr.m[298, "Source ID"])
    ...
    domain_opts = (_tr.m[299, "all variables that appear in input tables"],
                   _tr.m[300, "only variables that appear in all tables"])

    id_roles = (_tr.m[301, "Class attribute"], _tr.m[302, "Attribute"], _tr.m[303, "Meta attribute"])
    ...
    def __init__(self):
        ...
        self.mergebox = gui.vBox(self.controlArea, _tr.m[304, "Variable Sets Merging"])
        gui.widgetLabel(
            self.mergebox, self.tr(_tr.m[305, "When there is no primary table, "] +
                                   _tr.m[306, "the output should contain"]))
        ...
        label = gui.widgetLabel(
            self.mergebox,
            self.tr(_tr.m[307, "The resulting table will have a class only if there "] +
                    _tr.m[308, "is no conflict between input classes."]))
        label.setWordWrap(True)

        box = gui.vBox(self.controlArea, _tr.m[309, "Variable matching"])
        gui.checkBox(
            box, self, "ignore_names",
            (_tr.m[310, "Use column names from the primary table,\n"] + _tr.m[311, "and ignore names in other tables."]),
            callback=self.ignore_names_changed, stateWhenDisabled=False)

        gui.separator(self.controlArea)
        gui.checkBox(
            box, self, "ignore_compute_value",
            (_tr.m[312, "Treat variables with the same name as the same variable,\n"] + _tr.m[313, "even if they are computed using different formulae."]),
            callback=self.commit.deferred, stateWhenDisabled=False)
        ###
        box = gui.vBox(
            self.controlArea, self.tr(_tr.m[314, "Source Identification"]),
        )

        cb = gui.checkBox(
            box, self, "append_source_column",
            self.tr(_tr.m[315, "Append data source IDs"]),
        ...
        form.addRow(
            self.tr(_tr.m[316, "Feature name:"]),
            gui.lineEdit(ibox, self, "source_attr_name", valueType=str,
                         callback=self._source_changed))

        form.addRow(
            self.tr(_tr.m[317, "Place:"]),
    ...
    def send_report(self):
        items = OrderedDict()
        if self.primary_data is not None:
            items[_tr.m[318, "Domain"]] = _tr.m[319, "from primary data"]
        else:
            items[_tr.m[320, "Domain"]] = self.tr(self.domain_opts[self.merge_type]).lower()
        if self.append_source_column:
            items[_tr.m[321, "Source data ID"]] = _tr.m[322, "{} (as {})"].format(
