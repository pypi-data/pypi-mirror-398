...
INSTANCEID = _tr.m[1033, "Instance id"]
INDEX = _tr.m[1034, "Row index"]
...
class ConditionBox(QWidget):
    ...
    def add_row(self):
        ...
        row_items = self.RowItems(
            QLabel(_tr.m[1035, "and"] if row else self.pre_label),
...
class DomainModelWithTooltips(DomainModel):
    def data(self, index, role=Qt.DisplayRole):
        if role == Qt.ToolTipRole \
            ...
            if index.row() == 0:
                return _tr.m[1036, "Match rows sequentially"]
            if index.row() == 1:
                return (_tr.m[1037, "Re-match rows from tables obtained from the same "] + (_tr.m[1038, "source,\n"] + (_tr.m[1039, "e.g. data from the same file that was split within "] + _tr.m[1040, "the workflow."])))
...
class OWMergeData(widget.OWWidget):
    name = _tr.m[1041, "Merge Data"]
    description = _tr.m[1042, "Merge datasets based on the values of selected features."]
    category = _tr.m[1043, "Transform"]
    icon = "icons/MergeData.svg"
    priority = 1110
    keywords = _tr.m[1044, "merge data, join"]

    class Inputs:
        data = Input(_tr.m[1045, "Data"], Orange.data.Table, default=True, replaces=["Data A"])
        extra_data = Input(_tr.m[1046, "Extra Data"], Orange.data.Table, replaces=["Data B"])

    class Outputs:
        data = Output(_tr.m[1047, "Data"],
                      Orange.data.Table,
                      replaces=["Merged Data A+B", "Merged Data B+A", "Merged Data"],
                      dynamic=False)

    LeftJoin, InnerJoin, OuterJoin = range(3)
    OptionNames = (_tr.m[1048, "Append columns from Extra data"],
                   _tr.m[1049, "Find matching pairs of rows"],
                   _tr.m[1050, "Concatenate tables"])
    OptionDescriptions = (
        (_tr.m[1051, "The first table may contain, for instance, city names,\n"] + (_tr.m[1052, "and the second would be a list of cities and their coordinates.\n"] + _tr.m[1053, "Columns with coordinates would then be appended to the output."])),

        (_tr.m[1054, "Input tables contain different features describing the same data "] + (_tr.m[1055, "instances.\n"] + _tr.m[1056, "Output contains matched instances. Rows without matches are removed."])),

        (_tr.m[1057, "Input tables contain different features describing the same data "] + (_tr.m[1058, "instances.\n"] + (_tr.m[1059, "Output contains all instances. Data from merged instances is "] + _tr.m[1060, "merged into single rows."])))
    )

    UserAdviceMessages = [
        widget.Message(
            _tr.m[1061, "Confused about merging options?\nSee the tooltips!"],
            "merging_types")]
    ...
    class Warning(widget.OWWidget.Warning):
        renamed_vars = Msg((_tr.m[1062, "Some variables have been renamed "] + _tr.m[1063, "to avoid duplicates.\n{}"]))
        nonunique_left = Msg(
            (_tr.m[1064, "Some (unused) combinations of values in Data appear in "] + _tr.m[1065, "multiple rows."]))
        nonunique_right = Msg(
            (_tr.m[1066, "Some (unused) combinations of values in Extra Data appear in "] + _tr.m[1067, "multiple rows."]))

    class Error(widget.OWWidget.Error):
        matching_numeric_with_nonnum = Msg(
            _tr.m[1068, "Numeric and non-numeric columns ({} and {}) cannot be matched."])
        matching_index_with_sth = Msg(_tr.m[1069, "Row index cannot be matched with {}."])
        matching_id_with_sth = Msg(_tr.m[1070, "Instance cannot be matched with {}."])
        nonunique_left_matched = Msg(
            (_tr.m[1071, "Some combinations of values in Data appear in multiple rows."] + _tr.m[1072, "\nEvery matched combination may appear at most once."]))
        nonunique_left = Msg(
            (_tr.m[1073, "Some combinations of values in Data appear in multiple rows."] + _tr.m[1074, "\nEvery combination may appear at most once."]))
        nonunique_right_matched = Msg(
            (_tr.m[1075, "Some combinations of values in Extra Data appear in multiple rows."] + _tr.m[1076, "\nEvery matched combination may appear at most once."]))
        nonunique_right = Msg(
            (_tr.m[1077, "Some combinations of values in Extra Data appear in multiple rows."] + _tr.m[1078, "\nEvery combination may appear at most once."]))
    ...
    def send_report(self):
        ...
        self.report_items((
            (_tr.m[1082, "Merging"], self.OptionNames[self.merging]),
            (_tr.m[1083, "Match"],
             ", ".join(
                 _tr.e(_tr.c(1084, f"{self._get_col_name(left)} with {self._get_col_name(right)}"))
