...
class OWSplit(OWWidget):
    name = _tr.m[1639, "Split"]
    description = _tr.m[1640, "Split text or categorical variables into indicator variables"]
    category = _tr.m[1641, "Transform"]
    icon = "icons/Split.svg"
    keywords = [_tr.m[1642, "text to columns"], _tr.m[1643, "word encoding"], _tr.m[1644, "questionnaire"], _tr.m[1645, "survey"],
                _tr.m[1646, "term"], _tr.m[1647, "word presence"], _tr.m[1648, "word counts"], _tr.m[1649, "categorical encoding"],
                _tr.m[1650, "indicator variables"]]
    priority = 700

    class Inputs:
        data = Input(_tr.m[1651, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[1652, "Data"], Table)

    class Warning(OWWidget.Warning):
        no_disc = Msg(_tr.m[1653, "Data contains only numeric variables."])

    want_main_area = False
    resizing_enabled = False

    Categorical, Numerical, Counts = range(3)
    OutputLabels = (_tr.m[1654, "Categorical (No, Yes)"], _tr.m[1655, "Numerical (0, 1)"], _tr.m[1656, "Counts"])
    ...
    def __init__(self):
        ...
        variable_select_box = gui.vBox(self.controlArea, _tr.m[1657, "Variable"])
        ...
        gui.comboBox(variable_select_box, self, "attribute",
                     ...
                     model=DomainModel(valid_types=(StringVariable,
                                                    DiscreteVariable)))
        le = gui.lineEdit(
                variable_select_box, self, "delimiter", _tr.m[1658, "Delimiter: "],
    ...
    def _get_new_columns(self, values, computer):
        ...
        if self.output_type == self.Categorical:
            return tuple(
                DiscreteVariable(
                    name, (_tr.m[1660, "No"], _tr.m[1661, "Yes"]), compute_value=computer(value))
