...
class OWSieveDiagram(OWWidget, VizRankMixin(SieveRank)):
    name = _tr.m[3898, "Sieve Diagram"]
    description = (_tr.m[3899, "Visualize the observed and expected frequencies "] + _tr.m[3900, "for a combination of values."])
    icon = "icons/SieveDiagram.svg"
    priority = 200
    keywords = _tr.m[3901, "sieve diagram"]

    class Inputs:
        data = Input(_tr.m[3902, "Data"], Table, default=True)
        features = Input(_tr.m[3903, "Features"], AttributeList)

    class Outputs:
        selected_data = Output(_tr.m[3904, "Selected Data"], Table, default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table)

    graph_name = "canvas"  # QGraphicsScene

    class Warning(OWWidget.Warning):
        cochran = Msg(_tr.m[3905, "Data does not meet the Cochran's rule\n{}"])
    ...
    def __init__(self):
        ...
        button = self.vizrank_button(_tr.m[3906, "Score Combinations"])
    ...
    def init_vizrank(self):
        errmsg = ""
        if self.data is None:
            errmsg = _tr.m[3907, "No data"]
        elif len(self.data) <= 1 or len(self.data.domain.attributes) <= 1:
            errmsg = _tr.m[3908, "Not enough data"]
        elif self.data.is_sparse():
            errmsg = _tr.m[3909, "Data is sparse"]
    ...
    def resolve_shown_attributes(self):
        ...
        if not features:
            self.warning(
                _tr.m[3910, "Features from the input signal are not present in the data"])
            return
    ...
    def update_graph(self):
        ...
        def make_tooltip():
            ...
            def _oper(attr, txt):
                if self.data.domain[attr.name] == ddomain[attr.name]:
                    return " = "
                return " " if txt[0] in "<≥" else _tr.m[3911, " in "]
            ...
            ct = _tr.m[3912, """<b>combination of values: </b><br/>
        ...
        if not disc_x.values or not disc_y.values:
            text_ = _tr.m[3913, "Features {} and {} have no values"].format(disc_x, disc_y) \
                if not disc_x.values and \
                   not disc_y.values and \
                          disc_x != disc_y \
                else \
                    _tr.m[3914, "Feature {} has no values"].format(
    ...
    def _check_cochran(chi):
        ...
        cells = expected.size
        if cells == 0:
            return _tr.m[3915, "no cells in contingency table"]
        eps = 1e-12
        num_lt1 = (expected < 1.0 - eps).sum()
        num_lt5 = (expected < 5.0 - eps).sum()
        if num_lt1 > 0:
            return _tr.m[3916, "some expected frequencies are below 1"]
        if num_lt5 > 0.2 * cells:
            return _tr.m[3917, "more than 20% of expected frequencies are below 5"]
        return None

    def get_widget_name_extension(self):
        if self.data is not None:
            return _tr.m[3918, "{} vs {}"].format(self.attr_x.name, self.attr_y.name)
        return None
