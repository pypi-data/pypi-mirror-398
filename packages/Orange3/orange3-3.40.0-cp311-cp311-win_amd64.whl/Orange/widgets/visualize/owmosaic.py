...
class MosaicVizRank(VizRankDialogAttrs):
    ...
    def __init__(self, parent, data, attr_color, attr_range_index):
        ...
        box = gui.hBox(self)
        label = QLabel(_tr.m[3701, "Score Mosaics with "])

        combo = self.attrs_combo = QComboBox()
        combo.addItems(
            [_tr.m[3702, "a single variable"], _tr.m[3703, "two variables"], _tr.m[3704, "three variables"],
             _tr.m[3705, "four variables"], _tr.m[3706, "at most two variables"],
             _tr.m[3707, "at most three variables"], _tr.m[3708, "at most four variables"]])
    ...
    def on_attrs_changed(self):
        ...
        if new_attrs == self.attr_range():
            ...
            self.set_button_state(
                label=_tr.m[3709, "Restart with new settings"],
                enabled=True)
...
class OWMosaicDisplay(OWWidget, VizRankMixin(MosaicVizRank)):
    name = _tr.m[3710, "Mosaic Display"]
    description = _tr.m[3711, "Display data in a mosaic plot."]
    icon = "icons/MosaicDisplay.svg"
    priority = 220
    keywords = _tr.m[3712, "mosaic display"]

    class Inputs:
        data = Input(_tr.m[3713, "Data"], Table, default=True)
        data_subset = Input(_tr.m[3714, "Data Subset"], Table)

    class Outputs:
        selected_data = Output(_tr.m[3715, "Selected Data"], Table, default=True)
    ...
    class Warning(OWWidget.Warning):
        incompatible_subset = Msg(_tr.m[3716, "Data subset is incompatible with Data"])
        no_valid_data = Msg(_tr.m[3717, "No valid data"])
        no_cont_selection_sql = \
            Msg(_tr.m[3718, "Selection of numeric features on SQL is not supported"])
    ...
    def __init__(self):
        ...
        self.attr_combos = [
            ...
            for i in range(1, 5)]
        box.layout().addWidget(self.vizrank_button(_tr.m[3720, "Find Informative Mosaics"]))
        self.vizrankSelectionChanged.connect(self.set_attr)
        self.vizrankRunStateChanged.connect(self.store_vizrank_attr_range)

        box2 = gui.vBox(self.controlArea, box=_tr.m[3721, "Interior Coloring"])
        self.color_model = DomainModel(
            order=DomainModel.MIXED, valid_types=DomainModel.PRIMITIVE,
            placeholder=_tr.m[3722, "(Pearson residuals)"])
        ...
        self.bar_button = gui.checkBox(
            box2, self, 'use_boxes', label=_tr.m[3723, 'Compare with total'],
    ...
    def init_vizrank(self):
        if self.discrete_data is not None and len(self.discrete_data) > 1 \
            ...
            self.disable_vizrank(_tr.m[3724, "Not enough data"])
    ...
    def update_graph(self):
        ...
        def add_rect(x0, x1, y0, y1, condition,
            ...
            if self.variable_color is None:
                ...
                outer_rect.setToolTip(
                    condition + "<hr/>" +
                    (_tr.m[3725, "Expected instances: %.1f<br>"] + (_tr.m[3726, "Actual instances: %d<br>"] + _tr.m[3727, "Standardized (Pearson) residual: %.1f"])) %
                ...
                if n_actual > 0:
                    ...
                    text = "<br/>".join(
                        _tr.m[3728, "<b>%s</b>: %d / %.1f%% (Expected %.1f / %.1f%%)"] %
                    ...
                    text = ""
                outer_rect.setToolTip(
                    _tr.m[3729, "{}<hr>Instances: {}<br><br>{}"].format(
                        condition, n_actual, text[:-4]))

        def create_legend():
            if self.variable_color is None:
                names = ["<-8", "-8:-4", "-4:-2", "-2:2", "2:4", "4:8", ">8",
                         _tr.m[3730, "Residuals:"]]
        ...
        if attrs:
            CanvasText(self.canvas,
                       _tr.m[3731, "Feature {} has no values"].format(attrs[0]),
