...
async def set_reply(
    session: async_scoped_session, key: str, group_id: str, message_json: str, creator_id: str
) -> bool:
    """
    设置或更新一个快捷回复。
...
async def delete_all_replies_in_context(session: async_scoped_session, context_id: str) -> int:
    """根据上下文ID删除其创建的所有快捷回复。"""
    stmt = (
        delete(QuickReply)
...
async def delete_all_replies_by_user(session: async_scoped_session, user_id: str) -> int:
    """
...
async def get_all_keywords_by_user(session: async_scoped_session, user_id: str) -> list[str]:
