...
app = typer.Typer(
    ...
    *   `calculer-stats-par-groupe` : Analyse les notes groupées par une colonne spécifiée (ex: département, matière, UE).
    *   `calculer-stats-enseignants` : Fournit des statistiques détaillées par enseignant.
    *   `classer-etudiants` : Classe les étudiants par note au sein de groupes spécifiés (UE, matière, département).
    *   `lister-admis` : Liste les étudiants dont la moyenne générale est supérieure ou égale à 10.
    *   `classer-enseignants` : Classe les enseignants en fonction de leur performance (moyenne, taux de réussite).
...
def calculate_stats_by_group(
    ...
    grouper_par: Annotated[
        str,
        typer.Option("--grouper-par", "-g", help="[Obligatoire] Nom de la colonne dans le fichier CSV par laquelle les statistiques doivent être groupées (ex: 'departement_nom', 'matiere', 'ue_nom').")
    ],
    sortie: Annotated[
        str,
...
def plot_boxplot(
    fichier: Annotated[str, typer.Argument(help="Chemin complet vers le fichier de données CSV contenant les notes.")],
    colonne_x: Annotated[str, typer.Option("--colonne-x", help="[Obligatoire] Nom de la colonne dans le fichier CSV pour grouper les données sur l'axe des X (ex: 'departement_nom', 'matiere').")],
    sortie: Annotated[str, typer.Option("--sortie", "-s", help="[Obligatoire] Chemin pour sauvegarder le graphique. L'extension du fichier (ex: .png, .jpg, .pdf) déterminera le format de sortie.")]
):
    """
...
@app.command(name="lister-admis")
def list_admitted_cli(
    fichier: Annotated[
        str,
        typer.Argument(help="Chemin complet vers le fichier de données CSV contenant les notes.")
    ],
    sortie: Annotated[
        str,
        typer.Option("--sortie", "-s", help="[Optionnel] Chemin pour sauvegarder la liste des étudiants admis. Format .csv ou .xlsx.")
    ] = None
):
    """
    Liste les étudiants admis (moyenne générale >= 10).
    Affiche la liste des étudiants dont la moyenne générale est supérieure ou égale à 10.

    Exemple d'utilisation :

    `epl-analytics lister-admis data/notes_epl_simulees.csv`

    Pour sauvegarder les résultats :

    `epl-analytics lister-admis data/notes_epl_simulees.csv -s etudiants_admis.csv`
    """
    try:
        df = pd.read_csv(fichier, sep=';', decimal=',')
        df['note'] = pd.to_numeric(df['note'], errors='coerce')

        admitted_df = analysis.get_admitted_students(df)

        if admitted_df.empty:
            console.print("[yellow]Aucun étudiant admis trouvé dans le fichier.[/yellow]")
            raise typer.Exit()

        console.print("\n:mortar_board: [bold green]Liste des étudiants admis (moyenne générale >= 10):[/bold green]")
        
        table = Table(show_header=True, header_style="bold magenta")
        for col in admitted_df.columns:
            table.add_column(col)
        ...
        if sortie:
            save_df(admitted_df, sortie)

    except FileNotFoundError:
        console.print(f"[bold red]Erreur :[/bold red] Fichier non trouvé à '{fichier}'. Veuillez vérifier le chemin.")
        raise typer.Exit(code=1)
    except Exception as e:
        console.print(f"[bold red]Une erreur est survenue lors de la liste des étudiants admis :[/bold red] {e}")
        raise typer.Exit(1)

@app.command(name="classer-enseignants")
def rank_teachers_cli(
    fichier: Annotated[
        str,
        typer.Argument(help="Chemin complet vers le fichier de données CSV contenant les notes.")
    ],
    classer_par: Annotated[
        str,
        typer.Option("--classer-par", "-c", help="[Obligatoire] Métrique pour classer les enseignants (ex: 'Moyenne', 'Taux de Réussite (%)').")
    ],
    sortie: Annotated[
        str,
        typer.Option("--sortie", "-s", help="[Optionnel] Chemin pour sauvegarder les résultats du classement des enseignants. Format .csv ou .xlsx.")
    ] = None
):
    """
    Classe les enseignants en fonction d'une métrique spécifiée (moyenne des notes ou taux de réussite).

    Exemple d'utilisation :

    `epl-analytics classer-enseignants data/notes_epl_simulees.csv --classer-par Moyenne`

    Pour sauvegarder les résultats :

    `epl-analytics classer-enseignants data/notes_epl_simulees.csv -c "Taux de Réussite (%)" -s classement_enseignants.xlsx`
    """
    try:
        df = pd.read_csv(fichier, sep=';', decimal=',')
        df['note'] = pd.to_numeric(df['note'], errors='coerce')

        ranked_teachers_df = analysis.rank_teachers(df, classer_par)

        if ranked_teachers_df.empty:
            console.print("[yellow]Aucune donnée trouvée pour classer les enseignants.[/yellow]")
            raise typer.Exit()

        console.print(f"\n:school_satchel: [bold green]Classement des enseignants par '{classer_par}':[/bold green]")
        
        table = Table(show_header=True, header_style="bold magenta")
        for col in ranked_teachers_df.columns:
            table.add_column(col)
        
        for _, row in ranked_teachers_df.iterrows():
            table.add_row(*[str(item) for item in row.values])
        console.print(table)
        
        if sortie:
            save_df(ranked_teachers_df, sortie)

    except FileNotFoundError:
        console.print(f"[bold red]Erreur :[/bold red] Fichier non trouvé à '{fichier}'. Veuillez vérifier le chemin.")
        raise typer.Exit(code=1)
    except ValueError as ve:
        console.print(f"[bold red]Erreur :[/bold red] {ve}")
        raise typer.Exit(code=1)
    except Exception as e:
        console.print(f"[bold red]Une erreur est survenue lors du classement des enseignants :[/bold red] {e}")
        raise typer.Exit(1)
