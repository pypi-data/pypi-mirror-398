...
def test_search_pos_parameter():
    ...
    assert result.named['age'] in [20, 30]  # May find 20 or 30 depending on implementation
...
def test_search_endpos_parameter():
    ...
    assert result is not None
    assert result.named['age'] in [10, 20]  # May find either depending on implementation
...
def test_search_with_extra_types():
    ...
    result = search("Value: {:Number}", "Text, Value: 42, More", extra_types={"Number": parse_number})
    assert result is not None
