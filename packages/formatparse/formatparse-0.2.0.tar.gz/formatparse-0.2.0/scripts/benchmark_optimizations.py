...
from formatparse import parse as parse_formatparse, search as search_formatparse, findall as findall_formatparse
...
def print_comparison(name, original_stats, formatparse_stats):
    ...
    if original_stats and formatparse_stats:
        ...
        if speedup > 1:
            ...
            print(f"⚖️  formatparse is about the same speed")
        print(f"{'='*70}")
...
def test_pattern_caching():
    ...
    original_stats = benchmark_function(parse_original, pattern, string, iterations=iterations)
    ...
    formatparse_stats = benchmark_function(parse_formatparse, pattern, string, iterations=iterations)
...
def test_type_conversions():
    ...
    pattern = "ID:{id:d} Price:{price:f} Active:{active:b} Count:{count:d} Score:{score:f}"
    string = "ID:12345 Price:99.99 Active:True Count:42 Score:87.5"
    iterations = 50000
    
    print(f"\nPattern: {pattern}")
    print(f"String: {string}")
    print(f"Types: integers, floats, booleans")
    print(f"Iterations: {iterations:,}")
    print("-"*70)
...
def test_search_optimization():
    ...
    print(f"Match position: near middle")
    print(f"Iterations: {iterations:,}")
    print("-"*70)
    
    print("Benchmarking original parse library (search)...")
    original_stats = benchmark_function(search_original, pattern, long_string, iterations=iterations)
    print(f"  Mean:   {original_stats['mean']:.4f} ms")
    print(f"  Total:  {original_stats['total']:.2f} ms")
    
    print("Benchmarking formatparse (with pre-compiled search regex)...")
    formatparse_stats = benchmark_function(search_formatparse, pattern, long_string, iterations=iterations)
    print(f"  Mean:   {formatparse_stats['mean']:.4f} ms")
    print(f"  Total:  {formatparse_stats['total']:.2f} ms")
...
def test_findall_optimization():
    ...
    print(f"\nPattern: {pattern}")
    print(f"String: {string}")
    print(f"Matches: 10 matches per string")
    print(f"Iterations: {iterations:,}")
    print("-"*70)
...
def test_case_insensitive():
    ...
    original_stats = benchmark_function(parse_original, pattern, string, case_sensitive=False, iterations=iterations)
    print(f"  Mean:   {original_stats['mean']:.4f} ms")
    print(f"  Total:  {original_stats['total']:.2f} ms")
    
    print("Benchmarking formatparse (with pre-compiled case-insensitive regex)...")
    formatparse_stats = benchmark_function(parse_formatparse, pattern, string, case_sensitive=False, iterations=iterations)
    print(f"  Mean:   {formatparse_stats['mean']:.4f} ms")
    print(f"  Total:  {formatparse_stats['total']:.2f} ms")
...
def test_cache_warmup():
    ...
    print(f"\nPattern: {pattern}")
    print(f"String: {string}")
    print(f"Testing: First 10 calls (cold) vs next 990 calls (warm)")
    print("-"*70)
