...
async def cost_update(
    ...
    Breaking Change (v0.2.6): This function's signature changed to match the
    BPA API requirements. The old `amount` parameter is now `fix_value`, and
    `cost_type` is now required. The API uses `costType` as a discriminator
    between FIX and FORMULA cost types. currencyId is optional per API docs.

    Args:
        cost_id: ID of the cost to update (required).
        cost_type: Type of cost ("FIX" or "FORMULA") (required).
        name: New name for the cost (optional).
        fix_value: New fixed value for fixed costs (optional, use for FIX type).
        formula: New formula for formula costs (optional, use for FORMULA type).
