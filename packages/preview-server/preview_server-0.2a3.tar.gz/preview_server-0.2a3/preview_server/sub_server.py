...
class SubServerManager:
    ...
    def __init__(
        ...
        self._ref_locks: Dict[str, asyncio.Lock] = {}  # Per-ref locks for startup
    ...
    async def get_port_for_ref(self, ref: str) -> int:
        ...
        Uses per-ref locks to prevent blocking requests for already-running
        servers while other servers are starting up (issue #10).
        ...
        """
        # Fast path: check if already running (brief global lock)
        async with self._lock:
            ...
            # Get or create per-ref lock (still under global lock)
            if ref not in self._ref_locks:
                self._ref_locks[ref] = asyncio.Lock()
            ref_lock = self._ref_locks[ref]

        # Slow path: acquire per-ref lock and start server
        # This releases the global lock so other refs can proceed
        async with ref_lock:
            # Double-check under per-ref lock (another request may have started it)
            async with self._lock:
                if ref in self.servers and self.is_ref_running(ref):
                    return self.servers[ref].port

            # Start new server (without holding global lock)
    ...
    async def _start_server_for_ref(self, ref: str) -> int:
        ...
        # Record server info (under global lock to protect dict)
        import time
