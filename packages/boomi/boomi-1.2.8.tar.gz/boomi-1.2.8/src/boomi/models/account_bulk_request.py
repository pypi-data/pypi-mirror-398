...
class AccountBulkRequestType(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, AccountBulkRequestType._member_map_.values())
        )
...
class AccountBulkRequest(BaseModel):
    ...
    :type type_: AccountBulkRequestType, optional
    """

    def __init__(
        self,
        request: List[BulkId] = SENTINEL,
        type_: AccountBulkRequestType = SENTINEL,
        **kwargs,
    ):
        """AccountBulkRequest
        ...
        if type_ is not SENTINEL:
            self.type_ = self._enum_matching(
                type_, AccountBulkRequestType.list(), "type_"
            )
        self._kwargs = kwargs
