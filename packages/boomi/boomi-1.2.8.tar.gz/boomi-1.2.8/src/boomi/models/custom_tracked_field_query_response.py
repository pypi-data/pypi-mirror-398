...
from .custom_tracked_field import CustomTrackedField


@JsonMap({"number_of_results": "numberOfResults", "query_token": "queryToken"})
class CustomTrackedFieldQueryResponse(BaseModel):
    ...
    :type result: List[CustomTrackedField], optional
    """
    ...
    def __init__(
        ...
        result: List[CustomTrackedField] = SENTINEL,
        **kwargs,
    ):
        """CustomTrackedFieldQueryResponse
        ...
        if result is not SENTINEL:
            self.result = self._define_list(result, CustomTrackedField)
        self._kwargs = kwargs
