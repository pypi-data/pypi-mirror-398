...
from .api_usage_count import ApiUsageCount


@JsonMap({"number_of_results": "numberOfResults", "query_token": "queryToken"})
class ApiUsageCountQueryResponse(BaseModel):
    ...
    :type result: List[ApiUsageCount], optional
    """
    ...
    def __init__(
        ...
        result: List[ApiUsageCount] = SENTINEL,
        **kwargs,
    ):
        """ApiUsageCountQueryResponse
        ...
        if result is not SENTINEL:
            self.result = self._define_list(result, ApiUsageCount)
        self._kwargs = kwargs
