...
class ProcessingGroupPartnerStandardRouteStandard(Enum):
    ...
    def list():
        ...
        return list(
            map(
                lambda x: x.value,
                ProcessingGroupPartnerStandardRouteStandard._member_map_.values(),
            )
        )


@JsonMap({"document_type_route": "DocumentTypeRoute", "process_id": "processId"})
class ProcessingGroupPartnerStandardRoute(BaseModel):
    """ProcessingGroupPartnerStandardRoute

    :param document_type_route: document_type_route, defaults to None
    :type document_type_route: List[ProcessingGroupPartnerDocumentRoute], optional
    :param process_id: process_id, defaults to None
    :type process_id: str, optional
    :param standard: standard, defaults to None
    :type standard: ProcessingGroupPartnerStandardRouteStandard, optional
    """

    def __init__(
        self,
        document_type_route: List[ProcessingGroupPartnerDocumentRoute] = SENTINEL,
        process_id: str = SENTINEL,
        standard: ProcessingGroupPartnerStandardRouteStandard = SENTINEL,
        **kwargs,
    ):
        """ProcessingGroupPartnerStandardRoute
        ...
        if document_type_route is not SENTINEL:
            self.document_type_route = self._define_list(
                document_type_route, ProcessingGroupPartnerDocumentRoute
            )
        if process_id is not SENTINEL:
            self.process_id = process_id
        if standard is not SENTINEL:
            self.standard = self._enum_matching(
                standard, ProcessingGroupPartnerStandardRouteStandard.list(), "standard"
            )
        self._kwargs = kwargs
