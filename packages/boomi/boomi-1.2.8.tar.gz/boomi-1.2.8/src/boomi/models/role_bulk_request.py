...
class RoleBulkRequestType(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, RoleBulkRequestType._member_map_.values()))
...
class RoleBulkRequest(BaseModel):
    ...
    :type type_: RoleBulkRequestType, optional
    """

    def __init__(
        self,
        request: List[BulkId] = SENTINEL,
        type_: RoleBulkRequestType = SENTINEL,
        **kwargs,
    ):
        """RoleBulkRequest
        ...
        if type_ is not SENTINEL:
            self.type_ = self._enum_matching(type_, RoleBulkRequestType.list(), "type_")
        self._kwargs = kwargs
