...
class ComponentEnvironmentAttachmentService(BaseService):
    ...
    def create_component_environment_attachment(
    ...
    ) -> Union[ComponentEnvironmentAttachment, str]:
        ...
        :rtype: Union[ComponentEnvironmentAttachment, str]
        """
        ...
        if content == "application/json":
            return ComponentEnvironmentAttachment._unmap(response)
        if content == "application/xml":
            return ComponentEnvironmentAttachment._unmap(parse_xml_to_dict(response))
        raise ApiError("Error on deserializing the response.", status, response)

    @cast_models
    def query_component_environment_attachment(
        self, request_body: ComponentEnvironmentAttachmentQueryConfig = None
    ) -> Union[ComponentEnvironmentAttachmentQueryResponse, str]:
        ...
        ...
        :return: The parsed response data.
        :rtype: Union[ComponentEnvironmentAttachmentQueryResponse, str]
        """
        ...
        response, status, content = self.send_request(serialized_request)
        if content == "application/json":
            return ComponentEnvironmentAttachmentQueryResponse._unmap(response)
        if content == "application/xml":
            return ComponentEnvironmentAttachmentQueryResponse._unmap(parse_xml_to_dict(response))
        raise ApiError("Error on deserializing the response.", status, response)
