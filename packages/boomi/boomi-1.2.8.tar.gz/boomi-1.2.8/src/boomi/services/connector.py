...
class ConnectorService(BaseService):
    ...
    @cast_models
    def get_connector(self, connector_type: str) -> Union[Connector, str]:
        ...
        if content == "application/json":
            return Connector._unmap(response)
        if content == "application/xml":
            return Connector._unmap(parse_xml_to_dict(response))
        raise ApiError("Error on deserializing the response.", status, response)

    @cast_models
    def bulk_connector(
        self, request_body: ConnectorBulkRequest = None
    ) -> Union[ConnectorBulkResponse, str]:
        ...
        ...
        :return: The parsed response data.
        :rtype: Union[ConnectorBulkResponse, str]
        """
        ...
        response, status, content = self.send_request(serialized_request)
        if content == "application/json":
            return ConnectorBulkResponse._unmap(response)
        if content == "application/xml":
            return ConnectorBulkResponse._unmap(parse_xml_to_dict(response))
        raise ApiError("Error on deserializing the response.", status, response)

    @cast_models
    def query_connector(
        self, request_body: ConnectorQueryConfig = None
    ) -> Union[ConnectorQueryResponse, str]:
        ...
        ...
        :return: The parsed response data.
        :rtype: Union[ConnectorQueryResponse, str]
        """
        ...
        response, status, content = self.send_request(serialized_request)
        if content == "application/json":
            return ConnectorQueryResponse._unmap(response)
        if content == "application/xml":
            return ConnectorQueryResponse._unmap(parse_xml_to_dict(response))
        raise ApiError("Error on deserializing the response.", status, response)
