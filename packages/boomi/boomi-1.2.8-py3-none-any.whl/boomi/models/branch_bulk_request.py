...
class BranchBulkRequestType(Enum):
    ...
    def list():
        ...
        :rtype: list
        """
        return list(map(lambda x: x.value, BranchBulkRequestType._member_map_.values()))
...
class BranchBulkRequest(BaseModel):
    ...
    :type type_: BranchBulkRequestType, optional
    """

    def __init__(
        self,
        request: List[BulkId] = SENTINEL,
        type_: BranchBulkRequestType = SENTINEL,
        **kwargs,
    ):
        """BranchBulkRequest
        ...
        if type_ is not SENTINEL:
            self.type_ = self._enum_matching(
                type_, BranchBulkRequestType.list(), "type_"
            )
        self._kwargs = kwargs
