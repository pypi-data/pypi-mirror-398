...
class ProcessBulkRequestType(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, ProcessBulkRequestType._member_map_.values())
        )
...
class ProcessBulkRequest(BaseModel):
    ...
    :type type_: ProcessBulkRequestType, optional
    """

    def __init__(
        self,
        request: List[BulkId] = SENTINEL,
        type_: ProcessBulkRequestType = SENTINEL,
        **kwargs,
    ):
        """ProcessBulkRequest
        ...
        if type_ is not SENTINEL:
            self.type_ = self._enum_matching(
                type_, ProcessBulkRequestType.list(), "type_"
            )
        self._kwargs = kwargs
