"""Shared data structures for backend implementations."""
...
class ContentBlock(dict):
    """Represents a content block detected in a document page.

    This is the standardized output format for all backends, containing:
    - type: The block type (text, table, image, equation, etc.)
    - bbox: Normalized bounding box coordinates [x1, y1, x2, y2] in 0-1 range
    - angle: Optional rotation angle (None, 0, 90, 180, 270)
    - content: Extracted text content (None for layout-only mode or image blocks)
    """
    ...
    def __init__(
        ...
        Args:
            type: Type of the block (e.g., 'text', 'image', 'table').
            bbox: Bounding box coordinates [xmin, ymin, xmax, ymax] normalized to 0-1.
            angle: Rotation angle of the block. Must be one of {None, 0, 90, 180, 270}.
            content: The content of the block (if exists).
        """
        super().__init__()
