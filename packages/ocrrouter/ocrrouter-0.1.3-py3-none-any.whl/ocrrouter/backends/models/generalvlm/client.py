...
class GeneralVLMSamplingParams(SamplingParams):
    """Sampling parameters optimized for General VLM OCR."""
...
class GeneralVLMClient:
    """General VLM client for document OCR.

    This client supports OCR extraction with any VLM model (GPT, Claude, Gemini, etc.).
    It does not support layout detection.

    Example:
        >>> client = GeneralVLMClient()
    ...
    def __init__(
        ...
        # Create VLM client with General VLM configuration
        self.client = new_vlm_client(
            backend=backend,
            model_name=settings.generalvlm_model_name,
            server_url=settings.openai_base_url,
            api_key=settings.openai_api_key,
            system_prompt="",  # General VLM uses empty system prompt
        ...
        self.preprocessor = GeneralVLMPreprocessor()
        self.postprocessor = GeneralVLMPostprocessor(debug=settings.debug)
    ...
    async def aio_batch_content_extract(
        ...
        return await gather_tasks(
            ...
            tqdm_desc="GeneralVLM OCR Extraction",
        )
    ...
    async def aio_full_page_ocr(
        ...
        Parses OCR output into text blocks.
