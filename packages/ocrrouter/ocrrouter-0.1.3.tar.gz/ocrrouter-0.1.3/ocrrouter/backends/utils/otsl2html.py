"""OTSL (Optimized Table Structure Language) to HTML converter.

This module provides utilities to convert OTSL format table representations
to standard HTML tables. OTSL uses special tokens to represent table structure:
- <nl>: New line/row
- <fcel>: Filled cell (cell with content)
- <ecel>: Empty cell
- <lcel>: Left merge (horizontal span)
- <ucel>: Up merge (vertical span)
- <xcel>: Both horizontal and vertical merge (2D span)
"""
...
def otsl_parse_texts(texts, tokens):
    ...
    # Check and pad the matrix to make it complete
    if split_row_tokens:
        # Find the maximum number of columns
        max_cols = max(len(row) for row in split_row_tokens)

        # Pad each row to reach the maximum number of columns
        for row_idx, row in enumerate(split_row_tokens):
            while len(row) < max_cols:
                row.append(OTSL_ECEL)

        # Also need to pad <ecel> in texts accordingly
        # Rebuild texts to include the padded <ecel>
        new_texts = []
        text_idx = 0
        ...
        for row_idx, row in enumerate(split_row_tokens):
            for col_idx, token in enumerate(row):
                ...
                # If this token has corresponding text content in the original texts, add it
                if text_idx < len(texts) and texts[text_idx] == token:
                    text_idx += 1
                    # Check if the next one is text content (not a token)
...
def convert_otsl_to_html(otsl_content: str) -> str:
    ...
    Args:
        otsl_content: OTSL formatted string containing table structure tokens.
