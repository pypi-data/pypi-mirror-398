"""Hunyuan-OCR client for VLM inference."""
...
from .preprocessor import HunyuanOCRPreprocessor
from .postprocessor import HunyuanOCRPostprocessor
...
class HunyuanSamplingParams(SamplingParams):
    """Sampling parameters optimized for Hunyuan-OCR."""
...
DEFAULT_SAMPLING_PARAMS = HunyuanSamplingParams()


class HunyuanOCRClient:
    """Hunyuan-OCR client for document OCR.
    ...
    def __init__(
        ...
        # Create VLM client with Hunyuan configuration
        self.client = new_vlm_client(
            backend=backend,
            model_name=settings.hunyuan_model_name,
            server_url=settings.openai_base_url,
            api_key=settings.openai_api_key,
            system_prompt="",  # Hunyuan uses empty system prompt
        ...
        self.preprocessor = HunyuanOCRPreprocessor()
        self.postprocessor = HunyuanOCRPostprocessor(debug=settings.debug)
    ...
    async def aio_batch_content_extract(
        ...
        return await gather_tasks(
            ...
            tqdm_desc="Hunyuan OCR Extraction",
        )
    ...
    async def aio_full_page_ocr(
        ...
        Parses OCR output into text and image blocks based on coordinates.
