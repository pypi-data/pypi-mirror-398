"""Hunyuan-OCR backend implementation for document processing."""
...
class HunyuanOCRBackend(BaseModelBackend):
    """Hunyuan-OCR backend for document analysis using Vision Language Models.
    ...
    Example:
        ...
        >>> backend = HunyuanOCRBackend(settings)
        >>> middle_json, results = await backend.analyze(pdf_bytes)
    """

    def __init__(self, settings: Settings):
        """Initialize the Hunyuan backend.
    ...
    @property
    def client(self) -> HunyuanOCRClient:
        """Get the Hunyuan client instance."""
        if self._client is None:
            self._client = HunyuanOCRClient(self._settings)
        return self._client
    ...
    async def analyze(
        ...
        **kwargs: Any,
        ...
        # Validate output_mode - Hunyuan only supports OCR
        if output_mode == "layout_only":
            raise ValueError(
                "HunyuanOCR does not support 'layout_only' mode. "
                "Use 'ocr_only' or use CompositeBackend with a different layout_model."
            )
        if output_mode == "all":
            raise ValueError(
                "HunyuanOCR does not support 'all' mode (layout + OCR). "
                "Use 'ocr_only' or use CompositeBackend with layout_model and ocr_model='hunyuanocr'."
            )
        ...
        logger.debug("Running Hunyuan full-page OCR (ocr_only mode)")
