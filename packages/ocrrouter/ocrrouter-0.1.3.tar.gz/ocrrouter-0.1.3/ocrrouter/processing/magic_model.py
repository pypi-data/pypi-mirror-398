...
# Maximum index gap between caption/footnote and body for reassignment
MAX_CAPTION_DISTANCE = 3
...
def isolated_formula_clean(txt):
    ...
    # Remove $$ delimiters (but preserve content after closing $$, e.g., equation numbers)
    if latex.startswith("$$"):
        latex = latex[2:]  # Remove opening $$
        # Find and remove closing $$
        closing_pos = latex.find("$$")
        if closing_pos != -1:
            # Remove the closing $$ but keep everything after it
            latex = latex[:closing_pos] + latex[closing_pos + 2 :]

    # Remove \[ \] delimiters (but preserve content after closing \])
    latex = latex.strip()
    if latex.startswith("\\["):
        latex = latex[2:]  # Remove opening \[
        # Find and remove closing \]
        ...
        for caption, original_block_idx in misplaced_captions:
            ...
            for idx, block in enumerate(need_fix_blocks):
                ...
                if body_index >= caption_index and idx != original_block_idx:
                    ...
                    if distance < min_distance and distance <= MAX_CAPTION_DISTANCE:
