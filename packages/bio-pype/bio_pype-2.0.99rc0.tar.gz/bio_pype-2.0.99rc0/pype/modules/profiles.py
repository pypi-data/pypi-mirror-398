...
from pype.__config__ import PYPE_DOCKER, PYPE_LOGDIR, PYPE_SINGULARITY_CACHE
...
def pull_profiles(parser, profiles_list, argv):
    ...
    Singularity cache or Docker. Checks conda-namespace environments and
    optionally creates them from embedded specifications. Images and
    environments are stored using paths that match the Namespace class
    construction.
    ...
    subparser.add_argument(
        ...
        action="store",
        default=None,
        help="Path to conda executable (default: PYPE_CONDA environment variable)",
    )
    subparser.add_argument(
        "--create",
        dest="create",
        action="store_true",
        help="Create missing conda environments from profile specifications",
    )
    args = subparser.parse_args(argv)
    ...
    # Pull images and check/create conda environments
    results = pull_profile_images(
        ...
        log,
        force=args.force,
        conda_exec=args.conda,
        create_conda_envs=args.create,
    )
