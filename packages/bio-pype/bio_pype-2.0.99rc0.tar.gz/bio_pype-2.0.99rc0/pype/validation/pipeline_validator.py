...
from pype.exceptions import (
    InvalidTemplateVariableMixError,
    PipelineError,
    PipelineItemError,
...
from pype.utils.pipeline import Pipeline, pipeline_argparse_ui
from pype.utils.queues import snippet_module_description
...
logger = logging.getLogger("bio_pype_lsp.validation")
...
class PipelineValidator:
    ...
    def __init__(
        self, context: ValidationContext, server_caches: Optional[dict] = None
    ) -> None:
        """Initialize pipeline validator.

        Args:
            context: ValidationContext for workspace information
            server_caches: Optional dict with server-level caches:
                - 'snippet_modules': Cache for loaded snippet modules
                - 'snippet_args': Cache for parsed snippet arguments
        """
        ...
        # Server-level caches (shared across all validators)
        if server_caches:
            self._server_snippet_modules_cache = server_caches.get(
                "snippet_modules", {}
            )
            self._server_snippet_args_cache = server_caches.get("snippet_args", {})
        else:
            self._server_snippet_modules_cache = {}
            self._server_snippet_args_cache = {}

        # Instance-level caches (for backwards compatibility)
    ...
    def _load_pipeline(self, file_path: Path) -> None:
        ...
        pype_modules_path = str(file_path.parents[1])
        os.environ["PYPE_MODULES"] = pype_modules_path

        # Check server cache first to avoid expensive reload_pype_snippets()
        if pype_modules_path in self._server_snippet_modules_cache:
            self.pype_snippets_modules = self._server_snippet_modules_cache[
                pype_modules_path
            ]
            logger.debug(f"Using cached snippet modules for {pype_modules_path}")
        else:
            ...
            self._server_snippet_modules_cache[pype_modules_path] = (
                self.pype_snippets_modules
            )
            logger.debug(f"Loaded and cached snippet modules for {pype_modules_path}")
        ...
        try:
        ...
        except InvalidTemplateVariableMixError as e:
            # Extract the offending value from the error message and find its line
            message = str(e)
            # The error message contains the value in quotes, e.g.:
            # "...in argument '-o': '%(var)s_suffix'..."
            # Look for the pattern: in argument 'KEY': 'VALUE'
            value_match = re.search(r"in argument '[^']+': '([^']+)'", message)
            if value_match:
                offending_value = value_match.group(1)
                # Search for the exact value in the file to find its line
                line = self._get_line_nr(offending_value)
            else:
                line = 0
            ...
            self._add_diagnostic(
                ...
                0,
                10,
                f"Invalid argument format: {message}",
                "invalid-template-variable-mix",
            )
    ...
    def _extract_snippet_arguments(
        ...
        """
        # Check server-level cache first (shared across all validations)
        if item_name in self._server_snippet_args_cache:
            return self._server_snippet_args_cache[item_name]

        # Fall back to instance cache for backwards compatibility
        ...
        try:
            ...
            self._server_snippet_args_cache[item_name] = {}
            return {}
        ...
        # Cache the result in server-level cache (shared across validations)
        self._server_snippet_args_cache[item_name] = snippet_expected_args
        return snippet_expected_args

    def _extract_pipeline_arguments(self) -> Dict[str, str]:
        """Extract all pipeline arguments using pipeline_argparse_ui.

        This function uses the pipeline's own argument parsing logic to get all
        variables, including those auto-generated from the YAML structure.
        ...
        try:
            # Create a temporary parser with subparsers
            temp_parser = ArgumentParser(add_help=False)
            temp_subparsers = temp_parser.add_subparsers()

            # Call pipeline_argparse_ui to build the argument parser
            # This mimics how the pipeline parses arguments at runtime
            parse_snippets = pipeline_argparse_ui(
                self.pipeline, temp_subparsers, self._dummy_log
            )

            # Extract arguments from the parser
            pipeline_args = {}
            if hasattr(parse_snippets, "_action_groups"):
                # Loop through all action groups (Required, Optional, Batches)
                for group in parse_snippets._action_groups:
                    if hasattr(group, "_group_actions"):
                        for action in group._group_actions:
                            if hasattr(action, "dest") and action.dest:
                                # Get the help text from the action
                                help_text = getattr(action, "help", "")
                                if help_text:
                                    pipeline_args[action.dest] = help_text
                                else:
                                    # Fall back to dest name if no help text
                                    pipeline_args[action.dest] = action.dest

            return pipeline_args

        except Exception as e:
            logger.debug(f"Could not extract pipeline arguments: {e}")
            return {}
        ...
        try:
            ...
            for req_arg in required_args:
                ...
                if not pipeline_arg_keys.intersection(option_strings):
                    ...
                    line = self._get_line_nr(
                        item_name, context="name:", in_step=step_id
                    )
                    self._add_diagnostic(
                        DiagnosticSeverity.ERROR,
                        line,
            ...
            try:
                ...
                if mock_args:
                    ...
                    if result_key not in results_dict:
                        ...
                        line = self._get_line_nr(
                            result_key, in_step=step_id, in_arg=arg_key
                        )
                        self._add_diagnostic(
                            DiagnosticSeverity.ERROR,
                            line,
    ...
    def validate(self, file_path: Path) -> ValidationResult:
        ...
        for snippet_name, snippet_module in self.pype_snippets_modules.items():
            snippets_descriptions[snippet_name] = snippet_module_description(
                snippet_module
            )
        ...
        result = ValidationResult(
            ...
            self.diagnostics,
            is_valid,
            available_snippets=snippets_descriptions,
        )

        # Extract pipeline arguments for completions
        if self.pipeline:
            result.parsed_pipeline_arguments = self._extract_pipeline_arguments()
