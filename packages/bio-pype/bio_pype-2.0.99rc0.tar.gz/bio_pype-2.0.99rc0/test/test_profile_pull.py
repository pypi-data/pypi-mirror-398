...
class TestPullProfileImages(unittest.TestCase):
    ...
    def test_pull_profile_mixed_namespaces(self):
        ...
        with patch("pype.utils.profiles.pull_singularity_image") as mock_pull:
            ...
            self.assertTrue(results["gcc"]["success"])
            self.assertIn("Skipped", results["gcc"]["message"])
            self.assertIn("env_module", results["gcc"]["message"])

            self.assertTrue(results["custom_tool"]["success"])
            self.assertIn("Skipped", results["custom_tool"]["message"])
            self.assertIn("path", results["custom_tool"]["message"])
