...
class BioPypeLspServer(LanguageServer):
    ...
    def __init__(self):
        ...
        # Validation context cache: stores module discovery results per workspace root
        # This prevents repeated filesystem scanning in handlers
        self.validation_context_cache: dict = {}  # {workspace_root: ValidationContext}

        # Snippet modules cache: stores loaded snippet modules per PYPE_MODULES path
        # Avoids expensive reload_pype_snippets() on every validation (100-500ms)
        self.snippet_modules_cache: dict = {}  # {pype_modules_path: snippet_modules}

        # Snippet arguments cache: stores parsed snippet arguments per snippet name
        # Avoids re-extracting arguments for same snippets across validations
        self.snippet_args_cache: dict = {}  # {snippet_name: args_dict}
    ...
    def initialize_workspace(self, root_uri: str) -> None:
        ...
        try:
            ...
            if (
                ...
                logger.info(
                    "No modules found at root, searching subdirectories recursively..."
                )
                all_snippets = []
                all_profiles = []
                all_pipelines = []
