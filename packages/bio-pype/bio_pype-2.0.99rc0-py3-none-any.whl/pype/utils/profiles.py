...
class ProfileProgram:
    ...
    Attributes:
        namespace: Namespace format string 'type@item' where type is one of:
            ...
            - 'conda': Conda environment (format: conda@env_name)
            - 'path': Direct filesystem path (format: path@/absolute/path)
        version: Program version tag or identifier
        path: Alternative path to executable or conda environment location
            - For conda: custom environment location (uses conda run -p)
            - For others: rarely used with namespace
        ...
        environment: Conda environment specification dict (conda namespaces only)
            Format matches conda environment.yaml structure:
            {
                'channels': ['conda-forge', 'bioconda', ...],
                'dependencies': ['python>=3.8', 'samtools', ...]
            }
            Note: 'name' field is automatically added from namespace.str
    """
...
def check_conda_environment(
    namespace: Namespace, program: ProfileProgram, conda_exec: str, log: Any = None
) -> Dict[str, Any]:
    """Check if conda environment exists.

    Args:
        namespace: Namespace object (must be conda type)
        program: ProfileProgram object (for accessing environment spec and path)
        conda_exec: Path to conda executable
        log: Logger instance

    Returns:
        Dict with keys:
            - success: bool - True if environment exists
            - env_name: str - Name of conda environment
            - env_path: str - Full path if path-based, None if name-based
            - message: str - Status message
            - has_spec: bool - Whether environment spec is provided in profile
    """
    import subprocess

    # Skip non-conda namespaces
    if namespace.type != "conda":
        return {
            "success": True,
            "env_name": None,
            "env_path": None,
            "message": "Skipped (not conda namespace)",
            "has_spec": False,
        }

    # Get environment name and optional custom path
    conda_env = namespace.str
    env_path = getattr(program, "path", None)
    has_spec = program.environment is not None

    # Build full path if path-based
    if env_path:
        full_env_path = os.path.join(env_path, conda_env)
    else:
        full_env_path = None

    if log:
        if full_env_path:
            log.log.info(f"Checking conda environment at path: {full_env_path}")
        else:
            log.log.info(f"Checking conda environment: {conda_env}")

    try:
        # For path-based envs, check if directory exists
        if full_env_path:
            # Path-based environment - check directory exists
            if os.path.isdir(full_env_path):
                # Verify it's a valid conda env by checking for conda-meta
                conda_meta = os.path.join(full_env_path, "conda-meta")
                if os.path.isdir(conda_meta):
                    return {
                        "success": True,
                        "env_name": conda_env,
                        "env_path": full_env_path,
                        "message": f"Environment exists at {full_env_path}",
                        "has_spec": has_spec,
                    }
                else:
                    return {
                        ...
                        "env_name": conda_env,
                        "env_path": full_env_path,
                        "message": f"Directory exists but is not a valid conda environment",
                        "has_spec": has_spec,
                    }
            else:
                # Path doesn't exist
                if has_spec:
                    message = f"Environment not found at {full_env_path} (can be created from spec)"
                else:
                    message = (
                        f"Environment not found at {full_env_path} (no spec provided)"
                    )
            ...
            # Name-based environment - use conda env list
            cmd = [conda_exec, "env", "list"]
            ...
            if result.returncode != 0:
                return {
                    ...
                    "env_name": conda_env,
                    "env_path": None,
                    "message": f"Failed to list conda environments: {result.stderr.strip()}",
                    "has_spec": has_spec,
                }

            # Parse output to find environment
            # Lines look like:
            # base                     /path/to/conda
            ...
            for line in result.stdout.splitlines():
                ...
                # First part is environment name
                env_name_in_line = parts[0]
                if env_name_in_line == conda_env:
                    return {
                        "success": True,
                        "env_name": conda_env,
                        "env_path": None,
            ...
            if has_spec:
                message = (
                    f"Environment '{conda_env}' not found (can be created from spec)"
                )
            else:
                message = f"Environment '{conda_env}' not found (no spec provided)"
        ...
        return {
            ...
            "env_name": conda_env,
            "env_path": full_env_path,
            "message": "Timeout checking conda environments",
            "has_spec": has_spec,
        }
    except Exception as e:
        return {
...
def create_conda_environment(
    ...
    """Create conda environment from embedded specification.

    Args:
        namespace: Namespace object with environment name
        program: ProfileProgram with environment spec and optional path
        conda_exec: Path to conda executable
        log: Logger instance
    ...
    if not program.environment:
        return {
            "success": False,
            "env_name": conda_env,
            "env_path": env_path,
            "message": "No environment specification provided",
        }
    ...
    if log:
        if full_env_path:
            log.log.info(f"Creating conda environment at: {full_env_path}")
        else:
            log.log.info(f"Creating conda environment: {conda_env}")

    # Build complete environment spec with name
    env_spec = dict(program.environment)
    env_spec["name"] = conda_env  # Inject name from namespace

    # Write spec to temporary YAML file
    try:
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".yaml", delete=False
        ) as tmp_file:
            yaml.dump(env_spec, tmp_file, default_flow_style=False)
            tmp_path = tmp_file.name

        # Build conda env create command
        if full_env_path:
            # Path-based: conda env create -p <path> -f environment.yaml
            cmd = [
                conda_exec,
                "env",
                "create",
            ...
            # Name-based: conda env create -f environment.yaml
            # (name comes from the YAML file)
            cmd = [conda_exec, "env", "create", "-y", "-f", tmp_path]

        if log:
            log.log.info(f"Running: {' '.join(cmd)}")

        result = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            timeout=600,  # 10 min timeout for package installation
        )
        ...
        if result.returncode != 0:
            return {
                ...
                "env_name": conda_env,
                "env_path": full_env_path,
                "message": f"Failed to create environment: {result.stderr.strip()}",
            }

        if full_env_path:
            message = f"Environment created successfully at {full_env_path}"
        else:
        ...
        return {
            ...
            "env_name": conda_env,
            "env_path": full_env_path,
            "message": "Timeout creating environment (>10 minutes)",
        }
    except Exception as e:
        return {
            "success": False,
...
def pull_profile_images(
    ...
    """Pull container images and check/create conda environments for a profile.

    Iterates through all programs in the profile and:
    - Pulls docker-namespace images to Singularity cache or Docker
    - Checks conda-namespace environments and optionally creates them
    - Skips path and env_module namespaces

    Uses Namespace class for consistent namespace parsing and DockerConfig
    ...
    Args:
        ...
        docker_exec: Path to container runtime executable (docker/singularity)
        log: Optional logger instance for info/error messages
        force: Re-pull images even if they already exist (currently ignored)
        conda_exec: Path to conda executable (default: PYPE_CONDA from config)
        create_conda_envs: If True, create missing conda environments from specs

    Returns:
        Dict mapping program names to result dicts, each containing:
            - success: bool indicating operation succeeded or was skipped
            - message: str status message or error details
            - Additional keys depend on namespace type (sif_path, env_name, etc.)
    ...
    for program_name, program_obj in profile.programs.items():
        # program_obj is now a ProfileProgram object
        namespace = Namespace(program_obj, log.log, profile)

        # Route based on namespace type
        if namespace.type == "docker":
            if log:
        ...
        elif namespace.type == "conda":
            # Check if environment exists
            check_result = check_conda_environment(
                namespace, program_obj, conda_exec, log
            )

            if (
                not check_result["success"]
                and create_conda_envs
                and check_result["has_spec"]
            ):
                # Environment missing but we have spec and --create flag
                if log:
                    log.log.info(f"Creating conda environment for {program_name}...")
                create_result = create_conda_environment(
                    namespace, program_obj, conda_exec, log
                )
                result = create_result
            else:
                # Just report check result
                result = check_result

        else:
            # path or env_module - skip
            result = {
                "success": True,
                "message": f"Skipped ({namespace.type} namespace)",
            }
