...
from pype.exceptions import ArgumentError, InvalidTemplateVariableMixError
...
class Argument:
    def __init__(self, argument: Dict[str, Any]):
        ...
        # Validate template variable format before continuing
    ...
    def _validate_template_format(self) -> None:
        """Validate that template variables are not mixed with static text.

        Raises InvalidTemplateVariableMixError if value contains:
        - Template variables %(...)s/i/f AND static text/prefixes/suffixes
        - Multiple template variables with non-space separators

        Valid:
        - "%(var)s" - pure template
        - "static_text" - no templates
        - 123 - numeric value

        Invalid:
        - "%(var)s_suffix" - template with suffix
        - "prefix_%(var)s" - template with prefix
        - "%(var1)s_%(var2)s" - multiple templates joined with non-space
        """
        # Skip non-string values (numbers, booleans, etc)
        if not isinstance(self.value, str):
            return

        # Find all template variable patterns %(...)s/i/f
        template_pattern = r"%\([^)]+\)[sif]"
        matches = list(re.finditer(template_pattern, self.value))

        if not matches:
            # No templates found - pure static string (valid)
            return

        if len(matches) == 1:
            # Single template - check if it's the ONLY content
            match = matches[0]
            if match.start() == 0 and match.end() == len(self.value):
                # Pure template variable (valid)
                return
            else:
                # Template mixed with other text (invalid)
                raise InvalidTemplateVariableMixError(
                    f"Template variable cannot be mixed with static text in argument '{self.key}': '{self.value}'. "
                    f"Use either a pure variable or a static string. "
                    f"Implement prefix/suffix logic in your launching script or snippet code.",
                    argument_key=self.key,
                )

        # Multiple templates - check separators
        # Note: Space-only separators may be allowed in API 2.1.0
        for i in range(len(matches) - 1):
            between = self.value[matches[i].end() : matches[i + 1].start()]
            if between and not between.isspace():
                # Non-space separator between templates (invalid)
                raise InvalidTemplateVariableMixError(
                    f"Multiple template variables cannot be joined with non-space characters in argument '{self.key}': '{self.value}'. "
                    f"Implement concatenation logic in your launching script or snippet code.",
                    argument_key=self.key,
                )
