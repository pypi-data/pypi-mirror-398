...
"""
arifOS Caged Gemini Demo with PHOENIX SOVEREIGNTY (v36.1Omega)

This script demonstrates Python-sovereign governance for Google Gemini models.

PHOENIX SOVEREIGNTY:
    - Real Gemini via existing adapter (llm_gemini.py)
    - Python-sovereign verdict via ApexMeasurement.judge()
    - If Amanah fails, verdict is VOID (no negotiation)
    - Same governance as Claude/OpenAI/SEA-LION (One Law for All Models)

Usage (from repo root):

    # Mock mode (default):
    python -m scripts.arifos_caged_gemini_demo --query "Explain neural networks."

    # Real Gemini mode:
    GOOGLE_API_KEY=... python -m scripts.arifos_caged_gemini_demo --query "..." --real

    # Force mock mode explicitly:
    python -m scripts.arifos_caged_gemini_demo --mock --query "..."
...
# PHOENIX SOVEREIGNTY: Import real Gemini adapter (optional)
_REAL_LLM_AVAILABLE = False
_make_llm_generate: Optional[Callable] = None

try:
    from arifos_core.adapters.llm_gemini import make_llm_generate as _make_llm_generate
...
def mock_llm_call(prompt: str) -> str:
    ...
    return (
        ...
        "This is a mock response. In a real deployment, Gemini would "
...
def _get_llm_generator(use_real: bool, model: str) -> Callable[[str], str]:
    ...
    if use_real and _REAL_LLM_AVAILABLE and _make_llm_generate is not None:
        ...
        if not api_key:
            print("[WARN] --real specified but GOOGLE_API_KEY not set. Falling back to mock.")
...
def _run_apex_judgment(answer: str, high_stakes: bool) -> Dict[str, Any]:
    ...
    dials = {
        "A": 0.86,  # Ability - Gemini is capable
        "P": 0.80,  # Prosociality - governed prompt
        "E": 0.76,  # Energy - standard operation
        "X": 0.85,  # Experience - Gemini has training
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="arifOS caged Gemini demo with PHOENIX SOVEREIGNTY (v36.1Omega)."
    )
    parser.add_argument(
        "--query",
        type=str,
    ...
    parser.add_argument(
        "--real",
        action="store_true",
        help="Use real Gemini API (requires GOOGLE_API_KEY).",
    )
    parser.add_argument(
        "--mock",
        action="store_true",
    ...
    llm_mode = "REAL GEMINI" if use_real else "MOCK"

    print("\n" + "=" * 60)
    print("arifOS PHOENIX SOVEREIGNTY - Gemini Demo (v36.1Omega)")
    print("=" * 60)
