...
class Zoo:
    ...
    def backtest(self,
        ...
        with tqdm(total=task_num, desc="Generating report", leave=False) as bar:
            for alpha_name in alpha_names:
                bar.set_postfix_str(alpha_name)
                logger.info(f"ALPHA NAME: {alpha_name}")
                if reverse:
                    factor_data = factor_data.with_columns((-pl.col(alpha_name)).alias(alpha_name))
                weight_df = weight_fn(factor_data)
