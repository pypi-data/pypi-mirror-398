...
def assign_category(input_genes, species):
    """
    Assign each input gene to one (or more) predefined categories
    (Rb, Mrp, other_Rb, chrM, MALAT1, chrX, chrY, etc.).

    Parameters
    ----------
    input_genes : list-like
        Gene identifiers (column names of the input matrix).
    species : {"hs", "mm"}
        Human ("hs") or mouse ("mm"). Controls which gene list file is used.

    Returns
    -------
    pandas.DataFrame
        Boolean matrix of shape (n_genes, n_categories), where rows are
        input genes (same order as `input_genes`) and columns are categories.
        True indicates that the gene belongs to that category.
    """
    assert species in ["hs", "mm"], "species must be 'hs' or 'mm'"

    if species == "hs":
        gene_list_path = os.path.join(
            os.path.split(__file__)[0], "txt", "genelist.hs.new.txt"
        )
    else:  # species == "mm"
        gene_list_path = os.path.join(
            os.path.split(__file__)[0], "txt", "genelist.mm.new.txt"
        )

    gene_list = pd.read_table(gene_list_path, sep="\t", header=None)
    ...
    # Make column names explicit for clarity
    gene_df.columns = ["category", "gene"]

    # Convert input gene list to numpy array for fast membership checks
    input_arr = np.array(input_genes_short)

    # Build a boolean matrix: rows = input_genes, columns = categories
    categories = gene_df["category"].unique()
    logic = []

    for cat in categories:
        # Subset rows of gene_df that belong to this category
        subset = gene_df[gene_df["category"] == cat]
        genes_in_cat = subset["gene"].to_numpy()

        # Boolean vector: for each input gene, is it in this category?
        logic.append(np.isin(input_arr, genes_in_cat))

    # Stack along columns to shape (n_genes, n_categories)
