...
def run_scoped_http_server(
    ...
    logger.info(
        ...
        services=registry.get_registered_services(),
        endpoints=[
            f"http://{config.host}:{config.port}/mcp/message",
            f"http://{config.host}:{config.port}/mcp/auth/message",
            f"http://{config.host}:{config.port}/mcp/reg/message",
        ],
    )
