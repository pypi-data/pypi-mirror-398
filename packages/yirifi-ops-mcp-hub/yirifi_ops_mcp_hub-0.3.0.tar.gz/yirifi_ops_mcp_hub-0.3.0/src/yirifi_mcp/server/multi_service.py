...
from yirifi_mcp.core.config import AuthServiceConfig, RegServiceConfig, ServiceConfig
...
async def create_service_server(
    ...
    if service == "auth":
        ...
        config = AuthServiceConfig(mode=mode, output_format=output_format)
        return await create_auth_server(config=config)

    elif service == "reg":
        from yirifi_mcp.servers.reg_service.server import create_reg_server

        config = RegServiceConfig(mode=mode, output_format=output_format)
...
""",
    ...
    for service in services:
        try:
            ...
            # Create service config for the mapping
            if service == "auth":
                service_config = AuthServiceConfig(mode=mode, output_format=output_format)
            elif service == "reg":
                service_config = RegServiceConfig(mode=mode, output_format=output_format)
            else:
                raise ValueError(f"Unknown service: {service}")
    ...
    # Use AuthServiceConfig as default fallback (has mode attribute needed by wrap_response)
    default_config = AuthServiceConfig(
        mode=mode,
        output_format=output_format,
        server_name="yirifi-ops-hub",
    )
