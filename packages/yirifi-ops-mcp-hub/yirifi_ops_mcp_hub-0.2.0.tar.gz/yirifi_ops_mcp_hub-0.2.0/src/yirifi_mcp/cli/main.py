...
def _run_server(
    ...
    async def create_server():
        ...
        if service == "all":
            ...
            return await create_multi_service_server(services=services, mode=mode)
    ...
    try:
        if transport == "stdio":
            ...
            from yirifi_mcp.server.http import run_http_server
            ...
            click.echo("Transport: HTTP (Streamable)")
            click.echo(f"Listening on: http://{http_config.host}:{http_config.port}{http_config.path}")
            click.echo("Auth: X-API-Key passthrough to upstream")

            mcp = asyncio.run(create_server())
            run_http_server(mcp, http_config)
...
def scaffold(name: str, base_url: str, openapi_path: str):
    ...
    click.echo(f"2. Customize routes.py to exclude unwanted endpoints")
    click.echo(f"3. Register the service in cli/main.py")
...
def show_spec(service: str, mode: str):
    ...
    async def show():
        ...
        click.echo(f"Fetching OpenAPI spec from: {config.base_url}{config.openapi_path} (mode: {mode})")
