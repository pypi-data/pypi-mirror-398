...
impl StreamingTranscriber {
    ...
    fn transcribe_buffer(&mut self, is_final: bool) -> Result<Vec<StreamingEvent>, AntennaError> {
        ...
        // For partial results, we PEEK to avoid consuming the buffer
        // This allows the final transcription to have all the audio
        // For final results, we READ ALL to consume and clear the buffer
        let samples = if is_final {
            self.buffer.read_all()
        } else {
            // Peek at all samples without consuming them
            // This allows the full audio to be available for the final transcription
