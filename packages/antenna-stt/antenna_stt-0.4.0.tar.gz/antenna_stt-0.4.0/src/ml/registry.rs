...
#[cfg(feature = "sherpa")]
use crate::ml::parakeet::ParakeetModel;
...
impl ModelSpec {
    ...
    pub fn parse(model_id: &str) -> Result<Self, AntennaError> {
        ...
        // Note: Wav2Vec2 uses community ONNX exports since facebook models don't include ONNX
        let hf_model_id = match family {
            ...
            ModelFamily::Wav2Vec2 => Some(wav2vec2_variant_to_hf_id(&variant)),
            _ => None,
        };
    ...
    pub fn hf_id(&self) -> String {
        self.hf_model_id.clone().unwrap_or_else(|| {
            match self.family {
                ...
                ModelFamily::Wav2Vec2 => wav2vec2_variant_to_hf_id(&self.variant),
...
/// Map Wav2Vec2 variant names to HuggingFace ONNX model IDs
///
/// Official facebook models don't include ONNX exports, so we use community exports.
fn wav2vec2_variant_to_hf_id(variant: &str) -> String {
    match variant {
        "base-960h" => "onnx-community/wav2vec2-base-960h-ONNX".to_string(),
        "large-xlsr-53-english" => "Xenova/wav2vec2-large-xlsr-53-english".to_string(),
        // For unknown variants, try the Xenova namespace which has many exports
        _ => format!("Xenova/wav2vec2-{}", variant),
    }
}
...
pub enum DynSpeechModel {
    ...
    /// Parakeet model (sherpa-rs backend)
    #[cfg(feature = "sherpa")]
...
impl SpeechModel for DynSpeechModel {
    fn info(&self) -> &ModelInfo {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(m) => SpeechModel::info(m),
        }
    }
    ...
    fn device(&self) -> &Device {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(m) => SpeechModel::device(m),
        }
    }
    ...
    fn transcribe(
        ...
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(m) => SpeechModel::transcribe(m, audio, options),
        }
    }
    ...
    fn detect_language(&mut self, audio: &AudioData) -> Result<String, AntennaError> {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(m) => SpeechModel::detect_language(m, audio),
        }
    }
    ...
    fn preprocess_audio(&self, audio: &AudioData) -> Result<AudioData, AntennaError> {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(m) => SpeechModel::preprocess_audio(m, audio),
        }
    }
}
...
impl DynSpeechModel {
    ...
    pub fn family(&self) -> ModelFamily {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(_) => ModelFamily::Parakeet,
        }
    }
    ...
    pub fn backend(&self) -> Backend {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(_) => Backend::Sherpa,
        }
    }
    ...
    pub fn translate(&mut self, audio: &AudioData) -> Result<TranscriptionResult, AntennaError> {
        match self {
            ...
            #[cfg(feature = "sherpa")]
            DynSpeechModel::Parakeet(m) => SpeechModel::translate(m, audio),
        }
    }
...
pub fn load_model(
    ...
    match (spec.family, selected_backend) {
        ...
        #[cfg(feature = "sherpa")]
        (ModelFamily::Parakeet, Backend::Sherpa) => {
            let model = ParakeetModel::from_size(&spec.variant, device)?;
            Ok(DynSpeechModel::Parakeet(model))
        }

        #[cfg(not(feature = "sherpa"))]
        (ModelFamily::Parakeet, _) => {
            Err(AntennaError::ModelError(
...
pub fn list_models() -> Vec<ModelEntry> {
    ...
    // Note: Uses community ONNX exports since official facebook models don't include ONNX
    for (size, hf_id, desc) in [
        ("base-960h", "onnx-community/wav2vec2-base-960h-ONNX", "Base model, 960h training (ONNX export)"),
        ("large-xlsr-53-english", "Xenova/wav2vec2-large-xlsr-53-english", "Large multilingual model, English fine-tuned"),
    ] {
        models.push(ModelEntry {
    ...
    // Parakeet models (require sherpa feature)
    // NVIDIA FastConformer-TDT models via sherpa-onnx
    for (variant, desc) in [
        ("tdt-0.6b-v2", "Parakeet TDT 0.6B v2 - English, int8 quantized (~600M params)"),
        ("tdt-0.6b-v3", "Parakeet TDT 0.6B v3 - Multilingual, 25 European languages"),
        ("tdt-0.6b-en", "Parakeet TDT 0.6B - English, original version"),
    ] {
        models.push(ModelEntry {
            id: format!("parakeet/{}", variant),
            hf_id: format!("nvidia/parakeet-{}", variant),
            description: desc.to_string(),
            family: ModelFamily::Parakeet,
            default_backend: Backend::Sherpa,
            feature_flag: Some("sherpa".to_string()),
        });
    }
