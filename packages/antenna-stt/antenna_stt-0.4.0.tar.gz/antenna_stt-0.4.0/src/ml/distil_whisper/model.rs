...
use crate::ml::whisper::config::LANGUAGES;
...
impl DistilWhisperModel {
    ...
    fn transcribe_chunk(
        ...
        let mut prompt_tokens = vec![special_tokens.sot];
        ...
        match options.task {
            Task::Translate => prompt_tokens.push(special_tokens.translate),
            Task::Transcribe => prompt_tokens.push(special_tokens.transcribe),
        }

        if !options.timestamps {
            prompt_tokens.push(special_tokens.no_timestamps);
        }
    ...
    fn detect_language_internal(&mut self, mel: &Tensor) -> Result<String, AntennaError> {
        ...
        let special_tokens = self.tokenizer.special_tokens();
        let sot_tensor = Tensor::new(&[special_tokens.sot], &self.device)
        ...
        let lang_start = special_tokens.language_token_start as usize;
