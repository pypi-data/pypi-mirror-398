//! Local Agreement Policy for Streaming Transcription
//!
...
impl LocalAgreementPolicy {
    ...
    pub fn with_config(config: AgreementConfig) -> Self {
        Self {
            config,
            state: Mutex::new(AgreementState::new()),
        }
    }
    ...
    pub fn process(&self, tokens: Vec<TokenInfo>) -> AgreementResult {
        let mut state = self.state.lock();

        // Add to history
        state.history.push_back(tokens.clone());

        // Keep only the last N transcriptions
        while state.history.len() > self.config.agreement_count {
            state.history.pop_front();
        }

        // Find agreed-upon tokens
        let (confirmed, pending, forced) = self.find_agreement(&state, &tokens);

        // Update confirmed count
        state.confirmed_count += confirmed.len();

        // Build result
        let confirmed_text = confirmed.iter().map(|t| t.text.as_str()).collect::<Vec<_>>().join("");
        let pending_text = pending.iter().map(|t| t.text.as_str()).collect::<Vec<_>>().join("");
    ...
    fn find_agreement(
        ...
        current_tokens: &[TokenInfo],
    ) -> (Vec<TokenInfo>, Vec<TokenInfo>, bool) {
        // If we don't have enough history, nothing is confirmed yet
        if state.history.len() < self.config.agreement_count {
            return (
                vec![],
                current_tokens.to_vec(),
                false,
            );
        }

        // Start from the already confirmed position
        let start_pos = state.confirmed_count;
        ...
        'outer: for pos in start_pos..current_tokens.len() {
            ...
            for past in state.history.iter() {
                if pos >= past.len() {
    ...
    pub fn confirmed_count(&self) -> usize {
        self.state.lock().confirmed_count
    }
...
mod tests {
    ...
    fn test_forced_emission() {
        ...
        let policy = LocalAgreementPolicy::with_config(config);
