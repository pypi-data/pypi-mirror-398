...
def Link(**props):
    ...
    # Determine if this is an internal routing link
    is_external = to.startswith("http") or to.startswith("//") or to.startswith("mailto:")
    is_new_tab = anchor_props.get("target") == "_blank"
    
    should_use_router = not is_external and not is_new_tab
    ...
    # Only attach the handler if we need to intercept navigation OR if there is a custom user handler
    # Note: If there is a user handler on an external link, it will run on server, 
    # BUT the current client script (cli.py) prevents default if ANY handler is present.
    # So for external links, we must simpler NOT attach handler if we want browser navigation,
    # unless we fix client script.
    # For now, to solve the user's issue: prioritized browser navigation for external links.
    
    if should_use_router or (user_on_click and not is_external and not is_new_tab):
        anchor_props["onClick"] = handle_click
    
    # If it is external but has user_on_click, we currently skip user_on_click to allow navigation
    # because our client script is aggressive with preventDefault.
