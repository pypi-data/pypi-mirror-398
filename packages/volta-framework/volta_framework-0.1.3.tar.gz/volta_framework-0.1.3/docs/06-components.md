...
**Benefits:**
- ✅ **No full page reload**: Instant navigation for internal routes.
- ✅ **Preserves application state**: Hooks and state stay alive.
- ✅ **Active state detection**: Built-in styling for active links.
- ✅ **Smart External Links**: Automatically detects `http://`, `https://`, or `target="_blank"` and falls back to native browser behavior for external sites.
