...
- **Automatic Port Hunting**: If the requested port (default 3000) is in use, Volta will automatically try the next available one (e.g., 3001, 3002).
- **Graceful Error Handling**: Stale event handlers from reloaded pages return 410 Gone to keep the console clean.
...
| File/Folder | Purpose |
|-------------|---------|
| `app/App.vpx` | Root application component (maps to `/`) |
| `app/about/App.vpx` | About page component (maps to `/about`) |
| `app/[:id]/App.vpx` | Dynamic page component (maps to `/:id`) |
