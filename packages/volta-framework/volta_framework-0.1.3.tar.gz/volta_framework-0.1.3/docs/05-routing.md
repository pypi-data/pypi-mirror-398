...
1. [Directory-Based Routing (Recommended)](#directory-based-routing)
2. [Basic Setup (Manual)](#basic-setup)
3. [Route Component](#route-component)
...
8. [Catch-All Routes](#catch-all-routes)
9. [404 Not Found Handling](#404-not-found-handling)
10. [Route Guards](#route-guards)
11. [Active Link Styling](#active-link-styling)

---

## Directory-Based Routing

Volta uses **Directory-Based Routing** by default. This means your file system is your router. Any `.vpx` file inside the `app/` directory automatically becomes a route.

### File Mapping

Volta maps URL paths to files following these conventions:

| URL Path | File Path |
|----------|-----------|
| `/` | `app/App.vpx` |
| `/about` | `app/about/App.vpx` |
| `/blog` | `app/blog/App.vpx` |
| `/users/123` | `app/users/[:id]/App.vpx` |

### Dynamic Routes in Directories

To create a dynamic route, use a directory name wrapped in brackets like `[:paramName]` or `[paramName]`.
...
├── App.vpx             # Matches /
├── about/
│   └── App.vpx         # Matches /about
└── products/
    └── [:id]/
        └── App.vpx     # Matches /products/123, /products/apple, etc.
```

### Accessing Parameters

When a component is matched via directory-based routing, any dynamic segments are automatically passed as props to your `App` component.

```python
# app/products/[:id]/App.vpx
...
> **Note:** Volta is smart! If your `App` component doesn't define the parameter (e.g., `def App():`), it will safely ignore the extra props instead of crashing.
...
The `Link` component is smart enough to handle external links. If you pass a URL starting with `http`, `https`, or use `target="_blank"`, Volta will automatically render a standard anchor tag that skips the internal router.

```python
# Internal navigation - uses client-side routing
<Link to="/about">About Us</Link>

# External link - opens in new tab natively
<Link to="https://github.com/volta-framework" target="_blank">
    GitHub
</Link>

# External link - navigates away natively
<Link to="https://google.com">Search</Link>
```
