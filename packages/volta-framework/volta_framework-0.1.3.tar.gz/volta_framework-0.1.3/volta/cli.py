...
def resolve_route(path):
    """
    Resolves a URL path to a file in the app/ directory using file-system routing.
    Supports:
    - / -> app/App.vpx
    - /about -> app/about/App.vpx
    - /users/[:id] -> app/users/[:id]/App.vpx
    
    Returns: (file_path, params_dict) or (None, {})
    """
    base_dir = os.path.join(os.getcwd(), "app")
    if not os.path.exists(base_dir):
        # Fallback for flat structure if app/ doesn't exist
        if path == "/":
             for f in ["App.vpx", "app.vpx", "App.py", "app.py"]:
    ...
    for segment in segments:
        # 1. Check for exact match directory
        exact_match = os.path.join(current_dir, segment)
        if os.path.isdir(exact_match):
            current_dir = exact_match
            continue
            
        # 2. Check for dynamic match directory ([:param])
        # We look for children of current_dir that match [:.+]
        found_dynamic = False
        if os.path.isdir(current_dir):
            for child in os.listdir(current_dir):
                child_path = os.path.join(current_dir, child)
                if os.path.isdir(child_path):
                     # Check format [:id] or [id]
                     if (child.startswith("[:") and child.endswith("]")) or (child.startswith("[") and child.endswith("]")):
                         # It is a dynamic route
                         # Extract param name
                         if child.startswith("[:"):
                             param_name = child[2:-1]
                         else:
                             param_name = child[1:-1]
    ...
    # We reached the end of segments (or there were none)
    # Provide robustness: check for App.vpx, app.vpx
    # Also index.vpx? User said App.vpx.
    
    candidates = ["App.vpx", "app.vpx"]
    for c in candidates:
...
<body>
    ...
    def do_POST(self):
        ...
        if self.path == '/_event':
            ...
            # Handler not found (stale?)
            self.send_response(410) # 410 Gone - indicates resource (handler) is no longer available
            self.end_headers()
    ...
    def render_app(self):
        ...
        try:
            ...
            # Clean all 'app' submodules to ensure fresh reload
            keys_to_remove = [k for k in sys.modules if k.startswith('app.') or k == 'app']
            ...
            # Resolve Route based on current path
            # We use the path that was just set in router.set_current_path
            from volta.router import get_current_path
            current_path = get_current_path()
            
            resolved_file, params = resolve_route(current_path)
            
            if not resolved_file:
                 # Render the NotFoundPage component
                 # We need to initialize the renderer stack to render it properly using the framework
                 from volta.router import NotFoundPage
                 ...
                 reconciler.render(h(NotFoundPage), root_node)
                 
                 # We still want to return the full HTML shell
                 app_html = str(root_node)
                 # We can reuse the HTML shell generation code below or extract it.
                 # For now, let's keep it DRY by setting app_module to a dummy object
                 # that has an App component which IS NotFoundPage.
                 
                 class DummyModule:
                      pass
                 
                 app_module = DummyModule()
                 app_module.App = NotFoundPage
                 params = {}
                 
                 # Skip the module loading block
                 resolved_file = "NotFound" # Marker to skip loading
            
            # Load the module specifically for this route
            if resolved_file != "NotFound":
                try:
                    # We need to load this specific file as a module
                    # We can generate a unique module name based on path
                    mod_name = "app." + resolved_file.replace("/", ".").replace(".vpx", "").replace(".py", "")
                    
                    # Use standard import mechanism if possible or manual load
                    # For .vpx files, we MUST use our custom loader
                    if resolved_file.endswith(".vpx"):
                        from volta.loader import VoltaImporter
                        loader = VoltaImporter()
                        spec = importlib.util.spec_from_loader(mod_name, loader, origin=resolved_file)
                    else:
                        # Fallback for .py files
                    ...
                    traceback.print_exc()
                    return f"<h1>Error loading {resolved_file}</h1><pre>{traceback.format_exc()}</pre>"

            if not hasattr(app_module, "App"):
                 return f"<h1>Error: {resolved_file} does not export an 'App' component.</h1>"
            ...
            # Render with params!
            reconciler.render(h(app_module.App, params), root_node)
...
def start_dev_server(port=3000):
    ...
    for i in range(max_retries):
        try:
            ...
            if e.errno == 98: # Address already in use
                print(f"[Volta] Port {port} is in use, trying {port+1}...")
                port += 1
            else:
                raise e
    
    if httpd is None:
        print(f"[Volta] Error: Could not find an available port after {max_retries} attempts.")
        return
    
    # Create a perfectly aligned banner
    ...
    def box_line(content, color=None, bold=False):
        # Calculate visible length of content (e.g. ⚡ is 2 cells)
        import unicodedata
        visible_len = 0
        for char in content:
        ...
        padding = " " * (width - visible_len - 2) # -2 for the left padding we add
        start_color = color if color else ""
        if bold: start_color += BOLD
        
        return f"{MAGENTA_BOLD}║{RESET}  {start_color}{content}{RESET}{padding}{MAGENTA_BOLD}║{RESET}"

    url = f"http://localhost:{port}"
    
    print(f"\n{MAGENTA_BOLD}╔{'═' * width}╗{RESET}")
    print(box_line(""))
    print(box_line("⚡ Volta Dev Server started", bold=True))
    print(box_line(f"➤ URL: {url}", color=BLUE))
    print(box_line("Watching for file changes...", color=DIM))
    print(box_line(""))
    print(f"{MAGENTA_BOLD}╚{'═' * width}╝{RESET}\n")
