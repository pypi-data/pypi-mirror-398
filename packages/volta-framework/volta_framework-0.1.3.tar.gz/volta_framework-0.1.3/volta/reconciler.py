...
class Reconciler:
    ...
    def update_function_component(self, fiber: Fiber):
        ...
        # Robust Prop Passing:
        # Check signature of the component to avoid TypeError for unexpected args
        try:
             sig = inspect.signature(fiber.type)
             # If component accepts **kwargs (VAR_KEYWORD), pass all props
             has_var_keyword = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values())
             
             if not has_var_keyword:
                 # Only pass props that are in the signature
                 allowed_args = set(sig.parameters.keys())
                 func_props = {k: v for k, v in func_props.items() if k in allowed_args}
        except ValueError:
             # Built-ins might not have signature, pass all and hope
             pass
