...
The low-level interface allows you to work with messages directly through the :class:`MessageBus <asyncdbus.message_bus.MessageBus>` with the :class:`Message <asyncdbus.Message>` class. This might be useful in the following cases:

- Implementing an application that works with DBus directly like ``dbus-send(1)`` or ``dbus-monitor(1)``.
- Creating a new implementation of the :class:`MessageBus <asyncdbus.message_bus.MessageBus>`.
...
- :func:`MessageBus.send() <asyncdbus.message_bus.MessageBus.send>`
- :func:`MessageBus.add_message_handler() <asyncdbus.message_bus.MessageBus.add_message_handler>`
- :func:`MessageBus.remove_message_handler() <asyncdbus.message_bus.MessageBus.remove_message_handler>`
- :func:`MessageBus.next_serial() <asyncdbus.message_bus.MessageBus.next_serial>`
