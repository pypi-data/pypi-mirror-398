"""
Migration v9: Add quality scores table

Creates baselinr_quality_scores table for storing data quality scores.
"""
...
def create_quality_scores_table(conn):
    """Create quality scores table with dialect-specific SQL."""
    ...
    if is_snowflake:
        ...
        conn.execute(
            text(
                """
                CREATE TABLE IF NOT EXISTS baselinr_quality_scores (
        ...
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_quality_scores_table
                ON baselinr_quality_scores (table_name, schema_name)
            """
            )
        )
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_quality_scores_calculated_at
                ON baselinr_quality_scores (calculated_at DESC)
            """
            )
        )
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS idx_quality_scores_run_id
                ON baselinr_quality_scores (run_id)
            """
            )
        )
    elif is_sqlite:
...
migration = Migration(
    version=9,
    description="Add quality scores table for storing data quality scores",
    up_python=create_quality_scores_table,
    down_sql="DROP TABLE IF EXISTS baselinr_quality_scores",
)
