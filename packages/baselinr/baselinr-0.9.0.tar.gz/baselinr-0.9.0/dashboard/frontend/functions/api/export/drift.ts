/**
 * Cloudflare Pages Function for /api/export/drift endpoint
 * Handles GET /api/export/drift - Export drift alert data
 */

import { getDemoDataService } from '../../lib/demo-data-service';
import { getDemoDataBaseUrl, parseQueryParams, errorResponse, parseIntSafe } from '../../lib/utils';
...
export async function onRequestGet(context: any): Promise<Response> {
  try {
    ...
    const data = await service.exportDrift(filters);

    if (format === 'csv') {
      return new Response(data as string, {
        headers: {
          'Content-Type': 'text/csv',
          'Content-Disposition': 'attachment; filename="drift.csv"',
    ...
    return new Response(JSON.stringify(data), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="drift.json"',
        'Access-Control-Allow-Origin': '*',
      },
    });
  } catch (error) {
    console.error('Error in /api/export/drift:', error);
