        ...
        if not data:
            ...
            if isinstance(data[0], Zone):
                ...
                if print_details:
                    ...
                    keys = ["name", "type", "ttl", "data"]
                    extra = ["disabled", "priority"]

                sortby = keys.index("type")
                last_column = keys.index("data")
            ...
            if "keys" in locals():
                ...
                for item in data:
                    ...
                    for c, key in enumerate(keys):
                        if key == "data" and item.type in ["MX", "SRV"]:
                            value = str(item.extra["priority"]) + " " + getattr(item, key, "@")
                        else:
