...
class APIParser:
    ...
    def _get_serializer_fields(self, view_class: Type) -> tuple:
        ...
        if serializer_class is None:
            # Method 3: Aggressive Instantiation
        ...
        if serializer_class is None:
            # Method 6: Manual Parameter Discovery (The "Raw Data" method)
            ...
            if manual_request_fields:
                return manual_request_fields, response_fields
    ...
    def _find_serializer_in_source(self, view_class: Type) -> Optional[Type]:
        ...
        This fixes issues where APIView doesn't declare serializer_class.
        """
        try:
            # 1. Get source code of the view class
            source = inspect.getsource(view_class)
            
            # 2. Look for patterns like "MySerializer(data=...)" or "MySerializer("
            # We match Words ending in 'Serializer' followed by '('
            matches = re.findall(r'(\w+Serializer)\(', source)
            
            if not matches:
                return None
                
            # 3. Get the most likely serializer (the first one found)
            # Usually the first one in 'post' method is likely the request serializer.
            # We pick the first one that we can resolve
            
            module = inspect.getmodule(view_class)
            
            for candidate_name in matches:
                # 4. Try to find this class in the view's module
                if hasattr(module, candidate_name):
                    candidate_class = getattr(module, candidate_name)
                    # Verify it is actually a serializer
    ...
    def _scan_for_manual_parameters(self, view_class: Type) -> List[FieldInfo]:
        ...
        try:
            ...
            required_params = set(re.findall(r"request\.data\['(\w+)'\]", source))
            
            # Pattern 2: request.data.get('param') -> Optional
            optional_params = set(re.findall(r"request\.data\.get\(['\"](\w+)['\"]", source))
            
            # Merge (if in both, treat as required essentially, or handle duplicates)
