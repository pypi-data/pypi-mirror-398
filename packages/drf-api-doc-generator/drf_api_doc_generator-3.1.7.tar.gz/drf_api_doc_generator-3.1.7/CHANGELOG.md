...
### Fixed
- **Deep Scraper & Flexible Regex**: Significantly improved the docstring parser with an ultra-flexible regex that captures fields in almost any format (quoted or unquoted, with colons or equals signs).
- **Advanced Debug Mode**: Added verbose terminal logging that highlights exactly which fields were discovered and where, ensuring developers can verify extraction in real-time.
- **Section Marker Expansion**: Added support for more docstring markers like `Params:` and `Payload:`.

## [3.1.6] - 2025-12-25

### Fixed
- **Ultra-Robust Docstring Parsing**: Rewrote the docstring parser to use marker-based splitting. It now reliably captures fields even if the docstring is complex, contains nested objects, or uses irregular formatting.
- **Enhanced Debug Visibility**: Added clearer terminal logging to show exactly which fields are being extracted from docstrings during the generation process.

## [3.1.5] - 2025-12-25

### Fixed
- **Docstring Parsing Priority**: Moved Docstring parsing to the highest priority. If fields are defined in the docstring, they will now correctly override or supplement any found serializers.
- **Robust Key Discovery**: Relaxed the docstring regex to handle unquoted keys (e.g., `key: value`) and improved extraction from nested JSON-like structures.

## [3.1.4] - 2025-12-25

### Fixed
- **Docstring Parser Enhancements**: Added support for parsing docstrings from both view classes and specific methods (like `post()`, `get()`). This fixes issues where parameters defined in method docstrings were not being captured.
- **Improved UI Presentation**: Cleaned up the endpoint description by removing raw `Body:` and `Response:` blocks, ensuring they only appear as organized tables and JSON examples in the UI.
