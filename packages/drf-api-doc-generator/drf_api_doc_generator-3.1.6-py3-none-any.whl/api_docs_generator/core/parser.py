...
class APIParser:
    ...
    def _parse_docstring_params(self, view_class: Type, callback=None) -> Tuple[List[FieldInfo], List[FieldInfo]]:
        ...
        Uses a robust marker-based splitting strategy to find sections.
        """
        req_fields = []
        res_fields = []
        ...
        try:
            ...
            markers = list(re.finditer(r'(?i)(Body|Request|Payload|Response|Returns|Result)\s*:', full_doc))
            
            if not markers:
                # Fallback: Just look for any "key": pattern in the whole doc if markers missing
                keys = set(re.findall(r"['\"]?(\w+)['\"]?\s*:", full_doc))
                if keys:
                     print(f"  [INFO] Found {len(keys)} potential keys in docstring for {view_class.__name__}")
                     for key in keys:
                        req_fields.append(FieldInfo(name=key, type='string', description='Auto-detected'))
                return req_fields, res_fields
            ...
            for i, match in enumerate(markers):
                ...
                # Extract keys with flexible regex (quotes optional)
                ...
                is_request = label in ['body', 'request', 'payload']
                target_list = req_fields if is_request else res_fields
                
                print(f"  [+] Docstring {'Request' if is_request else 'Response'} Fields for {view_class.__name__}: {list(keys)}")
