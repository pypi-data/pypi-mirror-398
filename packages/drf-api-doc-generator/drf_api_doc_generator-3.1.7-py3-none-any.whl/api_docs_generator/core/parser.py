...
class APIParser:
    ...
    def _parse_docstring_params(self, view_class: Type, callback=None) -> Tuple[List[FieldInfo], List[FieldInfo]]:
        ...
        Uses an ultra-flexible regex to find keys in any format.
        """
        req_fields = []
        res_fields = []
        
        # 1. Get cleanest docstrings from class and method
        cls_doc = inspect.getdoc(view_class) or ""
        cb_doc = ""
        if callback:
            cb_doc = inspect.getdoc(callback) or ""
        ...
        try:
            # 2. Find markers for Request/Response sections
            # Support Body, Request, Payload, Response, Returns, Result, Params
            markers = list(re.finditer(r'(?i)(Body|Request|Payload|Params|Response|Returns|Result)\s*:', full_doc))
            
            if not markers:
                # Fallback: Scrape ALL "key": or key: patterns from the whole doc
                # This is a "safety net" to ensure we don't miss anything
                keys = set(re.findall(r"[\"']?([\w_-]+)[\"']?\s*[:=]", full_doc))
                if keys:
                     print(f"  [DEBUG] Found {len(keys)} keys in docstring via Fallback Scraper for {view_class.__name__}")
                     for key in keys:
                        # Skip keywords common in docstrings
                        if key.lower() in ['post', 'get', 'put', 'patch', 'delete', 'http', 'api', 'body', 'response']:
                            continue
                        req_fields.append(FieldInfo(name=key, type='string', description='Auto-detected from docstring'))
                return req_fields, res_fields
            ...
            for i, match in enumerate(markers):
                ...
                start_pos = match.end()
                end_pos = markers[i+1].start() if i+1 < len(markers) else len(full_doc)
                
                block = full_doc[start_pos:end_pos]
                
                # Extract keys with ultra-flexible regex: matches "key":, 'key':, key:, key=
                keys = set(re.findall(r"[\"']?([\w_-]+)[\"']?\s*[:=]", block))
                
                if not keys:
                    continue
                    
                # Map to Request or Response
                is_request = label in ['body', 'request', 'payload', 'params']
                target_list = req_fields if is_request else res_fields
                
                print(f"  [FOUND] {len(keys)} {'Request' if is_request else 'Response'} fields in docstring of {view_class.__name__}: {list(keys)}")
                
                for key in keys:
                    target_list.append(FieldInfo(
                        name=key, type='string', required=False, description='Defined in docstring'
                    ))
                     
        except Exception as e:
            print(f"  [ERROR] Docstring parser failed for {view_class.__name__}: {e}")
