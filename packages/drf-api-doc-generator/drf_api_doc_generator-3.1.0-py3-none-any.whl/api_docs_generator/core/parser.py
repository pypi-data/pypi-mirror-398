...
class APIParser:
    ...
    def _get_serializer_fields(self, view_class: Type) -> tuple:
        ...
        if serializer_class is None:
            # Method 6: Docstring Parsing (The "Smart Read" method) 🌟
            # Check if fields are defined in the docstring (Body: {...})
            doc_request_fields, doc_response_fields = self._parse_docstring_params(view_class)
            ...
            # Method 7: Source Code Inspection (The "Sherlock Holmes" method)
    ...
    def _parse_docstring_params(self, view_class: Type) -> Tuple[List[FieldInfo], List[FieldInfo]]:
        """
        Parse view docstrings for 'Body: {...}' or 'Response: {...}' sections.
        This allows users to define params in docstrings when serializers are hidden.
        """
        req_fields = []
        res_fields = []
        ...
        try:
            ...
            # Look for Body: ... up to next double newline or section (curly brace block)
            # We match mostly { ... } content
            body_match = re.search(r'(?:Body|Request)\s*:\s*({[^}]+})', doc, re.DOTALL | re.IGNORECASE)
            if body_match:
                json_str = body_match.group(1)
                # Extract keys: "key" or 'key' followed by :
                keys = set(re.findall(r"['\"](\w+)['\"]\s*:", json_str))
                for key in keys:
            ...
            res_match = re.search(r'(?:Response|Returns)\s*:\s*({[^}]+})', doc, re.DOTALL | re.IGNORECASE)
            if res_match:
