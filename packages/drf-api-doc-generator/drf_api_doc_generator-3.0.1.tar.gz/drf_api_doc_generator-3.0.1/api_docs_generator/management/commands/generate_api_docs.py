...
class Command(BaseCommand):
    ...
    def _discover_consumer_files(self) -> list:
        """Auto-discover consumers.py files in all installed apps"""
        discovered_files = []
        for app_config in apps.get_app_configs():
            # Check for consumers.py in the app directory
            app_path = app_config.path
            consumers_path = os.path.join(app_path, 'consumers.py')
            if os.path.exists(consumers_path):
                discovered_files.append(consumers_path)
