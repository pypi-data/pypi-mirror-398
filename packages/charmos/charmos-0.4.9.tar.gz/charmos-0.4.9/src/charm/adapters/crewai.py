...
class CharmCrewAIAdapter(BaseAdapter):
    ...
    def invoke(self, inputs: Dict[str, Any], callbacks: List[Any] = None) -> Dict[str, Any]:
        ...
        if history_data:
            ...
            # [Fix] 安全地注入 Context，避免 KeyError
            if "topic" in native_input:
                native_input["topic"] += history_str
            elif "input" in native_input:
                native_input["input"] += history_str
            else:
                # 如果沒有 input/topic，我們創造一個 context 變數
                # CrewAI 會把這個 dict 傳給 template，如果 template 裡有 {context} 就能用到
                native_input["context"] = history_str
                # 為了保險，同時也設為 topic，因為很多預設範例用 topic
                if not native_input: 
                    native_input["topic"] = history_str

        # 2. 正規化 input (Proactive Mode)
        # 如果是空啟動，native_input 可能為空，CrewAI 通常能接受空 dict 啟動 (視 Task 定義而定)
