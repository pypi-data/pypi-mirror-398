...
def create_connection(target, token):
    ...
    headers = {"X-SVPS-TOKEN": encoded_token, "X-SVPS-USER": user}
...
def start_upload(target, token):
    ...
    try:
        ws = create_connection(target, token)
        print(f"[*] Starting upload: {local_path} -> {remote_path}")
        ...
        print(f"\n[+] Done. File saved to {remote_path}")
