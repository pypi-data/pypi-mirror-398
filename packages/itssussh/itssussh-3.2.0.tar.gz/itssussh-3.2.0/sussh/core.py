...
def create_connection(target, token):
    ...
    try:
        ...
        if e.status_code == 429:
            print("\n[!] Error: SERVER BUSY (Sesi aktif terdeteksi di server)")
        elif e.status_code == 403:
            print("\n[!] Error: ACCESS DENIED (Password Salah)")
        else:
            print(f"\n[!] Connection failed: Status {e.status_code}")
        sys.exit(1)
...
def start_shell(target, token):
    ...
    print(f"[*] Connected to {target} (Eternals 6.6-ULTRA)")
...
def main():
    parser = argparse.ArgumentParser(description="sussh - Eternals Ultra Nitro Client")
    ...
    if c == 'n':
        ...
        if input("Save profile? (y/n): ").lower() == 'y':
            save_profile(input("Profile Name: "), t, pw)
        start_shell(t, pw)
    elif c == 'u':
        t = input("Target Host: ")
