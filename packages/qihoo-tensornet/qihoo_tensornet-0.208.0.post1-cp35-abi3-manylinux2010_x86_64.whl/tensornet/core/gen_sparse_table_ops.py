...
Original C++ source file: core/kernels/sparse_table_ops_dummy.cc, core/ops/sparse_table_ops.cc
"""
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "SparseTablePull", name, resources, values, "table_handle",
        table_handle)
      return _result
      ...
      pass
    try:
      _result = _dispatcher_for_sparse_table_pull(
          (resources, values, table_handle, name,), None)
      ...
      _result = _dispatch.dispatch(
            sparse_table_pull, (), dict(resources=resources, values=values,
...
_dispatcher_for_sparse_table_pull = sparse_table_pull._tf_type_based_dispatcher.Dispatch
...
def sparse_table_push(values, grads, feature_labels, table_handle, name=None):
  ...
  Args:
    ...
    feature_labels: A list with the same length as `values` of `Tensor` objects with type `int64`.
    table_handle: An `int`.
    name: A name for the operation (optional).
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "SparseTablePush", name, values, grads, feature_labels,
      ...
      pass
    try:
      _result = _dispatcher_for_sparse_table_push(
          (values, grads, feature_labels, table_handle, name,), None)
      if _result is not NotImplemented:
        return _result
      return sparse_table_push_eager_fallback(
          values, grads, feature_labels, table_handle=table_handle, name=name,
          ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      _result = _dispatch.dispatch(
            sparse_table_push, (), dict(values=values, grads=grads,
  ...
  if len(grads) != _attr_N:
    raise ValueError(
        ...
        (len(grads), _attr_N))
  if not isinstance(feature_labels, (list, tuple)):
    raise TypeError(
        "Expected list for 'feature_labels' argument to "
        "'sparse_table_push' Op, not %r." % feature_labels)
  if len(feature_labels) != _attr_N:
    raise ValueError(
        "List argument 'feature_labels' to 'sparse_table_push' Op with length %d "
        "must match length %d of argument 'values'." %
        (len(feature_labels), _attr_N))
  table_handle = _execute.make_int(table_handle, "table_handle")
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
...
_dispatcher_for_sparse_table_push = sparse_table_push._tf_type_based_dispatcher.Dispatch
