...
class GPLM:
    ...
    def __call__(
        ...
        *,
        ...
        jacobian: bool = False,
        metric: bool = False,
        # --- NEW (optional): one geodesic integration step ---
        velocity: Optional[Union[np.ndarray, list]] = None,
        dt: float = 0.05,
        fd_h: float = 1e-3,
        reg: float = 1e-8,
        ...
        If `velocity` is provided (shape (a,d) or (d,)), performs ONE geodesic step
        and appends (Q_ax, v_ax) to the return:

          - no jacobian/metric:            (Y, Q, v)
          - jacobian only:                 (Y, J, Q, v)
          - metric only:                   (Y, g, Q, v)
          - jacobian and metric:           (Y, J, g, Q, v)
        """
        ...
        # We'll integrate in float64 for stability (FD + inverses)
        Z_all = np.ascontiguousarray(R_ax.astype(np.float64, copy=False))

        # Parse velocity (optional)
        if velocity is not None:
            v_all = np.asarray(velocity, dtype=np.float64)
            if v_all.ndim == 1:
                v_all = v_all[None, :]
            if v_all.shape != Z_all.shape:
                raise ValueError(f"velocity must have shape {tuple(Z_all.shape)}, got {tuple(v_all.shape)}")
        else:
            v_all = None

        bs = int(batch_size) if batch_size is not None else Z_all.shape[0]

        Ys = []
        Js = [] if jacobian else None
        Gs = [] if metric else None
        Qs = [] if v_all is not None else None
        Vs = [] if v_all is not None else None

        for s in range(0, Z_all.shape[0], bs):
            Zc = Z_all[s:s + bs]

            if v_all is not None:
                vc = v_all[s:s + bs]
                Qc, vc_new = self._geodesic_step(
                    Zc, vc,
                    dt=float(dt),
                    fd_h=float(fd_h),
                    reg=float(reg),
                    keep_speed=bool(keep_speed),
                )
                decode_in = Qc
            else:
                Qc, vc_new = None, None
            ...
            # Decode using your existing implementation (expects self.dtype)
            decode_in_t = np.ascontiguousarray(decode_in.astype(self.dtype, copy=False))
            chunk = self._decode(decode_in_t, jacobian=jacobian, metric=metric)
            ...
            if v_all is not None:
                Qs.append(Qc)      # type: ignore[union-attr]
                Vs.append(vc_new)  # type: ignore[union-attr]
        ...
        # Append (Q, v) if velocity mode
        if v_all is not None:
            Q = np.vstack(Qs)  # type: ignore[arg-type]
            V = np.vstack(Vs)  # type: ignore[arg-type]
            if (not jacobian) and (not metric):
                out = (out, Q, V)  # type: ignore[assignment]
            elif jacobian and (not metric):
                Y0, J0 = out  # type: ignore[misc]
                out = (Y0, J0, Q, V)
            elif (not jacobian) and metric:
                Y0, g0 = out  # type: ignore[misc]
                out = (Y0, g0, Q, V)
            else:
                Y0, J0, g0 = out  # type: ignore[misc]
                out = (Y0, J0, g0, Q, V)
        ...
        if single:
            if velocity is None:
                ...
                if jacobian and (not metric):
                    Y1, J1 = out  # type: ignore[misc]
                    return Y1[0], J1[0]
                if (not jacobian) and metric:
                    Y1, g1 = out  # type: ignore[misc]
                    return Y1[0], g1[0]
                Y1, J1, g1 = out  # type: ignore[misc]
                return Y1[0], J1[0], g1[0]
            else:
                if (not jacobian) and (not metric):
                    Y1, Q1, V1 = out  # type: ignore[misc]
                    return Y1[0], Q1[0], V1[0]
                if jacobian and (not metric):
                    Y1, J1, Q1, V1 = out  # type: ignore[misc]
                    return Y1[0], J1[0], Q1[0], V1[0]
                if (not jacobian) and metric:
                    Y1, g1, Q1, V1 = out  # type: ignore[misc]
                    return Y1[0], g1[0], Q1[0], V1[0]
                Y1, J1, g1, Q1, V1 = out  # type: ignore[misc]
                return Y1[0], J1[0], g1[0], Q1[0], V1[0]

        return out

    # -------------------------
    # NEW: Geodesic step utilities
    # -------------------------
    def _metric_only(self, Z_ax: np.ndarray) -> np.ndarray:
        """
        Return g(z)=J^T J for a batch (a,d) using the existing decoder.
        Note: _decode(metric=True) returns (Y, g); we discard Y.
        """
        Z_ax_t = np.ascontiguousarray(Z_ax.astype(self.dtype, copy=False))
        _, g = self._decode(Z_ax_t, jacobian=False, metric=True)
        return np.asarray(g, dtype=np.float64)

    def _christoffel_fd(self, Z_ax: np.ndarray, *, fd_h: float, reg: float) -> Tuple[np.ndarray, np.ndarray]:
        """
        Finite-difference Christoffel symbols Γ^k_{ij}(z).
        Returns:
          Gamma_akij: (a,d,d,d)
          g_aij:      (a,d,d)
        """
        Z_ax = np.asarray(Z_ax, dtype=np.float64)
        a, d = Z_ax.shape

        g = self._metric_only(Z_ax)  # (a,d,d)
        g_inv = np.linalg.inv(g + reg * np.eye(d)[None, :, :])  # (a,d,d)

        # dg[a,i,j,l] = ∂_i g_{j l}
        dg = np.zeros((a, d, d, d), dtype=np.float64)
        for i in range(d):
            Zp = Z_ax.copy(); Zp[:, i] += fd_h
            Zm = Z_ax.copy(); Zm[:, i] -= fd_h
            gp = self._metric_only(Zp)
            gm = self._metric_only(Zm)
            dg[:, i, :, :] = (gp - gm) / (2.0 * fd_h)

        # A[a,i,j,l] = ∂_i g_{j l} + ∂_j g_{i l} - ∂_l g_{i j}
        A = dg + dg.swapaxes(1, 2) - dg.transpose(0, 3, 1, 2)

        # Gamma[a,k,i,j] = 1/2 * sum_l g_inv[a,k,l] * A[a,i,j,l]
        Gamma = 0.5 * np.einsum("akl,aijl->akij", g_inv, A, optimize=True)
        return Gamma, g
    ...
    def _geodesic_step(
        ...
        *,
        ...
        fd_h: float,
        reg: float,
        keep_speed: bool ) -> Tuple[np.ndarray, np.ndarray]:
        """
        One step of geodesic integration on (z,v):
          z_dot = v
          v_dot^k = - Γ^k_{ij}(z) v^i v^j

        Uses symplectic Euler:
          v <- v + dt * a(z,v)
          z <- z + dt * v

        If keep_speed=True, renormalize v to keep v^T g v constant.
        """
        Z_ax = np.asarray(Z_ax, dtype=np.float64)
        v_ax = np.asarray(v_ax, dtype=np.float64)

        a, d = Z_ax.shape
        if v_ax.shape != (a, d):
            raise ValueError(f"v_ax must have shape {(a, d)}, got {v_ax.shape}")

        Gamma, g0 = self._christoffel_fd(Z_ax, fd_h=float(fd_h), reg=float(reg))

        if keep_speed:
            s0 = np.sqrt(np.maximum(np.einsum("ai,aij,aj->a", v_ax, g0, v_ax, optimize=True), 1e-16))
        else:
            s0 = None

        # a^k = -Γ^k_{ij} v^i v^j
        acc = -np.einsum("akij,ai,aj->ak", Gamma, v_ax, v_ax, optimize=True)  # (a,d)

        v_new = v_ax + float(dt) * acc
        z_new = Z_ax + float(dt) * v_new

        if keep_speed:
            g1 = self._metric_only(z_new)
