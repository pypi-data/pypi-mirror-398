...
LastEditTime : 2025-07-14 12:02:20 +0200
...
from analysis_tool.utils.utils_uproot import parse_input_paths, load_variables_to_pd_by_uproot, evaluate_expression_and_assign, evaluate_expression_and_add_column
...
class DatasetConfig:
    ...
    Attributes:
        input_files: ROOT file paths (semicolon-separated string or list)
        tree_name: Name of the TTree in the ROOT files
        weight_expr: Expression for event weights ('none'/'NONE' for unit weights)
        selection: Selection criteria (automatically wrapped in parentheses)
        label: Display name for plots and legends
    """

    input_files: str | list[str]
    tree_name: str
    label: str  # Display name for plots and legends
    weight_expr: str = None  # Expression for event weights or 'none'/'NONE', None for no weight (default)
    selection: str = None  # Selection criteria (must be in parentheses), None for no selection (default)

    # Internal storage (computed fields)
    data: Optional[pd.DataFrame] = field(default=None, repr=False)
    weights: Optional[np.ndarray] = field(default=None, repr=False)
    input_file_list: list[str] = field(init=False, repr=False)

    def __post_init__(self):
        """Validate and normalize dataset configuration."""
        self._parse_and_validate_files()
        self._normalize_weight_expression()
        self._format_selection()

    def _parse_and_validate_files(self) -> None:
        """Parse file paths and validate existence."""
        self.input_file_list = parse_input_paths(self.input_files)

        if missing_files := [f for f in self.input_file_list if not os.path.exists(f)]:
            raise FileNotFoundError(f"Files not found: {missing_files}")

    def _normalize_weight_expression(self) -> None:
        """Normalize weight expression to None for unit weights."""
        if self.weight_expr.upper() in {"NONE", "1", "ONE"}:
            self.weight_expr = None

    def _format_selection(self) -> None:
        """Ensure selection is properly formatted with parentheses."""
        if self.selection:
            if self.selection.upper().replace(' ', '') in {'NONE', '1>0', '(1>0)'}:
                self.selection = None
            elif not self.selection.startswith('(') or not self.selection.endswith(')'):
    ...
    def from_string(cls, dataset_str: str) -> 'DatasetConfig':
        ...
        elements_dict: dict = {
            "input_files": elements[0],
            "tree_name": elements[1],
            "weight_expr": elements[2],
            "selection": elements[3],
            "label": elements[4],
        }

        logger.info(f"DatasetConfig.from_string: {elements_dict}")
    ...
    for var_title, var_expr in cmp_vars.items():
        ...
        try:
            ...
            # Evaluate and store the new column temporarily
                ...
                new_columns_pd[var_title] = evaluate_expression_and_add_column(
...
def load_data_from_root(datasets: list[DatasetConfig], cmp_vars: list[dict]):
    for i_dataset, dset in enumerate(datasets):
        ...
        variables_pd = load_variables_to_pd_by_uproot(
            input_file_path=dset.input_file_list,
...
def draw_hists(
    ...
    """
    Draw histograms for multiple datasets on a single axis with optional custom title.

    This function creates overlaid histograms for comparison between datasets, with flexible
    title and x-axis labeling behavior based on whether a custom plot title is provided.

    Title/Label Logic:
    - If plot_title is None: var_title becomes the plot title, no x-axis label shown
    - If plot_title is provided: plot_title becomes the plot title, var_title becomes x-axis label

    Args:
        ax (plt.Axes): Matplotlib axes object to plot on
        datas (list[np.array]): List of data arrays, one per dataset
        weights (list[np.array]): List of weight arrays corresponding to each dataset
        labels (list[str]): List of legend labels for each dataset
        var_title (str): Variable name used for plot title (default) or x-axis label (when plot_title given)
        ranges: Range for histogram bins [min, max]
        hist_settings (dict): Histogram style settings (bins, density, alpha, etc.)
        yscale (str, optional): Y-axis scale ('linear', 'log', 'symlog', 'logit'). Defaults to 'linear'.
        plot_title (str, optional): Custom plot title. If None, var_title is used as title. Defaults to None.

    Returns:
        list[hist.hist.Hist]: List of histogram objects for potential further processing (e.g., pull plots)

    Note:
        - Each dataset gets a different color from the hist_colors palette
        - Filled histograms also get different hatch patterns
        - Legend positioning is automatically optimized
        - Font sizes are dynamically calculated based on figure size and content
    """
    ...
    # Add legend with dynamic font sizing
    ...
    title_fontsize = calculate_dynamic_fontsize(fig=ax.figure, num_entries=1, base_size=22, max_size=29)

    if not plot_title:
        # Default: var_title as plot title, no x-axis label
        ax.set_title(var_title, fontsize=title_fontsize)
        # Make xlabel empty (implicit - no ax.set_xlabel call)
    else:
        # Custom title mode: plot_title as title, var_title as x-axis label
        ax.set_title(plot_title, fontsize=title_fontsize)

        # Set x-axis label using var_title
        xlabel_fontsize = calculate_dynamic_fontsize(fig=None, num_entries=1)
        ax.set_xlabel(var_title, fontsize=xlabel_fontsize)

    # Set the y-axis label
    ylabel_fontsize = calculate_dynamic_fontsize(fig=None, num_entries=1)
...
def plot_individual_variable_comparison(
    ...
    Parameters:
    - var_title (str):          Variable name used as x-axis label.
    ...
    - plot_title (str):         Title of the plot. Default is None.
    Returns:
...
def collect_all_plots_in_one_canvas(
    ...
    dpi: int = 200,
    cols: int = None,
    check_plot_exists: bool = True,
) -> bool:
    """
    Collects individual plots and arranges them into a single canvas with optimized file size.
    ...
    Args:
        ...
        exts (List[str], optional): List of file extensions for the combined plot. Defaults to ["pdf", "png"].
        ...
        single_figsize (tuple, optional): Size of individual figures. Defaults to (10, 8).
        dpi (int, optional): DPI for PDF conversion and image quality. Lower = smaller file. Defaults to 200.
        cols (int, optional): Number of columns for the grid. Defaults to None, which means the number of columns is determined by the number of plots.
        check_plot_exists (bool, optional): Whether to check if the plot exists. Defaults to True.
    Returns:
    ...
    for i_var in tqdm(list(range(nfigs)), ascii=' >=', desc="Collecting plots", leave=False):
        ...
        if check_plot_exists and not Path(fig_path).exists():
            raise FileNotFoundError(f"Collect_all_plots_in_one_canvas The plot is not found: {fig_path}")
        ...
        try:
            if ext.lower() == '.png':
                ...
                try:
                    ...
                    images = convert_from_path(fig_path, dpi=dpi, first_page=1, last_page=1)
                    if images:
                        img = images[0]
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Optional plot title. If not provided, no title will be shown.',
    )
