...
def create_engine_enhanced(debug: bool = False) -> NodeEngine:
    ...
    1. StuckNode: escape when all blocked (highest priority)
    2. WallNode: react to danger (front blocked or side friction)
    3. ForwardNode: default forward

    NOTE: IntersectionNode disabled - causes spinning in open areas.
    """
    engine = NodeEngine(debug=debug)
    engine.add_node(StuckNode())         # Priority 0 - escape from corners
    # engine.add_node(IntersectionNode())  # DISABLED - causes 360° spinning
    engine.add_node(WallNode())          # Priority 1 - danger reaction
    engine.add_node(ForwardNode())       # Priority 2 - default forward
    return engine
