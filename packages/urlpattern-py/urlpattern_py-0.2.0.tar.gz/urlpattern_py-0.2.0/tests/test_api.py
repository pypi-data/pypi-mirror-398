...
GENERATE_DATA = _load_json("urlpattern-generate-test-data.json")


@pytest.mark.parametrize("entry", COMPARE_DATA)
def test_compare_component(entry):
...
@pytest.mark.parametrize("entry", GENERATE_DATA)
def test_generate(entry):
    ...
    assert pattern.generate(entry["component"], entry["groups"]) == entry["expected"]
