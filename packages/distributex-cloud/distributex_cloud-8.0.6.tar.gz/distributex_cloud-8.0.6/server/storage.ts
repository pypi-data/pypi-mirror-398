// server/storage.ts - WITH WORKER ASSIGNMENT SUPPORT
import { db } from "./db";
import { users, workers, jobs, type User, type Worker, type Job } from "@shared/schema";
import { eq, desc, and, isNull, or, sql, lt, gt } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  // NEW: Get available workers for job assignment
    ...
    const availableWorkers = await db.select()
      ...
      .where(
        and(
          ...
          gt(workers.lastHeartbeat, threeMinutesAgo)
        )
      )
      .orderBy(desc(workers.lastHeartbeat))
      .limit(10);
  ...
  // IMPROVED: Job Status Update with Worker Assignment
        ...
        if (status === 'completed') {
          await db.execute(sql`
            ...
            SET jobs_completed = jobs_completed + 1,
                total_compute_time_seconds = total_compute_time_seconds + ${updateData.durationSeconds || 0}
            WHERE id = ${workerId}
          `);
        } else {
          await db.execute(sql`
