...
class IdGeneratorFactory:
    """Factory for managing multiple IdGenerator instances"""
    ...
    def __init__(
        ...
        """
        Initialize the IdGeneratorFactory.

        Args:
            segment_client: SegmentClient instance for allocating segments
            system_code: System code
            db_name: Database name
            segment_count: Default number of IDs to allocate per segment (default: 10000)
        """
        self.segment_client = segment_client
        self.system_code = system_code
        self.db_name = db_name
    ...
    def get_generator(
        ...
        """
        Get or create an IdGenerator for the specified table and field.

        Args:
            table_name: Table name
            field_name: Field name (default: "id")
            segment_count: Number of IDs to allocate per segment (optional, uses factory default if not specified)

        Returns:
            IdGenerator: The ID generator instance for this table/field combination
        """
        ...
        if key not in self._generators:
            with self._lock:
                if key not in self._generators:
                    self._generators[key] = IdGenerator(
                        segment_client=self.segment_client,
                        system_code=self.system_code,
                        db_name=self.db_name,
                        table_name=table_name,
                        field_name=field_name,
                        segment_count=segment_count or self.segment_count
                    )

        return self._generators[key]


class AsyncIdGeneratorFactory:
    """Factory for managing multiple AsyncIdGenerator instances"""
    ...
    def __init__(
        ...
        """
        Initialize the AsyncIdGeneratorFactory.
        ...
        self.db_name = db_name
        self.segment_count = segment_count
        self._generators: Dict[str, AsyncIdGenerator] = {}
        self._lock = asyncio.Lock()
    ...
    async def get_generator(
        ...
        """
        Get or create an AsyncIdGenerator for the specified table and field.
        ...
        Returns:
            AsyncIdGenerator: The async ID generator instance for this table/field combination
        """
        key = f"{table_name}.{field_name}"

        # Use lock to ensure thread-safety when creating generators
        async with self._lock:
            if key not in self._generators:
                self._generators[key] = AsyncIdGenerator(
                    segment_client=self.segment_client,
                    system_code=self.system_code,
                    db_name=self.db_name,
                    table_name=table_name,
