...
async def test_fetch_url_by_browser_errors(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO):
        await fetch_url_by_browser(
            mock_context_lookup, test_url_lookup, config_lookup
        )
...
def test_main_success(
    mock_build_parser, mock_setup_logging, mock_load_or_create_config, mock_run
):
    mock_parser = MagicMock()
    mock_parser.parse_args.return_value = argparse.Namespace(
        url="http://test.com", debug=False
    )
...
@patch("xsget.xsget.run")
@patch(
    "xsget.xsget.load_or_create_config", side_effect=Exception("Config Error")
)
@patch("xsget.xsget.setup_logging")
@patch("xsget.xsget.build_parser")
...
@patch("xsget.xsget.run")
@patch(
    "xsget.xsget.load_or_create_config", side_effect=Exception("Debug Error")
)
@patch("xsget.xsget.setup_logging")
@patch("xsget.xsget.build_parser")
...
def test_main_async_logic_range_invalid(mock_filter_urls, mock_fetch_urls):
    ...
    with pytest.raises(
        RuntimeError, match="invalid url range, start: 3, end: 1"
    ):
        asyncio.run(_main_async_logic(config))
...
def test_main_async_logic_single_url_crawl(
    mock_filter_urls, mock_fetch_urls, tmp_path, caplog
):
