...
ase_calc_args = pdict.get("calc_args", {}).get("ase", {})
...
### DFTD3
dftd3_args = ase_calc_args.get("dftd3", {})
if dftd3_args:
    ...
    calc = calc_ase  # calc defined in the exec() above
...
if ase_calc_args.get("exclude_result_dftd3", False) and dftd3_args:
    struct = atoms.copy()
    struct.calc = SinglePointCalculator(struct)
    struct.calc.results = calc_ase.results  # only ASE-calculator results (no DFTD3)
    dyn.attach(print_dynamic, interval=traj_freq, atoms=struct)
    dyn.attach(write_dyn_extxyz, interval=traj_freq, atoms=struct)
else:
    dyn.attach(print_dynamic, interval=traj_freq, atoms=atoms)
    dyn.attach(write_dyn_extxyz, interval=traj_freq, atoms=atoms)
