...
def load_data(model_name, data_name, Paras):
    ...
    # ---------------------------------------------------------
    if data_name == "MNIST":
        train_dataset, test_dataset, transform = DataHub.MNIST(binary=Paras["binary"])

    elif data_name == "QMNIST":
        train_dataset, test_dataset, transform = DataHub.QMNIST(binary=Paras["binary"])

    elif data_name == "CIFAR10":
        train_dataset, test_dataset, transform = DataHub.CIFAR10(binary=Paras["binary"])
    
    elif data_name == "CIFAR100":
        train_dataset, test_dataset, transform = DataHub.CIFAR100(binary=Paras["binary"])

    elif data_name == "SVHN":
        train_dataset, test_dataset, transform = DataHub.SVHN(binary=Paras["binary"])

    elif data_name == "Flowers102":
        train_dataset, test_dataset, transform = DataHub.Flowers102(binary=Paras["binary"])
    
    elif data_name == "OxfordPet":
        train_dataset, test_dataset, transform = DataHub.OxfordPet(binary=Paras["binary"])
        ...
        train_dataset, test_dataset, transform = DataHub.Caltech101_Resize_32(Paras["model_type"][model_name])
