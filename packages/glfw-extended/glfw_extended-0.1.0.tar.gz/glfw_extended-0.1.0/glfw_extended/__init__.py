...
This package extends the standard pyGLFW bindings to support TheCherno's GLFW fork,
which adds:
- TITLEBAR window hint for custom titlebar rendering
- set_titlebar_hit_test_callback() for handling titlebar hit testing

Based on pyGLFW by Florian Rhiem.
"""
...
__author__ = 'Izaiah Meyer'
__copyright__ = 'Copyright (c) 2025 Izaiah Meyer'
__license__ = 'MIT'
__version__ = '0.1.0'
...
# Added to support TheCherno's GLFW fork
TITLEBAR = 0xc2000d
...
_GLFWtitlebarhittestfun = ctypes.CFUNCTYPE(None,
                                           ctypes.c_void_p,
                                           ctypes.c_int,
                                           ctypes.c_int,
...
def set_window_pos_callback(window, cbfun):
    ...
    :param window:
    :param cbfun: params: (window: _GLFWwindow, xpos: int, ypos: int)
    """
    window_addr = ctypes.cast(ctypes.pointer(window),
                              ctypes.POINTER(ctypes.c_long)).contents.value
...
def set_window_size_callback(window, cbfun):
    ...
    :param window:
    :param cbfun: params: (window: _GLFWwindow, width: int, height: int)
    """
    window_addr = ctypes.cast(ctypes.pointer(window),
                              ctypes.POINTER(ctypes.c_long)).contents.value
...
def set_window_close_callback(window, cbfun):
    ...
    :param window:
    :param cbfun: params: (window: _GLFWwindow)
    """
    window_addr = ctypes.cast(ctypes.pointer(window),
                              ctypes.POINTER(ctypes.c_long)).contents.value
...
def set_window_focus_callback(window, cbfun):
    ...
    :param window:
    :param cbfun: params: (window: _GLFWwindow, focused: int)
    """
    window_addr = ctypes.cast(ctypes.pointer(window),
                              ctypes.POINTER(ctypes.c_long)).contents.value
...
def set_window_iconify_callback(window, cbfun):
    ...
    :param window:
    :param cbfun: params: (window: _GLFWwindow, iconified: int)
    """
    window_addr = ctypes.cast(ctypes.pointer(window),
                              ctypes.POINTER(ctypes.c_long)).contents.value
...
if hasattr(_glfw, 'glfwSetWindowMaximizeCallback'):
    ...
    def set_window_maximize_callback(window, cbfun):
        ...
        :param window:
        :param cbfun: params: (window: _GLFWwindow, maximized: int)
        """
        window_addr = ctypes.cast(ctypes.pointer(window),
                                  ctypes.POINTER(ctypes.c_long)).contents.value
...
if hasattr(_glfw, 'glfwSetWindowContentScaleCallback'):
    ...
    def set_window_content_scale_callback(window, cbfun):
        ...
        :param window:
        :param cbfun: params: (window: _GLFWwindow, xscale: float, yscale: float)
        """
        window_addr = ctypes.cast(ctypes.pointer(window),
                                  ctypes.POINTER(ctypes.c_long)).contents.value
...
# Added to support TheCherno's GLFW fork
if hasattr(_glfw, 'glfwSetTitlebarHitTestCallback'):
    _titlebar_hit_test_callback_repository = {}
    _callback_repositories.append(_titlebar_hit_test_callback_repository)
    _glfw.glfwSetTitlebarHitTestCallback.restype = _GLFWtitlebarhittestfun
    _glfw.glfwSetTitlebarHitTestCallback.argtypes = [
        ctypes.POINTER(_GLFWwindow),
        _GLFWtitlebarhittestfun]


    def set_titlebar_hit_test_callback(window, cbfun):
        """
        Sets the titlebar hit test callback for the specified window.

        Wrapper for:
            GLFWtitlebarhittestfun glfwSetTitlebarHitTestCallback(GLFWwindow* window, GLFWtitlebarhittestfun cbfun);

        :param window:
        :param cbfun: params: (window: _GLFWwindow, x: int, y: int, hit: ctypes.POINTER(ctypes.c_int))
        """
        window_addr = ctypes.cast(ctypes.pointer(window),
                                  ctypes.POINTER(ctypes.c_long)).contents.value
        if window_addr in _titlebar_hit_test_callback_repository:
            previous_callback = _titlebar_hit_test_callback_repository[
                window_addr]
        else:
            previous_callback = None
        if cbfun is None:
            cbfun = 0
        c_cbfun = _GLFWtitlebarhittestfun(cbfun)
        _titlebar_hit_test_callback_repository[window_addr] = (cbfun, c_cbfun)
        cbfun = c_cbfun
        _glfw.glfwSetTitlebarHitTestCallback(window, cbfun)
        if previous_callback is not None and previous_callback[0] != 0:
            return previous_callback[0]
    ...
    def set_titlebar_hit_test_callback(window, cbfun):
        ...
        NOTE: This function requires TheCherno's GLFW fork.
        """
        raise RuntimeError(
            "set_titlebar_hit_test_callback requires TheCherno's GLFW fork. "
            "The custom glfw3.dll was not loaded. "
            "This package only supports Windows."
        )
