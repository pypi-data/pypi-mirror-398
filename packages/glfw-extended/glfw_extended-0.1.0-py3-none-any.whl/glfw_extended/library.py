...
if os.environ.get('PYGLFW_LIBRARY', ''):
    ...
    # try package directory FIRST (this is where our custom DLL lives)
    try:
        _package_dir = os.path.abspath(os.path.dirname(__file__))
        # load Microsoft Visual C++ 2013 runtime
        msvcr = ctypes.CDLL(os.path.join(_package_dir, 'msvcr120.dll'))
        _glfw_library_path = os.path.join(_package_dir, 'glfw3.dll')
        glfw = ctypes.CDLL(_glfw_library_path)
        # Set PYGLFW_LIBRARY so regular `import glfw` uses the same custom DLL
        os.environ['PYGLFW_LIBRARY'] = _glfw_library_path
    except OSError:
        pass

    # fallback: try Windows default search path
    if glfw is None:
        try:
    ...
    # fallback: try conda's default location on Windows
    if glfw is None:
        try:
