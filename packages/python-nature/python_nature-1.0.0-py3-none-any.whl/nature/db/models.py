...
class SpeciesState(NatureBase):
    ...
    # Fitness values
    fitness_values: Mapped[list[float]] = mapped_column(
    ...
    @classmethod
    def save(
        cls, species: Species, session: Session, tags: Sequence[str] | None = None
    ) -> UUID:
    ...
    def fetch_by_id(
        ...
        with sync_session(session, autocommit=False) as db:
            entity = db.scalar(
                select(cls).options(selectinload(cls.pickle)).filter(cls.id == id)
            )
    ...
    @classmethod
    async def fetch_by_id_async(
        cls, id: UUID, session: AsyncSession | None = None
    ) -> Self | None:
