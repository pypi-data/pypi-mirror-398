...
- Aggregation (Mean, WMean, DfMean, SWMean, Reduce, Gather, IndexSequence)
...
#
# Some specific notes:
# - KalmanSmoothV2 uses `self.anal.kalman()` which is from nature.ats.analysis.analyzer.Analyzer
# - If you need standalone Kalman filtering, consider using pykalman or filterpy libraries
# - Most other self.anal methods are general pandas operations (rolling_mean, etc.)


# === Series and DataFrame Codon Classes ===


class AbsDiffRollingMedianVolatility(Codon):
    """Volatility as rolling median of absolute differences."""

    def __call__(self, x: Series, win: int) -> Series:
        return x.diff().abs().rolling(win).median()




class ApplyLowerThreshold(Codon):
    """Apply lower threshold to create binary signal."""

    def __call__(self, x: Series, thresh: float) -> Series:
...
class ApplyRecencyFilter(Codon):
    """Filter events by recent frequency."""

    def __call__(self, events: Series, window: int, thresh: float) -> Any:
        return self.anal.gt(
            self.anal.rolling_mean(self.anal.rolling_sum(events, window), max(1, window)),
            thresh,
        )
...
class ApplyUpperThreshold(Codon):
    """Apply upper threshold to create binary signal."""
...
class ComparativeResidual(Codon):
    """Residual relative to reference series."""

    def __call__(self, x: Series, x_ref: Series, win_smooth: int) -> Series:
        if win_smooth > 1:
            x_ref = x_ref.rolling(win_smooth, min_periods=1).mean()
...
class DynRoll(Codon):
    ...
    def __call__(self, x: Series, wins: Series) -> Series:
        return self.anal.dyn_rolling(x, wins, self.name)
...
class EwmDiffAbsVolatility(Codon):
    """Volatility as EWM of absolute differences."""

    def __call__(self, x: Series, win: int) -> Series:
        return x.diff().abs().ewm(win).mean()




class Gather(Codon):
    """Gather multiple elements into sequence."""

    def __init__(
        self,
        n: int,
        skip_none=False,
        element_type: type | str | tuple[str | type, ...] = object,
        seq_type: type = list,
        **params,
    ) -> None:
        super().__init__(
            n_elements=n,
            seq_type=str(seq_type.__name__),
            **params,
        )
        self.arg_types = [tuple(flatten(element_type))] * max(1, n)
        self.arg_names = [f"x{i+1}" for i in range(max(1, n))]
        self.seq_type = seq_type
        self.skip_none = skip_none

    def __call__(self, *elements: Any) -> list:
        if getattr(self, "skip_none", False):
            return self.seq_type([e for e in elements if e is not None])
        else:
            return self.seq_type(elements)
...
class IndexSequence(Codon):
    """Index into sequence."""

    def __init__(self, index: int, **kwargs):
        super().__init__(params={"index": index}, **kwargs)
        self.index = index

    def __call__(self, seq: Sequence) -> Any:
...
class KalmanSmoothV2(Codon):
    ...
    NOTE: This codon requires self.anal.kalman() from nature.ats.analysis.analyzer.Analyzer.
    The Analyzer class provides optimized Kalman filtering functionality.
    For standalone usage, consider implementing with pykalman or filterpy libraries.
    """

    def __call__(self, x: Series, win: int, trans_mult: float) -> Series:
        return self.anal.kalman(x, win, obs_mult=1, trans_mult=trans_mult, smooth=False)
...
class LowerDynBband(Codon):
    """Lower Bollinger Band with dynamic windows."""
...
class MTFMaCrossUpEvents(Codon):
    """Multi-timeframe MA cross-up events with trend filter."""
    ...
    def __call__(
        ...
        slow_window: int,
        trend_period: int,
        trend_thresh_pct: float,
    ) -> Series:
        fast_ma = self.anal.rolling_mean(price, fast_window)
        slow_ma = self.anal.rolling_mean(price, slow_window)
        trend_pct_change = self.anal.pct_change(price, trend_period)
        return (
            self.anal.gt(trend_pct_change, trend_thresh_pct)
            & (self.anal.crossed_up(fast_ma, slow_ma))
        ).astype(float)




class MTFMomentumSignChangeEvents(Codon):
    """Multi-timeframe momentum sign change events."""
    ...
    def __call__(
        ...
        v_fast = self.anal.diff(price, window)
        v_slow = self.anal.diff(price, slow_window)
        return (
            self.anal.gt(v_slow, slow_thresh) & (self.anal.sign_flipped(v_fast, positive=True))
        ).astype(float)




class MTFPastHighCrossUpEvents(Codon):
    """Cross-up above past high."""
    ...
    def __call__(
        ...
        h_past = self.anal.shift(h, period)
        return (self.anal.crossed_up(c, h_past)).astype(float)
...
class MedianDeviationVolatility(Codon):
    """Volatility as median absolute deviation from rolling median."""

    def __call__(self, x: Series, win: int) -> Series:
        return (x - x.rolling(win).median()).abs().rolling(win).median()
...
class NDiff(Codon):
    ...
    def __call__(self, x: Series, *windows: int) -> Series:
        y = self.anal.diff(x, window=windows[0])
        for win in windows[1:]:
            y = self.anal.diff(y, win)
        return y
...
class NPctChange(Codon):
    ...
    def __call__(self, x: Series, *windows: int) -> Series:
        y = self.anal.pct_change(x, window=windows[0])
        for win in windows[1:]:
            y = self.anal.pct_change(y, win)
        return y
...
class PctChangeRelativeToMean(Codon):
    """Percent change relative to moving average."""

    def __call__(self, x: Series, win: int, exp: bool) -> Series:
        ma = x.rolling(win).mean()
        return (x - ma) / ma
...
class Reduce(Codon):
    """Reduce operation over sequence."""

    def __init__(
        self,
        reducer_type: type | tuple[type, ...],
        seq_type: type | tuple[type, ...] = list,
        **params,
    ) -> None:
        super().__init__(
            arg_types={"reducer": reducer_type, "args": seq_type},
            seq_type=str(seq_type),
            red_type=str(reducer_type),
            **params,
        )

    def __call__(self, reducer: Callable, args: Sequence) -> Any:
...
class RollVol(Codon):
    """Rolling volatility using percent change."""

    def __call__(self, x: Series, win_pct_change: int, win_std: int) -> Series:
        return self.anal.pct_change(x, win_pct_change).rolling(win_std).std()
...
class SLogicGate(Codon):
    """Base class for Series logic operations."""
    ...
    def __init__(
        ...
        **params,
        ...
        super().__init__(
            arg_types={"a": series_type, "b": series_type} if series_type else None,
            **params,
        )

    @property
    def func_name(self) -> str:
        return "series_logic_gate"
...
class SWMean(Codon):
    """Weighted mean of Series."""

    def __init__(
        self,
        n: int,
        weight_type: type | str | tuple[str | type, ...] = float,
        term_type: type | str | tuple[type | str, ...] = Series,
        **kwargs,
    ) -> None:
        super().__init__(
            n_factors=n,
        ...
        self.arg_names = []
        self.arg_names.extend(f"w{i+1}" for i in range(n))
        self.arg_types.extend(tuple(flatten(weight_type)) for i in range(n))
        self.arg_names.extend(f"x{i+1}" for i in range(n))
        self.arg_types.extend(tuple(flatten(term_type)) for i in range(n))
        self.n_factors = n

    def __call__(self, *factors: Any) -> Any:
        weights = factors[: self.n_factors]
        values = factors[self.n_factors :]
        return self.anal.weighted_average_columns(weights, values)

    @property
    def func_name(self):
        return f"series_wmean_{self.n_factors}"
...
class WMean(Codon):
    """Weighted mean of scalar values."""

    def __init__(
        self,
        n: int,
        weight_type: type | str | tuple[str | type, ...] = float,
        value_type: type | str | tuple[str | type, ...] = float,
        **params,
    ) -> None:
        super().__init__(**params)
        arg_types = []
        ...
        for i in range(n or self.arity):
            arg_names.append(f"w{i+1}")
            arg_types.append(tuple(flatten(weight_type)))
            arg_names.append(f"x{i+1}")
            arg_types.append(tuple(flatten(value_type)))
    ...
    def __call__(self, *args: Any) -> Any:
        return self.anal.weighted_average_columns(args[0::2], args[1::2])
