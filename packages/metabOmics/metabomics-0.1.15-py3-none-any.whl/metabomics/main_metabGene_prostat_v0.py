...
if __name__ == "__main__":
    ...
    # Read data
    X_breast, y_breast = SkUtilsIO("../omicNetwork/Databases/metabData_prostat.csv").from_csv(label_column='Factors')
    X_tr, y_tr = SkUtilsIO("../omicNetwork/Databases/geneData_prostat.csv").from_csv(label_column='Factors')

    name_mapping_transformer = FeatureRenaming(load_metabolite_mapping("new-synonym", X_breast, "Prostat_Cancer" ))
    ...
    dosya_adi = "prostat_react_error_v0.txt"
    count = 1
    ...
    with open(dosya_adi, 'w') as json_file:
        ...
        for train_index, test_index in skf.split(X_breast, y_breast):
            try :
                ...
                react_diff_score = "prostat_react_v0_diff_score_fold_" + str(count) + ".txt"
                with open(react_diff_score, 'w') as react_diff_file:
                    react_diff_file.write(f'\nTrain diff score: {X_train_diff}\n')
                    react_diff_file.write(f'\nTest diff score: {X_test_diff}\n')
                    react_diff_file.write(f'\ny_train: {y_train}\n')
                    react_diff_file.write(f'\ny_test: {y_test}\n')
                ...
                break
    with open('scores_react_prostat_v0.txt', 'w') as f:
        try:
