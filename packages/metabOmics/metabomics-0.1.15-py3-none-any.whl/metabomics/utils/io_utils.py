...
DATASET_PATH = os.path.join(CURRENT_DIR, '..', 'datasets', 'network_models')
...
def load_network_model(model):
    ...
    if type(model) == str:
        if model in ['ecoli', 'textbook', 'salmonella']:
            ...
            path = os.path.join(DATASET_PATH, 'recon2.json')
            return cb.io.load_json_model(path)
        elif model == 'recon3D':
            path = os.path.join(DATASET_PATH, 'Recon3D.json')
            return cb.io.load_json_model(path)
        elif model == 'recon301':
            path = os.path.join(DATASET_PATH, 'Recon3DModel_301.mat')
            return cb.io.load_matlab_model(path)
