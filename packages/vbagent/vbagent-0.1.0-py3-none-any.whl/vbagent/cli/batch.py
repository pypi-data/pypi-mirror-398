...
class CaffeinateManager:
    """Manage caffeinate process to prevent sleep during batch processing."""
    ...
    def start(self):
        ...
        if sys.platform == "darwin":  # macOS
            try:
                ...
                self._get_console().print("[dim]☕ Caffeinate started - system will stay awake[/dim]")
    ...
    def stop(self):
        ...
        if self.process:
            ...
            try:
                ...
                self.process.kill()
            self.process = None
            self._get_console().print("[dim]☕ Caffeinate stopped[/dim]")
...
def _run_batch(
    ...
    with CaffeinateManager(console=console):
