...
class Bot:
    ...
    def run(self, id: str) -> None:
        ...
        credentials = self.credential_repository.get_credentials(id)

        strategy.executor.set_credentials(credentials)
        data = strategy.data_pipeline.run()
        strategy.run(data)

        strategy.executor.clear_credentials()
