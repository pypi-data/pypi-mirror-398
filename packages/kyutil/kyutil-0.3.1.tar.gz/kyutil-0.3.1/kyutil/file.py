...
from kyutil.excel import excel_to_html
...
def save_ijo_file_html(path_save):
    """将集成单转为html"""
    html = excel_to_html(path_save)
    html = f"""
    <html><head><style>
            table {{
    ...
    open(path_save + ".html", "w", encoding="utf-8").write(html)


def send_chunk(fp):
    """流式读取"""
    with open(fp, 'rb') as target_file:
        while True:
