...
def is_related_tag(iso, tags, koji_ins):
    """
    查看koji上的tag是否与当前iso有关
    Args:
        iso: ISO对象
        tags: koji上本软件包的tag
        koji_ins: koji实例
    ...
    # 取继承Tag
    inherit_tags = koji_ins.get_inherit_tag(iso.tag)
    inherit_tags.append(iso.tag)
    r.extend(list(set(inherit_tags) & set(tags)))

    # 取产品相关Tag
    product_tags = iso.product_series_info.related_tags
    if product_tags:
        r.extend(list(set(tags) & set(product_tags.split(','))))

    return list(set(r))


def is_into_base_repo(rpms, rim_repo):
    """
    判断软件包是否集成进Base仓库。通过获取仓库的源码包列表，然后比对
    Args:
        rpms:
        rim_repo:
    ...
    """
    rpms_repo = []
    for ri in rim_repo.rpm_info_list:
        rpms_repo.append(ri.fullname)
    rpms_pkg = []
    for arch, rpms_of_arch in rpms.items():
        rpms_pkg.extend(rpms_of_arch)
    return len(set(rpms_pkg) & set(rpms_repo))
