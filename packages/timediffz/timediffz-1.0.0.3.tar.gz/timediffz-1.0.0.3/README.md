...
WITH THIS COMMAND:
```batch
python (full setupfractionals.py file path, found in the library files) build_ext --inplace
```
or the same but with python as python3:
```batch
python3 (full setupfractionals.py file path, found in the library files) build_ext --inplace
```
