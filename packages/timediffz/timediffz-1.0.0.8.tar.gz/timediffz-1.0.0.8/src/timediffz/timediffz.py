...
except Exception: warnings.warn(f"timediffz 1.0.0.6 (fractions mode) failed to load correctly :(using any fractions mode related function WOULD give an error and fall because of this, for fix do pip install td1inner")
try: import td2inner as td2
except Exception: warnings.warn(f"timediffz 1.0.0.6 (fractionals mode) failed to load correctly :(using any fractionals mode related function WOULD give an error and fall because of this, to fix either/both do pip install td2inner and you need to setup fractionals via this command cd (full path of folder where setupfractionals.py is in, found in the library files) & python(3 or nothing) setupfractionals.py build_ext --inplace')")
