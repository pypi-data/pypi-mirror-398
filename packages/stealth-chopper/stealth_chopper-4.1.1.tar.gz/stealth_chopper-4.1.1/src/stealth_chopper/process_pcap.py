...
def process_pcap(pcap_file, filter_ip=None, filter_url=None, filter_country=None, filter_time=None,
    ...
    # Set the correct path for the TLD file (hardcoded path based on your structure)
    tld_file_path = 'myvenv/lib/python3.13/site-packages/stealth_chopper/assets/tld.txt'

    # Ensure the TLD file exists at the specified path
    if not os.path.isfile(tld_file_path):
        print(Fore.RED + f"[!] TLD file not found at: {tld_file_path}")
        sys.exit(1)
    
    # Load the TLD mapping from the correct path
