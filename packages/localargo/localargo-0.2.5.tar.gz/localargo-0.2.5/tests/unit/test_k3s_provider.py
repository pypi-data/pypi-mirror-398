...
class TestK3sProviderBasics:
    """Basic tests for K3sProvider."""
    ...
    def test_init_creates_temp_kubeconfig(self) -> None:
...
class TestK3sProviderAvailability:
    """Test K3sProvider availability checks."""

    def test_is_available_with_k3s_present(self) -> None:
        """Test is_available returns True when k3s is found and works."""
        provider = K3sProvider(name="test")
        assert provider.is_available() is True

    def test_is_available_with_k3s_not_found(self) -> None:
        """Test is_available returns False when k3s is not found."""
        with patch("shutil.which", return_value=None):
            provider = K3sProvider(name="test")
            assert provider.is_available() is False

    def test_is_available_with_k3s_command_failure(self, mock_subprocess_run: MagicMock) -> None:
...
class TestK3sProviderCreateCluster:
    """Test K3sProvider cluster creation."""

    def test_create_cluster_success(self, mock_subprocess_popen: MagicMock) -> None:
...
class TestK3sProviderDeleteCluster:
    """Test K3sProvider cluster deletion."""

    def test_delete_cluster_raises_operation_error(self) -> None:
    ...
    def test_delete_cluster_with_custom_name_raises_operation_error(self) -> None:
...
class TestK3sProviderGetClusterStatus:
    """Test K3sProvider cluster status retrieval."""

    def test_get_cluster_status_with_kubeconfig_exists_and_ready(self, mock_subprocess_run: MagicMock) -> None:
        ...
        with patch("pathlib.Path.exists", return_value=True):
            ...
            # Verify kubectl command was called (using assert_command_contains for flexibility)
            assert mock_subprocess_run.call_count >= 1

    def test_get_cluster_status_with_kubeconfig_not_exists(self, mock_subprocess_run: MagicMock) -> None:
    ...
    def test_get_cluster_status_with_kubeconfig_exists_but_not_ready(self) -> None:
    ...
    def test_get_cluster_status_explicit_patch(self) -> None:
        ...
        with (
            ...
            # Verify kubectl command was called with expected arguments
            mock_run.assert_called_once()
            call_args = mock_run.call_args
            # Check key command parts (path-agnostic)
            assert_command_contains(call_args, ["kubectl", "--kubeconfig", "cluster-info"])
            # Verify kubeconfig path is in the command
            cmd = call_args[0][0]
            assert any("test-kubeconfig.yaml" in str(arg) for arg in cmd)
