...
class TestStepStatus:
    ...
    def test_step_status_creation(self) -> None:
    ...
    def test_step_status_with_details(self) -> None:
    ...
    def test_step_status_is_completed(self) -> None:
...
class TestExecutionStep:
    ...
    def test_step_creation(self) -> None:
        ...
        def dummy_check(_manifest: UpManifest, _client: ArgoClient | None) -> StepStatus:
            return StepStatus(state="completed", reason="OK")

        def dummy_execute(_manifest: UpManifest, _client: ArgoClient | None) -> None:
            pass
    ...
    def test_step_check_execution(self) -> None:
        """Test step check execution."""
        check_result = StepStatus(state="completed", reason="Mock check")

        def mock_check(_manifest: UpManifest, _client: ArgoClient | None) -> StepStatus:
            return check_result
    ...
    def test_step_execute_execution(self) -> None:
        ...
        def mock_execute(_manifest: UpManifest, _client: ArgoClient | None) -> None:
...
class TestExecutionEngine:
    ...
    def test_engine_creation(self) -> None:
        ...
        assert len(engine.steps) == 2  # noqa: PLR2004
        assert not engine.results

    def test_execute_with_all_completed_steps(self) -> None:
        """Test execution when all steps are already completed."""
        executed_steps = []

        def make_completed_step(name: str) -> ExecutionStep:
            return ExecutionStep(
                name=name,
                description=f"Step {name}",
                check_function=lambda _m, _c: StepStatus(state="completed", reason="Already done"),
    ...
    def test_execute_with_pending_steps(self) -> None:
        """Test execution when steps need to be executed."""
        executed_steps = []

        def make_pending_step(name: str) -> ExecutionStep:
            return ExecutionStep(
                name=name,
                description=f"Step {name}",
                check_function=lambda _m, _c: StepStatus(state="pending", reason="Needs work"),
    ...
    def test_execute_with_mixed_states(self) -> None:
        """Test execution with mix of completed and pending steps."""
        executed_steps = []

        def make_step(name: str, state: str) -> ExecutionStep:
            return ExecutionStep(
                name=name,
                description=f"Step {name}",
                check_function=lambda _m, _c: StepStatus(state=state, reason=f"State: {state}"),
                execute_function=lambda _m, _c: executed_steps.append(name),
            )
    ...
    def test_execute_force_mode(self) -> None:
        ...
        def make_step(name: str) -> ExecutionStep:
            return ExecutionStep(
                name=name,
                description=f"Step {name}",
                check_function=lambda _m, _c: StepStatus(state="completed", reason="Would skip"),
                execute_function=lambda _m, _c: executed_steps.append(name),
            )
    ...
    def test_execute_with_client_requirement(self) -> None:
        """Test execution with steps that require ArgoCD client."""
        executed_with_client = []

        def make_step(name: str, requires_client: bool) -> ExecutionStep:
            return ExecutionStep(
                name=name,
                description=f"Step {name}",
                check_function=lambda _m, _c: StepStatus(state="pending", reason="Needs work"),
        ...
        assert len(executed_with_client) == 2  # noqa: PLR2004
    ...
    def test_execute_stops_on_failure(self) -> None:
        ...
        def failing_execute(_m: UpManifest, _c: ArgoClient | None) -> Never:
            msg = "Step failed"
        ...
        assert len(engine.results) == 2  # noqa: PLR2004 # Success and failure steps recorded
    ...
    def test_get_status_summary(self) -> None:
        ...
        assert summary["completed"] == 2  # noqa: PLR2004
        assert summary["skipped"] == 1
        assert summary["failed"] == 1
        assert summary["pending"] == 1
        # Note: get_status_summary returns counts of each state, not a 'total' key


class TestUpExecutionEngine:  # pylint: disable=too-few-public-methods
    """Test cases for the up-specific execution engine."""

    def test_create_up_execution_engine(self) -> None:
        """Test creation of up-specific execution engine."""
        engine = create_up_execution_engine()

        # Should have the standard 10 steps (includes coredns and ca-distribution)
        assert len(engine.steps) == 10  # noqa: PLR2004
        ...
        assert "CoreDNS" in descriptions[3]
        assert "Install cert-manager" in descriptions[4]
        assert "Set up dev Root CA and certificates" in descriptions[5]
        assert "CA certificate" in descriptions[6]
        ...
        assert engine.steps[3].requires_client is False  # coredns
        assert engine.steps[4].requires_client is False  # cert-manager
        assert engine.steps[5].requires_client is False  # ca
        assert engine.steps[6].requires_client is False  # ca-distribution
        assert engine.steps[7].requires_client is False  # secrets
        assert engine.steps[8].requires_client is True  # repo-creds
        assert engine.steps[9].requires_client is True  # apps
    ...
    def test_lazy_client_initialization(self) -> None:
        ...
        assert len(engine.results) == 2  # noqa: PLR2004
