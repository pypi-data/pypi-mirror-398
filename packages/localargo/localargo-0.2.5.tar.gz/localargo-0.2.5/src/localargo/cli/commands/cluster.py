...
def _display_cluster_status(cluster_status: dict[str, str | bool], namespace: str, *, argocd_installed: bool) -> None:
...
def _log_kind_hints_if_applicable(provider: str) -> None:
    ...
    if provider != "kind":
        return
    logger.info("🌐 ArgoCD UI will be available at: https://argocd.localtest.me (after installation)")
...
def list_clusters() -> None:
    ...
    for c in clusters:
        ...
        name = str(c.get("name", ""))
        cluster_status = "ready" if c.get("ready", False) else ("exists" if c.get("exists", False) else "missing")
