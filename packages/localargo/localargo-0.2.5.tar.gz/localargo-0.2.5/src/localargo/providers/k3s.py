...
class K3sProvider(ClusterProvider):
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            if not k3s_path:
                return False
            result = run_subprocess([k3s_path, "--version"])
    ...
    def get_cluster_status(self, name: str | None = None) -> dict[str, Any]:
        ...
        if kubeconfig.exists():
            ...
            try:
                ...
                run_subprocess([kubectl_path, "--kubeconfig", str(kubeconfig), "cluster-info"])
                status["ready"] = True
    ...
    def _wait_for_cluster_ready(
        self,
        context_name: str | subprocess.Popen,
        timeout: int = 60,
    ) -> None:
        ...
        while time.time() - start_time < timeout:
            check_process_exited_with_error(
                context_name,
                error_msg="k3s server process exited with error",
            )
