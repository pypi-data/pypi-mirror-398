...
def calculate_pdf_hash(report_file_path: Path) -> Optional[str]:
    ...
    try:
        if not pdf_file_path.is_file():
            logger.error(f"Corresponding report file not found for {report_file_path.name}: {pdf_file_path}")
            return None
        ...
        logger.error(f"Could not calculate report hash for {report_file_path.name}: [Errno 2] No such file or directory: '{pdf_file_path}'")
        return None
    except Exception as e:
...
def _process_pdf_file(
    ...
    if not extracted_meta:
        logger.warning(f"Metadata extraction failed for report {filename}. Skipping append.")
        return 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Collect patient names from JSON/report reports using different extraction methods. Writes incrementally to JSON Lines (.jsonl).")
