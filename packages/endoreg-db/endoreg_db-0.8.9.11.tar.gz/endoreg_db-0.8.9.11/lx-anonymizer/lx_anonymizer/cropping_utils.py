...
und Batch-Verarbeitung von reports.
"""
...
class BatchCropper:
    """
    Klasse für die Batch-Verarbeitung mehrerer reports mit Cropping-Funktionalität.
    """
    ...
    def process_pdf_directory(self, 
        ...
        """
        Verarbeitet alle reports in einem Verzeichnis.
        
        Args:
            pdf_dir: Verzeichnis mit reports
            file_pattern: Dateimuster für report-Dateien
        ...
        if not pdf_dir.exists():
            raise ValueError(f"report-Verzeichnis existiert nicht: {pdf_dir}")
        
        # Finde alle report-Dateien
        pdf_files = list(pdf_dir.glob(file_pattern))
        if not pdf_files:
            logger.warning(f"Keine report-Dateien gefunden in: {pdf_dir} mit Muster: {file_pattern}")
            return {'files': [], 'stats': self.stats}
        
        logger.info(f"Gefunden {len(pdf_files)} report-Dateien zur Verarbeitung")
    ...
    def _process_single_pdf(self, pdf_path: Path) -> Dict[str, Any]:
        """
        Verarbeitet eine einzelne report-Datei.
        
        Args:
            pdf_path: Pfad zur report-Datei
            
        Returns:
            Dictionary mit Verarbeitungsergebnis
        """
        try:
            # Erstelle Ausgabeverzeichnis für diese report
