...
Dieses Modul erkennt sensitive Textregionen in reports und erstellt gecropte Versionen
...
class SensitiveRegionCropper:
    ...
    def crop_sensitive_regions(self, 
        ...
        """
        Croppt sensitive Regionen aus einem report und speichert sie als separate Bilder.
        
        Args:
            pdf_path: Pfad zum report
        ...
        try:
            ...
            # Konvertiere report zu Bildern
            logger.info(f"Konvertiere report zu Bildern: {pdf_path}")
            ...
            for page_num in page_numbers:
                if page_num >= len(images):
                    logger.warning(f"Seite {page_num} existiert nicht in report")
                    continue
    ...
    def create_anonymized_pdf_with_crops(self, 
        ...
        """
        Erstellt eine anonymisierte Version des reports, bei der sensitive Regionen
        durch schwarze Rechtecke überdeckt werden.
        
        Args:
            pdf_path: Ursprüngliches report
            crop_output_dir: Verzeichnis mit den Crop-Daten
            anonymized_pdf_path: Pfad für das anonymisierte report
        """
        try:
            import fitz  # PyMureport
            
            logger.info(f"Erstelle anonymisiertes report: {anonymized_pdf_path}")
            
            # Öffne das ursprüngliche report
            doc = fitz.open(pdf_path)
            
            # Konvertiere report zu Bildern für die Analyse
            images = convert_pdf_to_images(pdf_path)
            ...
            for page_num, image in enumerate(images):
                ...
                if sensitive_regions:
                    ...
                    # Konvertiere Pixel-Koordinaten zu report-Koordinaten
                    # report-Koordinaten haben Ursprung unten links, Bilder oben links
                    page_rect = page.rect
                    ...
                    for x1, y1, x2, y2 in sensitive_regions:
                        # Konvertiere Bildkoordinaten zu report-Koordinaten
                        pdf_x1 = x1 * scale_x
            ...
            # Speichere das anonymisierte report
            doc.save(anonymized_pdf_path)
            doc.close()
            
            logger.info(f"Anonymisiertes report gespeichert: {anonymized_pdf_path}")
            
        except ImportError:
            logger.error("Pymureport not installed.")
            raise
        except Exception as e:
            logger.error(f"Fehler beim Erstellen des anonymisierten reports: {e}")
            raise
...
def crop_sensitive_regions_from_pdf(pdf_path: str, 
    ...
    """
    Convenience-Funktion zum Cropping sensitiver Regionen aus einem report.
