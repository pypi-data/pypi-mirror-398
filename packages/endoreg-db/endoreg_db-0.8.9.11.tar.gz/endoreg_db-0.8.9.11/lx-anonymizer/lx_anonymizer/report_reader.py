...
class ReportReader:
    ...
    def read_pdf(self, pdf_path):
        ...
        if not isinstance(pdf_path, (str, os.PathLike)):
            logger.error(f"report path must be a string or PathLike object, got {type(pdf_path)}: {pdf_path}")
            return ""
    ...
    def extract_report_meta(self, text, pdf_path):
        ...
        try:
            if pdf_path and isinstance(pdf_path, (str, os.PathLike)) and os.path.exists(pdf_path):
                ...
                logger.warning(f"Cannot calculate report hash: invalid or missing pdf_path: {pdf_path}")
    ...
    def process_report(self, pdf_path=None, image_path=None, use_ensemble=False, verbose=True, use_llm_extractor='deepseek', text=None):
        ...
        if not text or len(text.strip()) < 50: # Trigger OCR if text is empty or very short
            ...
            try:
                ...
                if pdf_path:
                    if not isinstance(pdf_path, (str, os.PathLike)):
                        logger.error(f"Cannot apply OCR: report path is not valid: {pdf_path}")
                        return "", "", {}
                    if not os.path.exists(pdf_path):
                        logger.error(f"Cannot apply OCR: report file not found: {pdf_path}")
                        return "", "", {}
    ...
    def process_report_with_cropping(self, pdf_path=None, image_path=None, use_ensemble=False, 
        ...
        Args:
            ...
            image_path: Pfad zum Bild (alternative zu report)
        ...
        if not anonymization_output_dir:
            # Setze ein Standard-Ausgabeverzeichnis für anonymisierte reports
        ...
        if crop_sensitive_regions and crop_output_dir and pdf_path:
            try:
                ...
                # Erstelle automatisch ein anonymisiertes report
                if cropped_regions_info:
                    ...
                    try:
                        ...
                        logger.info(f"Anonymisiertes report erstellt: {anonymized_pdf_path}")
                    except Exception as pdf_error:
                        logger.warning(f"Konnte anonymisiertes report nicht erstellen: {pdf_error}")
