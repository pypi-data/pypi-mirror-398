...
mouse_label_dict = {}
df = []
def alignment_model(img_channel, slice_,registration_channel,  p_name = './processed_pickles', file_name = 'sample.tif', atlas_path = 'Atlas_Brains', template_path = 'Atlas_Brains' ):
  global df, mouse_label_dict
  os.makedirs(p_name, exist_ok = True)
  df = pd.read_csv("Updated_Cortical_Atlas_MortX_Labels.csv")
  if registration_channel == "P04": registration_channel = "P4"
  for counter in df.index:
    mouse_label_dict[df['acronym'][counter]] = [ast.literal_eval(df[f'RGB_{registration_channel}'][counter])]
  if slice_ == None:
    slice_ = 'A1_S1_slice'
  print("registration::", img_channel, slice_)
  _, slice_num, _ = slice_.split("_")
  fldr = registration_channel
  
  template_section_path = f'{template_path}/{fldr}/Ref_{slice_num}_GAD.png'
  # f'{atlases_dir}/{fldr}/{slice_num}/{slice_}.png'
  label_name = slice_.replace('slice', 'labels')
  template_atlas_path = f'{atlas_path}/{fldr}/Ref_{slice_num}_atlas_new.png'
  template_section = np.array(Image.open(template_section_path))
...
def area_quantification(p_name, file_name ='sample.tif', result_path = './Results/Quantitative_results/'):
		global df, mouse_label_dict
		if not(os.path.exists(result_path)): os.makedirs(result_path)
		...
		area, name = [], []
		for reg_id in temp.keys():
			area.append( np.sum(temp[reg_id]))
			name.append(df[df['acronym'] == reg_id]['Region'].values[0])
		...
		quant_df.to_csv(result_path+f'{file_name.split(".")[0]}quant_area_raw_csv.csv') #, sep='\t', encoding='utf-8')
