...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `funcy_bear`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in funcy_bear.__all__ or not hasattr(funcy_bear, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in funcy_bear.__all__:
