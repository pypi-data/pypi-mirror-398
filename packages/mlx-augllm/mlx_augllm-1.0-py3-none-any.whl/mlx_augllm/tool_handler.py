...
def _get_chat_response(llm, messages, temp=0.0):
    """
    ...
    gen = llm.generate(messages, images=None, temp=temp, stream=False)
    for response in gen:
...
def select_tool(selector_llm, tool_dict, system_prompt: dict, user_message: dict, context: dict = None):
    ...
    response_dict = _get_chat_response(selector_llm, messages, temp=0.0)
