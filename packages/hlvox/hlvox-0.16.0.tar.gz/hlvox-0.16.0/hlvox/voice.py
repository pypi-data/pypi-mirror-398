...
class CutModifier(Modifier):
    ...
    def __init__(self, start_time_s: float, end_time_s: float) -> None:
        # Start time must be less than end time, and both must be positive
        if start_time_s >= end_time_s or start_time_s < 0 or end_time_s < 0:
            raise ModifierArgumentsInvalid
    ...
    def from_str(cls, string: str) -> "CutModifier":
        ...
        return CutModifier(
            start_time_s=float(start_time_s_str), end_time_s=float(end_time_s_str)
        )

    def as_str(self) -> str:
        return f"{self.IDENTIFIER}{self.start_time_s}-{self.end_time_s}"

    def modify_audio(self, audio: Audio) -> Audio:
        start_index = int(self.start_time_s * audio.sample_rate)
        start_index = max(start_index, 0)
        end_index = int(self.end_time_s * audio.sample_rate)
        end_index = min(end_index, len(audio.data) - 1)
        return Audio(
