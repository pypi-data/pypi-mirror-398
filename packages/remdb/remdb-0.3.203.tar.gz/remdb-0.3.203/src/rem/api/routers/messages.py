...
async def _resolve_session_id_to_name(session_id: str) -> str | None:
    """
    Resolve a session_id to the session name for message filtering.

    Messages store session_id as the session's `name` field, not the UUID.
    This function looks up the session by UUID and returns its name.

    Args:
        session_id: Either a session UUID or session name

    Returns:
        The session name to use for message filtering, or None if not found
    """
    # If it doesn't look like a UUID, assume it's already a session name
    if not _is_uuid(session_id):
        return session_id

    # Look up the session by UUID to get its name
...
async def list_messages(
    ...
    if session_id:
        # Resolve session_id (UUID or name) to the session name used in messages
        resolved_session_id = await _resolve_session_id_to_name(session_id)
        if resolved_session_id:
            filters["session_id"] = resolved_session_id
        else:
            # Session not found - return empty result
            logger.debug(f"[messages] Session not found for id: {session_id}")
            return MessageListResponse(data=[], total=0, has_more=False)
    if message_type:
