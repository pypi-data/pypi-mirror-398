...
class ProcessTracking:
    ...
    """
    # PR-47941: Update the assign_to_process function to support the new step-name and request-name parameters.
    @staticmethod
    def assign_to_process(context: UserIdentifier, data_type: DataTypeIdentifier, records: list[RecordIdentifier],
                          ...
                          request: RecordIdentifier | None = None, step_name: str | None = None,
                          request_name: str | None = None) -> None:
        """
        ...
        Supports assigning to both experiment processes and custom processes, although this class is focused on
        experiment processes. See the QueueItemHandler class for functions that support custom processes.
        ...
        :param request: The request that the tracked records are a part of, to be used for the assigned process record's
            request record ID field. If not provided, and no request name is provided, then the tracked records will
            not be associated with a request.
        :param step_name: If the given process name is for a custom process, then a step name should be provided instead
            of a step number. This is required by the system for custom processes.
        :param request_name: The name of a new request to be created for the tracked records. Able to be provided in
            lieu of a request ID. If not provided, and no request ID is provided, then the tracked records will
            not be associated with a request.
        """
        ...
        payload = {
            ...
            "branch-id": branch_id,
            "step-name": step_name,
            "request-record-id": AliasUtil.to_record_id(request) if request is not None else None,
