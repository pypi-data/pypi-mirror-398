...
async def main():
    ...
    try:
        ...
        async with ContextNestClient() as client:
            ...
            info_mcp("=" * 70)
            info_mcp("\nYou can now choose to test one of the tools:")
            ...
            try:
                if choice == "1":
                    ...
                    save_path = input("  Enter save path (press Enter for default): ").strip() or None
                    result = await client.web_scrape(url=url, save_path=save_path)
                    info_mcp(f"\n✓ Result:\n{result}")
                    ...
                    title = input("  Enter title (press Enter to auto-extract): ").strip() or None
                    content = input("  Enter content (press Enter to scrape from URL): ").strip() or None
                    result = await client.insert_knowledge(url=url, title=title, content=content)
                    info_mcp(f"\n✓ Result:\n{result}")
