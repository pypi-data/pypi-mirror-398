...
mean_episode_return_buf = {
    p: deque(maxlen=100) for p in ["landlord", "landlord_up", "landlord_down"]
}
...
def train(flags):
    ...
    def batch_and_learn(
        i, device, position, local_lock, position_lock, lock=threading.Lock()
    ):
    ...
    locks = [
        ...
        for _ in range(flags.num_actor_devices)
    ]
    position_locks = {
        "landlord": threading.Lock(),
        "landlord_up": threading.Lock(),
