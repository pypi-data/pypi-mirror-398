...
REFRESH_PROMPT = '''You are a professional content editor. Update the following article based on the user's instructions.
...
2. Maintain the overall structure and tone
3. Keep SEO optimization intact
4. Preserve any existing formatting

Output the updated article in the following JSON format:
{{
  "title": "Updated title (or original if unchanged)",
  "html": "Full updated article in HTML",
  "markdown": "Full updated article in Markdown",
...
async def refresh_content_with_gemini(
    content: str,
    instructions: str,
    api_key: str
) -> Dict[str, Any]:
    """Refresh/update content using Gemini API."""
    try:
        from google import genai
        from google.genai import types
    except ImportError:
...
@app.command()
def refresh(
    input_file: str = typer.Argument(..., help="HTML, Markdown, or JSON file to refresh"),
    instructions: str = typer.Argument(..., help="Instructions for updating the content"),
) -> None:
    """Refresh/update an existing article with new instructions."""
    show_banner(mini=True)
    ...
    # If JSON, extract the HTML or markdown content
    if input_path.suffix.lower() == '.json':
        try:
            data = json.loads(content)
            content = data.get('html', data.get('markdown', data.get('content', content)))
        except json.JSONDecodeError:
            pass
    ...
    rprint(Panel(
        ...
        f"[bold]Instructions:[/bold] {instructions[:100]}{'...' if len(instructions) > 100 else ''}",
        title="[bold cyan]Refreshing Article[/bold cyan]",
        border_style="cyan"
    ))
    rprint()
    ...
    with Progress(
        ...
        task = progress.add_task("[cyan]Refreshing content...", total=100)

        async def run_refresh():
            progress.update(task, completed=30, description="[cyan]Analyzing content...")
            result = await refresh_content_with_gemini(
                content=content,
                instructions=instructions,
                api_key=config_data["gemini_key"]
            )
            progress.update(task, completed=90, description="[cyan]Saving...")
            return result
    ...
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    base_name = input_path.stem + f"-refreshed-{timestamp}"

    # Save HTML
    html_content = result.get("html", result.get("markdown", ""))
    output_path = DOWNLOADS_DIR / f"{base_name}.html"
    ...
    rprint()
    rprint(Panel(
        f"[bold green]Article Refreshed Successfully![/bold green]\n\n"
        f"[bold]Time:[/bold] {elapsed:.1f}s\n"
        f"[bold]Changes:[/bold] {len(changes)} modifications\n\n"
        f"[bold]Saved to:[/bold]\n"
        f"  [cyan]{output_path}[/cyan]\n"
        f"  [cyan]{json_path}[/cyan]",
        border_style="green"
    ...
    if changes:
        rprint()
        rprint("[bold]Changes made:[/bold]")
        for change in changes[:5]:
            rprint(f"  [dim]•[/dim] {change}")
        if len(changes) > 5:
            rprint(f"  [dim]... and {len(changes) - 5} more[/dim]")
