...
class APIMonitorMiddleware(BaseHTTPMiddleware):
    ...
    def _get_client_ip(self, request: Request) -> str:
        ...
        3. X-Forwarded-For (generic proxy header, take first IP)
        4. request.client.host (direct connection fallback)
        """
        # Cloudflare's header - most reliable when behind CF proxy
        ...
        # X-Forwarded-For (generic proxy header, take first IP)
