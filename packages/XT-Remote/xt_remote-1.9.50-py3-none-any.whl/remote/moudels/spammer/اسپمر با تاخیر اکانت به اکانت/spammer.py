...
# 📊 Stats (اختیاری؛ اگر نبود، ارسال از کار نمی‌افتد)
try:
    from ..analytics_manager import update_stats  # type: ignore
except Exception:  # pragma: no cover
    def update_stats(*args, **kwargs):  # type: ignore
        # اگر analytics_manager نبود، چیزی لاگ نکن ولی نذار اسپمر بترکه
        return
...
async def _get_accounts_from_manager(spam_config: Dict[str, Any]) -> List[str]:
    ...
    """
    # 1) اول از client_manager استفاده می‌کنیم
    ...
    # 2) سپس از account_manager.accounts()
    ...
    # 3) در نهایت از لیست داخل تنظیمات
...
def _record_stats(spam_config: Dict[str, Any], acc_phone: str, success: bool) -> None:
    """
    رپپر کوچک روی update_stats تا اگر ارور داد، اسپمر متوقف نشود.
    فرض منطقی: update_stats(target, acc_phone, success)
    """
...
async def safe_send_real(
    ...
    فقط یک بار برای هر acc_phone و هر پیام، روی stats اثر می‌گذاریم:
      - اگر در نهایت موفق شد → success=True
      - اگر همهٔ تلاش‌ها شکست خورد / ارور جدی → success=False
    """
...
async def run_spammer(
    ...
    منطق زمان‌بندی جدید:
      - delay روی ست‌ها اعمال می‌شود، نه تک‌تک اکانت‌ها.
      - یعنی:
          ست 1 → base_delay ثانیه → ست 2 → base_delay ثانیه → ...
    """
    base_delay = float(spam_config.get("TimeSleep", 2.0))
    batch_size = max(1, int(spam_config.get("BATCH_SIZE", 1)))
    # برای دقت زمانی بالا، عملاً concurrency = 1
    concurrency = 1
    total_ok = 0
    ...
    try:
        ...
        # بستن کلاینت‌ها به عهده stop_manager/client_manager.stop_all_clients است.
