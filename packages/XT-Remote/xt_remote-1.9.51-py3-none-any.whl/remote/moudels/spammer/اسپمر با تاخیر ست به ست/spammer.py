...
async def run_spammer(
    ...
    منطق زمان‌بندی (نسخه‌ی جدید بر اساس ست / batch):
      - اکانت‌ها در batchها (ست‌ها) پردازش می‌شوند.
      - بین «شروع هر batch» دقیقاً base_delay فاصله داریم.
      - داخل هر batch، پیام همه اکانت‌ها پشت سر هم ارسال می‌شود
        (فقط delay کوچک داخلی safe_send_real).
      مثال:
        ست ۱  → ۲ ثانیه → ست ۲ → ۲ ثانیه → ست ۳ → ...
    """
    base_delay = float(spam_config.get("TimeSleep", 2.0))
    batch_size = max(1, int(spam_config.get("BATCH_SIZE", 1)))
    # برای دقت و کنترل ساده
    concurrency = 1
    total_ok = 0
    ...
    # برای این‌که اولین batch بدون انتظار انجام شود
    ...
    try:
        while spam_config.get("run", False):
            ...
            # تقسیم اکانت‌ها به ست‌ها (batchها)
            for i in range(0, len(accounts), batch_size):
                if not spam_config.get("run", False):
                    break

                # ⏱ زمان‌بندی بین ست‌ها / batchها
                now = time.monotonic()
                ...
                # در این‌جا دیگر delay سراسری بین اکانت‌ها نداریم؛
                # فقط delay کوچک داخلی safe_send_real وجود دارد.
                for acc in batch:
                    if not spam_config.get("run", False):
                        break
