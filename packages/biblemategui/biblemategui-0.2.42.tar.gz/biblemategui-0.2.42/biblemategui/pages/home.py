...
from biblemategui.pages.tools.introduction import book_introduction
...
class BibleMateGUI:
    ...
    def open_book_context_menu(self, db, b, c, v, note=False):
        ...
        def open_introduction(section):
            app.storage.user["tool_query"] = section
            open_tool("Introduction")
        with ui.context_menu() as menu:
            ui.menu_item(f'⏳ {get_translation("Timelines")}', on_click=lambda: open_tool("Timelines"))
            ui.separator()
            ui.menu_item(f'🔘 {get_translation("Overview")}', on_click=lambda: open_introduction("Overview"))
            ui.menu_item(f'📐 {get_translation("Structural Outline")}', on_click=lambda: open_introduction("Structural Outline"))
            ui.menu_item(f'🧩 {get_translation("Logical Flow")}', on_click=lambda: open_introduction("Logical Flow"))
            ui.menu_item(f'🕰️ {get_translation("Historical Setting")}', on_click=lambda: open_introduction("Historical Setting"))
            ui.menu_item(f'🎨 {get_translation("Themes")}', on_click=lambda: open_introduction("Themes"))
            ui.menu_item(f'🏷️ {get_translation("Keywords")}', on_click=lambda: open_introduction("Keywords"))
            ui.menu_item(f'⛪ {get_translation("Theology")}', on_click=lambda: open_introduction("Theology"))
            ui.menu_item(f'🚩 {get_translation("Canonical Placement")}', on_click=lambda: open_introduction("Canonical Placement"))
            ui.menu_item(f'🌱 {get_translation("Practical Living")}', on_click=lambda: open_introduction("Practical Living"))
            ui.menu_item(f'💎 {get_translation("Summary")}', on_click=lambda: open_introduction("Summary"))
    ...
    def create_menu(self):
                ...
                with ui.row().classes('items-center no-wrap'):
                    ...
                    with ui.button(icon='build').props('flat color=white round').tooltip(get_translation("Tools")):
                        with ui.menu():
                            ...
                            ui.separator()
                            ui.menu_item(get_translation("Book Introduction"), on_click=lambda: self.load_area_2_content(title='Introduction', sync=True))
                    ...
                    with ui.button(icon='auto_awesome').props('flat color=white round').tooltip(get_translation("AI")):
                        with ui.menu():
                            ui.menu_item(get_translation("AI Introduction"), on_click=lambda: self.load_area_2_content(title='Introduction'))
        ...
        with ui.drawer('left') \
            ...
            with ui.expansion(get_translation("Tools"), icon='build').props('header-class="text-secondary"'):
                ...
                ui.separator()
                ui.item(get_translation("Book Introduction"), on_click=lambda: (
                    self.load_area_2_content(title='Introduction', sync=True),
                    app.storage.user.update(left_drawer_open=False)
                )).props('clickable')
            ...
            with ui.expansion(get_translation("AI"), icon='auto_awesome').props('header-class="text-secondary"'):
                ui.item(get_translation("AI Introduction"), on_click=lambda: (
                    self.load_area_2_content(title='Introduction'),
                    app.storage.user.update(left_drawer_open=False)
                )).props('clickable')
