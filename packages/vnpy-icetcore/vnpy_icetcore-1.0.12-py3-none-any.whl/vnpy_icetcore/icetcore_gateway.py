...
from vnpy.trader.object import TickData, OrderData, TradeData, PositionData, AccountData, ContractData, OrderRequest, CancelRequest, SubscribeRequest
...
# 多空方向映射
DIRECTION_VT2ICE: Dict[Direction, int] = {Direction.LONG: 1, Direction.SHORT: 2}
...
ORDERTYPE_VT2ICE: Dict[OrderType, tuple] = {OrderType.LIMIT: (2, 1), OrderType.MARKET: (1, 2), OrderType.FAK: (2, 2), OrderType.FOK: (2, 3), OrderType.STOP: (3, 2), OrderType.STOPLIMIT: (4, 1)}
...
OFFSET_VT2ICE: Dict[Offset, int] = {Offset.OPEN: 0, Offset.CLOSE: 1, Offset.CLOSETODAY: 2, Offset.CLOSEYESTERDAY: 3, Offset.NONE: 4, Offset.COVEREDCALLWRITING: 10, Offset.COVEREDCALLCLOSE: 11}
...
# 产品类型映射
PRODUCT_ICE2VT: Dict[str, Product] = {"FUT": Product.FUTURES, "OPT": Product.OPTION, "STK": Product.EQUITY}

# 期权类型映射
OPTIONTYPE_ICE2VT: Dict[str, OptionType] = {"C": OptionType.CALL, "P": OptionType.PUT}
...
class IceTCoreGateway(BaseGateway):
    default_name: str = "ICETCore"
    default_setting: Dict[str, str] = {"客户端路径": "C:/MCTraderSTD/APPs"}
    ...
    def __init__(self, event_engine: EventEngine, gateway_name: str) -> None:
        ...
        if "datafeed.apppath" not in SETTINGS.keys():
            SETTINGS["datafeed.apppath"] = "C:/MCTraderSTD/APPs"
    ...
    def subscribe(self, req: SubscribeRequest) -> None:
        ...
        if contract.product == Product.FUTURES:
            ...
            symbolhead = "TC.F2."
        threading.Thread(
            target=self.api.subquote,
            args=(
                symbolhead+contract.name,
            ),
        ).start()
        ...
        if data:
            ...
            for acc in self.api.getaccountlist():
                ...
                for data1 in data2:
                    if data1["Side"] != 0:
                        ...
                        symbol: str = (
                            (symbsplit[2] + "." + self.api.getsymbol_id(data1["Symbol"])) if "TC.F2." not in data1["Symbol"] else (data1["Exchange"] + "." + data1["Symbol"].replace("TC.F2.", ""))
                        )
            ...
            for orderdata in orderreport:
                if orderdata["ExecType"] != 10 and orderdata["ExecType"] != 12:
                    ...
                    symbol: str = (
                        (symbsplit[2] + "." + self.api.getsymbol_id(orderdata["Symbol"]))
                        if "TC.F2." not in orderdata["Symbol"]
                        else (orderdata["Exchange"] + "." + orderdata["Symbol"].replace("TC.F2.", ""))
                    )
            ...
            for filldata in fillreport:
                ...
                symbol: str = (
                    (symbsplit[2] + "." + self.api.getsymbol_id(filldata["Symbol"])) if "TC.F2." not in filldata["Symbol"] else (filldata["Exchange"] + "." + filldata["Symbol"].replace("TC.F2.", ""))
                )
                contract: ContractData = symbol_contract_map.get(symbol, None)
    ...
    def onmargin(self, accmask, data):
        ...
        for data1 in data:
            ...
            account: AccountData = AccountData(
                accountid=data1["Account"], balance=float(data1["TotalEquity"]), frozen=data1["FrozenCash"] if data1["FrozenCash"] else 0, gateway_name=self.gateway_name
            )
    ...
    def onposition(self, accmask, data):
        ...
        for data1 in data:
            if data1["Side"] != 0:
                symbsplit = data1["Symbol"].split(".")
                symbol: str = (
                    (symbsplit[2] + "." + self.gateway.api.getsymbol_id(data1["Symbol"])) if "TC.F2." not in data1["Symbol"] else (data1["Exchange"] + "." + data1["Symbol"].replace("TC.F2.", ""))
                )
                contract: ContractData = symbol_contract_map.get(symbol, None)
                if contract:
                    if data1["SumLongQty"] != 0:
