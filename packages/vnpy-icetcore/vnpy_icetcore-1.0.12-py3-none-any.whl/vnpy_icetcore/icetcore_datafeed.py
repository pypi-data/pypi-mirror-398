...
class IceTCoreDatafeed(BaseDatafeed):
        ...
        if rq_interval<5:
            while(True):
                if stime<end.date()+timedelta(days=1):
                    his=self.api.getquotehistory(rq_interval,1,symbolhead+contract.name,(stime-timedelta(days=1)).strftime('%Y%m%d')+"08",stime.strftime('%Y%m%d')+"08")
                    if his:
                        df=df+his
    ...
    def query_tick_history(self, req: HistoryRequest, output: Callable = print) -> Optional[List[TickData]]:
        ...
        while(True):
            if stime<end.date():
                his=self.api.getquotehistory(2,1,symbolhead+contract.name,(stime-timedelta(days=1)).strftime('%Y%m%d')+"08",stime.strftime('%Y%m%d')+"08")
                if his:
                    df=df+his
                stime=stime+timedelta(days=1)
            else:
