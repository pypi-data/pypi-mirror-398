"""OptaSense module from dastools.
...
def filegenerator(experiment: str, directory: Path = '.'):
    log = logging.getLogger('OptaSense.filegenerator')
    # Loop through files to check that they are in OptaSense format
    if not os.path.isdir(directory):
        return

    # Sort based on the name of the Direntry
    for expday in sorted(os.scandir(directory), key=lambda x: x.name):  # type: os.DirEntry
        # Entry must be a directory
        if not expday.is_dir():
            continue
        # Check format of the directory name (experiment_YYYYMMDD)
        try:
            dt = datetime.datetime.strptime(expday.name, experiment + '_%Y%m%d')
        except ValueError:
            log.warning('Unexpected format in directory name! Date expected... (%s)' % expday.name)
            continue

        daydir = os.path.join(directory, expday.name)
        for exptime in sorted(os.scandir(daydir), key=lambda x: x.name):  # type: os.DirEntry
            # Entry must be a directory
            if not exptime.is_dir():
                continue

            # Check format of the directory name (experiment_YYYYMMDD_YYYY-MM-dd_HH-MM-SSZ.h5)
            try:
                dt = datetime.datetime.strptime(exptime.name, expday.name + '_%Y-%m-%dT%H_%M_%SZ')
            except ValueError:
                log.warning('Unexpected format in directory name! DateTime expected... (%s)' % exptime.name)
                continue

            datadir = os.path.join(directory, expday.name, exptime.name)
            for file in sorted(os.scandir(datadir), key=lambda x: x.name):
                ...
                try:
                    dt = datetime.datetime.strptime(file.name, expday.name + '_%Y-%m-%dT%H%M%SZ.h5')
                except ValueError:
                    log.warning('Unexpected format in file name! Time expected... (%s)' % file.name)
                    continue

                # Read with h5py
                fname = os.path.join(datadir, file.name)
                fin = File(fname)
                yield fname
    return


class OptaSenseDetector(DASDetector):
    def checkDASdata(self, experiment: str, directory: Path = '.') -> bool:
        log = logging.getLogger('OptaSenseDetector')
        for aux in filegenerator(experiment, directory):
            return True
        return False


class OptaSense2D(Das):
    """Class to read seismic waveforms in OptaSense format with a 2D approach
...
Therefore, the units of the output will be usually strain rate.
    ...
    def __getalldata(self) -> np.array:
        ...
        auxdata = self.__fi['data'] * self.metadata['header']['dataScale']
...
class OptaSenseReader(Das):
    """Class to read seismic waveforms in OptaSense format
...
Therefore, the units of the output will be usually strain rate.
    ...
    def __init__(self, experiment: str, directory: DirectoryPath = '.',
                 ...
                 channelcode: constr(strip_whitespace=True, to_upper=True, min_length=3, max_length=3) = 'HSF',
                 loglevel: str = 'INFO'):
        """Initialize the OptaSense object selecting the data and channels
        ...
        # Create a DAS-RCN metadata object and retrieve the items from the "metadata" attribute
        self.dasrcn = DASMetadata(network_code=networkcode,
                                  interrogators=[InterrogatorModel(manufacturer='OptaSense')])
    ...
    def __inspect_files(self) -> List[DASFile]:
        ...
        # Loop through files and load them in __available as 'YYYYMMDD/TTmmSS'
        result = list()
        for fname in filegenerator(self.__experiment, self.__directory):
            # Read with h5py
            # TODO Check if this is needed or we can skip it by using the part of the filename with the time
            self.__fi = File(fname)
            try:
    ...
    def __select_file(self):
        ...
        if self.__currentfile is None:
            ...
            for idx, fi in enumerate(self.__available):
                if fi.dt <= self.__twstart <= fi.dtend:
                    # filename = os.path.join(self.__directory, self.__available[idx].name)
                    filename = self.__available[idx].name
                    self.__currentfile = idx
                    self.__logs.debug(
                        'Opening %s; Starttime: %s' % (self.__available[self.__currentfile].name, self.__twstart))
                    break
    ...
    def __readgroup(self, grp: Group):
        ...
        for k, v in grp.items():
            # print(v.name)
            if isinstance(v, Dataset) and not v.name.endswith('/RawData'):
                grpdict[k] = v[()]
            elif isinstance(v, Dataset) and v.name.endswith('/RawData'):
                self.__data = v
                grpdict[k] = v.dtype
            elif isinstance(v, Group):
                grpdict[k] = self.__readgroup(v)
    ...
    def __readmetadata(self):
        ...
        self.metadata = self.__readgroup(self.__fi)
        self.__datatype = self.metadata['Acquisition']['Raw[0]']['RawData']
        # rd = self.metadata['Acquisition']['Raw[0]']['RawData']
        rdt = self.metadata['Acquisition']['Raw[0]']['RawDataTime']
        ts = (rdt[1] - rdt[0])/1000000

        if self.sampling_rate is None:
            self.sampling_rate = 1.0/ts
            # Set the sampling rate in the DAS-RCN metadata
            self.dasrcn.interrogators[0].acquisitions[0].acquisition_sample_rate = self.sampling_rate
            self.dasrcn.interrogators[0].acquisitions[0].acquisition_sample_rate_unit = 'Hertz'
        ...
        curstarttime = datetime.datetime.utcfromtimestamp(rdt[0]/1000000)
        if self.starttime is None:
            self.starttime = curstarttime
        # TODO Check if nChannels is available in version 8!
        # print(rd.shape, rdt.shape)
        self.numchannels = self.__data.shape[1]

        # If channels were not selected we can safely set that here to all channels
        if self.channels is None:
            self.channels = [c for c in range(self.numchannels)]
        ...
        self.__samples = rdt.shape[0]

        # Set endtime based on the last entry of RawDataTime
        self.endtime = datetime.datetime.utcfromtimestamp(rdt[-1]/1000000)
        ...
        # Sample to start extraction from based on the initial datetime of the file (__twstart)
        if self.__twstart is not None:
            self.__samplestart = max(np.searchsorted(rdt, self.__twstart.timestamp()*1000000, 'left'), 0)
        else:
            self.__samplestart = 0
        ...
        if (self.__twend is None) or (self.__twend >= self.endtime):
            ...
            # Otherwise calculate which one is the last sample to read
            self.__sampleend = min(np.searchsorted(rdt, self.__twend.timestamp()*1000000, 'right'), rdt.shape[0])
...
class OptaSense(Das):
    """Class to read, process and export seismic waveforms in OptaSense format
