#!/usr/bin/env python
'''
锐窗(rywin): 用 派神(python) 开发的 锐派(RyPi) 图形窗口，方便在图形界面系统或终端进行图形可视化窗口创建。
'''

VER = r'''
rywin version: 2025.8.1.1.0
'''
...
if __name__ == '__main__' and __package__ is None:
    ...
    __package__ = 'rypi.rywin'

# 导入依赖
try:  # 相对导入
    from . import conf as rywin_conf
    from .. import comm
except ImportError:
    try:  # 包名导入
        from rypi.rywin import conf as rywin_conf
        from rypi import comm
    except ImportError:  # 绝对导入
        import conf as rywin_conf
        import rypi.comm
