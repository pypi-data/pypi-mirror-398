...
class RootPolynomialMatrix(Node):
    ...
    def solve(self, source: NDArray[Any], target: NDArray[Any]):
        # Setting Matrix with Moore-Penrose solution for speed
        self.matrix = matrix_colour_correction_Finlayson2015(
            source, target, degree=self.degree, root_polynomial_expansion=True
        )
    ...
    @staticmethod
    def apply_matrix(
        RGB: NDArray[Any], matrix: NDArray[Any], degree: int
    ) -> NDArray[Any]:
        shape = RGB.shape
        RGB = np.reshape(RGB, (-1, 3))
...
class TetrahedralMatrix(Node):
    ...
    def apply_matrix(RGB: NDArray[Any], matrix: NDArray[Any]) -> NDArray[Any]:
        ...
        blk = np.array([0, 0, 0])
        wht = np.array([1, 1, 1])
        red, grn, blu, cyn, mag, yel = matrix
        ...
        n_RGB[case_1] = tetra_case(
            case_1, r, (red - blk), g, (yel - red), b, (wht - yel), blk
        )
        n_RGB[case_2] = tetra_case(
            case_2, r, (red - blk), g, (wht - mag), b, (mag - red), blk
        )
        n_RGB[case_3] = tetra_case(
            case_3, r, (mag - blu), g, (wht - mag), b, (blu - blk), blk
        )
        n_RGB[case_4] = tetra_case(
            case_4, r, (wht - cyn), g, (cyn - blu), b, (blu - blk), blk
        )
        n_RGB[case_5] = tetra_case(
            case_5, r, (wht - cyn), g, (grn - blk), b, (cyn - grn), blk
        )
        n_RGB[case_6] = tetra_case(
            case_6, r, (yel - grn), g, (grn - blk), b, (wht - yel), blk
        )
