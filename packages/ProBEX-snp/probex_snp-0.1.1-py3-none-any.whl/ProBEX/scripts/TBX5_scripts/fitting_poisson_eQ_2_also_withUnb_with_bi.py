...
results_dir = "./output/TBX5/fitting/iteration_results_withunb_with_bi"
os.makedirs(results_dir, exist_ok=True)
merged_df_combined = pd.read_csv("./output/TBX5/merged_df_TBX5_without_pseudocounts.csv")
denom_df = pd.read_csv("./output/TBX5/fitting/results_1000/denominators_iteration_361.csv")
concentrations = np.array([100, 1000, 1500, 2000, 3000])
initial_denominators_R1 = denom_df[["100_R1", "1000_R1", "1500_R1", "2000_R1", "3000_R1"]].values.flatten().astype(float)
initial_denominators_R2 = denom_df[["100_R2", "1000_R2", "1500_R2", "2000_R2", "3000_R2"]].values.flatten().astype(float)
...
def update_denominators_withLambda(denominators, K_values, lambda_Li_R1_opt, lambda_Li_R2_opt, T_L_R1, T_L_R2, bi, concentrations):
    ...
    for j in range(10):
        ...
        denominator_term = (1 / (1 + (1 / (K_values * concentrations[j % 5])))) * Frac_lib_conc * (1 - bi)
...
def calculate_unbound_denominators(denominators, K_values, lambda_Li_R1_opt, lambda_Li_R2_opt, T_L_R1, T_L_R2, bi, concentrations):
    ...
    for j in range(10):
        ...
        Frac_lib_conc = lambda_L1_i / T_L
        denominator_term = (1 - (1 / (1 + (1 / (K_values * concentrations[j % 5]))))) * Frac_lib_conc 
        new_denominators[j] = np.sum(denominator_term)
    return new_denominators
...
def neg_log_likelihood(params, n_sij_R1_i, n_sij_R2_i, n_Uij_R1_i, n_Uij_R2_i, n_L1, n_L2, 
    ...
    K= 10 ** params[0]  # Single Kvalue for both replicates
...
def optimize_K_and_lambda_Li(merged_df_combined, T_cj, T_cj_R1, T_cj_R2, initial_denominators, 
    ...
    for iteration in range(max_iters):
        ...
        denominators_R1 = denominators[:5]
        denominators_R2 = denominators[5:]
        unbound_denominators_R1 = unbound_denominators[:5]
        unbound_denominators_R2 = unbound_denominators[5:]
