...
Date: [March 26, 2025]
Description:
	This script iteratively fits binding model parameters (K, λ for replicatesⱼ) for SNP count data using maximum likelihood estimation.
...
results_dir = "../../output/NKX25/fitting/iteration_results_merged_withunb_without_bi"
...
def update_denominators_withLambda(denominators, K_values, lambda_Li_R1_opt, lambda_Li_R2_opt, T_L_R1, T_L_R2, concentrations):
    ...
    for j in range(12):
        ...
        denominator_term = (1 / (1 + (1 / (K_values * concentrations[j % 6])))) * Frac_lib_conc
        new_denominators[j] = np.sum(denominator_term)

    return new_denominators


def calculate_unbound_denominators(denominators, K_values, lambda_Li_R1_opt, lambda_Li_R2_opt, T_L_R1, T_L_R2, concentrations):
...
def neg_log_likelihood(params, n_sij_R1_i, n_sij_R2_i, n_Uij_R1_i, n_Uij_R2_i, n_L1, n_L2, 
    ...
    C = 10 ** params[0]  # Single C value for both replicates
    ...
    lambda_Sij_R1 = ((T_cj_R1 * lambda_Li_R1) / (denominators_R1 * T_L_R1)) * (1 / (1 + (1 / (C * concentrations))))
    lambda_Sij_R2 = ((T_cj_R2 * lambda_Li_R2) / (denominators_R2 * T_L_R2)) * (1 / (1 + (1 / (C * concentrations))))

    lambda_Uij_R1 = ((T_Uj_R1 * lambda_Li_R1) / (unbound_denominators_R1 * T_L_R1)) * (1 - (1 / (1 + (1 / (C * concentrations)))))
    lambda_Uij_R2 = ((T_Uj_R2 * lambda_Li_R2) / (unbound_denominators_R2 * T_L_R2)) * (1 - (1 / (1 + (1 / (C * concentrations)))))
...
def optimize_C_and_lambda_Li(merged_df_combined, T_cj, T_cj_R1, T_cj_R2, initial_denominators, 
    ...
    for iteration in range(max_iters):
        ...
        C_opt, lambda_Li_R1_opt, lambda_Li_R2_opt = [], [], []
        
        for i in range(len(merged_df_combined)):
            n_L1_i, n_L2_i = Counts_Lib_1[i], Counts_Lib_2[i]
            initial_params = np.array([np.log10(1.0), 1.0, 1.0])
            bounds = [(-6, 1), (0, None), (0, None)]
        ...
        merged_df_combined[["rsID", f"Fitted_K_iter{iteration+1}", 
                      ...
                      f"Fitted_lambda_R2_iter{iteration+1}"]].to_csv(
        ...
        denominators = update_denominators_withLambda(
            denominators, np.array(C_opt), np.array(lambda_Li_R1_opt), np.array(lambda_Li_R2_opt), 
            T_L_R1, T_L_R2, concentrations)
        
        # Update unbound denominators
        unbound_denominators = calculate_unbound_denominators(
            unbound_denominators, np.array(C_opt), np.array(lambda_Li_R1_opt), np.array(lambda_Li_R2_opt),
            T_L_R1, T_L_R2, concentrations)
...
denominators, unbound_denominators = optimize_C_and_lambda_Li(
    merged_df_combined, T_cj, T_cj_R1, T_cj_R2, initial_denominators, 
    initial_denominators_R1, initial_denominators_R2, concentrations, 
    Counts_Lib_1, Counts_Lib_2, max_iters=10)
