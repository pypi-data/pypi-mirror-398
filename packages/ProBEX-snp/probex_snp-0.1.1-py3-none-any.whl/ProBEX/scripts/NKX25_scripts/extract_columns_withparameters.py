...
Description: Script for processing rsID filtering data and applying specific filters based on threshold values. 
	...
	Functionality:
	1. Reads an input Excel file containing rsID filtering data and another Excel file with NKX25 labels.
	...
	4. Merges the filtered data with the NKX25 labels to include the '5/MPass' column.
	...
	- `--nkx25_file_path`: Path to the NKX25 labels Excel file.
	- `--output_file`: (Optional) Path to save the output Excel file (default: 'parameters.xlsx').

	Example Usage:
	python script_name.py --file_path rsID_filtering.xlsx --nkx25_file_path NKX25_labels.xlsx --output_file extracted_rsIDs.xlsx
"""
...
def main(file_path, nkx25_file_path, output_file):
    ...
    nkx25_df = pd.read_excel(nkx25_file_path)
    ...
    logging.info("Merging filtered data with NKX25 labels...")
    merged_df = pd.merge(extracted_df, nkx25_df[['rsID', '5/MPass']], on='rsID', how='left')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_path", default="./output/NKX25/rsID_filtering.xlsx", help="Path to the rsID filtering Excel file.")
    parser.add_argument("--nkx25_file_path", default="./output/NKX25_labels.xlsx", help="Path to the NKX25 labels Excel file.")
    parser.add_argument("--output_file", default="./output/NKX25/extracted_rsIDs.xlsx", help="Path to save the output Excel file.")

    args = parser.parse_args()
    main(args.file_path, args.nkx25_file_path, args.output_file)