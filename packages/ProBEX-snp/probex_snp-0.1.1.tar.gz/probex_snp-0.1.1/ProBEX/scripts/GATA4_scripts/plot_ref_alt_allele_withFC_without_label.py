...
df = pd.read_excel("./output/GATA4/fitting/updated_CORREL_with_bi.xlsx")
...
accepted_rsIDs = pd.read_csv("./output/GATA4_accepted_rsIDs.csv")
...
if merged.empty:
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig("./output/GATA4/fitting/ref_vs_alt_correlation_plot_without_label.png")
    plt.close()
    ...
    with pd.ExcelWriter("./output/GATA4/fitting/FC_table_with_bi.xlsx", engine="openpyxl") as writer:
...
file_path = "./output/GATA4/fitting/FC_table_with_bi.xlsx"
output_dir = "./output/GATA4/motif_affect_GATA4"
os.makedirs(output_dir, exist_ok=True)
sheet2 = pd.read_excel(file_path, sheet_name="Increased")
sheet3 = pd.read_excel(file_path, sheet_name="Decrease")
sheet2.iloc[:, 0].dropna().to_csv(os.path.join(output_dir, "GATA4increased_rsIDs.txt"), index=False, header=False)
sheet3.iloc[:, 0].dropna().to_csv(os.path.join(output_dir, "GATA4decreased_rsIDs.txt"), index=False, header=False)
end_time = time.time()
