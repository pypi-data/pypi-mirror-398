...
merged.to_csv("./output/NKX25/fitting/merged_scatter_ref_alt_NKX25_df.csv", index=False)
...
if merged.empty:
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig("./output/NKX25/fitting/ref_vs_alt_correlation_plot_without_label.png")
    plt.close()
    s = 0.05 * axis_max
    S = ((1 + np.sqrt(3)) / 2) * s
