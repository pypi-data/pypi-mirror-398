        ...
        case _:
            match value.split(":"):
                ...
                case ("public", url):
                    ...
                    value = getter.get_ip()
                case _:
                    logger.warning("Unsupportted value {:}, use TXT as default".format(value))
                    record_type = "TXT"
...
def load_config_from_path(path: Path) -> list[dict[str, Any]]:
    ...
    for domain_config in origin_configs.values():
        assert "domain" in domain_config, "Config must contain `domain`, config: {:}".format(domain_config)
        assert "setter_name" in domain_config, "Config must contain `setter_name`, config: {:}".format(domain_config)
