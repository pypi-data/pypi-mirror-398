...
# TODO: move fp8 conversion code here.
        ...
        def __load_weights(device_id: int, model_path: str) -> None:
            ...
            with torch.cuda.device(device_id):
                ...
                for layer_id in range(self.NUM_DENSE_LAYERS):
                    ...
                    # Yellow storm style hack
                    # Use empty tensor to save money if fp8 ops are disabled
                    ...
                    if self.enable_fp8_ops:
                        # Disabled for now
                        # upgate_all_reduce_fp8_params = gen_rmsnorm_up_gate_silu_fp8_params(
                        #     state_dicts[f"layer_{layer_id}_upgate_weights_dev_{device_id}"],
                        #     state_dicts[f"layer_{layer_id}_upgate_scales_dev_{device_id}"],
                        # )
                ...
                for layer_id in range(3, 3 + self.NUM_MOE_LAYERS):
                    ...
                    if self.enable_fp8_ops:
                        # Disabled for now
                        # expert_up_gate_fp8_params = gen_expert_select_up_gate_silu_fp8_params(
                        #     state_dicts[f"layer_{layer_id}_exp_upgate_weights_dev_{device_id}"],
                        #     state_dicts[f"layer_{layer_id}_exp_upgate_scales_dev_{device_id}"],
                        # )
