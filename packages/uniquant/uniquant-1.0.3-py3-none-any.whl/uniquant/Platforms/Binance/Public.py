...
class OneSymbole(SymbolIndex):
    ...
    async def _update_ob_snapshot(self):
        ...
        try:
            async with self.rest_session.get(url, params=params) as r:
                ...
                logger.debug("Get Orderbook Snapshot | Binance REST API >> Success")
    ...
    async def orderbook_stream(self,limit:int=5000):
        ...
        try:
            async with websockets.connect(url) as ws:
                logger.debug(f"WebSocket Orderbook | Binance WS API >> Connected to {url}")
    ...
    async def trades_stream(self):
        ...
        try:
            logger.debug(f"WebSocket Trades | Binance WS API >> Connected to {url}")
            async with websockets.connect(url) as ws:
                while True:
                    resp = await ws.recv()
                    msg = json.loads(resp)
    ...
    async def klines_stream(self, interval:str):
        ...
        try:
            async with websockets.connect(url) as ws:
                logger.debug(f"WebSocket Klines | Binance WS API >> Connected to {url}")
                while True:
                    resp = await ws.recv()
                    msg = json.loads(resp)
                    kline = msg['k']
    ...
    async def get_symbol_info(self):
        ...
        try:
            async with self.rest_session.get(url, params=params) as r:
                ...
                logger.debug("Get Symbol Info | Binance REST API >> Success")
    ...
    async def klines_rest(self, interval, limit = 1000, start_time = None, end_time = None):
        ...
        logger.debug("Fetching klines from Binance REST API...")
        params = {
        ...
        try:
            async with self.rest_session.get(url, params=params) as r:
                ...
                logger.debug("Historical Kilnes | Binance REST API >> Success")
    ...
    async def get_all_spot_symbols(self,_type_:str="short"):
        ...
        try:
            async with self.rest_session.get(url) as r:
                ...
                logger.debug("Get All Spot Symbols | Binance REST API >> Success")
