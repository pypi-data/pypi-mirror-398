...
#   filename:  tmp_jelyhrw.json
#   timestamp: 2025-12-27T00:14:20+00:00
...
class Event(BaseModel):
    ...
    model: str = Field(..., description='Model name used (if provided)')
    provider: str = Field(..., description='Provider name (if provided)')
    inputTokens: float = Field(..., description='Input tokens consumed (if provided)')
    outputTokens: float = Field(..., description='Output tokens generated (if provided)')
    totalTokens: float = Field(..., description='Total tokens for the event')
    latencyMs: float = Field(..., description='Request latency in milliseconds (if provided)')
    costCents: float = Field(..., description='Cost in cents (if provided)')
...
class UsageBreakdownItem(BaseModel):
    dimension: str = Field(..., description='Dimension value (e.g., model name, customer ID)')
...
class QuotasQuotaIdAlertsHistoryGetResponse(PaginationCursor):
