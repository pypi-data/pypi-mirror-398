...
from viiv.utils import load_json_file, load_package_json
...
class Config(dict):
    ...
    def __init__(self, config_path=None):
        ...
        if config_path is None:
            config_path = f"{os.getcwd()}/conf/config.json"
...
BUILT_IN_THEMES = PACKAGE['contributes']['themes']
...
logger.add(
    sys.stderr,
    level=CONFIG.options.get("log_level", "INFO"),
    colorize=True,
)
...
def convert_to_best_light_color(
    ...
    **kwargs,
    ...
    while not found_best_light_color:
        logger.trace(f"in while loop to find best_light_color, counter: {counter}, base_light_color: {base_light_color}, target_dark_color: {target_dark_color}")
        ...
        if is_too_dark:
            ...
            logger.trace(f"too_dark, contrast_ratio: {max(better_lighter_colors_contrast_ratios)}, {counter}: {len(better_light_colors)}, most_light_color: {most_light_color}, target_dark_color: {target_dark_color}, base_light_color: {base_light_color}")
        elif is_too_light:
            reversed_color_offset_rate /= counter
            ...
            logger.trace(f"too_light, contrast_ratio: {min(better_lighter_colors_contrast_ratios)}, {counter}: {len(better_light_colors)}, most_dark_color: {most_dark_color}, target_dark_color: {target_dark_color}, base_light_color: {base_light_color}")
        else:
            logger.trace(f"Bingo! contrast_ratio: {min(better_lighter_colors_contrast_ratios)} - {max(better_lighter_colors_contrast_ratios)}, min_contrast_ratio: {min_contrast_ratio}, max_contrast_ratio: {max_contrast_ratio}, base_light_color: {base_light_color}, target_dark_color: {target_dark_color}")

        better_lighter_colors_contrast_ratios = [
            C.calculate_contrast_ratio(x, target_dark_color) for x in better_light_colors
        ]
        ...
        if counter > MAX_ATTEMPTS_CONVERT_TO_BEST_COLOR:
            logger.warning(f"I tried {MAX_ATTEMPTS_CONVERT_TO_BEST_COLOR} times to find the best light color. However, I cannot find best light color in contrast ratio range {min_contrast_ratio}-{max_contrast_ratio} for {target_dark_color}. Use the best light colors I found {better_light_colors}.")
...
def convert_to_best_dark_color(
    ...
    **kwargs,
    ...
    while not found_best_dark_color:
        logger.trace(f"in while loop to find best_dark_color for {target_light_color} and base dark color is {base_dark_color}, counter: {counter}")
        ...
        if is_too_light:
            ...
            logger.trace(f"too_light, contrast_ratio: {contrast_ratio}, {counter}: {len(better_dark_colors)}, most_dark_color: {most_dark_color}, target_light_color: {target_light_color}, base_dark_color: {base_dark_color}, min_contrast_ratio: {min_contrast_ratio}, max_contrast_ratio: {max_contrast_ratio}")
        elif is_too_dark:
            reversed_color_offset_rate *= counter
            ...
            logger.trace(f"too_dark, contrast_ratio: {contrast_ratio}, min_contrast_ratio: {min_contrast_ratio}, max_contrast_ratio: {max_contrast_ratio}, {counter}: {len(better_dark_colors)}, min(better_lighter_colors_contrast_ratios) is {min(better_dark_colors_contrast_ratios)}, max(better_lighter_colors_contrast_ratios) is {max(better_dark_colors_contrast_ratios)}")
        else:
            logger.debug(f"Bingo! contrast_ratio: {contrast_ratio}, min_contrast_ratio: {min_contrast_ratio}, max_contrast_ratio: {max_contrast_ratio}, {counter}: {len(better_dark_colors)}")

        better_dark_colors_contrast_ratios = list(
            map(lambda x: C.calculate_contrast_ratio(target_light_color, x), better_dark_colors)
        )
        filtered_better_colors = list(
            filter(
        ...
        counter += 1
        if counter > MAX_ATTEMPTS_CONVERT_TO_BEST_COLOR:
            logger.warning(f"I tried {MAX_ATTEMPTS_CONVERT_TO_BEST_COLOR} times to find the best light color. However, I cannot find best light color in contrast ratio range {min_contrast_ratio}-{max_contrast_ratio} for {target_light_color}. Use the best light colors I found {better_dark_colors}.")
            filtered_better_colors = better_dark_colors
            break
        logger.trace(f"{counter}: {len(better_dark_colors)}, {len(filtered_better_colors)}")
...
def _generate_random_theme_file(
    ...
    **kwargs,
    ...
    if workbench_editor_background_color:
        ...
        # editor background
        logger.info(
            ("workbench_editor_background_color: ", workbench_editor_background_color, " >> workbench_editor_foreground_color:", workbench_editor_foreground_color)
        )
...
def generate_themes(target_theme=None, themes=CONFIG.config["themes"]):
    ...
    for theme_config in themes:
        theme_mode = theme_config.get("theme_mode", theme_config.get("uiTheme", CONFIG.options.get("theme_mode")))
        if theme_mode:
...
def generate_random_theme(workbench_base_background_color: str = None):
    """
    ...
    workbench_base_background_color = (
        workbench_base_background_color or CONFIG.options.get("workbench_base_background_color")
    )
