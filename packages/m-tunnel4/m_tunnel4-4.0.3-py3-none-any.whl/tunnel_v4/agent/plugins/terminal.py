...
class TerminalPlugin(Plugin):
    ...
    async def _handle_input(self, session_id: str, data: bytes):
        ...
        if data == b'\x00' or data == b'\\x00' or (isinstance(data, str) and data in ['\x00', '\\x00']):
            self.logger.info(f"Heartbeat received for session {session_id}")  # 改为info级别便于观察
            return
        ...
        try:
            while session_id in self.sessions:
                try:
                    ...
                    if not data:
                        # EOF - shell已退出
                        self.logger.info(f"Shell exited (EOF), sent {send_count} messages")
                        break
                    
                    # 发送给客户端
                    send_count += 1
                    self.logger.info(f"PTY output #{send_count}: {len(data)} bytes")
                    ...
                    if e.errno == 5:  # Input/output error
                        self.logger.info(f"Shell exited (PTY closed), sent {send_count} messages")
                    else:
                        self.logger.error(f"PTY read error: {e}")
                    break
                    
        except Exception as e:
            self.logger.error(f"Error reading from PTY: {e}, sent {send_count} messages")
        finally:
            await self._cleanup_session(session_id)
