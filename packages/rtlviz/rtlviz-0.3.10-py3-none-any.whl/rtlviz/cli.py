...
Usage:
    ...
    rtlviz generate /path/to/verilog --output diagram.html --llm
"""
...
from .llm_analyzer import enhance_instances, is_llm_available
...
def cmd_generate(args):
    ...
    # Enhance with LLM (default) unless --no-llm flag is set
    top = modules.get(top_module)
    use_llm = not args.no_llm
    if top and use_llm:
        if is_llm_available():
            print("🤖 Enhancing with LLM analysis...")
            enhance_instances(top.instances, modules, use_llm=True)
        else:
            print("⚠️  LLM not available, using pattern-based detection")
    elif top:
        print("⚡ Using pattern-based detection (--no-llm)")
    
    # Generate DOT with validation loop
    title = args.title or f"{top_module} Block Diagram"
    dot_code = generate_dot(modules, top_module, title)
    
    # Validate diagram quality
    try:
        from .diagram_validator import validate_diagram, should_regenerate
    ...
    # Telemetry
    ping_cli_generate(len(modules), use_llm)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  rtlviz generate ./src -o diagram.html            # Uses LLM by default
  rtlviz generate ./src -o diagram.html --no-llm   # Pattern-based only (faster)
  rtlviz setup                                     # Configure all detected IDEs
    ...
    gen_parser.add_argument("--no-llm", action="store_true", help="Disable LLM, use pattern-based detection (faster)")
