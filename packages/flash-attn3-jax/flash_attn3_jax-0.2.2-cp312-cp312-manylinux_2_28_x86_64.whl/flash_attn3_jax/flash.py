...
from .flash_sharding import _flash_mha_bwd_hlo_sharded, _flash_mha_fwd_hlo_sharded
...
mlir.register_lowering(
    _flash_mha_fwd_p, mlir.lower_fun(_flash_mha_fwd_hlo_sharded), platform="gpu"
)

mlir.register_lowering(
    _flash_mha_bwd_p, mlir.lower_fun(_flash_mha_bwd_hlo_sharded), platform="gpu"
)
...
def mha_fwd_batch(vector_arg_values, batch_axes, **kwargs):
    ...
    if mapped == (True, True, True):
        ...
        def unsquish(val):
            return einops.rearrange(val, "(x n) ... -> x n ...", x=x)
