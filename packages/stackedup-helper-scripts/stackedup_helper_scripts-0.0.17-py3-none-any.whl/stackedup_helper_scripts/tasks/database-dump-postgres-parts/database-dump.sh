...
# database-dump.sh is a script to create on demand postgres database dumps from
# an ECS service. When the database dump task is request this script will call
# pg_dump to create a custom compressed dump file and be uploaded to an
...
export FILE_NAME="${APPLICATION_NAME}-${ENVIRONMENT_TYPE}-$(date +%F_%H-%M-%S).backup"
...
echo "pg_dump -h ${DB_HOST} -p ${DB_PORT} -Fc -Z2 -U ${DB_USER} ${DB_DB} > /tmp/${FILE_NAME}"

pg_dump -h ${DB_HOST} -p ${DB_PORT} -Fc -Z2 -U ${DB_USER} ${DB_DB} > "/tmp/${FILE_NAME}"
