        ...
        if self.counter >= 1.0:
            ...
            for i, item in enumerate(self.active_chars):
                if i == 0:
                    ...
                    item[2] = 1 # Dark Green Fade (Pair 1 is Dark Green in sexy mode logic? No wait.)
                    # Check setup_colors:
                    # Pair 1 = 22 (Dark Green)
                    # Pair 2 = Custom Sushi
                    # Pair 3 = 46 (Bright Green)
                    item[2] = 1 # Pair 1 is Dark Green
        ...
        for y, char, br in self.active_chars:
            if 0 <= y < self.height:
                try:
                    ...
                    elif br == 4: # Bright Green (Sparkle) - NEW MAPPING internal 4 -> curses 3
                         attr = curses.color_pair(3) | curses.A_BOLD
...
def main(stdscr, args):
    ...
    curses.curs_set(0)
    # Use timeout for non-blocking wait (approx 50ms = 20FPS)
    stdscr.timeout(50)
    curses.start_color()
    ...
    def create_columns():
        ...
        for x in range(0, sw, step):
            if random.random() < args.mix:
                cols.append(QuoteColumn(x, sh, DEFAULT_CHARSETS['matrix'], args.speed, args.horror, 
                                        quote_manager, args.persist, args.glitch_rate, args.tail_min, args.tail_max))
            else:
                cols.append(Column(x, sh, DEFAULT_CHARSETS['matrix'], args.speed, args.tail_min, args.tail_max))
        return cols
