...
class CacheInvalidator:
    ...
    def _perform_invalidation_async_safe(
        ...
        try:
            ...
            # We're in an async context - schedule the async invalidation
            asyncio.create_task(self._perform_invalidation_async(keys, patterns))
            # Return 0 since we can't know the result synchronously
            # The actual invalidation will complete asynchronously
            logger.debug(
                f"Scheduled async cache invalidation for {len(keys)} keys and "
            ...
            try:
                return asyncio.run(self._perform_invalidation_async(keys, patterns))
            except RuntimeError as e:
                # Can't run async in this context (e.g., nested event loops)
                logger.warning(
                    f"Cannot run async cache invalidation in sync context: {e}. "
