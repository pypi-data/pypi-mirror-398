...
from android_use.agent.views import AgentHistoryList
...
def create_history_gif(
	...
	# Get both highlighted (element) and action-highlighted screenshots
	highlighted_screenshots = history.highlighted_screenshots()
	action_screenshots = history.screenshots()

	if not highlighted_screenshots:
	...
	for screenshot in highlighted_screenshots:
		if screenshot:
	...
	if show_task and task:
		...
		for item in history.history:
			screenshot_b64 = item.state.highlight_screenshot
			if screenshot_b64:
	...
	# Process each history item with both highlighted and action screenshots
	for i, (item, highlight_screenshot, action_screenshot) in enumerate(
		zip(history.history, highlighted_screenshots, action_screenshots), 1
	):
		# First, add the highlighted screenshot (element highlights)
		if highlight_screenshot:
			# Convert base64 screenshot to PIL Image
			img_data = base64.b64decode(highlight_screenshot)
			...
			if show_goals and item.model_output:
				image = _add_overlay_to_image(
					image=image,
					step_number=i,
					goal_text=item.model_output.think if item.model_output.think else '',
		...
		# Then, add the action-highlighted screenshot
		if action_screenshot:
			# Convert base64 screenshot to PIL Image
			img_data = base64.b64decode(action_screenshot)
			image = Image.open(io.BytesIO(img_data))
...
def _add_overlay_to_image(
	...
	if display_step:
		...
		y_step = image.height - 2*margin - step_height - 10  # Slight offset from bottom
...
def _wrap_text(text: str, font: ImageFont.FreeTypeFont, max_width: int) -> str:
	...
	Supports both English (word-based wrapping) and Chinese (character-based wrapping).
	...
	has_cjk = any(_is_cjk_character(char) for char in text)
	
	if not has_cjk:
		# Use word-based wrapping for non-CJK text (original logic)
		words = text.split()
		lines = []
		current_line = []
		...
		# Use character-based wrapping for CJK text
		lines = []
		current_line = ''
		...
		while i < len(text):
			...
			# If it's a CJK character, add it character by character
			if _is_cjk_character(char):
				...
				# For non-CJK characters (like English words, numbers, punctuation),
				# try to keep them together as a word
				word_start = i
				while i < len(text) and not _is_cjk_character(text[i]) and text[i] not in (' ', '\n'):
					i += 1
