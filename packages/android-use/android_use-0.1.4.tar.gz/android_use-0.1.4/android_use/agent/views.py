...
from android_use.android.views import AndroidStateHistory, DOMElementNode, DOMHistoryElement
from android_use.tools.registry.views import ActionModel
from android_use.android.views import SelectorMap
...
class AgentOutput(BaseModel):
    ...
    def model_json_schema(cls, **kwargs):
        ...
        schema['required'] = ['think', 'evaluation_previous_goal', 'action']
        return schema
...
class AgentHistory(BaseModel):
    ...
    def get_interacted_element(model_output: AgentOutput, selector_map: SelectorMap) -> list[DOMHistoryElement | None]:
        elements = []
        # model_output.action is already a list
        for action in model_output.action:
            index = action.get_index()
            if index and index in selector_map:
                el: DOMElementNode = selector_map[index]
                elements.append(el.convert_dom_element_to_history_element(index))
            else:
...
class AgentHistoryList(BaseModel):
    ...
    def highlighted_screenshots(self) -> list[str | None]:
        """Get all screenshots from history"""
        return [h.state.highlight_screenshot if h.state.highlight_screenshot is not None else None for h in self.history]
