...
class BucketObjectsApi:
    ...
    def list_objects_buckets(
        ...
        This endpoint lists objects in a bucket with pagination, filtering, and sorting.      **Filtering**: Use dot notation for metadata fields     - Example: ?metadata.type=video&metadata.status=ready      **Sorting**: Specify field and direction     - Example: ?sort_field=metadata.created_at&sort_direction=desc     - Direction: asc (ascending) or desc (descending), defaults to asc      **Pagination**: Use limit and offset     - Example: ?limit=20&offset=40
