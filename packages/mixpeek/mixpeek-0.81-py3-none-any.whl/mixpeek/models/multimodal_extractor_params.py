...
class MultimodalExtractorParams(BaseModel):
    """
    Parameters for the multimodal extractor.  The multimodal extractor processes video, image, text, and GIF content in a unified embedding space. Videos/GIFs are decomposed into segments with transcription, visual analysis, OCR, and embeddings. Images and text are embedded directly without decomposition.  **When to Use**:     - Video content libraries requiring searchable segments     - Media platforms with search across spoken and visual content     - Educational content with lecture videos and demonstrations     - Surveillance/security footage requiring event detection     - Social media platforms with user-generated video content     - Broadcasting/streaming services with large video catalogs     - Training video repositories with instructional content     - Marketing/advertising analytics for video campaigns  **When NOT to Use**:     - Static image collections → Use image_extractor instead     - Audio-only content → Use audio_extractor instead     - Very short videos (<5 seconds) → Overhead not worth it     - Real-time live streams → Use specialized streaming extractors     - Extremely high-resolution videos (8K+) → Consider downsampling first  **Decomposition Methods**:      | Method | Use Case | Accuracy | Segments/Min | Best For |     |--------|----------|----------|--------------|----------|     | **TIME** | Fixed intervals | N/A | 60/interval_sec | General purpose, consistent chunking |     | **SCENE** | Visual changes | 85-90% | Variable (2-20) | Movies, dynamic content, shot changes |     | **SILENCE** | Audio pauses | 80-85% | Variable (5-30) | Lectures, presentations, conversations |  **Feature Extraction Options**:     - Transcription: Speech-to-text using Whisper (95%+ accuracy)     - Multimodal Embeddings: Unified embeddings from Vertex AI (1408D) for video/image/gif/text     - Transcription Embeddings: Text embeddings from E5-Large (1024D)     - OCR: Text extraction from video frames using Gemini Vision     - Descriptions: AI-generated segment summaries using Gemini     - Thumbnails: Visual preview images for each segment  **Performance Characteristics**:     - Processing Speed: 0.5-2x realtime (depends on features enabled)     - Example: 10min video → 5-20 minutes processing time     - Transcription: ~200ms per second of audio     - Visual Embedding: ~50ms per segment     - OCR: ~300ms per segment     - Description: ~2s per segment (if enabled)  **Cost Estimates** (per minute of video):     - Minimal (transcription + embeddings): $0.01     - Standard (+ OCR): $0.05     - Full (+ descriptions): $0.15  Requirements:     - video URL: REQUIRED (accessible video file)     - All feature parameters: OPTIONAL (defaults provided)
    """ # noqa: E501
