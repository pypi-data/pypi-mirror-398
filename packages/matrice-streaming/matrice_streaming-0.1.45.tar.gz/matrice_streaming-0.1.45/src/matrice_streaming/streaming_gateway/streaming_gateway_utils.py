...
class StreamingGatewayUtil:
    ...
    def get_input_streams(self) -> List[InputStream]:
        ...
        # Fetch simulated URLs concurrently
        stream_url_lookup = {}
        if file_cameras:
            max_workers = min(len(file_cameras), 100)  # Max 100 concurrent requests
            start = time.time()
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_camera = {
                    executor.submit(self.get_simulated_stream_url, c["id"]): c["id"]
                    for c in file_cameras
                }
                for future in as_completed(future_to_camera):
                    ...
                    try:
                        ...
                        if result and result.get("url"):
                            stream_url_lookup[camera_id] = result["url"]
                    except Exception as e:
                        logging.warning(f"Failed to get stream URL for camera {camera_id}: {e}")

            urls_time = time.time() - start
            logging.info(
                f"Concurrent fetch of {len(file_cameras)} stream URLs took {urls_time:.3f}s "
