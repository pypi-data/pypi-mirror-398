...
## 元素查找
- 优先根据元素文本内容、图标、位置等特征匹配目标元素
- 若文本内容模糊或不完整，可结合上下文、元素类型辅助定位
- 元素信息上下文中包含每个元素上、下、左、右相邻的元素id信息，可结合用户指令找到相邻的目标元素，比如 left_elem_ids: [1, 2, 3]，"点击xx元素左测的第一个元素"，则目标元素是id为1的元素
    - left_elem_ids -> 左侧相邻的元素id列表
    - right_elem_ids -> 右侧相邻的元素id列表
    - top_elem_ids -> 上方相邻的元素id列表
    - bottom_elem_ids -> 下方相邻的元素id列表
