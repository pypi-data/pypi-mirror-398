...
class CustomGenericOAuthenticator(GenericOAuthenticator):
    ...
    async def refresh_user(self, user, handler=None):
        ...
        if self.auth_refresh_age > rest_time:
            ## Infraproxy: do not refresh access token. Currently not working as expected
            if self.revocation_type.lower() == "infraproxy":
                self.log.debug(
                    f"{user.name} - Infraproxy configured. Do not refresh access token."
                )
                return True
