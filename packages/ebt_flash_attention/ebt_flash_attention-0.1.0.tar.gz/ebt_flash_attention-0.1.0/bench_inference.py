...
def parse_args():
    parser = argparse.ArgumentParser(
        description="EBT Inference Benchmark: Standard vs Flash Attention"
    )
...
def run_step(energy_net, x, p_init, p_target, n_steps, alpha, use_flash_attention, causal, mask_type):
    """
    Executes one full iteration (MCMC loop + Backward).
    ...
    with torch.set_grad_enabled(True):
        for mcmc_step in range(n_steps):
            ...
            grad_p = torch.autograd.grad(energy.sum(), p_current, create_graph=False)[0]
    ...
    try:
        ...
        if use_flash_attention and warmup_iters > 0:
            for _ in range(warmup_iters):
                run_step(energy_net, x, p_init, p_target, n_steps, alpha, use_flash_attention, causal, mask_type)
        ...
        for run_idx in range(num_runs):
            ...
            L = run_step(energy_net, x, p_init, p_target, n_steps, alpha, use_flash_attention, causal, mask_type)
...
def benchmark_sequence_length(seq_len, config, args_template):
    ...
    flash_nc_result = test_inference(
        energy_net, x, p_init, p_target, config["n_steps"], config["alpha"],
...
def save_results_csv(all_results, filename):
    ...
    output_dir = "benchmark/inference"
