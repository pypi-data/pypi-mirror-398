...
from .keys import Provider, get_default_provider, get_provider_key
from .llm import (
    LLMProvider,
    ModelType,
    create_llm_client,
...
def get_provider_config() -> tuple[LLMProvider, str]:
    ...
    Priority:
    1. Default provider (if set and has key)
    2. First available provider with key
    ...
    if default and (key := get_provider_key(default)):
        return provider_map[default], key
...
def interactive_selection(message: CommitMessage, existing: str = "") -> str | None:
    """
    ...
    console.print(
        Panel.fit(
            "🤖 [bold cyan]AI Commit Message Generator[/bold cyan]", border_style="cyan"
        )
    )
    ...
    while True:
        ...
        if choice == "e":
            custom: str = Prompt.ask("   Enter custom commit message")
            if custom.strip():
                return custom.strip()
            continue
    ...
    try:
        ...
        if apply:
            ...
            if not commit_msg_file.exists():
                error_console.print("[red]Not in a git repository[/red]")
                raise typer.Exit(1)
            ...
            if selected:
                ...
                console.print(
                    "[green]✅ Message written to .git/COMMIT_EDITMSG[/green]"
                )
            return
