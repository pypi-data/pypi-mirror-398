...
def encrypt_keystore(keystore: KeyStore, password: str) -> EncryptedKeyStore:
    ...
    return EncryptedKeyStore(
        version=2, salt=salt.hex(), nonce=nonce.hex(), ciphertext=ciphertext.hex()
    )
...
def decrypt_keystore(encrypted: EncryptedKeyStore, password: str) -> KeyStore:
    ...
    try:
        ...
        raise ValueError("Invalid password or corrupted keystore") from err
...
def write_cached_password(password: str, ttl_seconds: int = DEFAULT_TTL):
    ...
    data = {"password": password, "expires_at": int(time.time()) + ttl_seconds}
...
def load_keystore() -> KeyStore:
    ...
    if password:
        ...
        except (KeyError, ValueError, json.JSONDecodeError, OSError, TypeError):
...
def set_provider_key(provider: Provider, value: str | None = None):
    """
...
def list_providers() -> dict[Provider, bool]:
    ...
    return {
        provider: (
            PROVIDER_KEYS[provider] in keystore.keys
            or f"{provider.name}_API_KEY" in keystore.keys
        )
...
DEFAULT_PROVIDER_KEY = "_default_provider"


def set_default_provider(provider: Provider):
    """
    ...
    """
    keystore = load_keystore()
    keystore.keys[DEFAULT_PROVIDER_KEY] = provider.value
    save_keystore(keystore)
    print(f"✓ Default provider set to {provider.value}")
...
def get_default_provider() -> Provider | None:
    ...
    Returns:
        The default provider if set, None otherwise
    """
    try:
        keystore = load_keystore()
    except FileNotFoundError:
    ...
    value = keystore.keys.get(DEFAULT_PROVIDER_KEY)
    if value:
        for p in Provider:
            if p.value == value:
                return p
