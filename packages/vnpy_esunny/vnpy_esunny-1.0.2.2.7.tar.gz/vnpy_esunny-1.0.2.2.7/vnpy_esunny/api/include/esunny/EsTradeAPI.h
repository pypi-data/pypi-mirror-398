...
public:
	...
	*/
    ...
    /**
     * @brief 设置备用登录地址(北斗星专用)
     * @param pAddress 登录地址
     * @return 0 设置成功，非0 错误码
     * 
     * @ingroup G_T_Login
     */
    virtual TAPIINT32 ES_CDECL SetBackUpAddress(const TAPISTR_20 UserNo, const TapAPIBackUpAddress *pAddress) = 0;
    /**
     * @brief 身份验证请求(北斗星，股票专用)
     * @details 当客户忘记密码时，可以自行重置密码，第一步就需要进行身份验证。
     * @param UserNo 身份验证用户
     * @param pIdentity 验证信息
     * @return 0 调用成功，非0 错误码
     * 
     * @ingroup G_T_Login
     */
	...
	*/
	...
	/**
	* @brief	请求发送二次认证认证码(北斗星，股票专用)
	* @details	根据回掉函数OnRtnContactInfo中的联系方式，选择其中一个请求二次认证码，
	*			收到正确应答后可以通过SetVertificateCode 设置二次认证授权码完成登陆过程。
	* @param[in]	UserNo 登录用户UserNo
	* @param[in] nSessionID 本次请求的会话ID
	* @param[in]  pReqInfo 要接收二次认证的邮箱或者电话
	* @attention 该函数调用需要在登陆应答后返回10003错误，API会回调客户二次认证的联系方式，用户选择其中一个联系方式（邮箱或者电话）请求二次认证。
	*    再通过设置二次认证授权码完成登陆。
	* 
	* @ingroup G_T_Login
	*/
	...
	*/
	...
	/**
	* @brief 获取用户下属的所有资金账号。从SeqID开始查询，每次最多返回nOutLen条。如果没有结束，需要下次继续从查询到的最后一个数据的SeqID+1继续查询。
	...
	*/
	...
	/**
	* @brief 获取指定用户下的所有交易所信息。从SeqID开始，每次最多返回nOutLen条。如果没有结束，需要下次继续从查询到的最后一个数据的SeqID+1继续查询。
	...
	* @param[out] isLast 标示满足本次查询的应答信息是否填充完毕
	* @retval retval>=0 本次查询到的符合条件的应答信息的总个数，retval可以作为续查时的一个依据; retval<0 本次查询失败，表示错误码
	* @operationtype 同步操作(Synchronous operation)
	* 
	* @ingroup G_T_TradeSystem
	*/
	...
	/**
	* @brief 获取指定用户下的所有品种信息。从SeqID开始，每次最多返回nOutLen条。如果没有结束，需要下次继续从查询到的最后一个数据的SeqID+1继续查询。
	...
	* @details TapAPIFundReq需要填写资金账号。TapAPIFundReq还需要填写查询数据的起始流号。
	...
	* @param[in] UserNo 登录用户UserNo
	* @param[in] nRequestID 请求ID
	* @param[in] pReqInfo 查询深度行情的请求结构体指针
	* @retval 0 请求成功；非0 错误码
	* @operationtype 异步操作
	* 
	* @ingroup G_T_DeepQuote
	...
	*/
	...
	/**
	* @brief 获取指定用户下所有的币种的信息。从SeqID开始查询，每次最多返回100条，如果没有结束，需要下次继续从查询到的最后一个数据的SeqID继续查询（北斗星、股票专用）
	...
	*/
    ...
    /**
	* @brief 客户特殊业务委托下单(ETF专用)
	* @details 用户下单的操作函数。用户的下单操作会造成用户的资金、组合持仓数据的变化，所以用户下的单成交后，会有多个回调通知来向用户展示数据的变化。
	* @param[in] UserNo 登录用户UserNo
	* @param[in] nRequestID 请求ID
	* @param[in] pOrder 新的委托
	* @retval 0 请求成功;非0 错误码
	...
	*/
    ...
    /**
	* @brief 获取指定用户下指定条件的当前最新委托信息。(ETF专用)
	* @details 此函数查询的是本地的查询,用户和后台断开重连后会清空本地数据。
	* @param[in] UserNo 登录用户UserNo
	* @param[in] pReqInfo 查询客户委托请求的结构体指针
	* @param[in] pOutInfo 本次查询应答信息指针数组的起始地址
	...
	*/
    ...
    /**
	* @brief 查询用户信任设备信息(北斗星、股票专用)
	* @param[in] UserNo 登录用户UserNo
	* @param[in] nRequestID 请求ID
	* @param[in] info 用户信任设备查询请求结构体
	* @retval 0 请求成功；非0 错误码
	* @operationtype 异步操作
	* 
	* @ingroup G_T_UserInfo
	*/
    ...
    /**
	* @brief 增加用户信任设备信息(北斗星、股票专用)
	* @param[in] UserNo 登录用户UserNo
	* @param[in] nRequestID 请求ID
	* @param[in] info 用户信任设备增加请求结构体
	* @retval 0 请求成功；非0 错误码
	* @operationtype 异步操作
	* 
	* @ingroup G_T_UserInfo
	*/
    ...
    /**
	* @brief 删除用户信任设备信息(北斗星、股票专用)
	* @param[in] UserNo 登录用户UserNo
	* @param[in] nRequestID 请求ID
	* @param[in] info 用户信任设备删除请求结构体
	* @retval 0 请求成功；非0 错误码
	* @operationtype 异步操作
	* 
	* @ingroup G_T_UserInfo
	*/
    ...
    /**
	* @brief 获取阶梯最小变动价。（股票后台专用）
	* @details 此函数执行的是本地的查询,运行中不会删除已经从后台获取到的信息数据
	* @param[in] UserNo 登录用户UserNo
	* @param[in] nDataSeqID 起始查询数据编号（1表示第一个数据，本次查询包含这个编号的数据）
	* @param[in] pOutInfo 本次查询应答信息指针数组的起始地址
	* @param[in] nOutLen 本次查询应答信息指针数组的长度
    ...
    * @param[in] UserNo 登录用户UserNo
    * @param[in] nRequestID 请求ID
    * @param[in] pReqInfo 柜台配置文件查询请求结构
    * @retval 0 请求成功；非0 错误码
	* @operationtype 异步操作
	* 
	* @ingroup G_T_TradeSystem
