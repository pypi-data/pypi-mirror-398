...
class List:
    """
    Lists provide a data structure for retrieving multiple Objects (Devices, Signals,
    Signal Instances, or Maps) as a result of a query.
    """
    ...
    def __init__(self, ref, status):
        if status is not None:
            ...
            self._idx = mpr.mpr_sig_get_num_inst(ref, status)
            self._ref = ref
        else:
            mpr.mpr_list_get_cpy.argtypes = [c_void_p]
            mpr.mpr_list_get_cpy.restype = c_void_p
            self._ref = mpr.mpr_list_get_cpy(ref)
        self._status = status
    ...
    def __del__(self):
        if self._status is None:
            ...
            mpr.mpr_list_free(self._ref)
            self._ref = None

    def __contains__(self, obj):
        if self._status is None and self._ref and obj and isinstance(obj, Object):
            # use a copy
            cpy = List(mpr.mpr_list_get_cpy(self._ref), None)
            for _obj in cpy:
                if _obj == obj:
                    return True
        return False
    ...
    def _objectify(ptr):
        ...
        if _type == Type.DEVICE:
            ...
            print("libmapper.List error: object is not a Device, Signal, Signal Instance, or Map")
            return None
    ...
    def next(self):
        ...
        if self._status:
            ...
            self._idx -= 1
            if self._idx >= 0:
                mpr.mpr_sig_get_inst_id.argtypes = [c_void_p, c_int, c_int, c_void_p]
                mpr.mpr_sig_get_inst_id.restype = c_int
                inst_status = mpr.mpr_sig_get_inst_id(self._ref, self._idx, self._status, byref(id))
                if inst_status != 0:
                    return Signal(self._ref).Instance(id.value)
            raise StopIteration
        elif self._ref:
            # self._ref is the address of result, need to dereference
            result = cast(self._ref, POINTER(c_void_p)).contents.value
            mpr.mpr_list_get_next.argtypes = [c_void_p]
            mpr.mpr_list_get_next.restype = c_void_p
            self._ref = mpr.mpr_list_get_next(self._ref)
    ...
    def filter(self, key_or_idx, val, op=Operator.EQUAL):
        ...
        if self._status:
            # filtering instance lists is not currently supported
            return self
        ...
        if _type is int:
            if key_or_idx == Property.ID or key_or_idx == 'id':
                # use int64
                self._ref = mpr.mpr_list_filter(self._ref, idx, key, 1, Type.INT64, byref(c_longlong(val)), op)
            else:
                self._ref = mpr.mpr_list_filter(self._ref, idx, key, 1, Type.INT32, byref(c_int(val)), op)
        elif _type is bool:
            # convert value to int
            val = val * 1
            self._ref = mpr.mpr_list_filter(self._ref, idx, key, 1, Type.INT32, byref(c_int(val)), op)
        elif _type is float:
            self._ref = mpr.mpr_list_filter(self._ref, idx, key, 1, Type.FLOAT, byref(c_float(val)), op)
        elif _type is str:
            self._ref = mpr.mpr_list_filter(self._ref, idx, key, 1, Type.STRING, c_char_p(val.encode('utf-8')), op)
        else:
    ...
    def join(self, rhs):
        ...
        if self._status:
            # joining instance lists is not currently supported
            return self
    ...
    def intersect(self, rhs):
        ...
        if self._status:
            # intersecting instance lists is not currently supported
            return self
        ...
        self._ref = mpr.mpr_list_get_isect(self._ref, cpy)
        return self
    ...
    def subtract(self, rhs):
        ...
        if self._status:
            # subtracting instance lists is not currently supported
            return self
        ...
        self._ref = mpr.mpr_list_get_diff(self._ref, cpy)
        return self
    ...
    def __getitem__(self, index):
        ...
        if self._status:
            if index < 0:
                mpr.mpr_sig_get_num_inst.argtypes = [c_void_p, c_int]
                mpr.mpr_sig_get_num_inst.restype = c_int
                index += mpr.mpr_sig_get_num_inst(self._ref, self._status)
            if index >= 0:
                id = c_int()
                mpr.mpr_sig_get_inst_id.argtypes = [c_void_p, c_int, c_int, c_void_p]
                mpr.mpr_sig_get_inst_id.restype = c_int
                inst_status = mpr.mpr_sig_get_inst_id(self._ref, index, self._status, byref(id))
                if inst_status != 0:
                    return Signal(self._ref).Instance(id.value)
        elif self._ref:
            if index < 0:
    ...
    def __len__(self):
        ...
        if self._status:
            mpr.mpr_sig_get_num_inst.argtypes = [c_void_p, c_int]
            mpr.mpr_sig_get_num_inst.restype = c_int
            return mpr.mpr_sig_get_num_inst(self._ref, self._status)
        else:
            mpr.mpr_list_get_size.argtypes = [c_void_p]
            mpr.mpr_list_get_size.restype = c_int
            return mpr.mpr_list_get_size(self._ref)
...
class Object:
    ...
    class Event(IntFlag):
        """Possible events for a libmapper object, used to register and inform callbacks"""
        ...
        def __str__(self):
            if self._name_ is not None:
                return self._name_
            return '|'.join([str(m._name_ or m._value_) for m in self.__class__ if m.value & self.value])
    ...
    def get_status(self, clear_volatile=False):
        """
        Get the status of an object

        Args:
            clear_volatile (boolean): True to clear volatile status bits, False otherwise

        Returns:
            status flags for this object.
        """

        mpr.mpr_obj_get_status.argtypes = [c_void_p, c_int]
        mpr.mpr_obj_get_status.restype = c_int
        return Status(mpr.mpr_obj_get_status(self._obj, int(clear_volatile)))
    ...
    def print(self, properties=0):
        """Print information about an object."""

        mpr.mpr_obj_print.argtypes = [c_void_p, c_int]
        mpr.mpr_obj_print(self._obj, properties)
        return self
...
def signal_cb_py(_sig, _evt, _inst, _len, _type, _val, _time):
    ...
    cb(Signal(_sig), Object.Event(_evt), _inst, val, Time(_time))
...
class Signal(Object):
    ...
    def set_callback(self, callback, events=Object.Event.ANY):
        """
        Set or unset the message handler for a signal.

        Args:
            callback: a function to be called when the Signal is updated.
            events (bitflags: libmapper.Object.Event): The type(s) of events that will trigger the
                callback.
    ...
    def get_status(self, clear_volatile=False):
        ...
        Args:
            clear_volatile (boolean): True if volatile status bits should be cleared, False otherwise
        ...
        if self._is_sig_inst:
            mpr.mpr_sig_get_inst_status.argtypes = [c_void_p, c_longlong, c_int]
            mpr.mpr_sig_get_inst_status.restype = c_int
            clear_volatile = int(clear_volatile)
            return Status(mpr.mpr_sig_get_inst_status(self._obj, self.id, clear_volatile))
        else:
            return super().get_status(clear_volatile)

    def num_instances(self, status=Object.Status.ANY):
        """
        Get the number of instances for this Signal.

        Args:
            status (libmapper.Object.Status): The statuses of the instances to search for.
    ...
    def instances(self, status=Object.Status.ANY):
        """
        Get a list of Signal Instances matching a given status.
    ...
    def maps(self, direction=Direction.ANY):
        ...
        return List(mpr.mpr_sig_get_maps(self._obj, direction), None)
...
class Map(Object):
    ...
    def signals(self, location=Location.ANY):
        ...
        return List(mpr.mpr_map_get_sigs(self._obj, location), None)

    def index(self, signal, location):
        """
        ...
        if not isinstance(signal, Signal):
            ...
            return None
        mpr.mpr_map_get_sig_idx.argtypes = [c_void_p, c_void_p, c_int]
        mpr.mpr_map_get_sig_idx.restype = c_int
        idx = mpr.mpr_map_get_sig_idx(self._obj, signal._obj, location)
        return idx
...
def graph_cb_py(_graph, c_obj, evt, user):
    ...
    if _type == Type.DEVICE:
        for f in graph_dev_cbs:
            f(Type.DEVICE, Device(c_obj), Object.Event(evt))
    elif _type == Type.SIGNAL:
        for f in graph_sig_cbs:
            f(Type.SIGNAL, Signal(c_obj), Object.Event(evt))
    elif _type == Type.MAP:
        for f in graph_map_cbs:
            f(Type.MAP, Map(c_obj), Object.Event(evt))
...
#        self._finalizer = weakref.finalize(self, mpr_graph_free, self._obj)
    ...
    def devices(self):
        ...
        return List(mpr.mpr_graph_get_list(self._obj, Type.DEVICE), None)

    def signals(self):
        """Retrieve a list of Signals from the Graph."""

        return List(mpr.mpr_graph_get_list(self._obj, Type.SIGNAL), None)

    def maps(self):
        """Retrieve a list of Maps from the Graph."""

        return List(mpr.mpr_graph_get_list(self._obj, Type.MAP), None)
...
class Device(Object):
    ...
    def add_signal(self, dir, name, length=1, datatype=Type.FLOAT, unit=None, min=None, max=None,
                   num_inst=None, callback=None, events=Object.Event.ANY):
        """
        ...
        Args:
            ...
            events (libmapper.Object.Event): The types of events to trigger a call to `callback`.
        ...
        ptr = mpr.mpr_sig_new(self._obj, dir.value, name.encode('utf-8'), length,
                              datatype.value, None, None, None, None, None, Object.Event.NONE)
    ...
    def signals(self, direction=Signal.Direction.ANY):
        ...
        return List(mpr.mpr_dev_get_sigs(self._obj, direction), None)
    ...
    def maps(self, direction=Signal.Direction.ANY):
        ...
        return List(mpr.mpr_dev_get_maps(self._obj, direction), None)
