...
def _get_target_length(
    ...
    shortening = int((initial_length - min_length) * (random.random() ** throttling_factor)) + 1 \
        if (shortening_rate > 0 and initial_length > min_length and random.random() < shortening_rate) \
        else 0
    lengthening = int((max_length - initial_length) * (random.random() ** throttling_factor)) + 1 \
        if (lengthening_rate > 0 and max_length > initial_length and random.random() < lengthening_rate) \
        else 0
