...
try:
    ...
    STARTUP_FAILURE = None  # type: ignore
try:
    from uvicorn.supervisors import ChangeReload, Multiprocess
except ImportError:
    # Pyodide compatibility: these are not available without C extensions
    ChangeReload = None  # type: ignore
...
def run(root: Optional[Callable] = None, *,
    ...
    if config.should_reload:
        if ChangeReload is None:
            raise RuntimeError('Reload mode requires uvicorn[standard] which is not available in Pyodide')
        sock = config.bind_socket()
        ChangeReload(config, target=Server.instance.run, sockets=[sock]).run()
    elif config.workers > 1:
        if Multiprocess is None:
            raise RuntimeError('Multiple workers require uvicorn[standard] which is not available in Pyodide')
        sock = config.bind_socket()
