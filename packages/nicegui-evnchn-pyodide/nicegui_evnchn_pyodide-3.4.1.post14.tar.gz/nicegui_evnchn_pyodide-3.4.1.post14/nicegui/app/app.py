...
class App(FastAPI):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # Add stub methods when FastAPI is unavailable
        if FastAPI is object:
            self.mount = lambda *args, **kwargs: None  # type: ignore
            self.get = lambda *args, **kwargs: lambda f: f  # type: ignore
            self.post = lambda *args, **kwargs: lambda f: f  # type: ignore
            self.exception_handler = lambda *args, **kwargs: lambda f: f  # type: ignore
            # Create stub router with prefix attribute
            class StubRouter:
                prefix = ''
            self.router = StubRouter()  # type: ignore
