...
"""
    ...
    def check_for_updates(self):
        """
        Checks if the persistence file has been modified and reloads if necessary.
        Used by the dashboard to stay in sync with the live system.
        """
        if self.persistence.exists():
            mtime = os.path.getmtime(self.persistence.filepath)
            if not hasattr(self, "_last_load_time") or mtime > self._last_load_time:
                print("🔄 Persistence file updated, reloading...")
                self._load_from_persistence()
