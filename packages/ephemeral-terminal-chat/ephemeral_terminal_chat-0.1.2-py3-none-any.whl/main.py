...
def check_for_updates():
    """Checks PyPI for a newer version and auto-updates if found."""
    try:
        url = "https://pypi.org/pypi/ephemeral-terminal-chat/json"
        with urllib.request.urlopen(url, timeout=1.5) as response:
            ...
            if v_to_tuple(latest_version) > v_to_tuple(VERSION):
                print(f"\n[!] New version detected: {latest_version} (Current: {VERSION})")
                print("[*] Performing automatic update...")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "-U", "ephemeral-terminal-chat"])
                print("[+] Update successful! Restarting...\n")
                # Restart the current process
                os.execv(sys.executable, ['python'] + sys.argv)
    except Exception:
        # Fail silently if no internet or other issues to not block startup
        pass
    ...
    async def _watch_and_refresh(self):
        while self.running and self.supabase:
            try:
                ...
                # Show Invitation Command at Top
