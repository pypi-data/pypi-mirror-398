...
class Items:
    Bone: Item
    """The bones of an ancient creature."""  # 多行注释，描述Bone物品的来源

    Cactus: Item
    """Obtained by harvesting sorted cacti."""  # 多行注释，描述Cactus物品的获取方式

    Carrot: Item
    """Obtained by harvesting carrots."""  # 多行注释，描述Carrot物品的获取方式

    Fertilizer: Item
    """Call `use_item(Items.Fertilizer)` to instantly remove 2s from the plants remaining grow time."""  # 多行注释，描述Fertilizer物品的使用方法

    Gold: Item
    """Found in treasure chests in mazes."""  # 多行注释，描述Gold物品的发现位置

    Hay: Item
    """Obtained by cutting grass."""  # 多行注释，描述Hay物品的获取方式

    Piggy: Item
    """This item has been removed from the game but remains as a nostalgia trophy."""  # 多行注释，说明Piggy物品已被移除但保留作为怀旧纪念品

    Power: Item
    """Obtained by harvesting sunflowers. The drone automatically uses this to move twice as fast."""  # 多行注释，描述Power物品的获取方式及效果

    Pumpkin: Item
    """Obtained by harvesting pumpkins."""  # 多行注释，描述Pumpkin物品的获取方式
