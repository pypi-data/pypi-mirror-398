...
class CQL3D_GUI(tk.Tk):
    def __init__(self, yaml_file):
        ...
        defaultinput = os.path.join(self.base_dir, 'cql3dtests/cqlinput_tests/test1_cqlinput_eoved_.16_multi-flux-surface_test1.0')
        defaultaux = os.path.join(self.base_dir, 'cql3dtests/aux_file_folder')
        
        # Initialize paths
        self.executable_path = None
        self.current_file = defaultinput
        self.additional_files = defaultaux
        ...
        if self.current_file:
            self.set_namelist(self.current_file)
    ...
    def init_ui(self):
        ...
        ToolTip(load_btn, "Load a Fortran namelist (cqlinput) from file")
        ToolTip(save_btn, "Save a Fortran namelist (cqlinput) to file")
        ...
        bottom_controls.pack(fill=tk.X, pady=(10, 0))
        
        # Define grid weights so the three main groups split the width evenly
        ...
        # ---------------------------
        # CATEGORY 1: COMPILING
        # ---------------------------
        compile_frame = ttk.LabelFrame(bottom_controls, text="Compilation", padding=5)
        compile_frame.grid(row=0, column=0, sticky="nsew", padx=5)
        
        # Put the Status Label on bottom
        self.exec_path_label = ttk.Label(compile_frame, text="No executable loaded", foreground="gray", anchor="center")
        self.exec_path_label.pack(side=tk.BOTTOM, fill=tk.X, pady=(0, 5))
        
        # Pack buttons to the LEFT to make them horizontal
        # expand=True + fill=tk.X makes them resize to fit the frame nicely
        self.compile_btn = ttk.Button(
            compile_frame,
            text="Compile CQL3D",
            command=self.launch_compiler,
            #bootstyle="secondary"
        )
        self.compile_btn.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
        ...
        self.load_exec_btn.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        # ---------------------------
        # CATEGORY 2: EXECUTION
        # ---------------------------
        exec_frame = ttk.LabelFrame(bottom_controls, text="Execution", padding=5)
        exec_frame.grid(row=0, column=1, sticky="nsew", padx=5)
        
        self.aux_folder_label = ttk.Label(exec_frame, text="No auxiliary folder chosen", foreground="gray", anchor="center")
        self.aux_folder_label.pack(side=tk.BOTTOM, fill=tk.X, pady=(0, 5))

        # 2. Create a sub-container for the buttons to keep them horizontal below the label
        exec_btns_frame = ttk.Frame(exec_frame)
        exec_btns_frame.pack(side=tk.TOP, fill=tk.X, expand=True)
        ...
        self.file_btn.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        self.run_btn = ttk.Button(
            exec_btns_frame,
            text="RUN CQL3D",
            command=self.run_cql3d,
            state="disabled",
            bootstyle="success"
        )
        ...
        # CATEGORY 3: PLOTTING
        # ---------------------------
        plot_frame = ttk.LabelFrame(bottom_controls, text="Plotting", padding=5)
        plot_frame.grid(row=0, column=2, sticky="nsew", padx=5)
        
        self.ncfile_label = ttk.Label(plot_frame, text="No NetCDF file.", foreground="gray", anchor="center")
        self.ncfile_label.pack(side=tk.BOTTOM, fill=tk.X, pady=(0, 5))
        ...
        self.plot_btn.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        # --- Initial State Check ---
        if self.executable_path is not None:
            self.exec_path_label.config(text=f"Executable: {os.path.basename(self.executable_path)}")
            self.run_btn.config(state="normal")
            
        if self.additional_files is not None:
            self.aux_folder_label.config(text=f"Auxiliary File Folder: {os.path.basename(self.additional_files)}")
        ...
        ToolTip(self.file_btn, "Choose auxiliary file folder")
        ToolTip(self.plot_btn, "Plot results from NetCDF file")
        ToolTip(self.compile_btn, "Open the Compiler Utility to build CQL3D from source")
...
#                widget.bind('<FocusOut>', on_focus_out)
    ...
    def load_jconfig(self):
        ...
        try:
            ...
            return {}#print(f"Error loading configuration: {e}")
    ...
    def import_namelist(self):
        """Opens a file dialog to select a namelist, then loads it."""
        # 1. Determine initial directory
        initial_dir = None
        if hasattr(self, 'current_file') and self.current_file:
            path_obj = Path(self.current_file)
            initial_dir = path_obj.parent
            initial_file = path_obj.suffix
        else:
            initial_dir = None
        ...
        # 3. If user selected a file, pass it to the loader function
        if fname:
            self.set_namelist(fname)

    def set_namelist(self, fname):
        """
        Parses the given namelist file and updates the UI widgets.
        
        Args:
            fname (str): The full path to the namelist file.
        """
        if not fname:
            return
        ...
        # 2. Pre-process file (patching)
        fname1 = fname + '.tmp'
        self.patch_cqlinput(fname, fname1)
        
        # 3. Reset UI to defaults before loading new data
        self.apply_defaults()
        ...
        try:
            ...
            # --- Pass 1: Scan for start indices and restructure ---
            for group, content in nml_dict.items():
                if group not in start_ind: start_ind[group] = {}
                if group not in nml1: nml1[group] = {}
            ...
            # Merge found start indices with the global state
            if hasattr(self, 'start_ind'):
                self.start_ind = self.start_ind | start_ind
            else:
                self.start_ind = start_ind
            
            # --- Helper: Deep Prepend for nested arrays ---
            def deep_prepend(structure, value):
                # Check if this is the bottom level (a list, but contents are NOT lists)
            ...
            # --- Pass 2: Set Widget Values ---
            for group, content in nml1.items():
                for key, val in content.items():
                    if key in self.widgets:
                        # Handle array offset logic (indices > 1)
                        if key in vars_greater_than_1:
                            try:
                                # Fetch default value to fill the gap
                                default_cfg = self.config['namelist_groups'][group]['variables'][key]['default']
                                
                                # Ensure default is a list
                                if not isinstance(default_cfg, list):
                                    default_cfg = [default_cfg]
                                    
                                # Calculate how many items we need to prepend
                                missing_count = vars_greater_than_1[key] - 1
                                
                                if len(default_cfg) < missing_count:
                                    if len(default_cfg) > 0:
                                        defval = [default_cfg[0]] * missing_count
                                    else:
                                        defval = [0] * missing_count
                                else:
                                    defval = default_cfg[0:missing_count]
                                    
                                # Prepend the default values to the read values
                                val = deep_prepend(val, defval)
                                
                                # Reset start index to 1 since we filled the gap
                                if '_start_index' in start_ind[group] and key in start_ind[group]['_start_index']:
                                     start_ind[group]['_start_index'][key][0] = 1

                            except Exception as e:
                                print(f"Warning handling start index for {key}: {e}")
                                # If logic fails, just use original val
                                pass

                        # Finally set the value in the UI
                        self.set_widget_value(key, val)
            
            # 5. Post-load triggers
            self.after(10, self.trigger_dependency_check)
            
            # Check for associated NetCDF file to enable plotting
            nc_filepath = Path(self.get_nc_filename())
            if nc_filepath.is_file():
                self.plot_btn.config(state="normal")
            
            #messagebox.showinfo("Success", f"Loaded {os.path.basename(fname)}")
    ...
    # --- Compile CQL3D Functions ---
            
    def launch_compiler(self):
        package_dir = os.path.dirname(os.path.abspath(__file__))
        sourcefiles = os.path.join(package_dir, 'sourcefiles')
        
        if not os.path.isdir(sourcefiles):
             sourcefiles = os.getcwd()

        root = tk.Toplevel()
        root.title("CQL3D Compiler")
        root.geometry("700x650")
        
        # PASS THE CALLBACK HERE using lambda or direct reference
        # We pass self.set_executable so the tab can call it on success
        app = CompilerTab(root, initial_source_dir=sourcefiles, on_success=self.set_executable)
            
    # --- CQL3D Execution Functions ---

    def set_executable(self, fname):
        """
        Sets the executable path, updates the UI, and saves config.
        This can be called by the file dialog OR the compiler.
        """
        if not fname:
            return
        ...
        if os.name != 'nt':
            if not os.access(fname, os.X_OK):
                # Try to fix permissions automatically if we just compiled it
                try:
        ...
        #print(f"Executable updated to: {fname}") # Debug confirmation

    def load_executable(self):
        """Opens file dialog to manually select executable."""
        initial_dir = None
        ...
        # Pass the result to the setter
        self.set_executable(fname)
    ...
    def _on_cql3d_finished(self, stdout, stderr, returncode, exception, run_folder):
        ...
        if os.path.isfile(nc_filepath):
            ...
            self.ncfile_label.config(text=f"{os.path.basename(nc_filepath)}")
...
#        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def plot_density(self):
        ...
        try:
            ...
            # --- THE FIX ---
            # style='sci': Forces scientific notation (e.g. 1e13)
            # scilimits=(0,0): Forces scientific notation for all numbers outside range 10^0
            # useOffset=False: Prevents the "+1.5e13" offset notation that confuses the axis
...
class CompilerTab(ttk.Frame):
    def __init__(self, parent, initial_source_dir=None, on_success=None):
        super().__init__(parent)
        self.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        self.on_success_callback = on_success  # Save the callback for later
        self.os_type = platform.system()
        
        if initial_source_dir and os.path.isdir(initial_source_dir):
            start_dir = initial_source_dir
        else:
            start_dir = os.getcwd()
            
        self.source_dir_var = tk.StringVar(value=start_dir)
        self.netcdf_inc_var = tk.StringVar()
        self.netcdf_lib_var = tk.StringVar()
        self.pgplot_lib_var = tk.StringVar()
    ...
    def create_header(self):
        ...
        lbl_os = ttk.Label(header_frame, text=f"Detected System: {self.os_type}", font=("Helvetica", 12, "bold"))
        lbl_os.pack(side=tk.LEFT)
        if self.os_type == "Windows":
            ttk.Label(header_frame, text="(⚠️ Requires WSL)", foreground="red").pack(side=tk.LEFT, padx=10)

    def create_source_section(self):
        source_frame = ttk.LabelFrame(self, text="Source Code Directory", padding=10)
        source_frame.pack(fill=tk.X, pady=5)
        entry_path = ttk.Entry(source_frame, textvariable=self.source_dir_var, width=60)
        entry_path.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        ttk.Button(source_frame, text="Browse...", command=self.browse_directory).pack(side=tk.LEFT)

    def browse_directory(self):
        selected_dir = filedialog.askdirectory(initialdir=self.source_dir_var.get())
        if selected_dir:
            self.source_dir_var.set(selected_dir)
            self.log(f"Target directory set to: {selected_dir}")

    def create_config_section(self):
        config_frame = ttk.LabelFrame(self, text="Library Configuration", padding=10)
        config_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(config_frame, text="NetCDF Include (-I):").grid(row=0, column=0, sticky="w", pady=2)
        ttk.Entry(config_frame, textvariable=self.netcdf_inc_var, width=50).grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Label(config_frame, text="NetCDF Libs (-L):").grid(row=1, column=0, sticky="w", pady=2)
        ttk.Entry(config_frame, textvariable=self.netcdf_lib_var, width=50).grid(row=1, column=1, sticky="ew", padx=5, pady=2)

        ttk.Label(config_frame, text="PGPLOT Path:").grid(row=2, column=0, sticky="w", pady=2)
        ttk.Entry(config_frame, textvariable=self.pgplot_lib_var, width=50).grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        
        ttk.Button(config_frame, text="Re-Detect Paths", command=self.run_auto_detect).grid(row=3, column=1, sticky="e", pady=10)
        config_frame.columnconfigure(1, weight=1)

    def create_action_section(self):
        action_frame = ttk.Frame(self)
        action_frame.pack(fill=tk.X, pady=10)
        self.btn_compile = ttk.Button(action_frame, text="GENERATE MAKEFILE & COMPILE", command=self.start_compilation_thread)
        self.btn_compile.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
        ttk.Button(action_frame, text="Clean Build", command=self.run_clean).pack(side=tk.LEFT, padx=(5, 0))

    def create_console_section(self):
        lbl_cons = ttk.Label(self, text="Compilation Output:")
        lbl_cons.pack(anchor="w")
        self.console = scrolledtext.ScrolledText(self, height=12, state="disabled", bg="#1e1e1e", fg="#00ff00", font=("Consolas", 10))
        self.console.pack(fill=tk.BOTH, expand=True)

    def log(self, message, error=False):
        self.console.config(state="normal")
        tag = "err" if error else "norm"
        self.console.tag_config("err", foreground="#ff5555")
    ...
    def run_auto_detect(self):
        """Finds paths, falling back to standard defaults if nc-config is unavailable."""
        self.log("--- Scanning System for Dependencies ---")
        
        # 1. NetCDF Detection
        nc_inc = ""
        nc_lib = ""
        
        # Attempt A: Try nc-config
        # We explicitly look in common bin paths in case PATH environment is missing
        search_paths = os.environ['PATH'] + ":/opt/homebrew/bin:/opt/local/bin:/usr/local/bin"
        nc_config_path = shutil.which("nc-config", path=search_paths)

        if nc_config_path:
            try:
                libs_out = subprocess.check_output([nc_config_path, "--flibs"], text=True).strip()
                incs_out = subprocess.check_output([nc_config_path, "--fflags"], text=True).strip()
                if libs_out and incs_out:
                    nc_lib = libs_out
                    nc_inc = incs_out
                    self.log(f"Found NetCDF via config tool.")
            except Exception:
                pass
        
        # Attempt B: Fallback to hardcoded defaults if Attempt A failed
        if not nc_inc or not nc_lib:
            self.log("nc-config not found. Checking standard locations...", error=False)
            
            # Check for MacPorts (/opt/local) or Homebrew (/opt/homebrew)
            if os.path.exists("/opt/local/include/netcdf.inc"):
                 nc_inc = "-I/opt/local/include"
                 # Combine common lib paths to be safe
                 nc_lib = "-L/opt/local/lib -lnetcdff -lnetcdf"
                 self.log("Found NetCDF headers in /opt/local/include")
                 
            elif os.path.exists("/opt/homebrew/include/netcdf.inc"):
                nc_inc = "-I/opt/homebrew/include"
                nc_lib = "-L/opt/homebrew/lib -L/opt/homebrew/opt/netcdf/lib -L/opt/homebrew/opt/netcdf-fortran/lib -lnetcdff -lnetcdf"
                self.log("Found NetCDF headers in /opt/homebrew/include")
                
            elif os.path.exists("/usr/local/include/netcdf.inc"):
                 nc_inc = "-I/usr/local/include"
                 nc_lib = "-L/usr/local/lib -lnetcdff -lnetcdf"
                 self.log("Found NetCDF headers in /usr/local/include")
            
            else:
                # Only show RED error if both methods fail
                self.log("Could not auto-detect NetCDF. Please enter paths manually.", error=True)

        self.netcdf_inc_var.set(nc_inc)
        self.netcdf_lib_var.set(nc_lib)

        # 2. PGPLOT Detection
        pg_candidates = ["/opt/local/lib", "/opt/homebrew/lib", "/usr/local/lib"]
        found_pg = False
        for path in pg_candidates:
            if os.path.exists(os.path.join(path, "libpgplot.a")) or os.path.exists(os.path.join(path, "libpgplot.dylib")):
                self.pgplot_lib_var.set(path)
                self.log(f"Found PGPLOT at: {path}")
                found_pg = True
                break
        
        if not found_pg:
            self.pgplot_lib_var.set("/opt/local/lib") # Default backup
            self.log("PGPLOT not detected. Defaulting to /opt/local/lib", error=False)

    def get_makefile_content(self):
        # Use explicit \t for indentation to prevent "missing separator" errors
        template = """
SHELL     = /bin/sh
...
INCLUDE_FLAGS = ${NETCDF_INC}
LOCATION      = ${NETCDF_LIB} -L${PGPLOT_LIB}
...
\tzblock.f \\
\tzcunix.f  zfreya.f    znonsym.f

OBJECTS   = $(SOURCES:.f=.o)

DEBUG     = -g
OPTIMIZE  = -O1
CSPECIAL  =  -fdefault-real-8 -fdefault-double-8 -std=legacy

COMPILE   =  $(COMPILER) -c $(CSPECIAL) $(OPTIMIZE) $(INCLUDE_FLAGS)
LOAD      = $(BUILDER) -o $(NAME)
...
clean:
\trm -f  $(NAME) $(OBJECTS) *.lst
"""
        s = Template(template)
        return s.safe_substitute(
            NETCDF_INC=self.netcdf_inc_var.get(),
            NETCDF_LIB=self.netcdf_lib_var.get(),
            PGPLOT_LIB=self.pgplot_lib_var.get()
        )

    def start_compilation_thread(self):
        self.btn_compile.config(state="disabled")
        t = threading.Thread(target=self.run_compilation)
        t.start()

    def run_compilation(self):
        target_dir = self.source_dir_var.get()
        if not os.path.isdir(target_dir):
            self.after(0, lambda: messagebox.showerror("Error", "Invalid Source Directory."))
            self.after(0, lambda: self.btn_compile.config(state="normal"))
            return

        try:
            makefile_path = os.path.join(target_dir, "Makefile")
            self.log(f"--- Generating Makefile in {target_dir} ---")
            
            with open(makefile_path, "w") as f:
                f.write(self.get_makefile_content())
            ...
            if process.returncode == 0:
                self.after(0, lambda: messagebox.showinfo("Success", "Compiled Successfully!"))
                self.after(0, lambda: self.log("--- BUILD SUCCESSFUL ---"))
                
                # Trigger the callback to update the Main UI
                if self.on_success_callback:
                    # Based on your makefile, the output name is 'xcql3d_gfortran64'
                    # We construct the full absolute path
                    new_exe_path = os.path.join(target_dir, "xcql3d_gfortran64")
                    
                    # We must run this on the main thread using 'after'
                    # because we are currently inside a background thread.
                    self.after(0, lambda: self.on_success_callback(new_exe_path))
            else:
                self.after(0, lambda: messagebox.showerror("Error", "Compilation Failed."))
                self.after(0, lambda: self.log("--- BUILD FAILED ---", True))

        except Exception as e:
            self.after(0, lambda: self.log(f"Error: {e}", True))
        finally:
            self.after(0, lambda: self.btn_compile.config(state="normal"))
    ...
    def run_clean(self):
        target_dir = self.source_dir_var.get()
        try:
            subprocess.run(["make", "clean"], cwd=target_dir, check=True)
            self.log(f"Cleaned build files in {target_dir}")
        except Exception as e:
            self.log(f"Error cleaning: {e}", True)
