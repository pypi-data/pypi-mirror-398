...
# Параметры индексатора (защита от "зависаний" на огромных файлах)
# NOTE: Defaults are conservative; override via env vars as needed.
DEFAULT_MAX_FILE_BYTES = 5_000_000  # 5 MB
DEFAULT_MAX_CHUNKS_PER_FILE = 5_000
DEFAULT_SLOW_FILE_SECONDS = 5.0
...
def get_max_file_bytes() -> int | None:
    """
    Максимальный размер файла (в байтах), который индексируем.

    Можно отключить лимит, установив CURSOR_RAG_MAX_FILE_BYTES=0.
    """
    raw = os.getenv("CURSOR_RAG_MAX_FILE_BYTES")
    if raw is None:
...
def get_max_chunks_per_file() -> int | None:
    """
    Максимальное число чанков на файл. Если превышено — файл пропускается.

    Можно отключить лимит, установив CURSOR_RAG_MAX_CHUNKS_PER_FILE=0.
    """
    raw = os.getenv("CURSOR_RAG_MAX_CHUNKS_PER_FILE")
    if raw is None:
...
def get_slow_file_seconds() -> float:
    """
    Порог (в секундах), после которого файл считается "медленным" и логируется с деталями.
    """
    raw = os.getenv("CURSOR_RAG_SLOW_FILE_SECONDS")
    if raw is None:
        return DEFAULT_SLOW_FILE_SECONDS
    try:
        value = float(raw)
    except ValueError:
        return DEFAULT_SLOW_FILE_SECONDS
