        ...
        try:
            ...
            for idx, file_path in enumerate(files, start=1):
                try:
                    ...
                    print(f"\r⏳ Обработка: {idx}/{len(files)} ({progress_pct:.1f}%) - {file_path.name}", end="")
                    sys.stdout.flush()
                    ...
                    print(f"\r📖 Чтение: {idx}/{len(files)} ({progress_pct:.1f}%) - {file_path.name}", end="")
                    sys.stdout.flush()
                    ...
                    print(f"\r✂️  Чанкинг: {idx}/{len(files)} ({progress_pct:.1f}%) - {file_path.name}", end="")
                    sys.stdout.flush()
                    ...
                    print(f"\r🧠 Энкодинг: {idx}/{len(files)} ({progress_pct:.1f}%) - {file_path.name}", end="")
                    sys.stdout.flush()
                    ...
                    print(f"\r💾 Сохранение: {idx}/{len(files)} ({progress_pct:.1f}%) - {file_path.name}", end="")
                    sys.stdout.flush()
                    collection.add(
                        ids=ids,
                        documents=documents,
                    ...
                    count += 1
                    progress_pct = (count / len(files)) * 100
                    print(f"\r✅ Обработано: {count}/{len(files)} ({progress_pct:.1f}%) - {file_path.name}", end="")
                    sys.stdout.flush()
