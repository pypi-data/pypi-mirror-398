...
_OPENAI_CHAT_COMPLETION_CREATE_ORIGINAL = _openai_chat_completion_create


def _openai_chat_completion_create_with_retries(**params: Any) -> Tuple[Any, Optional[int]]:
    if _openai_chat_completion_create is not _OPENAI_CHAT_COMPLETION_CREATE_ORIGINAL:
        return _openai_chat_completion_create(**params), None
    client = _get_openai_client()
    if client is not None:
        chat = getattr(client, "chat", None)
        completions = getattr(chat, "completions", None) if chat is not None else None
        raw = getattr(completions, "with_raw_response", None)
        if raw is not None and hasattr(raw, "create"):
            raw_response = raw.create(**params)
            retries = _extract_openai_retries_from_api_response(raw_response)
            return raw_response.parse(), retries
    return _openai_chat_completion_create(**params), None
...
_OPENAI_RESPONSES_CREATE_ORIGINAL = _openai_responses_create


def _openai_responses_create_with_retries(**params: Any) -> Tuple[Any, Optional[int]]:
    if _openai_responses_create is not _OPENAI_RESPONSES_CREATE_ORIGINAL:
        return _openai_responses_create(**params), None
    client = _get_openai_client()
    if client is not None:
...
def _extract_openai_retries_from_headers(headers: Any) -> Optional[int]:
    if headers is None:
        return None
    value = None
    try:
        if hasattr(headers, "get"):
            ...
            value = headers.get("x-stainless-retry-count") or headers.get("X-Stainless-Retry-Count")
    except Exception:  # noqa: BLE001
        return None
    return _coerce_int(value)


def _extract_openai_retries_from_api_response(raw_response: Any) -> Optional[int]:
    return _coerce_int(getattr(raw_response, "retries_taken", None))


def _extract_openai_retries_from_stream(stream: Any) -> Optional[int]:
    response = getattr(stream, "response", None)
    if response is None:
        return None
    request = getattr(response, "request", None)
    retries = _extract_openai_retries_from_headers(getattr(request, "headers", None))
    if retries is not None:
        return retries
    return _extract_openai_retries_from_headers(getattr(response, "headers", None))


def _extract_openai_retries_from_exc(exc: Exception) -> Optional[int]:
...
def _maybe_add_retry_metadata(internal: Dict[str, Any], openai_retries: Optional[int]) -> None:
    if openai_retries is None:
        return
    internal["openai_retries"] = int(openai_retries)
...
class ChatCompletion:
    ...
    def create(**kwargs: Any):
        ...
        openai_retries: Optional[int] = None
        ...
        try:
            ...
            if stream:
                ...
                openai_retries = _extract_openai_retries_from_stream(response)
            else:
                response, openai_retries = _openai_chat_completion_create_with_retries(**params)
        except Exception as exc:  # noqa: BLE001
            duration_ms = (time.perf_counter() - start) * 1000
            openai_retries = _extract_openai_retries_from_exc(exc)
            internal = {"stream": stream}
            _maybe_add_retry_metadata(internal, openai_retries)
            _safe_log_event(
                provider="openai",
                model=model,
    ...
    def create(**kwargs: Any):
        ...
        try:
            ...
            if stream:
                ...
                response, openai_retries = _openai_responses_create_with_retries(**params)
        except Exception as exc:  # noqa: BLE001
            duration_ms = (time.perf_counter() - start) * 1000
            openai_retries = _extract_openai_retries_from_exc(exc)
            internal = {"stream": stream}
...
class _LoggedStream:
    def __init__(
        ...
        *,
        ...
        self._openai_retries = openai_retries
    ...
    def __next__(self):
        try:
            ...
            if not self._done:
                ...
                _maybe_add_retry_metadata(internal, self._openai_retries)
                _safe_log_event(
                    provider="openai",
                    model=self._model,
                    type="chat_completion",
