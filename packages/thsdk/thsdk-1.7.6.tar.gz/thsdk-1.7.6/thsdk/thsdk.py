    ...
    def __init__(self, ops: Optional[Dict[str, Any]] = None):
        ...
        Args:
            ops (Dict[str, Any], optional): 配置信息，用于设置与行情服务器交互的参数。
                ...
                # 推荐方式2：从环境变量读取（适合生产环境）
                # 设置环境变量：export THS_USERNAME=your_username THS_PASSWORD=your_password
                ths = THS()  # 会自动从环境变量读取
                
                # 不推荐：使用 demo 账户（仅用于测试，会自动切换为临时游客账户）
                ths = THS({"username": "demo", "password": "demo"})
                ```
                
                账户配置优先级：
                1. 用户传入的账户密码（最高优先级）
                2. 环境变量 THS_USERNAME 和 THS_PASSWORD
                3. 临时游客账户（最低优先级，仅用于测试）
        ...
        # 如果用户传入的账户和密码都是 "demo"，则使用随机账户
        if ops.get("username") == "demo" and ops.get("password") == "demo":
            logger.warning("⚠️  检测到使用 demo 账户，已自动切换为临时游客账户。")
            logger.warning("⚠️  提示：demo 账户仅用于测试，建议使用您自己的账户以确保服务稳定性。")
            account = rand_account()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.error == "":
                    ...
                    logger.info(f"✅ TCP成功连接到服务器 account:{username}")
                    if str(username).startswith("thsguest_"):
                        ...
                        logger.warning("⚠️  配置方式：THS({'username': 'your_username', 'password': 'your_password'})")
                        logger.warning("=" * 70)
                    return response
                else:
