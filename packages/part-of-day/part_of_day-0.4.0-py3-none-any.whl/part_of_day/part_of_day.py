...
from part_of_day.exceptions import MixedPartOfDayError, PartOfDayCalculationError, TwilightNotIncludedError
...
class PartOfDay(str, Enum):
    ...
    def __eq__(self, value):
        if not isinstance(value, PartOfDay):
            raise MixedPartOfDayError()

        return super().__eq__(value)


class PartOfDayWithTwilights(str, Enum):
    DAY = "Day"
    NIGHT = "Night"
    DAWN = "Dawn"
    DUSK = "Dusk"
    ...
    def __eq__(self, value):
        if not isinstance(value, PartOfDayWithTwilights):
            raise MixedPartOfDayError()
...
class PartOfDayCalculator:
    ...
    def _get_part_of_day_with_twilights(self, dt: datetime | None = None) -> PartOfDayWithTwilights:
        if dt is None:
            dt = datetime.now(timezone.utc)
    ...
    def is_it_day(self, dt: datetime | None = None) -> bool:
        ...
        if self.include_twilights:
            return self.get_part_of_day(dt) == PartOfDayWithTwilights.DAY
        else:
    ...
    def is_it_dawn(self, dt: datetime | None = None) -> bool:
        if not self.include_twilights:
            raise TwilightNotIncludedError()

        if dt is None:
            dt = datetime.now(timezone.utc)

        return self.get_part_of_day(dt) == PartOfDayWithTwilights.DAWN
    ...
    def is_it_dusk(self, dt: datetime | None = None) -> bool:
        ...
        return self.get_part_of_day(dt) == PartOfDayWithTwilights.DUSK
