...
from ..templates import usage_tracking
...
def create_agents_sdk_folder(base_path: Path) -> bool:
    """Create agents_sdk/ folder with README and usage_tracking if it doesn't exist."""
    agents_sdk_dir = base_path / "agents_sdk"
    
    if agents_sdk_dir.exists():
        console.print(f"  [yellow]![/yellow]  agents_sdk/ already exists")
        # Still create usage_tracking.py if it doesn't exist
        usage_tracking_path = agents_sdk_dir / "usage_tracking.py"
        if not usage_tracking_path.exists():
            usage_tracking_path.write_text(usage_tracking.USAGE_TRACKING_TEMPLATE, encoding="utf-8")
            console.print(f"  [green]+[/green]  Created agents_sdk/usage_tracking.py")
        return False
