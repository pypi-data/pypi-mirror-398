...
# Usage tracking (optional) - tracks token usage and costs
# from ..usage_tracking import TokenUsageHook, persist_token_usage
...
class {class_name}:
    ...
    Usage tracking (optional):
        # Pass user and tag to track costs per run
        manager = {class_name}(user=request.user, tag="{base_name}:workflow")
    """
    
    def __init__(self, user=None, tag: str = None):
        """Initialize the manager with any shared resources.
        
        Args:
            user: Optional user for usage tracking (Django User or similar)
            tag: Optional tag for categorizing usage (e.g., "{base_name}:task_name")
        """
        self.user = user
        self.tag = tag or "{base_name}"
        # Uncomment to enable usage tracking:
        # self.hooks = TokenUsageHook(user=user, tag=self.tag)
    ...
    #     # To track usage, pass hooks to Runner.run:
    #     # result = await Runner.run(planner_agent, f"Request: {{user_request}}", hooks=self.hooks)
...
def run_{base_name}_sync(user_request: str, user=None, tag: str = None) -> Any:
    """Run the {base_name} service synchronously.
    
    Args:
        user_request: The user's input request
        user: Optional user for usage tracking
        tag: Optional tag for categorizing usage
    """
    manager = {class_name}(user=user, tag=tag)
