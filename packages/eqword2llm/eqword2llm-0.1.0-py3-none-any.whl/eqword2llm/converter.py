...
class WordToMarkdownConverter:
    ...
    def _final_cleanup(self, markdown: str) -> str:
        ...
        for letter in GREEK_LETTERS:
            markdown = re.sub(rf"(\\{letter})([a-zA-Z_])", rf"\1 \2", markdown)
    ...
    def _convert_paragraph(
        self, para: ET.Element, list_state: dict[str, Any]
    ) -> str:
        """Convert a paragraph."""
    ...
    def _clean_equation_number(self, latex: str) -> str:
        ...
        latex = re.sub(
            r"#\\left\(\s*SEQ\s+Equation\s*\\\*\s*ARABIC\s*\d+\\right\)", "", latex
        )
    ...
    def _fix_latex_spacing(self, latex: str) -> str:
        ...
        # Add space after Greek letter commands followed by alphabet
        for letter in GREEK_LETTERS:
            latex = re.sub(rf"(\\{letter})([a-zA-Z])", rf"\1 \2", latex)
    ...
    def _convert_delimiter(self, delim: ET.Element) -> str:
        ...
        for e in delim.findall("m:e", NAMESPACES):
