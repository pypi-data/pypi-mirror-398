...
def main() -> int:
    ...
    parser.add_argument(
        "--no-equation-numbers",
        action="store_true",
        help="Disable automatic equation numbering (enabled by default)",
    )
    parser.add_argument(
        "--format",
        choices=["markdown", "structured", "prompt"],
        default="markdown",
        help="Output format: markdown (default), structured (with YAML frontmatter), prompt (LLM-ready)",
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    try:
        converter = WordToMarkdownConverter(
            args.input,
            equation_numbers=not args.no_equation_numbers,
        )

        # Generate output based on format
        if args.format == "structured":
            result = converter.convert_structured()
            output = result.to_structured()
        elif args.format == "prompt":
            output = converter.to_llm_prompt()
        else:
