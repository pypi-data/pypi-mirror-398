...
class WordToMarkdownConverter:
    ...
    def _final_cleanup(self, markdown: str) -> str:
        ...
        # This handles cases where multiple bold runs are concatenated
        # We need to merge **text1****text2** into **text1text2**
        # Note: Only merge if there's no space or newline between the markers
        # (spaces and newlines indicate intentional separation: **text1** **text2** should stay separate)
        def merge_bold_markers(text: str) -> str:
            # Pattern: **text1****text2** or **text1****text2****text3**
            # Match: ** followed by non-asterisk, non-newline chars, then **** (no space/newline), then non-asterisk, non-newline chars, then **
            # This ensures we only merge when there's no space or newline between ** and ****
            # Use [^*\n] instead of [^*] to exclude newlines
            pattern = r"\*\*([^*\n]+?)\*\*\*\*([^*\n]+?)\*\*"
        ...
        # But only on the same line (not across newlines)
        # This prevents merging bold markers that are on separate lines
        # Split by newlines, process each line separately, then rejoin
        lines = markdown.split("\n")
        processed_lines = []
        for line in lines:
            # Only replace **** with ** on single lines (not across newlines)
            line = re.sub(r"\*{4,}", "**", line)
            processed_lines.append(line)
        markdown = "\n".join(processed_lines)

        # Remove empty bold (** **) - but only if it's truly empty (no text between)
        # This pattern matches ** followed by optional whitespace and ** with nothing else
        # We need to be careful not to match **text1** **text2** (which should stay separate)
        # Only match if there's nothing between the markers except whitespace
        # Use word boundaries or line boundaries to ensure we don't match **text1** **text2**
        # Pattern: ** followed by whitespace, then **, but only if not preceded/followed by word chars
        # Only remove empty bold markers that are truly empty (no text before or after)
        # Don't match **text1** **text2** where there's actual text
        # Use negative lookbehind/lookahead to ensure we don't match when there's text
        markdown = re.sub(
            r"(?<![a-zA-Z0-9_])\*\*\s+\*\*(?![a-zA-Z0-9_])", "", markdown
        )  # ** ** (with space, empty, not part of **text** **text**)
        markdown = re.sub(
            r"(?<![a-zA-Z0-9_])\*\*\*\*(?![a-zA-Z0-9_])", "", markdown
        )  # **** (no space, empty, not part of **text****text**)
