...
**`class HttpServer(port=80, handler=staticResource, args=(), threads=True, reuse=True, block=True, start=True)`**

    port
        The port to listen on.  If a list is specified the server will use the first one it can successfully bind to.
    ...
    reuse
        If True the server will set the SO_REUSEADDR socket option to get around TIME_WAIT issues.
