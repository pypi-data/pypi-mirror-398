...
from django_keycloak_oidc.utils import absolutify, import_from_settings
...
class KeyCloakAuthenticationBackend(OIDCAuthenticationBackend):
    ...
    def user_permissions_mapping(self, user, payload):
        self.user_admin_roles_mapping(user, payload)
        self.user_roles_to_groups_mapping(user, payload)
        self.user_roles_to_permissions_mapping(user, payload)

    @staticmethod
    def user_roles_to_permissions_mapping(user, payload):
    ...
    @staticmethod
    def user_roles_to_groups_mapping(user, payload):
        user.groups.clear()
        claim_roles = payload.get("roles", [])
        for claim_role in claim_roles:
            keycloak_mappings = KeyCloakPermissionMapping.objects.filter(
    ...
    def user_admin_roles_mapping(self, user, payload):
        user.is_superuser = False
        user.is_staff = False
        if self.user_is_superuser(payload):
            user.is_superuser = True
            user.is_staff = True
        elif self.user_is_staff(payload):
            user.is_staff = True
        user.save()

    def user_is_staff(self, payload):
        return f"{self.OIDC_RP_CLIENT_ID}#staff" in payload.get("roles")

    def user_is_superuser(self, payload):
        return f"{self.OIDC_RP_CLIENT_ID}#superuser" in payload.get("roles")
