...
.. code-block:: python
   ...
   # Add 'django_keycloak_oidc' to INSTALLED_APPS
   INSTALLED_APPS = (
       # ...
       'django.contrib.auth',
       'django_keycloak_oidc',  # Load after auth
       # ...
   )

   # Add 'django_keycloak_oidc' authentication backend
   AUTHENTICATION_BACKENDS = (
       'django_keycloak_oidc.auth.OIDCAuthenticationBackend',
       # ...
   )
...
:py:class:`django_keycloak_oidc.middleware.SessionRefresh` middleware.
...
To add it to your site, put it in the settings::
    ...
    MIDDLEWARE = [
        ...
        # ...
        'django_keycloak_oidc.middleware.SessionRefresh',
        # ...
    ]


The :py:class:`django_keycloak_oidc.middleware.SessionRefresh` middleware will
...
By default, django-keycloak-oidc looks up a Django user matching the email field
...
:py:class:`django_keycloak_oidc.auth.OIDCAuthenticationBackend` class and
...
.. code-block:: python
   ...
   from django_keycloak_oidc.auth import OIDCAuthenticationBackend
...
django-keycloak-oidc will create a new Django user account. It will create the
...
Preventing django-keycloak-oidc from creating new Django users
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you don't want django-keycloak-oidc to create Django users, you can add this
setting::
...
When a user logs out, by default, django-keycloak-oidc will end the current
...
django-keycloak-oidc ends the Django session.
...
django-keycloak-oidc logs using the ``django_keycloak_oidc`` logger. Enable that
