...
def test_GetPValue( length ):
    ...
    with pytest.raises(ValueError):
        irr.Metrics.DFK.GetPValue( TS, n = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.DFK.GetPValue( TS, n = -1 )

    with pytest.raises(ValueError):
        irr.Metrics.DFK.GetPValue( TS, L = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.DFK.GetPValue( TS, L = -1 )
...
def test_GetStatistic( length ):
    ...
    result = irr.Metrics.DFK.GetStatistic( TS )
...
def test_Optimisation():
    ...
    from irreversibility.ParameterOptimisation.Opt_DFK import Optimisation
