...
def test_GetPValue( length ):
    ...
    with pytest.raises(ValueError):
        irr.Metrics.MSTrends.GetPValue( TS, wSize = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.MSTrends.GetPValue( TS, wSize = -1 )

    with pytest.raises(ValueError):
        irr.Metrics.MSTrends.GetPValue( TS, wSize2 = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.MSTrends.GetPValue( TS, wSize2 = -1 )
...
def test_GetStatistic( length ):
    ...
    result = irr.Metrics.MSTrends.GetStatistic( TS )
...
def test_Optimisation():
    ...
    from irreversibility.ParameterOptimisation.Opt_MSTrends import Optimisation
