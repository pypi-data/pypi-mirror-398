...
def test_GetPValue( length ):
    ...
    with pytest.raises(ValueError):
        irr.Metrics.TernaryCoding.GetPValue( TS, segL = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.TernaryCoding.GetPValue( TS, segL = -1 )

    with pytest.raises(ValueError):
        irr.Metrics.TernaryCoding.GetPValue( TS, alpha = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.TernaryCoding.GetPValue( TS, alpha = -1 )
...
def test_GetStatistic( length ):
    ...
    result = irr.Metrics.TernaryCoding.GetStatistic( TS )
...
def test_Optimisation():
    ...
    from irreversibility.ParameterOptimisation.Opt_TernaryCoding import Optimisation
