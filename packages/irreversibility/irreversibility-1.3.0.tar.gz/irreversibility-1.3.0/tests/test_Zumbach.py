...
def test_GetPValue( length ):
    ...
    with pytest.raises(ValueError):
        irr.Metrics.Zumbach.GetPValue( TS, deltaT = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.Zumbach.GetPValue( TS, deltaT = -1 )

    with pytest.raises(ValueError):
        irr.Metrics.Zumbach.GetPValue( TS, granularity = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.Zumbach.GetPValue( TS, granularity = -1 )
...
def test_GetStatistic( length ):
    ...
    result = irr.Metrics.Zumbach.GetStatistic( TS )
...
def test_Optimisation():
    ...
    from irreversibility.ParameterOptimisation.Opt_Zumbach import Optimisation
