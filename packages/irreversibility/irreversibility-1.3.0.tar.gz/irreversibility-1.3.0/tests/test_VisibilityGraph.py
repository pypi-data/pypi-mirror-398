...
def test_TS_Does_Not_Change( horizon, length ):

    TS = np.random.uniform( 0., 1., (length) )
    TS2 = copy.deepcopy( TS )

    irr.Metrics.VisibilityGraph.GetPValue( TS, horizon = horizon )
...
def test_GetPValue( length ):
    ...
    with pytest.raises(ValueError):
        irr.Metrics.VisibilityGraph.GetPValue( TS, horizon = 1. )
    with pytest.raises(ValueError):
        irr.Metrics.VisibilityGraph.GetPValue( TS, horizon = -2 )
...
def test_GetStatistic( length ):
    ...
    result = irr.Metrics.VisibilityGraph.GetStatistic( TS )
...
def test_Optimisation():
    ...
    from irreversibility.ParameterOptimisation.Opt_VisibilityGraph import Optimisation
