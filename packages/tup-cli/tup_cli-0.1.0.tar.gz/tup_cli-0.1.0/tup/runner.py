"""Runner for tup jobs inside the container.
...
Usage:
    python -m tup.runner /app/job_config.pickle
"""
...
def run_job(config: JobConfig) -> int:
    ...
    """
    print(f"[tup] Starting job: {config.log_relpath}")
    print(f"[tup] Entrypoint: {config.entrypoint}")

    # Set up logging
    log_dir = setup_logging(config.log_relpath)
    print(f"[tup] Log directory: {log_dir}")

    # Import the entrypoint function
    try:
        main_fn = get_module_member(config.entrypoint)
    except Exception as e:
        print(f"[tup] Failed to import entrypoint: {e}")
        return 1

    print(f"[tup] Running {main_fn.__name__}...")
    print("-" * 60)
    ...
    try:
        ...
        print("-" * 60)
        print(f"[tup] Job completed successfully")
        ...
        print("-" * 60)
        print(f"[tup] Job failed with error: {e}")
        import traceback
        traceback.print_exc()
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m tup.runner <config_path>")
        sys.exit(1)
