# Google Ads Keyword Ideas Generator

Generate keyword ideas using the Google Ads API to discover new keyword opportunities. Features intelligent keyword expansion from seed terms or URLs, with comprehensive metrics including search volumes, competition data, and trend analysis for strategic SEO and content planning.

## What does Google Ads Keyword Ideas Generator do?

This Actor connects to the Google Ads API to generate and expand keyword ideas for SEO and PPC professionals:

- **Expand seed keywords** into hundreds or thousands of related keyword suggestions using Google's keyword planner
- **Generate keyword ideas** from landing pages or entire websites
...
**Perfect for**: SEO professionals discovering new keyword opportunities, content marketers planning content strategies, PPC specialists expanding their keyword portfolios, and digital agencies conducting keyword research for clients.
...
### Example with Whole-Site Keyword Ideas
...
|-------|------|-------------|----------|---------|
| `keywords` | Array | List of seed keywords (up to 20) to expand into related ideas | Optional* | - |
...
| `max_ideas` | Integer | Maximum number of keyword ideas to generate | Optional | `100` |
...
- This actor automatically generates keyword ideas from your seed keywords or URL
- **Seed Keywords**: Provide up to 20 seed keywords to expand
- **Max Ideas**: Use `max_ideas` to limit the number of generated keywords (recommended: 100-1000, max: 10,000)
- If more than 20 keywords are provided, only the first 20 will be used

**URL-based Keyword Generation:**
- **Landing Page Mode**: Provide a `url` parameter to generate keyword ideas from a specific landing page
- **Whole Site Mode**: Use domain-only URL (e.g., "example.com") and set `whole_site: true` for site-wide keyword ideas
- Works independently of `keywords` parameter - you can use either or both
