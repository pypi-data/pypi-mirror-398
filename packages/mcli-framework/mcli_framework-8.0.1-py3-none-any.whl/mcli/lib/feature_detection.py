...
def has_ml_features() -> bool:
    ...
    return (
        os.getenv("MCLI_ENABLE_ML", "").lower() in ("true", "1", "yes") or
        _check_ml_imports()
    )
...
def has_dashboard_features() -> bool:
    ...
    return (
        os.getenv("MCLI_ENABLE_DASHBOARD", "").lower() in ("true", "1", "yes") or
        _check_dashboard_imports()
    )

def has_trading_features() -> bool:
    """Check if user is actually using trading features."""
    return (
        os.getenv("MCLI_ENABLE_TRADING", "").lower() in ("true", "1", "yes") or
        _check_trading_imports()
    )
