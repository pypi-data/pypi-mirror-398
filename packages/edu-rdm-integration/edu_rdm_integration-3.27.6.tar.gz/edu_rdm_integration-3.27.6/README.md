...
## Подключение РВД к отдельной БД
При подключении отдельной БД для РВД необходимо:
1. Добавить в конфигурационный файл проекта следующие настройки:
    [rdm_general]
    USE_RDM_DB = True
    ...
    HOST = 192.127.0.1
    PORT = 5432
2. В настройках продукта в DATABASE_ROUTERS добавить 'edu_rdm_integration.routers.RdmRouter' или его наследника.
Учитывая, что роутер сервисной БД должен быть последним.
3. В настройках продукта в DATABASES добавить настройки БД РВД, используя RDM_DB_ALIAS из edu_rdm_integration.core.consts
в качестве псевдонима БД.
4. В настройках продукта необходимо добавить вычисляемую настройку "USING_RDM_DB_NAME".
Например:
  USE_RDM_DB = conf.get_bool('rdm_general', 'USE_RDM_DB')
  USING_RDM_DB_NAME = RDM_DB_ALIAS if USE_RDM_DB else DEFAULT_DB_ALIAS
5.Применить миграции к новой БД командой "django-admin migrate --database=rdm"
...
- Определение значений по умолчанию настроек в settings.py:
    ...
    PROJECT_DEFAULT_CONFIG.update({
        ...
        ('rdm_transfer_task', 'NEXT_TASK_DELAY_SECONDS'): 5,
        ...
        ('rdm_transfer_task_fast', 'NEXT_TASK_DELAY_SECONDS'): 5,
        ...
        ('rdm_transfer_task_long', 'NEXT_TASK_DELAY_SECONDS'): 5,
        ...
        ('rdm_database', 'ENGINE'): 'django.db.backends.postgresql_psycopg2',
        ('rdm_database', 'NAME'): 'db_name',
        ('rdm_database', 'USER'): 'username',
        ('rdm_database', 'PASSWORD'): 'password',
        ('rdm_database', 'HOST'): '127.0.0.1',
        ('rdm_database', 'PORT'): '',
    })  
    ```
- Получение значений настроек из конфигурационного файла в settings.py:
    ...
    RDM_TRANSFER_TASK_NEXT_TASK_DELAY_SECONDS = conf.get_int('rdm_transfer_task', 'NEXT_TASK_DELAY_SECONDS')
    ...
    RDM_FAST_TRANSFER_TASK_NEXT_TASK_DELAY_SECONDS = conf.get_int('rdm_transfer_task_fast', 'NEXT_TASK_DELAY_SECONDS')
    ...
    RDM_LONG_TRANSFER_TASK_NEXT_TASK_DELAY_SECONDS = conf.get_int('rdm_transfer_task_long', 'NEXT_TASK_DELAY_SECONDS')
    ...
    # Период проверки статуса запроса, в минутах. Время, спустя которое происходит следующая проверка статуса.
...
| USE_RDM_DB                                   | Использование отдельной БД для РВД                                                                                                               | False                   |
...
- В дефолтный конфиг проекта необходимо добавить:
    ...
    # Настройка отдельной БД для РВД. Работает при включенной настройке USE_RDM_DB = True
    [rdm_database]
    ENGINE = django.db.backends.postgresql_psycopg2
    NAME = db_name
    USER = username
