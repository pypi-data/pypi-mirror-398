...
class Session:
    ...
    deployments_cache: Optional[list] = None
...
def get_deployments(client, force_refresh: bool = False) -> Optional[list]:
    """Get deployments with caching."""
    if session.deployments_cache and not force_refresh:
        return session.deployments_cache

    result = client.deployments.list(limit=50)
    if result and result.data:
        session.deployments_cache = result.data
        return result.data
    return None


def select_deployment(client, prompt: str = "Select deployment:") -> Optional[str]:
    """Select a deployment, offering to use the remembered one."""
    deployments = get_deployments(client)
    if not deployments:
        print_info("No deployments found")
        return None

    # If we have a remembered deployment, offer to use it
    if session.deployment_key:
        choices = [
            questionary.Choice(
                f"Use: {session.deployment_key}",
                value=session.deployment_key
            ),
            questionary.Choice("Select different deployment...", value="__select__"),
            questionary.Separator(),
            questionary.Choice("Clear selection", value="__clear__"),
        ]

        choice = questionary.select(prompt, choices=choices).ask()

        if choice == "__clear__":
            session.deployment_key = None
            console.print("[dim]Selection cleared[/dim]")
            return select_deployment(client, prompt)
        elif choice == "__select__":
            pass  # Fall through to full selection
        elif choice:
            return choice
        else:
            return None

    # Full deployment selection
    key = questionary.select(
        prompt,
        choices=[d.key for d in deployments]
    ).ask()

    if key:
        session.deployment_key = key
...
def show_main_menu() -> Optional[str]:
    ...
    # Show current deployment if set
    if session.deployment_key:
        console.print(f"[dim]Current deployment: [cyan]{session.deployment_key}[/cyan][/dim]")
...
def deployments_menu():
    """Deployments submenu."""
    while True:
        # Show current deployment in menu
        current = f" [cyan]({session.deployment_key})[/cyan]" if session.deployment_key else ""

        action = questionary.select(
            f"Deployments:{current}",
            choices=[
                ...
                questionary.Separator(),
                questionary.Choice("Change deployment", value="change"),
        ...
        try:
            ...
            if action == "list":
                deployments = get_deployments(client, force_refresh=True)
                if deployments:
                    rows = [{"key": d.key, "description": d.description or ""} for d in deployments]
                    table = Table(title="Deployments")
                    table.add_column("Key", style="cyan")
                    table.add_column("Description")
                    for row in rows:
                        table.add_row(row["key"], row["description"])
                    console.print(table)

            elif action == "change":
                # Force new selection
                session.deployment_key = None
                key = select_deployment(client)
                if key:
                    print_success(f"Now using: {key}")
