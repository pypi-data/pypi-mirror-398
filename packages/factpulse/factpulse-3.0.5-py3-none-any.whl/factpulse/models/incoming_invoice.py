...
class IncomingInvoice(BaseModel):
    ...
    net_amount: Annotated[str, Field(strict=True)] = Field(description="Total net amount (BT-109)", alias="netAmount")
    vat_amount: Annotated[str, Field(strict=True)] = Field(description="Total VAT amount (BT-110)", alias="vatAmount")
    gross_amount: Annotated[str, Field(strict=True)] = Field(description="Total gross amount (BT-112)", alias="grossAmount")
    ...
    def net_amount_validate_regular_expression(cls, value):
    ...
    def gross_amount_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$", value):
            raise ValueError(r"must validate the regular expression /^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$/")
        return value
