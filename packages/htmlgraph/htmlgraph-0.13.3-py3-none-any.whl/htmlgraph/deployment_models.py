...
class SemanticVersion(BaseModel):
    ...
    prerelease: Optional[str] = Field(None, description="Pre-release identifier (alpha, beta, rc)")
...
class BuildConfig(BaseModel):
    ...
    verify_artifacts: bool = Field(default=True, description="Verify build artifacts exist")
...
class PyPIConfig(BaseModel):
    ...
    verify_publication: bool = Field(
        default=True,
        description="Verify package appears on PyPI after publishing"
    )
...
class PluginConfig(BaseModel):
    ...
    command: str = Field(..., description="Update command with {package} and {version} placeholders")
    required: bool = Field(default=False, description="Fail deployment if plugin update fails")
    ...
    def validate_command(cls, v: str) -> str:
        ...
        if '{package}' not in v and '{version}' not in v:
            raise ValueError("Plugin command must contain {package} or {version} placeholder")
...
class DeploymentHooks(BaseModel):
    ...
    pre_build: List[str] = Field(default_factory=list, description="Commands to run before build")
    post_build: List[str] = Field(default_factory=list, description="Commands to run after build")
    pre_publish: List[str] = Field(default_factory=list, description="Commands to run before publish")
    post_publish: List[str] = Field(default_factory=list, description="Commands to run after publish")
...
class DeploymentConfig(BaseModel):
    ...
    pypi_package: Optional[str] = Field(None, description="PyPI package name (if different from project)")
    ...
    build: BuildConfig = Field(default_factory=BuildConfig, description="Build configuration")
    pypi: PyPIConfig = Field(default_factory=PyPIConfig, description="PyPI configuration")

    # Plugins and hooks
    plugins: List[PluginConfig] = Field(default_factory=list, description="Plugin update configs")
    hooks: DeploymentHooks = Field(default_factory=DeploymentHooks, description="Custom deployment hooks")

    # Flags
    dry_run: bool = Field(default=False, description="Simulate deployment without executing")
    skip_confirmations: bool = Field(default=False, description="Skip confirmation prompts")
    ...
    def validate_config_consistency(self) -> 'DeploymentConfig':
        ...
        if DeploymentStep.LOCAL_INSTALL in self.steps:
            if DeploymentStep.BUILD not in self.steps and DeploymentStep.PYPI_PUBLISH not in self.steps:
...
class DeploymentResult(BaseModel):
    ...
    steps_completed: List[DeploymentStep] = Field(default_factory=list, description="Steps that completed")
    steps_failed: List[DeploymentStep] = Field(default_factory=list, description="Steps that failed")
    errors: List[str] = Field(default_factory=list, description="Error messages")
    duration_seconds: float = Field(default=0.0, ge=0, description="Total deployment duration")
