...
class TestCodeOperations:
    ...
    def test_code_bash_detected(self, config):
        """Bash commands are no longer classified as code operations (bash_patterns is empty)."""
        # Note: Config has bash_patterns: [] - Bash is handled by guidance system, not blocking
        code_commands = [
            "npm install",
            "git commit -m 'test'",
            "python setup.py"
        ...
        for cmd in code_commands:
            # Should NOT be classified as code operation since bash_patterns is empty
            assert not is_code_operation("Bash", {"command": cmd}, config), f"Failed for: {cmd}"
...
class TestValidationLogicNoActiveWork:
    ...
    def test_write_denied_no_work(self, config, mock_no_active_work):
        ...
        assert decision["decision"] == "allow"
        # May include guidance about creating work item
...
class TestValidationLogicSpikeActive:
    ...
    def test_write_denied_with_spike(self, config, mock_spike_active):
        """Write should be allowed with guidance when spike active."""
        decision = validate_tool_call("Write", {"file_path": "src/test.py"}, config, [])
        assert decision["decision"] == "allow"
        # May include guidance about spike being for planning
...
class TestAlwaysDenyHtmlGraphWrites:
    ...
    def test_write_htmlgraph_denied_with_feature(self, config, mock_feature_active):
        ...
        assert "sdk" in decision["reason"].lower() or "direct" in decision["reason"].lower() or "blocked" in decision["reason"].lower()
