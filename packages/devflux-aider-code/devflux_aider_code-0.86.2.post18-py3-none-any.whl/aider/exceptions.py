...
EXCEPTIONS = [
    ...
    ExInfo(
        "NotFoundError",
        False,
        "The requested model or resource was not found. Please check the model name, provider prefix, and API key permissions.",
    ),
...
# <code change='예외 처리 수정' >
        return tuple(
            ex for ex in self.exceptions
...
# </code>
    ...
    def get_ex_info(self, ex):
        ...
        # 모든 모델 제공자의 NotFoundError 처리
        if isinstance(ex, litellm.NotFoundError):
            err_str = str(ex).lower()
            
            # 1. OpenRouter 특화 처리
            if "openrouter" in err_str:
                return ExInfo(
                    "NotFoundError",
                    False,
                    (
                        "The OpenRouter model endpoint was not found. If using a ':free' model, "
                        "it may be offline. Try checking the model ID at openrouter.ai/models."
                    ),
                )
            
            # 2. 일반적인 모델/엔드포인트 미검출 처리 (OpenAI, Anthropic, Google 등 공통)
            model_hint = ""
            if "model" in err_str:
                model_hint = " Check if the model name is spelled correctly and supported by your API key."
            
            return ExInfo(
                "NotFoundError",
                False,
                f"The requested API endpoint or model was not found (404).{model_hint}"
            )
