...
def calc_mat_os(xm, fx, bool_active_list, signal_type):
    # xm doit être le vecteur des positions réelles des éléments (en m ou mm)
    # fx est la fréquence spatiale (en m^-1 ou mm^-1)
    
    num_els = len(xm)
    num_cols = bool_active_list.shape[1]
    
    # On utilise la fonction continue échantillonnée comme Maïmouna
    if signal_type == 'cos':
        # (cos(2*pi*fx*Xm) > 0) -> donne 1100...
        mask = (np.cos(2 * np.pi * fx * xm) > 0).astype(float)
    elif signal_type == 'sin':
        # (sin(2*pi*fx*Xm) > 0) -> donne 0110...
        mask = (np.sin(2 * np.pi * fx * xm) > 0).astype(float)
    else:
        mask = np.ones(num_els) # Sécurité
        
    # On duplique sur le nombre de colonnes demandées
