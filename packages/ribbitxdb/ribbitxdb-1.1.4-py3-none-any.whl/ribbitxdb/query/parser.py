...
class SQLParser:
        ...
        try:
            if first_token.value == 'SELECT':
                ...
                raise SQLSyntaxError(
                    ...
                    hint="Supported commands: SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, PRAGMA, BEGIN, COMMIT, ROLLBACK, DESCRIBE, SHOW, EXPLAIN"
                )
    ...
    def parse_create(self) -> Dict[str, Any]:
        ...
        if next_token and next_token.value == 'INDEX':
            ...
            raise SQLSyntaxError(
                f"Expected TABLE or INDEX after CREATE, got {next_token.value if next_token else 'EOF'}",
                hint="Supported: CREATE TABLE, CREATE INDEX, CREATE UNIQUE INDEX"
            )
    ...
    def parse_drop(self) -> Dict[str, Any]:
        ...
        if next_token and next_token.value == 'TABLE':
            ...
            raise SQLSyntaxError(
                f"Expected TABLE or INDEX after DROP, got {next_token.value if next_token else 'EOF'}",
                hint="Supported: DROP TABLE, DROP INDEX"
            )
