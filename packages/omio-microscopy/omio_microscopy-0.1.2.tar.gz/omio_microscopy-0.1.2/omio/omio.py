...
def _squeeze_zarr_to_napari_cache(src, fname, axes="TZCYXS", cache_folder_name=".omio_cache"):
    ...
    This helper creates a new Zarr array on disk that removes all singleton
    dimensions from an input Zarr array. The goal is to provide a compact, Napari
    friendly representation where trivial axes with length 1 are dropped while the
    relative order of remaining axes is preserved.

    The function assumes that the input is in OME-like axis order and that the two
    spatial axes Y and X are the last two axes. Copying is performed in a
    memory-friendly way by iterating over all outer indices and copying one (Y, X)
    plane at a time.

    Parameters
    ----------
    src : zarr.core.array.Array
        Source Zarr array to be squeezed. The array is expected to follow the axis
        string given by `axes`.
    fname : str
        Path used as the on-disk destination Zarr store. If an existing path is
        present, it is removed and replaced. The argument name reflects the usual
        use case where this path is derived from an original filename.
    axes : str, optional
        Axis string corresponding to `src.shape`. Default is ``"TZCYXS"``.
    cache_folder_name : str, optional
        Name of a hidden cache folder colocated with `fname`. This argument is
        currently not used by the implementation but is kept for API stability and
        potential future path handling. Default is ``".omio_cache"``.

    Returns
    -------
    dst : zarr.core.array.Array
        Destination Zarr array on disk with all singleton dimensions removed.
    squeezed_axes : str
        Axis string corresponding to the returned array.

    Raises
    ------
    TypeError
        If `src` is not a Zarr array.
    ValueError
        If the length of `axes` does not match ``src.ndim``.

    Notes
    -----
    * The destination store is created at `fname` and will overwrite any existing
    directory at that location.
    * Chunking is preserved along kept axes if `src.chunks` is defined; otherwise
    Zarr chooses default chunking.
    * If the squeezed result has two dimensions or fewer, the data are copied in a
    single operation. Otherwise, copying proceeds plane-wise along the last two
    axes, iterating over all remaining axes with `tqdm` progress reporting.
    * This function creates a derived view on disk and does not modify the source
    array.
    """
    ...
    # determine which axes to keep (those with dim > 1):
    src_shape = src.shape
    axes_list = list(axes)
    ...
    # If the squeezed array is effectively 2D, copy in one go
    if len(squeezed_shape) <= 2:
        # Build full source index with zeros on dropped axes
        src_idx = []
    ...
    # Otherwise, copy per XY-slice, iterating over all outer indices
    outer_shape = squeezed_shape[:-2]
    total_outer = int(np.prod(outer_shape))

    # Mapping from original axes to squeezed axes:
    # For each original axis i: if kept, it gets an index in the squeezed array
    orig_to_squeezed = {}
    squeezed_pos = 0
    for i in range(len(src_shape)):
        if i in keep_indices:
            orig_to_squeezed[i] = squeezed_pos
            squeezed_pos += 1

    # iterate over all outer indices in squeezed space:
    for outer_idx in tqdm(np.ndindex(*outer_shape), total=total_outer,
        desc="creating Napari view Zarr (squeezed)"):
        # Build destination index: outer_idx + (:, :)
        dst_idx = outer_idx + (slice(None), slice(None))

        # Build source index in OME TZCYXS space:
        # For each original axis:
        # - if dropped (dim == 1): index 0
        # - if kept and not Y or X: use corresponding entry from outer_idx
        # - for Y, X: use slice(None)
        src_idx = []
        outer_idx_iter = iter(outer_idx)
        for i, dim in enumerate(src_shape):
            if i not in keep_indices:
                # singleton axis: always index 0
                src_idx.append(0)
            else:
                ax_label = axes_list[i]
                if ax_label in ("Y", "X"):
                    src_idx.append(slice(None))
                else:
...
def _squeeze_zarr_to_napari_cache_dask(src, fname, axes, cache_folder_name=".omio_cache"):
    ...
    * This function creates a derived representation for visualization and does not
    ...
    # convert store URI → filesystem path:
    ...
    # convert source Zarr to Dask:
    darr = da.from_zarr(src)

    # determine which axes to squeeze:
    squeeze_axes = [i for i, dim in enumerate(src.shape) if dim == 1]
    squeezed_axes = "".join(ax for ax, dim in zip(axes, src.shape) if dim > 1)

    # apply squeeze lazily:
    darr = da.squeeze(darr)

    # write the squeezed data to Zarr lazily (computes slices in parallel):
    # darr.to_zarr(target_path, overwrite=True)  #ol dask' to_zarr
