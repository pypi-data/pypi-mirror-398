...
def deploy_create(image, env_type, memory, replicas, port, disk, env_vars, project, expires):
    ...
    try:
        ...
        if btp_admin:
            ...
            click.echo(f"\n后台部署中，使用以下命令查看状态:")
            click.echo(f"  btp-admin deploy info {deployment.id}")
        else:
            click.echo(f"\n⚠️  无法启动后台进程，请手动执行:")
...
def deploy_list(status):
    ...
    try:
        ...
        click.echo(f"\n{'ID':<38} {'镜像':<20} {'环境':<6} {'状态':<10} {'副本':<6} {'账号'}")
        click.echo("-" * 110)
        ...
        for dep in deployments:
            ...
            click.echo(f"{dep.id} {dep.image:<20} {dep.env_type:<6} {dep.status:<10} {dep.replicas:<6} {account_email}")
        ...
        if deployment.replicas_list:
            for replica in deployment.replicas_list:
                ...
                if account:
                    ...
                    # 如果是Kyma，显示运行时过期时间
                    if replica.runtime_type == 'kyma' and replica.runtime_id:
                        runtime = db.query(KymaRuntime).filter(KymaRuntime.id == replica.runtime_id).first()
                        if runtime:
                            if runtime.expires_at:
                                click.echo(f"Kyma过期: {runtime.expires_at}")
                            else:
                                click.echo(f"Kyma过期: 未知")
                    
                    # 如果是CF，显示CF组织信息
                    elif replica.runtime_type == 'cf' and replica.runtime_id:
                        cf_org = db.query(CFOrg).filter(CFOrg.id == replica.runtime_id).first()
                        if cf_org:
                            click.echo(f"CF组织: {cf_org.org_name}")
                    break  # 只显示第一个副本的账号信息
