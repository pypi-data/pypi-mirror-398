...
def accounts_add(subdomain, email, password):
    ...
    try:
        ...
        account = Account(
            ...
            password=password,
            status='ACTIVE',
            expires_at=datetime.now(UTC).replace(tzinfo=None) + __import__('datetime').timedelta(days=90)
        )
        db.add(account)
        db.commit()
...
def deploy_create(image, env_type, memory, replicas, port, disk, env_vars, project, expires):
    ...
    try:
        ...
        if expires:
            ...
            expires_at = datetime.now(UTC).replace(tzinfo=None) + timedelta(days=expires)
