...
from models import Account, Deployment, KymaRuntime, CFOrg
from integrations.btp_cli import verify_account
from scheduler.core import select_account_for_deployment
...
def accounts_add(subdomain, email, password):
    ...
    try:
        ...
        account = Account(
            ...
            password=password,
            status='ACTIVE',
            expires_at=datetime.now(timezone).replace(tzinfo=None) + __import__('datetime').timedelta(days=90)
        )
        db.add(account)
        db.commit()
...
def deploy_create(image, env_type, memory, replicas, port, disk, env_vars, project, expires):
    ...
    try:
        ...
        # 选择账号
        account, runtime = select_account_for_deployment(db, env_type, memory, wait_for_creating=False)
        ...
        if expires:
            ...
            expires_at = datetime.now(timezone).replace(tzinfo=None) + timedelta(days=expires)
        ...
        from models import DeploymentReplica
...
def maintenance_cleanup():
    """清理过期资源"""
    from tasks.cleanup import cleanup_expired_resources
    click.echo("开始清理过期资源...")
...
def maintenance_cf_check():
    """CF 日检"""
    from tasks.scheduled import update_cf_active_history
    click.echo("开始 CF 日检...")
