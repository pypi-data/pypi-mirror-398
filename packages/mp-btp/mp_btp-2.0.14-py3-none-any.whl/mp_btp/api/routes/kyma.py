...
def get_pool_status(db: Session = Depends(get_db)):
    ...
    for r in all_runtimes:
        if r.status == "OK":
            if r.expires_at:
                expires = r.expires_at.replace(tzinfo=timezone) if r.expires_at.tzinfo is None else r.expires_at
...
def recreate_kyma(runtime_id: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if runtime.cooling_until:
        cooling = runtime.cooling_until.replace(tzinfo=timezone) if runtime.cooling_until.tzinfo is None else runtime.cooling_until
        if cooling > datetime.now(timezone):
...
def create_kyma_background(account_id: str, runtime_id: str):
    ...
    try:
        ...
        if existing:
            ...
            runtime.expires_at = datetime.now(timezone) + timedelta(days=14)
            db.commit()
