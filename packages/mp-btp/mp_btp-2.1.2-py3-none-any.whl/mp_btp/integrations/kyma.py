...
def get_service_url(kubeconfig_path: str, name: str, namespace: str = "default") -> Optional[str]:
    ...
    try:
        ...
        return None
def kyma_logs(kubeconfig_path: str, name: str, namespace: str = "default", tail: int = 100) -> Optional[str]:
    env = _env()
    env['KUBECONFIG'] = kubeconfig_path
    try:
        r = subprocess.run(['kubectl', 'logs', f'deployment/{name}', '-n', namespace, f'--tail={tail}'],
                          capture_output=True, text=True, timeout=30, env=env)
...
def kyma_restart(kubeconfig_path: str, name: str, namespace: str = "default") -> bool:
    ...
    env['KUBECONFIG'] = kubeconfig_path
    try:
        r = subprocess.run(['kubectl', 'rollout', 'restart', f'deployment/{name}', '-n', namespace],
                          capture_output=True, text=True, timeout=60, env=env)
        return r.returncode == 0
    except:
        return False
def kyma_scale(kubeconfig_path: str, name: str, replicas: int, namespace: str = "default") -> bool:
    env = _env()
    env['KUBECONFIG'] = kubeconfig_path
    try:
        r = subprocess.run(['kubectl', 'scale', f'deployment/{name}', f'--replicas={replicas}', '-n', namespace],
                          capture_output=True, text=True, timeout=60, env=env)
        return r.returncode == 0
    except:
        return False
def kyma_exec(kubeconfig_path: str, name: str, command: list, namespace: str = "default") -> tuple:
    env = _env()
    env['KUBECONFIG'] = kubeconfig_path
    try:
        r = subprocess.run(['kubectl', 'get', 'pods', '-l', f'app={name}', '-n', namespace, 
                           '-o', 'jsonpath={.items[0].metadata.name}'],
                          capture_output=True, text=True, timeout=30, env=env)
        if r.returncode != 0 or not r.stdout:
            return False, "Pod not found"
        pod = r.stdout.strip()
        r = subprocess.run(['kubectl', 'exec', pod, '-n', namespace, '--'] + command,
