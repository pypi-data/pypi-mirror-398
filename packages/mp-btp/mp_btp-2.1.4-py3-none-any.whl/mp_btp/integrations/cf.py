...
def cf_apps() -> list:
    try:
        ...
        return []
def cf_logs(name: str, recent: bool = True) -> Optional[str]:
    try:
        cmd = ["cf", "logs", name]
        if recent:
            cmd.append("--recent")
        ...
        return str(e)
def cf_scale(name: str, instances: int = None, memory: str = None, disk: str = None) -> bool:
    try:
        cmd = ["cf", "scale", name, "-f"]
        ...
        return False
def cf_ssh_exec(name: str, command: str) -> tuple:
    try:
        r = subprocess.run(["cf", "ssh", name, "-c", command],
