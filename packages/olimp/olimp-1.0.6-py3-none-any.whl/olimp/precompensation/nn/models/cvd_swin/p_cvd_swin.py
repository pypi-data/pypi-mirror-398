...
class PCVDSwin(nn.Module):
    r"""Swin Transformer

    .. image:: ../../../../_static/cvd_swin_3channels.svg
       :class: full-width
    ...
    def forward(self, inputs: Tensor) -> tuple[Tensor]:
        ...
        x1 = self._add_cvd_dim(self, x, hue_angle_deg)  # 4 channels
    ...
    def from_path(
        cls,
        path: PyOlimpHF = "hf://CVD/PCVDSwin_all.pth",
    ):
    ...
    @staticmethod
    def _add_cvd_dim(self, input: Tensor, hue_angle_deg: Tensor):
        # add to sRGB 3channel image four channel
        linRGB_batch = self._srgb2linRGB(self.trans_compose1(input))
        ...
        return torch.hstack((input, dim4))

    def preprocess(self, tensor: Tensor, hue_angle_deg: Tensor) -> Tensor:
        return self.trans_compose2(tensor), hue_angle_deg
...
def _demo():
    ...
    def demo_cvd_swin(
        ...
        p_cvd_swin = PCVDSwin.from_path()
        hue_angle_deg = torch.tensor([0.0])
        image = p_cvd_swin.preprocess(image, hue_angle_deg)
        progress(0.1)
        precompensation = p_cvd_swin(image)
        progress(1.0)
        return (p_cvd_swin.postprocess(precompensation)[0],)
