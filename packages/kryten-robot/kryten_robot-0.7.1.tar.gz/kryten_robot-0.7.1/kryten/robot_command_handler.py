...
class RobotCommandHandler:
    ...
    async def start(self) -> None:
        ...
        # New style subject: kryten.command.robot
        subject = "kryten.command.robot" 
    ...
    async def _handle_mute(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        # Access sender via connector if available, otherwise we can't do it
        # Note: RobotCommandHandler needs access to CytubeEventSender or similar
        # For now, we assume connector has methods or we need to pass sender to handler
    ...
    async def _handle_add_video(self, args: dict[str, Any]) -> dict[str, Any]:
         ...
         # This should probably be forwarded to playlist service or handled if we have direct access
         return {"error": "Playlist commands should be sent to playlist service"}
