...
<details open>
<summary>使用 nb-cli 安装（尚未实现）</summary>
...
**参数：**
- `关键词`: 要匹配的关键词。支持图片、@、纯文字，但不支持混合消息。
- `回复内容`: 当匹配成功时，BOT 会发送的内容。支持图片、@、纯文字，以及混合消息。
...
- `-f`, `--force`: 列出包含已被删除的所有词条。默认为`False`。
