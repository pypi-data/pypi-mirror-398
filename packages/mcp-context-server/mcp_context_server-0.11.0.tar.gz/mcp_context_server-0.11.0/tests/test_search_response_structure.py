...
class TestSearchContextResponseStructure:
    ...
    async def test_response_has_results_key(self) -> None:
        ...
        result = await search_context.fn(thread_id='test_response_structure', limit=10)
    ...
    async def test_response_has_count_field(self) -> None:
        ...
        result = await search_context.fn(thread_id='test_count_field', limit=10)
    ...
    async def test_stats_only_with_explain_query(self) -> None:
        ...
        result_without = await search_context.fn(
            thread_id='test_stats_conditional',
            limit=10,
            explain_query=False,
        )
        ...
        result_with = await search_context.fn(
            thread_id='test_stats_conditional',
            limit=10,
            explain_query=True,
        )
    ...
    async def test_entry_has_is_truncated_field(self) -> None:
        ...
        result = await search_context.fn(thread_id='test_truncated_field', limit=10)
    ...
    async def test_empty_results(self) -> None:
        ...
        result = await search_context.fn(thread_id='nonexistent_thread_xyz', limit=10)
        assert 'results' in result
        assert 'count' in result
...
class TestSearchToolsConsistency:
    ...
    async def test_search_context_uses_results_key(self) -> None:
        ...
        result = await search_context.fn(thread_id='consistency_test', limit=5)
        assert 'results' in result
    ...
    async def test_search_context_has_count(self) -> None:
        ...
        result = await search_context.fn(thread_id='count_consistency_test', limit=5)
        assert 'count' in result
    ...
    async def test_multiple_results_count_matches(self) -> None:
        ...
        result = await search_context.fn(thread_id=thread_id, limit=10)
        assert result['count'] == len(result['results'])
