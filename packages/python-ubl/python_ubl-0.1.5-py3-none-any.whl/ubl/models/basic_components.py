...
@dataclass
class TaxExemptionReason(StrElement):
    """UBL cbc:TaxExemptionReason - Textual reason for tax exemption (BT-120)."""


@dataclass
class TaxExemptionReasonCode(StrElement):
    """UBL cbc:TaxExemptionReasonCode - Coded reason for tax exemption (BT-121)."""
...
@dataclass
class BuyerReference(StrElement):
    """
    UBL cbc:BuyerReference - Buyer's reference identifier (BG-3).

    PEPPOL BIS requirement: A buyer reference or purchase order reference MUST be provided.
    If no actual reference exists, use 'N/A' as the default value.
    """
