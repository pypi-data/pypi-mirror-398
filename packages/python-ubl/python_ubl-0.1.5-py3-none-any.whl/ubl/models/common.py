...
class SimpleTypeCastMixin:
    ...
    def _simple_cast(self, value, expected_type, field_name):
        try:
            ...
            raise TypeError(
                f'Cannot cast field "{field_name}" from {getattr(type(value), "__name__", "None")} to {expected_type.__name__}: {e}'
            )
...
class BaseFromMixin:
    ...
    @classmethod
    def _get_class_name_to_attr_mapping(cls) -> dict[str, str]:
        """
        Get the class_name_to_attr mapping safely from a classmethod context.

        This handles both ClassVar and @property versions of class_name_to_attr.
        When class_name_to_attr is a property, we access _base_class_name_to_attr instead.

        Returns:
            Dictionary mapping XML element names to attribute names
        """
        # Try _base_class_name_to_attr first (used when class_name_to_attr is a property)
        if hasattr(cls, '_base_class_name_to_attr'):
            return cls._base_class_name_to_attr

        # Check if class_name_to_attr is a property
        class_attr = getattr(cls, 'class_name_to_attr', None)
        if isinstance(class_attr, property):
            # Property case - search parent classes for base mapping
            for base in cls.__mro__[1:]:
                if hasattr(base, '_base_class_name_to_attr'):
                    return base._base_class_name_to_attr
                elif hasattr(base, 'class_name_to_attr'):
                    base_attr = getattr(base, 'class_name_to_attr', None)
                    if not isinstance(base_attr, property):
                        return base_attr
            # If we can't find a base mapping, return empty dict
            return {}
        else:
            # ClassVar case - return directly
            return class_attr if class_attr is not None else {}
...
class FromXMLMixin(BaseFromMixin):
        ...
        try:
            ...
            for attr_name in cls._get_property_attr_names():
                ...
                if attr_name == 'value' and element.text:
                    value = cls._parse_value(attr_name, element.text.strip()) \
                        if len(element.text) \
                        else None
                else:
            ...
            for child in element:
                ...
                mapping = cls._get_class_name_to_attr_mapping()
                attr_name = mapping.get(tag)
...
class FromDictMixin(BaseFromMixin):
    @classmethod
    def from_dict(cls, element: dict, all_elements=None) -> 'FromDictMixin':
        """
        Parse a dict element into a dataclass instance.
        ...
        try:
            ...
            # Parse child elements
            mapping = cls._get_class_name_to_attr_mapping()
            for tag, child in element.items():
                ...
                child_class = getattr(
                    ...
                    all_elements,
                    mapping[tag].strip()
                ) else builtins_dict.get(mapping[tag].strip())

                if tag in ['created', ]:
                    print(tag, child_class, child, f'**{mapping[tag]}**', type(mapping[tag]), getattr(all_elements, mapping[tag], '???'), dir(all_elements))
                if child_class is None:
                    continue
                ...
                if isinstance(child, list):
                    # List field: create list if doesn't exist, then append
                    kwargs[tag] = [
                        getattr(child_class, 'from_dict', lambda x, _: child_class(x))(kid, all_elements)
                        for kid in child
                    ]
                else:
                    # Scalar field: assign directly
                    kwargs[tag] = getattr(child_class, 'from_dict', lambda x, _: child_class(x))(child, all_elements)

            # Filter out fields with init=False (auto-generated fields)
            filtered_kwargs = cls.filter_kwargs(kwargs)

            # Disable auto_cast during deserialization
            orig = cls.auto_cast
            cls.auto_cast = False
            rv = cls(**filtered_kwargs)
            cls.auto_cast = orig
...
@dataclass
class BaseElement(BaseXMLNS, ToXMLMixin, FromXMLMixin, FromDictMixin, SimpleTypeCastMixin, ABC):
    """
