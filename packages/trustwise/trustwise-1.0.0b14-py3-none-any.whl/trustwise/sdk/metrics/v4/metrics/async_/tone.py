...
class ToneMetricAsync(BaseMetric[ToneRequest, ToneResponse]):
    ...
    def _build_request(self, text: str, tones: list[str] | None = None, **kwargs) -> dict:
        ...
        return self.validate_request_model(ToneRequest, text=text, **kwargs).to_dict()

    async def evaluate(self, *, text: str, tones: list[str] | None = None, **kwargs) -> ToneResponse:
    ...
    async def batch_evaluate(self, inputs: list[dict]) -> list[ToneResponse]:
