...
def crop_to_alpha(img, target_size=(512, 512)):
    """根据 alpha 通道裁剪非透明区域，并调整为目标尺寸"""
    
    if img.shape[2] == 4:  # 确保图像有 alpha 通道
        alpha = img[:, :, 3]  # 提取 alpha 通道
        coords = np.argwhere(alpha > 0)  # 获取非透明区域的坐标
        
        if coords.size == 0:  # 如果全图透明，直接返回原图
            return img
        ...
        # 使用 PIL 进行等比例缩放
        resized_img = np.array(Image.fromarray(cropped).resize((new_width, new_height), Image.LANCZOS))
        
        # 创建一个透明背景的 512x512 图像
        ...
        # 将缩放后的图像放置到透明背景中
        final_img[offset_y:offset_y + new_height, offset_x:offset_x + new_width] = resized_img
        
        return final_img
    
    return img  # 如果没有 alpha 通道，直接返回原图
...
def to_thumbnail(gaussian):
    ...
    resized_img = crop_to_alpha(rgba_img)
