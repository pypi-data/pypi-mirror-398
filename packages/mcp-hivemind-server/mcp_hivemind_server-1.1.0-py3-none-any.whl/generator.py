...
async def _generate_svg(mmd_path: Path, svg_path: Path) -> Optional[str]:
    """
    ...
    try:
        ...
        if mmdc_cmd:
            ...
            # Fallback to npx
            # Use shell=True for npx on Windows to avoid some path issues, or rely on shutil.which('npx')
            npx_cmd = shutil.which("npx")
            if not npx_cmd:
                return "Mermaid CLI not found (mmdc) and npx not available."
        ...
        if process.returncode != 0:
            return f"Mermaid CLI failed: {process.stderr}"
...
async def write_files(
    ...
    if result.get('success'):
        ...
        error = await _generate_svg(flowchart_path, svg_path)
