...
from generator import generate_and_write, generate_hivemind, generate_flowchart, write_files, generate_svg
...
# Constants
FLOWCHART_FILENAME = "hivemind.mmd"
...
TOOLS = [
    Tool(
        ...
        description="Document the code being built in real-time. AI calls this while actively creating code to preserve context and user requirements.",
        inputSchema={
            "type": "object",
            "properties": {
                "path": {
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "include_external": {
                    "type": "boolean",
                    "description": "Include external npm packages in trace results",
                    "default": False
                }
            },
            "required": ["anchor"]
    ...
    Tool(
        name="render_hive_svg",
        description="Manually re-render an SVG from a Mermaid (.mmd) file. Use this if you have manually edited the .mmd file and want to update the visualization.",
        inputSchema={
            "type": "object",
            "properties": {
                "mmd_path": {
                    "type": "string",
                    "description": "Path to the .mmd file (defaults to hivemind.mmd in current directory if not specified)"
                },
                "output_path": {
                    "type": "string",
                    "description": "Path to output .svg file (defaults to hivemind.svg alongside mmd file)"
                }
            }
        }
    ),
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "include_file_contents": {
                    "type": "boolean",
                    "description": "Include first 100 lines of each file in response (faster context)",
                    "default": True
                }
            },
            "required": ["root_path"]
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        elif name == "render_hive_svg":
            return await handle_render_hive_svg(arguments)
...
async def handle_document_current_work(args: dict) -> list[TextContent]:
    ...
    if result.get('success'):
        messages = [
            ...
            f"   - Generated hivemind.mmd ({result['flowchart_lines']} lines)",
            f"   - Generated hivemind.svg",
        ]
...
async def handle_build_hive(args: dict) -> list[TextContent]:
    ...
    messages.extend([
        ...
        f"   - Generated {stats['directories'] * 3} files (hivemind.md + .mmd + .svg)",
    ])
...
async def handle_trace_usage(args: dict) -> list[TextContent]:
    ...
    # File-to-directory resolution: If anchor points to a file, use its parent directory
    resolved_anchor = anchor
    file_hint = None
    if anchor.startswith("anchor://"):
        anchor_path = anchor[len("anchor://"):]
        test_path = normalize_path(anchor_path)
        if test_path.is_file():
            # User provided a file path, resolve to parent directory
            file_hint = test_path.name
            resolved_anchor = f"anchor://{test_path.parent.as_posix()}"
    
    result = await trace_connections(resolved_anchor, direction, max_depth, root_path, include_external)
    
    if result.get('error'):
        # If we resolved from file to dir, add helpful message
        if file_hint:
            return [TextContent(type="text", text=f"Error: {result['error']}\n\nNote: Resolved file '{file_hint}' to parent directory.")]
        return [TextContent(type="text", text=f"Error: {result['error']}")]
    ...
    if file_hint:
        messages.append(f"   (Resolved from file '{file_hint}' to directory)")
    messages.extend([
        f"   Direction: {direction}, Max depth: {max_depth}",
        f"   Include external: {include_external}",
        "",
        formatted,
    ])
...
async def handle_render_hive_svg(args: dict) -> list[TextContent]:
    """
    Handle render_hive_svg tool call.
    """
    mmd_path_str = args.get("mmd_path")
    output_path_str = args.get("output_path")
    
    current_cwd = Path.cwd()
    
    if mmd_path_str:
        mmd_path = normalize_path(mmd_path_str)
    else:
        # Default to hivemind.mmd in current working directory
        mmd_path = current_cwd / FLOWCHART_FILENAME
        
    if not mmd_path.exists():
        return [TextContent(type="text", text=f"Error: MMD file not found at {mmd_path}")]
        
    if output_path_str:
        svg_path = normalize_path(output_path_str)
    else:
        # Default to same name but .svg extension
        svg_path = mmd_path.with_suffix('.svg')
        
    error = await generate_svg(mmd_path, svg_path)
    
    if error:
        # Check if SVG was created despite error (some warnings are non-fatal)
        if svg_path.exists():
            if "MMDC_NOT_INSTALLED" in error:
                return [TextContent(type="text", text=f"Warning: {error}\n\nSVG generation skipped.")]
            return [TextContent(type="text", text=f"SVG generated with warnings at {svg_path}\n\nWarning: {error}")]
        return [TextContent(type="text", text=f"Error generating SVG: {error}")]
    
    if not svg_path.exists():
        return [TextContent(type="text", text=f"Error: SVG generation completed but file not found at {svg_path}")]
        
    return [TextContent(type="text", text=f"Successfully generated SVG at {svg_path}")]
...
async def handle_start_hive_build(args: dict) -> list[TextContent]:
    ...
    _hive_build_state = {
        ...
        'include_file_contents': args.get("include_file_contents", True)
    }
    ...
    file_previews = []
    if args.get("include_file_contents", True):
        for f in structure.get('files', []):
            try:
                f_path = first_dir / f['name']
                if f_path.is_file():
                    content = f_path.read_text(encoding='utf-8', errors='ignore')
                    lines = content.split('\n')
                    preview = '\n'.join(lines[:100])
                    truncated = "\n... (more)" if len(lines) > 100 else ""
...
async def handle_continue_hive_build(args: dict) -> list[TextContent]:
    ...
    # Get file contents if requested
    file_previews = []
    if _hive_build_state.get('include_file_contents', True):
        for f in next_structure.get('files', []):
            try:
                f_path = next_dir / f['name']
                if f_path.is_file():
                    content = f_path.read_text(encoding='utf-8', errors='ignore')
                    lines = content.split('\n')
                    preview = '\n'.join(lines[:100])
