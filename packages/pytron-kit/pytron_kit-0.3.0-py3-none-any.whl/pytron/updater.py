        ...
        try:
            with urllib.request.urlopen(url, timeout=5) as response:
                ...
                if parse_version(remote_version) > parse_version(self.current_version):
                    self.logger.info(f"Update available: {remote_version} (Current: {self.current_version})")
                    return data
                else:
    ...
    def download_and_install(self, update_info: dict, on_progress=None):
        ...
        try:
            ...
            if sys.platform == 'win32':
                ...
                subprocess.Popen([str(download_path)], shell=True, creationflags=subprocess.DETACHED_PROCESS if hasattr(subprocess, 'DETACHED_PROCESS') else 0)
