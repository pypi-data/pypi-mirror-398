...
class Plugin:
    ...
    def load(self, app_instance):
        ...
        if ':' not in entry_str:
            raise PluginError(f"Invalid entry_point format '{entry_str}'. Expected 'module:function' or 'module:Class'")
        ...
        try:
            ...
            if not hasattr(module, object_name):
                 raise PluginError(f"Entry point '{object_name}' not found in module '{module_name}'")
            ...
            if callable(entry_obj) and not isinstance(entry_obj, type):
                self.logger.info(f"Initializing plugin '{self.name}' via function '{object_name}'")
                entry_obj(app_instance)
                
            # 2. If it's a class, instantiate it with `app`
            elif isinstance(entry_obj, type):
                self.logger.info(f"Initializing plugin '{self.name}' via class '{object_name}'")
