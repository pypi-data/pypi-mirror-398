...
def cmd_uninstall(args: argparse.Namespace) -> int:
    ...
    if not venv_python.exists():
        log(f"Virtual environment not found at {venv_dir}. Only updating {REQUIREMENTS_JSON}.", style="warning")
    else:
    ...
    for dep in current_deps:
        base_name = dep.split('==')[0].split('>=')[0].split('<=')[0].split('<')[0].split('>')[0].strip().lower()
