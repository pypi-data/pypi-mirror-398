...
class AndroidBuilder:
    def __init__(self, arch="aarch64", target_dir=None):
        self.arch = arch
        self.target = "aarch64-linux-android" if arch == "aarch64" else f"{arch}-linux-android"
        self.target_dir = target_dir
        self.zig_version = "0.13.0"
        
        # Paths
        self.zig_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tools", "zig")
        self.zig_exe = self._ensure_zig()
        
        # Flattened libs cache (for Dependency Flattening Strategy)
        self.flattened_libs_dir = os.path.join(os.path.dirname(self.zig_dir), "flattened_libs", self.arch)
    ...
    def repair_wheel(self, wheel_path):
        ...
        try:
            ...
            if repaired:
                print(f"[AndroidBuilder] Repaired and flattened: {os.path.basename(wheel_path)}")
                # Re-pack the wheel
    ...
    def _patch_so(self, so_path, wheel_root):
        ...
        try:
            ...
            for dep in deps_to_fix:
                ...
                if dep in ["libc.so", "libm.so", "libdl.so", "liblog.so", "libjnigraphics.so", "libandroid.so"]:
                    continue
                
                # Check if this dependency exists inside the wheel (it might be in a subdir)
                dep_name = os.path.basename(dep)
                dep_search = glob.glob(os.path.join(wheel_root, "**", dep_name), recursive=True)
            ...
            print(f"[AndroidBuilder] Warning: Failed to patch {os.path.basename(so_path)}: {e}")
            return False
    ...
    def _ensure_zig(self):
        ...
        try:
            ...
            for d in os.listdir(os.path.dirname(self.zig_dir)):
                if d.startswith("zig-") and os.path.isdir(os.path.join(os.path.dirname(self.zig_dir), d)):
                    extracted = self._norm(os.path.join(os.path.dirname(self.zig_dir), d))
    ...
    def _find_ndk_info(self):
        ...
        if ndk_path and os.path.exists(ndk_path):
            ...
            sysroot = self._norm(os.path.join(ndk_path, "toolchains", "llvm", "prebuilt", "windows-x86_64", "sysroot"))
            if not os.path.exists(sysroot):
                 sysroot_glob = glob.glob(os.path.join(ndk_path, "toolchains", "llvm", "prebuilt", "*", "sysroot"))
                 if sysroot_glob: sysroot = self._norm(sysroot_glob[0])
            
            if os.path.exists(sysroot):
                return {
                    "path": ndk_path,
                    "sysroot": self._get_short_path(sysroot),
                    "include": self._get_short_path(self._norm(os.path.join(sysroot, "usr", "include"))),
                    "lib": self._get_short_path(self._norm(os.path.join(sysroot, "usr", "lib", "aarch64-linux-android")))
                }
        ...
        py_inc = glob.glob(os.path.join(py_target, "**", "usr", "include", "python*"), recursive=True)
        ...
        return {
            ...
            "lib": self._get_short_path(py_lib[0]) if py_lib else self._get_short_path(sysroot),
    ...
    def _rename_wheel(self, output_dir):
        ...
        for whl in os.listdir(output_dir):
            ...
            if new_name != original:
                ...
                os.rename(os.path.join(output_dir, original), os.path.join(output_dir, new_name))

    def _create_linker_wrapper(self):
        """Creates a batch wrapper for Zig CC to act as a Linker for Rust/Cargo."""
        if not self.zig_exe: return None
    ...
    def _get_cross_env(self, sysconfig_dir=None):
        ...
        if cargo_linker:
            ...
            env["RUSTFLAGS"] = "-C link-arg=-Wl,--allow-shlib-undefined"
             ...
             env["RUSTFLAGS"] = f"-C linker={zig_safe} -C link-arg=-target -C link-arg={target}"
        ...
        if sysconfig_dir:
            env["PYTHONPATH"] = f"{sysconfig_dir}{os.pathsep}{env.get('PYTHONPATH', '')}"
    ...
    def build_wheel(self, package, output_dir, cpp_include=None):
        ...
        try:
            if os.path.exists(package) and os.path.isdir(package):
                ...
                archives = [f for f in os.listdir(temp_dir) if f.endswith(('.tar.gz', '.zip'))]
            ...
            build_deps = ["wheel", "setuptools", "Cython", "meson-python", "maturin", "pybind11", "scikit-build-core", "ninja", "meson"]
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install"] + build_deps, env=os.environ.copy())
            except: pass
            ...
            if os.path.exists(pyproject_path):
                with open(pyproject_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if 'maturin' in content: is_maturin = True
            
            # 4. Execute Build
            if is_maturin:
                print(f"[AndroidBuilder] Building {package} with Maturin...")
                subprocess.check_call(["rustup", "target", "add", "aarch64-linux-android"])
                cmd = [sys.executable, "-m", "maturin", "build", "--release", "--target", "aarch64-linux-android", "--out", output_dir, "--strip"]
                subprocess.check_call(cmd, cwd=source_dir, env=env)
            else:
                print(f"[AndroidBuilder] Building {package} with Pip/Zig (Isolation OFF)...")
                cmd = [sys.executable, "-m", "pip", "wheel", ".", "--no-deps", "--no-build-isolation", "--wheel-dir", output_dir, "-v"]
                subprocess.check_call(cmd, cwd=source_dir, env=env)
