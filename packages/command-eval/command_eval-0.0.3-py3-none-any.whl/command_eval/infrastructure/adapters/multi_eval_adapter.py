...
class MultiEvalAdapter(EvaluationPort):
    ...
    def evaluate(
        ...
        for sdk_name, cases in sdk_test_cases.items():
                ...
                for tc in cases:
                    if tc.id not in all_results:
                        all_results[tc.id] = PortTestCaseResult(
                            ...
                            passed=False,
                            metrics={},
                            reason=f"No adapter registered for SDK: {sdk_name}",
                        )
                continue
            ...
            # Merge results (later SDK results can override earlier ones)
            for detail in response.details:
                _logger.debug("    Result: %s score=%.2f passed=%s",
                             detail.test_case_id, detail.score, detail.passed)
                existing = all_results.get(detail.test_case_id)
                if existing is None:
                    all_results[detail.test_case_id] = detail
                else:
                    # Merge metrics from both results
                    merged_metrics = {**existing.metrics, **detail.metrics}
                    # Average the scores
                    avg_score = (existing.score + detail.score) / 2
                    all_results[detail.test_case_id] = PortTestCaseResult(
                        test_case_id=detail.test_case_id,
                        score=avg_score,
                        passed=avg_score >= config.threshold,
                        metrics=merged_metrics,
                        reason=detail.reason or existing.reason,
                    )
        ...
        final_response = EvaluationResponse.create(tuple(ordered_results), config.threshold)
        _logger.info("  Final: overall_score=%.2f passed=%d failed=%d",
                    final_response.overall_score,
