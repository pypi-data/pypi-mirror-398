...
class ResultOutputPolicy:
    ...
    def _build_result_data(
        ...
        result_data: dict[str, Any] = {"query": test_input.query}

        # Add actual output from test case
        if input_id in tc_by_input:
            result_data["actual_output"] = tc_by_input[input_id].actual

        # Add console output from execution
        if input_id in exec_by_input:
            exec_result = exec_by_input[input_id].result
            if exec_result and exec_result.console_output:
                result_data["console_output"] = exec_result.console_output
