...
class ExecuteEvaluationRequest:
    ...
    Attributes:
        ...
        trigger_event: The TestCaseCreated event that triggered this request
                      (should have is_last_test_case=True).
    """
...
class EvaluationExecutionPolicy:
    ...
    - Receiving a TestCaseCreated event with is_last_test_case=True
    ...
    def execute(self, request: ExecuteEvaluationRequest) -> ExecuteEvaluationResult:
        ...
        Raises:
            ValueError: If no test cases are provided or trigger event
                       does not indicate last test case.
        """
        # Validate that this is triggered by the last test case
        if not request.trigger_event.is_last_test_case:
            raise ValueError(
                "Evaluation can only be triggered by the last test case"
            )
