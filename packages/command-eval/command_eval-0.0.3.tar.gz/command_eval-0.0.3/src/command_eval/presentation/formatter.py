...
from command_eval.application.evaluation_use_case import EvaluationUseCaseResult
...
class ResultFormatter(Protocol):
    ...
    def format(self, result: EvaluationUseCaseResult, verbose: bool = False) -> str:
...
class TextFormatter(BaseFormatter):
    ...
    def _format_summary(self, result: EvaluationUseCaseResult) -> list[str]:
    ...
    def _format_evaluation_status(self, result: EvaluationUseCaseResult) -> list[str]:
        ...
        lines.append(f"  Overall score: {evaluation.result.overall_score:.4f}")
        lines.append(
            f"  Passed/Failed: {evaluation.result.passed_count}/{evaluation.result.failed_count}"
        )
        return lines

    def _format_detailed_results(self, result: EvaluationUseCaseResult) -> list[str]:
        ...
        for i, tc_result in enumerate(result.evaluation.result.test_case_results):
            ...
            lines.append(f"    Score: {tc_result.score:.4f}")
...
class JsonFormatter(BaseFormatter):
    ...
    def _build_data(
        self, result: EvaluationUseCaseResult, verbose: bool
    ) -> dict:
        ...
        if result.evaluation:
            data["evaluation"] = {
                ...
                "overall_score": result.evaluation.result.overall_score,
