...
from lionweb.language import (Annotation, Concept, Containment, Enumeration,
                              EnumerationLiteral, Interface, Language,
...
def _add_placeholder_node_annotation(language: Language, ast_node: Concept):
    ...
    annotation_type.id = (
        f"{placeholder_node_id.removesuffix('-id')}-PlaceholderNodeType-id"
    )
    annotation_type.key = (
    ...
    original_node.id = f"{placeholder_node_id.removesuffix('-id')}-originalNode-id"
    ...
    type_prop.id = f"{placeholder_node_id.removesuffix('-id')}-type-id"
    ...
    message_prop.id = f"{placeholder_node_id.removesuffix('-id')}-message-id"
...
def ast_language() -> Language:
    ...
    language.id = "com-strumenta-StarLasu"
    ...
    ast_node = Concept(lion_web_version=lw_version, name="ASTNode")
    ...
    issue_type_enum.id = "com-strumenta-StarLasu_IssueType"
    ...
    issue_severity_enum.id = "com-strumenta-StarLasu_IssueSeverity"
    issue_severity_enum.key = "IssueSeverity"
    for name in ["ERROR", "WARNING", "INFO"]:
        literal = EnumerationLiteral(lion_web_version=lw_version, name=name)
        issue_severity_enum.add_literal(literal)
        literal.id = f"com-strumenta-StarLasu_IssueSeverity-{name}"
