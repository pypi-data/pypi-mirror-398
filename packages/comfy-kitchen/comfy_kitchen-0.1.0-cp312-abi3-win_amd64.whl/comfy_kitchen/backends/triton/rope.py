...
def apply_rope_kernel(
    ...
    """Triton kernel for RoPE (Rotary Position Embedding) on query and key tensors.

    Args:
        xq_ptr: Query tensor pointer (batch, n_heads, seq_len, head_dim)
        xk_ptr: Key tensor pointer (batch, n_heads, seq_len, head_dim)
        freqs_ptr: Frequency tensor pointer (batch, heads, seq_len, head_dim//2, 2, 2)
        ...
        freqs_batch, freqs_heads: Frequency tensor batch and head dimensions (for broadcasting)
        stride_*: Stride information for memory access
    ...
    n_pairs = head_dim // 2
    total_elements = batch * n_heads * seq_len * n_pairs
    ...
    # freqs_cis can have batch=1 and/or heads=1 for broadcasting
    freqs_batch_idx = tl.where(freqs_batch == 1, 0, batch_idx)
    freqs_head_idx = tl.where(freqs_heads == 1, 0, head_idx)

    # Calculate offsets for freqs_cis
...
def _apply_rope(x1: torch.Tensor, freqs_cis: torch.Tensor, x2: torch.Tensor = None):
    batch, n_heads, seq_len, head_dim = x1.shape
    freqs_batch, freqs_heads = freqs_cis.shape[0], freqs_cis.shape[1]
    ...
    # Get strides
    stride_x_batch, stride_x_heads, stride_x_seq, stride_x_dim = x1.stride()
