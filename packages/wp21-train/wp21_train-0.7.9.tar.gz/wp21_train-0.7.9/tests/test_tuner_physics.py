...
from wp21_train.training.tuner import PhysicsTuner, PhysicsScore
from wp21_train.physics.performance import PerformanceSummary
from wp21_train.training.trainers import KerasTrainer
import wp21_train as train

class TunerTest(PhysicsTuner):
    def compute_physics_score(self, model, params):
        turnons = [{'pt':[10,20,30]}]
        counts = [500]
        trigger_names=['dummy']
        ps = [PerformanceSummary(e, r, n) for e,r,n in zip(turnons, counts, trigger_names)]
        score = PhysicsScore(ps)
        return score

def test_tuner_physics():
    ...
    trainer = KerasTrainer(test_file, ds, keras_mlp)
    tune = TunerTest("hat", trainer)
    tune.run()
