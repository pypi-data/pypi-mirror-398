...
class UsageFlowClient:
    ...
    def allocate_request(self, ledger_id: str, metadata: Dict[str, Any], has_rate_limit: bool, response_tracking_field: Optional[str] = None) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """Allocate usage for a request"""
        if not self.socket_manager.is_connected():
            return False, {"error": "WebSocket not connected", "status_code": 520}

        amount = 1
        if response_tracking_field:
            amount = 1000  # Set larger amount for response tracking
    ...
    def fulfill_request(self, ledger_id: str, allocation_id: str, metadata: Dict[str, Any], response_tracking_value: Optional[int] = None) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """Fulfill the request by finalizing usage"""
        if not self.socket_manager.is_connected():
            return True, None  # Fail silently if not connected

        amount = 1
        if response_tracking_value:
            amount = response_tracking_value
    ...
    def extract_schema(self, obj, path=''):
        """Extract schema from Python dict/object"""
        ...
        if isinstance(obj, list):
            ...
            return {
                'type': 'array',
                'items': self.extract_schema(obj[0], path)
            }
            ...
            for key, value in obj.items():
                ...
                if value is None:
                    ...
                    schema[key] = {
                        'type': 'array',
                        'path': field_path,
                        'items': self.extract_schema(value[0], field_path) if len(value) > 0 else 'unknown'
                    }
                elif isinstance(value, dict):
                    schema[key] = {
                        'type': 'object',
                        'path': field_path,
                        'properties': self.extract_schema(value, field_path)
                    }
                else:
                    schema[key] = {
