...
def WeeklySellThruReport(mw, progress_callback):
        all_text = ""
        all_text = all_text + "Starting SellThru Report Process\n"
        progress_callback.emit(all_text)
        mw.infotextmisc_lable.setText(r"Started Sellthru Report Process")
        ...
        df = df.filter(pl.col('Status').eq('Active'))
        ...
        pIdx = ["Brand Code","Country","City","Location Code","ShortName","Division","Product Group","Item Category","Item Class","RefCode","Style Code","Colour Code","Season Code","Remarks"]
        ...
        df.write_csv(f"C:\\Reports\\Output\\SellThruData.csv",)
        mw.infotextmisc_lable.setText(r"Sellthru Report Process ended")
        return "Done..."