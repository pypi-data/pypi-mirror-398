...
def startVincciSellthruWorker(mw, progress_callback):
    all_text = ""
    # Get all user inputs
    ...
    stFile = os.path.join(mw.output_folder,"VI_StockSellThruStore.csv")
        ...
        if brand == "VI":
            if merchWeek != "All":
                try:
                    ...
                    SellThru_macro = wb.macro('VincciSellThruReport')
                    StorePages_macro_macro = wb.macro('VincciBestSellerReport')
                    InsertImage_macro = wb.macro('InsertImage')
                    all_text = all_text + "Vincci SellThru Report Process Started\n"
                    progress_callback.emit(all_text)
                    SellThru_macro(stFile, merchWeek)
                    wb.close()
                    app.kill()
                    all_text = all_text + "Vincci SellThru Report Process Ended\n"
                    progress_callback.emit(all_text)
                except:
                    wb.close()
                    app.kill()
