...
class TaskManager:
    ...
    def get_task(self, task_id: str, suppress_error: bool = False) -> Optional[TaskResponse]:
        """Retrieve task information by ID.

        Args:
            task_id: Task identifier
            suppress_error: Whether to suppress error logging if retrieval fails

        Returns:
            Optional[TaskResponse]: Task information if found, None otherwise
        """
        path = f'/task/{task_id}'
        options = {'suppress_error': suppress_error}
        response = self.sws_client.discoverable.get(self.endpoint, path, options=options)
    ...
    def wait_completion(self, task_id: str, poll_interval: int = 10) -> TaskResponse:
        ...
        while True:
            task_response = self.get_task(task_id, suppress_error=True)
