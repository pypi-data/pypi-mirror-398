...
class Discover:
    ...
    def call(self, method: str, endpoint: str, path: str, params: dict = None, headers: dict = None, 
        ...
        try:
            ...
            if options and options.get('suppress_error'):
                logger.debug(f"HTTP Error (suppressed): {errh}")
            else:
                logger.error(f"HTTP Error: {errh}")
                logger.debug(f"HTTP Status Code: {errh.response.status_code}")
                logger.debug(f"Response Text: {errh.response.text}")
                logger.debug(f"Request URL: {errh.request.url}")
                logger.debug(f"Request Body: {response.request.body}")
                logger.debug(f"Request Headers: {response.request.headers}")
        except requests.exceptions.RequestException as err:
            if options and options.get('suppress_error'):
                logger.debug(f"Request Exception (suppressed): {err}")
            else:
