        ...
        submitButtons.forEach(function(button) {
            ...
            switch (self.feedbackState) {
                ...
                case Utils.CONSTANTS.FEEDBACK_PROCESSING:
                    const processingText = window.i18nManager ? window.i18nManager.t('buttons.processing') : '处理中...';
                    button.innerHTML = `<span class="loading" style="width: 14px; height: 14px; border-width: 2px; vertical-align: middle; margin-right: 8px; border-top-color: currentColor; border-left-color: transparent; border-right-color: currentColor; border-bottom-color: currentColor;"></span>${processingText}`;
                    button.className = 'btn btn-secondary';
                    button.disabled = true;
                    break;
                case Utils.CONSTANTS.FEEDBACK_SUBMITTED:
                    const submittedText = window.i18nManager ? window.i18nManager.t('buttons.submitted') : '已提交';
                    button.innerHTML = `<span style="margin-right: 6px;">✓</span>${submittedText}`;
