...
def check_auth(request: Request) -> bool:
    ...
    debug_log(f"认证检查 - 启用状态: {auth_manager.enabled}")
    if not auth_manager.enabled:
        debug_log("认证已禁用，跳过检查")
        return True
    session_id = auth_manager.get_session_from_request(request)
    result = auth_manager.validate_session(session_id) is not None
    debug_log(f"认证检查结果: {result}, session_id: {session_id}")
    return result
...
def load_user_layout_settings() -> str:
    ...
    try:
        # 使用統一的設定檔案路徑
        config_dir = Path.home() / ".config" / "mcp-ai-forever"
