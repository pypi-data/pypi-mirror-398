...
此檔案允許套件透過 `python -m mcp_ai_forever` 執行。

使用方法:
  python -m mcp_ai_forever        # 啟動 MCP 伺服器
  python -m mcp_ai_forever test   # 執行測試
"""
...
#### 連結和引用
- 專案連結：[MCP Feedback Enhanced](https://github.com/example/mcp-ai-forever)
        ...
        try:
            ...
            def import_desktop_app():
                ...
                if os.path.exists(tauri_python_path):
                    ...
                    try:
                        from mcp_ai_forever_desktop import (  # type: ignore
            ...
            if launch_desktop_app_func is None:
                ...
                print("   3. Web 模式指令：uvx mcp-ai-forever test --web")
                return False
...
def show_version():
    ...
    print("GitHub: https://github.com/Minidoracat/mcp-ai-forever")
