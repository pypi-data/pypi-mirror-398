...
(function() {
     ...
     * 初始化 VIP 狀態顯示
     */
    FeedbackApp.prototype.initializeVipStatus = function() {
        const self = this;
        console.log('🎫 初始化 VIP 狀態...');
        ...
        fetch('/api/auth/status')
            ...
            .then(function(data) {
                if (data.authenticated && data.is_vip && data.vip_expire_at) {
                    self.updateVipDisplay(data.vip_expire_at);
                    // 每小時更新一次 VIP 剩餘時間
                    self._vipUpdateInterval = setInterval(function() {
                        self.updateVipDisplay(data.vip_expire_at);
                    }, 3600000); // 1小時 = 3600000ms
                } else {
                    // 非 VIP 用戶，隱藏 VIP 顯示
                    const vipContainer = document.getElementById('statsVipContainer');
                    if (vipContainer) {
            ...
            .catch(function(error) {
                console.warn('⚠️ 獲取 VIP 狀態失敗:', error);
                // 隱藏 VIP 顯示
                const vipContainer = document.getElementById('statsVipContainer');
                if (vipContainer) {
                    vipContainer.style.display = 'none';
     ...
     * 更新 VIP 剩餘時間顯示
     */
    FeedbackApp.prototype.updateVipDisplay = function(expireAt) {
        const vipContainer = document.getElementById('statsVipContainer');
        const vipRemaining = document.getElementById('statsVipRemaining');
        
        if (!vipContainer || !vipRemaining) return;
        ...
        if (diffMs <= 0) {
            // VIP 已過期
            vipContainer.classList.add('vip-expired');
            vipRemaining.textContent = '已过期';
            return;
        }
        ...
        let displayText = '';
        if (diffDays > 0) {
            displayText = diffDays + '天' + diffHours + '时';
        } else if (diffHours > 0) {
            displayText = diffHours + '时' + diffMinutes + '分';
        } else {
            displayText = diffMinutes + '分';
        }

        vipRemaining.textContent = displayText;

        // 如果剩餘不到 3 天，添加警告樣式
        if (diffDays < 3) {
            vipContainer.classList.add('vip-expiring');
        } else {
            vipContainer.classList.remove('vip-expiring');
        }
    };
    ...
    FeedbackApp.prototype._originalHandleSessionUpdated = function(data) {
        ...
        if (data.session_info) {
            ...
            if (data.session_info.project_directory) {
                ...
                document.title = 'MCP AI Forever - ' + projectName;
            }
    ...
    FeedbackApp.prototype.handleSubmitError = function() {
        ...
        if (feedbackState === window.MCPFeedback.Utils.CONSTANTS.FEEDBACK_SUBMITTED) {
            const submittedWarning = window.i18nManager ? window.i18nManager.t('feedback.alreadySubmitted') : '反馈已提交，请等待下次 MCP 调用';
            ...
            if (feedbackData) {
                ...
                const connectingMessage = window.i18nManager ? window.i18nManager.t('feedback.connectingMessage') : 'WebSocket 连接中，反馈将在连接就绪后自动提交...';
    ...
    FeedbackApp.prototype.collectFeedbackData = function() {
        ...
        if (!feedback && images.length === 0) {
            const message = window.i18nManager ? 
                window.i18nManager.t('feedback.provideTextOrImage', '请提供反馈文字或上传图片') : 
                '请提供反馈文字或上传图片';
...
`;
    ...
    FeedbackApp.prototype.cleanup = function() {
        ...
        // 清理 VIP 更新定時器
        if (this._vipUpdateInterval) {
            clearInterval(this._vipUpdateInterval);
        }
