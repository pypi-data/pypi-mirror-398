...
class TestSuiteSpharaFilter(unittest.TestCase):
    ...
    def test_filter_unit_allpass_simple(self):
        ...
        sf_unit_simple = sf.SpharaFilter(testtrimesh, mode="unit", specification=0)

        # the data to filter
        data = np.concatenate(
            [[[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]], np.transpose(sf_unit_simple.basis()[0])]
        )

        # apply SPHARA based spatial allpass filter
        data_filt_unit_simple = sf_unit_simple.filter(data)

        self.assertTrue(np.allclose(data_filt_unit_simple, data))
    ...
    def test_filter_unit_dc_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_unit_simple = sf.SpharaFilter(testtrimesh, mode="unit", specification=1)
        ...
        data_filt_ref[3] = [0.0, 0.0, 0.0]
        data_filt_ref[4] = [0.0, 0.0, 0.0]
    ...
    def test_filter_unit_low_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_unit_simple = sf.SpharaFilter(testtrimesh, mode="unit", specification=[1.0, 1.0, 0.0])
    ...
    def test_filter_ie_allpass_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_ie_simple = sf.SpharaFilter(testtrimesh, mode="inv_euclidean", specification=0)

        # the data to filter
        data = np.concatenate(
            [[[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]], np.transpose(sf_ie_simple.basis()[0])]
        )

        # apply SPHARA based spatial allpass filter
        data_filt_ie_simple = sf_ie_simple.filter(data)

        self.assertTrue(np.allclose(data_filt_ie_simple, data))
    ...
    def test_filter_ie_dc_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_ie_simple = sf.SpharaFilter(testtrimesh, mode="inv_euclidean", specification=1)
        ...
        self.assertTrue(np.allclose(data_filt_ie_simple, data_filt_ref))
    ...
    def test_filter_ie_low_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_ie_simple = sf.SpharaFilter(
            testtrimesh, mode="inv_euclidean", specification=[1.0, 1.0, 0.0]
        )
    ...
    def test_filter_fem_allpass_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_fem_simple = sf.SpharaFilter(testtrimesh, mode="fem", specification=0)

        # the data to filter
        data = np.concatenate(
            [[[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]], np.transpose(sf_fem_simple.basis()[0])]
        )
    ...
    def test_filter_fem_dc_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_fem_simple = sf.SpharaFilter(testtrimesh, mode="fem", specification=1)
        ...
        self.assertTrue(np.allclose(data_filt_fem_simple, data_filt_ref))
    ...
    def test_filter_fem_low_simple(self):
        ...
        # create a SPHARA filter instance for the mesh
        sf_fem_simple = sf.SpharaFilter(testtrimesh, mode="fem", specification=[1.0, 1.0, 0.0])
