...
TESTDATA_SPHARABASISUNITWEIGHT = os.path.join(
    os.path.dirname(__file__), "testdata/spharabasisunitweight.csv"
)

# SPHARA natural frequencies unit weight
TESTDATA_SPHARANATFREQUNITWEIGHT = os.path.join(
    os.path.dirname(__file__), "testdata/spharanatfrequnitweight.csv"
)

# SPHARA basis inverse euclidean weight
TESTDATA_SPHARABASISIEWEIGHT = os.path.join(
    os.path.dirname(__file__), "testdata/spharabasisieweight.csv"
)

# SPHARA natural frequencies inverse euclidean weight
TESTDATA_SPHARANATFREQIEWEIGHT = os.path.join(
    os.path.dirname(__file__), "testdata/spharanatfreqieweight.csv"
)
...
# SPHARA basis FEM
TESTDATA_SPHARABASISFEMWEIGHT = os.path.join(
    os.path.dirname(__file__), "testdata/spharabasisfemweight.csv"
)

# SPHARA natural frequencies FEM
TESTDATA_SPHARANATFREQFEMWEIGHT = os.path.join(
    os.path.dirname(__file__), "testdata/spharanatfreqfemweight.csv"
)
...
class TestSuiteSpharaBasis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(TESTDATA_SPHARABASISUNITWEIGHT) as f:
            reader = csv.reader(f)
        ...
        with open(TESTDATA_SPHARANATFREQUNITWEIGHT) as f:
            reader = csv.reader(f)
        ...
        with open(TESTDATA_SPHARABASISIEWEIGHT) as f:
            reader = csv.reader(f)
            temp = [[float(y) for y in x] for x in list(reader)]
        ...
        with open(TESTDATA_SPHARANATFREQIEWEIGHT) as f:
            reader = csv.reader(f)
            temp = np.asarray([[float(y) for y in x] for x in list(reader)])
        ...
        with open(TESTDATA_SPHARABASISFEMWEIGHT) as f:
            reader = csv.reader(f)
            temp = [[float(y) for y in x] for x in list(reader)]
        ...
        with open(TESTDATA_SPHARANATFREQFEMWEIGHT) as f:
            reader = csv.reader(f)
            temp = np.asarray([[float(y) for y in x] for x in list(reader)])
    ...
    def test_basis_unit_simple(self):
        ...
        """
        testtrimesh = tm.TriMesh([[0, 1, 2]], [[1, 0, 0], [0, 2, 0], [0, 0, 3]])
        ...
        self.assertTrue(
            ...
            and np.allclose(sb_unit_freq, [2.22044605e-15, 3.00000000e00, 3.00000000e00])
        )
    ...
    def test_basis_unit_result(self):
        ...
        self.assertTrue(
            np.allclose(
                np.absolute(
                    np.matmul(np.transpose(sb_unit_fun), self.testdataspharabasisunitweight)
                ),
                np.identity(np.size(sb_unit_freq)),
            )
            and np.allclose(sb_unit_freq, self.testdataspharanatfrequnitweight)
        )
    ...
    def test_basis_inveuclid_simple(self):
        ...
        self.assertTrue(
            ...
            and np.allclose(sb_ie_freq, [0.0, 0.886644827600501, 1.19493809165832])
        )
    ...
    def test_basis_ie_result(self):
        ...
        self.assertTrue(
            np.allclose(
                np.absolute(np.matmul(np.transpose(sb_ie_fun), self.testdataspharabasisieweight)),
                np.identity(np.size(sb_ie_freq)),
            )
            and np.allclose(sb_ie_freq, self.testdataspharanatfreqieweight)
        )
    ...
    def test_basis_fem_simple(self):
        ...
        self.assertTrue(
            ...
            and np.allclose(sb_fem_freq, [2.33627569e-16, 1.71428571e00, 5.14285714e00])
        )
    ...
    def test_basis_fem_result(self):
        ...
        self.assertTrue(
            np.allclose(
                np.absolute(
                    np.matmul(
                        np.matmul(np.transpose(sb_fem_fun), self.testdatamassmatrix),
