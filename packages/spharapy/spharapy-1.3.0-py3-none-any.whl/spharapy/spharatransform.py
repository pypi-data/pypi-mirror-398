...
class SpharaTransform(SpharaBasis):
    ...
    def __init__(self, triangsamples=None, mode="fem"):
        ...
        self._basis: FloatArray | None = None
        self._frequencies: FloatArray | None = None
        self._massmatrix: FloatArray | None = None

    def _ensure_basis(self) -> None:
        """Ensure basis (and mass matrix for FEM) are initialized."""
        if self._basis is None or self._frequencies is None:
            self.basis()
        if self._basis is None:
            raise RuntimeError("SPHARA basis not initialized; call basis() or fit().")
        if self._mode == "fem" and self._massmatrix is None:
            raise RuntimeError("Mass matrix not initialized for FEM mode.")

    def analysis(self, data: ArrayLike) -> FloatArray:
        ...
        SPHARA basis functions :math:`\tilde{\boldsymbol{X}} =
        \boldsymbol{X} \cdot \boldsymbol{S}`, with the SPHARA basis
        :math:`\boldsymbol{S}`, the data matrix :math:`\boldsymbol{X}`
        and the SPHARA coefficients matix
        :math:`\tilde{\boldsymbol{X}}`. In the forward transformation
        using SPHARA basic functions determined by discretization with
        FEM approach, the modified scalar product including the mass
        matrix is used :math:`\tilde{\boldsymbol{X}} = \boldsymbol{X}
        \cdot \boldsymbol{B} \cdot \boldsymbol{S}`, with the mass
        ...
        if self._mode == "fem":
            ...
            coefficients: FloatArray = data @ M @ basis
        else:
            coefficients = data @ basis

        return coefficients

    def synthesis(self, coefficients: ArrayLike) -> FloatArray:
        ...
        matrix with SPHARA basis functions
        :math:`\tilde{\boldsymbol{X}} = \boldsymbol{X} \cdot
        \boldsymbol{S}`, with the SPHARA basis :math:`\boldsymbol{S}`,
        the data matrix :math:`\boldsymbol{X}` and the SPHARA
        coefficients matix :math:`\tilde{\boldsymbol{X}}`. In the
        forward transformation using SPHARA basic functions determined
        by discretization with FEM approach, the modified scalar
        product including the mass matrix is used
        :math:`\tilde{\boldsymbol{X}} = \boldsymbol{X} \cdot
        \boldsymbol{B} \cdot \boldsymbol{S}`, with the mass matrix
        :math:`\boldsymbol{B}`.
        ...
        # the basis functions?
        if basis.shape[0] != coefficients.shape[1]:
            raise ValueError("Dimension mismatch")

        # compute the data from SPHARA coefficients
        data: FloatArray = coefficients @ basis.T
