...
class TestLoadToolFromFunction:
    ...
    def test_load_tool_with_name_override(self):
        from agent_framework import AIFunction
        
        # AIFunction should be recreated with new name (from tools.yaml key)
        func = load_tool_from_function(
            f"{__name__}:sample_tool",
            name_override="retrieve_standard_regulation",
        )
        assert callable(func)
        assert isinstance(func, AIFunction)
        # Name should be overridden
        assert func.name == "retrieve_standard_regulation"
        # Description should remain the same
        assert func.description == "Original description"

    def test_load_tool_with_name_and_description_override(self):
        from agent_framework import AIFunction
        
        # Both name and description overridden
        func = load_tool_from_function(
            f"{__name__}:sample_tool",
            name_override="retrieve_doc_chunk",
            description_override="Search document chunks",
        )
        assert callable(func)
        assert isinstance(func, AIFunction)
        assert func.name == "retrieve_doc_chunk"
        assert func.description == "Search document chunks"
