...
def load_tool_from_function(
    ...
    *,
    ...
    The @ai_function decorator's `name` parameter serves as a fallback default.
    When loaded via tools.yaml, the tool key overrides this name, allowing
    the same function to be reused by multiple tools with different names
    and configurations.

    Args:
        function_path: Python path to the function (module:function)
        name_override: Override the @ai_function name (e.g., from tools.yaml key).
            When tools.yaml defines a tool, this is automatically set to the tool key.
    ...
    if name_override or description_override or parameter_overrides:
        func = _apply_overrides(func, name_override, description_override, parameter_overrides)
    ...
    *,
    ...
    if ref in manifest.tools:
        ...
        tool = load_tool_from_function(
            ...
            name_override=ref,  # Use tools.yaml key as the tool name
...
def _wrap_tool_with_config(
    ...
    *,
    ...
    if isinstance(func, AIFunction):
        ...
        # Preserve the original signature to avoid forwarding runtime kwargs.
        try:
            wrapped_func.__signature__ = inspect.signature(func.func)  # type: ignore[attr-defined]
        except (TypeError, ValueError):
            pass
        wrapped_ai = AIFunction(
            name=func.name,
        ...
        # Recompute runtime-kwargs forwarding based on the (restored) signature.
        try:
            sig = inspect.signature(wrapped_func)
            wrapped_ai._forward_runtime_kwargs = any(
                p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()
            )
        except (TypeError, ValueError):
            wrapped_ai._forward_runtime_kwargs = False
        return wrapped_ai
...
def _apply_overrides(
    ...
    """Apply name, description and parameter overrides to an AI function.
    ...
    if isinstance(func, AIFunction):
        return _create_overridden_ai_function_from_instance(
            func, name_override, description_override, parameter_overrides
        )
...
def _create_overridden_ai_function_from_instance(
    ...
    """Create a new AIFunction with overridden name, description and parameters.
    
    This creates a new AIFunction instance that wraps the original,
    with modified name, description and/or parameter schema.
    """
    from agent_framework import AIFunction
    ...
    new_name = name_override if name_override else original_name
    ...
    try:
        original_sig = inspect.signature(original.func) if getattr(original, "func", None) else None
    except (TypeError, ValueError):
        original_sig = None
    if original_sig is not None:
        try:
            wrapped_invoke.__signature__ = original_sig  # type: ignore[attr-defined]
            sync_wrapped.__signature__ = original_sig  # type: ignore[attr-defined]
        except (TypeError, ValueError):
            pass
    ...
    # Recompute runtime-kwargs forwarding based on the (restored) signature.
    if original_sig is not None:
        new_ai_function._forward_runtime_kwargs = any(
            p.kind == inspect.Parameter.VAR_KEYWORD for p in original_sig.parameters.values()
        )
    
    LOGGER.debug(
        "Created overridden AIFunction '%s' (original: '%s') with description='%s', parameter_overrides=%s",
        new_name,
        original_name,
