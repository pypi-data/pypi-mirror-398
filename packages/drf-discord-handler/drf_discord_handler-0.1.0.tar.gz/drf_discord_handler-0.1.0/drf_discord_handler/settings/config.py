...
class DiscordHandlerConfig:
    ...
    def load_config(cls) -> Dict[str, Any]:
        ...
        cls._config = {
            ...
            "n_plus_one_webhook_url": config.get("N_PLUS_ONE_WEBHOOK_URL"),
            "n_plus_one_enabled": config.get("N_PLUS_ONE_ENABLED", False),
            "n_plus_one_threshold": config.get("N_PLUS_ONE_THRESHOLD", 50),
    ...
    @classmethod
    def get_n_plus_one_webhook_url(cls) -> Optional[str]:
        """Return Discord Webhook URL for N+1 query detection."""
        config = cls.load_config()
        return config.get("n_plus_one_webhook_url")

    @classmethod
    def is_n_plus_one_enabled(cls) -> bool:
        """Check if N+1 query detection is enabled."""
        config = cls.load_config()
        return config.get("n_plus_one_enabled", False)

    @classmethod
    def get_n_plus_one_threshold(cls) -> int:
        """Return N+1 query detection threshold."""
        config = cls.load_config()
        return config.get("n_plus_one_threshold", 50)
