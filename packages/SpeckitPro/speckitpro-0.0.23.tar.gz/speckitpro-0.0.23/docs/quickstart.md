...
> [!NOTE]
> All automation scripts now provide both Bash (`.sh`) and PowerShell (`.ps1`) variants. The `specifypro` CLI auto-selects based on OS unless you pass `--script sh|ps`.
...
### Step 1: Install SpecifyPro

**In your terminal**, run the `specifypro` CLI command to initialize your project:
...
```bash
uvx --from git+https://github.com/github/spec-kit.git specifypro init <PROJECT_NAME> --script ps  # Force PowerShell
uvx --from git+https://github.com/github/spec-kit.git specifypro init <PROJECT_NAME> --script sh  # Force POSIX shell
```
