...
All notable changes to the SpecifyPro CLI and templates are documented here.
...
- You can now see Git-sourced errors in the SpecifyPro CLI.
...
- Support for using `.` as a shorthand for current directory in `specifypro init .` command, equivalent to `--here` flag but more intuitive for users.
