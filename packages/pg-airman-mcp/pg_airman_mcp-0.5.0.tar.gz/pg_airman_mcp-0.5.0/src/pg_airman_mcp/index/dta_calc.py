...
class DatabaseTuningAdvisor(IndexTuningBase):
    def __init__(
        ...
        :param min_time_improvement: stop when relative improvement
    ...
    async def _generate_recommendations(
        ...
        existing_index_defs: set[str] = {
            idx["definition"] for idx in await self._get_existing_indexes()
        }

        logger.debug(
            f"Existing indexes ({len(existing_index_defs)}): "
            f"{pp_list(list(existing_index_defs))}"
        )

        # generate initial candidates
        all_candidates = await self.generate_candidates(
            query_weights, existing_index_defs
        )

        self.dta_trace(
            f"All candidates ({len(all_candidates)}): {candidate_str(all_candidates)}"
        )
    ...
    async def generate_candidates(
        ...
        for table, cols in table_columns.items():
            # TODO: Optimize by prioritizing columns from filters/joins;
            # current approach generates all combinations
            col_list = list(cols)
        ...
        if len(condition_filtered) > 0:
            ...
            result = await self.sql_driver.execute_query(
                "SELECT index_name, hypopg_relation_size(indexrelid) as index_size "
    ...
    async def _enumerate_greedy(
        ...
        self.dta_trace(
            f"  - Parameters: alpha={alpha}, "
            f"min_time_improvement={min_time_improvement}"
        )
        self.dta_trace(
            f"  - Initial indexes: {len(current_indexes)}, Candidates: "
        ...
        self.dta_trace(
            ...
            f"Space={humanize.naturalsize(current_space)} (Base: "
            f"{humanize.naturalsize(base_relation_size)}, "
        ...
        while True:
            ...
            for candidate in candidate_indexes:
                ...
                if (
                    ...
                    self.dta_trace(
                        ...
                        "because total "
                        "index size ("
                        f"{humanize.naturalsize(test_space - base_relation_size)}) "
                        "exceeds "
                ...
                test_time = await self._evaluate_configuration_cost(
                    queries,
                    frozenset(
                        idx.index_definition for idx in current_indexes | {candidate}
                    ),
                )
                ...
                if time_improvement < min_time_improvement:
                    self.dta_trace(
                        f"  - Skipping candidate: {candidate_str([candidate])} because "
                        "time improvement is below threshold"
                    )
                    continue
                ...
                # Select the index with the best time improvement
                # that meets our threshold
                if (
                    test_objective < best_objective
                    and time_improvement > best_time_improvement
                ):
                    self.dta_trace(
                        f"  - Updating best candidate: {candidate_str([candidate])}"
                    )
                    ...
                    self.dta_trace(
                        f"  - Skipping candidate: {candidate_str([candidate])} because "
                        "it doesn't have the best objective improvement"
                    )

            # If no improvement or no valid candidates, stop
            if best_index is None:
                self.dta_trace(
                    "STOPPED SEARCH: No indexes found with time improvement >= "
                    f"{min_time_improvement:.2%}"
                )
                break
            ...
            self.dta_trace(
                ...
                f"\n    + New objective: {best_objective:.4f} (improvement: "
                f"{objective_improvement:.4f})"
            )
    ...
    def _filter_candidates_by_query_conditions(
        self,
        workload: list[tuple[str, SelectStmt, float]],
    ...
    async def _filter_long_text_columns(
        ...
        for row in result:
            ...
            # Use avg_width from pg_stats as a heuristic - if it's high,
            # likely contains long text
            if potential_long and (
                avg_width is None or avg_width > max_text_length * 0.4
            ):
                problematic_columns.add((table, column))
                logger.debug(
                    f"Identified potentially long text column: {table}.{column} "
                    f"(avg_width: {avg_width})"
                )
            elif potential_long:
        ...
        for candidate in candidates:
            ...
            for column in candidate.columns:
                if (candidate.table, column) in problematic_columns:
                    valid = False
                    logger.debug(
                        "Skipping index candidate with long text column: "
    ...
    def _index_exists(
        self, index: IndexRecommendation, existing_defs: set[str]
    ) -> bool:
        """
        ...
        try:
            ...
            if isinstance(expr, FuncCall):
                ...
                if hasattr(expr, "funcname") and expr.funcname:
                    func_name = ".".join(
                        [name.sval for name in expr.funcname if hasattr(name, "sval")]
                    )
                else:
            ...
            # Check for ColumnRef type directly
                ...
                if hasattr(expr, "fields") and expr.fields:
                    return ".".join(
                        [field.sval for field in expr.fields if hasattr(field, "sval")]
                    )
    ...
    def _is_same_index(self, index1: dict[str, Any], index2: dict[str, Any]) -> bool:
        ...
        # Except when a primary key (which is unique) exists and we're
        # considering a non-unique index on same column
