...
class DocumentProcessor:
    ...
    def recursive_split(self, text: str) -> List[str]:
        chunks = []
        start = 0
        chunk_size = max(1, int(self.config.chunk_size) if self.config.chunk_size else 1)
        chunk_overlap = max(0, min(int(self.config.chunk_overlap) if self.config.chunk_overlap else 0, chunk_size - 1))
        ...
        while start < len(text):
            ...
            for sep in separators:
                ...
                if idx > start and idx < end:
                    split_index = end if sep == '' else idx + len(sep)
                    break
            ...
            next_start = max(start + 1, split_index - chunk_overlap)
            start = next_start
