...
class ReportGenerator:
    ...
    def create_initial_json_report(self, device_info: Dict[str, Any] = None,
        ...
        report_data = {
            "report_info": {
                ...
                "status": "failed",  # 初始状态为失败
                "description": "未正常执行完成并生成报告，可能任务超时停止"  # 初始状态描述
            },
            "summary": {
                "total_tests": 0,
                "passed": 0,
    ...
    def generate_report(self, test_results: List[TestResult], 
                       ...
                       exit_code: int = None,
                       execution_start_time: Optional[datetime] = None) -> Union[str, List[str]]:
        """
        ...
        if self.initial_json_path and self.initial_json_path.exists():
            ...
            json_report_path = self._update_json_report(test_results, device_info, timestamp, global_monitor_result, exit_code, execution_start_time)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            json_report_path = self._generate_json_report(test_results, device_info, timestamp, global_monitor_result, exit_code, execution_start_time)
    ...
    def _update_json_report(self, test_results: List[TestResult], 
        ...
        # 根据退出码和测试结果更新状态和描述
        # 如果退出码为None，保持初始状态（未正常执行完成）
        if exit_code is None:
            # 保持初始状态，表示任务未正常执行完成
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "未正常执行完成并生成报告，可能任务超时停止"
        elif exit_code == 0:
            # 成功
            report_data["report_info"]["status"] = "success"
            report_data["report_info"]["description"] = "任务正常执行完成"
        elif exit_code == 2:
            # RUNNER_ERROR - 其他脚本异常
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "脚本执行异常"
        elif exit_code == 3:
            # INSTALL_ERROR - 安装失败
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "应用安装失败"
        elif exit_code == 5:
            # SCRIPT_ASSERT_ERROR - 脚本断言失败
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "测试用例断言失败"
        elif exit_code == 10:
            # DEVICE_OFFLINE - 手机掉线
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "设备离线"
        elif exit_code == 17:
            # CRASH - 应用崩溃
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "应用崩溃"
        elif exit_code == 18:
            # ANR - 应用无响应
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = "应用无响应(ANR)"
        else:
            # 其他未知退出码
            report_data["report_info"]["status"] = "failed"
            report_data["report_info"]["description"] = f"任务异常结束，退出码: {exit_code}"
    ...
    def _generate_json_report(self, test_results: List[TestResult], 
        ...
        # 根据退出码和测试结果确定状态和描述
        if exit_code is None:
            # 退出码为None，表示任务未正常执行完成
            status = "failed"
            description = "未正常执行完成并生成报告，可能任务超时停止"
        elif exit_code == 0:
            status = "success"
            description = "任务正常执行完成"
        elif exit_code == 2:
            status = "failed"
            description = "脚本执行异常"
        elif exit_code == 3:
            ...
            status = "failed"
            description = "应用无响应(ANR)"
        else:
            status = "failed"
            description = f"任务异常结束，退出码: {exit_code}"
