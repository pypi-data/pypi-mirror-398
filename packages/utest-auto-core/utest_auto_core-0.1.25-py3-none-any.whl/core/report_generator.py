...
class ReportGenerator:
    ...
    def _update_json_report(self, test_results: List[TestResult], 
        ...
        if exit_code is None:
            base_description = "未正常执行完成并生成报告，可能任务超时停止"
        elif exit_code == 0:
            base_description = "任务正常执行完成"
        elif exit_code == 2:
            base_description = "脚本执行异常"
        elif exit_code == 3:
            base_description = "应用安装失败"
        elif exit_code == 5:
            base_description = "测试用例断言失败"
        elif exit_code == 10:
            base_description = "设备离线"
        elif exit_code == 17:
            base_description = "应用崩溃"
        elif exit_code == 18:
            base_description = "应用无响应(ANR)"
        else:
            base_description = f"任务异常结束，退出码: {exit_code}"
