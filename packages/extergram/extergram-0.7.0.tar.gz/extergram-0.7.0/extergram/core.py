...
# Ensure ChatPermissions is imported from api_types.py
...
class Bot:
    ...
    def __init__(self, token: str, default_parse_mode: str = None):
        ...
        # --- Attributes for the anti-flood system ---
        self._request_timestamps = deque()
        self._min_delay = 0.1  # Minimum delay under moderate load
        self._max_delay = 0.5  # Maximum delay under high load

    async def _apply_anti_flood(self):
        """
        Automatically adjusts the request rate to prevent exceeding API limits.
        This method introduces a dynamic delay depending on the frequency of recent requests.
        """
        current_time = time()
        
        # Remove timestamps older than 2 seconds from the queue
        while self._request_timestamps and self._request_timestamps[0] <= current_time - 2:
            self._request_timestamps.popleft()

        # Determine the required delay based on the number of recent requests
        recent_requests = len(self._request_timestamps)
        delay = 0
        
        # Dynamic delay logic
        if recent_requests > 25:  # High load, apply maximum delay
            delay = self._max_delay
        elif recent_requests > 15: # Medium-high load
            delay = (self._min_delay + self._max_delay) / 2 # e.g., 0.3s
        elif recent_requests > 5: # Moderate load, apply minimum delay
    ...
    async def promote_chat_member(self, chat_id: int, user_id: int, **permissions):
        """Promotes a user to an administrator and/or changes their permissions."""
        params = {'chat_id': chat_id, 'user_id': user_id}
        # permissions are can_change_info, can_post_messages, etc.
...
class ContextTypes:
    ...
    def message(self) -> Message:
        """Returns the message object from the update, if it exists."""
        return self.update.message or self.update.edited_message

    @property
    def callback_query(self) -> CallbackQuery:
        """Returns the callback query object from the update, if it exists."""
