...
class LoadCalData:
    ...
    def load_base_data_partition(self):
        ...
        partition_dct = mtqs.get_date_hour_partitions(schema_table_name=self.base_data_source)
        self.base_data_p_date_str = partition_dct['date']
        self.base_data_p_hour_str = partition_dct['hour']
        logger.info(f"算价基础数据表的最新分区读取完成 p_date_str:{self.base_data_p_date_str},p_hour_str:{self.base_data_p_hour_str}")
