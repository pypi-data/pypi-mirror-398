...
(function() {
    ...
    function SessionUIRenderer(options) {
        ...
        // WebSocket 監聽器設置標志
        this.webSocketListenerSetup = false;

        // 保存原始的 WebSocket 消息處理方法
        this.originalHandleWebSocketMessage = null;
    ...
    SessionUIRenderer.prototype.initializeElements = function() {
        ...
        this.activeConversationsList = DOMUtils.safeQuerySelector('#activeConversationsList');
        ...
        const self = this;
        setTimeout(function() {
            self.fetchAndRenderActiveConversations();
        }, 500);
     ...
     * 綁定刷新按鈕事件
     */
    SessionUIRenderer.prototype.bindRefreshButton = function() {
        const self = this;
        const refreshBtn = DOMUtils.safeQuerySelector('#refreshConversationsBtn');
        if (refreshBtn) {
    ...
    /**
     * 設置 WebSocket 監聽器，動態監聽會話變化並自動刷新
     */
    SessionUIRenderer.prototype.setupWebSocketListeners = function() {
        const self = this;

        // 如果已經設置過監聽器，不再重複設置
        if (this.webSocketListenerSetup) {
            logger.info('WebSocket 監聽器已設置，跳過重複設置');
            return;
        }

        // 保存原始的消息處理方法（只保存一次）
        if (!this.originalHandleWebSocketMessage) {
            this.originalHandleWebSocketMessage = window.feedbackApp ? window.feedbackApp._originalHandleWebSocketMessage : null;
        }

        if (!this.originalHandleWebSocketMessage) {
            logger.warn('WebSocket 監聽器設置失敗：找不到 feedbackApp._originalHandleWebSocketMessage');
            return;
        }

        // 攔截 WebSocket 消息處理
        window.feedbackApp._originalHandleWebSocketMessage = function(data) {
            // 先執行原始的消息處理
            self.originalHandleWebSocketMessage.call(this, data);

            // 檢查是否需要刷新活躍會話列表
            const shouldRefresh = self.shouldRefreshConversations(data);

            if (shouldRefresh) {
                if (DEBUG_MODE) console.log('🔄 WebSocket 消息觸發活躍會話列表刷新:', data.type || data.action);

                // 使用防抖避免頻繁刷新
                if (self.refreshDebounceTimer) {
                    clearTimeout(self.refreshDebounceTimer);
                }

                self.refreshDebounceTimer = setTimeout(function() {
                    self.fetchAndRenderActiveConversations();
                }, 300);
            }
        };

        // 設置頁面可見性變化監聽（當用戶切換回標簽頁時刷新）
        this.setupVisibilityListener();

        this.webSocketListenerSetup = true;
        logger.info('WebSocket 監聽器已設置，將自動刷新活躍會話列表');
    };

    /**
     * 設置頁面可見性變化監聽
     */
    SessionUIRenderer.prototype.setupVisibilityListener = function() {
        const self = this;

        // 當頁面從後台切換到前台時刷新
        document.addEventListener('visibilitychange', function() {
            if (!document.hidden) {
                if (DEBUG_MODE) console.log('🔄 頁面可見性變化，刷新活躍會話列表');

                // 使用防抖避免頻繁刷新
                if (self.visibilityDebounceTimer) {
                    clearTimeout(self.visibilityDebounceTimer);
                }

                self.visibilityDebounceTimer = setTimeout(function() {
                    self.fetchAndRenderActiveConversations();
                }, 200);
            }
        });

        logger.info('頁面可見性監聽器已設置');
    };

    /**
     * 判斷是否需要刷新活躍會話列表
     */
    SessionUIRenderer.prototype.shouldRefreshConversations = function(data) {
        ...
        // 檢查 type 字段
        if (data.type && refreshTypes.includes(data.type)) {
            return true;
        }

        // 檢查 action 字段
        if (data.action && refreshActions.includes(data.action)) {
            return true;
        }
        ...
        if (data.type === 'notification') {
            ...
            if (data.code && sessionCodes.some(code => data.code.includes(code))) {
                return true;
            }
        }
     ...
     * 獲取並渲染活躍會話列表
     */
    SessionUIRenderer.prototype.fetchAndRenderActiveConversations = function() {
        const self = this;
        ...
        fetch('/api/conversations')
            ...
            .then(function(data) {
                self.renderActiveConversations(data.conversations || []);
            })
            .catch(function(error) {
                logger.error('獲取活躍會話失敗:', error);
            });
    };
     ...
     * 渲染活躍會話列表
     */
    SessionUIRenderer.prototype.renderActiveConversations = function(conversations) {
        if (!this.activeConversationsList) {
            this.activeConversationsList = DOMUtils.safeQuerySelector('#activeConversationsList');
        }
        ...
        // 獲取當前的 conversation_id
        const currentConversationId = window.feedbackApp ?
            window.feedbackApp.conversationId : null;

        if (!conversations || conversations.length === 0) {
            const noActiveText = window.i18nManager ?
                window.i18nManager.t('conversations.noActive') : '暫無活躍會話';
            this.activeConversationsList.innerHTML =
                '<div class="no-conversations">' + noActiveText + '</div>';
            return;
        }

        const self = this;
        const html = conversations.map(function(conv) {
            const isCurrentClass = conv.conversation_id === currentConversationId ? 'current' : '';
            const statusClass = 'status-' + conv.status;

            // 格式化時間
            const createdAt = TimeUtils.formatElapsedTime(conv.created_at);

            // 截取項目路徑
            const projectPath = self.truncatePath(conv.project_directory || '', 40);

            // 截取摘要
            const summary = self.truncateText(conv.summary || '', 60);

            return '<div class="conversation-item ' + isCurrentClass + '" ' +
                   'data-conversation-id="' + conv.conversation_id + '" ' +
                   'onclick="window.feedbackApp && window.feedbackApp.switchConversation(\'' + conv.conversation_id + '\')">' +
                   '<div class="conversation-status-indicator ' + statusClass + '"></div>' +
                   '<div class="conversation-info">' +
                   '<div class="conversation-project" title="' + (conv.project_directory || '') + '">' + projectPath + '</div>' +
                   '<div class="conversation-summary" title="' + (conv.summary || '') + '">' + summary + '</div>' +
                   '<div class="conversation-time">' + createdAt + '</div>' +
                   '</div>' +
                   (isCurrentClass ? '<div class="current-badge">當前</div>' : '') +
                   '</div>';
        }).join('');

        this.activeConversationsList.innerHTML = html;

        // 更新頂部活動會話選擇器
        this.updateActiveSessionSelector(conversations);

        logger.info('渲染了', conversations.length, '個活躍會話');
    };

    /**
     * 更新頂部活動會話選擇器
     */
    SessionUIRenderer.prototype.updateActiveSessionSelector = function(conversations) {
        if (!conversations) return;

        const countElement = document.getElementById('activeSessionCount');
        const badgeElement = document.getElementById('activeSessionBadge');
        const listElement = document.getElementById('activeSessionList');

        if (!countElement || !badgeElement || !listElement) {
            logger.warn('找不到頂部活動會話選擇器的元素');
            return;
        }
        ...
        if (conversations.length === 0) {
            const noActiveText = window.i18nManager ?
                window.i18nManager.t('activeSessions.empty') : '暫無活動會話';
            listElement.innerHTML = '<div class="active-session-empty">' + noActiveText + '</div>';
            return;
        }
        ...
        const self = this;
        const html = conversations.map(function(conv) {
            const isCurrent = conv.conversation_id === currentConversationId;
            const statusClass = conv.status || 'waiting';

            // 格式化時間
            const createdAt = TimeUtils.formatElapsedTime(conv.created_at);

            // 截取項目路徑和摘要
            const projectPath = self.truncatePath(conv.project_directory || '', 35);
            const summary = self.truncateText(conv.summary || '', 45);

            return '<div class="active-session-item ' + (isCurrent ? 'current' : '') + '" ' +
                   'data-conversation-id="' + conv.conversation_id + '" ' +
                   'onclick="window.feedbackApp && window.feedbackApp.switchConversation(\'' + conv.conversation_id + '\')">' +
                   '<div class="active-session-status ' + statusClass + '"></div>' +
                   '<div class="active-session-info">' +
                   '<div class="active-session-project" title="' + (conv.project_directory || '') + '">' + projectPath + '</div>' +
                   '<div class="active-session-summary" title="' + (conv.summary || '') + '">' + summary + '</div>' +
                   '</div>' +
                   (isCurrent ? '<div class="active-session-current-badge">當前</div>' : '') +
                   '</div>';
        }).join('');
        ...
        // 綁定下拉菜單事件
        this.bindActiveSessionDropdownEvents();

        logger.debug('更新頂部活動會話選擇器，會話數量:', conversations.length);
    };

    /**
     * 綁定活動會話下拉菜單事件
     */
    SessionUIRenderer.prototype.bindActiveSessionDropdownEvents = function() {
        const dropdownBtn = document.getElementById('activeSessionDropdownBtn');
        const dropdownMenu = document.getElementById('activeSessionDropdownMenu');
     ...
     * 截斷路徑
     */
    SessionUIRenderer.prototype.truncatePath = function(path, maxLength) {
     ...
     * 截斷文本
     */
    SessionUIRenderer.prototype.truncateText = function(text, maxLength) {
     ...
     * 開始定時刷新活躍會話（可選）
     */
    SessionUIRenderer.prototype.startActiveConversationsPolling = function(intervalMs) {
        const self = this;
        intervalMs = intervalMs || 5000; // 預設 5 秒

        // 先執行一次
        this.fetchAndRenderActiveConversations();

        // 設置定時器
        this.activeConversationsPollingTimer = setInterval(function() {
            self.fetchAndRenderActiveConversations();
        }, intervalMs);

        logger.info('啟動活躍會話輪詢，間隔:', intervalMs + 'ms');
    };

    /**
     * 停止定時刷新活躍會話
     */
    SessionUIRenderer.prototype.stopActiveConversationsPolling = function() {
        if (this.activeConversationsPollingTimer) {
            clearInterval(this.activeConversationsPollingTimer);
            this.activeConversationsPollingTimer = null;
            logger.info('停止活躍會話輪詢');
        }
    };
    ...
    SessionUIRenderer.prototype.cleanup = function() {
        // 停止定時器
        this.stopActiveTimeTimer();
        this.stopActiveConversationsPolling();
        ...
        this.currentSessionCard = null;
        this.historyList = null;
        this.activeConversationsList = null;
        this.statsElements = {};
        this.currentSessionData = null;
        this.lastRenderedData = {
            stats: null,
