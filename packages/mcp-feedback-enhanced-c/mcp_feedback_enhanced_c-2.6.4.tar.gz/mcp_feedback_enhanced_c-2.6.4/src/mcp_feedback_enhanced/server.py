...
def _infer_mime_type(image_data: dict) -> str:
    mime_type = image_data.get("mimeType") or image_data.get("type")
    if isinstance(mime_type, str) and mime_type.strip():
        return mime_type.strip()

    file_name = image_data.get("name", "image.png").lower()
...
def _normalize_image_base64(image_data: dict) -> str | None:
    ...
    if isinstance(raw_data, bytes):
        if len(raw_data) == 0:
            debug_log(f"圖片 {image_name} bytes 為空")
            return None
    ...
    if isinstance(raw_data, str):
        ...
        if data_str.startswith("data:"):
            ...
            if len(parts) != 2 or not parts[1]:
                debug_log(f"圖片 {image_name} data URL 解析失敗")
                return None
            data_str = parts[1]
        try:
            base64.b64decode(data_str)
        except Exception:
            debug_log(f"圖片 {image_name} base64 解碼失敗")
            return None
        return data_str
...
def process_images(images_data: list[dict]) -> list[ImageContent]:
    ...
    Returns:
        List[ImageContent]: MCP 圖片對象列表
    """
    mcp_images: list[ImageContent] = []
    ...
    for i, img in enumerate(images_data, 1):
        try:
            ...
            image_base64 = _normalize_image_base64(img)
            if not image_base64:
                debug_log(f"圖片 {i} 數據格式無法解析: {type(img['data'])}")
                continue

            mime_type = _infer_mime_type(img)
            mcp_image = ImageContent(type="image", data=image_base64, mimeType=mime_type)
            mcp_images.append(mcp_image)

            file_name = img.get("name", "image.png")
            debug_log(f"圖片 {i} ({file_name}) 處理成功，格式: {mime_type}")
...
async def interactive_feedback(
    ...
    conversation_id: Annotated[
        str | None,
        Field(
            description="會話唯一標識符，用於區分不同的對話窗口。"
            "為同一個對話使用一致的 ID 以保持會話連續性。"
            "如果未提供，將自動生成新 ID。"
        ),
    ] = None,
) -> list:
    ...
    QUICK OPTIONS FEATURE:
    You can provide quick selection options in the summary using XML format. Users can click checkboxes to select options.

    Basic Format:
    <options>
    <option>A. First option description</option>
    <option>B. Second option description</option>
    <option>C. Third option description</option>
    </options>

    Format with Attributes (optional title, key, description):
    <options title="Choose next action">
    <option key="A">Continue optimization</option>
    <option key="B" description="Takes about 5 minutes">Update documentation</option>
    <option key="C">Add unit tests</option>
    </options>

    Multiple option groups are supported:
    <options title="Task Type">
    <option>1. Task type A</option>
    <option>2. Task type B</option>
    </options>

    <options title="Priority Level">
    <option key="high">High Priority</option>
    <option key="low">Low Priority</option>
    </options>
    ...
    Args:
        ...
        summary: Summary of AI work completed for user review (supports Markdown and <options> XML tags)
        timeout: Timeout in seconds for waiting user feedback (default: 600 seconds)
        conversation_id: Unique identifier for the conversation/chat window (optional, auto-generated if not provided)
    ...
    try:
        ...
        result = await launch_web_feedback_ui(
            project_directory, summary, timeout, conversation_id
        )
...
async def launch_web_feedback_ui(
    project_dir: str, summary: str, timeout: int, conversation_id: str | None = None
) -> dict:
    """
    啟動 Web UI 收集回饋，支援自訂超時時間和會話 ID
    ...
    if conversation_id:
        debug_log(f"會話 ID: {conversation_id}")

    try:
        # 使用新的 web 模組
        from .web import launch_web_feedback_ui as web_launch

        # 傳遞 timeout 和 conversation_id 參數給 Web UI
        return await web_launch(project_dir, summary, timeout, conversation_id)
