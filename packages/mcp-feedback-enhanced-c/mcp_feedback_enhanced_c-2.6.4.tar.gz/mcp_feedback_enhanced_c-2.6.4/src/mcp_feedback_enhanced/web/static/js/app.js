...
(function() {
    ...
    function FeedbackApp(sessionId) {
        ...
        if (this.conversationId) {
            console.log('🔗 當前 conversation_id:', this.conversationId);
        }
    }

    /**
     * 從 URL 解析 conversation_id
     */
    FeedbackApp.prototype.getConversationIdFromUrl = function() {
     ...
     * 切換到其他會話
     */
    FeedbackApp.prototype.switchConversation = function(conversationId) {
        if (conversationId === this.conversationId) {
            console.log('🔗 已經在當前會話中');
            return;
        }
        console.log('🔗 切換到會話:', conversationId);
        window.location.href = '/?c=' + encodeURIComponent(conversationId);
    };
                ...
                self.settingsManager.loadSettings()
                    .then(function(settings) {
                        ...
                        self.webSocketManager = new window.MCPFeedback.WebSocketManager({
                            ...
                            conversationId: self.conversationId,  // 傳入 conversation_id
                            onOpen: function() {
    ...
    FeedbackApp.prototype.saveImageSettings = function() {
        if (this.imageHandler && this.settingsManager) {
            this.settingsManager.setMultiple({
                ...
                imageCompressionQuality: this.imageHandler.imageCompressionQuality
            });
        }
    };
    ...
    FeedbackApp.prototype.collectFeedbackData = function() {
        ...
        textFeedback = combinedFeedbackInput ? combinedFeedbackInput.value.trim() : '';

        // 獲取快速選項的內容
        let quickOptionsFeedback = '';
        if (this.uiManager && this.uiManager.getQuickOptionsContent) {
            quickOptionsFeedback = this.uiManager.getQuickOptionsContent().trim();
        }

        // 合併快速選項和文本反饋
        let feedback = '';
        if (quickOptionsFeedback && textFeedback) {
            feedback = quickOptionsFeedback + '\n\n' + textFeedback;
        } else if (quickOptionsFeedback) {
            feedback = quickOptionsFeedback;
        } else {
        ...
        return {
            ...
            settings: {
                ...
                image_compression_quality: this.imageHandler ? this.imageHandler.imageCompressionQuality : 1
            }
        };
    };
