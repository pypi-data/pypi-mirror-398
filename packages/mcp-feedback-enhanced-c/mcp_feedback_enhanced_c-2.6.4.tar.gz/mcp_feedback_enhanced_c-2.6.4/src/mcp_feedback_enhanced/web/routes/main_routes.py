...
def setup_routes(manager: "WebUIManager"):
    ...
    async def index(request: Request, c: str | None = None):
        """統一回饋頁面 - 支持多會話模式

        Args:
            c: conversation_id（可選，指定顯示哪個會話）
        """
        # 根據 conversation_id 獲取會話
        if c:
            current_session = manager.get_session_by_conversation(c)
            debug_log(f"通過 conversation_id 獲取會話: {c}")
        else:
            # 向後兼容：獲取當前活躍會話
    ...
    @manager.app.get("/api/conversations")
    async def get_active_conversations(request: Request):
        """獲取所有活躍的會話（按 conversation 分組）"""
        try:
            conversations_data = []

            # 遍歷所有活躍的會話
            for conversation_id, session in manager.active_conversations.items():
                # 只返回等待或活躍狀態的會話
                if session.status.value in ["waiting", "active"]:
                    conversation_info = {
            ...
            debug_log(f"返回 {len(conversations_data)} 個活躍會話")
            return JSONResponse(content={"conversations": conversations_data})
            ...
            return JSONResponse(
                status_code=500,
                content={
                    "error": f"Failed to get conversations: {e!s}",
                    "messageCode": get_msg_code("get_conversations_failed"),
                },
            )

    @manager.app.get("/api/conversation/{conversation_id}")
    async def get_conversation_session(conversation_id: str, request: Request):
        """獲取指定 conversation 的會話詳情"""
        try:
            ...
            if not session:
                return JSONResponse(
                    status_code=404,
                    content={
                        "error": f"No session for conversation {conversation_id}",
                        "messageCode": get_msg_code("conversation_not_found"),
                    },
                )
            ...
            debug_log(f"獲取會話詳情失敗: {e}")
            return JSONResponse(
                status_code=500,
                content={
                    "error": f"Failed to get conversation: {e!s}",
                    "messageCode": get_msg_code("get_conversation_failed"),
                },
            )
    ...
    async def websocket_endpoint(
        websocket: WebSocket, c: str | None = None, lang: str = "zh-TW"
    ):
        """WebSocket 端點 - 支持多會話模式

        Args:
            c: conversation_id（可選，綁定到指定會話）
            lang: 語言代碼（由前端處理）
        """
        # 根據 conversation_id 獲取會話
        if c:
            session = manager.get_session_by_conversation(c)
            if not session:
                await websocket.close(
                    code=4004, reason=f"No session for conversation {c}"
                )
                return
            debug_log(f"WebSocket 綁定到 conversation: {c}")
        else:
            # 向後兼容：獲取當前活躍會話
        ...
        debug_log(
            f"WebSocket 連接建立: session_id={session.session_id}, conversation_id={session.conversation_id}"
        )
        ...
        try:
            while True:
                ...
                if session.websocket == websocket:
                    ...
                    debug_log("WebSocket 連接不匹配，忽略消息")
                    break
