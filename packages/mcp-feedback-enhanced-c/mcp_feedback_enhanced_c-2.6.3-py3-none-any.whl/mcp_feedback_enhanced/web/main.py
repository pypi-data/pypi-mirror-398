...
class WebUIManager:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int | None = None):
        ...
        # 新增：多會話支持的數據結構
        self.conversation_sessions: dict[str, str] = {}  # conversation_id -> session_id 映射
        self.active_conversations: dict[str, WebFeedbackSession] = {}  # 活躍的會話 (conversation_id -> session)
    ...
    def create_session(
        self, project_directory: str, summary: str, conversation_id: str | None = None
    ) -> tuple[str, str]:
        """創建新的回饋會話 - 支持多會話模式
        ...
        Args:
            ...
            summary: AI 工作摘要
            conversation_id: 會話 ID（可選，自動生成）

        Returns:
            tuple[str, str]: (session_id, conversation_id)
        """
        # 生成 conversation_id（如果未提供）
        if conversation_id is None:
            conversation_id = str(uuid.uuid4())
            debug_log(f"自動生成 conversation_id: {conversation_id}")

        # 檢查此會話是否已有活躍 session
        old_session = None
        if conversation_id in self.conversation_sessions:
            old_session_id = self.conversation_sessions[conversation_id]
            old_session = self.sessions.get(old_session_id)
            if old_session:
                debug_log(f"會話 {conversation_id} 替換舊 session {old_session_id}")

        # 保存舊會話的 WebSocket 連接
        old_websocket = None
        if old_session and old_session.websocket:
            old_websocket = old_session.websocket
            debug_log("保存舊會話的 WebSocket 連接以轉移")

        # 創建新會話
        session_id = str(uuid.uuid4())
        session = WebFeedbackSession(
            session_id, project_directory, summary, conversation_id=conversation_id
        )
        ...
        if old_session:
            ...
            if conversation_id in self.active_conversations:
                ...
                debug_log(f"從活躍會話中移除 conversation {conversation_id}")
        ...
        debug_log(f"創建新會話: session_id={session_id}, conversation_id={conversation_id}")
    ...
    def remove_session(self, session_id: str):
        ...
        if session_id in self.sessions:
            ...
            if hasattr(session, "conversation_id") and session.conversation_id:
                ...
                if session.conversation_id in self.active_conversations:
                    del self.active_conversations[session.conversation_id]
    ...
    def remove_conversation_session(self, conversation_id: str):
        """移除指定 conversation 的會話"""
        if conversation_id in self.conversation_sessions:
...
async def launch_web_feedback_ui(
    ...
    session_id, conversation_id = manager.create_session(
        project_directory, summary, conversation_id
    )
    ...
    # 使用帶有 conversation_id 的 URL
    base_url = manager.get_server_url()
    if conversation_id:
        feedback_url = f"{base_url}?c={conversation_id}"
    else:
    ...
    debug_log(f"[DEBUG] conversation_id: {conversation_id}")
