...
class EnumErrorCode(IntEnum):
    ...
    DMERR_LICENSE_CACHE_USED = -20012
...
class InstancePoolStatus:
    """ 
    Stores the Status of InstancePool. 
    
    Attributes:
    -----------
    - authorized_instances_count <int> : The number of authorized instances, which represents the maximum available instances.
    - remaining_instances_count <int> : The count of available instances that are currently not in use.
    - waiting_creation_instances <int> : The number of 'GetInstance' requests that are currently waiting because there are no available instances in the pool.
    - total_wait_occurrences <int> : The total count of times that the 'GetInstance' operation has encountered a wait state.
    """
    def __init__(self, Instance_Pool_Status):
        self.authorized_instances_count = Instance_Pool_Status["AuthorizedInstancesCount"]
        self.remaining_instances_count = Instance_Pool_Status["RemainingInstancesCount"]
        self.waiting_creation_instances = Instance_Pool_Status["WaitingCreationInstances"]
        self.total_wait_occurrences = Instance_Pool_Status["TotalWaitOccurrences"]
...
class BarcodeReader():
    ...
    def __init__(self,instancemod = False):
        ''' Init Function '''
        self.version = "dbr-python 9.6.40"
...
#region License Functions
    ...
    def get_instance():
        ''' Creates an instance of Dynamsoft Barcode Reader.
            :return instance  <class BarcodeReader> : Returns an instance of Dynamsoft Barcode Reader. If failed, returns NULL.
        '''
    ...
    @staticmethod
    def set_max_concurrent_instance_count(count_for_this_device,count_for_this_process = 0,timeout = 0):
        ''' Sets the max concurrent instance count used for current device and process.
            ...
            :param timeout <int> : The maximum time (in milliseconds) to wait for an available authorization or instance when calling InitLicense, GetInstance, or Decode functions.
        '''
        BarcodeReader.__dbr__.SetMaxConcurrentInstanceCount(count_for_this_device,count_for_this_process,timeout)
    
    @staticmethod
    def set_license_cache_path(directory_path):
        return BarcodeReader.__dbr__.SetLicenseCachePath(directory_path)
    
    @staticmethod
    def get_device_uuid(uuidGenerationMethod):
        msg = BarcodeReader.__dbr__.GetDeviceUUID(uuidGenerationMethod)
        if msg[0] != 0:
            raise BarcodeReaderError(msg[1])
        else:
            return msg[1]
        
    @staticmethod
    def get_instance_pool_status():
        ''' Gets a struct to represent the status of an instance pool.
            :return  <*class InstancePoolStatus*> 
        '''
        result = BarcodeReader.__dbr__.GetInstancePoolStatus()
        final = InstancePoolStatus(result)
        return final
...
#region Image Decoding Functions
    ...
    def decode_file(self, image_file_name, template_name=""):
        ...
        if error_code == EnumErrorCode.DBR_OK or error_code == EnumErrorCode.DBRERR_LICENSE_EXPIRED or error_code == EnumErrorCode.DBRERR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_RECOGNITION_TIMEOUT or \
            ...
            error_code == EnumErrorCode.DMERR_FAILED_TO_REACH_DLS or error_code == EnumErrorCode.DBRERR_PHARMACODE_LICENSE_INVALID or error_code == EnumErrorCode.DMERR_LICENSE_CACHE_USED:
