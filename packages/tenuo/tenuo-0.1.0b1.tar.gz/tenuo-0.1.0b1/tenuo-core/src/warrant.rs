        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for WarrantVisitor {
                ...
                A: serde::de::SeqAccess<'de>,
            {
                ...
                // The signature is valid, so the payload integrity is guaranteed.
                // Structural validation (e.g. depth limits) is handled by the
                // deserializer recursion limits and subsequent usage checks.
...
impl WarrantBuilder {
    ...
    pub fn build(mut self, signing_key: &SigningKey) -> Result<Warrant> {
        ...
        // Canonicalize lists for deterministic serialization
        if let Some(tools) = &mut self.issuable_tools {
            tools.sort();
        }
        if let Some(approvers) = &mut self.required_approvers {
            // PublicKey doesn't implement Ord, so sort by bytes
...
impl<'a> IssuanceBuilder<'a> {
    ...
    pub fn build(mut self, signing_key: &SigningKey) -> Result<Warrant> {
        // Canonicalize lists for deterministic serialization
        // Note: IssuanceBuilder only creates execution warrants, so no issuable_tools
        if let Some(approvers) = &mut self.required_approvers {
            approvers.sort_by_key(|a| a.to_bytes());
        }
...
mod tests {
    ...
    fn test_max_depth_protocol_cap() {
        ...
        let result = Warrant::builder()
            .tool("test", ConstraintSet::new())
            .ttl(Duration::from_secs(60))
            .max_depth(100) // Above MAX_DELEGATION_DEPTH
            .holder(keypair.public_key())
            .build(&keypair);
