    ...
    def pc_host_ip(self) -> str:
        ...
        try:
            ...
            if e.response is not None:
                content = e.response.text.strip()
                if content:
                    error_details += f", response: {content}"
            raise PCHostIPException(f"Failed to get PC host IP: {error_details}")
    ...
    def wait_for_service_ready(self) -> None:
        ...
        while time.time() - start_time < self.timeout:
            ...
            try:
                ...
                last_error = e
                logger.debug(
                    f"Fastpusher API not ready, retry after 1 second (elapsed: {int(time.time() - start_time)}s), error: {e}"
                )
                time.sleep(1)

        raise DeviceException(f"Fastpusher service not ready after {self.timeout}s") from last_error
