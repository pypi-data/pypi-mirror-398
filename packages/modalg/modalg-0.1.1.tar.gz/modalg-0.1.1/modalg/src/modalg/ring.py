...
class Ring(AbelianGroup):
    ...
    def getRing(self):
        return self.group
    def __str__(self):
        return 'Ring: {}'.format(modalg.myfunctions.flatten2(self))
    ...
    def multiply(self, obj1, obj2):
        ...
        if (obj1 in self.group or type(obj1) == MultiplicativeIdentity or type(obj1) == AdditiveIdentity or type(obj1) == AdditiveInverse) and (obj2 in self.group or type(obj2) == MultiplicativeIdentity or type(obj2) == AdditiveIdentity or type(obj2) == AdditiveInverse):
            ...
            if ((type(obj1) == list) or (type(obj1) == set)) and booler == False:
                if modalg.myfunctions.is_nested_list(obj1) == True:
                    obj1 = modalg.myfunctions.flatten(modalg.myfunctions.insert(obj1))
                    list1 = []
                    for i in obj1:
                        if i != '@%$7h^%':
                            list1.append(i)
                    ...
                    for elt in self.group:
                        if type(elt) == list:
                            if modalg.myfunctions.is_nested_list(elt) == True:
                                set1.add(tuple(elt))
                    self.group.append(set1)
                    self.group.append(AdditiveInverse(set1))
                    booler = True
            if ((type(obj2) == list) or (type(obj2) == set)) and booler == False:
                if modalg.myfunctions.is_nested_list(obj2) == True:
                    obj2 = modalg.myfunctions.flatten(modalg.myfunctions.insert(obj2))
                    list2 = []
                    for i in obj2:
                        if i != '@%$7h^%':
                            list2.append(i)
