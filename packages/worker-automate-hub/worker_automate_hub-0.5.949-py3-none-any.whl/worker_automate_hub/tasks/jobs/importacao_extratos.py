...
async def importacao_extratos(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    try:
        ...
        for entry in os.scandir(PASTA):
            ...
            # 2️⃣ Considera apenas arquivos com extensões permitidas
        ...
        for idx, sel in enumerate(selecionados, 1):
            ...
            try:
                ...
                try:
                    ...
                    # 2) Fallback via UNC (compartilhadas$\Nexera\Extrato\ano)
                    try:
                        ...
                        try:
                            ...
                            # Se der 1219 (já conectado), ignora
        ...
        if importados:
            ...
            retorno_payload = {
                ...
                "importados_count": 0,
                "falhas": falhas,
                "mensagem": f"Nenhum arquivo foi importado/movido com sucesso (extensões permitidas: {EXT_STR}).",
            }
            retorno_str = json.dumps(retorno_payload, ensure_ascii=False, indent=2)
            return RpaRetornoProcessoDTO(
                sucesso=False,
