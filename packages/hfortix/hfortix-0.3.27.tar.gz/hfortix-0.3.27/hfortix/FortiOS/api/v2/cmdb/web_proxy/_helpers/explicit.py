...
def validate_explicit_put(
    ...
    if "secure-web-proxy" in payload:
        ...
        if value and value not in VALID_BODY_SECURE_WEB_PROXY:
            return (
                False,
                f"Invalid secure-web-proxy '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_SECURE_WEB_PROXY)}",
            )
    ...
    if "ftp-over-http" in payload:
        ...
        if value and value not in VALID_BODY_FTP_OVER_HTTP:
            return (
                False,
                f"Invalid ftp-over-http '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_FTP_OVER_HTTP)}",
            )
    ...
    if "socks" in payload:
        ...
        if value and value not in VALID_BODY_SOCKS:
            return (
                False,
                f"Invalid socks '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_SOCKS)}",
            )
    ...
    if "http-connection-mode" in payload:
        ...
        if value and value not in VALID_BODY_HTTP_CONNECTION_MODE:
            return (
                False,
                f"Invalid http-connection-mode '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_HTTP_CONNECTION_MODE)}",
            )
    ...
    if "ipv6-status" in payload:
        ...
        if value and value not in VALID_BODY_IPV6_STATUS:
            return (
                False,
                f"Invalid ipv6-status '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_IPV6_STATUS)}",
            )
    ...
    if "strict-guest" in payload:
        ...
        if value and value not in VALID_BODY_STRICT_GUEST:
            return (
                False,
                f"Invalid strict-guest '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_STRICT_GUEST)}",
            )
    ...
    if "pref-dns-result" in payload:
        ...
        if value and value not in VALID_BODY_PREF_DNS_RESULT:
            return (
                False,
                f"Invalid pref-dns-result '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_PREF_DNS_RESULT)}",
            )
    ...
    if "unknown-http-version" in payload:
        ...
        if value and value not in VALID_BODY_UNKNOWN_HTTP_VERSION:
            return (
                False,
                f"Invalid unknown-http-version '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_UNKNOWN_HTTP_VERSION)}",
            )
    ...
    if "https-replacement-message" in payload:
        ...
        if value and value not in VALID_BODY_HTTPS_REPLACEMENT_MESSAGE:
            return (
                False,
                f"Invalid https-replacement-message '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_HTTPS_REPLACEMENT_MESSAGE)}",
            )
    ...
    if "message-upon-server-error" in payload:
        ...
        if value and value not in VALID_BODY_MESSAGE_UPON_SERVER_ERROR:
            return (
                False,
                f"Invalid message-upon-server-error '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_MESSAGE_UPON_SERVER_ERROR)}",
            )
    ...
    if "pac-file-server-status" in payload:
        ...
        if value and value not in VALID_BODY_PAC_FILE_SERVER_STATUS:
            return (
                False,
                f"Invalid pac-file-server-status '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_PAC_FILE_SERVER_STATUS)}",
            )
    ...
    if "pac-file-through-https" in payload:
        ...
        if value and value not in VALID_BODY_PAC_FILE_THROUGH_HTTPS:
            return (
                False,
                f"Invalid pac-file-through-https '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_PAC_FILE_THROUGH_HTTPS)}",
            )
    ...
    if "trace-auth-no-rsp" in payload:
        ...
        if value and value not in VALID_BODY_TRACE_AUTH_NO_RSP:
            return (
                False,
                f"Invalid trace-auth-no-rsp '{value}'. Must be one of: {
                    ', '.join(VALID_BODY_TRACE_AUTH_NO_RSP)}",
            )
