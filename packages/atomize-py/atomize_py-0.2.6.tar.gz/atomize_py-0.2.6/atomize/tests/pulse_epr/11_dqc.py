...
# 10.1007/0-306-47109-4_9         -> phase cycling
...
PULSE_5_LENGTH = '16 ns'
PULSE_6_LENGTH = '32 ns'
PULSE_1_START = '0 ns'
PULSE_2_START = '120 ns'
PULSE_3_START = '240 ns'
# this delta is 35 ns in 10.1039/C5CP03671B
# we can use only pi-pulse length + 40 ns
PULSE_4_START = '312 ns'
PULSE_5_START = '384 ns'
PULSE_6_START = '6384 ns'
PULSE_SIGNAL_START = '12384 ns'
...
# probably 3D; but in the 10.1039/C5CP03671B the step of t1 is equal to the step of t2; t3 was fixed
...
pb.pulser_pulse(name = 'P3', channel = 'MW', start = PULSE_4_START, length = PULSE_4_LENGTH, delta_start = str(3 * STEP) + ' ns', \
                phase_list = ['+x', '+x', '+x', '+x', '+y', '+y', '+y', '+y', '-x', '-x', '-x', '-x', '-y', '-y', '-y', '-y', \
                              ...
                              '+x', '+x', '+x', '+x', '+y', '+y', '+y', '+y', '-x', '-x', '-x', '-x', '-y', '-y', '-y', '-y', \
                              '+x', '+x', '+x', '+x', '+y', '+y', '+y', '+y', '-x', '-x', '-x', '-x', '-y', '-y', '-y', '-y'])
pb.pulser_pulse(name = 'P4', channel = 'MW', start = PULSE_5_START, length = PULSE_5_LENGTH, delta_start = str(4 * STEP) + ' ns', \
                phase_list = ['+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', \
                              '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', '-x', \
                              '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y', '+y',\
                              '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y', '-y'])
pb.pulser_pulse(name = 'P5', channel = 'MW', start = PULSE_6_START, length = PULSE_6_LENGTH, delta_start = str(5 * STEP) + ' ns', \
                phase_list = ['+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', \
                              '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', \
                              '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x',\
                              '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x', '+x'])
pb.pulser_pulse(name = 'P6', channel = 'TRIGGER', start = PULSE_SIGNAL_START, length = '100 ns', delta_start = str(6 * STEP) + ' ns')
...
for j in general.scans(SCANS):
    ...
    for i in range(POINTS):
        ...
        process = general.plot_1d(EXP_NAME, x_axis, ( data_x, data_y ), xname = 'Delay',\
            xscale = 'ns', yname = 'Area', yscale = 'V*s', timeaxis = 'False', label = CURVE_NAME, pr = process, \
