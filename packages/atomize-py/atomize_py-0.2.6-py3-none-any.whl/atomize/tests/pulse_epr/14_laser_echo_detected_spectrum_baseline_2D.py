...
pb.pulser_pulse(name ='P1', channel = 'MW', start = PULSE_2_START, length = PULSE_1_LENGTH, phase_list = ['+x', '-x'], delta_start = str(int(TIME_STEP)) + ' ns')
pb.pulser_pulse(name ='P2', channel = 'MW', start = PULSE_3_START, length = PULSE_2_LENGTH, phase_list = ['+x', '+x'], delta_start = str(int(TIME_STEP)) + ' ns')
pb.pulser_pulse(name ='P3', channel = 'TRIGGER', start = PULSE_SIGNAL_START, length = '100 ns', delta_start = str(int(TIME_STEP)) + ' ns')

pb.pulser_repetition_rate( REP_RATE )
pb.pulser_update()

# Data saving
header = 'Date: ' + str(datetime.datetime.now().strftime("%d-%m-%Y %H-%M-%S")) + '\n' + 'Laser ED Spectrum 2D; Phase Cycling\n' + \
