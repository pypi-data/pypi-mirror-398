...
* ==== global settings ====
$if not set fixedcapsfromgdx    $setglobal fixedcapsfromgdx     None

$iftheni.fcfgdx not %fixedcapsfromgdx%==None
$setglobal capsfromgdx          %fixedcapsfromgdx%
$endif.fcfgdx
...
converter_units(%selscen%accNodesModel,accYears,converter_techs,vintage,capType)
    = ceil(converter_units(%selscen%accNodesModel,accYears,converter_techs,vintage,capType) * 1e6) / 1e6;
storage_units(%selscen%accNodesModel,accYears,storage_techs,vintage,capType)
    = ceil(storage_units(%selscen%accNodesModel,accYears,storage_techs,vintage,capType) * 1e6) / 1e6;
transfer_links(%selscen%linksModel,years,transfer_techs,vintage,capType)
    = ceil(transfer_links(%selscen%linksModel,years,transfer_techs,vintage,capType) * 1e6) / 1e6;
$endif.cfgdx