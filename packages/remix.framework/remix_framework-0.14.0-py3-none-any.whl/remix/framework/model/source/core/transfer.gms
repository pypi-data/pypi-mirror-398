...
set pc_transfer_techParam
    ...
    flowUpperLimit      "Flow upper limit | Upper limit for the flow factor per time step | minimum:0;default:1 | number | {none} | OEO_00000104:constraint"
    /;
...
set pc_transfer_coefficient
    ...
    delay               "Commodity delay | Delay in hours between the inflow and outflow for the link. | | number | {none} | MISSING_TERM:transmission delay"
    /;
...
* // Title: Transfer Coefficients per Length
...
set pc_transfer_coefPerLength
    ...
    delayPerLength    "Commodity delay per length | Delay in hours per length between the inflow and outflow for the link. | | number | {none} | MISSING_TERM:transmission delay"
    /;
...
* // Title: Transfer Reactance per Length
...
* Technologies to optimize become unavailable if they have a linksUpperLimit of 0
...
* Technologies already built become unavailable if they have a linksBuild of 0
transfer_availTech(linksModelToCalc,years,transfer_techs,vintage)
...
transfer_decomTech(linksModel,years,transfer_techs,vintage)
  $(sum(years_a$(transfer_usedTech(linksModel,years_a,transfer_techs,vintage)
...
transfer_decomTech(linksModel,years,transfer_techs,vintage)
  $(sum(years_a$(transfer_usedTech(linksModel,years_a,transfer_techs,vintage)), 1)
    and sum(yearsToCalc
      $(sameas(years, yearsToCalc)
        and (vintage.val + transfer_techParam(transfer_techs,vintage,"lifeTime")) < smin(yearsToCalc_a, yearsToCalc_a.val)), 1))
  = yes;
...
* aggregate coefficient delays per link and reduce based on time resolution 
...
transfer_linkBoundsFixed(linksModel,years,transfer_techs)
  $(not sum(vintage$transfer_usedTech(linksModel,years,transfer_techs,vintage),
        transfer_techParam(transfer_techs,vintage,"freeDecom"))
...
parameter transfer_linksDelta_decom(linksModel,years,transfer_techs,vintage);
...
equations
  ...
  Eq_transfer_linksBalanceFreeDecom(linksModel,years,transfer_techs,vintage
    ) "Ensures the links balance over the planning period."
...
Eq_transfer_linksBalance(linksModelToCalc,yearsSel,transfer_techs,vintage)
    ...
    sum ((years_a)
            $(transfer_availTech(linksModelToCalc,years_a,transfer_techs,vintage)
                ...
                and years_a.val + transfer_techParam(transfer_techs,vintage,'lifeTime') > yearsSel.val
    ...
    + sum ((yearsToCalc)
            $(transfer_availTech(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
                ...
                and yearsToCalc.val + transfer_techParam(transfer_techs,vintage,'lifeTime') > yearsSel.val
...
Eq_transfer_linksBalanceFreeDecom(linksModelToCalc,yearsSel,transfer_techs,vintage)
    $((transfer_usedTech(linksModelToCalc,yearsSel,transfer_techs,vintage)
          or sum(years$sameas(years,yearsSel), transfer_usedTech(linksModelToCalc,years-1,transfer_techs,vintage)))
        and not transfer_linkBoundsFixed(linksModelToCalc,yearsSel,transfer_techs)
        and transfer_techParam(transfer_techs,vintage,"freeDecom") = 1)
...
/
  Eq_transfer_linksBalance
  Eq_transfer_linksBalanceFreeDecom
  Eq_transfer_linksFreeDecom
  Eq_transfer_linksLowerLimit
  Eq_transfer_linksUpperLimit
  Eq_transfer_linksTotalMIP
