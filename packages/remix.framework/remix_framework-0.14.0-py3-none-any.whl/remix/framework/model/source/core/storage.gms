...
* // ### storage_unitsDecom
* // Title: Storage units decommissioned
...
* // Title: Storage units decommissioning tracker
...
set pc_storage_reservoirParam
    ...
    initialLevel        "Initial storage level | Storage level before first timestep for nonCircular storage |  minimum:0;maximum:1 | number | {none} | MISSING_TERM:State of charge"
    /;
...
storage_decomTech(nodesModel,years,storage_techs,vintage)
  $(sum(years_a$(storage_usedTech(nodesModel,years_a,storage_techs,vintage)
...
storage_decomTech(nodesModel,years,storage_techs,vintage)
  $(sum(years_a$(storage_usedTech(nodesModel,years_a,storage_techs,vintage)), 1)
    and sum(yearsToCalc
      $(sameas(years, yearsToCalc)
        and (vintage.val + storage_techParam(storage_techs,vintage,"lifeTime")) < smin(yearsToCalc_a, yearsToCalc_a.val)), 1))
  = yes;
...
* Precompute maps for eRate and cRate for SoC technologies
set storage_useeRateSoC(storage_techs,vintage);
storage_useeRateSoC(storage_techs,vintage)
  $(storage_validSoCRange(storage_techs,vintage)
    and sum(soc_states, storage_SoCParam(storage_techs,vintage,soc_states,"eRate")) < inf)
  = yes;

set storage_usecRateSoC(storage_techs,vintage);
storage_usecRateSoC(storage_techs,vintage)
  $(storage_validSoCRange(storage_techs,vintage)
...
storage_level_hasProfile(nodesModelToCalc,yearsToCalc,storage_techs,pc_storage_levelProfile)
    = sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc),
            storage_level_hasProfileIn(nodesDataToCalc,yearsToCalc,storage_techs,pc_storage_levelProfile));
...
storage_finiteUnitLimit(nodesModelToCalc,yearsToCalc,storage_techs) = sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc), storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit")) > 0
                                                 and sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc), storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit")) < inf;
...
storage_levelProfile(timeModelToCalc,storage_usedTech(nodesModelToCalc,yearsToCalc,storage_techs,vintage),pc_storage_levelProfile)
    ...
    = sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc),
            sum(timeData$timeMapping(timeData,timeModelToCalc),
                  storage_levelProfileIn(timeData,nodesDataToCalc,yearsToCalc,storage_techs,pc_storage_levelProfile))
              / timeLength(timeModelToCalc)
            * storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit"))
    / sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc),
            storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit"));

storage_levelProfile(timeModelToCalc,storage_usedTech(nodesModelToCalc,yearsToCalc,storage_techs,vintage),pc_storage_levelProfile)
    $( storage_level_hasProfile(nodesModelToCalc,yearsToCalc,storage_techs,pc_storage_levelProfile)
        and sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc),
                    storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit")) = inf )
    = sum(nodesDataToCalc$(aggregateNodesModel(nodesDataToCalc,nodesModelToCalc)
                    and storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit") = inf ),
            sum(timeData$timeMapping(timeData,timeModelToCalc),
                  storage_levelProfileIn(timeData,nodesDataToCalc,yearsToCalc,storage_techs,pc_storage_levelProfile))
              / timeLength(timeModelToCalc))
    / sum(nodesDataToCalc$(aggregateNodesModel(nodesDataToCalc,nodesModelToCalc)
                    and storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit") = inf ), 1);
...
storage_reservoirParam(nodesModelToCalc,yearsToCalc,storage_techs,"initialLevel")
    $( storage_reservoirParam(nodesModelToCalc,yearsToCalc,storage_techs,"nonCircular")
        and storage_finiteUnitLimit(nodesModelToCalc,yearsToCalc,storage_techs))
    = sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc),
          storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"initialLevel")
            * storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit"))
    / sum(nodesDataToCalc$aggregateNodesModel(nodesDataToCalc,nodesModelToCalc),
            storage_reservoirParamIn(nodesDataToCalc,yearsToCalc,storage_techs,"unitsUpperLimit"));
...
$endif.fcfgdx
  = sum((accNodesModel,accYears)
         $(sameas(accNodesModel,nodesModel) and sameas(accYears,years)),
        storage_units(accNodesModel,accYears,storage_techs,vintage,"decom"));
$endif.cfgdx
...
storage_unitBoundsFixed(nodesModel,years,storage_techs)
  $(not sum(vintage$storage_usedTech(nodesModel,years,storage_techs,vintage),
        storage_techParam(storage_techs,vintage,"freeDecom"))
...
parameter storage_unitsDelta_decom(nodesModel,years,storage_techs,vintage);
...
equations
  ...
  Eq_storage_unitsBalanceFreeDecom(nodesModel,years,storage_techs,vintage
    ) "Ensures the units balance over the planning period."
...
Eq_storage_unitsBalance(nodesModelSel,yearsSel,storage_techs,vintage)
    ...
    sum ((years_a)
                    $(storage_availTech(nodesModelSel,years_a,storage_techs,vintage)
                        ...
                        and years_a.val + storage_techParam(storage_techs,vintage,'LifeTime') > yearsSel.val
    ...
    + sum ((yearsToCalc)
                    $(storage_availTech(nodesModelSel,yearsToCalc,storage_techs,vintage)
                        ...
                        and yearsToCalc.val + storage_techParam(storage_techs,vintage,'LifeTime') > yearsSel.val
...
Eq_storage_unitsBalanceFreeDecom(nodesModelSel,yearsSel,storage_techs,vintage)
    $((storage_usedTech(nodesModelSel,yearsSel,storage_techs,vintage)
          or sum(years$sameas(years,yearsSel), storage_usedTech(nodesModelSel,years-1,storage_techs,vintage)))
        and not storage_unitBoundsFixed(nodesModelSel,yearsSel,storage_techs)
        and storage_techParam(storage_techs,vintage,"freeDecom") = 1)
    ...
    + storage_unitsDelta_decom(nodesModelSel,yearsSel,storage_techs,vintage)
        ;

* // ### Storage Units Free Decommission
...
Eq_storage_cRateLimit_SoC(timeModelSel(timeModelToCalc),nodesModelSel,yearsSel,storage_techs,vintage,commodity)
    $(storage_usedTechCom(nodesModelSel,yearsSel,storage_techs,vintage,commodity)
      and storage_usecRateSoC(storage_techs,vintage))
    ..
    storage_level(timeModelSel,nodesModelSel,yearsSel,storage_techs,vintage,commodity)
    - storage_level(timeModelToCalc--1,nodesModelSel,yearsSel,storage_techs,vintage,commodity)
        $(not storage_reservoirParam(nodesModelSel,yearsSel,storage_techs,"nonCircular"))
...
Eq_storage_eRateLimit_SoC(timeModelSel(timeModelToCalc),nodesModelSel,yearsSel,storage_techs,vintage,commodity)
    $(storage_usedTechCom(nodesModelSel,yearsSel,storage_techs,vintage,commodity)
      and storage_useeRateSoC(storage_techs,vintage))
    ..
    storage_level(timeModelSel,nodesModelSel,yearsSel,storage_techs,vintage,commodity)
    - storage_level(timeModelToCalc--1,nodesModelSel,yearsSel,storage_techs,vintage,commodity)
        $(not storage_reservoirParam(nodesModelSel,yearsSel,storage_techs,"nonCircular"))
...
/
  Eq_storage_unitsBalance
  Eq_storage_unitsBalanceFreeDecom
  Eq_storage_unitsFreeDecom
  Eq_storage_unitsLowerLimit
  Eq_storage_unitsUpperLimit
  Eq_storage_levelUpperLimit_degradation
