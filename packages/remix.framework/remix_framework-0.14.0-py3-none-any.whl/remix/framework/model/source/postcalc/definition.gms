...
* == converters ==
    sum ((converter_techs(techs),vintage)
                $(converter_availTech(nodesModelToCalc,yearsToCalc,converter_techs,vintage)
                    and accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"useAnnuity") = 0),
        converter_unitsBuild.l(nodesModelToCalc,yearsToCalc,converter_techs,vintage)
        * accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perUnitBuild"))

    + sum ((years_a,converter_techs(techs),vintage)
                $(converter_availTech(nodesModelToCalc,years_a,converter_techs,vintage)
                    and accounting_converterUnits(indicator,nodesModelToCalc,years_a,converter_techs,vintage,"useAnnuity") = 1
                    and years_a.val + accounting_converterUnits(indicator,nodesModelToCalc,years_a,converter_techs,vintage,"amorTime") > yearsToCalc.val
                    and years_a.val <= yearsToCalc.val ),
        converter_unitsBuild.l(nodesModelToCalc,years_a,converter_techs,vintage)
        * accounting_converterUnits(indicator,nodesModelToCalc,years_a,converter_techs,vintage,"perUnitBuild")
        * accounting_annuityFactor_converter(indicator,nodesModelToCalc,years_a,converter_techs,vintage) )

    + sum ((converter_techs(techs),vintage)
                $(converter_decomTech(nodesModelToCalc,yearsToCalc,converter_techs,vintage)
        ...
        * accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perUnitDecom") )
    ...
    + sum ((converter_techs(techs),vintage)
        ...
        * accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perUnitTotal") )
    ...
    + sum ((timeModelToCalc,converter_techs(techs),vintage,activity)
        ...
        * accounting_converterActivity(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,activity,"perActivity") )
        / timefrac
    ...
    + sum ((timeModelToCalc,converter_techs(techs),vintage)
        ...
        * accounting_converterStartup(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perStartup") )
        / timefrac

    + sum ((timeModelToCalc,converter_techs(techs),vintage)
                $converter_usedTech(nodesModelToCalc,yearsToCalc,converter_techs,vintage),
        converter_rampPos.l(timeModelToCalc,nodesModelToCalc,yearsToCalc,converter_techs,vintage)
        * (accounting_converterStartup(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perRamp")
            + accounting_converterStartup(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perRampPos"))

        + converter_rampNeg.l(timeModelToCalc,nodesModelToCalc,yearsToCalc,converter_techs,vintage)
        * (accounting_converterStartup(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perRamp")
            + accounting_converterStartup(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perRampNeg")))
        / timefrac

* == storage ==
    + sum ((storage_techs(techs),vintage)
                $(storage_availTech(nodesModelToCalc,yearsToCalc,storage_techs,vintage)
                    and accounting_storageUnits(indicator,nodesModelToCalc,yearsToCalc,storage_techs,vintage,"useAnnuity") = 0),
        storage_unitsBuild.l(nodesModelToCalc,yearsToCalc,storage_techs,vintage)
        * accounting_storageUnits(indicator,nodesModelToCalc,yearsToCalc,storage_techs,vintage,"perUnitBuild") )

    + sum ((years_a,storage_techs(techs),vintage)
                $(storage_availTech(nodesModelToCalc,years_a,storage_techs,vintage)
                    and accounting_storageUnits(indicator,nodesModelToCalc,years_a,storage_techs,vintage,"useAnnuity") = 1
                    and years_a.val + accounting_storageUnits(indicator,nodesModelToCalc,years_a,storage_techs,vintage,"amorTime") > yearsToCalc.val
                    and years_a.val <= yearsToCalc.val ),
        storage_unitsBuild.l(nodesModelToCalc,years_a,storage_techs,vintage)
        * accounting_storageUnits(indicator,nodesModelToCalc,years_a,storage_techs,vintage,"perUnitBuild")
        * accounting_annuityFactor_storage(indicator,nodesModelToCalc,years_a,storage_techs,vintage) )

    + sum ((storage_techs(techs),vintage)
                $(storage_decomTech(nodesModelToCalc,yearsToCalc,storage_techs,vintage)
        ...
        * accounting_storageUnits(indicator,nodesModelToCalc,yearsToCalc,storage_techs,vintage,"perUnitDecom"))
    ...
    + sum ((storage_techs(techs),vintage)
        ...
        * accounting_storageUnits(indicator,nodesModelToCalc,yearsToCalc,storage_techs,vintage,"perUnitTotal") )

* == transfer ==
    + sum ((linksModelToCalc(linksModel),transfer_techs(techs),vintage)
                $(transfer_availTech(linksModel,yearsToCalc,transfer_techs,vintage)
                    ...
                    and accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"useAnnuity") = 0),
        0.5
        * transfer_linksBuild.l(linksModel,yearsToCalc,transfer_techs,vintage)
        * accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perLinkBuild") )

    + sum ((linksModelToCalc(linksModel),years_a,transfer_techs(techs),vintage)
                $(transfer_availTech(linksModel,years_a,transfer_techs,vintage)
                    and transfer_incidenceModel(nodesModelToCalc,linksModel) <> 0
                    and accounting_transferLinks(indicator,linksModel,years_a,transfer_techs,vintage,"useAnnuity") = 1
                    and years_a.val + accounting_transferLinks(indicator,linksModel,years_a,transfer_techs,vintage,"amorTime") > yearsToCalc.val
                    and years_a.val <= yearsToCalc.val ),
        0.5
        * transfer_linksBuild.l(linksModel,years_a,transfer_techs,vintage)
        * accounting_transferLinks(indicator,linksModel,years_a,transfer_techs,vintage,"perLinkBuild")
        * accounting_annuityFactor_transferLink(indicator,linksModel,years_a,transfer_techs,vintage) )

    + sum ((linksModelToCalc(linksModel),transfer_techs(techs),vintage,link_types)
                $(transfer_availTech(linksModel,yearsToCalc,transfer_techs,vintage)
                    and transfer_incidenceModel(nodesModelToCalc,linksModel) <> 0
                    and accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"useAnnuity") = 0 ),
        0.5
        * transfer_linksBuild.l(linksModel,yearsToCalc,transfer_techs,vintage)
        * transfer_lengthParam(linksModel,link_types,"length")
        * accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perLengthBuild") )

    + sum ((linksModelToCalc(linksModel),years_a,transfer_techs(techs),vintage,link_types)
                $(transfer_availTech(linksModel,years_a,transfer_techs,vintage)
                    and transfer_incidenceModel(nodesModelToCalc,linksModel) <> 0
                    and accounting_transferPerLength(indicator,linksModel,years_a,transfer_techs,vintage,link_types,"useAnnuity") = 1
                    and years_a.val + accounting_transferPerLength(indicator,linksModel,years_a,transfer_techs,vintage,link_types,"amorTime") > yearsToCalc.val
                    and years_a.val <= yearsToCalc.val ),
        0.5
        * transfer_linksBuild.l(linksModel,years_a,transfer_techs,vintage)
        * transfer_lengthParam(linksModel,link_types,"length")
        * accounting_transferPerLength(indicator,linksModel,years_a,transfer_techs,vintage,link_types,"perLengthBuild")
        * accounting_annuityFactor_transferPerLength(indicator,linksModel,years_a,transfer_techs,vintage,link_types) )
    ...
    + sum ((linksModelToCalc(linksModel),transfer_techs(techs),vintage)
        ...
        * accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perLinkDecom")

        + 0.5
        * sum (link_types,
            transfer_linksDecom.l(linksModel,yearsToCalc,transfer_techs,vintage)
            * transfer_lengthParam(linksModel,link_types,"length")
            * accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perLengthDecom")
        ) )
    ...
    + sum ((linksModelToCalc(linksModel),transfer_techs(techs),vintage)
        ...
        * accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perLinkTotal")

        + 0.5
        * sum (link_types,
            + transfer_linksTotal.l(linksModel,yearsToCalc,transfer_techs,vintage)
            * transfer_lengthParam(linksModel,link_types,"length")
            * accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perLengthTotal"))

        + 0.5
        * sum (timeModelToCalc,
            transfer_flowAlong.l(timeModelToCalc,linksModel,yearsToCalc,transfer_techs,vintage)
            * timeLength(timeModelToCalc)
            * ( accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perFlow")
                + accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perFlowAlong"))

            + transfer_flowAgainst.l(timeModelToCalc,linksModel,yearsToCalc,transfer_techs,vintage)
            * timeLength(timeModelToCalc)
            * ( accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perFlow")
                + accounting_transferLinks(indicator,linksModel,yearsToCalc,transfer_techs,vintage,"perFlowAgainst")))
            / timefrac
        ...
        * sum ((timeModelToCalc, link_types),
            ...
            * timeLength(timeModelToCalc)
            * transfer_lengthParam(linksModel,link_types,"length")
            * (accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perFlow")
                + accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perFlowAlong"))

            + transfer_flowAgainst.l(timeModelToCalc,linksModel,yearsToCalc,transfer_techs,vintage)
            * timeLength(timeModelToCalc)
            * transfer_lengthParam(linksModel,link_types,"length")
            * (accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perFlow")
                + accounting_transferPerLength(indicator,linksModel,yearsToCalc,transfer_techs,vintage,link_types,"perFlowAgainst")))
            / timefrac
        )
...
indicator_accounting(%selscen%accNodesModel,accYears,indicator)
    ...
    =
    + sum((accNodesModel_a,accYears_a,indicator_a)
            $(compoundIndicatorsFullExt(accNodesModel,accYears,indicator,accNodesModel_a,accYears_a,indicator_a,accNodesModel_a,accYears_a,indicator_a)
...
indicator_accounting_var(%selscen%accNodesModel,accYears,indicator)
...
indicator_accounting_links(%selscen%indicator,nodesModelToCalc,nodesModelToCalc_a,linksModelToCalc,yearsToCalc,techs)
    ...
    sum ((transfer_techs(techs),vintage)
                $(transfer_availTech(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
                    and accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"useAnnuity") = 0),
        transfer_linksBuild.l(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
        * accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perLinkBuild") )
    ...
    + sum ((years_a,transfer_techs(techs),vintage)
                $(transfer_availTech(linksModelToCalc,years_a,transfer_techs,vintage)
                    ...
                    and years_a.val + accounting_transferLinks(indicator,linksModelToCalc,years_a,transfer_techs,vintage,"amorTime") > yearsToCalc.val
                    and years_a.val <= yearsToCalc.val ),
    ...
    + sum ((transfer_techs(techs),vintage,link_types)
                $(transfer_availTech(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
                    and accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"useAnnuity") = 0 ),
        transfer_linksBuild.l(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
        * transfer_lengthParam(linksModelToCalc,link_types,"length")
        * accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perLengthBuild") )
    ...
    + sum ((years_a,transfer_techs(techs),vintage,link_types)
                $(transfer_availTech(linksModelToCalc,years_a,transfer_techs,vintage)
                    ...
                    and years_a.val + accounting_transferPerLength(indicator,linksModelToCalc,years_a,transfer_techs,vintage,link_types,"amorTime") > yearsToCalc.val
                    and years_a.val <= yearsToCalc.val ),
        transfer_linksBuild.l(linksModelToCalc,years_a,transfer_techs,vintage)
        * transfer_lengthParam(linksModelToCalc,link_types,"length")
    ...
    + sum ((transfer_techs(techs),vintage)
        ...
        * accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perLinkDecom")

        + transfer_linksTotal.l(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
        * accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perLinkTotal")

        + sum (link_types,
            transfer_linksDecom.l(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
            * transfer_lengthParam(linksModelToCalc,link_types,"length")
            * accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perLengthDecom")

            + transfer_linksTotal.l(linksModelToCalc,yearsToCalc,transfer_techs,vintage)
            * transfer_lengthParam(linksModelToCalc,link_types,"length")
            * accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perLengthTotal"))

        + sum (timeModelToCalc,
            transfer_flowAlong.l(timeModelToCalc,linksModelToCalc,yearsToCalc,transfer_techs,vintage)
            * timeLength(timeModelToCalc)
            * ( accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perFlow")
                + accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perFlowAlong"))

            + transfer_flowAgainst.l(timeModelToCalc,linksModelToCalc,yearsToCalc,transfer_techs,vintage)
            * timeLength(timeModelToCalc)
            * ( accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perFlow")
                + accounting_transferLinks(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,"perFlowAgainst")))
            / timefrac
        ...
        + sum ((timeModelToCalc, link_types),
            ...
            * timeLength(timeModelToCalc)
            * transfer_lengthParam(linksModelToCalc,link_types,"length")
            * (accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perFlow")
                + accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perFlowAlong"))

            + transfer_flowAgainst.l(timeModelToCalc,linksModelToCalc,yearsToCalc,transfer_techs,vintage)
            * timeLength(timeModelToCalc)
            * transfer_lengthParam(linksModelToCalc,link_types,"length")
            * (accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perFlow")
                + accounting_transferPerLength(indicator,linksModelToCalc,yearsToCalc,transfer_techs,vintage,link_types,"perFlowAgainst")))
            / timefrac
    );
...
* // Update decommissioned units for units build during yearsToCalc (=after initialising converter_unitsDecom)
converter_units_ext(%selscen%map_accNodesPostCalc(accNodesModel,nodesModelToCalc),map_accYearsPostCalc(accYears,years),converter_techs(techs),vintage,"decom")
    $(converter_decomTech(nodesModelToCalc,years,converter_techs,vintage)
     and not converter_techParam(converter_techs,vintage,"freeDecom") = 1)
    = sum(years_a$
          (converter_availTech(nodesModelToCalc,years_a,converter_techs,vintage)
           and years_a.val > sum(yearsToCalc$sameas(yearsToCalc+1, years), yearsToCalc.val) - converter_techParam(converter_techs,vintage,'lifeTime')
...
* // Update decommissioned links for links build during yearsToCalc (=after initialising transfer_linksDecom)
transfer_links(%selscen%linksModel,years,transfer_techs,vintage,"decom")
    $(transfer_decomTech(linksModel,years,transfer_techs,vintage)
     and not transfer_techParam(transfer_techs,vintage,"freeDecom") = 1)
    = sum(years_a$
          (transfer_availTech(linksModel,years_a,transfer_techs,vintage)
           and years_a.val > sum(yearsToCalc$sameas(yearsToCalc+1, years), yearsToCalc.val) - transfer_techParam(transfer_techs,vintage,'lifeTime')
...
transfer_caps(%selscen%nodesModelToCalc_start,nodesModelToCalc_end,linksModel,years,transfer_techs(techs),commodity,"lowerLimit")
    $(transfer_linksParam(linksModel,years,transfer_techs,'linksLowerLimit') > 0
        and transfer_usedStartEnd(nodesModelToCalc_start,nodesModelToCalc_end,linksModel,years,transfer_techs)
        and sum(vintage$(transfer_usedTech(linksModel,years,transfer_techs,vintage)
                            and transfer_coefficient(transfer_techs,vintage,commodity,"coefficient")), 1))
...
transfer_caps(%selscen%nodesModelToCalc_start,nodesModelToCalc_end,linksModel,years,transfer_techs(techs),commodity,"upperLimit")
    $(transfer_linksParam(linksModel,years,transfer_techs,'linksUpperLimit') < inf
        and transfer_usedStartEnd(nodesModelToCalc_start,nodesModelToCalc_end,linksModel,years,transfer_techs)
        and sum(vintage$(transfer_usedTech(linksModel,years,transfer_techs,vintage)
                            and transfer_coefficient(transfer_techs,vintage,commodity,"coefficient")), 1))
...
* // Update decommissioned units for units build during yearsToCalc (=after initialising storage_unitsDecom)
storage_units_ext(%selscen%map_accNodesPostCalc(accNodesModel,nodesModelToCalc),map_accYearsPostCalc(accYears,years),storage_techs(techs),vintage,"decom")
    $(storage_decomTech(nodesModelToCalc,years,storage_techs,vintage)
     and not storage_techParam(storage_techs,vintage,"freeDecom") = 1)
    = sum(years_a$
          (storage_availTech(nodesModelToCalc,years_a,storage_techs,vintage)
           and years_a.val > sum(yearsToCalc$sameas(yearsToCalc+1, years), yearsToCalc.val) - storage_techParam(storage_techs,vintage,'lifeTime')
...
storage_caps_novintage_ext(%selscen%map_accNodesPostCalc(accNodesModel,nodesModelToCalc),map_accYearsPostCalc(accYears,years),storage_techs(techs),commodity,"lowerLimit")
    $(storage_reservoirParam(nodesModelToCalc,years,storage_techs,"unitsLowerLimit") > 0
...
storage_caps_novintage_ext(%selscen%map_accNodesPostCalc(accNodesModel,nodesModelToCalc),map_accYearsPostCalc(accYears,years),storage_techs(techs),commodity,"upperLimit")
    $(storage_reservoirParam(nodesModelToCalc,years,storage_techs,"unitsUpperLimit") < inf
...
marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator) = 0;
marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator)
    $Eq_accounting_indicatorCalc_fixed.m(accNodesModel,accYears,indicator)
    = 
    marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator)
        + Eq_accounting_indicatorCalc_fixed.m(accNodesModel,accYears,indicator);
marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator)
    $Eq_accounting_indicatorCalc_upper.m(accNodesModel,accYears,indicator)
    = 
    marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator)
        + Eq_accounting_indicatorCalc_upper.m(accNodesModel,accYears,indicator);
marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator)
    $Eq_accounting_indicatorCalc_lower.m(accNodesModel,accYears,indicator)
    = 
    marginals_indicator_bounds(%selscen%accNodesModel,accYears,indicator)
        + Eq_accounting_indicatorCalc_lower.m(accNodesModel,accYears,indicator);
...
r2a_has_converter_cost(indicator,nodesModelToCalc,yearsToCalc,converter_techs(techs),vintage,commodity)
    = (sum(activity$(accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perUnitBuild")
                or accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perUnitDecom")
                or accounting_converterUnits(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,"perUnitTotal")
                or accounting_converterActivity(indicator,nodesModelToCalc,yearsToCalc,converter_techs,vintage,activity,"perActivity")), 1)
