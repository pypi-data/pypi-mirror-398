...
$if not set timeend        $setglobal timeend           8760

$eval timemodelend ceil(%timeend%/%timeres%)
$eval timemodelstart ceil(%timestart%/%timeres%)

abort$(%timeres% <= 0)
      "Time resolution must be a positive value."
abort$(%timestart% < 1 or %timestart% > 8760)
      "Start time must be in the range of 1 to 8760."
abort$(%timeend% < 1 or %timeend% > 8760)
      "End time must be in the range of 1 to 8760."
abort$(%timemodelstart% > %timemodelend%)
      "Specified start time must be smaller or equal to end time."

set timeModelToCalc(timeModel) "Time steps to be calculated"  / tm%timemodelstart%*tm%timemodelend% /;

scalar timefrac;
$ifthene %scaletimefrac%
timefrac = (%timemodelend% - %timemodelstart% + 1) / %timemodelsteps% ;
$else 
timefrac = 1;
$endif
...
set timeMapping_t(timeModel, timeData);
option timeMapping_t < timeMapping;
...
* select subset of required data nodes
set nodesDataToCalc(nodesData);
nodesDataToCalc(nodesData) = no;
nodesDataToCalc(nodesData)$sum(nodesModelToCalc$map_nodesModel(nodesModelToCalc,nodesData), 1) = yes;

* ==== model years ====
* enforce chronological ordering of years by adding dummy years to UEL
set dummy_horizon                / horizon /;
...
set yearsSel(years) "Years selected, used to determine the years to calculate." ;
...
$onVerbatim
$ifthene.aggregateLinks %aggregatelinks%
$offVerbatim
...
/
  ...
  #nodesModel
/;
alias(accNodesModel,accNodesModel_a,accNodesModel_aa,accNodesModel_aaa,accNodesModel_aggregation);
...
/
  ...
  #years
/;
alias(accYears,accYears_a,accYears_aa,accYears_aaa);
