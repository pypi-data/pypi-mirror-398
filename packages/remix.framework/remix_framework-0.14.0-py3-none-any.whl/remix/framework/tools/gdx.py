...
def read_gdx_symbol(
    gdx_path: Union[str, dict], gdx_symbol: str, sel_dict=None, fill_ts=True
) -> pd.DataFrame:
    ...
    if fill_ts and ("timeModel" in df.index.names) and isinstance(df.index, pd.MultiIndex):
        if "timeModelToCalc" in cont.listSymbols():
            ...
            timetocalc_records.columns = [
                re.sub(r"_\d+$", "", c) for c in timetocalc_records.columns
            ]
            ...
            timemodel_records.columns = [re.sub(r"_\d+$", "", c) for c in timemodel_records.columns]
            df = fill_missing_values_in_timeseries(df, timemodel_records.set_index("timeModel"))
        else:
            df = fill_missing_values_in_timeseries(
                df, pd.DataFrame(index=df.index.get_level_values("timeModel").unique())
            )
...
def write_gdx_symbol_as_csv(gdx: str, symbol: str, directory: str):
    ...
    gams_43 = all(title in df.reset_index().columns for title in ["uni", "element_text"])
...
class GDXEval:
    def __init__(
        self,
        gdx: Union[str, dict[str | tuple, str]],
        fill_ts: bool = True,
        lazy: bool = True,
        parallel: bool = True,
        ...
        ---------
        gdx = {
            ('H2', 'budget_066', '5', 'nolimit', 'nodelay'): '.../remix_a.gdx',
            ('H2', 'budget_066', '5', 'nolimit', 'nogrid35'): '.../remix_b.gdx',
            ('H2', 'budget_066', '5', 'RE0', 'nodelay'): '.../remix_c.gdx'
            }

        Example 2
        ---------
        gdx = {"scen_1": ".../remix.gdx", "scen_2": ".../remix2.gdx"}
        """
        ...
        if isinstance(gdx, dict):
            self._gdx_dict = True
            gdx = {i: Path(j) for i, j in gdx.items()}
        else:
            self._gdx_dict = False
            gdx = Path(gdx)
    ...
    def __repr__(self) -> str:
        ...
        """
        return "Available symbols:\n" + "\n".join([f"{m}" for m in sorted(self._symbols)])
    ...
    def __getitem__(self, key: str) -> pd.DataFrame:
        ...
        if key not in self._dfs.keys():
            self._dfs[key] = self.get_symbol(key)
    ...
    def get_symbol(
        self,
        key: str,
        sel_dict: Optional[dict[str, str]] = None,
    ) -> pd.DataFrame:
        ...
        key : str
            Symbol name from the gdx file.
        sel_dict : Optional[dict[str, str]]
            Key/value pairs corresponding to index names and values to select
            by
        ...
        scen_names = ["scenario", "allocation", "co2_budget", "buildrate", "delay"]
        results_full = GDXEval(gdx, scen_names=scen_names)
        """
        if self._gdx_dict:
            assert all(
                len(self._scen_names) <= len(t) if not isinstance(t, str) else 1 for t in self._gdx
            ), "You can give at most as many scen_names as you have subscenarios "
            "mentioned in the dict-key tuples."

            # Build tuple of paths and symbols to be loaded
            arg_list = tuple(
                (pth, key, sel_dict, self._fill_ts) for pth in list(self._gdx.values())
            )
            ...
            if self._parallel:
                ...
                df_list = [
                    read_gdx_symbol(pth, key, sel_dict, fill_ts)
                    for pth, key, sel_dict, fill_ts in arg_list
                ]
            ...
            for i, j in enumerate(self._scen_names):
                idxn[i] = j
            df.index.names = idxn
        else:
            df = read_gdx_symbol(self._gdx, key, sel_dict, self._fill_ts)
