...
def merge_dfs(df_list: list):
    ...
    """
    col_ele = [ele for df in df_list for ele in df.columns.to_list()]
    col = list(dict.fromkeys(col_ele))

    # Get common index
    idx_ele = [ele for df in df_list for ele in df.index.to_list()]
    if len(idx_ele) == 0:
        return pd.DataFrame(columns=col)

    if isinstance(idx_ele[0], tuple):
        idx = pd.MultiIndex.from_tuples(list(dict.fromkeys(idx_ele)))
    else:
        idx = pd.Index(list(dict.fromkeys(idx_ele)))
    idx.names = df_list[0].index.names

    # Concat and return non-empty dataframes with common index and columns
    df_list = [df for df in df_list if not df.empty]
    if len(df_list) > 0:
        df_out = pd.concat(df_list, axis=0)
        df_out = df_out.groupby(level=list(range(df_out.index.nlevels))).agg("last").sort_index()
        return pd.DataFrame(df_out, index=idx, columns=col).sort_index()

    return pd.DataFrame(index=idx, columns=col).sort_index()
...
def merge_dicts(dict1, dict2, update_only=False):
    ...
    for key, value in dict2.items():
        ...
        if isinstance(value, Mapping):
            result[key] = merge_dicts(result.get(key, {}), value, update_only=update_only)
        else:
...
def to_dat(df: pd.DataFrame, fname: str, float_format=None):
    ...
    if isinstance(df_out.index, pd.MultiIndex):
        idx_gms = pd.Index([" . ".join(map(str, df_out.iloc[i].name)) for i in range(len(df_out))])
        df_out = df_out.set_index(idx_gms)
    ...
    #     float_format = None
    if not df.empty:
        contents = df_out.to_string(index_names=False, sparsify=False, float_format=float_format)
    else:
...
def get_limited_df(df: pd.DataFrame, file: str, _logger: FunctionType = lambda x: None):
    ...
    for digits in [None, 4, 3, 2, 1, 0]:
        ...
        if digits is None:
            ...
            df_new.to_csv(textbuf)
        lmax = max([len(line) for line in textbuf.getvalue().splitlines()])
        if lmax < 80e3:
            if _logger is not None:
                _logger("Maximum line length {} for file {} - continuing".format(lmax, file))
            return df_new
        else:
            if _logger is not None:
                _logger(
                    "Maximum line length {} for file {} - automatically reducing digits".format(
                        lmax, file
                    )
                )
    return df_new
...
def read_remix_csv(file, schema=None):
    ...
    if len(df.columns) == 1:
        ...
        if all(" . " in s for s in list(df.index)):
            ...
            df.index = pd.MultiIndex.from_tuples([tuple(s.split(" . ")) for s in df.index])
