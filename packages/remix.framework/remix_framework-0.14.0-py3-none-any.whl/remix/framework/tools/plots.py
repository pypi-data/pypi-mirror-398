...
def load_polypatches(shp, shp_attr, crs=3857):
    ...
    for k, i in coll.items():
        if isinstance(i, MultiPolygon):
            patch_coll[k] = [
                PolyPatch(xy=np.array(j.exterior.coords.xy).T, closed=True) for j in i.geoms
            ]
...
def build_linestrings(centroids: dict):
    return {
        (i, j): LineString((i_pos, j_pos))
        for i, i_pos in centroids.items()
        for j, j_pos in centroids.items()
        if i != j
    }
...
def plot_choropleth(
    ...
    if background:
        ax.add_feature(
            cfeature.NaturalEarthFeature(
                "physical", "land", "10m", edgecolor="face", facecolor="#e0e0e0"
            )
        )
    ...
    if cbar:
        ...
        norm = (
            matplotlib.colors.Normalize(vmin=limits[0], vmax=limits[1])
            if limits is not None
            else None
        )
    ...
    if "plots" in cfg_netw:
        for k, cfg_plot in cfg_netw["plots"].items():
            ...
            for y in set(data.index.get_level_values(years)):
                ...
                fig, ax = plot_network(
                    data_plt, shp, shp_attr, lat=lat, lon=lon, patch_coll=patch_coll
