...
def distribute_dictionary(dictionary: dict) -> dict:
    ...
    dict
        Deep dictionary.
    """
    _parameters = {k: v for k, v in dictionary.items() if not k.lower().startswith("set_")}
    _parameters = {k: v for k, v in _parameters.items() if not k.lower() == "version"}
    parameters = {k: v for k, v in _parameters.items() if not (is_timeseries(k) or is_map(k))}
...
def empty_dataframe(schema: dict, index_names: bool = False, column_names: bool = False):
