...
from remix.framework import __remixhome__, __version__, __versionhome__
...
LOCAL_PROFILE = (
    Path(__remixhome__)
    .joinpath("specifications")
    .joinpath("frictionless")
    .joinpath("table-schema.json")
)
...
class Manager:
        ...
        if internal_check:
            ...
            if not LOCAL_PROFILE.exists():
                ...
                if response.status_code == 200:
                    ...
                    with open(LOCAL_PROFILE, "w", encoding="utf-8") as fp:
                ...
                with open(LOCAL_PROFILE, "r", encoding="utf-8") as fp:
            ...
            try:
                ...
                logging.INFO("The local schemas are invalid, deleting and downloading again")
                parse()
            errors = {}
            ...
            if len(errors.keys()) != 0:
                ...
                raise jsonschema.ValidationError(f"There was errors in: {wrong_schemas}")
    ...
    def get_resources(self, version="latest"):
        ...
        available_versions = [
            ...
            if (bool(re.search(r"\d*\.\d*\.\d*", str(s)))) or (f"{__version__}" in str(s))
        ]
        ...
        schemas = {PurePath(s).parts[-1].split(".")[0]: self.load_schema(s) for s in schema_paths}
        schemas["version"] = version if version != "latest" else PurePath(latest_version).parts[-1]
        return schemas
    ...
    def calculate_value(string: str) -> Union[str, int]:
        ...
        return vals[0] * 100 + vals[1] * 10 + vals[2] * 1 if string != "latest" else string
