...
from remix.framework import __gamscode__, __versionhome__

# MANAGE USING A REMIX ENVIRONMENTAL VARIABLE?
MODEL_VERSION = "latest"
TARGET_DIRECTORY_INPUT_YAML = (
    Path(__versionhome__).joinpath("schema").joinpath("input").joinpath("yaml")
)
TARGET_DIRECTORY_INPUT_JSON = (
    Path(__versionhome__).joinpath("schema").joinpath("input").joinpath("json")
)
TARGET_DIRECTORY_OUTPUT_YAML = (
    Path(__versionhome__).joinpath("schema").joinpath("output").joinpath("yaml")
)
TARGET_DIRECTORY_OUTPUT_JSON = (
    Path(__versionhome__).joinpath("schema").joinpath("output").joinpath("json")
)
TARGETS = [
    TARGET_DIRECTORY_INPUT_YAML,
    TARGET_DIRECTORY_INPUT_JSON,
...
def main(as_resource=True):
    ...
    for gams_file in file_list:
        ...
        if "converter" in gams_file:
            ffk_coeffprofile = [
                field for field in fields_fks if field[0] == "converter_coefficientprofile"
            ][0]
        ...
        # END OF UGLYINESS
        schemas = [build_schemas(f, set_data, set_equalities, set_profile) for f in fields_fks]
...
def write_schemas(
    ...
    for n, t, d, s in schemas:
        ...
        n = n.lower()
        resource = {"name": n, "title": t, "description": d, "schema": s}
...
def extract_output_info(source_file: Path):
    ...
    with open(source_file, "r") as f:
        lines = f.readlines()
    chunk = "".join(lines)
    all_outputs = re.findall(
        r"(?s)(\*\s\/\/\sOUTPUT\s*:\s*.*?)(?=\"[ \t]*\;)", chunk, re.IGNORECASE
    )
...
def extract_source_data(source_file: Path):
    ...
    all_tables = re.findall(
        r"(?s)((\*\*\s\/\/\sINPUT\s*:\s*.*?)(?=/;).*?)(?=\))\)", chunk, re.IGNORECASE
    )
...
def get_fields(chunk):
    ...
    elements = [e.strip() for e in raw_list if len(e.strip()) != 0]
    ...
    for field in fields:
        ...
        if ("constraints" in field) and (field["constraints"] != ""):
            ...
            field["constraints"] = {c[0]: eval(c[1]) for c in constraints}
        else:
...
def parse_data(chunk):
    ...
    table = re.findall(
        r"(?s)(\*\s\/\/\s###\s\w+.*\*\s*\/\/\s*Title.*?)(?=\/;)", chunk, re.IGNORECASE
    )[0]
    ...
    title = raw_list[1].replace("* //", "").strip().replace("Title: ", "")
    description = raw_list[2].replace("* //", "").strip().replace("Description: ", "")
...
def parse_output_data(chunk):
    ...
    foreign_keys = (
        re.findall(r"(?s)\(((.*)?)((?=\)))", chunk)[0][1]
        .replace("%scenidx%", "scenario,")
        .replace(" ", "")
        .replace("\n", "")
        .split(",")
    )
...
def build_schemas(name_fields_fks, set_data, equalities, profile_set):
    ...
    fields = [f for f in fields if f["name"] not in PROFILE_FIELDS] if has_profiletypes else fields
    ...
    for fk in fks:
        ...
        schema_fks += [
            {
                "fields": [fk],
                "reference": {"fields": [set_maps[fk][0]], "resource": set_maps[fk][3]},
            }
        ]
...
def get_all_maps(chunk):
    ...
    for mapd_d in map_data:
        ...
        map_description = [i for i in map_info if "Description: " in i][0].split("Description: ")[
            -1
        ]
...
def build_map_schemas(map_, sets, equalities):
    ...
    fields = [{"name": m, "title": set_maps[m][0], "type": "string"} for m in map_[0:-1]]
    foreignKeys = [
