...
try:
    ...
    logging.warning(
        "To use the Excel backend, install openpyxl. Tip: use pip install remix.framework[excel]"
    )
    load_workbook = None
...
class ExcelInstance(Instance):
    ...
    def __init__(self, **kwargs) -> None:
        if load_workbook is None:
            raise ImportError(
                "Please install the openpyxl library to use this feature. Tip: use pip install remix.framework[excel]"
            )
        ...
        for element in ["parameters", "timeseries", "maps"]:
            for param in dataframes[element].keys():
                if param.lower() in sheet_names:
                    ...
                    dataframes[element][param] = self._merge_dfs(
                        [dataframes[element][param], file_dataframe]
                    )
    ...
    def write(self, inspect=False, fileformat="csv", infer=True):
        ...
        if fileformat == "xlsx":
            if self.workbook_path is not None:
                ...
                self.workbook = ExcelWriter(
                    Path(self.datadir).joinpath("remix.xlsx"), engine="openpyxl"
                )
