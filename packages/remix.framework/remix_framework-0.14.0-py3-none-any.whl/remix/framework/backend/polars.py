...
class PolarsDataset(DirectoryDataset):
    ...
    def __init__(self, **kwargs):
        if pl is None:
            raise ImportError(
                "Please install the polars library to use this feature. Tip: use pip install remix.framework[polars]"
            )
    ...
    def _merge_dfs(self, df_list, file_schema=None):
        ...
        if len(df_list) > 0:
            ...
            if "Value" in df_out.columns:
                df_out = df_out.pivot(
                    values="Value",
                    index=index_columns[:-1],
                    on=index_columns[-1],
                    aggregate_function="first",
                )
            if len(value_columns) != 0:
                if "t0001" not in df_out.columns:
                    df_out.group_by(index_columns).agg(value_columns)
                else:
    ...
    def _get_limited_df(self, df, file, _logger=None, file_schema=None):
        ...
        index_columns = [
            fk["fields"][0]
            for fk in file_schema["foreignKeys"]
            if fk["fields"][0].lower() not in ["timedata"]
        ]
        for digits in [None, 4, 3, 2, 1, 0]:
            if digits is None:
                ...
                value_columns = [
                    pl.col(vc).round(decimals=digits).last() for vc in build_timeseries_range()
                ]
    ...
    def write_profile_file(self, data, parent, name, extension, **kwargs):
        ...
        if profile_format == "tall":
            index_columns = [c for c in data.columns if not re.match(r"t\d\d\d\d", c)]
            value_cols = [c for c in data.columns if re.match(r"t\d\d\d\d", c)]
            data = data.unpivot(
                index=index_columns,
                on=value_cols,
                value_name="Value",
                variable_name="timeData",
            )
    ...
    def read_file(self, _input: str):
        ...
        if "map_" in _input:
            ...
            index_columns = [
                fk["fields"][0] for fk in file_schema["foreignKeys"] if fk["fields"][0]
            ]
            if not (
                set([id.lower() for id in index_columns])
                == set([id.lower() for id in dataframe.columns])
            ):
                ...
                dataframe = dataframe.with_columns(
                    [
                        ...
                        *[
                            pl.col("map")
                            .map_elements(lambda s, i=i: s.split(" . ")[i], return_dtype=pl.String)
                            .alias(col_name)
