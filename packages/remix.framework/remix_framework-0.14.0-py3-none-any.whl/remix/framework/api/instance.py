...
from remix.framework.api.run import default_run_remix_kwargs, run_remix
from remix.framework.schema.templates import (
    default_dataframes,
    distribute_dictionary,
    extract_sets,
    sm,
)
from remix.framework.tools.gdx import GDXEval
from remix.framework.tools.utilities import is_empty, merge_dfs, merge_dicts
...
class Instance(DirectoryDataset):
    ...
    def write(
        ...
        infer=True,
        reassign=False,
        fill_missing_with_defaults=False,
        **kwargs,
    ):
        ...
        dataframes = self.build_dataframes_to_write(fill_missing_with_defaults)
        dataframes = super().write(
            output_path,
            inspect=inspect,
    ...
    def infer_set_data(self):
        ...
        non_empty_parameters = {
            k.lower(): v for k, v in serialized_params.items() if not is_empty(v)
        }
        non_empty_profiles = {
            k.lower(): v for k, v in serialized_profiles.items() if not is_empty(v)
        }
                ...
                if parameter in self.schemas:
                    ...
                    if isinstance(element[parameter].index, MultiIndex):
                        for i in range(len(element[parameter].index.levels)):
                            index = element[parameter].index.get_level_values(i).unique()
                            current_level = [
                                entry for entry in index if entry not in ["global", "horizon"]
                            ]
                        ...
                        current_level = [
                            entry
                            for entry in element[parameter].index.tolist()
                            if entry not in ["global", "horizon"]
                        ]
        ...
        for _map in non_empty_maps:
            ...
            if map_name in self.schemas:
                ...
                if isinstance(non_empty_maps[_map], list):
                    ...
                    infer_sets = self._infer_sets_from_maps_dfs(
                        infer_sets, non_empty_maps[_map], map_sets
                    )

        infer_sets = {
            k: list(v)
            for k, v in infer_sets.items()
            if k.lower() not in ["set_profiletypes", "set_accnodesdata"]
        }
    ...
    def build_dataframes_to_write(self, fill_missing_with_defaults):
...
def container_factory(descriptor, name="Container"):
    ...
    class GenericDC:
        ...
        def _check_label_on_add(self, label):
            ...
            if label not in self._attrs:
                msg = (
                    f"The Instance '{self.name}' does not have any data with "
                    f"the label '{label}'. Please check if you specified the "
                    "correct label. The following labels are available: \n"
                    f"{', '.join(self._attrs)}"
                )
        ...
        def __init__(self):
            ...
            for key, value in descriptor.items():
                if isinstance(value, list):
                    ...
                    raise TypeError(
                        f"The element {k} has type {wrong_type} which is not valid, use a list instead."
                    )
    ...
    class DataFrameDC(GenericDC):
        ...
        def _check_datatype_on_add(self, data):
            ...
            if not isinstance(data, (DataFrame, list)):
                wrong_type = type(data)
                raise TypeError(
                    f"Datatype {wrong_type} cannot be added, please use a pandas.DataFrame."
                )
    ...
    class DataFrameDCWithDefaults(DataFrameDC):
        ...
        def _set_default_values_and_dtypes_from_schemas(self):
            ...
            required_schemas = {
                schema: self.schemas[schema] for schema in self.schemas if schema in self._attrs
            }
            self.defaults = {}
            self.datatypes = {}
            ...
            for schema_name in required_schemas:
                index_names = [
                    fk["fields"][0] for fk in required_schemas[schema_name]["foreignKeys"]
                ]
        ...
        def get_dataframes_with_defaults(self):
            ...
            for label in self._attrs:
                if isinstance(self.__dict__[f"_{label}"], list):
                    ...
                    dataframes[label] = (
                        self.__dict__[f"_{label}"]
                        .infer_objects(copy=False)
                        .fillna(self.defaults[label])
                    )

                if not dataframes[label].empty:
                    dataframes[label] = dataframes[label].loc[
                        :, ~(np.isnan(dataframes[label]).all())
                    ]
                    ...
                    try:
                        dataframes[label] = dataframes[label].astype(
                            {
                                k: v
                                for k, v in self.datatypes[label].items()
                                if k in dataframes[label].columns
                            }
                        )
