...
from remix.framework.tools.utilities import (
    build_timeseries_range,
    get_limited_df,
    is_map,
    is_timeseries,
    merge_dfs,
    ...
    def write(
        ...
        **kwargs,
        ...
        if support_sets:
            ...
            sets["set_accnodesdata"] = list(
                set(
                    dataframes["sets"].get("set_nodesdata", [])
                    + dataframes["sets"].get("set_accnodes", [])
                    + ["global"]
                )
            )
            sets["set_linksdata"] = list(
                set(dataframes["sets"].get("set_linksdata", []) + ["global"])
            )
...
class DirectoryDataset(Dataset):
    ...
    def _split_parameters_sets(self) -> None:
        ...
        """
        self.sets = {k: v for k, v in self.arranged_files.items() if k.lower().startswith("set_")}
        parameters = {
            k: v for k, v in self.arranged_files.items() if not (k.lower().startswith("set_"))
        }
    ...
    def _inherit_sets(self):
        ...
        for name, path_list in self.sets.items():
            ...
            dataframe = self.read_file(path_list[-1])
            ...
            if _name in ["set_accnodesdata", "set_linksdata", "set_years"]:
                setlist = [
                    set_element
                    for set_element in setlist
                    if set_element not in ["global", "horizon"]
                ]
            files[_name] = setlist
