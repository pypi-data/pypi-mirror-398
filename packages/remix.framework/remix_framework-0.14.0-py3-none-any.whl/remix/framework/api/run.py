...
class GAMSPathError(Exception):
    """Exception for missing GAMS installation."""
...
default_run_remix_help = {
    ...
    "roundcoefs": "Activates REMix-internal rounding of profiles for converter "
    "activities, converter coefficients, sourcesink profiles and annuities to "
    "five decimal places (can be customised via 'roundcoefsdigits' argument), "
    "default 0.",
    "roundcoefsdigits": "Specifies number of decimal places profiles for "
    "converter activities, converter coefficients, sourcesink profiles and "
    "annuities are rounded to, default 5.",
    "keep": "Instruct GAMS to keep the scratch directory (225a) after the run is finished.",
}
...
def _find_gams_pythonapi():
    ...
    if int(version) < 45:
        msg = "Support for GAMS versions below 45 will be dropped with REMix version 0.14.0."
    ...
    if int(version) < 42:
        if not os.path.isdir(gamspy_api):
            msg = (
                ...
                f"The following directory could not be found: {gamspy_api}"
            )
            raise GAMSPathError(msg)
    ...
    if pythonpath is None:
        ...
        os.environ["PYTHONPATH"] = os.pathsep.join(sitepkg + [os.environ["PYTHONPATH"]] + apifiles)
