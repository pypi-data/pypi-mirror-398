...
from remix.framework.api.dataset import DataPackage, DirectoryDataset
...
def transform_dataset(
    ...
    if len(in_format) > 1:
        raise ValueError(
            "The input dataset has mixed data. You need to either have only .csv or .dat files"
        )
    ...
    if in_format == ".csv":
        non_set_map = [
            fil
            for fil in Path(datadir).iterdir()
            if (("set_" not in fil.name) and ("map_" not in fil.name))
        ]
        with open(non_set_map[0], "r") as check_file:
            first_line = check_file.readline()
            no_names = bool(re.search(r"\,\,+", first_line)) or bool(
                re.search(r"\,\s+\,\s+", first_line)
            )
    else:
        no_names = False
