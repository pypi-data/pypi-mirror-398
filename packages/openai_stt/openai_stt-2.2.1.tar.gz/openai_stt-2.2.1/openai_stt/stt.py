...
class STT:
    ...
    - You do NOT need to call close() after each record call (auto_close_audio=True by default).
    - If you want to keep the mic open across calls, use: with STT(...) as stt:
    - Backup cleanup happens on process exit and GC.
    """
    ...
    def __init__(
        ...
        # Audio is lazily opened per record call
        ...
        if model in remote_only_models:
            ...
            if self.backend == "faster":
                ...
                self._local_model = whisper.load_model(  # type: ignore[name-defined]
                    model,
                    download_root=model_root,
                    device=device,
                )
    ...
    def close(self) -> None:
        # closes audio resources; instance can be reused (audio will re-open lazily)
    ...
    def _save_wav_temp(self, frames: list[bytes]) -> str:
        ...
        tmp.close()  # Windows file lock avoidance
    ...
    def _whisper_transcribe_file(self, filename: str) -> str:
        ...
        # Make Pylance happy: whisper returns dict; faster-whisper returns (segments, info).
        res: Any = self._local_model.transcribe(  # type: ignore[attr-defined]
            filename,
            temperature=0.0,
        ...
        # Safety: if a backend ever returns the tuple form here, handle it anyway.
