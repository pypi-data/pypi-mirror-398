...
/* On Android we must not include X11 or desktop GL headers; the X11 backend
   should be excluded from Android builds. When building for Android keep this
   file as a no-op to avoid cross-compile failures. */
#if defined(ANDROID) || defined(__ANDROID__)

/* Android: skip X11/window_x11 backend compilation */
...
{
#if defined(ANDROID) || defined(__ANDROID__)
	/* Android: this backend is disabled; function is a stub to satisfy
	   linker when accidental compilation occurs. */
	(void)width; (void)height; (void)x; (void)y; (void)resizable; (void)fullscreen;
	(void)border; (void)above; (void)CWOR; (void)title;
	return;
#endif
