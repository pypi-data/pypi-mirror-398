...
# OpenGL capability constants (from opengl_utils_def.pxi)
...
# Python constants for runtime use
GLCAP_BGRA = c_GLCAP_BGRA
...
# Simplified OpenGL utils functions for Android compatibility
cdef int gl_has_capability(int cap):
    # Simplified for Android mode - assume common capabilities
    if cap == c_GLCAP_BGRA:
        return 1  # Assume BGRA is supported
    elif cap == c_GLCAP_NPOT:
        return 1  # Assume NPOT is supported
    elif cap == c_GLCAP_S3TC:
        return 0  # Assume S3TC not supported
    elif cap == c_GLCAP_DXT1:
        return 0  # Assume DXT1 not supported
    elif cap == c_GLCAP_ETC1:
        return 1  # Assume ETC1 supported
    elif cap == c_GLCAP_UNPACK_SUBIMAGE:
        return 1  # Assume supported
    return 0

cdef int gl_get_version_major():
    return 3  # Assume OpenGL ES 3.0
