...
def load_dataset(repo_id, cache_dir="datasets"):
    """
    تحميل الداتا ست مع شريط تقدم أنيق.
    """
    if "/" not in repo_id:
    ...
    print(f"{Fore.CYAN}⬇️  Initiating download for: {repo_id}...{Style.RESET_ALL}")
    
    url = f"{BASE_URL}/datasets/{repo_id}/download"
    
    # تمرير اسم الملف ليظهر في شريط التحميل
    success = _download_file(url, file_path, description=f"Downloading {repo_name}")
    
    if success:
        print(f"\n{Fore.CYAN}📦 Extracting files...{Style.RESET_ALL}")
        try:
            with zipfile.ZipFile(file_path, 'r') as zip_ref:
                # استخدام tqdm لفك الضغط أيضاً إذا كانت الملفات كثيرة
                file_list = zip_ref.namelist()
                with tqdm(total=len(file_list), unit="file", desc="Extracting", colour="green") as pbar:
            ...
            print(f"{Fore.GREEN}✅ Dataset loaded successfully at: {save_dir}{Style.RESET_ALL}")
            return save_dir

        except zipfile.BadZipFile:
            print(f"{Fore.RED}❌ Error: Invalid zip file received.{Style.RESET_ALL}")
            return None
    else:
        return None
...
# =====================================================
# 4. Helper Internal Functions (Updated)
# =====================================================
...
def _upload_file(full_repo_name, file_path, type_category, description):
    ...
    try:
        # ملاحظة: requests لا تدعم شريط رفع natively بسهولة مثل التحميل
        # ولكن يمكن استخدام requests-toolbelt إذا أردت مستقبلاً
...
def _download_file(url, save_path, description="Downloading"):
    """
    دالة التحميل مع شريط التقدم (Loading Bar)
    """
    try:
