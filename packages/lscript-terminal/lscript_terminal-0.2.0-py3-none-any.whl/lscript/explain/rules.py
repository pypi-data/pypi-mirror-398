...
def create_default_registry() -> RuleRegistry:
    ...
    # === Chargement Premium ===
    try:
        from lscript.license import has_valid_license
        if has_valid_license():
            from lscript.premium import GitDetachedHeadRule
            # Insère en haute priorité (au début de la liste si possible, ou juste append)
            # Ici on append, le moteur checke dans l'ordre, donc à ajouter AVANT les génériques idéalement
            # Mais comme c'est spécifique ("detached HEAD"), ça ira.
            registry.register(GitDetachedHeadRule())
    except ImportError:
        pass  # Pas de module premium ou licence
