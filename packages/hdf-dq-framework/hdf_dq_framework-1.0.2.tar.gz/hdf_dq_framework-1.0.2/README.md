...
- **Value Normalization Expectation**: Added `expect_column_values_to_be_normalized` for in-place value transformation
  - Normalizes column values based on a value mapping dictionary
  - Supports case-sensitive and case-insensitive matching
  - Transforms values in-place before validation
  - Example: Normalize gender values (Female/Male → F/M)

- **Regex Matching and Replacement Expectation**: Added `expect_column_values_to_match_regex_and_replace` for pattern-based transformation
  - Matches values using regular expressions and replaces them with specified replacement strings
  - Supports regex groups in replacement (e.g., `$1`, `$2`)
  - Optional validation of transformed values
  - Example: Format phone numbers from various formats to standard XXX-XXX-XXXX

#### Improvements

- **Transformation Support**: Framework now supports transformation expectations that modify data in-place
  - Transformations are applied before validation
  - Transformed values appear in qualified DataFrame
  - Failed transformations are marked in bad DataFrame

- **Enhanced Rule Processing**: Improved handling of transformation vs validation expectations
  - Automatic detection of transformation expectations
  - Seamless integration with existing validation rules
  - Combined transformation and validation in single rule set
...
### 4a. Value Transformation Expectations

#### `expect_column_values_to_be_normalized`

**Transforms and validates** column values by normalizing them based on a value mapping dictionary. This expectation performs in-place transformation of values before validation.

```python
# Normalize gender values: Female/Male → F/M
{
    "expectation_type": "expect_column_values_to_be_normalized",
    "kwargs": {
        "column": "gender",
        "value_mapping": {
            "Female": "F",
            ...
            "female": "F",
            "male": "M",
            "F": "F",  # Already normalized
            "M": "M"   # Already normalized
        },
        "case_sensitive": False  # Optional, default False
    }
}

# Normalize status codes: Active/Inactive → A/I
{
    "expectation_type": "expect_column_values_to_be_normalized",
    "kwargs": {
        "column": "status",
...
**Key Features:**
- **In-place transformation**: Values are transformed in the DataFrame before validation
- **Case-sensitive option**: Control whether matching is case-sensitive (default: False)
- **Validation**: Ensures values can be normalized or are already normalized
- **Transformed values**: Normalized values appear in the qualified DataFrame

#### `expect_column_values_to_match_regex_and_replace`

**Transforms and validates** column values by matching them with a regular expression pattern and replacing matched values with a specified replacement string. Supports regex groups in replacement.

```python
# Format phone numbers to XXX-XXX-XXXX format
{
    "expectation_type": "expect_column_values_to_match_regex_and_replace",
    "kwargs": {
        "column": "phone",
        "regex": r"(\d{3})[^\d]*(\d{3})[^\d]*(\d{4})",  # Match 3-3-4 digit pattern
        "replacement": "$1-$2-$3",  # Format as XXX-XXX-XXXX using regex groups
        "validation_regex": r"^\d{3}-\d{3}-\d{4}$"  # Optional: validate final format
    }
}
...
# Normalize date format: MM/DD/YYYY → YYYY-MM-DD
{
    ...
    "kwargs": {
        "column": "date",
        "regex": r"(\d{2})/(\d{2})/(\d{4})",  # Match MM/DD/YYYY
        "replacement": "$3-$1-$2",  # Reorder to YYYY-MM-DD
        "validation_regex": r"^\d{4}-\d{2}-\d{2}$"  # Validate ISO format
    }
}
...
# Remove special characters from product codes
{
    ...
    "kwargs": {
        "column": "product_code",
        "regex": r"([A-Z0-9]+)[^A-Z0-9]*([A-Z0-9]+)",  # Extract alphanumeric parts
        "replacement": "$1-$2",  # Join with hyphen
        "validation_regex": r"^[A-Z0-9]+-[A-Z0-9]+$"  # Validate format
    }
}
```

**Key Features:**
- **Regex pattern matching**: Uses regular expressions to identify values to transform
- **Replacement with groups**: Supports regex groups (`$1`, `$2`, etc.) in replacement strings
- **Optional validation**: Can validate transformed values using `validation_regex`
- **In-place transformation**: Values are transformed in the DataFrame before validation
- **Transformed values**: Formatted values appear in the qualified DataFrame

**Use Cases:**
- Phone number formatting: `(123) 456-7890` → `123-456-7890`
- Date format normalization: `01/15/2023` → `2023-01-15`
- Removing special characters: `ABC-123` → `ABC123`
- Extracting and reformatting: `Name: John` → `John`
...
## Complete Example: Comprehensive Data Quality with Transformations

Here's a comprehensive example showing validation, normalization, and regex replacement working together on a single DataFrame:

```python
from pyspark.sql import SparkSession
from hdf_dq_framework import DQFramework

# Initialize Spark session
spark = SparkSession.builder.appName("Comprehensive_DQ").getOrCreate()

# Sample data with various data quality issues
data = [
    (1, "John Doe", 25, "john@email.com", "active", "Male", "(123) 456-7890"),
    (2, "Jane Smith", -5, "invalid-email", "active", "Female", "123-456-7890"),  # Bad: negative age, invalid email
    (3, "Bob Johnson", 30, "bob@email.com", "active", "M", "1234567890"),
    (4, None, 35, "alice@email.com", "active", "female", "123.456.7890"),  # Bad: null name
    (5, "Charlie Brown", 150, "charlie@email.com", "invalid_status", "male", "123 456 7890"),  # Bad: age too high, invalid status
    (6, "Diana Prince", 28, "diana@email.com", "active", "F", "invalid-phone"),  # Bad: invalid phone
]

columns = ["id", "name", "age", "email", "status", "gender", "phone"]
df = spark.createDataFrame(data, columns)

# Comprehensive quality rules - validation, normalization, and regex replacement
quality_rules = [
    # Validation Rules
    {
        "expectation_type": "expect_column_values_to_not_be_null",
...
print("Qualified Data (with normalized gender and formatted phone):")
qualified_df.show()

print("Bad Data (with failure details):")
bad_df.show()
```

This example demonstrates:
- **Validation**: Null checks, range validation, regex validation, set validation
- **Normalization**: Gender values transformed from Female/Male to F/M
- **Regex Replacement**: Phone numbers formatted to standard XXX-XXX-XXXX format
- **Combined Processing**: All transformations applied before validation
