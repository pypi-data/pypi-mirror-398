...
class Wechat:
    ...
    def get_new_message(
        ...
        def _handle_msgs(msgs: List[ChatMessage]):
            ...
            class ReplyProxy:
                """
                - reply(msg, text): 兼容旧逻辑，发文字
                - reply.image(msg, image_path): 新增能力，发图片
                """

                def __call__(self, msg: ChatMessage, text: str) -> bool:
                    # 默认文字回复
                    return watcher.reply(msg, text, reply_type="text", image_path=None)

                def image(self, msg: ChatMessage, image_path: str) -> bool:
                    # 统一按“图片”作为消息文本（与微信原生气泡一致，兼容你上游追溯逻辑）
                    return watcher.reply(msg, "图片", reply_type="image", image_path=image_path)
