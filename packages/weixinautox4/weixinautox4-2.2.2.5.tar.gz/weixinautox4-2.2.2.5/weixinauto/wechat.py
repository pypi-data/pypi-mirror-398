...
class Wechat:
    ...
    def get_new_message(
            self,
            on_new: Callable[[List[ChatMessage], object], None],
            *,
        ...
        def _handle_msgs(msgs: List[ChatMessage]):
            ...
            class ReplyProxy:
                """
                统一回复代理（兼容 build_reply_func 的 kwargs 调用）：
                  - reply(msg, text)                         -> 发文字（旧用法）
                  - reply(msg, "", reply_type="image", image_path="xx.png") -> 发图片（新用法）
                  - reply.image(msg, "xx.png")               -> 便捷发图片
                """
                ...
                def __call__(
                    ...
                    rt = (reply_type or "text").strip().lower()

                    if rt == "image":
                        # 兼容两种传法：
                        # 1) image_path="xx.png"
                        # 2) text="xx.png"（你 reply() 里也做了兼容，但这里显式更稳）
                        path = (image_path or text or "").strip()
                        if not path:
                            return False
                        # 这里 text 传不传“图片”其实都行；你 watcher.reply 内部会把回声统一成“图片”
                        return watcher.reply(msg, "图片", reply_type="image", image_path=path)

                    # 默认文字
                    return watcher.reply(msg, (text or "").strip(), reply_type="text", image_path=None)

                def image(self, msg: ChatMessage, image_path: str) -> bool:
                    return self(msg, "", reply_type="image", image_path=image_path)
