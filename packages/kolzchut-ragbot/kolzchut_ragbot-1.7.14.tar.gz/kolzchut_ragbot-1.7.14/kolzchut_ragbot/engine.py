...
import torch
import os
from .absConfigs import AbstractConfigs
import urllib.parse
...
class Engine:
    ...
    def __init__(self, configs_class: AbstractConfigs,elastic_model=None, models=None, reranker_tokenizer=None,
    ...
    def search_documents(self, query: str, top_k: int, retrieval_size: int, max_documents_from_same_page: int):
        ...
        Args:
            ...
            max_documents_from_same_page (int, optional): The maximum number of documents (paragraphs acutally) to return from the same page.
        Returns:
        ...
        for fused_id in fused_ids[:top_k]:
            for doc in all_docs:
                if doc["_source"]["page_id"] == fused_id:
                    ...
                    top_k_documents.append(source_with_id)
    ...
    def get_page_content_by_page_id(self, page_id: int) -> tuple:
        ...
        additional_page_documents = self.get_full_document_by_page_id(page_id, should_unite_documents=False)
        after_getting_additional_page = time.perf_counter()
        elapsed_time = after_getting_additional_page - before_getting_additional_page
        return additional_page_documents, elapsed_time
    ...
    def create_body(self, query, top_k_docs):
        ...
        identifier = current_config["paragraph_identifier"]
    ...
    def transform_document_to_full_page(self, document: dict) -> dict:
        ...
        if not document.get("page_id"):
            return document
        full_document = self.get_full_document_by_page_id(document["page_id"], should_unite_documents=True)
        if full_document and full_document.get("content"):
            document["content"] = full_document["content"]
        return document

    def get_full_document_by_page_id(self, page_id: int, should_unite_documents: Optional[bool] = False) -> dict | None:
        """
        ...
        Args:
            ...
            unite_documents(bool, optional) determs if to unite them all to one document or not.
        ...
        if not parts_of_documents:
            return None
        if not should_unite_documents:
            return parts_of_documents
...
def engine_factory(configs_class: AbstractConfigs, es_client=None):
    global engine
    if engine is None:
        engine = Engine(configs_class=configs_class, es_client=es_client)
    return engine
