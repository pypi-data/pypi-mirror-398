...
class TestEngine:
    ...
    def test_answer_query(self, Elasticsearch, Model, LLMClient):
        ...
        with patch.object(kolzchut_ragbot.engine.Engine, 'retrieve_documents') as mock_retrieve_documents:
            ...
            llm_client.answer.return_value = ('answer', 0.5, 100, {}, 'full user prompt')
            with patch('time.perf_counter', side_effect=[100.0, 100.0]):
                result = asyncio.run(engine.answer_query("test query", 5, 'gpt-4o'))
                actual_top_k_documents, actual_gpt_answer, actual_stats, all_docs_and_score, request_params, full_user_prompt = result
            ...
            assert 'timers_ms' in request_params
            assert full_user_prompt == 'full user prompt'
