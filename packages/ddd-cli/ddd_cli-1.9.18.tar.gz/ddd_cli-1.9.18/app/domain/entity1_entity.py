...
class Entity1Entity:
    ...
    def validate(self) -> None:
        ...
        if not self.attributeName or len(self.attributeName) < 3:
            raise Entity1ValueError(field="attributeName", detail="attributeName must be at least 3 characters")

        if self.attributeEmail and len(self.attributeEmail) > 500:
            raise Entity1ValueError(field="attributeEmail", detail="attributeEmail must not exceed 500 characters")
        ...
        for key, value in data.items():
            if hasattr(self, key) or addMode:
                try:
                    ...
                    raise Entity1ValueError(field=key, detail=f"Error in data structure: {str(e)}") from e
    ...
    def from_dict(data: dict) -> "Entity1Entity":
        ...
        try:
            ...
            raise Entity1ValueError("Error building entity:", str(e)) from e
