...
class DomainsApi(object):
    ...
    def migrate(self, domain_backend_migrator,  **kwargs):  # noqa: E501
        ...
        """
        kwargs['_return_http_data_only'] = True
        return self.migrate_with_http_info(domain_backend_migrator,  **kwargs)  # noqa: E501

    def migrate_with_http_info(self, domain_backend_migrator,  **kwargs):  # noqa: E501
        """Migrate storage backend  # noqa: E501

        Migrate the domain's storage backend to a new one.  Launches a background task to copy the domain's artifacts over to the supplied storage backend. Then updates the domain's storage settings to the new storage backend. This task does not delete the stored files of the artifacts from the previous backend.  **IMPORTANT** This task will block all other tasks within the domain until the migration is completed, essentially putting the domain into a read only state. Content will still be served from the old storage backend until the migration has completed, so don't remove the old backend until then. Note, this endpoint is not allowed on the default domain.  This feature is in Tech Preview and is subject to future change and thus not guaranteed to be backwards compatible.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.migrate_with_http_info(domain_backend_migrator, async_req=True)
        >>> result = thread.get()
        ...
        if self.api_client.client_side_validation and ('domain_backend_migrator' not in local_var_params or  # noqa: E501
                                                        local_var_params['domain_backend_migrator'] is None):  # noqa: E501
            raise ApiValueError("Missing the required parameter `domain_backend_migrator` when calling `migrate`")  # noqa: E501
        ...
        body_params = None
        if 'domain_backend_migrator' in local_var_params:
            body_params = local_var_params['domain_backend_migrator']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501
        ...
        return self.api_client.call_api(
            '/pulp/api/v3/domains/migrate/', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
