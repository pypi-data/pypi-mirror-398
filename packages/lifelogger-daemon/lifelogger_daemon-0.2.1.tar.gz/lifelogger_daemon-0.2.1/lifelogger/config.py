...
# Default keys that trigger screenshots (when screenshot_on_keypress is enabled)
DEFAULT_SCREENSHOT_KEYS = [
    "enter",
    "return",
    "space",
    "escape",
...
class CaptureConfig(BaseSettings):
    ...
    enable_screenshot_on_keypress: bool = False
    # Keys that trigger screenshots (empty list = all keys)
    screenshot_keypress_keys: list[str] = Field(default_factory=lambda: DEFAULT_SCREENSHOT_KEYS.copy())
    # Also screenshot on any key pressed with modifiers (Ctrl, Alt, Cmd)
    screenshot_keypress_with_modifiers: bool = True
