...
@click.option("--screenshot-on-keypress/--no-screenshot-on-keypress", default=None, help="Enable/disable screenshot on keypress")
@click.option("--screenshot-keypress-keys", type=str, default=None,
              help="Comma-separated list of keys that trigger screenshots (e.g., 'enter,space,escape')")
@click.option("--screenshot-keypress-add", type=str, default=None,
              help="Add key(s) to screenshot trigger list (comma-separated)")
@click.option("--screenshot-keypress-remove", type=str, default=None,
              help="Remove key(s) from screenshot trigger list (comma-separated)")
@click.option("--screenshot-keypress-with-modifiers/--no-screenshot-keypress-with-modifiers", default=None,
              help="Enable/disable screenshot on modifier combos (Ctrl/Alt/Cmd + key)")
...
def config_set(
    ...
    screenshot_on_keypress: bool | None,
    screenshot_keypress_keys: str | None,
    screenshot_keypress_add: str | None,
    screenshot_keypress_remove: str | None,
    screenshot_keypress_with_modifiers: bool | None,
    ...
    if screenshot_on_keypress is not None:
        config.capture.enable_screenshot_on_keypress = screenshot_on_keypress

    # Handle screenshot keypress keys configuration
    if screenshot_keypress_keys is not None:
        # Replace entire list
        config.capture.screenshot_keypress_keys = [
            k.strip().lower() for k in screenshot_keypress_keys.split(",") if k.strip()
        ]
    if screenshot_keypress_add is not None:
        # Add keys to existing list
        new_keys = [k.strip().lower() for k in screenshot_keypress_add.split(",") if k.strip()]
        existing = set(config.capture.screenshot_keypress_keys)
        existing.update(new_keys)
        config.capture.screenshot_keypress_keys = list(existing)
    if screenshot_keypress_remove is not None:
        # Remove keys from list
        remove_keys = {k.strip().lower() for k in screenshot_keypress_remove.split(",") if k.strip()}
        config.capture.screenshot_keypress_keys = [
            k for k in config.capture.screenshot_keypress_keys if k.lower() not in remove_keys
        ]
    if screenshot_keypress_with_modifiers is not None:
        config.capture.screenshot_keypress_with_modifiers = screenshot_keypress_with_modifiers
...
def clear(ctx: click.Context, yes: bool) -> None:
    """Clear all captured data (events, screenshots, summaries)."""
    config: Config = ctx.obj["config"]

    if not yes:
        click.confirm("This will delete all events, screenshots, and summaries. Continue?", abort=True)

    # Stop daemon if running
    pid = LifeloggerDaemon.get_running_pid(config)
    if pid:
        LifeloggerDaemon.stop_running(config)
        click.echo("Stopped running daemon.")

    import shutil
    for d in [config.events_dir, config.screenshots_dir, config.summaries_dir, config.state_dir]:
        if d.exists():
            shutil.rmtree(d)
    config.ensure_dirs()
