...
pub mod funcs {
    ...
    pub fn is_frozen_set(lines: &[LineItem]) -> bool {
    ...
    pub fn deliverable_lines(
        ...
        let plan_name: String = match lines_context.plan_name {
            Some(name) => name,
            None => {
                if lines_context.allow_soft_failure {
                    return Ok(lines);
        ...
        let pricing: Pricing = match lines_context.pricing {
            Some(price_spec) => price_spec,
            None => {
                if lines_context.allow_soft_failure {
                    return Ok(lines);
                }
