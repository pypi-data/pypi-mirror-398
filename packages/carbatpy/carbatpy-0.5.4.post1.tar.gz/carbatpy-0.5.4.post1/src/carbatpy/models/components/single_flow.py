...
class FlowDeviceOld:
    ...
    def compressor(self):
        ...
        if self.calc_type == "const_eta":
            ...
            self.warning = 100
            self.warning_message = f"""The option{self.calc_type} is not yet implemented for
            compressors"""
            raise Exception(
                self.warning_message)

        power = self.m_dot * work_specific
        delta_s = (state_out-state_in)[4]
        exergy_loss = delta_s * cb.CB_DEFAULTS['General']['T_SUR']

        #
        self.output = {"state_out": state_out,
                       "work_specific": work_specific,
                       "power": power,
                       "entropy_production_specific":delta_s,
                       "exergy_destruction_spcific": exergy_loss}
...
if __name__ == "__main__":
    ...
    # Compressor-------------
    compressor = FlowDeviceOld(myFluid, P_OUT, M_DOT,
                            device_type="machine",
                            name="compressor",
                            calc_type="const_eta",
    ...
    expander = FlowDeviceOld(myFluid, P_LOW, M_DOT,
                          device_type="machine",
                          name="expander",
                          calc_type="const_eta",
                          calc_parameters={"eta_s": ETA_S},
    ...
    pump = FlowDeviceOld(myFluid, P_OUT, M_DOT,
                      device_type="pump",
                      name="pump-A",
                      calc_type="const_eta",
                      calc_parameters={"eta_s": ETA_S},
