...
try:
    ...
    class PydanticBaseModel:
        ...
        def model_json_schema(cls) -> dict:
            raise RuntimeError(
                "Called `model_json_schema` of a placeholder class `BaseModel`"
            )


from pyfnx_utils import __version__ as pyfnx_utils_version
from pyfnx_utils.models.manifest import Manifest, NDJSON, JSON, Var
from pyfnx_utils.models.pyfunc import PyFuncVariant
from pyfnx_utils.models.env import Python3_CondaPip, PipDependency
...
class PyfuncBuilder:
    def __init__(
        ...
        self._producer_name: str = "https://github.com/OKUA1/pyfnx_utils"
        self._producer_version: str = pyfnx_utils_version
        ...
        return {
            "python3::conda_pip": asdict(
                Python3_CondaPip(
                    ...
                    dependencies=[PipDependency(p) for p in self._rt_dependencies],
                )
            )
        }
