...
class PloverRepeat:
    ...
    config_file = os.path.join(CONFIG_DIR, 'repeat_config.json')
    MAX_HISTORY = 15
    
    # Default stroke mappings
    DEFAULT_REPEAT_STROKES = {
    ...
    DEFAULT_MEMORY_TOGGLE_STROKE = 'PO*FP'
    DEFAULT_MEMORY_PASTE_STROKE = 'SKWR*PL'
    DEFAULT_MEMORY_RESET_STROKE = 'R*ET'
    DEFAULT_UNDO_STROKE = '*'
        ...
        try:
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                    self.REPEAT_STROKES = config.get('repeat_strokes', self.DEFAULT_REPEAT_STROKES)
                    self.MEMORY_TOGGLE_STROKE = config.get('memory_toggle', self.DEFAULT_MEMORY_TOGGLE_STROKE)
                    self.MEMORY_PASTE_STROKE = config.get('memory_paste', self.DEFAULT_MEMORY_PASTE_STROKE)
                    self.MEMORY_RESET_STROKE = config.get('memory_reset', self.DEFAULT_MEMORY_RESET_STROKE)
                    self.UNDO_STROKE = config.get('undo', self.DEFAULT_UNDO_STROKE)
    ...
    def use_defaults(self):
        """Use default stroke mappings"""
        self.REPEAT_STROKES = self.DEFAULT_REPEAT_STROKES.copy()
        self.MEMORY_TOGGLE_STROKE = self.DEFAULT_MEMORY_TOGGLE_STROKE
        self.MEMORY_PASTE_STROKE = self.DEFAULT_MEMORY_PASTE_STROKE
        self.MEMORY_RESET_STROKE = self.DEFAULT_MEMORY_RESET_STROKE
        self.UNDO_STROKE = self.DEFAULT_UNDO_STROKE
    ...
    def save_config(self):
        ...
        try:
            config = {
                'repeat_strokes': self.REPEAT_STROKES,
                'memory_toggle': self.MEMORY_TOGGLE_STROKE,
                'memory_paste': self.MEMORY_PASTE_STROKE,
                'memory_reset': self.MEMORY_RESET_STROKE,
                'undo': self.UNDO_STROKE
            }
    ...
    @staticmethod
    def get_option_info():
        """Provide configuration options for Plover's plugin manager"""
        return {
            'repeat_strokes': (
                'Repeat stroke mappings (JSON format: {"STROKE": count, ...})',
                str,
                str({
                    'RA*PT': 1, 'RO*PT': 2, 'RAO*PT': 3, 'R*EPT': 4, 'RA*EPT': 5,
                    'RO*EPT': 6, 'RAO*EPT': 7, 'R*UPT': 8, 'RA*UPT': 9, 'RO*UPT': 10,
