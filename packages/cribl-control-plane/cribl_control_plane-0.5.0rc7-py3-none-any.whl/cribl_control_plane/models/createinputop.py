...
class CreateInputDataCompressionFormat(str, Enum, metaclass=utils.OpenEnumMeta):
    NONE = "none"
    GZIP = "gzip"
...
class PersistenceSystemStateTypedDict(TypedDict):
    ...
    max_data_time: NotRequired[str]
    r"""Maximum amount of time to retain data (examples: 2h, 4d). When limit is reached, older data will be deleted."""
    compress: NotRequired[CreateInputDataCompressionFormat]
    dest_path: NotRequired[str]
...
class PersistenceSystemState(BaseModel):
    ...
    def serialize_compress(self, value):
        if isinstance(value, str):
            try:
                return models.CreateInputDataCompressionFormat(value)
            except ValueError:
                return value
        return value
...
class CreateInputInputCollectionTypedDict(TypedDict):
    id: str
    r"""Unique ID for this input"""
    type: NotRequired[CreateInputTypeCollection]
    disabled: NotRequired[bool]
    pipeline: NotRequired[str]
...
class CreateInputInputCollection(BaseModel):
    ...
    type: Optional[CreateInputTypeCollection] = CreateInputTypeCollection.COLLECTION
