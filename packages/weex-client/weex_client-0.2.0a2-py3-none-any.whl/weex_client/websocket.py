...
class WeexWebSocketClient:
    ...
    def _build_connect_kwargs(
        ...
        # websockets < 15.0 uses 'extra_headers', >= 15.0 uses 'additional_headers'
        ws_version = version.parse(websockets.__version__)
        header_param = (
            "additional_headers"
            if ws_version >= version.parse("15.0")
            else "extra_headers"
        )

        headers_kwargs = {}
        if private:
            headers_kwargs[header_param] = self._build_auth_headers()
        else:
            headers_kwargs[header_param] = {
