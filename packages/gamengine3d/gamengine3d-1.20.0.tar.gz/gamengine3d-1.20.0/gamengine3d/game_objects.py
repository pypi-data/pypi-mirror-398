...
class Cuboid(GameObject):
    def __init__(self, pos: vector3d = vector3d.zero,
                 ...
                 visible: bool = True, emit: bool = False):
        super().__init__(name)
        self.name = name
        self.pos = pos
        self.size = size
    ...
    def deserialize(data, context):
        ...
        obj = Cuboid(pos=vector3d.fromtuple(pos), color=Color.RGB(*color), size=vector3d.fromtuple(size), name=name, rotation=vector3d.fromtuple(rotation), visible=visible, emit=emit)
...
class Sphere(GameObject):
    ...
    def draw(self, renderer: Renderer):
        if not self.visible:
            return
        renderer.render_sphere(self.pos, self.radius, self.color, self.segments, self.rings, emit=self.emit)
    ...
    def deserialize(data, context):
        ...
        obj = Sphere(pos=vector3d.fromtuple(pos), color=Color.RGB(*color), radius=radius, segments=segments, rings=rings, name=name, visible=visible, emit=emit)
        obj.attach_scripts(scripts, context)
...
class Cylinder(GameObject):
    ...
    def draw(self, renderer: Renderer):
        ...
        renderer.render_cylinder(center=self.pos, height=self.length, radius=self.radius,
                                 color=self.color, segments=self.segments, rotation=self.rotation, emit=self.emit)
    ...
    def deserialize(data, context):
        ...
        obj = Cylinder(pos=vector3d.fromtuple(pos), length=length, radius=radius, color=Color.RGB(*color), segments=segments, rotation=vector3d.fromtuple(rotation), name=name, visible=visible, emit=emit)
        obj.attach_scripts(scripts, context)
...
class ObjModel(GameObject):
    def __init__(self, pos: vector3d = vector3d.zero,
                 ...
                 rotation: vector3d = vector3d.zero,
                 file_path: str = "", visible: bool = True, emit: bool = False):
        super().__init__(name)
        self.name = name
        self.pos = pos
        self.color = color
    ...
    def draw(self, renderer: Renderer):
        ...
        for face in self.faces:
            ...
            renderer.render_plane(p1=p1, p2=p2, p3=p3, color=self.color, emit=self.emit)
    ...
    def deserialize(data, context):
        ...
        obj = ObjModel(pos=pos, name=name, color=color, scale=scale, rotation=rotation, visible=visible, file_path=file_path, emit=emit)
        obj.attach_scripts(scripts, context)
        return obj
...
class Text(GameObject):
    def __init__(self, pos: vector3d = vector3d.zero,
                 ...
                 spacing: float = .2,
                 rotation: vector3d = vector3d.zero,
                 center: bool = False, emit: bool = False):
        super().__init__(name)
        self.name = name
        self.pos = pos
