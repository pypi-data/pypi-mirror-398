...
class PlayerMovement:  # Class name must match file name in PascalCase
    def __init__(self, obj: Cuboid, context: Context):
        ...
        self.context.on_key_press("left shift", callback=self.handle_controls, dt=True, args=["faster"])
        self.context.on_key_released("left shift", callback=self.handle_controls, dt=True, args=["slower"])
    ...
    def handle_controls(self, dt, direction):
        ...
        if direction == "forward":
            ...
            if any([
                ...
                self.context.functions.is_colliding(self.obj.name, "Front"),
            ]):
                self.obj.pos -= delta_pos
