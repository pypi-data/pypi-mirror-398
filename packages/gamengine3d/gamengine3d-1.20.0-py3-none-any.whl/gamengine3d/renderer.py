...
class Renderer:
    ...
    def render_shadow_pass(self, objects):
        # If there are no lights, skip
        if not self.lights:
            return

        # Find the first point light (has .position) to use for shadow map.
        # This avoids crashing when the first light is an AreaLight (which has no .position).
        light = None
        ...
        # If no point-light found (only area-lights or none), skip shadow pass.
        if light is None:
            return

        # Compute light-space matrix from chosen point light
    ...
    def _send_lights(self):
        ...
        # Helper: sample a quad (p1,p2,p3,p4) into n_samples in a grid
        ...
        # Helper: sample a triangle using barycentric / grid sampling
        ...
        # --- per-frame emissive surfaces collected during draw calls ---
        for em in self.frame_emissives:
            shape = em.get('shape')
            samples = int(em.get('samples', 4))
            em_color = tuple(c / 255 for c in em.get('color', (255, 255, 255)))
            em_intensity = float(em.get('intensity', 1.0))
            if shape == 'quad':
                corners = em['params']['corners']
                pts = sample_quad(corners, samples)
            elif shape == 'tri':
                corners = em['params']['corners']
                pts = sample_tri(corners, samples)
            else:
                pts = []
            if not pts:
                continue
            per_sample_int = em_intensity / max(1, len(pts))
            for p in pts:
                flattened.append({
                    'position': p.totuple(),
                    'color': em_color,
