...
import tensorplay
from tensorplay.types import _dtype  # noqa
...
def is_autocast_available(device_type: str) -> bool:
    ...
    """
    pass
    # return tensorplay._C._is_autocast_available(device_type)
...
class autocast:
    ...
    must be invoked in that thread.  This affects :class:`tensorplay.nn.DataParallel` and
    :class:`tensorplay.nn.parallel.DistributedDataParallel` when used with more than one GPU per process
    ...
    Args:
        ...
        enabled(bool, optional):  Whether autocasting should be enabled in the region.
            Default: ``True``
        dtype(tensorplay_dtype, optional):  Data type for ops run in autocast. It uses the default value
    ...
    def __init__(
        ...
        if dtype is None:
            dtype = tensorplay.get_autocast_dtype(device_type)
        if tensorplay._jit_internal.is_scripting():
        ...
        self.custom_backend_name = tensorplay._C._get_privateuse1_backend_name()
        self.fast_dtype = tensorplay.get_autocast_dtype(self.device)
        if self.device == self.custom_backend_name:
            ...
            message += "a module by `tensorplay._register_device_module`, and the module must have these funcs: \n"
            message += "`get_amp_supported_dtype() -> List[tensorplay.dtype]`. \n"

            assert hasattr(tensorplay, self.custom_backend_name), message
            self.custom_device_mod = getattr(tensorplay, self.custom_backend_name)
            for func in necessary_funcs:
                assert hasattr(self.custom_device_mod, func), (
                    message + f"But the func `{func}` is missing. \n"
                )

        self._cache_enabled = tensorplay.is_autocast_cache_enabled()
        if (
            enabled
            and self.device == "cuda"
            and tensorplay.cuda.amp.common.amp_definitely_not_available()
        ):
            warnings.warn(
        ...
        if self.device == "cpu":
            supported_dtype = [tensorplay.bfloat16, tensorplay.float16]
            ...
            if self.fast_dtype not in supported_dtype:
                ...
                error_message += "MTIA Autocast only supports dtypes of tensorplay.bfloat16 and tensorplay.float16 currently."
                warnings.warn(error_message)
                enabled = False
            ...
            if self.fast_dtype not in supported_dtype:
                ...
                error_message += "MAIA Autocast only supports dtypes of tensorplay.bfloat16 and tensorplay.float16 currently."
                warnings.warn(error_message)
                enabled = False
            ...
            if self.fast_dtype not in supported_dtype:
                ...
                error_message += "XPU Autocast only supports dtypes of tensorplay.bfloat16 and tensorplay.float16 currently."
                warnings.warn(error_message)
                enabled = False
        elif self.device == "ipu":
            supported_dtypes = [tensorplay.bfloat16, tensorplay.float16]
            if self.fast_dtype not in supported_dtypes:
                ...
                error_message += "IPU Autocast only supports dtypes of tensorplay.bfloat16 and tensorplay.float16 currently."
                warnings.warn(error_message)
                enabled = False
            ...
            if self.fast_dtype not in supported_dtype:
                ...
                error_message += "HPU Autocast only supports dtypes of tensorplay.bfloat16 and tensorplay.float16 currently."
                warnings.warn(error_message)
                enabled = False
            ...
            if (
                enabled
                and self.fast_dtype == tensorplay.bfloat16
                and not tensorplay.cuda.is_bf16_supported()
            ):
                raise RuntimeError(
            ...
            if self.fast_dtype not in supported_dtype:
                error_message = (
                    ...
                    "MPS Autocast only supports dtype of tensorplay.bfloat16 and tensorplay.float16 currently."
                )
                warnings.warn(error_message)
                enabled = False
            elif self.fast_dtype == tensorplay.bfloat16:
                if not tensorplay.backends.mps.is_macos_or_newer(14, 0):
                    error_message = (
                        "In MPS autocast, but the target dtype tensorplay.bfloat16 is not supported "
                        "on macOS versions below 14. Disabling autocast."
                    )
                    warnings.warn(error_message)
                    enabled = False
        elif self.device == "xla":
            supported_dtype = [tensorplay.float16, tensorplay.bfloat16]
            if self.fast_dtype not in supported_dtype:
                error_message = "In XLA autocast, but the target dtype is not supported. Disabling autocast.\n"
                error_message += (
                    "XLA Autocast only supports dtype of tensorplay.bfloat16 currently."
                )
                warnings.warn(error_message)
                enabled = False
        self._enabled = enabled
    ...
    def __enter__(self):
        ...
        self.prev_cache_enabled = tensorplay.is_autocast_cache_enabled()
        self.prev = tensorplay.is_autocast_enabled(self.device)
        self.prev_fastdtype = tensorplay.get_autocast_dtype(self.device)
        tensorplay.set_autocast_enabled(self.device, self._enabled)
        tensorplay.set_autocast_dtype(self.device, self.fast_dtype)  # type: ignore[arg-type]
        tensorplay.autocast_increment_nesting()
        tensorplay.set_autocast_cache_enabled(self._cache_enabled)
        ...
        # for preserving autocast in tensorplay.export.export.
        if tensorplay._C._is_tensorplay_function_mode_enabled():
            stacks = tensorplay.overrides._get_current_function_mode_stack()
            for mode in stacks:
                if isinstance(
                    mode,
                    tensorplay.fx.experimental.proxy_tensor.PreDispatchTorchFunctionMode,
                ):
                    args = (
                        self.device,
                        self.fast_dtype,
                    ...
                    return mode.__tensorplay_function__(tensorplay.amp._enter_autocast, (), args)
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any):  # type: ignore[override]
        ...
        if tensorplay.autocast_decrement_nesting() == 0:
            tensorplay.clear_autocast_cache()
        tensorplay.set_autocast_enabled(self.device, self.prev)
        tensorplay.set_autocast_dtype(self.device, self.prev_fastdtype)
        tensorplay.set_autocast_cache_enabled(self.prev_cache_enabled)
        ...
        if tensorplay._C._is_tensorplay_function_mode_enabled():
            ...
            for mode in stacks:
                if isinstance(
                    ...
                    return mode.__tensorplay_function__(tensorplay.amp._exit_autocast, (), ())
        return False
...
def _enter_autocast(*vals):
    ...
    if tensorplay._C._is_tensorplay_function_mode_enabled():
        return tensorplay.overrides.handle_tensorplay_function(
            tensorplay.amp._enter_autocast, [], *vals
        )
    mode = tensorplay.amp.autocast(*vals)
    mode.__enter__()
    return mode
...
def _exit_autocast(mode):
    if tensorplay._C._is_tensorplay_function_mode_enabled():
        return tensorplay.overrides.handle_tensorplay_function(tensorplay.amp._exit_autocast, [], mode)
...
def custom_fwd(
    ...
    *,
    ...
    def decorate_fwd(*args, **kwargs):
        args[0]._dtype = tensorplay.get_autocast_dtype(device_type)
        if cast_inputs is None:
            args[0]._fwd_used_autocast = tensorplay.is_autocast_enabled(device_type)
            return fwd(*args, **kwargs)
        else:
            autocast_context = tensorplay.is_autocast_enabled(device_type)
