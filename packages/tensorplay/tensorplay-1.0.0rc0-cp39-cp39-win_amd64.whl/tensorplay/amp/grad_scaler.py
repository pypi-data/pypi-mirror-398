...
class _MultiDeviceReplicator:
    ...
    def __init__(self, master_tensor: tensorplay.Tensor) -> None:
        ...
        self._per_device_tensors: dict[tensorplay.types.Device, tensorplay.Tensor] = {}

    def get(self, device: tensorplay.types.Device) -> tensorplay.Tensor:
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(
        ...
        if self._device == "cuda":
            # if enabled and tensorplay.cuda.amp.common.amp_definitely_not_available():
            #     warnings.warn(
        ...
        if self._enabled:
            ...
            self._scale: Optional[tensorplay.Tensor] = None
            self._growth_factor = growth_factor
            self._backoff_factor = backoff_factor
            self._growth_interval = growth_interval
            self._init_growth_tracker = 0
            # self._growth_tracker will be lazily initialized during the first call to scale()
            self._growth_tracker: Optional[tensorplay.Tensor] = None
    ...
    def _lazy_init_scale_growth_tracker(self, dev: tensorplay.types.Device) -> None:
        assert self._growth_tracker is None, "_growth_tracker initialized before _scale"
        self._scale = tensorplay.full((), self._init_scale, dtype=tensorplay.float32, device=dev)
    ...
    @overload
    def scale(self, outputs: tuple[tensorplay.Tensor, ...]) -> tuple[tensorplay.Tensor, ...]:
        ...

    @overload
    def scale(self, outputs: Iterable[tensorplay.Tensor]) -> Iterable[tensorplay.Tensor]:
        ...

    def scale(
        self,
        outputs: Union[tensorplay.Tensor, Iterable[tensorplay.Tensor]],
    ) -> Union[tensorplay.Tensor, Iterable[tensorplay.Tensor]]:
        """
        ...
        def apply_scale(val: Union[tensorplay.Tensor, Iterable[tensorplay.Tensor]]):
    ...
    def _unscale_grads_(
        ...
        allow_fp16: bool,
    ) -> dict[tensorplay.types.Device, tensorplay.Tensor]:
        ...
        per_device_and_dtype_grads: dict[
            tensorplay.types.Device, dict[tensorplay.dtype, list[tensorplay.Tensor]]
        ] = defaultdict(lambda: defaultdict(list))
        with tensorplay.no_grad():
            for group in optimizer.param_groups:
                for param in group["params"]:
                    assert isinstance(param, tensorplay.Tensor)
                    ...
                    #     if param.grad.dtype is tensorplay.float16:
    ...
    def unscale_(self, optimizer: tensorplay.optim.Optimizer) -> None:
        ...
        inv_scale = (
            ...
            # if self._scale.device != tensorplay.device("mps:0")
            # else self._scale.reciprocal()
        )
        found_inf = tensorplay.full((), 0.0, dtype=tensorplay.float32, device=self._scale.device)
    ...
    def update(self, new_scale: Optional[Union[float, tensorplay.Tensor]] = None) -> None:
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = "new_scale should be a float or a 1-element tensorplay.cuda.FloatTensor or \
                    tensorplay.FloatTensor with requires_grad=False."
            ...
            tensorplay._C._amp_update_scale_(
                _scale,
                _growth_tracker,
                found_inf_combined,
                self._growth_factor,
    ...
    def _get_scale_async(self) -> Optional[tensorplay.Tensor]:
        return self._scale
    ...
    def _check_inf_per_device(self, optimizer: tensorplay.optim.Optimizer) -> dict[str, Any]:
        _scale, _ = self._check_scale_growth_tracker("_check_inf_per_device")

        dummy_inv_scale = tensorplay.full((), 1.0, dtype=tensorplay.float32, device=_scale.device)
        found_inf = tensorplay.full((), 0.0, dtype=tensorplay.float32, device=_scale.device)
    ...
    def _found_inf_per_device(self, optimizer: tensorplay.optim.Optimizer) -> dict[str, Any]:
        return self._per_optimizer_states[id(optimizer)]["found_inf_per_device"]
