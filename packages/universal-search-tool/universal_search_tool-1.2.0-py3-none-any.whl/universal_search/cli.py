...
📝 使用示例:
   ...
   universal-search "AI羊毛" --time-filter w          # 最近一周
   universal-search "热门新闻" --time-filter d        # 最近一天
   universal-search "技术趋势" --time-filter m        # 最近一个月
   universal-search --setup-guide

💡 多Key配置示例:
   export SERPAPI_API_KEYS="key1,key2,key3"
   # 或配置文件: {"api_keys": ["key1", "key2", "key3"]}
        '''
    )
    ...
    parser.add_argument('--time-filter', '-t', choices=['h', 'd', 'w', 'm', 'y'],
                       help='时间筛选: h=小时, d=天, w=周, m=月, y=年 (仅Google支持)')
    parser.add_argument('--version', action='version', version='Universal Search Secure 1.2.0')
    ...
    try:
        ...
        search.print_results(args.query, args.engine, args.count, args.time_filter)
    except SystemExit:
        print("\n🔑 配置您的 API Key 后即可搜索")
...
def check_config():
        ...
        try:
            ...
            if 'api_keys' in config:
                ...
                if isinstance(api_keys, list) and api_keys:
                    print(f"✅ 配置文件: 已配置 {len(api_keys)} 个API keys")
                    for i, key in enumerate(api_keys, 1):
                        print(f"   Key #{i}: {key[:8]}...{key[-8:]}")
                else:
                    print("❌ 配置文件: api_keys 为空或格式错误")
            # 检查旧格式（单key）
            elif 'api_key' in config:
                file_key = config['api_key']
                if file_key:
                    print("✅ 配置文件: 已配置 (单key格式)")
                    print(f"   Key: {file_key[:8]}...{file_key[-8:]}")
                else:
                    print("❌ 配置文件: 无 api_key")
            else:
                print("❌ 配置文件: 无 api_key 或 api_keys 字段")
        except Exception:
            print("❌ 配置文件: 格式错误")
    else:
        print("❌ 配置文件: 不存在")
    ...
    try:
        search = UniversalSearch()
        results = search.search("Python", "google", 1, None)
        if results:
            print("✅ 搜索连接: 正常")
            print("🎊 您可以开始搜索了！")
        else:
