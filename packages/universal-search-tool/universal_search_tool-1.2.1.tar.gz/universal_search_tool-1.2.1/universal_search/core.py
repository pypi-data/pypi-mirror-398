        ...
        for attempt in range(max_retries):
            try:
                ...
                if "error" in results:
                    ...
                    if self._handle_search_failure(Exception(f"API Error: {error_msg}")):
                        print(f"⚠️  当前key失败: {error_msg}")
                        continue  # 尝试下一个key
                    else:
                        break  # 所有key都试过了
                ...
                if "organic_results" in results:
                    ...
                    # 没有搜索结果，但可能是正常情况（比如搜索词太特殊）
                    # 不切换key，返回空结果
                    print(f"⚠️  API key #{self.current_key_index + 1} 没有找到结果，但key正常")
                    return []
