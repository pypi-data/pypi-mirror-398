...
def get_cci_means( obj, cci_idx_lst = None, 
                   cells = [], genes = [], pval_cutoff = 0.05 ):

    if isinstance(obj, anndata.AnnData): 
        cci_df_dct = obj.uns['CCI_sample']
    elif isinstance(obj, dict):
        cci_df_dct = obj
    else:
        print('ERROR: Input object must be either AnnData or dictionary containing CCI results. ')
        return None
    ...
    if isinstance(obj, dict):
        ...
        df.insert( 0, 'Group', df.index.values.tolist() )
        df['Group'] = df['Group'].replace(obj.uns['lut_sample_to_cond'])
...
def plot_population(df_pct, bar_width = 0.6, title = None, title_fs = 12, ylabel = None,
                    ...
                    legend_fs = 10, legend_loc = 'center left', bbox_to_anchor = (1,0.5), 
...
def plot_cnv( adata_in, groupby = 'ploidy_dec', N_cells_min = 50, N_cells_max = 0,
    ...
    if spot_llst is None:
        if show_cna_spots:                
            results = find_signif_CNV_gain_regions( adata, groupby = groupby,
