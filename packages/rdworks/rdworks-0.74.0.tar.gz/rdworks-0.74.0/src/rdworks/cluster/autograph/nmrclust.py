...
def memberIndices(communityID, communityAssignment):
    indices = [
        x
        for x in range(len(communityAssignment))
        if communityAssignment[x] == communityID
    ]
    return indices
...
def averageSpread(rmsdMatrix, communityAssignment):
    ...
    spreads = [
        spread(rmsdMatrix, memberIndices(C, communityAssignment)) for C in communities
    ]
...
def mergeClusters(rmsdMatrix, communityAssignment, aveLinkArray=None):
    ...
    aveLinkArray[range(numCommunities), range(numCommunities)] = (
        np.max(aveLinkArray) + 0.01
    )
...
def NMRCLUST(rmsdMatrix):
    ...
    while singletonPresent:
        communityAssignment, aveLinkArray = mergeClusters(
            rmsdMatrix, communityAssignment, aveLinkArray
        )
        communities = list(set(communityAssignment))
        commcount = [
            len([x for x in communityAssignment if x == C]) for C in communities
        ]
    ...
    nClustList = [
        len(set(x)) for x in assignList
    ]  # number of clusters from when it was began recoded to one cluster, decreasing by one cluster at each step
