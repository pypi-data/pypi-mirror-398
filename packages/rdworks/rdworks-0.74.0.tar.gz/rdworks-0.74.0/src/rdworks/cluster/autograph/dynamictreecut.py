...
def treeCutCore(H, I, tau=5):
    ...
    significant_breakpoints = [
        ...
        if trans_indices[x] - breakpoints[x] > tau
    ]
...
def getClusterNodeIndices(comprehensive_path_list, cluster_substring):
    ...
    return [
        x
        for x in range(len(comprehensive_path_list))
        if comprehensive_path_list[x].startswith(cluster_substring)
    ]
...
def dynamicTreeCut(tree, n, tau=5):
    ...
    allHeights = (
        [0] + allHeights + [0]
    )  # Sandwitch the heights with zero so that the ends can be included or left out from major clusters.
    breakpoints = [0, -1]
    updateList = [-1]
