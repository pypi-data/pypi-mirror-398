...
def getModularity(affinityMatrix, communityAssignments, resolution=1.0):
    ...
    Q = 0
    affinityMatrix[range(arrayDim), range(arrayDim)] += affinityMatrix[
        range(arrayDim), range(arrayDim)
    ]
    two_m = np.sum(affinityMatrix)
    for C in communities:
        communityIndices = [
            i for i in range(len(communityAssignments)) if communityAssignments[i] == C
        ]
...
def LouvainPhase1(
    affinityMatrix, communityAssignment, Q_threshold, max_iter, resolution
):
    ...
    affinityMatrix[range(num_nodes), range(num_nodes)] += affinityMatrix[
    ...
    while changeModularity > Q_threshold and iterations < max_iter:
        ...
        for i in range(num_nodes):  # for each node
            neighborIndices = [
                j for j in range(num_nodes) if adjacencyMatrix[i, j] == 1
            ]
            ...
            for C in communities:
                ...
                deltaQ = (
                    ki_in / two_m - resolution * sigma_tot * ki / (two_m**2 / 2)
                )  # simplified formula. Derivation from (https://hal.archives-ouvertes.fr/hal-01231784/document)
        ...
        modularity_current = getModularity(
            affinityMatrix, communityAssignment, resolution
        )
...
def LouvainPhase2(affinityMatrix, communityAssignment):
    ...
    for i in range(num_communities):
        i_members = [
            x for x in range(num_nodes) if communityAssignment[x] == communities[i]
        ]
        for j in range(num_communities):
            j_members = [
                x for x in range(num_nodes) if communityAssignment[x] == communities[j]
            ]
            phase2graph[i, j] = np.sum(affinityMatrix[i_members, :][:, j_members]) * (
                1 / 2
            ) ** (i == j)
...
def Louvain(affinityMatrix, Q_threshold=0.001, max_iter=50, resolution=1.0):
    ...
    while changeModularity > Q_threshold and iterations < max_iter:
        ...
        modularity_past = getModularity(
            graph, comm, resolution
        )  # Note: we want to compare Q before and after reassignments in phase 1
    ...
    for i in range(len(communityAssignmentRecord) - 2, 0, -1):
        ...
        for key in changeDict:
            oldIndices = [
                x
                for x in range(len(communityAssignmentRecord[i - 1]))
                if communityAssignmentRecord[i - 1][x] == key
            ]
            for j in oldIndices:
    ...
    while upperIndex != lowerIndex - 1:  # Until the indices are consecutive, iterate
        ...
        for i in nodesVisited:
            newNodes = [
                ...
                if adjacencyMatrix[i, j] == 1 and j not in nodesVisited
            ]
...
def AutoGraph(rmsdMatrix):
    ...
    communityAssignment = Louvain(
        filteredAffinityMatrix, Q_threshold=0.0, max_iter=50, resolution=1.0
    )
