...
from pycharter.runtime_validator.runtime_validator import get_model_from_contract
        ...
        def wrapper(*args, **kwargs):
            ...
            if not result.is_valid:
                if on_error:
                    ...
                    # Replace with validated data (None if invalid)
                    if param_name in kwargs:
                        kwargs[param_name] = result.data if result.is_valid else None
                    else:
                        # Can't modify positional args easily, so we'll pass through
                        pass
            ...
            if result.is_valid:
                if param_name in kwargs:
                    ...
                    # For positional args, we need to reconstruct
                    param_index = param_names.index(param_name)
                    args_list = list(args)
        ...
        def wrapper(*args, **kwargs):
            ...
            if not result.is_valid:
                if on_error:
                    ...
                    # Return None or original value on error
                    return None

            return result.data if result.is_valid else result_value
