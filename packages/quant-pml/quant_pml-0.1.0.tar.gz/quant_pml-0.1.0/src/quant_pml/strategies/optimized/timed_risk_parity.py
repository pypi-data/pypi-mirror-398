...
class TimedRiskParityStrategy(BaseEstimatedStrategy):
    MIN_NUM_POINTS = 3
    ...
    def _optimize(self, prediction_data: PredictionData) -> pd.DataFrame:  # noqa: ARG002
        weights = (
            np.array(self._vols_history[:-1]).mean(axis=0) / self._current_vols
            if len(self._vols_history) > self.MIN_NUM_POINTS
    ...
    @staticmethod
    def _combine_strategies(weights: pd.Series) -> pd.Series[float]:
