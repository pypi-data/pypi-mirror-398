...
class VolatilityTargeting:
    ...
    def transform(
        self,
        strategy_excess_r: pd.DataFrame | pd.Series,
        rebal_dates: pd.DatetimeIndex | pd.Index,
        ...
        if isinstance(strategy_excess_r, pd.DataFrame):
            strategy_excess_r = strategy_excess_r.iloc[:, 0]

        strategy_rv = self.get_rolling_rv(strategy_excess_r, rebal_dates)
        target = np.sqrt(self.get_rolling_rv(baseline, rebal_dates)) if baseline is not None else target_vol
