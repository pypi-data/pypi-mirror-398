        ...
        for key, value in self.__dict__.items():
            ...
            if isinstance(value, dict):
                for sub_key, sub_val in value.items():
                    res.append([f"{key}_{sub_key}", sub_val])
            else:
                res.append([key, value])
        return pd.DataFrame(res, columns=["metric", "value"]).set_index("metric")
...
class Assessor:
    ...
    def _run(self, strategy_total: pd.Series) -> StrategyStatistics:
        ...
        if self.factor_annual is None:
            factor_annual = get_factor_annual(strategy_total.index)
        else:
        ...
        if final_nav < 0:
            msg = "Strategy defaulted (Final NAV < 0)!"
        ...
        buy_hold_ir = buy_hold_alpha / buy_hold_tracking_error if buy_hold_tracking_error != 0 else 0.0
