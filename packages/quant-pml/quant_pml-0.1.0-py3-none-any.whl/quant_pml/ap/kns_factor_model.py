...
from quant_pml.features.ols_betas import get_exposures
...
class KNSFactorModel(BaseAssetPricer):
    ...
    def fit(self, test_assets_xs_r: pd.DataFrame) -> None:
        ...
        factors = pd.DataFrame(factors, index=test_assets_xs_r.index, columns=factor_names)
        ...
        self._w_mean_var = (factor_means - self.l1_penalty) / (factor_vars + self.l2_penalty)
        self._w_mean_var = np.where(factor_means >= self.l1_penalty, self._w_mean_var, 0)
    ...
    def _get_deviations(self, test_assets_xs_r: pd.DataFrame, factors: pd.DataFrame) -> tuple[pd.Series, pd.Series]:  # noqa: ARG002
    ...
    @staticmethod
    def _estimate_risk_premia(exposures: pd.DataFrame, test_assets_xs_r: pd.DataFrame) -> pd.Series:
