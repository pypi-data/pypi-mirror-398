...
class QuadraticProgram:
    def __init__(
        self,
        P: np.ndarray | spa.csc_matrix,
        q: np.ndarray,
        G: np.ndarray | spa.csc_matrix | None = None,
        h: np.ndarray | None = None,
        A: np.ndarray | spa.csc_matrix | None = None,
        ...
        **kwargs,
        ...
        if self.solver_settings["solver"] not in USABLE_SOLVERS:
            msg = f"Solver '{self.solver_settings['solver']}' is not available. Choose from: {USABLE_SOLVERS}"
    ...
    def solve(self) -> None:
        ...
        """
        if self.solver_settings["solver"] in ["ecos", "scs", "clarabel"] and self.problem_data.get("b").size == 1:
        ...
        if self.solver_settings["solver"] in SPARSE_SOLVERS and self.solver_settings["sparse"]:
    ...
    def objective_value(self, x: np.ndarray | None = None, constant: bool | float = True) -> float:
        ...
        if isinstance(constant, bool):
            constant = 0 if self.problem_data.get("constant") is None else self.problem_data.get("constant").item()
        elif not isinstance(constant, (float, int)):
            msg = "constant must be a boolean, float, or int."
            raise ValueError(msg)
