...
class FactorCovEstimator(BaseCovEstimator):
    ...
    def _predict(self, prediction_data: PredictionData) -> pd.DataFrame:
        factor_cov = self.factor_cov_estimator.predict(prediction_data).to_numpy().astype(float)
        ...
        covmat = pd.DataFrame(covmat, index=self._available_assets, columns=self._available_assets)

        return covmat.astype(float)

    def predict_residual(self, prediction_data: PredictionData) -> pd.DataFrame:
        residual_cov = self.residual_cov_estimator.predict(prediction_data).to_numpy().astype(float)

        return pd.DataFrame(residual_cov, index=self._available_assets, columns=self._available_assets).astype(float)
