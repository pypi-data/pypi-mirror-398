...
def compute_pnl(positions, targets):
    ...
    if len(long_indices) > 0:
        total_long = np.sum(targets[long_indices])
        
    total_short = 0.0
    if len(short_indices) > 0:
        total_short = np.sum(targets[short_indices])
    
    total_positions = len(long_indices) + len(short_indices)
    
    if total_positions > 0:
        daily_pnl = (total_long - total_short) / total_positions
    else:
        daily_pnl = 0.0
        
    return daily_pnl

def gen_pnl(positions, targets):
    ...
    for pos, tgt in zip(positions, targets):
        ...
        pnl = compute_pnl(pos, tgt_tensor)
