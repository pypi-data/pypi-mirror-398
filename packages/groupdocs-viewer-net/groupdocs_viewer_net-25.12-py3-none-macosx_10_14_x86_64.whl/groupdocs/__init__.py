...
os.environ['NETCORE_WRAPPER_NETCORE_DIR_groupdocs'] = os.path.join(_pkg_dir, 'netcore')
    
os.environ['NETCORE_WRAPPER_ASSEMBLY_DIR_groupdocs'] = os.path.join(_pkg_dir, 'assemblies')
    
os.environ['WRAPPER_MODULE_DIR_groupdocs'] = _pkg_dir
...
@imp_error_t
class GroupDocsSubModuleLoader(impabc.Loader):
...
class GroupDocsMetaPathFinder(impabc.MetaPathFinder):
    _chk_re = re.compile(__package__+r'\.([^.]+)(\.)?')
    _submod_loader = GroupDocsSubModuleLoader()
...
sys.meta_path.append(GroupDocsMetaPathFinder())
