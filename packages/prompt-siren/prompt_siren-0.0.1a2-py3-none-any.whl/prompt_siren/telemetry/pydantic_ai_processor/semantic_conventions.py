...
class PydanticModelRequestParameters:
    TOOLS = "output_tools"
    FUNCTION_TOOLS = "function_tools"  # pydantic-ai specific
    NAME = "name"
...
class PydanticModelRequestParametersTool:
    ...
    DESCRIPTION = "description"
    PARAMETERS = "parameters"
    PARAMETERS_JSON_SCHEMA = "parameters_json_schema"  # pydantic-ai specific
...
class PydanticAllMessagesEvents:
    ...
    ALL_MESSAGES = "pydantic_ai.all_messages"  # v2 AGENT spans


class PydanticMessagePartType:
    TEXT = "text"
    TOOL_CALL = "tool_call"
    TOOL_CALL_RESPONSE = "tool_call_response"


class PydanticMessageRole:
    USER = "user"
    ASSISTANT = "assistant"
    SYSTEM = "system"
    TOOL = "tool"
...
class GenAIMessageAttributes:
    INPUT_MESSAGES = "gen_ai.input.messages"
    OUTPUT_MESSAGES = "gen_ai.output.messages"


class MessagePartFields:
    ROLE = "role"
    PARTS = "parts"
    TYPE = "type"
    CONTENT = "content"
...
class InvocationParameterNames:
...
def _extract_common_attributes(
    ...
    if GenAIConversationID.CONVERSATION_ID in gen_ai_attrs:
        yield (
            ...
            gen_ai_attrs[GenAIConversationID.CONVERSATION_ID],
        )
...
def _extract_agent_attributes(
    ...
    if PydanticFinalResult.FINAL_RESULT in gen_ai_attrs:
        yield (
            ...
            gen_ai_attrs[PydanticFinalResult.FINAL_RESULT],
        )

    # Extract input value from pydantic_ai.all_messages (v2 AGENT spans)
    if PydanticAllMessagesEvents.ALL_MESSAGES in gen_ai_attrs:
        all_messages_str = gen_ai_attrs[PydanticAllMessagesEvents.ALL_MESSAGES]
            ...
            try:
                ...
                if isinstance(all_messages, list) and all_messages:
                    ...
                    for msg in all_messages:
                        if (
                            msg.get(MessagePartFields.ROLE) == PydanticMessageRole.USER
                            and MessagePartFields.PARTS in msg
                        ):
                            for part in msg[MessagePartFields.PARTS]:
                                if (
                                    isinstance(part, dict)
                                    and part.get(MessagePartFields.TYPE)
                                    == PydanticMessagePartType.TEXT
                                ):
                                    yield (
                                        SpanAttributes.INPUT_VALUE,
                                        part.get(MessagePartFields.CONTENT, ""),
                                    )
                                    break
                            break
...
def _extract_llm_attributes(
    ...
    if (
        GenAIMessageAttributes.INPUT_MESSAGES in gen_ai_attrs
        and OTELConventions.EVENTS not in gen_ai_attrs
    ):
        ...
        if events:
            ...
            for index, message in enumerate(input_messages):
                for key, value in message.items():
                    yield (
                        f"{SpanAttributes.LLM_INPUT_MESSAGES}.{index}.{key}",
                        value,
                    )
...
def _flatten_message(message: dict[str, Any]) -> dict[str, Any]:
    ...
    flattened = {k: v for k, v in message.items() if k != MessageAttributes.MESSAGE_TOOL_CALLS}
...
def _extract_tool_attributes(
    ...
    if GEN_AI_TOOL_NAME in gen_ai_attrs and gen_ai_attrs[GEN_AI_TOOL_NAME] is not None:
        yield SpanAttributes.TOOL_NAME, gen_ai_attrs[GEN_AI_TOOL_NAME]

    if (
        GEN_AI_TOOL_DESCRIPTION in gen_ai_attrs
        and gen_ai_attrs[GEN_AI_TOOL_DESCRIPTION] is not None
    ):
        yield (
            SpanAttributes.TOOL_DESCRIPTION,
            gen_ai_attrs[GEN_AI_TOOL_DESCRIPTION],
        )

    if GEN_AI_TOOL_CALL_ID in gen_ai_attrs and gen_ai_attrs[GEN_AI_TOOL_CALL_ID] is not None:
        yield ToolCallAttributes.TOOL_CALL_ID, gen_ai_attrs[GEN_AI_TOOL_CALL_ID]

    # Extract tool arguments as input.value
    if (
        PydanticToolAttributes.TOOL_ARGUMENTS in gen_ai_attrs
        and gen_ai_attrs[PydanticToolAttributes.TOOL_ARGUMENTS] is not None
    ):
        yield (
            SpanAttributes.INPUT_VALUE,
            gen_ai_attrs[PydanticToolAttributes.TOOL_ARGUMENTS],
        )

    # Extract tool response as output.value
    # Check both "tool_response" (from Pydantic AI) and our custom attribute name
    if "tool_response" in gen_ai_attrs and gen_ai_attrs["tool_response"] is not None:
        yield SpanAttributes.OUTPUT_VALUE, gen_ai_attrs["tool_response"]
    elif (
        PydanticToolAttributes.TOOL_RESPONSE in gen_ai_attrs
        and gen_ai_attrs[PydanticToolAttributes.TOOL_RESPONSE] is not None
    ):
        yield (
            SpanAttributes.OUTPUT_VALUE,
            gen_ai_attrs[PydanticToolAttributes.TOOL_RESPONSE],
        )

    # Also support direct input.value and output.value attributes
    if (
        PydanticToolAttributes.INPUT_VALUE in gen_ai_attrs
        and gen_ai_attrs[PydanticToolAttributes.INPUT_VALUE] is not None
    ):
        yield (
            SpanAttributes.INPUT_VALUE,
            gen_ai_attrs[PydanticToolAttributes.INPUT_VALUE],
        )

    if (
        PydanticToolAttributes.OUTPUT_VALUE in gen_ai_attrs
        and gen_ai_attrs[PydanticToolAttributes.OUTPUT_VALUE] is not None
    ):
        yield (
            SpanAttributes.OUTPUT_VALUE,
            gen_ai_attrs[PydanticToolAttributes.OUTPUT_VALUE],
        )

    # Extract tool parameters if present
    if (
        PydanticToolAttributes.TOOL_PARAMETERS in gen_ai_attrs
        and gen_ai_attrs[PydanticToolAttributes.TOOL_PARAMETERS] is not None
    ):
        yield (
            SpanAttributes.TOOL_PARAMETERS,
            gen_ai_attrs[PydanticToolAttributes.TOOL_PARAMETERS],
        )
            ...
            if event.get(OTELConventions.EVENT_NAME) == GenAIEventNames.TOOL_MESSAGE:
                if GenAIToolMessageBodyFields.ID in event:
                    yield (
                        ...
                        event[GenAIToolMessageBodyFields.ID],
                    )
...
def _extract_tools_attributes(
    ...
    try:
        ...
        # Check for both 'function_tools' (pydantic-ai) and 'output_tools' keys
        tools_list = None
        if PydanticModelRequestParameters.FUNCTION_TOOLS in params and isinstance(
            ...
            tools_list = params[PydanticModelRequestParameters.FUNCTION_TOOLS]
        elif PydanticModelRequestParameters.TOOLS in params and isinstance(
            params[PydanticModelRequestParameters.TOOLS], list
        ):
            tools_list = params[PydanticModelRequestParameters.TOOLS]
        ...
        if tools_list:
            for tool in tools_list:
                ...
                # Check for name
                if MessagePartFields.NAME in tool and tool[MessagePartFields.NAME] is not None:
                    tool_info[SpanAttributes.TOOL_NAME] = tool[MessagePartFields.NAME]
                elif (
                    PydanticModelRequestParametersTool.NAME in tool
                    and tool[PydanticModelRequestParametersTool.NAME] is not None
                ):
                    tool_info[SpanAttributes.TOOL_NAME] = tool[
                        PydanticModelRequestParametersTool.NAME
                    ]

                # Check for description - only add if not None
                if (
                    PydanticModelRequestParametersTool.DESCRIPTION in tool
                    and tool[PydanticModelRequestParametersTool.DESCRIPTION] is not None
                ):
                    tool_info[SpanAttributes.TOOL_DESCRIPTION] = tool[
                        PydanticModelRequestParametersTool.DESCRIPTION
                    ]

                # Check for parameters/parameters_json_schema
                if PydanticModelRequestParametersTool.PARAMETERS_JSON_SCHEMA in tool and isinstance(
                    tool[PydanticModelRequestParametersTool.PARAMETERS_JSON_SCHEMA],
                    dict,
                ):
                    tool_info[ToolAttributes.TOOL_JSON_SCHEMA] = safe_json_dumps(
                        tool[PydanticModelRequestParametersTool.PARAMETERS_JSON_SCHEMA]
                    )
                elif PydanticModelRequestParametersTool.PARAMETERS in tool and isinstance(
        ...
        logfire.debug(f"Error parsing model request parameters: {e}")
...
def _extract_invocation_parameters(
    ...
    param_keys = [
        (GEN_AI_REQUEST_TEMPERATURE, InvocationParameterNames.TEMPERATURE),
        (GEN_AI_REQUEST_TOP_P, InvocationParameterNames.TOP_P),
        (GEN_AI_REQUEST_TOP_K, InvocationParameterNames.TOP_K),
        (GEN_AI_REQUEST_MAX_TOKENS, InvocationParameterNames.MAX_TOKENS),
        (
            GEN_AI_REQUEST_FREQUENCY_PENALTY,
            InvocationParameterNames.FREQUENCY_PENALTY,
        ),
        (
            GEN_AI_REQUEST_PRESENCE_PENALTY,
            InvocationParameterNames.PRESENCE_PENALTY,
        ),
        (GEN_AI_REQUEST_STOP_SEQUENCES, InvocationParameterNames.STOP),
        (GEN_AI_REQUEST_SEED, InvocationParameterNames.SEED),
    ]
...
def _find_llm_input_value(input_messages: list[dict[str, Any]]) -> str | None:
...
def _find_llm_output_value(output_messages: list[dict[str, Any]]) -> str | None:
...
def _extract_from_gen_ai_messages(
    gen_ai_attrs: Mapping[str, Any],
) -> Iterator[tuple[str, Any]]:
