...
def iconolatry_parser():
    """CLI parser."""
    options = {}
    icon_parser = argparse.ArgumentParser(
        description=__summary__, epilog="version: " + __version__
    )
    icon_subparsers = icon_parser.add_subparsers(
        dest="mode", help="Select if you want to read or to write an `.ico` / `.cur`."
    )

    # Decode parser.
    dec_parser = icon_subparsers.add_parser(
        "decode", add_help=False, allow_abbrev=False
    )
    dec_parser.register("action", "extend", ExtendAction)
    dec_required = dec_parser.add_argument_group("required arguments")
    dec_required.add_argument(
        "-i",
        "--icocurs-paths",
        required=True,
        nargs="+",
...
class Check(object):
    ...
    def setup(self):
        ...
        if len(self.list_in) > len(self.list_out):
            ...
            self.list_out.extend(
                [self.default] * (len(self.list_in) - len(self.list_out))
            )
    ...
    def paths(self, msg):
        ...
        for indx, path in enumerate(self.list_out):
            if not isinstance(path, str):
                print_err(
                    "Input error: %s directory path '%s' not a string." % (msg, path)
                )
            else:
                if path == "":
                    ...
                    print_err(
                        "Input error: %s directory path '%s' not found." % (msg, path)
                    )
        ...
        for indx, frmt in enumerate(self.list_out):
            if not isinstance(frmt, str):
                ...
                if frmt == "":
                    ...
                    if (
                        self.default == ".png"
                        and frmt[1:].upper() not in Image.SAVE.keys()
                    ) or (self.default == ".ico" and frmt not in [".ico", ".cur"]):
                        print_err(
                            "Input error: %s format '%s' not recognized." % (msg, frmt)
                        )
...
class Decode(object):
    def __init__(
        ...
        self.want_save = (
            ...
            if all(
                x == []
                for x in [self.paths_image, self.names_image, self.formats_image]
            )
            else True
        )
        self.build()
                ...
                if isinstance(self.path_icocur, str):
                    if isfile(self.path_icocur):
                        ...
                        if isdir(self.path_icocur):
                            ...
                            self.all_icocur_readed.update(
                                {
                                    self.path_icocur: "Input error: file/directory not found."
                                }
                            )
                    ...
                    self.all_icocur_readed.update(
                        {
                            self.path_icocur: "Input error: neither a file/directory nor bytes."
                        }
                    )
        else:
    ...
    def extract(self, dataimage, offset):
        ...
        (
            ...
            biClrUsed,
            biClrImportant,
        ) = unpack_from("<3L2H6L", dataimage[0:40])

        biHeight = int(biHeight / 2.0)
    ...
    def from_icocur(self):
        ...
        if identf not in [1, 2]:
            self.all_icocur_readed.update(
                {self.path_icocur: "Icon/Cursor error: invalid `.ico` / `.cur`."}
            )
            return
        else:
        ...
        icondirentries = [
            unpack_from("<4B2H2L", self.data_icocur[6 + 16 * i : 22 + 16 * i])
        ...
        for cnt in range(count):
            ...
            (
                ...
                bColorCount,
                bReserved,
                wPlanes_or_wXHotSpot,
                wBitCount_or_wYHotSpot,
                dWBytesInRes,
                dWImageOffset,
            ) = icondirentries[cnt]
            ...
            icocurdata_with_header = self.data_icocur[
                dWImageOffset : dWImageOffset + dWBytesInRes
            ]
            ...
            if not png_flag:
                if bWidth >= 256 or bHeight >= 256:
                    add_warning(
                        icocur_readed,
                        cnt,
                        "Is a large uncompressed `bmp` ! Should be `png` format.",
                    )

                ## Get bmp parameters.
                self.extract(icocurdata_with_header, dWBytesInRes)
                ## Get mask and check it.
                self.parameters, chk = Mask().rebuild_AND_mask(
                    icocurdata_with_header, self.parameters, self.rebuild
                )
                if not chk:
                    add_warning(
                        icocur_readed,
                        cnt,
                        "Bad mask found ! Will display incorrectly in some places (Windows).",
                    )
                ...
                try:
                    ...
                    if identf == 1:
                        assert (wPlanes_or_wXHotSpot in [0, 1]) and (
                            self.parameters["planes"] == 1
                        ), "planes"
                        assert (wBitCount_or_wYHotSpot == 0) or (
                            wBitCount_or_wYHotSpot == self.parameters["bpp"]
                        ), "bits"
                    assert self.parameters["compress"] == 0, "compression"
                    if (self.parameters["size_img"] != 0) and (
                        self.parameters["size_img"]
                        != self.parameters["size_xor"] + self.parameters["size_and"]
                    ):
                        # it seems legal to put a wrong 'size_img' !
                        add_warning(icocur_readed, cnt, "Size image malformed value !")

                    assert (
                        (bColorCount == self.parameters["colors"])
                        or (
                            bColorCount == 0
                            and self.parameters["colors"] == 1 << wBitCount_or_wYHotSpot
                        )
                        or (
                            bColorCount == 0
                            and self.parameters["colors"] == 1 << self.parameters["bpp"]
                        )
                        or (
                            bColorCount == 1 << wBitCount_or_wYHotSpot
                            and self.parameters["colors"] == 0
                        )
                        or (
                            bColorCount == 1 << self.parameters["bpp"]
                            and self.parameters["colors"] == 0
                        )
                    ), "color count"
                except AssertionError as e:
                    icocur_readed.update(
                        {"image_%s" % cnt: "Image error: malformed %s." % e.args[0]}
                    )
                    continue

                try:
                    image = self.load()
                    icocur_readed["image_%s" % cnt].update(
                        {"im_obj": image, "depth": self.parameters["bpp"]}
                    )
                    if self.parameters["num_pal"] > 0:
                        icocur_readed["image_%s" % cnt].update(
                            {"num_pal": self.parameters["num_pal"]}
                        )
                except:
                    icocur_readed.update(
                        {"image_%s" % cnt: "Image error: image not supported."}
                    )
                    continue
                ...
                if image:
                    ...
                    try:
                        ...
                        if identf == 1:
                            ...
                            assert (wBitCount_or_wYHotSpot == 0) or (
                                wBitCount_or_wYHotSpot == bpp
                            ), "bits"
                            assert (bColorCount == 0) or (
                                bColorCount == 1 << wBitCount_or_wYHotSpot
                            ), "color count"
                        elif identf == 2:
                ...
                icocur_readed.update(
                    {"image_%s" % cnt: "Image error: neither `bmp` nor `png`."}
                )
                continue
            ...
            if identf == 2:
                icocur_readed["image_%s" % cnt].update(
                    {
                        "hotspot_x": wPlanes_or_wXHotSpot,
        ...
        if datasize != totalsize:
            self.all_icocur_readed.update(
                {
                    self.path_icocur: "Icon/Cursor error: invalid %s, unexpected EOF."
                    % typ[identf]
                }
            )
            return
            ...
            for indx, key in enumerate(result):
                ...
                if isinstance(subresult, dict):
                    ...
                    if "info" in subresult:
                        ...
                        inf = ", ".join(
                            "{} = {}".format(k, v) for k, v in subresult["info"].items()
                        )
                        self.print_std("info --> %s" % inf)

                    self.print_std(
                        "(width, height) = %s" % str(subresult["im_obj"].size)
                    )
                    ...
                    if "hotspot_x" in subresult:
                        # print `.cur` hotspots.
                        self.print_std(
                            "(hotspot_x, hotspot_y) = %s"
                            % str((subresult["hotspot_x"], subresult["hotspot_y"]))
                        )
                    # save.
                    if self.want_save or self.is_cli:
                        # define current path, name and format.
                        path, name, frmt = (
                            self.paths_image[self.index],
                        ...
                        couple = (path, name)
                        current_indx = (
                            indx + self.remind[couple] + 1
                            if couple in self.remind.keys()
                            else indx
                        )
                        # define current name with index.
                        current_name = (
                            name + "_" + str(current_indx)
                            if len(result) > 1 or couple in self.remind.keys()
                            else name
                        )
    ...
    def work(self, is_byte=False):
        ...
        if not is_byte:
            if self.path_icocur.lower().endswith(
                ".ico"
            ) or self.path_icocur.lower().endswith(".cur"):
...
class Mask(object):
    ...
    def check_AND_mask(self, width, height, xordata, anddata):
        ...
        for y in range(height):
            for x in range(width):
                ...
                mask = bool(
                    ord(bytes([anddata[y * andbytes + x // 8]])) & (1 << (7 - (x % 8)))
                )
                if mask:
                    if alpha > 0:
    ...
    def rebuild_AND_mask(self, dataimage, parameters, rebuild=False):
        ...
        if parameters["bpp"] != 32:
            ...
            if rebuild:
                parameters["and"] = self.compute_AND_mask(
                    parameters["width"], parameters["height"], parameters["xor"]
                )
                return parameters, check
            else:
                return parameters, self.check_AND_mask(
                    parameters["width"],
                    parameters["height"],
                    parameters["xor"],
                    parameters["and"],
...
class Encode(object):
    ...
    def add_name2path(self, name, frmt, indx):
        ...
        current_name = (
            name + "_" + str(current_indx) if couple in self.remind.keys() else name
        )
    ...
    def check_output(self):
        ...
        if not isinstance(self.type_resize, (tuple, str)):
            ...
            if isinstance(self.type_resize, tuple) and not (
                len(self.type_resize) == 2
                and all(
                    isinstance(tyr, int)
                    for tyr in [self.type_resize[0], self.type_resize[1]]
                )
                and self.type_resize[0] <= 256
                and self.type_resize[1] <= 256
            ):
                print_err("Input error: option `type_resize` tuple not proper defined.")
            elif isinstance(self.type_resize, str) and (
                self.type_resize not in ["up256_prop", "up256_no_prop", "square"]
            ):
                print_err(
                    "Input error: option `type_resize` unknown '%s' method."
                    % self.type_resize
                )
        ...
        for i, frmt in enumerate(self.formats_icocur):
            if isinstance(frmt, tuple):
                if frmt[0] == ".ico":
                    print_err(
                        "Input error: hotspot specification invalid for `.ico` conversion."
                    )
                if all(not isinstance(hot, int) for hot in frmt[1::]) or (
                    len(frmt[1::]) != 2
                ):
    ...
    def build(self):
        ...
        if self.paths_images and isinstance(self.paths_images, list):
            ...
            for indx, (path_image, self.path_icocur, name, frmt, hotspot) in enumerate(
                groups
            ):
                self.print_std("#" * 80)
                no_err, paths = True, []
                ...
                if isinstance(path_image, list):
                    if not path_image:
                        ...
                        for imapath in path_image:
                            if not isinstance(imapath, str):
                                no_err = False
                                message = (
                                    "Input error: image file/directory path '%s' not a string."
                                    % imapath
                                )
                            else:
                                if isfile(imapath):
                                    ...
                                    if isdir(imapath):
                                        paths.extend(
                                            [
                                                join(imapath, file)
                                                for file in listdir(imapath)
                                            ]
                                        )
                                    else:
                                        no_err = False
                                        message = (
                                            "Input error: file/directory '%s' not found."
                                            % imapath
                                        )
                        ...
                        if len(paths) > 1:
                            if frmt == ".cur":
                                if name != "":
                                    no_err = False
                                    message = (
                                        "Input error: can't create multi-size '.cur'."
                                    )
                                else:
                    ...
                    no_err = False
                    message = (
                        "Input error: image file/directory path/s not a list of lists."
                    )
    ...
    def extract(self, path):
        ...
        try:
            ...
            raise EncodeErr(
                code=1,
                msg="Image error: format '%s' not recognized or corrupted." % ext,
            )
        ...
        if coltyp == 4 and self.mode == "RGBA":
            # fix this PIL mode.
            self.mode, self.parameters["wBitCount"] = (
                "LA",
                int(self.parameters["wBitCount"] / 2),
            )
    ...
    def load(self, path_image):
        ...
        forced = True
        if (
            (self.mode == "LA" and self.parameters["wBitCount"] in [16, 32])
            or ("transparency" in image.info)
            or (self.mode == "RGBA" and self.parameters["wBitCount"] == 64)
        ):
            image = image.convert("RGBA")
            self.mode, self.parameters["wBitCount"], string_mode = (
                image.mode,
                32,
                "truecolor+alpha",
            )
        ...
        if forced:
            dizio = {"new_mode": string_mode, "new_depth": self.parameters["wBitCount"]}
        ...
        if self.mode == "I":
            self.mode = "L"
            table = [
                i / (2 ** int(self.parameters["wBitCount"] / 2))
                for i in range(2 ** self.parameters["wBitCount"])
            ]
        ...
        if self.mode in ["1", "L", "I"]:
            if self.parameters["wBitCount"] in [1, 8]:
                pad = calc_rowsize(
                    self.parameters["wBitCount"], self.parameters["bWidth"]
                )
            ...
            if not image.palette:
                if self.custom_palettes:
                    if isinstance(self.custom_palettes, dict):
                        try:
                            palvalues = self.custom_palettes[
                                (self.mode, self.parameters["wBitCount"])
                            ]
                        except:
                            print_err(
                                "Input error: option `custom_palettes` not proper defined."
                            )
                    else:
                        print_err(
                            "Input error: option `custom_palettes` not proper defined."
                    ...
                    is_fallback = True
                    fallback_palettes = {
                        ("1", 1): self.ico_palette_gpl(
                            join(working_path, "palettes/11.gpl")
                        ),
                        ("L", 2): self.ico_palette_gpl(
                            join(working_path, "palettes/L2.gpl")
                        ),
                        ("L", 4): self.ico_palette_gpl(
                            join(working_path, "palettes/L4.gpl")
                        ),
                        ("L", 8): self.ico_palette_gpl(
                            join(working_path, "palettes/L8.gpl")
                        ),
                        ("P", 1): self.ico_palette_gpl(
                            join(working_path, "palettes/P1.gpl")
                        ),
                        ("P", 2): self.ico_palette_gpl(
                            join(working_path, "palettes/P2.gpl")
                        ),
                        ("P", 4): self.ico_palette_gpl(
                            join(working_path, "palettes/P4.gpl")
                        ),
                        ("P", 8): self.ico_palette_gpl(
                            join(working_path, "palettes/P8.gpl")
                        ),
                    }
                    palvalues = fallback_palettes[
                        (self.mode, self.parameters["wBitCount"])
                    ]

                if isinstance(palvalues, list):
                    if all(
                        isinstance(pal, tuple)
                        and len(pal) == 3
                        and all(isinstance(num, int) for num in pal)
                        ...
                        if is_fallback:
                            ...
                            self.parameters["palette"] = bytes(
                                [
                                    elem
                                    for quad in [[pal] * 3 + [0] for pal in palvalues]
                                    for elem in quad
                                ]
                            )
                    else:
    ...
    def ico_resize(self, image, how="up256_prop", method=Image.LANCZOS):
        ...
        self.all_icocur_written[self.path_icocur][self.index].update(
            {"size": "%s x %s" % (old_w, old_h)}
        )
        ...
        if resized:
            ...
            self.all_icocur_written[self.path_icocur][self.index].update(
                {
                    "resize": "%s x %s"
                    % (self.parameters["bWidth"], self.parameters["bHeight"])
                }
            )
        return image
    ...
    def header_icondir(self):
        ...
        return pack(
            ...
            self.parameters["idType"],
            self.parameters["idCount"],
        )
    ...
    def header_bmpinfo(self):
        ...
        self.parameters["size_xor"] = (
            calc_rowsize(self.parameters["wBitCount"], self.parameters["bWidth"])
            * self.parameters["bHeight"]
        )
        self.parameters["size_and"] = (
            calc_masksize(self.parameters["bWidth"]) * self.parameters["bHeight"]
        )
    ...
    def to_icocur(self, path_image, hotspot):
        ...
        ## Write AND mask.
        if self.mode == "RGBA":
            icobytes += Mask().compute_AND_mask(
                self.parameters["bWidth"], self.parameters["bHeight"], xordata
            )
        else:
        ...
        if self.parameters["bWidth"] >= 256:
            self.parameters["bWidth"] = 0
        if self.parameters["bHeight"] >= 256:
            self.parameters["bHeight"] = 0

        ## Pack icondirentry header.
        icondirentry = pack(
            "4B2H2I",
            self.parameters["bWidth"],
            self.parameters["bHeight"],
            self.parameters["bColorCount"],
            self.parameters["bReserved"],
            (self.parameters["wPlanes"] if hotspot == "" else hotspot[0]),
            (self.parameters["wBitCount"] if hotspot == "" else hotspot[1]),
            self.parameters["dwBytesInRes"],
            dataoffset,
        )
    ...
    def printsave(self, how, header, data, hotspot):
        ...
        def printresult(indx):
            ...
            if hotspot != "":
                self.print_std(
                    "{:<30} {:>10} {:>10}".format("", "", "hotspot = %s" % str(hotspot))
                )
    ...
    def work(self, paths, name, frmt, hotspot):
        ...
        if name == "":
            ...
            self.parameters["dwImageOffset"] = calcsize("4B2H2I") * self.parameters[
                "idCount"
            ] + calcsize("HHH")
        ...
        for self.index, path_image in enumerate(paths):
            try:
                if how == "single":
                    ...
                    icocur_header, icocur_data = self.header_icondir(), b""
                    self.parameters["dwImageOffset"] = calcsize(
                        "4B2H2I"
                    ) * self.parameters["idCount"] + calcsize("HHH")
                    self.path_icocur = join(
                        path_temp, splitext(basename(path_image))[0] + frmt
                    )
