...
export interface NameOrObjectWithNameKey {
    name: string;
}
...
export function convertKeysToCamelCaseForObject(obj: object) {
    ...
    return mapKeys(newObj, (_v: any, k: any) => camelCase(k));
}
...
export function renameKeysForObject(
    ...
    if (isArray(o)) {
        return o.map((x: any) => renameKeysForObject(x, keysOriginal, keysRenamed));
    }
