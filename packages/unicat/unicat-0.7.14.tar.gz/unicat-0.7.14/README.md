...
module.logs: list(UnicatModuleLog)
```
...
```python
module_action.name: str
module_action.configuration: dict(str, Any)
```
...
```python
module_log.timestamp: timestamp  # 1610635123.351925
module_log.version: str  # in 1.2.3 format
module_log.action: str
module_log.configuration: dict(str, Any)
module_log.command: str
module_log.started_at: timestamp  # 1610635123.351925
module_log.ended_at: timestamp  # 1610635123.351925
module_log.duration: float # seconds
module_log.status: str
module_log.output: str
```
...
Fixed: UnicatModuleLog timestamp, started_at, and ended_at should be timestamps, not datetimes.

### v0.7.13

Expose the `created_on` and `updated_on` properties on records and assets.
