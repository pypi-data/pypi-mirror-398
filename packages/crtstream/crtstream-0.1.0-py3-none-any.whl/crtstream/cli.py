...
HEADERS = {"User-Agent": "crtstream/0.1.0"}

# ---------- Helpers ----------

def load_logs(logs_file: str | None):
    """
    Load CT logs from a local JSON file.
    Priority:
      1) --logs <file>
      2) ./logs.json (current directory)
    """

    path = logs_file or "logs.json"

    if not os.path.isfile(path):
        raise RuntimeError(
            f"CT logs file not found: {path}\n"
            "Provide one with --logs <file> or create ./logs.json"
        )
...
def extract_certificate_info(cert):
    ...
    return {
        ...
        "validity": {
            ...
            if cert.not_valid_before else None,
            "not_after": cert.not_valid_after.isoformat()
            if cert.not_valid_after else None,
        },
