...
def _filter_and_pivot_data(
    ...
    """
    # Start with a base filter for the year, which is always set.
    ...
    if config.end_date:
        ...
        # If only week/year is used (i.e., no explicit start/end dates),
        # apply the week filter unless config.week >= 52 (full year).
        if config.week < 52:
