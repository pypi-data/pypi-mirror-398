...
import module_with_deprecations
import pytest

from neuro_api._deprecate import (
    deprecated,
    deprecated_alias,
    deprecated_async_alias,
    warn_deprecated,
)
...
def test_warn_deprecated_formatting(
    recwarn_always: pytest.WarningsRecorder,
) -> None:
...
async def async_new_hotness() -> str:
    return "new hotness"


old_hotness = deprecated_alias("old_hotness", new_hotness, "1.23", issue=1)
async_old_hotness = deprecated_async_alias(
    "async_old_hotness",
    async_new_hotness,
    "1.23",
    issue=1,
...
@pytest.mark.trio
async def test_deprecated_async_alias(
    recwarn_always: pytest.WarningsRecorder,
) -> None:
    assert await async_old_hotness() == "new hotness"
    got = recwarn_always.pop(DeprecationWarning)
    assert isinstance(got.message, Warning)
    assert (
        "test_deprecate.async_old_hotness is deprecated" in got.message.args[0]
    )
    assert "1.23" in got.message.args[0]
    assert "test_deprecate.async_new_hotness instead" in got.message.args[0]
    assert "issues/1" in got.message.args[0]

    assert isinstance(async_old_hotness.__doc__, str)
    assert ".. deprecated:: 1.23" in async_old_hotness.__doc__
    assert (
        "test_deprecate.async_new_hotness instead" in async_old_hotness.__doc__
    )
    assert "issues/1>`__" in async_old_hotness.__doc__
...
def test_deprecated_alias_method(
    recwarn_always: pytest.WarningsRecorder,
) -> None:
    obj = Alias()
...
.. deprecated:: 2.1
   Use hi instead.
   For details, see `issue #1 <https://github.com/CoolCat467/Neuro-API/issues/1>`__.
...
def test_module_with_deprecations(
    recwarn_always: pytest.WarningsRecorder,
) -> None:
