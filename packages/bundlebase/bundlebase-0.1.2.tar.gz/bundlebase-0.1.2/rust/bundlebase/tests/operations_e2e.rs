...
async fn test_rename() -> Result<(), BundlebaseError> {
    ...
    assert_eq!(vec!["registration_dttm", "id", "new_name", "last_name", "email", "gender", "ip_address", "cc", "country", "birthdate", "salary", "title", "comments"],
               field_names(&bundle.schema().await?));

    bundle.rename_column("new_name", "newer_name").await?;

    assert_eq!(vec!["registration_dttm", "id", "newer_name", "last_name", "email", "gender", "ip_address", "cc", "country", "birthdate", "salary", "title", "comments"],
               field_names(&bundle.schema().await?));
    Ok(())
}
...
async fn test_rename_case_sensitive() -> Result<(), BundlebaseError> {
    ...
    assert_eq!(vec!["Index", "Customer Id", "First Name", "Last Name", "Company", "City", "Country", "Phone 1", "Phone 2", "email", "Subscription Date", "Website"],
               field_names(&bundle.schema().await?));

    bundle.rename_column("email", "e-mail").await?;

    assert_eq!(vec!["Index", "Customer Id", "First Name", "Last Name", "Company", "City", "Country", "Phone 1", "Phone 2", "e-mail", "Subscription Date", "Website"],
               field_names(&bundle.schema().await?));
    Ok(())
}
