...
impl FilterAnalyzer {
    ...
    fn analyze_expr(expr: &Expr) -> Result<IndexableFilter, BundlebaseError> {
        match expr {
            ...
            // Future: Handle range predicates
            // Expr::BinaryExpr with AND combining >= and <=
    ...
    // Future: implement extract_range() for BETWEEN and >= / <= combinations
}
...
mod tests {
    ...
    fn test_multiple_filters() {
        ...
        let filters = vec![
            ...
            col("age").gt(Expr::Literal(ScalarValue::Int64(Some(18)), None)), // Not supported yet
            col("status").eq(Expr::Literal(ScalarValue::Utf8(Some("active".to_string())), None)),
        ];

        let indexable = FilterAnalyzer::extract_indexable(&filters);

        // Should extract only the two equality predicates
