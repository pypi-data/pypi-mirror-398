...
class TestAsyncClientFinalCoverage:
    ...
    def test_init_with_empty_base_url(self):
        ...
        with pytest.raises(EmbeddingServiceConfigError, match="base_url must be provided"):
            EmbeddingServiceAsyncClient(base_url="")
...
class TestResponseParsersFinalCoverage:
    ...
    def test_extract_embeddings_new_format_missing_body(self):
        """Test extract_embeddings() with missing body field."""
        result = {
            "result": {
                "data": {
                    "results": [
                        {"embedding": [0.1, 0.2, 0.3]}  # Missing body
                    ]
                }
            }
        }
    ...
    def test_extract_embeddings_old_format_missing_body(self):
        """Test extract_embeddings() old format with missing body."""
        result = {
            "result": {
                "data": [
                    {"embedding": [0.1, 0.2, 0.3]}  # Missing body
...
class TestConfigPresetsFinalCoverage:
    ...
    def test_validate_client_config_missing_host(self):
        ...
        assert any("host" in str(err).lower() for err in errors)
    ...
    def test_validate_client_config_missing_port(self):
        ...
        assert any("port" in str(err).lower() for err in errors)
