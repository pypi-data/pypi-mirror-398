    ...
    def test_extract_chunks(self, base_client):
        ...
        result = {
            "result": {
                "data": {
                    "results": [
                        {"body": "test", "chunks": ["chunk1", "chunk2"]},
                    ]
                }
            }
        }
    ...
    def test_extract_tokens(self, base_client):
        ...
        result = {
            "result": {
                "data": {
                    "results": [
                        {"body": "test", "tokens": ["token1", "token2"]},
                    ]
                }
            }
        }
    ...
    def test_extract_bm25_tokens(self, base_client):
        ...
        result = {
            "result": {
                "data": {
                    "results": [
                        {"body": "test", "bm25_tokens": ["bm25_token1", "bm25_token2"]},
                    ]
                }
            }
        }
    ...
    def test_extract_texts(self):
        ...
        result = {
            "result": {
                "data": {
                    "results": [
                        {"body": "test1", "embedding": [0.1]},
                        {"body": "test2", "embedding": [0.2]},
                    ]
                }
            }
        }
    ...
    def test_extract_tokens(self):
        ...
        result = {
            "result": {
                "data": {
                    "results": [
                        {"body": "test", "tokens": ["token1", "token2"], "embedding": [0.1]},
                    ]
                }
            }
        }
    ...
    def test_extract_bm25_tokens(self):
        ...
        result = {
            "result": {
                "data": {
                    "results": [
                        {"body": "test", "bm25_tokens": ["bm25_token1", "bm25_token2"], "embedding": [0.1]},
                    ]
                }
            }
        }
...
class TestConfigPresetsCoverage:
    ...
    def test_create_http_config_for_class(self):
        ...
        config_dict = create_http_config_for_class(config)
        assert isinstance(config_dict, dict)
        # Check that it's HTTP config
        assert config_dict.get("server", {}).get("host") == "localhost"
    ...
    def test_create_http_token_config_for_class(self):
        ...
        config.set("server.port", 8001)
        config.set("auth.method", "api_key")
        config.set("auth.api_key.key", "test-token")
        
        config_dict = create_http_token_config_for_class(config)
        assert isinstance(config_dict, dict)
        assert config_dict.get("auth", {}).get("method") == "api_key"
    ...
    def test_create_https_config_for_class(self):
        ...
        config_dict = create_https_config_for_class(config)
        assert isinstance(config_dict, dict)
        assert config_dict.get("ssl", {}).get("enabled") is True
    ...
    def test_create_https_token_config_for_class(self):
        ...
        config_dict = create_https_token_config_for_class(config)
        assert isinstance(config_dict, dict)
        assert config_dict.get("ssl", {}).get("enabled") is True
    ...
    def test_create_mtls_config_for_class(self):
        ...
        config.set("ssl.cert_file", "/path/to/cert.pem")
        config.set("ssl.key_file", "/path/to/key.pem")
        config.set("ssl.ca_cert_file", "/path/to/ca.pem")
        
        config_dict = create_mtls_config_for_class(config)
        assert isinstance(config_dict, dict)
        assert config_dict.get("ssl", {}).get("enabled") is True
