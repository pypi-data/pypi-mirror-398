...
class UsageFlowMiddleware(BaseHTTPMiddleware):
    ...
    """
    def __init__(self, app: FastAPI, api_key: str, pool_size: int = 10, whitelist_routes: List[str] = None, tracklist_routes: List[str] = None):
        super().__init__(app)
    ...
    def _guess_ledger_id(self, request: Request) -> str:
        ...
        if policy and policy.identity_field and policy.identity_location:
            result = self._extract_identity_from_location(policy.identity_field, policy.identity_location, request, method, url)
            if result:
                return result
        ...
        for key in ["userId", "accountId"]:
            if value := request.query_params.get(key):
                return method + " " + url + " " + self.client.transform_to_ledger_id(value)

        # Check JSON body
        if hasattr(request.state, "body_data") and isinstance(request.state.body_data, dict):
            for key in ["userId", "accountId"]:
                if key in request.state.body_data:
                    return method + " " + url + " " + self.client.transform_to_ledger_id(request.state.body_data[key])
