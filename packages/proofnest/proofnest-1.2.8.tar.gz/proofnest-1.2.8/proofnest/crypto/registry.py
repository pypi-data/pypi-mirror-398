"""
ProofNest - Cryptographic Algorithm Registry

Implements algorithm agility for long-term durability.
Allows protocol-level algorithm migration without breaking changes.

Design rationale: Cryptographic agility enables smooth transitions
when algorithms become deprecated (per NIST recommendations).
"""
...
class CryptoRegistry:
    ...
    Design: Centralized algorithm registry with versioning enables
    smooth cryptographic transitions without breaking existing proofs.
    """
...
class MultiAlgorithmSignature:
    ...
    Design: M-of-N algorithm verification enables graceful migration
    when transitioning between cryptographic schemes.
...
def domain_separate(domain: str, data: bytes) -> bytes:
    ...
    Design: Domain separation prevents cross-protocol attacks
    (per NIST SP 800-185 customization recommendations).
