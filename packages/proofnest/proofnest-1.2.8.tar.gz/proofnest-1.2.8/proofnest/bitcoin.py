...
Methods:
1. OP_RETURN - Up to 80 bytes directly in blockchain (NOT IMPLEMENTED)
2. Merkle root - Summary of entire chain as single hash
3. OpenTimestamps - Free timestamping via Bitcoin (IMPLEMENTED)

OpenTimestamps Workflow:
========================
1. SUBMISSION: Hash is submitted to OTS calendar servers (~instant)
   - Returns an "incomplete" proof blob
   - Proof is valid but NOT yet on Bitcoin
   - This is what ProofNest stores immediately

2. UPGRADE (~2h): OTS server aggregates hashes into Bitcoin tx
   - Incomplete proof becomes "complete" with Bitcoin block data
   - Requires running `ots upgrade <file.ots>` or waiting for auto-upgrade

3. FULL VERIFY: Complete proof verified against Bitcoin blockchain
   - Requires `ots verify <file.ots>` with Bitcoin node access
   - Proves the hash existed BEFORE the Bitcoin block timestamp

ProofNest Implementation:
- anchor() = Step 1 (submission + format validation)
- verify_ots_full() = Step 3 (full verification, requires OTS CLI)
- Upgrade happens automatically on OTS servers
