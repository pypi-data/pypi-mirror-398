...
class ProofNest:
    ...
    def verify_storage(self, warn_no_anchor: bool = True, verify_signatures: bool = True) -> bool:
        """
        Verify chain integrity by RELOADING from storage.

        This method reloads the chain from disk before verification,
        ensuring that file-level tampering is detected. Use this instead
        of verify() when you need to validate against potential disk
        modifications that occurred after the instance was loaded.
        ...
        Returns:
            True if stored chain is valid, False if tampered

        Example:
            # Create fresh instance and verify storage
            pn = ProofNest(agent_id="my-agent", storage_path=path)
            assert pn.verify_storage()  # Reloads and verifies

            # Or for existing instance, this forces reload
            if not pn.verify_storage():
                raise SecurityError("Chain tampered on disk!")
        """
        # Create new instance to force reload from storage
        fresh = ProofNest(
            agent_id=self.actor.id,
            storage_path=self.storage_path.parent,
            agent_model=self.actor.model,
            enable_bitcoin=self._bitcoin_enabled,
            enable_signatures=self._enable_signatures,
        )
        return fresh.verify(warn_no_anchor=warn_no_anchor, verify_signatures=verify_signatures)
