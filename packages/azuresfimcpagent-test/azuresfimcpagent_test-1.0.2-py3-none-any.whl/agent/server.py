...
PIPELINE_TEMPLATE_MAP = {
    "codeql": "templates/CodeQL_Pipeline.yml",           # Dev/Non-Prod
    "codeql-1es": "templates/CodeQL_1ES_Pipeline.yml",  # Prod (1ES)
}
...
def _get_pipeline_template(pipeline_type: str) -> str:
    ...
    alternate_type = "codeql" if "1es" in pipeline_type else "codeql-1es"
...
# --- TOOLS ---
@mcp.tool()
def list_azure_permissions(user_principal_name: str = None, force_refresh: bool = True) -> str:
    """
...
@mcp.tool()
def list_fabric_permissions(user_principal_name: str = None) -> str:
    """
...
@mcp.tool()
def check_nsp_in_resource_group(resource_group: str) -> str:
    """
    ...
    # Check for "NSP FOUND: {name}" pattern from check-nsp.ps1
    ...
    # Check for "NSP '{name}' created" pattern
    ...
    # Check for "NSP NOT FOUND" pattern
...
@mcp.tool()
def attach_resource_to_nsp(resource_group: str, nsp_name: str, resource_id: str) -> str:
    """
...
@mcp.tool()
def check_log_analytics_in_resource_group(resource_group: str) -> str:
    """
    ...
    # Parse the result to extract workspace information
        ...
        for line in result.split('\n'):
            if '/workspaces/' in line.lower():
                ...
                if match:
                    ...
                    # Try to extract full resource ID
                    id_match = re.search(r'(/subscriptions/[^\s]+)', line)
                    if id_match:
                        workspace_ids.append(id_match.group(1))
        
        if len(workspace_names) > 1:
            return json.dumps({
                "count": len(workspace_names),
                "workspaces": [{"name": name, "id": wid} for name, wid in zip(workspace_names, workspace_ids)] if workspace_ids else [{"name": name} for name in workspace_names],
                "message": f"Found {len(workspace_names)} Log Analytics Workspace(s) in '{resource_group}'",
                "requires_selection": True,
                "prompt": "Multiple workspaces detected. Please specify which workspace to use."
            })
    ...
    return json.dumps({
        ...
        "workspaces": [],
        "raw_output": result,
        "message": "Could not parse Log Analytics Workspace information from output"
    })
...
def agent_dispatch(user_input: str) -> str:
    ...
    if intent == "create":
        return (
            ...
            "  NSP Management:\n"
            "    - check_nsp_in_resource_group() - Check for NSP (handles multiple NSPs)\n"
            "    - create_azure_resource('nsp', ...) - Create NSP if doesn't exist\n"
            "    - attach_resource_to_nsp() - Attach resource to NSP\n"
            "  Log Analytics Management:\n"
            "    - check_log_analytics_in_resource_group() - Check for workspace (handles multiple)\n"
            "    - create_azure_resource('log-analytics', ...) - Create workspace if doesn't exist\n"
            "    - attach_diagnostic_settings() - Configure diagnostic settings\n\n"
...
@mcp.tool()
def create_devops_project(organization: str = None, project_name: str = None, repo_name: str = None, description: str = None) -> str:
    """
...
@mcp.tool()
def create_devops_repo(organization: str = None, project_name: str = None, repo_name: str = None) -> str:
    """
...
@mcp.tool()
def list_devops_projects(organization: str = None) -> str:
    """
...
@mcp.tool()
def list_devops_repos(organization: str = None, project_name: str = None) -> str:
    """
...
@mcp.tool()
def create_devops_branch(organization: str = None, project_name: str = None, 
...
@mcp.tool()
def create_devops_pipeline(organization: str = None, project_name: str = None,
...
@mcp.tool()
def create_fabric_workspace(capacity_id: str = None, workspace_name: str = None, 
                           description: str = "") -> str:
    """
...
@mcp.tool()
def attach_fabric_workspace_to_git(workspace_id: str = None, organization: str = None,
