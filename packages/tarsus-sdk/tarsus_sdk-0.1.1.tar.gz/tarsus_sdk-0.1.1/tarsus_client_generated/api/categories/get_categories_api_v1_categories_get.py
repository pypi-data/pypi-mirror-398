...
) -> Any | HTTPValidationError | list[CategoryModel] | None:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for response_200_item_data in _response_200:
            response_200_item = CategoryModel.from_dict(response_200_item_data)
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | HTTPValidationError | list[CategoryModel]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Extracts unique categories from all products in the database.
    If id is provided, returns only that category (if it exists in products).
...
def sync(
    *,
    ...
    Returns:
        Any | HTTPValidationError | list[CategoryModel]
    """
