...
from raystack.utils.functional import Promise


class RaystackUnicodeDecodeError(UnicodeDecodeError):
    def __str__(self):
...
def force_str(s, encoding="utf-8", strings_only=False, errors="strict"):
    ...
    try:
        ...
        raise RaystackUnicodeDecodeError(*e.args) from None
    return s
...
def iri_to_uri(iri):
    ...
    Take an IRI (string or UTF-8 bytes, e.g. '/I ♥ Raystack/' or
    b'/I \xe2\x99\xa5 Raystack/') and return a string containing the encoded
    result with ASCII chars only (e.g. '/I%20%E2%99%A5%20Raystack/').
    """
...
def uri_to_iri(uri):
    ...
    Take an URI in ASCII bytes (e.g. '/I%20%E2%99%A5%20Raystack/') and return
    a string containing the encoded result (e.g. '/I%20♥%20Raystack/').
    """
    if uri is None:
        return uri
    uri = force_bytes(uri)
