...
from raystack.core.management import BaseCommand, CommandError
from raystack.utils.datastructures import OrderedSet
from raystack.utils.module_loading import import_string as import_dotted_path
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--command",
            help=(
                "Instead of opening an interactive shell, run a command as Raystack and "
                "exit."
            ),
        )
    ...
    def get_namespace(self, **options):
        ...
        try:
            ...
            if verbosity > 0:
                settings_env_var = os.getenv("RAYSTACK_SETTINGS_MODULE")
                self.stdout.write(
                    "Automatic imports are disabled since settings are not configured."
                    f"\nRAYSTACK_SETTINGS_MODULE value is {settings_env_var!r}.\n"
