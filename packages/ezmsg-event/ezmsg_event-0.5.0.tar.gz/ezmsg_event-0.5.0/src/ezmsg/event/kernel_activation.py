...
def exponential_activation(
    tau: float = 0.050,
    bin_duration: float = 0.020,
    aggregation: BinAggregation | str = BinAggregation.LAST,
    scale_by_value: bool = False,
    normalize: bool = True,
) -> BinnedKernelActivation:
    """
    Create exponential kernel activation processor.

    Args:
        tau: Time constant in seconds. Default 50ms.
        bin_duration: Output bin duration in seconds. Default 20ms.
        aggregation: Bin aggregation method. Default "last".
        scale_by_value: Weight events by value. Default False.
        normalize: Normalize kernel integral to 1. Default True.

    Returns:
        Configured BinnedKernelActivation instance.
    """
    if isinstance(aggregation, str):
        aggregation = BinAggregation(aggregation)
...
def alpha_activation(
    ...
    """
    Create alpha kernel activation processor.

    Args:
        tau: Time constant in seconds (peak time). Default 50ms.
        bin_duration: Output bin duration in seconds. Default 20ms.
        aggregation: Bin aggregation method. Default "last".
        scale_by_value: Weight events by value. Default False.
        normalize: Normalize kernel integral to 1. Default True.
...
def event_count(
    ...
    """
    Create simple event counter (no kernel, just count per bin).
