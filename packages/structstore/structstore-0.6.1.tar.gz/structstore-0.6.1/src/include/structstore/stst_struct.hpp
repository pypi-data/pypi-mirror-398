...
template<typename T>
class Struct : public StructBase, public FieldType<T> {
private:
    template<typename U>
    friend bool structstore::init_member_map();
    inline static MemberMap member_map;

protected:
    // constructor, assignment, destructor

    Struct() {
        if (!member_map.empty()) { member_map.finish_init(); }
    }
    ...
    inline void copy_from(const T& other) { member_map.copy_from(*this, other); }

public:
    inline static const MemberMap& get_member_map() { return member_map; }
