...
protected:
    ...
    friend class Struct;
    friend class py;
    friend int structstore::stst_fuzz(std::string line);
...
public:
    ...
    using CopyFn = std::function<void(void*, const void*)>;
...
private:
    ...
    static TypeInfo create_type_info(const std::string name) {
        ...
        ti.copy_fn = [](void* t, const void* other) { *(T*) t = *(const T*) other; };
        return ti;
    }
    ...
    static TypeInfo create_ptr_type_info(const std::string name) {
        ...
        ti.copy_fn = [](void*, const void*) {
            // do nothing, as the pointer might point to an address outside of this allocator
        };
        return ti;
    }
    ...
    static const TypeInfo& register_type_internal(const std::string& name) {
        ...
        auto [it, success] = get_type_hashes().insert({typeid(T), type_hash});
        if (!success) {
            STST_LOG_DEBUG() << "type '" << name << "' has already been registered";
            // only throw if a different type was provided
            if (it->second != type_hash) { throw already_registered_type_error(type_hash); }
        }
        TypeInfo type_info;
        if constexpr (std::is_void_v<T>) {
...
static const TypeInfo& register_type(const std::string& name) {
    static_assert(typing::is_field_type<T>);
    return typing::register_type<T>(name);
}
