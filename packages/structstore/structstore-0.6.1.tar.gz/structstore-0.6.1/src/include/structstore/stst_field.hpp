...
// todo: remove more "friend" occurrences
...
public:
    ...
    template<typename T>
    explicit FieldViewConst(const T& data, SharedAlloc& sh_alloc)
        : data{&data}, type_hash{typing::get_type_hash<T>()}, sh_alloc{sh_alloc} {
        static_assert(typing::is_field_type<T>);
    }

    explicit FieldViewConst(const void* data, type_hash_t type_hash, SharedAlloc& sh_alloc)
        : data{(void*) data}, type_hash{type_hash}, sh_alloc{sh_alloc} {}
    ...
    bool operator==(const FieldViewConst& other) const {
    ...
    const SharedAlloc& get_alloc() const { return sh_alloc; }
    SharedAlloc& get_alloc() { return sh_alloc; }
};

class FieldView : public FieldViewConst {
    friend class Field;
...
public:
    ...
    template<typename T>
    explicit FieldView(T& data, SharedAlloc& sh_alloc) : FieldViewConst{data, sh_alloc} {}

    explicit FieldView(void* data, type_hash_t type_hash, SharedAlloc& sh_alloc)
        : FieldViewConst{data, type_hash, sh_alloc} {}

    // query functions

    template<typename T>
    inline T& get() {
        return (T&) FieldViewConst::get<T>();
    }
...
protected:
    ...
    inline void assert_nonempty() const { view(nullptr_alloc).assert_nonempty(); }

    inline void assert_empty() const { view(nullptr_alloc).assert_empty(); }
    ...
    T& get_or_construct(SharedAlloc& sh_alloc, const FieldTypeBase* parent_field) {
        STST_LOG_DEBUG() << "Field::get_or_construct()";
        if (empty()) {
...
public:
    ...
    inline FieldViewConst view(SharedAlloc& sh_alloc) const {
        return FieldViewConst{data.get(), type_hash, sh_alloc};
    }

    inline FieldView view(SharedAlloc& sh_alloc) {
        return FieldView{data.get(), type_hash, sh_alloc};
    }

    inline void to_text(std::ostream& os) const { view(nullptr_alloc).to_text(os); }

    inline YAML::Node to_yaml() const { return view(nullptr_alloc).to_yaml(); }
    ...
    inline void check(const SharedAlloc& sh_alloc, const FieldTypeBase& parent_field) const {
        ...
        view(nullptr_alloc).check(sh_alloc, parent_field);
    }

    inline bool operator==(const Field& other) const {
        return view(nullptr_alloc) == other.view(nullptr_alloc);
    }
    ...
    inline const T& get() const {
        return view(nullptr_alloc).get<T>();
    }
    ...
    void construct(type_hash_t type_hash, SharedAlloc& sh_alloc, const FieldTypeBase* parent_field);
};
...
public:
    ...
    inline FieldAccess operator[](const char name[]) { return (*this)[std::string(name)]; }

    FieldAccess operator[](int idx);
    ...
    inline Field& get_field() { return field; }

    inline SharedAlloc& get_alloc() { return sh_alloc; }

    [[nodiscard]] inline type_hash_t get_type_hash() const { return field.get_type_hash(); }

    inline void clear() { field.clear(sh_alloc); }

    inline void construct(type_hash_t type_hash) {
        field.construct(type_hash, sh_alloc, parent_field);
    }
};
...
public:
    ...
    static void create_in_place(FieldRef* ref) {
        ...
        new (ref) FieldRef{*t, static_alloc, true};
    }
    ...
    static FieldRef create() {
        T* t = static_alloc.allocate<T>();
        StlAllocator<T>(static_alloc).construct(t);
        FieldRef ref{*t, static_alloc, true};
        return std::move(ref);
    }

    // construct with existing data
    FieldRef(T& data, SharedAlloc& sh_alloc, bool managed = false)
        : data{data}, sh_alloc{sh_alloc}, managed{managed} {}

    // move from other is valid
    FieldRef(FieldRef&& other) : FieldRef{other.data, other.sh_alloc, other.managed} {
        // we take ownership of the data; the other wrapper does not manage anymore
        other.managed = false;
    }

    // move from field is valid
    FieldRef(Field&& other, SharedAlloc& sh_alloc) : FieldRef{other.get<T>(), sh_alloc, true} {
    ...
    void check() & {
        ...
        void* sh_alloc_addr = &sh_alloc;
        stst_assert(sh_alloc_addr);
        if (managed) { stst_assert(sh_alloc.is_owned(&data)); }
        data.check();
    }
...
struct Unwrapper {
    ...
    T& t;
    SharedAlloc* sh_alloc;
    Unwrapper(W& w) : t{w}, sh_alloc{nullptr} {}
};
...
struct Unwrapper<FieldRef<T_>> {
    ...
    T& t;
    SharedAlloc* sh_alloc;
    Unwrapper(FieldRef<T>& w) : t{*w}, sh_alloc{&w.get_alloc()} {}
};

// specialization for Unwrapper<StructStore> in stst_structstore.hpp
...
template<typename W>
FieldViewConst unwrap_view(const W& w) {
    static_assert(std::is_same_v<W, std::remove_const_t<W>>);
    static_assert(std::is_same_v<W, std::remove_reference_t<W>>);
    Unwrapper u(w);
    return FieldViewConst{u.t, u.sh_alloc};
}

template<typename W>
FieldView unwrap_view(W& w) {
    static_assert(std::is_same_v<W, std::remove_const_t<W>>);
    static_assert(std::is_same_v<W, std::remove_reference_t<W>>);
    Unwrapper u(w);
    return FieldView{u.t, *u.sh_alloc};
}
...
template<typename T>
decltype(auto) ref_wrap(T& t, SharedAlloc& sh_alloc) {
    static_assert(std::is_same_v<T, std::remove_const_t<T>>);
    static_assert(std::is_same_v<T, std::remove_reference_t<T>>);
    return typename RefWrapper<T>::W(t, sh_alloc);
}

template<typename T>
decltype(auto) ref_wrap_view(FieldView view) {
    if constexpr (std::is_constructible_v<typename RefWrapper<T>::W, T&, SharedAlloc&>) {
        return typename RefWrapper<T>::W(view.get<T>(), view.get_alloc());
    } else {
        return typename RefWrapper<T>::W(view.get<T>());
    }
}
