...
public:
    ...
    using FromPythonFn = std::function<void(FieldView, const nb::handle&)>;
...
private:
    ...
    static nb::object get_entry(const MemberMap& member_map, StructBase& stru,
                                SharedAlloc& sh_alloc, const std::string& name);
    static nb::object get_entry(FieldMap& field_map, const std::string& name);
    // set Struct member or FieldMap field
    static void set_entry(const MemberMap& member_map, StructBase& stru, SharedAlloc& sh_alloc,
    ...
    static nb::object member_map_to_python(const MemberMap& member_map, StructBase& stru,
                                           SharedAlloc& sh_alloc, py::ToPythonMode mode);
    static void member_map_from_python(const MemberMap& member_map, StructBase& stru,
                                       SharedAlloc& sh_alloc, const nb::handle& value);
...
public:
    ...
    static void default_from_python_fn(FieldView view, const nb::handle& value) {
    ...
    static nb::object default_to_python_fn(FieldView view, ToPythonMode) {
        using T = unwrap_type_t<W>;
        return W_py(ref_wrap_view<T>(view));
    }

    template<typename W>
    static nb::object default_to_python_cast_fn(FieldView view) {
        using T = unwrap_type_t<W>;
        return nb::cast(ref_wrap_view<T>(view), nb::rv_policy::reference);
    }
    ...
    static void register_type(FromPythonFn from_python_fn, ToPythonFn to_python_fn,
        ...
        //     nb::handle type_handle = nanobind::type<T>();
        //     add_compatible_type<T>(type_handle);
        // }
    }
    ...
    // template<typename W>
    // static void register_compatible_type(nb::handle type_handle) {
    //     static_assert(std::is_same_v<W, std::remove_const_t<W>>);
    //     static_assert(std::is_same_v<W, std::remove_reference_t<W>>);
    //     // check that a wrapper type (or String) is provided:
    //     static_assert(std::is_same_v<std::remove_reference_t<wrap_type_w<W>>, W>);
    ...
    //     const type_hash_t type_hash = typing::get_type_hash<T>();
    //     std::string type_name = nb::repr(type_handle).c_str();
    //     bool inserted = get_typemap().insert({type_name, type_hash}).second;
    //     if (!inserted) {
    //         std::ostringstream str;
    //         str << "Python type already registered: " << type_name;
    //         throw std::runtime_error(str.str());
    //     }
    // }
    ...
    static void register_struct_ptr_type() {
        ...
        auto from_python_fn = [](FieldView view, const nb::handle& value) {
            if (!nb::isinstance<W>(value)) { throw_convert_error<T>(value); }
            W& w = nb::cast<W&>(value, false);
            T& t = unwrap(w);
            if (!view.get_alloc().is_owned(&t)) {
        ...
        py::ToPythonFn to_python_fn = [](FieldView view, py::ToPythonMode mode) {
            ...
            return member_map_to_python(T::get_member_map(), stru, view.get_alloc(), mode);
        };
        py::ToPythonCastFn to_python_cast_fn = [](FieldView view) {
            T* t_ptr = view.get<OffsetPtr<T>>().get();
            if (t_ptr == nullptr) { return nb::none(); }
            return nb::cast(ref_wrap(*t_ptr, view.get_alloc()), nb::rv_policy::reference);
        };
    ...
    static void register_struct_type(nb::class_<W>& cls) {
        ...
        auto from_python_fn = [](FieldView view, const nb::handle& value) {
            ...
            T& t = view.get<T>();
            member_map_from_python(T::get_member_map(), t, view.get_alloc(), dict);
        };
        ...
        cls.def("__getstate__", [](W& w) {
            ...
            nb::object obj = member_map_to_python(T::get_member_map(), unwrap(w), w.get_alloc(),
        ...
        cls.def("__dir__", [](W&) {
            nb::list slots;
            for (const MemberInfo& member_info: T::get_member_map().get_members()) {
        ...
        cls.def(
                ...
                [](W& w, const std::string& name) {
                    return get_entry(T::get_member_map(), unwrap(w), w.get_alloc(), name);
                },
                nb::arg("name"));
        ...
        cls.def(
                ...
                [](W& w, const std::string& name, const nb::handle& value) {
                    auto& stru = unwrap(w);
                    auto lock = stru.write_lock();
                    set_entry(T::get_member_map(), stru, w.get_alloc(), name, value);
                },
    ...
    static void register_complex_type_funcs(nb::class_<W>& cls) {
        using T = unwrap_type_t<W>;
        static_assert(!std::is_pointer_v<T>);
        cls.def("to_yaml", [](W& w) { return YAML::Dump(unwrap_view(w).to_yaml()); });
        cls.def("__repr__", [](W& w) {
            std::ostringstream str;
            unwrap_view(w).to_text(str);
            return str.str();
        });
        cls.def("copy",
                [](W& w) { return to_python(unwrap_view(w), ToPythonMode::NON_RECURSIVE); });
        cls.def("deepcopy",
                [](W& w) { return to_python(unwrap_view(w), ToPythonMode::RECURSIVE); });
        cls.def("__copy__",
                [](W& w) { return to_python(unwrap_view(w), ToPythonMode::NON_RECURSIVE); });
        cls.def("__deepcopy__", [](W& w, nb::handle&) {
            return to_python(unwrap_view(w), ToPythonMode::RECURSIVE);
        });
    ...
    static void register_field_map_funcs(nb::class_<W>& cls) {
        ...
        //         [](const T&) {
        //             // nb::object dict =
        //             //         field_map_to_python(t.field_map, py::ToPythonMode::NON_RECURSIVE);
        //             return nb::dict();
        //         },
        //         [](T&, nb::handle) {}, nb::rv_policy::automatic);
    ...
    static void from_python(FieldView view, const nb::handle& value, const std::string& field_name);
    ...
    // public:
    //     template<typename T>
    //     static void register_vector_funcs(nb::class_<structstore::vector<T>>& cls) {
    //         // todo: cls.def("lock", ...)
    ...
    //                     // todo: lock()
    //                     SharedAlloc& sh_alloc = vec.get_allocator().get_alloc();
    //                     T& t = *vec.insert(vec.begin() + index, T{});
    //                     from_python(*AccessView{t, sh_alloc}, value, std::to_string(index));
    //                 },
    ...
    //                 from_python(*AccessView{t, sh_alloc}, val, std::to_string(vec.size() - 1));
    //             }
    //         });
    //         cls.def(
    //                 "append",
    ...
    //                     STST_LOG_DEBUG() << 1.01;
    //                     SharedAlloc& sh_alloc = vec.get_allocator().get_alloc();
    //                     STST_LOG_DEBUG() << "got len " << vec.size();
    //                     if (vec.size() == 1) { STST_LOG_DEBUG() << "got elem at " << &vec.back(); }
    //                     STST_LOG_DEBUG() << 1.1;
    //                     vec.reserve(vec.size() + 1);
    //                     STST_LOG_DEBUG() << 1.15;
    //                     T& t = vec.emplace_back();
    //                     STST_LOG_DEBUG() << 1.2 << (&t == &vec.back());
    //                     STST_LOG_DEBUG() << 1.3;
    //                     from_python(*AccessView{t, sh_alloc}, value, std::to_string(vec.size() - 1));
    //                     STST_LOG_DEBUG() << 1.4;
    //                 },
    //                 nb::arg("value").none());
    //         cls.def("pop", [](structstore::vector<T>& vec, size_t index) {
    //             // todo: lock()
    //             auto ret = to_python(*FieldView{vec.at(index)}, ToPythonMode::RECURSIVE);
    ...
    //        index) {
    //            // todo: lock()
    //            STST_LOG_DEBUG() << "getting vec item of type " <<
    //            typing::get_type(vec[index].get_field().get_type_hash()).name
    //                     ;
