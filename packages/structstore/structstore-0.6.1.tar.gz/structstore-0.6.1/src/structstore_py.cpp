...
NB_MODULE(MODULE_NAME, m) {
    ...
    py::FromPythonFn field_map_from_python_fn = [](FieldView view, const nanobind::handle& value_) {
        nanobind::handle value = value_;
        if (py::copy_cast_from_python<FieldMap::Ref>(view, value)) { return; }
        FieldMap& field_map = view.get<FieldMap>();
        if (nb::hasattr(value, "__dict__")) { value = nb::dict(value.attr("__dict__")); }
        ...
        if (nb::isinstance<nb::dict>(value)) {
            ...
            for (const auto& [key, val]: dict) {
                ...
                py::from_python(field_map[key_str.c_str()], dict[key], key_str);
            }
            return;
        }
        if (nb::hasattr(value, "__slots__")) {
            auto slots = nb::iterable(value.attr("__slots__"));
            STST_LOG_DEBUG() << "copying __slots__ to " << &field_map;
            field_map.clear();
            for (const auto& key: slots) {
                std::string key_str = nb::cast<std::string>(key);
                py::from_python(field_map[key_str.c_str()], nb::getattr(value, key), key_str);
            }
            return;
        }
        py::throw_convert_error<FieldMap>(value);
    };
    py::ToPythonFn field_map_to_python_fn = [](FieldView view,
                                               py::ToPythonMode mode) -> nb::object {
        CallstackEntry entry{"field_map_to_python()"};
        auto& field_map = view.get<FieldMap>();
        auto obj = py::SimpleNamespace();
        for (shr_string_idx str_idx: field_map.get_slots()) {
            ...
            if (mode == py::ToPythonMode::RECURSIVE) {
                nb::setattr(
                        obj, key,
                        py::to_python(field_map.FieldMapBase::at(str_idx).view(view.get_alloc()),
                                      py::ToPythonMode::RECURSIVE));
            } else { // non-recursive convert
                nb::setattr(obj, key,
                            py::to_python_cast(
                                    field_map.FieldMapBase::at(str_idx).view(view.get_alloc())));
            }
        }
        return obj;
    };
    py::register_type<FieldMap::Ref>(field_map_from_python_fn, field_map_to_python_fn);
    ...
    shcls.def_prop_ro("fields",
                      [](StructStore& fields) { return ref_wrap(*fields, fields->get_alloc()); });
    ...
    py::register_type<structstore::String>(
            [](FieldView view, const nb::handle& value) {
    ...
    py::ToPythonFn list_to_python_fn = [](FieldView view, py::ToPythonMode mode) -> nb::object {
        ...
        auto ret = nb::list();
        for (FieldView view: list) {
            if (mode == py::ToPythonMode::RECURSIVE) {
                ret.append(py::to_python(view, py::ToPythonMode::RECURSIVE));
            } else { // non-recursive convert
                ret.append(py::to_python_cast(view));
            }
        }
        return ret;
    };
    py::FromPythonFn list_from_python_fn = [](FieldView view, const nanobind::handle& value) {
    ...
    list_cls.def("pop", [](List::Ref& list_ref, size_t index) {
        ...
        auto res = py::to_python((*list_ref)[index].get_field().view(list_ref.get_alloc()),
                                 py::ToPythonMode::RECURSIVE);
    ...
    list_cls.def("__add__", [=](List::Ref& list_ref, nb::list& value) {
        auto lock = list_ref->read_lock();
        // todo: this returns a recursive copy, is this wanted?
        return py::to_python(FieldView{*list_ref, list_ref.get_alloc()},
                             py::ToPythonMode::RECURSIVE) +
               value;
    });
    list_cls.def("__iadd__", [=](List::Ref& list_ref, nb::list& value) {
        ...
        return py::to_python_cast(FieldView{*list_ref, list_ref.get_alloc()});
    });
    list_cls.def(
            "__setitem__",
            [](List::Ref& list_ref, size_t index, nb::handle& value) {
    ...
    list_cls.def("__getitem__", [](List::Ref& list_ref, size_t index) {
        ...
        FieldView view = (*list_ref)[index].get_field().view(list_ref.get_alloc());
        return py::to_python_cast(view);
    });
    ...
    py::FromPythonFn matrix_from_python_fn = [](FieldView view, const nanobind::handle& value) {
...
template<>
struct type_caster<structstore::FieldView> {
public:
    NB_TYPE_CASTER(structstore::FieldView, const_name("FieldView"))

    static handle from_cpp(structstore::FieldView view, rv_policy, cleanup_list*) {
        return py::to_python_cast(view).release();
    }
};
}
