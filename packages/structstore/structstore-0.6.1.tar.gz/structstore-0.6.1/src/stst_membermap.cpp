...
FieldViewConst MemberInfo::get_view(const StructBase& self, SharedAlloc& sh_alloc) const {
    return FieldViewConst{(void*) get_ptr(self), type_hash, sh_alloc};
}

FieldView MemberInfo::get_view(StructBase& self, SharedAlloc& sh_alloc) const {
    return FieldView{get_ptr(self), type_hash, sh_alloc};
}
...
void MemberMap::store_ref(const std::string& name, ptrdiff_t member_offs, type_hash_t type_hash) {
    ...
    const MemberInfo member_info = {(int32_t) member_offs, type_hash, name};
    auto [it, inserted] = member_idx.emplace(name, members.size());
...
void MemberMap::copy_from(StructBase& self, const StructBase& other) const {
    ...
    for (const MemberInfo& member_info: members) {
        ...
        type_info.copy_fn(member_self, member_other);
    }
...
void MemberMap::check(const StructBase& self, const SharedAlloc* sh_alloc,
    ...
    for (const MemberInfo& member_info: members) {
        ...
        member_info.get_view(self, nullptr_alloc).check(*sh_alloc, parent_field);
    }
}
...
FieldViewConst MemberMap::get_member(const std::string& name, const StructBase& stru,
                                     SharedAlloc& sh_alloc) const {
    auto it = member_idx.find(name);
    if (it == member_idx.end()) { return FieldViewConst{nullptr, 0, sh_alloc}; }
    const MemberInfo& member_info = members.at(it->second);
    return member_info.get_view(stru, sh_alloc);
}

FieldView MemberMap::get_member(const std::string& name, StructBase& stru,
                                SharedAlloc& sh_alloc) const {
    auto it = member_idx.find(name);
    if (it == member_idx.end()) { return FieldView{nullptr, 0, sh_alloc}; }
    const MemberInfo& member_info = members.at(it->second);
    return member_info.get_view(stru, sh_alloc);
}
