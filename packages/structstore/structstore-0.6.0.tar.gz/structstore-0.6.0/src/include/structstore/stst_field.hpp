...
public:
    ...
    template<typename T>
    explicit FieldViewConst(const T& data) : data{&data}, type_hash{typing::get_type_hash<T>()} {
        static_assert(typing::is_field_type<T>);
    }

    explicit FieldViewConst(const void* data, type_hash_t type_hash)
        : data{(void*) data}, type_hash{type_hash} {}
...
public:
    ...
    template<typename T>
    explicit FieldView(T& data) : FieldViewConst{data} {}

    explicit FieldView(void* data, type_hash_t type_hash) : FieldViewConst{data, type_hash} {}
...
public:
    ...
    inline FieldViewConst view() const { return FieldViewConst{data.get(), type_hash}; }

    inline FieldView view() { return FieldView{data.get(), type_hash}; }
