...
__attribute__((__visibility__("default"))) nb::object py::entries_to_python(FieldMap& field_map,
    ...
    for (shr_string_idx str_idx: field_map.get_slots()) {
        ...
        if (mode == py::ToPythonMode::RECURSIVE) {
            nb::setattr(obj, key,
                        py::to_python(field_map.FieldMapBase::at(str_idx).view(),
                                      py::ToPythonMode::RECURSIVE));
        } else { // non-recursive convert
            nb::setattr(obj, key, py::to_python_cast(field_map.FieldMapBase::at(str_idx).view()));
        }
    }
    return obj;
}

__attribute__((__visibility__("default"))) nb::object py::entries_to_python(StructBase& stru,
                                                                            py::ToPythonMode mode) {
    CallstackEntry entry{"py::entries_to_python()"};
    auto obj = SimpleNamespace();
    const MemberMap& member_map = stru.get_member_map();
    for (const MemberInfo& member_info: member_map.get_members()) {
        auto key = nb::str(member_info.name.c_str());
        if (mode == py::ToPythonMode::RECURSIVE) {
            nb::setattr(obj, key,
                        py::to_python(member_info.get_view(stru), py::ToPythonMode::RECURSIVE));
        } else { // non-recursive convert
            nb::setattr(obj, key, py::to_python_cast(member_info.get_view(stru)));
        }
    }
    return obj;
}

__attribute__((__visibility__("default"))) void py::entries_from_python(StructBase& stru,
    ...
    for (const MemberInfo& member_info: member_map.get_members()) {
        ...
        py::set_entry(stru, key_str, dict[key_str]);
    }
}

__attribute__((__visibility__("default"))) void py::entries_from_python(FieldMap& field_map,
                                                                        const nb::handle& value) {
    // todo: strict check if this is one of dict, SimpleNamespace, FieldMap
    if (nb::isinstance<nb::dict>(value)) {
        ...
        for (const auto& [key, val]: dict) {
            if (!nb::isinstance<nb::str>(key)) {
                ...
                msg << "Key '" << nb::cast<std::string>(nb::str(key)) << "' has unsupported type '"
                    << nb::cast<std::string>(nb::str(key.type()))
...
py::from_python(FieldView view, SharedAlloc& sh_alloc, const nb::handle& value, const std::string& field_name) {
    ...
    from_python_fn(view, sh_alloc, value);
}
...
py::from_python(FieldAccess access, const nb::handle& value, const std::string& field_name) {
    ...
    if (!access.get_field().empty()) {
        ...
        from_python_fn(access, access.get_alloc(), value);
    } else {
        ...
        py_type.from_python_fn(access, access.get_alloc(), value);
    }
}

__attribute__((__visibility__("default"))) nb::object py::get_entry(StructBase& stru,
                                                                    const std::string& name) {
    FieldView view = stru.get_member(name);
...
__attribute__((__visibility__("default"))) void
py::set_entry(StructBase& stru, const std::string& name, const nb::handle& value) {
    from_python(stru.get_member(name), stru.get_alloc(), value, name);
}
