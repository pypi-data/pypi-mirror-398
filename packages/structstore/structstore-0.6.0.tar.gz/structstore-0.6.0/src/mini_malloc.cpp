...
#define MAX_PTRDIFF_VAL ((1ll << 31) * 4 - 1)
#define MAX_NODE_SIZE (1ll << 31)
...
typedef struct mini_malloc {
    ...
    ptrdiff_type last_node_offs;
    size_t allocated;
} mini_malloc;
size_t mini_malloc_size = sizeof(mini_malloc);
...
static memnode* get_next_free_node(memnode* node) {
    // this check fails for pseudo nodes within free_nodes: assert(!is_allocated(node));
    if (node->d_next_free_node == 0) { return NULL; }
    return (memnode*) ((byte*) node + 4 * (ptrdiff_t) node->d_next_free_node);
}

static memnode* get_prev_free_node(memnode* node) {
    assert(!is_allocated(node));
    if (node->d_prev_free_node == 0) { return NULL; }
    return (memnode*) ((byte*) node + 4 * (ptrdiff_t) node->d_prev_free_node);
}
...
static void set_next_free_node(memnode* node, memnode* next_free_node) {
    ...
    assert(ptrdiff < MAX_PTRDIFF_VAL);
    assert(ptrdiff % 4 == 0);
    node->d_next_free_node = (ptrdiff_type) (ptrdiff / 4);
}
...
static void set_prev_free_node(memnode* node, memnode* prev_free_node) {
    ...
    assert(ptrdiff < MAX_PTRDIFF_VAL);
    assert(ptrdiff % 4 == 0);
    node->d_prev_free_node = (ptrdiff_type) (ptrdiff / 4);
}
...
static void set_prev_node_size(memnode* node, size_type prev_node_size) {
    assert((prev_node_size & ALLOCATED_FLAG) == 0);
    // keep allocated flag
    node->prev_node_size = prev_node_size | (node->prev_node_size & ALLOCATED_FLAG);
}
...
void structstore::init_mini_malloc(mini_malloc* mm, size_t blocksize) {
    ...
    last_node->size = 0;
    set_prev_node_size(last_node, block_node_size);
    assert(get_prev_node(last_node) == block_node);
    assert(get_next_node(block_node) == last_node);
    assert((byte*) last_node + ALLOC_NODE_SIZE == (byte*) buffer + blocksize);

    ptrdiff_t ptrdiff = (ptrdiff_t) last_node - (ptrdiff_t) mm;
    assert(ptrdiff > -MAX_PTRDIFF_VAL);
    assert(ptrdiff < MAX_PTRDIFF_VAL);
    assert(ptrdiff % 4 == 0);
    mm->last_node_offs = ptrdiff / 4;
...
void* structstore::mm_allocate(mini_malloc* mm, size_t size) {
    ...
    set_allocated(node);
    mm->allocated += node->size + ALLOC_NODE_SIZE;
...
static void join_with_next(mini_malloc* mm, memnode* node) {
    ...
    if (is_allocated(next_node)) { return; }
    if ((size_t) node->size + (size_t) next_node->size + ALLOC_NODE_SIZE > MAX_NODE_SIZE) {
        // resulting node would be too large
        return;
    }
...
static void mm_free_node(mini_malloc* mm, memnode* node) {
    assert(node->size % ALIGN == 0);
    size_index_type size_index = get_size_index_lower(node->size);
...
void structstore::mm_enlarge(mini_malloc* mm, size_t sizediff) {
    assert(sizediff < MAX_PTRDIFF_VAL);
    assert(sizediff % 4 == 0);
    size_t new_size = 4 * (size_t) mm->last_node_offs + sizediff;
    if (sizediff > MAX_NODE_SIZE) {
        throw std::runtime_error(
                "could not enlarge memory block: exceeded maximum size difference");
    }
    if (new_size >= MAX_PTRDIFF_VAL) {
        throw std::runtime_error("could not enlarge memory block: reached maximum size");
    }

    // retrieve where the current last_node is
    memnode* old_last_node = (memnode*) ((ptrdiff_t) mm + 4 * (ptrdiff_t) mm->last_node_offs);

    // create a new last_node
    mm->last_node_offs += sizediff / 4;
    memnode* last_node = (memnode*) ((ptrdiff_t) mm + 4 * (ptrdiff_t) mm->last_node_offs);
    set_allocated(last_node);
    last_node->size = 0;

    // correct links between nodes
    old_last_node->size = sizediff - ALLOC_NODE_SIZE;
    set_prev_node_size(last_node, old_last_node->size);
    assert(get_prev_node(last_node) == old_last_node);
    assert(get_next_node(old_last_node) == last_node);

    // put enlarged old last_node into the free node pool
    mm_free_node(mm, old_last_node);
}

size_t structstore::mm_get_allocated_size(const mini_malloc* mm) { return mm->allocated; }
