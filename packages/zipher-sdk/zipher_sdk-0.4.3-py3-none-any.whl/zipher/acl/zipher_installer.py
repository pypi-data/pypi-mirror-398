...
class ZipherInstaller:
    ...
    def _use_existing_service_principal(self, sp_id: str, zipher_group: Group) -> ServicePrincipal:
        ...
        Args:
            sp_id: Application ID (UUID) of the service principal
        """
        # Search by application_id (UUID)
        all_sps = list(self.dbx_client.service_principals.list())
        matching_sp = [sp for sp in all_sps if sp.application_id == sp_id]
        if not matching_sp:
            raise ValueError(f"Service principal with application_id '{sp_id}' not found")
        sp = matching_sp[0]
        
        print(f"\nUsing existing service principal: {sp.display_name} (id={sp.id}, application_id={sp.application_id})")
