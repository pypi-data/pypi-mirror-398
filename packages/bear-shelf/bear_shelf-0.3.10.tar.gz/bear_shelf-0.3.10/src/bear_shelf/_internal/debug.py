...
if TYPE_CHECKING:
    from bear_shelf._internal._info import METADATA, _get_package_info, _Package
else:
    METADATA, _get_package_info, _Package = lazy("bear_shelf._internal._info").to(
        "METADATA", "_get_package_info", "_Package"
    )
...
def _print_debug_info(to_print: bool = False, no_color: bool = False) -> str:
    """Print debug/environment information with minimal clean formatting.

    Args:
        to_print: Whether to print the output to stdout.
        no_color: Whether to disable color formatting.
    """
    ...
    for index, (section_name, items) in enumerate(sections):
        if items:
            string.write(f"[green]{section_name}[/]{NEWLINE}")
            for key, value in items:
                string.write(f"{INDENT}[red]{key}:[/] [green]{value}[/]{NEWLINE}")
            if index != len(sections) - 1:
                string.write(NEWLINE)
    output: str = string.getvalue()
    if to_print and output:
        console = Console(file=sys.stdout, force_terminal=not no_color, markup=True)
        console.print(output)
    string.close()
    return output.replace("[/]", "").replace("[green]", "").replace("[red]", "")


if __name__ == "__main__":
    print(_print_debug_info(to_print=False, no_color=True))
