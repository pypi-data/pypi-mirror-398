#!/usr/bin/env python3
"""Release script for resourcespace-cli.

Interactive script that:
1. Prompts for version bump type (patch/minor/major/custom)
2. Updates version in pyproject.toml and src/resourcespace_cli/__init__.py
...
def main() -> None:
    ...
    update_version_in_file(Path("src/resourcespace_cli/__init__.py"), current_version, new_version)

    # Step 2: Clean old builds
    print("\n🧹 Cleaning old builds...")
    for folder in ["dist", "build", "src/resourcespace_cli.egg-info"]:
        path = Path(folder)
        if path.exists():
            shutil.rmtree(path)
    ...
    print(f"   PyPI: https://pypi.org/project/resourcespace-cli/{new_version}/")
    print(f"   GitHub: https://github.com/TidalStudio/ResourceSpace-CLI/releases/tag/v{new_version}")
