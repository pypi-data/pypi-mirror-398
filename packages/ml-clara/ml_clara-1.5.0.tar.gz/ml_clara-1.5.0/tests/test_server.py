...
def test_openai_chat_completions_error_envelope_empty_messages(client):
    r = client.post("/v1/chat/completions", json={"model": "gpt2", "messages": []})
...
def test_openai_chat_completions_error_envelope_validation_error(client):
    r = client.post("/v1/chat/completions", json={"model": "gpt2", "messages": "oops"})
    assert r.status_code == 400
    data = r.json()
    assert "error" in data
