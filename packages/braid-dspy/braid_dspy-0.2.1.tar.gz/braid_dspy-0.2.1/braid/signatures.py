...
class BraidPlanSignature(dspy.Signature):
    ...
    problem = dspy.InputField(desc="The problem to solve, described clearly and completely")
...
class BraidExecuteSignature(dspy.Signature):
    ...
    problem = dspy.InputField(desc="The original problem to solve")

    grd = dspy.InputField(desc="The Guided Reasoning Diagram (Mermaid format) to follow")

    current_step = dspy.InputField(desc="The current step in the GRD that needs to be executed")

    previous_results = dspy.InputField(desc="Results from previous steps (if any)", default="")
...
class BraidReasoningSignature(dspy.Signature):
    ...
    grd = dspy.OutputField(desc="A Guided Reasoning Diagram in Mermaid flowchart format")
...
class BraidStepSignature(dspy.Signature):
    ...
    step_description = dspy.InputField(desc="Description of the step to execute")

    context = dspy.InputField(desc="Context from previous steps and the original problem")

    step_output = dspy.OutputField(desc="Output from executing this step")
