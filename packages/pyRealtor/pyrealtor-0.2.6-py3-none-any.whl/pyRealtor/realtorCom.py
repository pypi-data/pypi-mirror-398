...
class RealtorCom(Realtor):
    ...
    def set_geo_coordinate_boundry(self, geo_location_obj: GeoLocationService):
        self.retry_count = 0
        self.bbox_boundary = [
            [geo_location_obj.longitude_min, geo_location_obj.latitude_min],
            [geo_location_obj.longitude_max, geo_location_obj.latitude_min],
            [geo_location_obj.longitude_max, geo_location_obj.latitude_max],
            [geo_location_obj.longitude_min, geo_location_obj.latitude_max],
            [geo_location_obj.longitude_min, geo_location_obj.latitude_min]
        ]
    ...
    def set_transaction_type(self, transaction_type: str = 'for_sale'):
        ...
        if keep_col[0] not in self.report_obj.column_lst:
    ...
    def transform(self, df):
        if "Rent" in self.report_obj.column_lst:
    ...
    def search_houses(self, use_proxy = False):
        ...
        try:
            ...
            elif realtor_api_response.status_code == 500:
                while self.retry_count == 0:
                    self.retry_count += 1
                    self.search_api_body["variables"]["query"]["boundary"] = {
                        "type": "Polygon",
                        "coordinates": [self.bbox_boundary]
                    }

                    return self.search_houses(use_proxy=use_proxy)

                raise Exception(f"Received 500 Status Code from REALTOR.COM API.")
