...
class HousingCom(Realtor):
    ...
    def transform(self, df):
        ...
        if "Rent" in df.columns.values:
            df["House Category"] = df["House Category"].str.extract(
                r".*\sBHK\s([A-Za-z\s]*[[A-Za-z]$)", 
                expand=False
            )
            df['Rent'] = df['Rent'].apply(lambda price: ";".join(str(x) for x in price) if isinstance(price, list) else price)
            houses_df = df
