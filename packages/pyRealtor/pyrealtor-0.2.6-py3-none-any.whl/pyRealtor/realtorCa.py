...
class RealtorCa(Realtor):
    ...
    def transform(self, df):
        ...
        if "Address" in self.report_obj.column_lst:
            df["Postal Code"] = df['Address'].str.split(' ').str[-1].replace(' ', '').str.upper()
            
        return df
    
    def set_listing_id(self, listing_id: str):
        self.search_api_params["ReferenceNumber"] = listing_id
        self.search_api_params["IncludeTombstones"] = 1
        self.search_api_params["IncludePins"] = 1
        return self
