...
class ReportingService:  
    ...
    def get_summary(self, dataframe: pd.DataFrame, summary_col_name: str, metric:str = "mean"):
        if (dataframe[summary_col_name].dtype == 'object') and (dataframe[summary_col_name].str.contains(';').any()):
            dataframe[summary_col_name] = dataframe[summary_col_name].str.split(';')
            dataframe[summary_col_name] = dataframe[summary_col_name].apply(lambda row: [float(x) for x in row if x!=''])
            dataframe[summary_col_name] = dataframe[summary_col_name].apply(lambda row: sum(row)/len(row) if len(row)>0 else row)
        dataframe[summary_col_name] = pd.to_numeric(dataframe[summary_col_name])
        dataframe['Bedrooms'] = dataframe['Bedrooms'].fillna('').astype("string")
        ...
        if metric == "mean":
            ...
            summary_df = summary_df.median()
            new_col_name = "Median "
        else:
            raise ValueError(f"Expected metric values are mean / median, but received {metric}")

        summary_df = summary_df.rename(new_col_name+summary_col_name)
