...
## 3.23.0
### Fixed
- `create-from-search` takes the `file:` search term in consideration to select the file for a new task ([#64](https://codeberg.org/pter/pter/issues/64))
- `relaxed` mode still moves a task's priority from, for example, `(A)` to `pri:A` to remain compatible with other todo.txt applications
