...
class TestSorting(unittest.TestCase):
    ...
    def testCreatedSorting(self):
        tasks = [Task("2020-10-05 id:1"),
                 Task("2012-05-01 id:2"),
                 Task("id:3"),
                 Task("2012-05-02 id:4")]
        sortorder = utils.build_sort_order("created")
        tasks.sort(key=lambda t: utils.sort_fnc(t, sortorder))

        self.assertEqual(tasks[0].attr_id, ['3'])
        self.assertEqual(tasks[1].attr_id, ['2'])
        self.assertEqual(tasks[2].attr_id, ['4'])
        self.assertEqual(tasks[3].attr_id, ['1'])
