...
class OlostepHook(BaseHook):
    ...
    def answer(
        ...
        **kwargs: Any,
        ...
        The AI will perform web searches and browse pages to find the answer.
        Execution time is typically 3-30s depending on complexity.
        
        :param task: The question or research task to answer
        :param json_schema: Optional JSON schema for structured output
        :param kwargs: Additional API parameters
        :return: Answer with sources and optional structured data
        ...
        Example::
            ...
            answer = result.get("result", {}).get("markdown_content")
            
            # Structured output
            result = hook.answer(
                task="Find information about Tesla",
                json_schema={"ceo": "", "headquarters": "", "founded": ""}
            )
            data = result.get("result", {}).get("json_content")
        """
        payload: Dict[str, Any] = {
            "task": task,
        }
        ...
        if json_schema:
            payload["json_format"] = json_schema
        
        payload.update(kwargs)
        
        self.log.info(f"Searching web for: {task[:80]}...")
        result = self._make_request("POST", "/answers", payload, timeout=120)
