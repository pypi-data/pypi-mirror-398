...
class EmbeddingRWKV(nn.Module):
    def __init__(self, model_path: str):
        ...
        if self.has_vision:
            ...
            config = SiglipVisionConfig.from_pretrained(vision_tower_path)
            self.vit = SiglipVisionModel(config, attn_implementation="sdpa")
    ...
    def _prepare_multimodal_inputs(self, tokens: Sequence[int], image_embed: torch.Tensor):
        token_tensor = torch.as_tensor(tokens, device=self.device, dtype=torch.long)
        text_embeds = self.rwkv.z['emb.weight'][token_tensor]
        eos_mask = token_tensor == self.eos_token_id

        image_eos_emb = self.rwkv.z['emb.weight'][self.eos_token_id].unsqueeze(0)
        image_embed = image_embed.to(self.device)
        seq_embeds = torch.cat([text_embeds, image_embed, image_eos_emb], dim=0)

        image_eos_mask = torch.zeros(image_embed.size(0) + 1, device=self.device, dtype=torch.bool)
        image_eos_mask[-1] = True
        eos_mask = torch.cat([eos_mask, image_eos_mask], dim=0)
        return seq_embeds, eos_mask
    ...
    def forward(
        ...
        if images is not None:
            ...
            token_batch = tokens if is_batch else [tokens]
            if isinstance(images, torch.Tensor) and images.dim() == 3:
                images = images.unsqueeze(0)
            if images.shape[0] != len(token_batch):
                raise ValueError("Number of images must match number of token sequences.")

            image_embeds = self.encode_images(images.to(self.device))
            pooled_features = []
            for token_seq, image_embed in zip(token_batch, image_embeds):
                seq_embeds, eos_mask = self._prepare_multimodal_inputs(token_seq, image_embed)
                seq_state = self.generate_zero_state(1)
                outputs = self.rwkv.forward_one_batch_alt(seq_embeds.unsqueeze(0).to(self.dtype), seq_state, full_output=True)
                outputs = outputs.squeeze(0)
                pooled = self.eos_pool(outputs.unsqueeze(0), eos_mask.unsqueeze(0))
                pooled_features.append(pooled.squeeze(0))

            embedding = torch.stack(pooled_features, dim=0)
            if task_type is None:
                task_type = "[RETR]"
            embedding = self.head(embedding, task_type)
