...
class RWKV_x070(MyModule):
    ...
    def forward_batch(self, tokens, state, full_output=False): # will modify state in-place
        ...
        if full_output == False:
            out = torch.empty((bsz, self.n_embd), dtype=DTYPE, requires_grad=False, device="cuda")
        else:
            out = [torch.empty((0, self.n_embd), dtype=DTYPE, requires_grad=False, device="cuda") for _ in range(bsz)]
        while True:
    ...
    def forward_one(self, idx:int, state:List[torch.Tensor]):
        with torch.no_grad(): 
            ...
            # x = x @ z['head.weight'] # comment for embedding model
            return x
