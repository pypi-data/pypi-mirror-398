...
class ImageInput(FileInput):
    ...
    class Media:
        js = ("pbx_admin/js/image_input_widget.js",)
...
class RemovableImageInput(ImageInput):
    remove_input_cls: type[Input] | Callable[[], Input] = CheckboxInput
...
class PrettyRemovableImageInput(RemovableImageInput):
    ...
    remove_input_cls: type[Input] | Callable[[], Input] = HiddenInput
...
class JSONTextarea(Textarea):
    def __init__(self, attrs=None):
        default_attrs = {"data-type": "code", "data-mode": "application/json"}
        if attrs:
...
class DateTimePickerWidget(DateTimeInput):
    template_name = "pbx_admin/forms/widgets/datetimepicker_widget.html"

    def __init__(self, attrs=None, widget_format: str = "YYYY-MM-DD HH:mm"):
        self.widget_format = widget_format
        super().__init__(attrs)

    def get_context(self, name, value, attrs=None):
        context = super().get_context(name, value, attrs)
        context["widget"]["format"] = self.widget_format
        return context


class DateTimeRangePickerWidget(MultiWidget):
    template_name = "pbx_admin/forms/widgets/datetimerangepicker_widget.html"

    def __init__(self, attrs=None, date_format="YYYY-MM-DD HH:mm"):
        attrs = attrs or {}
        widgets = (
            DateTimePickerWidget(
                attrs={"placeholder": "From", **attrs}, widget_format=date_format
            ),
            DateTimePickerWidget(
                attrs={"placeholder": "To", **attrs}, widget_format=date_format
            ),
        )
...
class CheckboxWithLabel(CheckboxInput):
    template_name = "pbx_admin/forms/widgets/boolean_input_with_label.html"
