...
class LoginView(auth_views.LoginView):
    def get_context_data(self, **kwargs):
        ...
        force_oauth_login = getattr(settings, "FORCE_OAUTH_LOGIN", False)
        show_oauth_button = force_oauth_login or is_from_trusted_network(self.request)
        context.update(
            {
                "title": f"PBX2 {settings.PRINTBOX_SITE_NAME}",
                "show_password_form": not force_oauth_login,
                "show_oauth_button": show_oauth_button,
            }
        )
        return context
