...
class TestLocalFAISSProvider:
    ...
    async def test_delete_index(self):
        ...
        with patch('telegram_rag_bot.vectorstore.local_faiss.Path.exists', return_value=True):
            with patch('telegram_rag_bot.vectorstore.local_faiss.asyncio.to_thread') as mock_thread:
...
class TestOpenSearchProvider:
    ...
    async def test_similarity_search(self, mock_client_class):
        ...
        results = await provider.similarity_search(query_embedding, mode="it_support", k=3)
