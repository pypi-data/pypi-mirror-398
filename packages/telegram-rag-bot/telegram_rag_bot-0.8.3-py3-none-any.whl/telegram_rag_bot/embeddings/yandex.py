...
class YandexEmbeddingsProvider(EmbeddingsProvider):
    ...
    def __init__(self, config: dict):
        ...
        # Replace ${YANDEX_FOLDER_ID} placeholder if present
            ...
            self.model_uri = self.model_uri.replace("${YANDEX_FOLDER_ID}", self.folder_id)
        
        self.timeout = config.get("timeout_seconds", 10)
        self.batch_size = 1  # Yandex API accepts only 1 text per request
        
        self.embeddings_url = "https://llm.api.cloud.yandex.net/foundationModels/v1/textEmbedding"
        self.client = httpx.AsyncClient(timeout=self.timeout)
        
        logger.info(f"YandexEmbeddingsProvider initialized (model: {self.model_uri}, timeout: {self.timeout}s)")
    ...
    async def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        logger.info(f"Embedding {len(texts)} documents via Yandex AI Studio (sequential processing)")
