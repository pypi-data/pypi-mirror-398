...
class OpenSearchProvider(VectorStoreProvider):
    ...
    def __init__(self, config: dict, embeddings_provider):
        ...
        logger.info(f"OpenSearchProvider initialized (host: {host}:{port}, dimension: {self.dimension})")
    ...
    async def _ensure_index_exists(self, mode: str) -> None:
        ...
        if exists:
            ...
            actual_dim = mapping[index_name]["mappings"]["properties"]["embedding"]["dimension"]
            ...
            logger.info(f"✅ OpenSearch index '{index_name}' exists with dimension {actual_dim}")
        else:
            ...
            logger.info(f"✅ Created OpenSearch index '{index_name}' with dimension {self.dimension}")
    ...
    async def add_documents(
        ...
        for text, embedding, metadata in zip(texts, embeddings, metadatas):
            ...
            bulk_data.append({
                "text": text,
                "embedding": embedding,
                "metadata": metadata_with_mode
            })
        
        logger.info(f"Indexing {len(texts)} documents to OpenSearch index '{index_name}'")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.get("errors"):
                    ...
                    logger.error(f"Bulk insert had errors: {failed_items[:5]}")  # Log first 5
                    raise RuntimeError(f"Bulk insert failed for {len(failed_items)} items")
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Bulk insert attempt {attempt+1} failed, retrying: {e}")
                    await asyncio.sleep(0.5)
                else:
                    raise RuntimeError(f"❌ OpenSearch bulk insert failed after retries: {e}") from e
    ...
    async def similarity_search(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"OpenSearch search timeout, retry {attempt+1}/{max_retries}")
                    await asyncio.sleep(0.5)
                else:
                    raise RuntimeError(
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"OpenSearch search failed, retry {attempt+1}/{max_retries}: {e}")
                    await asyncio.sleep(0.5)
                else:
                    raise RuntimeError(
    ...
    async def delete_index(self, mode: str) -> None:
        ...
        if exists:
            ...
            logger.warning(f"⚠️ OpenSearch index '{index_name}' not found (already deleted?)")
...
class OpenSearchRetriever(BaseRetriever):
    ...
    def __init__(self, vectorstore_provider, embeddings_provider, mode: str, k: int = 3):
        """
