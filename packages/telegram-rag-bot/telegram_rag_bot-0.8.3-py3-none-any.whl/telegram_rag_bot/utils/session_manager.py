...
class SessionManager:
    ...
    def __init__(self, redis_url: Optional[str] = None, ttl_seconds: int = 86400):
        ...
        if redis_url:
            try:
                ...
                logger.warning(f"⚠️ Redis unavailable: {e}. Falling back to memory storage.")
                self.use_redis = False
        else:
    ...
    async def get_session(self, user_id: int) -> Dict[str, Any]:
        ...
        Returns:
            Session dict (default if not found)
        """
        ...
        if user_id in self.memory_store:
            ...
            if created_at:
                ...
                try:
                    ...
                    if datetime.now() - created_dt > timedelta(seconds=self.ttl_seconds):
