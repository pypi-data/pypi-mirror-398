...
from langchain_classic.chains.retrieval import create_retrieval_chain  # LangChain 1.x: using langchain-classic package
from langchain_classic.chains.combine_documents.stuff import create_stuff_documents_chain  # LangChain 1.x: using langchain-classic package
...
from telegram_rag_bot.vectorstore.cloud_opensearch import OpenSearchProvider, OpenSearchRetriever
...
class RAGChainFactory:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(f"✅ Embeddings provider created: {type(self.embeddings_provider).__name__}")
        ...
        try:
            ...
            logger.info(f"✅ Vector store provider created: {type(self.vectorstore_provider).__name__}")
        except Exception as e:
    ...
    async def rebuild_index(self, mode: str, faq_file: str) -> None:
        ...
        async with self.mode_locks[mode]:
            ...
            metadatas = [{"source": faq_file, "mode": mode, **doc.metadata} for doc in chunks]
            ...
            if isinstance(self.vectorstore_provider, LocalFAISSProvider):
                # FAISS: Use legacy method for backward compatibility
                from langchain_community.vectorstores import FAISS
                
                # Get HuggingFace embeddings for FAISS
                ...
                await asyncio.to_thread(
                    self.vectorstore_provider.save_index,
                    vectorstore,
                    mode
                )
                ...
                raise RuntimeError(f"Unknown vector store provider: {type(self.vectorstore_provider)}")
    ...
    def create_chain(self, mode: str) -> Any:
        ...
        logger.debug(f"Created prompt template for mode '{mode}'")
