...
class GigaChatEmbeddingsProvider(EmbeddingsProvider):
    ...
    def __init__(self, config: dict):
        ...
        logger.info(f"GigaChatEmbeddingsProvider initialized (model: {self.model}, batch_size: {self.batch_size})")
            ...
            try:
                ...
                if e.response.status_code == 429:  # Rate limit
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f"⏳ GigaChat rate limit hit (429), retrying in {delay}s... (attempt {attempt+1}/{max_retries})")
                    ...
                    raise RuntimeError(f"❌ GigaChat API error: {e.response.status_code} - {e.response.text}") from e
    ...
    async def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        logger.info(f"Embedding {len(texts)} documents via GigaChat (batch_size={self.batch_size})")
