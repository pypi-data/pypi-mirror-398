...
def send_grpc_request(address, method="GetSignal"):
    ...
    try:
        ...
        logger.debug("gRPC response: status={}, signal={}, message={}".format(
            response.status, response.signal, response.message))
        
        return response.status, response.signal, response.message, response.diagnosis_id, response.options, response.timestamp
        ...
        if logger:
            logger.debug("gRPC request error: {}".format(error_msg))
        ...
        for filename in os.listdir(signal_dir):
            ...
            if "-" in filename and filename.startswith(("SYSOM_JOB_SLOW_DIAGNOSIS_SIGNAL", 
                ...
                if not os.path.exists(file_path):
                    ...
                    report_diagnosis_completion(signal_type, diagnosis_id)
                    
    except Exception as e:
        logger.error("Error checking completed diagnostics: {}".format(str(e)))

def report_diagnosis_completion(signal_type, diagnosis_id):
    """向服务器报告诊断完成"""
    aimaster_addr = os.environ.get("AIMASTER_ADDR")
    if not aimaster_addr:
        logger.error("AIMASTER_ADDR not set, cannot report completion")
        return
        
    try:
        # 发送报告结果请求
        status, _, message, _, _, _ = send_grpc_request(aimaster_addr, "ReportResult")
        if status == "OK":
            logger.info("Successfully reported completion of diagnosis {}-{}".format(signal_type, diagnosis_id))
        else:
            logger.warning("Failed to report completion: status={}, message={}".format(status, message))
    except Exception as e:
        logger.error("Exception reporting completion: {}".format(str(e)))
# 在全局变量部分添加
class SignalState:
    ...
    while running:
        try:
            ...
            if current_signal_state == SignalState.IDLE:
                # 发送gRPC请求获取Sysom信号
                status, signal_type, message, diagnosis_id, options, timestamp = send_grpc_request(aimaster_addr)
                
                # 如果状态为OK且有信号，则处理信号
                if status == "OK" and signal_type:
                    logger.info("Received Sysom signal: {}, diagnosis_id: {}, message: {}".format(
                        signal_type, diagnosis_id, message))
                    ...
                    logger.debug("Received status: {}, signal: {}, message: {}".format(status, signal_type, message))
