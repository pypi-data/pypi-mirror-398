...
def api_fuzzer():
    ...
    try:
        ...
        logger.error(f"💥 Error in api_fuzzer endpoint: {e!s}")
        return jsonify({
...
def graphql_scanner():
    ...
    try:
        ...
        logger.error(f"💥 Error in graphql_scanner endpoint: {e!s}")
        return jsonify({
            "error": f"Server error: {e!s}",
        }), 500
...
def jwt_analyzer():
    ...
    try:
        ...
        result = execute_command(command)
        logger.info("📊 JWT analysis completed")
        return jsonify(result)
    except Exception as e:
        logger.error(f"💥 Error in jwt_analyzer endpoint: {e!s}")
        return jsonify({
            "error": f"Server error: {e!s}",
        }), 500
...
def api_schema_analyzer():
    ...
    try:
        ...
        logger.error(f"💥 Error in api_schema_analyzer endpoint: {e!s}")
        return jsonify({
            "error": f"Server error: {e!s}",
        }), 500
