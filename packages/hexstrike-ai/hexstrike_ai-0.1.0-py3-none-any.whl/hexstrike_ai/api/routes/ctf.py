...
def create_ctf_challenge_workflow():
    ...
    try:
        ...
        logger.error(f"💥 Error creating CTF workflow: {e!s}")
...
def auto_solve_ctf_challenge():
    ...
    try:
        ...
        logger.error(f"💥 Error in CTF auto-solve: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def create_ctf_team_strategy():
    ...
    try:
        ...
        logger.error(f"💥 Error creating CTF team strategy: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def suggest_ctf_tools():
    ...
    try:
        ...
        logger.error(f"💥 Error suggesting CTF tools: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def ctf_cryptography_solver():
    ...
    try:
        ...
        logger.error(f"💥 Error in CTF crypto solver: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def ctf_forensics_analyzer():
    ...
    try:
        ...
        try:
            # File command
            file_result = subprocess.run(["file", file_path], check=False, capture_output=True, text=True, timeout=30)
        ...
        # Metadata extraction
        try:
            exif_result = subprocess.run(["exiftool", file_path], check=False, capture_output=True, text=True, timeout=30)
        ...
        if extract_hidden:
            try:
                binwalk_result = subprocess.run(["binwalk", "-e", file_path], check=False, capture_output=True, text=True, timeout=60)
        ...
        if check_steganography:
            ...
            for tool in steg_tools:
                try:
                    if tool == "steghide":
                        steg_result = subprocess.run([tool, "info", file_path], check=False, capture_output=True, text=True, timeout=30)
                    elif tool == "zsteg":
                        steg_result = subprocess.run([tool, "-a", file_path], check=False, capture_output=True, text=True, timeout=30)
                    elif tool == "outguess":
                        steg_result = subprocess.run([tool, "-r", file_path, "/tmp/outguess_output"], check=False, capture_output=True, text=True, timeout=30)
        ...
        # Strings analysis
        try:
            strings_result = subprocess.run(["strings", file_path], check=False, capture_output=True, text=True, timeout=30)
        ...
        logger.error(f"💥 Error in CTF forensics analyzer: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def ctf_binary_analyzer():
    ...
    try:
        ...
        try:
            file_result = subprocess.run(["file", binary_path], check=False, capture_output=True, text=True, timeout=30)
            if file_result.returncode == 0:
                results["file_info"]["type"] = file_result.stdout.strip()
        ...
        if check_protections:
            try:
                checksec_result = subprocess.run(["checksec", "--file", binary_path], check=False, capture_output=True, text=True, timeout=30)
        ...
        # Strings analysis
        try:
            strings_result = subprocess.run(["strings", binary_path], check=False, capture_output=True, text=True, timeout=30)
            if strings_result.returncode == 0:
        ...
        if find_gadgets and analysis_depth in ["comprehensive", "deep"]:
            try:
                ropgadget_result = subprocess.run(["ROPgadget", "--binary", binary_path, "--only", "pop|ret"], check=False, capture_output=True, text=True, timeout=60)
                ...
                results["gadgets"] = [f"Error finding gadgets: {e!s}"]

        # Function analysis with objdump
        if analysis_depth in ["comprehensive", "deep"]:
            try:
                objdump_result = subprocess.run(["objdump", "-t", binary_path], check=False, capture_output=True, text=True, timeout=30)
                ...
                results["interesting_functions"] = [f"Error analyzing functions: {e!s}"]
        ...
        logger.error(f"💥 Error in CTF binary analyzer: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
