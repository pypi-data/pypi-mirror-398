...
def execute_command(command: str, use_cache: bool = True, cache_instance: HexStrikeCache | None = None) -> dict[str, Any]:
    """
...
def execute_command_with_recovery(tool_name: str, command: str, parameters: dict[str, Any] = None,
                                 ...
                                 cache_instance: HexStrikeCache | None = None,
                                 error_handler_instance: IntelligentErrorHandler | None = None,
                                 degradation_manager_instance: GracefulDegradation | None = None) -> dict[str, Any]:
    """
    ...
    while attempt_count < max_attempts:
        ...
        try:
            ...
            if recovery_strategy.action == RecoveryAction.RETRY_WITH_REDUCED_SCOPE:
            ...
            if recovery_strategy.action == RecoveryAction.SWITCH_TO_ALTERNATIVE_TOOL:
            ...
            import traceback
            last_error = e
            logger.error(f"💥 Unexpected error in recovery attempt {attempt_count}: {e!s}")
    ...
    return {
        "success": False,
        "error": f"All recovery attempts exhausted: {last_error!s}",
        "recovery_info": {
            "attempts_made": attempt_count,
            "recovery_applied": True,
            "recovery_history": recovery_history,
