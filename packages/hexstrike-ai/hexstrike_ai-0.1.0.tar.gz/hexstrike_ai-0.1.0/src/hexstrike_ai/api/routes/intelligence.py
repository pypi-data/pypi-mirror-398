...
def analyze_target():
    ...
    try:
        ...
        logger.error(f"💥 Error analyzing target: {e!s}")
...
def select_optimal_tools():
    ...
    try:
        ...
        logger.error(f"💥 Error selecting tools: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def optimize_tool_parameters():
    ...
    try:
        ...
        logger.error(f"💥 Error optimizing parameters: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def create_attack_chain():
    ...
    try:
        ...
        logger.error(f"💥 Error creating attack chain: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
    ...
    try:
        ...
        def execute_single_tool(tool_name, target, profile):
            ...
            try:
                ...
                logger.error(f"❌ Error executing {tool_name}: {e!s}")
                return {
                    "tool": tool_name,
                    "status": "failed",
                    "timestamp": datetime.now().isoformat(),
        ...
        logger.error(f"💥 Error in intelligent smart scan: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
...
def detect_technologies():
    ...
    try:
        ...
        logger.error(f"💥 Error detecting technologies: {e!s}")
        return jsonify({"error": f"Server error: {e!s}"}), 500
