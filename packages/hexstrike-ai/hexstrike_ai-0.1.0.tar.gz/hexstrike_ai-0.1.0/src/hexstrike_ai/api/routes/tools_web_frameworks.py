...
def http_framework_route():
    ...
    try:
        ...
        if action == "repeater":
            ...
            logger.info("HTTP Framework: Sending custom request")
            result = http_framework.send_custom_request(request_spec)
            logger.info("HTTP Framework: Custom request completed")
            return jsonify(result)
        ...
        logger.error(f"Error in http-framework endpoint: {e!s}")
        return jsonify({
...
def browser_agent_route():
    ...
    try:
        ...
        if action == "active-test":
            ...
            logger.info("Browser Agent: Running active tests")
            result = browser_agent.run_active_tests(page_info, payload)
            logger.info("Browser Agent: Active tests completed")
            return jsonify(result)

        # Close browser
        if action == "close":
            browser_agent.close_browser()
            logger.info("Browser Agent: Browser closed")
        ...
        logger.error(f"Error in browser-agent endpoint: {e!s}")
        return jsonify({
            "error": f"Server error: {e!s}",
        }), 500
...
def burpsuite_alternative():
    ...
    try:
        ...
        if mode == "repeater":
            ...
            logger.info("Burpsuite Alternative: Repeater mode")
            result = http_framework.send_custom_request(request_spec)
            logger.info("Burpsuite Alternative: Repeater completed")
            return jsonify(result)
        ...
        logger.error(f"Error in burpsuite-alternative endpoint: {e!s}")
        return jsonify({
            "error": f"Server error: {e!s}",
        }), 500
