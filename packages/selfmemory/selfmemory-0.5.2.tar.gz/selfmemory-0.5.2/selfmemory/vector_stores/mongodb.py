...
class MongoDB(VectorStoreBase):
    ...
    def insert(
        ...
        try:
            ...
            logger.info(
                f"Inserted {len(data)} documents into '{self.collection_name}'."
            )
        except PyMongoError as e:
            logger.error(f"Error inserting data: {e}")

    def search(
        self, query: str, vectors: list[float], limit=5, filters: dict | None = None
    ) -> list[OutputData]:
        """
        ...
        output = [
            OutputData(
                id=str(doc["_id"]), score=doc.get("score"), payload=doc.get("payload")
            )
            for doc in results
        ]
        return output
        ...
        if update_fields:
            try:
                ...
                if result.matched_count > 0:
                    ...
                    logger.warning(
                        f"No document found with ID '{vector_id}' to update."
                    )
            except PyMongoError as e:
    ...
    def get(self, vector_id: str) -> OutputData | None:
        ...
        try:
            ...
            if doc:
                ...
                return OutputData(
                    id=str(doc["_id"]), score=None, payload=doc.get("payload")
                )
    ...
    def list_cols(self) -> list[str]:
        ...
        try:
            ...
            logger.info(
                f"Listing collections in database '{self.db_name}': {collections}"
            )
            return collections
        except PyMongoError as e:
