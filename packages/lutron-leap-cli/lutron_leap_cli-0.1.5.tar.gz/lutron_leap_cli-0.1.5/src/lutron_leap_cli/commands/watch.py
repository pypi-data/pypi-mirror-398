...
class ButtonWatcher:
    ...
    async def _recall_scene(self, scene_name: str):
        """Recall a scene."""
        ...
        console.print(f"[green]✓[/green] Recalled scene '{scene_name}'")
    ...
    async def _toggle_scenes(self, button_id: int, scenes: list[str]):
        ...
        if scene_name == "__off__":
            # Special "off" state - would need to know what zones to turn off
            console.print("[dim]Toggle: off[/dim]")
            # TODO: Track which zones were affected and turn them off
        else:
            await self._recall_scene(scene_name)
...
def buttons_cmd(
    ...
    if formatter.json_output:
        ...
        table.add_column("Parent Device")
