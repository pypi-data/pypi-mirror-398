...
# Logger
logger = logging.getLogger("paperlib_mcp.graph_maintenance")
...
def register_graph_maintenance_tools(mcp: FastMCP) -> None:
    ...
    async def extract_graph_missing(
        ...
        try:
            ...
            await report(f"发现 {total_docs} 个未抽取文档，开始处理...", 0, total_docs)
            ...
            async def process_single_doc(doc_row):
                ...
                async with sem:
                    ...
                    # Pass ctx to share progress? 
                    # extract_graph_v1_run accepts ctx.
                    # But if we run multiple docs in parallel, inner progress will be confusing.
                    # Better to pass None to inner, and report doc-level progress here.
                    await extract_graph_v1_run(
                        ...
                        ctx=None, # Suppress inner progress
                    )
                    
                    completed_count += 1
                    await report(f"处理进度: {completed_count}/{total_docs} (Doc: {doc_id[:8]}...)", completed_count, total_docs)
            ...
            await report(f"批量抽取完成: 成功 {len(processed_doc_ids)}/{total_docs}", total_docs, total_docs)
    ...
    async def rebuild_communities(
        ...
        try:
            ...
            with get_db() as conn:
                # 清除旧结果
                await report(f"清除 {level} 级旧社区数据...", 0, 6)
                ...
                # 获取 Paper->Entity 关系
                await report("获取 Paper->Entity 关系...", 1, 6)
                relations = query_all(
                    """
                    SELECT 
                ...
                await report(f"分析 {len(relations)} 条关系...", 2, 6)
                ...
                # 构建边
                await report(f"构建共现图 ({len(valid_nodes)} 节点)...", 3, 6)
                ...
                # Leiden 聚类
                await report(f"执行 Leiden 聚类 (res={resolution})...", 4, 6)
                ...
                await report(f"社区构建完成: 发现 {len(communities_result)} 个社区", 6, 6)
