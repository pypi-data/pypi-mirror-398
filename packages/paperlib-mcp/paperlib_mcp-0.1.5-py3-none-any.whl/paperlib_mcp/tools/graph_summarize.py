...
# Logger
logger = logging.getLogger("paperlib_mcp.graph_summarize")
...
async def summarize_community_v1_run(
    ...
    try:
        ...
        await report(f"开始生成社区 {comm_id} 摘要...", 0, 4)
        ...
        # 获取 top entities
        await report("获取社区 Top Entities...", 2, 4)
        ...
        # 调用 LLM (Async)
        await report(f"LLM 生成摘要 (model={actual_llm_model})...", 3, 4)
        ...
        await report(f"社区 {comm_id} 摘要生成完成", 4, 4)
...
async def summarize_all_communities_run(
    ...
    await report(f"开始批量生成摘要: 共 {count} 个社区", 0, count)
    ...
    async def process_one(cid):
        ...
        async with sem:
            # Pass ctx=None to suppress inner progress reporting
            res = await summarize_community_v1_run(comm_id=cid, ctx=None)
            
            completed_count += 1
            await report(f"处理进度: {completed_count}/{count} (CID: {cid})", completed_count, count)
    ...
    await report(f"批量摘要完成: 成功 {success_count}, 失败 {failed_count}", count, count)
