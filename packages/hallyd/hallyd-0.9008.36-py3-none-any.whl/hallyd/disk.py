...
class Disk:
    ...
    def __init__(self, dev_path: _fs.TInputPath):
        self.__path = _fs.Path(dev_path)
...
class Partition:
    ...
    def __init__(self, path: _fs.TInputPath):
        self.__path = _fs.Path(path)
...
def enable_swap(partition_dev: _fs.TInputPath, *, try_enable_instantly: bool = True) -> None:
    partition = Partition(partition_dev)
    ...
    if partition.fstype != PartitionTypes.ENCRYPTED_SWAP:
        ...
        subprocess.call(
            (
                ...
                "--part-type",
                partition.disk.path,
                str(partition.part_no),
                PartitionTypes.ENCRYPTED_SWAP.gpt_uuid,
            )
        )
