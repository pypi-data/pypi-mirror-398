...
class KiroHttpClient:
    ...
    async def _get_client(self, timeout: float = 300) -> httpx.AsyncClient:
        ...
        if self.client is None or self.client.is_closed:
            # Используем отдельные таймауты для connect и read
            timeout_config = httpx.Timeout(
                connect=10.0,  # Быстрое подключение
                read=timeout,  # Основной таймаут для чтения
    ...
    async def request_with_retry(
        ...
        """
        # Для streaming используем увеличенный таймаут для чтения (чтобы избежать ReadTimeout
        # между chunks), но проверяем первый токен отдельно через asyncio.wait_for
        # Для обычных запросов - 300 секунд
        if stream:
            # Используем более длинный таймаут для чтения всего потока (5 минут)
            # Первый токен проверяется отдельно через asyncio.wait_for в streaming.py
            timeout = 300.0
