...
pub mod validation {
    ...
    pub fn validate_secret(secret: &[u8]) -> Result<(), MirageError> {
        if secret.len() < 32 {
    ...
    pub fn validate_amount(amount: u64) -> Result<(), MirageError> {
        if amount == 0 {
            return Err(MirageError::InvalidInput(
    ...
    pub fn validate_hash32(hash: &[u8], name: &str) -> Result<(), MirageError> {
        if hash.len() != 32 {
            return Err(MirageError::InvalidInput(
    ...
    pub fn validate_proof_size(proof: &[u8]) -> Result<(), MirageError> {
