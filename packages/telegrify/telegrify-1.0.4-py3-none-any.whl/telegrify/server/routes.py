...
from telegrify.core.config import AppConfig, ButtonConfig, EndpointConfig
    ...
    for row in buttons:
        ...
        for btn in row:
            ...
            if btn.url:
                ...
                callback = (
                    Template(btn.callback_data).render(**payload) if payload else btn.callback_data
                )
...
def setup_webhook_handler(app: FastAPI, bot: TelegramBot, config: AppConfig) -> None:
        ...
        try:
            ...
            # Handle messages (including commands)
                ...
                if text.startswith("/"):
                    ...
                    for cmd_handler in config.commands:
                        if cmd_handler.command == command:
                            ...
                            response_text = (
                                Template(cmd_handler.response).render(**context)
                                if cmd_handler.response
                                else None
                            )

                            if response_text:
                                reply_markup = (
                                    build_inline_keyboard(cmd_handler.buttons, context)
                                    if cmd_handler.buttons
                                    else None
                                )
                                await bot.send_message(
                                    chat_id=chat_id,
                                    text=response_text,
                                    parse_mode=cmd_handler.parse_mode,
                                    reply_markup=reply_markup,
                                )
                            break
