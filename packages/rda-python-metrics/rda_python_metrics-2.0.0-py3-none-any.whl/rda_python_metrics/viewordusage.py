...
#      Date : 03/15/2022
#             2025-12-19 convert to class ViewORDUsage
...
class ViewORDUsage(PgView):
   ...
   def __init__(self):
      ...
      # keys self.FLDS - short field names
      ...
      # SHRTNM COLUMNNANE      FIELDNAME                              CNDNAME        TBLNAM      Size Prc Grp/Sum
         ...
         'M' : ['MONTH',        self.fmtym("date_request"),           'date_request','ousage',   7,  0,  'G'],
         ...
         'Y' : ['YEAR',         self.fmtyr("date_request"),           'date_request','ousage',   4,  0,  'G'],
   ...
   def check_enough_options(self):
      ...
      if 'E' in self.params:
         if 'z' in self.params:
            self.pglog(self.pgname + ": option -z and -E can not be present at the same time", self.LGWNEX)
   ...
   def build_query_strings(self, usgtable):
      ...
      if 'U' in self.params:    # reset units for file and read sizes
         if cols.find('B') > -1: self.FLDS['B'] = self.set_data_unit(self.FLDS['B'], self.params['U'][0], "sum(size_request)")
         if cols.find('F') > -1: self.FLDS['F'] = self.set_data_unit(self.FLDS['F'], self.params['U'][0], "sum(size_input)")
