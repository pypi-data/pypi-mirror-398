...
def test_decaes_parity_fixed_flip_no_reg() -> None:
    ...
    t2times = _load_csv_1d("tests/data/decaes_ref_t2times.csv")
    echotimes = _load_csv_1d("tests/data/decaes_ref_echotimes.csv")
    alpha_ref = float(_load_csv_1d("tests/data/decaes_ref_alpha.csv")[0])
    dist_ref = _load_csv_1d("tests/data/decaes_ref_dist.csv")

    m = DecaesT2Map(
        n_te=len(echotimes),
        te_s=float(echotimes[0]),
        n_t2=len(t2times),
        t2_range_s=(float(t2times[0]), float(t2times[-1])),
        reg="none",
    ...
    assert np.allclose(out["t2times_s"], t2times, rtol=0, atol=1e-12)
