...
class MemoryMonitor:
    ...
    def _get_memory_recommendations(self) -> List[str]:
        ...
        if self.stats.pressure_level == "critical":
            recommendations.append("Critical memory pressure. Force garbage collection or restart.")
        if len(self.tracked_objects) > 1000:
            recommendations.append("Many objects are being tracked. Consider reducing provider caching.")
...
class CacheManager:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        active_entries = [entry for entry in self.cache.values() if not entry.is_expired()]
...
class ProviderManager:
    ...
    def __init__(self, cache_manager: Optional[CacheManager] = None, memory_monitor: Optional[MemoryMonitor] = None):
    ...
    def register_provider(self, name: str, factory: Callable[[], Any], dispose_callback: Optional[Callable[[Any], None]] = None, cache_ttl: int = 300) -> None:
    ...
    def get_provider(self, name: str, use_cache: bool = True) -> Any:
        ...
        try:
            ...
            if self.memory_monitor:
                ...
                self.access_stats[name]["memory_usage"] = self.memory_monitor.stats.heap_used

            # Cache if enabled
            if use_cache and self.cache_manager:
                self.cache_manager.set(f"provider_{name}", provider, self.access_stats[name]["cache_ttl"])
    ...
    def cleanup_expired_providers(self, max_age_seconds: int = 3600) -> int:
        ...
        for name, stats in self.access_stats.items():
            if stats["last_accessed"] and (now - stats["last_accessed"]).total_seconds() > max_age_seconds:
...
_memory_monitor.on_memory_pressure(lambda stats: _provider_manager.cleanup_expired_providers(300))
...
def get_memory_report() -> Dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "loaded_providers": sum(1 for p in provider_stats.values() if p["is_loaded"])
        }
    }


# Convenience functions for common operations
def cached_operation(cache_key: str, operation: Callable[[], Any], ttl: int = 300) -> Any:
