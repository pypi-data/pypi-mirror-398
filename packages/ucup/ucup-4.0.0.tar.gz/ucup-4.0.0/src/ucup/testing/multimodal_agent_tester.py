...
class MultimodalAgentTester(AgentTestSuite):
    ...
    async def test_multimodal_agent(self, agent: Any,
                                    test_scenarios: List[str]) -> Dict[str, TestResult]:
        """
            ...
            for modality, content in test_case.items():
                if modality != 'semantic':
                    ...
                    if modality == 'text':
                        ...
                        inputs.text_content = content  # Fallback if image generation not available
        ...
        for config in test_configs:
            ...
            try:
                ...
                if hasattr(agent, 'analyze_multimodal_input'):
                    ...
                    fallback_quality = self._evaluate_fallback_quality(response, fused_analysis)
    ...
    def _compare_key_elements(self, elements1: List[str], elements2: List[str]) -> float:
    ...
    def _evaluate_fallback_quality(self, response: Any, fused_analysis: FusedAnalysis) -> float:
        ...
        if 'missing' in response_str or 'limited' in response_str or 'partial' in response_str:
