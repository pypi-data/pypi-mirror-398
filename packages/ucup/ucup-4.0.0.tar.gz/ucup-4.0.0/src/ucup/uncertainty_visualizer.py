...
from .validation import (
    validate_types, UCUPValidationError, UCUPValueError,
    validate_non_empty_string, create_error_message
)
...
class UncertaintyVisualizer:
    ...
    def create_confidence_distribution_plot(
        ...
        if not PLOTLY_AVAILABLE:
            return self._create_text_based_visualization(confidence_scores, chosen_action, title)
        ...
        recommendations = self._analyze_confidence_distribution(confidence_scores, chosen_action)

        # Identify risk indicators
        risk_indicators = self._identify_visualization_risks(confidence_scores, chosen_action)
    ...
    def _analyze_confidence_distribution(
        ...
        if max_score < 0.4:
            recommendations.append("All confidence scores are very low - consider fallback strategies")

        # Check for very close confidence scores
        sorted_scores = sorted(scores, reverse=True)
        if len(sorted_scores) > 1 and (sorted_scores[0] - sorted_scores[1]) < 0.1:
            recommendations.append("Multiple actions have very similar confidence scores")

        # Check chosen action confidence
        if chosen_action:
            chosen_confidence = confidence_scores.get(chosen_action, 0)
            if chosen_confidence < 0.5:
                recommendations.append(f"Chosen action '{chosen_action}' has low confidence ({chosen_confidence:.2f})")

        # Check for overconfidence
        if max_score > 0.9:
            recommendations.append("Some confidence scores are very high - verify calibration")

        return recommendations if recommendations else ["Confidence distribution appears reasonable"]
...
"""
        ...
        return UncertaintyVisualization(
            ...
            metadata={"type": "confidence_distribution", "format": "text", "plotly_available": False},
            recommendations=self._analyze_confidence_distribution(confidence_scores, chosen_action),
            risk_indicators=self._identify_visualization_risks(confidence_scores, chosen_action)
        )
    ...
    def create_uncertainty_evolution_plot(
        ...
        fig.add_trace(
            go.Scatter(x=steps, y=entropies, mode='lines+markers', name='Entropy',
                      line=dict(color='red')),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(x=steps, y=pressures, mode='lines+markers', name='Decision Pressure',
        ...
        fig.add_trace(
            go.Scatter(x=steps, y=variances, mode='lines+markers', name='Variance',
        ...
        fig.add_trace(
            go.Scatter(x=steps, y=ambiguities, mode='lines+markers', name='Ambiguity Index',
    ...
    def _analyze_uncertainty_trends(self, uncertainty_history: List[UncertaintyMetrics]) -> List[str]:
        ...
        if entropy_trend > 0.1:
            recommendations.append("Uncertainty is increasing over time - agent may be becoming less confident")
        elif entropy_trend < -0.1:
            recommendations.append("Uncertainty is decreasing - agent confidence is improving")
        ...
        if high_entropy_steps > len(uncertainty_history) * 0.3:
            recommendations.append(f"{high_entropy_steps} steps had high uncertainty - review decision criteria")

        # Check decision pressure
        low_pressure_steps = sum(1 for m in uncertainty_history if m.decision_pressure < 0.1)
        if low_pressure_steps > len(uncertainty_history) * 0.2:
            recommendations.append(f"{low_pressure_steps} decisions were made under high pressure")

        return recommendations if recommendations else ["Uncertainty evolution appears within normal ranges"]

    def _identify_uncertainty_risks(self, uncertainty_history: List[UncertaintyMetrics]) -> List[Dict[str, Any]]:
        ...
        avg_entropy = sum(m.entropy for m in uncertainty_history) / len(uncertainty_history)
        if avg_entropy > 1.2:
            risks.append({
    ...
    def _calculate_trend(self, values: List[float]) -> float:
        ...
        n = len(values)
        x = list(range(n))
        slope = sum((xi - sum(x)/n) * (yi - sum(values)/n) for xi, yi in zip(x, values)) / sum((xi - sum(x)/n)**2 for xi in x)
        return slope
...
"""
        ...
        return UncertaintyVisualization(
            plot_data={"text_visualization": text_output},
            metadata={"type": "uncertainty_evolution", "format": "text", "plotly_available": False},
            recommendations=self._analyze_uncertainty_trends(uncertainty_history),
            risk_indicators=self._identify_uncertainty_risks(uncertainty_history)
        )
    ...
    def create_decision_tree_visualization(
        ...
        try:
            ...
            return UncertaintyVisualization(
                plot_data={"error": create_error_message(
    ...
    def _analyze_decision_tree(self, trace: DecisionTrace) -> List[str]:
        ...
        if confidences:
            ...
            if avg_confidence < 0.5:
                recommendations.append("Overall decision confidence is low - consider improving agent certainty")
            ...
            if len(confidences) > 3:
                ...
                if second_half and first_half:
                    ...
                    if trend < 0.9:
                        recommendations.append("Decision confidence appears to be declining over time")

        # Check for repeated actions
        actions = [d.chosen_action for d in trace.decisions if d.chosen_action]
        if len(actions) != len(set(actions)):
            recommendations.append("Some actions are being repeated - check for decision loops")

        return recommendations if recommendations else ["Decision tree structure appears reasonable"]
...
"""
        ...
        return UncertaintyVisualization(
            plot_data={"text_visualization": text_output},
            metadata={"type": "decision_tree", "format": "text", "plotly_available": False},
            recommendations=self._analyze_decision_tree(trace),
            risk_indicators=self._identify_tree_risks(trace)
        )
    ...
    def create_comparative_analysis(
        ...
        for name, trace in traces:
            if trace.decisions:
                avg_confidence = sum(
                    ...
                    for d in trace.decisions if d.chosen_action
                ) / len([d for d in trace.decisions if d.chosen_action]) if trace.decisions else 0
        ...
        success_rates = [
            d["successful_decisions"] / max(d["decisions"], 1)
        ...
        fig.add_trace(go.Bar(x=names, y=confidences, name='Avg Confidence'), row=1, col=1)

        # Success rate comparison
        fig.add_trace(go.Bar(x=names, y=success_rates, name='Success Rate'), row=1, col=2)

        # Token usage comparison
        fig.add_trace(go.Bar(x=names, y=tokens, name='Total Tokens'), row=2, col=1)

        # Decision count comparison
        fig.add_trace(go.Bar(x=names, y=decision_counts, name='Decisions'), row=2, col=2)
        ...
        return UncertaintyVisualization(
            plot_data={
                ...
                "best_performer": max(comparison_data, key=lambda x: x["avg_confidence"])["name"] if comparison_data else None
            },
            metadata={
    ...
    def _generate_comparison_recommendations(self, comparison_data: List[Dict[str, Any]]) -> List[str]:
        ...
        if confidence_diff > 0.2:
            recommendations.append(f"{best_agent['name']} significantly outperforms {worst_agent['name']} in confidence")
            recommendations.append(f"Study {best_agent['name']}'s decision patterns for improvement ideas")

        # Check for efficiency differences
        if comparison_data:
            avg_tokens = sum(d["total_tokens"] for d in comparison_data) / len(comparison_data)
            high_token_agents = [d for d in comparison_data if d["total_tokens"] > avg_tokens * 1.5]

            if high_token_agents:
                recommendations.append(f"{' and '.join(d['name'] for d in high_token_agents)} use significantly more tokens - consider optimization")

        return recommendations if recommendations else ["Agent performances are comparable"]

    def _identify_comparison_risks(self, comparison_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Identify risks from comparative analysis."""
        risks = []

        # Check for very low confidence agents
        low_confidence_agents = [d for d in comparison_data if d["avg_confidence"] < 0.4]
        ...
        high_failure_agents = [d for d in comparison_data if d["failed_decisions"] > d["successful_decisions"]]
...
"""
        ...
        return UncertaintyVisualization(
            plot_data={"text_visualization": text_output},
            metadata={"type": "comparative_analysis", "format": "text", "plotly_available": False},
            recommendations=self._generate_comparison_recommendations([]),  # Would need to calculate comparison_data
            risk_indicators=[]
        )
