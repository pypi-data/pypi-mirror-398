...
from .validation import (
    ...
    validate_non_empty_string, validate_probability, create_error_message
)
...
class ProbabilisticDebugSession:
    ...
    probabilistic_breakpoints: List[ProbabilisticBreakpoint] = field(default_factory=list)
...
class ProbabilisticDebugger:
    ...
    @validate_types
    def create_probabilistic_session(
        self,
        trace: DecisionTrace
    ) -> str:
        """
        ...
        session = ProbabilisticDebugSession(
    ...
    def _analyze_confidence_profiles(self, session: ProbabilisticDebugSession):
        ...
        for decision in session.trace.decisions:
            if decision.confidence_scores and decision.chosen_action:
                chosen_confidence = decision.confidence_scores.get(decision.chosen_action, 0)
                ...
                if len(all_confidences) > 1:
                    ...
                    profile.calibration_score = self._calculate_calibration_score(all_confidences)
    ...
    def _analyze_uncertainty_evolution(self, session: ProbabilisticDebugSession):
        ...
        for i, decision in enumerate(session.trace.decisions):
            if decision.confidence_scores:
                ...
                decision_pressure = sorted_confidences[0] - sorted_confidences[1] if len(sorted_confidences) > 1 else 1.0

                # Ambiguity index (how spread out the confidence distribution is)
                ambiguity_index = entropy / math.log(len(confidences)) if len(confidences) > 1 else 0
        ...
        if len(session.uncertainty_history) > 1:
            ...
            entropy_stability = 1.0 - (statistics.stdev(entropies) / statistics.mean(entropies)) if statistics.mean(entropies) > 0 else 1.0
    ...
    def _assess_risks(self, session: ProbabilisticDebugSession):
        ...
        avg_confidence = statistics.mean([
            ...
            for profile in session.confidence_profiles.values()
        ]) if session.confidence_profiles else 0
        ...
        high_pressure_decisions = sum(
            1 for m in session.uncertainty_history
    ...
    def add_probabilistic_breakpoint(
        ...
        try:
            ...
            if condition_type not in ["confidence_threshold", "uncertainty_spike", "decision_pressure"]:
        ...
        breakpoint_id = f"prob_bp_{len(self.sessions[session_id].probabilistic_breakpoints)}"
        ...
        self.logger.info(f"Added probabilistic breakpoint '{description}' to session {session_id}")
    ...
    def analyze_decision_probabilities(
        ...
        uncertainty = session.uncertainty_history[decision_step] if decision_step < len(session.uncertainty_history) else None
        ...
        quality_assessment = self._assess_decision_quality(confidences, decision.chosen_action, decision.confidence_scores)
        ...
        return {
            ...
            "risk_indicators": self._identify_risk_indicators(confidences, decision.chosen_action, decision.confidence_scores)
        }
    ...
    def _assess_decision_quality(
        ...
        margin = chosen_confidence - sorted_confidences[1] if len(sorted_confidences) > 1 else 1.0

        # Confidence percentile
        percentile = sum(1 for c in confidences if c <= chosen_confidence) / len(confidences)
        ...
        return {
            ...
            "recommendation": self._generate_quality_recommendation(quality, margin, chosen_confidence)
        }
    ...
    def _generate_quality_recommendation(
        ...
        if quality == "excellent":
            ...
            return "Decision is reasonable but could benefit from additional validation."
        elif quality == "acceptable":
            return "Decision is marginal - consider fallback options or additional data."
        else:
    ...
    def generate_uncertainty_report(
        ...
        if session.confidence_profiles:
            ...
            confidence_stats = {
                "average_mean_confidence": statistics.mean([p.mean_confidence for p in all_profiles]),
                "overconfidence_incidents": sum(p.overconfidence_count for p in all_profiles),
                "underconfidence_incidents": sum(p.underconfidence_count for p in all_profiles),
                "average_calibration_score": statistics.mean([p.calibration_score for p in all_profiles])
            }
    ...
    def _generate_uncertainty_recommendations(self, session: ProbabilisticDebugSession) -> List[str]:
        """Generate recommendations based on uncertainty analysis."""
        recommendations = []

        # Check overall confidence
        if session.confidence_profiles:
            avg_confidence = statistics.mean([p.mean_confidence for p in session.confidence_profiles.values()])
            if avg_confidence < 0.6:
                recommendations.append("Consider improving agent training data to increase overall confidence levels")

        # Check decision pressure
        if session.uncertainty_history:
            avg_pressure = statistics.mean([m.decision_pressure for m in session.uncertainty_history])
            if avg_pressure < 0.2:
                recommendations.append("Many decisions are made under high pressure - consider adding more distinguishing features")

        # Check risk assessments
        if session.risk_assessments:
            high_risks = [r for r in session.risk_assessments if r.get("severity") == "high"]
            if high_risks:
                recommendations.append(f"Address {len(high_risks)} high-severity risk(s) identified in the analysis")

        if not recommendations:
            recommendations.append("Agent uncertainty characteristics appear within normal ranges")
