...
class AsyncTaskPool:
    ...
    async def submit_callable(self, func: Callable[..., Awaitable[R]], *args, priority: int = 0,
    ...
    async def _execute_task(self, task: Task) -> None:
        ...
        try:
            ...
            logger.debug(f"Task {task.id} completed successfully in {execution_time:.4f}s")

        except asyncio.TimeoutError:
            execution_time = time.time() - start_time
            error = TimeoutError(f"Task {task.id} timed out after {execution_time:.4f}s")
    ...
    def _update_average_execution_time(self, new_time: float) -> None:
        ...
        if completed_count == 1:
            ...
            alpha = 0.1
            self.stats['average_execution_time'] = alpha * new_time + (1 - alpha) * current_avg
...
class ParallelAgentExecutor:
    ...
    def __init__(self, max_parallel_agents: int = 5, task_pool: Optional[AsyncTaskPool] = None):
        self.max_parallel_agents = max_parallel_agents
        self.task_pool = task_pool or AsyncTaskPool(max_concurrent=max_parallel_agents * 2)
    ...
    def _distribute_tasks_to_agents(self, tasks: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for task in tasks:
            ...
            if not agent_id or agent_id not in self.agent_registry:
                ...
                agent_id = self.load_balancer.get_least_loaded_agent(list(self.agent_registry.keys()))
    ...
    async def _execute_agent_tasks(self, agent_id: str, tasks: List[Dict[str, Any]],
                                  execution_context: Dict[str, Any], execution_id: str) -> List[Dict[str, Any]]:
    ...
    async def _execute_single_task(self, agent: Any, task: Dict[str, Any],
...
class StreamingResultAggregator:
    ...
    def _update_average_batch_size(self, batch_size: int) -> None:
        ...
        if batch_count == 1:
            ...
            alpha = 0.1
            self.stats['average_batch_size'] = alpha * batch_size + (1 - alpha) * current_avg

    def get_stream_stats(self, stream_id: str) -> Dict[str, Any]:
        """Get statistics for a specific stream."""
        buffer_size = len(self.result_buffer.get(stream_id, []))
        queue_size = self.batch_queues.get(stream_id, asyncio.Queue()).qsize() if hasattr(asyncio.Queue, 'qsize') else 0
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'stream_details': {sid: self.get_stream_stats(sid) for sid in self.batch_queues.keys()}
        }
