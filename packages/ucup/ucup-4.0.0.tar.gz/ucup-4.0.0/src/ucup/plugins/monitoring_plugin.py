...
class MetricsMonitorPlugin(MonitorPlugin):
    ...
    def initialize(self, config: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            self.request_count = Counter('ucup_requests_total', 'Total requests', ['agent_id', 'method'])
            self.response_time = Histogram('ucup_response_time_seconds', 'Response time', ['agent_id'])
            self.error_count = Counter('ucup_errors_total', 'Total errors', ['agent_id', 'error_type'])
            self.confidence_gauge = Gauge('ucup_confidence', 'Current confidence level', ['agent_id'])
    ...
    def collect_metrics(self, agent_id: str) -> Dict[str, Any]:
        ...
        avg_confidence = sum(metrics["confidence_scores"]) / \
                             len(metrics["confidence_scores"]) if metrics["confidence_scores"] else 0
    ...
    def record_request(self, agent_id: str, method: str = "execute",
                       response_time: float = None, confidence: float = None,
                       error: bool = False):
        ...
        if self.prometheus_enabled:
            ...
            if error:
                self.error_count.labels(agent_id=agent_id, error_type="execution_error").inc()
    ...
    def get_performance_report(self, agent_id: str, time_range: str = "1h") -> Dict[str, Any]:
...
class TracingMonitorPlugin(MonitorPlugin):
    ...
    def collect_metrics(self, agent_id: str) -> Dict[str, Any]:
        ...
        avg_execution_time = sum(t.get("execution_time", 0) for t in traces) / total_traces
    ...
    def record_trace(self, agent_id: str, trace_data: Dict[str, Any]):
        ...
        self.trace_buffer[agent_id] = [
            t for t in traces
            if current_time - t["timestamp"] < self.trace_retention
        ]
