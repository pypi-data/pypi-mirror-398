...
class EmailConfig:
    ...
    def is_configured(self) -> bool:
        ...
        return (self.sender_password and
                self.recipient_emails and
                len(self.recipient_emails) > 0)
...
class TestReportFormatter:
    ...
    def format_test_results(self, test_results: Dict[str, Any], format_type: str = 'html') -> str:
...
"""
        ...
        for benefit in self.ucup_benefits.get('breakdown', [])[:4]:  # Show top 4 benefits
            html += f"""
...
class EmailReporter:
    ...
    def send_test_report(self,
        ...
        try:
            ...
            logger.info(f"Test report sent successfully to {len(self.config.recipient_emails)} recipients")
            return True
...
def setup_email_config_interactive() -> EmailConfig:
    ...
    # Sender Password
    import getpass
    sender_password = getpass.getpass("Sender Password (leave empty to use UCUP_EMAIL_PASSWORD env var): ")
    if sender_password:
