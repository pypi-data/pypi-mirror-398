...
class MetricsTracker:
    ...
    def record_test_run(self, tests_run: int, tests_passed: int,
                        tests_failed: int, duration: float) -> None:
        ...
        with self.lock:
            ...
            if self.benefits.total_tests > 0:
                self.benefits.avg_test_improvement = (
                    self.benefits.total_time_saved / max(1, self.benefits.total_builds + 1)
                )
...
class BenefitsDisplay:
    ...
    def show_build_benefits(cls, tracker: MetricsTracker) -> None:
        ...
        print(cls._colorize("║" + " " * padding + title + " " * (78 - padding - len(title)) + "║", 'cyan'))
        ...
        print(cls._colorize("║" + " " * padding + message + " " * (78 - padding - len(message)) + "║", 'cyan'))
    ...
    @classmethod
    def show_test_benefits(cls, tracker: MetricsTracker,
                           current_tests: int = 0,
                           current_duration: float = 0.0) -> None:
        ...
        padding = (78 - len(title)) // 2
        print(cls._colorize("│" + " " * padding + title + " " * (78 - padding - len(title)) + "│", 'blue'))

        print(cls._colorize("├" + "─" * 78 + "┤", 'blue'))

        # Current test session
        if current_tests > 0:
            print(cls._colorize("│ ", 'blue') +
