...
class APIDiscoveryEngine:
    ...
    def _discover_submodules(self):
        ...
        submodules = [
            "probabilistic", "coordination", "validation", "errors",
            "feature_flags", "memory_management", "metrics", "multimodal",
            "observability", "plugins", "pytest_plugin", "reliability",
            "smart_recommendations", "smart_suggestions", "testing",
            "test_environments", "toon", "advanced_probabilistic",
            "agent_monitoring", "build_wrapper", "cli", "cloud_deployment",
            "config", "deployment_automation", "email_reporting",
            "performance", "self_healing", "time_travel_debugger",
        ...
        for export_name in exports:
            try:
                ...
                if metadata:
                    ...
                    export_meta = self._create_export_metadata(obj, metadata, export_name)
                    self.export_registry[export_name] = export_meta

            except Exception as e:
                warnings.warn(f"Error analyzing {export_name} in {module_name}: {e}")

    def _extract_component_metadata(self, obj: Any, name: str, module: str, category: str) -> Optional[ComponentMetadata]:
    ...
    def _create_export_metadata(self, obj: Any, component_meta: ComponentMetadata, export_name: str) -> ExportMetadata:
    ...
    def _extract_examples(self, obj: Any) -> List[str]:
        ...
        if doc:
            ...
            example_match = re.search(r'Examples?:?\s*\n(.*?)(?:\n\n|\n[A-Z]|$)', doc, re.DOTALL | re.IGNORECASE)
            if example_match:
                example_text = example_match.group(1)
                # Extract code examples
                code_blocks = re.findall(r'```python\s*(.*?)\s*```|`([^`]+)`', example_text, re.DOTALL)
    ...
    def _build_relationships(self):
        ...
        relationships = {
            'ProbabilisticAgent': ['DecisionTracer', 'ProbabilisticResult', 'AlternativePath'],
    ...
    def get_components_by_category(self, category: str) -> List[ComponentMetadata]:
        ...
        return [comp for comp in self.discovered_components.values() if comp.category == category]

    def get_components_by_tag(self, tag: str) -> List[ComponentMetadata]:
        """Get all components with a specific tag."""
        return [comp for comp in self.discovered_components.values() if tag in comp.tags]
    ...
    def search_components(self, query: str) -> List[ComponentMetadata]:
        ...
        for component in self.discovered_components.values():
            if (query_lower in component.name.lower() or
                ...
                any(query_lower in tag for tag in component.tags)):
                results.append(component)

        return results

    def get_import_suggestions(self, current_imports: List[str] = None,
...
def get_smart_import_suggestions(current_imports: List[str] = None,
                               context: str = "") -> List[Dict[str, Any]]:
    """
