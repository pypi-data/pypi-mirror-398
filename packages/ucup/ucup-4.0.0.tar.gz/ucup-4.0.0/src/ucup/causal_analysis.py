...
try:
    ...
    logger.warning(f"NetworkX not available: {e}. Causal analysis features will be disabled.")
...
class CausalFactor:
    ...
    name: str
    description: str = ""
    factor_type: str = "unknown"  # 'root_cause', 'contributing_factor', 'symptom', 'mitigating_factor'
    ...
    def calculate_effectiveness_score(self) -> float:
        ...
        evidence_score = min(1.0, len(self.evidence) / 5.0)  # Cap at 5 pieces of evidence
...
class CausalChain:
    ...
    def calculate_chain_confidence(self) -> float:
        ...
        relationship_avg = np.mean(relationship_strengths) if relationship_strengths else 1.0
...
class CausalGraph:
    ...
    def find_root_causes(self, target_factor_id: str) -> List[CausalFactor]:
        ...
        root_candidates = [node for node in self.graph.nodes()
    ...
    def find_causal_paths(self, source_id: str, target_id: str,
    ...
    def get_shortest_causal_path(self, source_id: str, target_id: str) -> Optional[List[str]]:
    ...
    def analyze_graph_structure(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'average_clustering': nx.average_clustering(self.graph.to_undirected()) if len(self.graph) > 0 else 0.0,
            ...
            'contributing_factors': len([f for f in self.factors.values() if f.is_contributing()]),
            'symptoms': len([f for f in self.factors.values() if f.factor_type == 'symptom'])
        }
    ...
    def get_subgraph_for_factor(self, factor_id: str, depth: int = 2) -> 'CausalGraph':
        ...
        for rel in self.relationships:
            if (rel.source_factor_id in nodes_within_depth and
                rel.target_factor_id in nodes_within_depth):
...
class CausalFailureAnalyzer:
    ...
    async def analyze_failure(self, failure_data: Dict[str, Any],
        ...
        # Create causal chain
        causal_chain = await self._build_causal_chain(factors, relationships, failure_data)
        ...
        analysis_result = {
            ...
            'contributing_factors': [f.to_dict() for f in factors if f.is_contributing()],
        ...
        logger.info(f"Completed causal analysis {analysis_id} with confidence {causal_chain.confidence:.2f}")
        return analysis_result

    async def _extract_causal_factors(self, failure_data: Dict[str, Any],
                                    context_data: Optional[Dict[str, Any]]) -> List[CausalFactor]:
        ...
        config_issues = context_data.get('configuration_issues', []) if context_data else []
    ...
    async def _identify_causal_relationships(self, factors: List[CausalFactor],
                                           failure_data: Dict[str, Any]) -> List[CausalRelationship]:
    ...
    async def _build_causal_chain(self, factors: List[CausalFactor],
                                relationships: List[CausalRelationship],
                                failure_data: Dict[str, Any]) -> CausalChain:
    ...
    async def _generate_recommendations(self, causal_chain: CausalChain) -> List[str]:
        ...
        for root_cause in root_causes:
            if 'configuration' in root_cause.name.lower():
                recommendations.append(f"Review and fix configuration settings related to: {root_cause.description}")
            elif 'resource' in root_cause.name.lower():
                recommendations.append(f"Scale up or optimize resource allocation for: {root_cause.description}")
            elif 'network' in root_cause.name.lower():
                recommendations.append(f"Check network connectivity and configuration for: {root_cause.description}")
            else:
                recommendations.append(f"Address root cause: {root_cause.description}")

        # Add general recommendations
        if causal_chain.confidence < 0.7:
            recommendations.append("Consider gathering more diagnostic information for more accurate analysis")

        if len(root_causes) > 3:
            recommendations.append("Multiple root causes identified - prioritize fixes based on severity and impact")
    ...
    def _are_related(self, factor1: CausalFactor, factor2: CausalFactor) -> bool:
        ...
        return len(common_keywords) > 2 or any(keyword in text1 + text2 for keyword in causal_keywords)
    ...
    def get_failure_patterns(self) -> Dict[str, Any]:
        ...
        for analysis in self.analysis_history:
            failure_type = analysis.get('causal_chain', {}).get('metadata', {}).get('failure_type', 'unknown')
        ...
        return {
            ...
            'most_common_root_cause': max(root_cause_counts.keys(), key=lambda k: root_cause_counts[k]) if root_cause_counts else None,
            'most_common_failure_type': max(failure_type_counts.keys(), key=lambda k: failure_type_counts[k]) if failure_type_counts else None
        }
    ...
    def export_analysis_results(self, filepath: str) -> None:
        ...
        export_data = {
            ...
            'failure_chains': {cid: chain.to_dict() for cid, chain in self.failure_chains.items()},
