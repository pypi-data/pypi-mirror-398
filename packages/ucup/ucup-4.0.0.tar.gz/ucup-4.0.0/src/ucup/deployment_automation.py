...
from .cloud_deployment import (
    CloudDeploymentManager, CloudConfig, DeploymentSpec, DeploymentResult,
    create_cloud_config_from_env, create_deployment_spec_from_config
)
...
class DeploymentAutomator:
    ...
    async def deploy_with_strategy(self,
                                 ...
                                 spec: DeploymentSpec,
                                 config: CloudConfig,
                                 strategy: DeploymentStrategy = DeploymentStrategy.ROLLING_UPDATE) -> Deployment:
        ...
        try:
            ...
            deployment.status = DeploymentStatus.SUCCESS if result.success else DeploymentStatus.FAILED
            ...
            deployment.end_time = datetime.now()
            deployment.results.append(DeploymentResult(
                success=False,
    ...
    async def _deploy_immediate(self, deployment: Deployment) -> DeploymentResult:
        ...
        return await self.cloud_manager.deploy(deployment.config.platform, deployment.spec)
    ...
    async def _deploy_blue_green(self, deployment: Deployment) -> DeploymentResult:
        ...
        green_result = await self.cloud_manager.deploy(deployment.config.platform, deployment.spec)
    ...
    async def _deploy_canary(self, deployment: Deployment) -> DeploymentResult:
        ...
        canary_result = await self.cloud_manager.deploy(deployment.config.platform, canary_spec)
    ...
    async def rollback_deployment(self, deployment_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                await self.cloud_manager.destroy(deployment.config.platform, deployment_id)
    ...
    async def apply_scaling_policy(self, policy_name: str, deployment_id: str):
        ...
        if policy_name not in self.scaling_policies or deployment_id not in self.deployments:
            return False
    ...
    async def _get_deployment_metrics(self, deployment: Deployment) -> Optional[Dict[str, float]]:
    ...
    async def deploy_multi_region(self,
                                spec: DeploymentSpec,
                                config: MultiRegionConfig) -> List[DeploymentResult]:
        """Deploy across multiple regions."""
        results = []

        # Deploy to primary region first
        primary_config = CloudConfig(
            platform=config.primary_region.split('-')[0],  # Extract platform from region
        ...
        for region in config.secondary_regions:
            secondary_config = CloudConfig(
                platform=region.split('-')[0],
                region=region
            )
            ...
            result = await self.cloud_manager.deploy(secondary_config.platform, region_spec)
...
class DeploymentCLI:
        ...
        if pipeline_name not in self.automator.pipelines:
            pipeline = DeploymentPipeline(
                ...
                stages=[
                    ...
                    {"name": "validate", "type": "validate", "checks": ["health", "metrics"]},
    ...
    def create_scaling_policy(self, name: str, metric: str, target: float,
...
def deploy_agent_to_cloud(platform: str, agent_config: Dict[str, Any],
                         environment: str = "dev", strategy: str = "immediate") -> str:
