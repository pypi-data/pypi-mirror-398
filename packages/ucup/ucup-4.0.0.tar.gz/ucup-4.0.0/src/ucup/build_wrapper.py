...
from .metrics import record_build_metrics, show_build_benefits, should_show_donation_prompt, record_donation_prompt_shown, get_global_tracker
...
def run_build_command(command: List[str]) -> int:
    ...
    try:
        ...
        if result.returncode == 0 and project_analysis:
            _save_build_success(project_analysis, {
                'duration': duration,
                'errors': errors,
                'warnings': warnings,
                'command': command,
...
def _analyze_project_for_build() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Note: Could not analyze project for recommendations: {e}", file=sys.stderr)
        return None
...
def _infer_project_description(project_path: Path) -> Optional[str]:
    ...
    try:
        ...
        multimodal_files = list(project_path.glob('**/*vision*.py')) + list(project_path.glob('**/*audio*.py'))
        if multimodal_files:
...
def _show_build_recommendations(analysis: Dict[str, Any]):
    ...
    try:
        ...
        print(f"📋 Project Profile: {profile.project_type.title()} | {profile.domain.title()} | {profile.complexity.title()} complexity")
...
def _save_build_success(analysis: Dict[str, Any], build_metrics: Dict[str, Any]):
    ...
    try:
        ...
        learning_metrics = {
            ...
            'performance_score': 0.8 if build_metrics['duration'] < 60 else 0.6,  # Simple heuristic
