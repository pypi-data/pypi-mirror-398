...
class CoordinatorHealthMetrics:
    ...
    def calculate_health_score(self) -> float:
        ...
        scores = {
            'response_time': max(0, 1.0 - (self.response_time_ms / 5000.0)),  # 5s threshold
            'error_rate': max(0, 1.0 - self.error_rate),  # Error rate penalty
            'memory_usage': max(0, 1.0 - (self.memory_usage_mb / 1000.0)),  # 1GB threshold
            'cpu_usage': max(0, 1.0 - (self.cpu_usage_percent / 90.0)),  # 90% threshold
            'queue_depth': max(0, 1.0 - (self.queue_depth / 1000.0)),  # 1000 queue threshold
            'connections': min(1.0, self.active_connections / 100.0)  # More connections = healthier
        }
...
class CoordinatorHealthMonitor:
    ...
    def __init__(self, check_interval: float = 30.0, unhealthy_threshold: float = 0.6):
        ...
        self.health_history: Dict[str, List[Tuple[float, float]]] = {}  # coordinator_id -> [(timestamp, health_score)]

    def register_coordinator(self, coordinator_id: str, initial_metrics: Optional[CoordinatorHealthMetrics] = None) -> None:
        """Register a coordinator for monitoring."""
        if initial_metrics:
            self.coordinators[coordinator_id] = initial_metrics
        else:
            self.coordinators[coordinator_id] = CoordinatorHealthMetrics(coordinator_id=coordinator_id)
    ...
    def update_metrics(self, coordinator_id: str, metrics: CoordinatorHealthMetrics) -> None:
        ...
        if len(self.health_history[coordinator_id]) > 100:
            self.health_history[coordinator_id] = self.health_history[coordinator_id][-100:]
    ...
    def get_coordinator_health(self, coordinator_id: str) -> Optional[CoordinatorHealthMetrics]:
        """Get health metrics for a coordinator."""
        return self.coordinators.get(coordinator_id)

    def get_health_trend(self, coordinator_id: str, window_seconds: float = 300.0) -> List[Tuple[float, float]]:
        ...
        return [(t, h) for t, h in self.health_history[coordinator_id] if t >= cutoff_time]

    def get_overall_health_status(self) -> Dict[str, Any]:
        """Get overall health status of all coordinators."""
        total_coordinators = len(self.coordinators)
        healthy_count = sum(1 for m in self.coordinators.values() if m.is_healthy(self.unhealthy_threshold))
        unhealthy_count = total_coordinators - healthy_count

        avg_health_score = (sum(m.health_score for m in self.coordinators.values()) / total_coordinators
                          if total_coordinators > 0 else 0.0)
        ...
        return {
            ...
            'health_distribution': {
                'excellent': sum(1 for m in self.coordinators.values() if m.health_score >= 0.9),
                'good': sum(1 for m in self.coordinators.values() if 0.7 <= m.health_score < 0.9),
                'fair': sum(1 for m in self.coordinators.values() if 0.5 <= m.health_score < 0.7),
                'poor': sum(1 for m in self.coordinators.values() if m.health_score < 0.5)
            }
        }
    ...
    def _trigger_recovery(self, coordinator_id: str, metrics: CoordinatorHealthMetrics) -> None:
        """Trigger recovery actions for an unhealthy coordinator."""
        logger.warning(f"Coordinator {coordinator_id} is unhealthy (score: {metrics.health_score:.2f})")
    ...
    def _determine_recovery_strategy(self, metrics: CoordinatorHealthMetrics) -> RecoveryStrategy:
    ...
    def _get_strategy_parameters(self, strategy: RecoveryStrategy, metrics: CoordinatorHealthMetrics) -> Dict[str, Any]:
...
class RestartStrategy:
    ...
    def __init__(self, max_restart_attempts: int = 3, backoff_multiplier: float = 2.0):
        ...
        self.restart_history: Dict[str, List[Tuple[float, bool]]] = {}  # coordinator_id -> [(timestamp, success)]

    async def restart_coordinator(self, coordinator_id: str, coordinator_ref: Any,
        ...
        restart_count = len([r for r in self.restart_history[coordinator_id]
                           if time.time() - r[0] < 3600])  # Last hour

        if restart_count >= self.max_restart_attempts:
            logger.error(f"Max restart attempts ({self.max_restart_attempts}) exceeded for {coordinator_id}")
            return False
        ...
        try:
            ...
            logger.info(f"Restarting coordinator {coordinator_id} in {delay:.1f} seconds")
            ...
            if graceful and hasattr(coordinator_ref, 'shutdown'):
                try:
                    ...
                    logger.warning(f"Graceful shutdown failed for {coordinator_id}: {e}")
            ...
            self.restart_history[coordinator_id] = [
                (t, s) for t, s in self.restart_history[coordinator_id] if t > cutoff_time
            ]
...
class StateRecoveryManager:
    ...
    def __init__(self, checkpoint_dir: str = "./checkpoints"):
        ...
        self.checkpoints: Dict[str, Dict[str, Any]] = {}  # coordinator_id -> checkpoint_id -> metadata
        import os
        os.makedirs(checkpoint_dir, exist_ok=True)

    def create_checkpoint(self, coordinator_id: str, state: Any,
        ...
        logger.info(f"Created checkpoint {checkpoint_id} for coordinator {coordinator_id}")
        return checkpoint_id

    def restore_from_checkpoint(self, coordinator_id: str, checkpoint_id: Optional[str] = None) -> Optional[Any]:
        """
        ...
        if checkpoint_id:
            ...
            latest_checkpoint = max(coordinator_checkpoints.values(),
        ...
        logger.info(f"Restored state from checkpoint {checkpoint_data['checkpoint_id']} for {coordinator_id}")
        return state
    ...
    def delete_old_checkpoints(self, coordinator_id: str, keep_last: int = 5) -> int:
        ...
        logger.info(f"Deleted {deleted_count} old checkpoints for coordinator {coordinator_id}")
    ...
    def _delete_persisted_checkpoint(self, checkpoint_id: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to delete persisted checkpoint {checkpoint_id}: {e}")
...
class FailoverManager:
    ...
    def __init__(self):
        self.primary_coordinators: Dict[str, Any] = {}  # coordinator_id -> coordinator_ref
        self.backup_coordinators: Dict[str, Dict[str, Any]] = {}  # coordinator_id -> backup_id -> backup_ref
        self.active_failovers: Dict[str, asyncio.Task] = {}  # coordinator_id -> failover_task
    ...
    def register_backup(self, primary_id: str, backup_id: str, backup_ref: Any) -> None:
        ...
        logger.info(f"Registered backup coordinator {backup_id} for primary {primary_id}")
    ...
    async def initiate_failover(self, failed_coordinator_id: str) -> bool:
        ...
        if failed_coordinator_id not in self.primary_coordinators:
            logger.warning(f"No primary coordinator registered for {failed_coordinator_id}")
            return False
    ...
    async def _perform_failover(self, failed_coordinator_id: str) -> bool:
        ...
        if not backups:
            logger.warning(f"No backup coordinators available for {failed_coordinator_id}")
            return False

        # Try each backup in order
        for backup_id, backup_ref in backups.items():
            try:
                logger.info(f"Attempting failover from {failed_coordinator_id} to backup {backup_id}")
                ...
                logger.info(f"Successfully failed over {failed_coordinator_id} to {backup_id}")
                return True
    ...
    async def _transfer_state(self, from_coordinator_id: str, to_backup_id: str) -> None:
        """Transfer state from failed coordinator to backup."""
        # This would integrate with StateRecoveryManager to transfer state
        # For now, just log the intent
        logger.info(f"Transferring state from {from_coordinator_id} to backup {to_backup_id}")
...
class SelfHealingCoordinator:
    ...
    async def start(self) -> None:
        ...
        self.is_active = True
        logger.info(f"Self-healing coordinator {self.coordinator_id} started successfully")
    ...
    async def _handle_recovery_action(self, action: RecoveryAction) -> None:
        ...
        logger.info(f"Executing recovery action {action.action_id} for {action.coordinator_id}")
        ...
        try:
            if action.strategy == RecoveryStrategy.RESTART:
                ...
                success = await self.failover_manager.initiate_failover(action.coordinator_id)

            elif action.strategy == RecoveryStrategy.STATE_RECOVERY:
                restored_state = self.state_recovery.restore_from_checkpoint(
                    action.coordinator_id,
                    action.parameters.get('checkpoint_id')
                )
...
class SelfHealingCoordinatorFactory:
    ...
    def create_wrapped_coordinator(self, coordinator_id: str, coordinator: Any,
                                 enable_failover: bool = True) -> SelfHealingCoordinator:
        """
    ...
    def _calculate_overall_health(self, coordinator_healths: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
