...
try:
    ...
    warnings.warn("Visualization dependencies not available. Install with: pip install plotly pandas numpy requests")
...
class PlotlyVisualizer:
    ...
    def plot_uncertainty_analysis(self, results: Dict[str, Any],
        ...
        uncertainties = [r.get('uncertainty_score', 0) for r in results.get('results', [])]
        confidences = [r.get('confidence', 0) for r in results.get('results', [])]
        volatilities = [r.get('volatility_metrics', {}).get('current_volatility', 0)
                        for r in results.get('results', [])]

        # 1. Confidence Distribution Histogram
        if confidences:
            fig.add_trace(
                go.Histogram(x=confidences, nbinsx=20, name="Confidence Scores",
        ...
        if uncertainties and confidences:
            fig.add_trace(
                go.Scatter(x=confidences, y=uncertainties, mode='markers',
                           name="Uncertainty Points", marker_color=config.colors[1]),
                row=1, col=2
            )

        # 3. Volatility Trend
        if volatilities:
            volatility_trend = pd.Series(volatilities).rolling(window=5, min_periods=1).mean()
            fig.add_trace(
                go.Scatter(y=volatility_trend.values, mode='lines+markers',
                           name="Volatility Trend", line_color=config.colors[2]),
                row=2, col=1
            )
        ...
        if alternative_counts:
            fig.add_trace(
                go.Bar(x=list(range(len(alternative_counts))), y=alternative_counts,
                       name="Alternative Predictions", marker_color=config.colors[3]),
                row=2, col=2
            )
        ...
        annotation_text += f"Avg Uncertainty: {summary.get('avg_uncertainty_score', 0):.2f}<br>"
        annotation_text += f"Volatility Range: {summary.get('volatility_range', {}).get('avg', 0):.3f}"

        fig.add_annotation(
            text=annotation_text,
            xref="paper", yref="paper",
            x=1.1, y=1.0, showarrow=False,
            align="left"
        )

        return fig

    def plot_confidence_ranges(self, data: Dict[str, Any],
                               config: ChartConfig = None) -> go.Figure:
        """
    ...
    def plot_performance_trends(self, metrics: List[Dict[str, Any]],
                                config: ChartConfig = None) -> go.Figure:
        """
        ...
        fig = make_subplots(
            rows=3, cols=1,
            subplot_titles=('Confidence Trends', 'Uncertainty Trends', 'Performance Metrics'),
            shared_xaxes=True
        )

        if 'avg_confidence' in df.columns:
            fig.add_trace(
                go.Scatter(x=df.index, y=df['avg_confidence'], name="Avg Confidence",
        ...
        if 'uncertainty_score' in df.columns:
            fig.add_trace(
                go.Scatter(x=df.index, y=df['uncertainty_score'], name="Uncertainty Score",
        ...
        if 'total_failures' in df.columns:
            fig.add_trace(
                go.Scatter(x=df.index, y=df['total_failures'], name="Total Failures",
                           line=dict(color=config.colors[2]), yaxis="y2"),
                row=3, col=1
            )
    ...
    def visualize_agent_matrix(self, agent_data: Dict[str, Any],
                               config: ChartConfig = None) -> go.Figure:
        """
    ...
    def export_chart(self, fig: go.Figure, format: str = 'html',
                     filename: str = 'chart') -> str:
        """
...
class MarketAnalyzer:
    ...
    def fetch_market_data(self, symbol: str, days: int = 30,
                          source: str = 'free') -> List[MarketDataPoint]:
        """
    ...
    def analyze_market_uncertainty(self, symbol: str, days: int = 30) -> Dict[str, Any]:
        ...
        analysis_result = {
            ...
            'price_change_pct': ((prices[-1] - prices[0]) / prices[0] * 100) if prices and len(prices) > 1 else 0,
    ...
    def create_market_visualization(self, analysis_results: Dict[str, Any],
                                    config: ChartConfig = None) -> go.Figure:
        """
        ...
        fig.add_trace(
            go.Scatter(x=periods, y=mock_prices, name="Price Trend",
                       line=dict(color=config.colors[0])),
            row=1, col=1
        )

        # 2. Volatility Analysis
        mock_volatility = [np.random.normal(0.02, 0.005) + np.exp(-i /10) * 0.01 for i in periods]
        fig.add_trace(
            go.Scatter(x=periods, y=mock_volatility, name="Volatility",
                       fill='tozeroy', line=dict(color=config.colors[1])),
            row=1, col=2
        )

        # 3. Confidence Assessment
        confidence_trend = [(analysis_results.get('confidence_score', 0.5) + np.sin(i /5) * 0.1)
                            for i in periods[:15]]
        fig.add_trace(
            go.Bar(x=list(range(15)), y=confidence_trend, name="Analysis Confidence",
                   marker_color=config.colors[2]),
            row=2, col=1
        )

        # 4. Market Prediction (alternative scenarios)
        scenarios = ['Bullish', 'Bearish', 'Neutral', 'High Volatility', 'Low Volatility']
        probabilities = [0.25, 0.20, 0.35, 0.10, 0.10]

        fig.add_trace(
            go.Pie(labels=scenarios, values=probabilities, name="Market Scenarios",
    ...
    def create_multi_asset_comparison(self, symbols: List[str],
...
def create_ucup_visualization(results: Dict[str, Any],
                              visualization_type: str = 'uncertainty',
                              **kwargs) -> Union[go.Figure, str]:
    """
...
def quick_visualize_cli(results: Dict[str, Any], output_file: str = None,
                        format: str = 'html') -> str:
    """
    ...
    """
    if not VISUALIZATION_DEPS_AVAILABLE:
        return "Visualization dependencies not available. Install plotly, pandas, numpy."
