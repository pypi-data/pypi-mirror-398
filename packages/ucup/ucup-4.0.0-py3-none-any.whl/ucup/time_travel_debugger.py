...
class TimeTravelDebugger:
    ...
    def replay_execution(
        ...
        while current_step < len(session.execution_states):
            ...
            for bp in session.breakpoints:
                if bp.check_hit(state):
                    self.logger.info(f"Breakpoint hit: {bp.description} at step {current_step}")
                    breakpoint_hit = True
        ...
        self.logger.info(f"Replay completed for session {session_id}: {len(states)} steps")
        return states
        ...
        for action_info in state.available_actions:
            ...
            if action_name and action_name != state.chosen_action:
                ...
                if hasattr(session.trace.decisions[decision_step], 'rejection_reasons'):
                    rejection_reason = session.trace.decisions[decision_step].rejection_reasons.get(action_name)

                alternatives.append({
                    "action": action_name,
                    "confidence": confidence,
                    "rejection_reason": rejection_reason,
                    "would_have_led_to": self._simulate_alternative_path(session, decision_step, action_name)
                })
    ...
    def create_performance_profile(
        ...
        for state in states:
            ...
            if state.chosen_action and state.confidence_scores:
                confidence_scores.append(state.confidence_scores.get(state.chosen_action, 0))
        ...
        if len(confidence_scores) > 3:
            ...
            if second_half and first_half:
                trend_ratio = sum(second_half) / len(second_half) / (sum(first_half) / len(first_half))
                if trend_ratio > 1.1:
        ...
        return {
            ...
            "decision_efficiency": len([s for s in states if s.chosen_action]) / len(states) if states else 0,
    ...
    def export_debug_session(
        ...
        if format == "json":
            return json.dumps({
                ...
                "execution_states": [state.to_dict() for state in session.execution_states],
                "breakpoints": [
                    {
                        "id": bp.breakpoint_id,
                        ...
                        "last_hit": bp.last_hit.isoformat() if bp.last_hit else None
                    } for bp in session.breakpoints
                ],
            ...
            lines = ["step,timestamp,chosen_action,confidence,token_count,execution_time"]
            for state in session.execution_states:
                confidence = state.confidence_scores.get(state.chosen_action or "", 0)
                lines.append(f"{state.decision_step},{state.timestamp.isoformat()},{state.chosen_action},{confidence},{state.token_count},{state.execution_time}")
            return "\n".join(lines)
