...
class AgentValidator(ABC):
    ...
    @abstractmethod
    async def validate(self, agent_id: str, agent_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def validate(self, agent_id: str, agent_data: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        if response_time > self.thresholds["max_response_time"]:
            issues.append(ValidationIssue(
                ...
                severity=ValidationSeverity.WARNING if response_time < 10 else ValidationSeverity.ERROR,
                message=f"Response time too high: {response_time:.2f}s",
                details={"current": response_time, "threshold": self.thresholds["max_response_time"]},
        ...
        if error_rate > self.thresholds["max_error_rate"]:
            issues.append(ValidationIssue(
                ...
                details={"current": error_rate, "threshold": self.thresholds["max_error_rate"]},
                timestamp=datetime.now()
            ))
        ...
        if throughput < self.thresholds["min_throughput"]:
            issues.append(ValidationIssue(
                ...
                details={"current": throughput, "threshold": self.thresholds["min_throughput"]},
                timestamp=datetime.now()
            ))
        ...
        if memory_usage > self.thresholds["max_memory_usage"]:
            issues.append(ValidationIssue(
                ...
                details={"current": memory_usage, "threshold": self.thresholds["max_memory_usage"]},
                timestamp=datetime.now()
            ))
        ...
        if cpu_usage > self.thresholds["max_cpu_usage"]:
            issues.append(ValidationIssue(
                ...
                details={"current": cpu_usage, "threshold": self.thresholds["max_cpu_usage"]},
                timestamp=datetime.now()
            ))
        ...
        if uncertainty_scores:
            ...
            if high_uncertainty / len(uncertainty_scores) > 0.5:
                issues.append(ValidationIssue(
                    ...
                    details={"high_uncertainty_ratio": high_uncertainty / len(uncertainty_scores)},
                    timestamp=datetime.now()
                ))
...
class AgentMonitor:
    ...
    async def validate_agent(self, agent_id: str, agent_data: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        for validator in self.validators:
            try:
                ...
                self.logger.error(f"Validator {validator.name} failed for agent {agent_id}: {e}")
    ...
    def _calculate_health_score(self, metrics: AgentMetrics, issues: List[ValidationIssue]) -> float:
    ...
    def _generate_recommendations(self, metrics: AgentMetrics, issues: List[ValidationIssue]) -> List[str]:
        """Generate recommendations based on metrics and issues."""
        recommendations = []

        if metrics.response_time_avg > 2.0:
            recommendations.append("Consider optimizing agent response time - current average is high")

        if metrics.error_rate > 0.01:
            recommendations.append("Investigate and reduce error rate - currently above 1%")
        ...
        for issue in issues:
            if "repetitive" in issue.issue_type:
                recommendations.append("Diversify agent responses to avoid repetitive patterns")
            elif "low_confidence" in issue.issue_type:
                recommendations.append("Improve agent confidence through better training or calibration")
            elif "high_uncertainty" in issue.issue_type:
                recommendations.append("Address high uncertainty - consider additional training data")
        ...
        while self.is_monitoring:
            try:
                ...
                for agent_id in list(self.agent_metrics.keys()):
                    ...
                    if health_report and health_report.health_status in [AgentHealth.UNHEALTHY, AgentHealth.CRITICAL]:
...
async def validate_agent(agent_id: str, agent_data: Dict[str, Any]) -> List[ValidationIssue]:
