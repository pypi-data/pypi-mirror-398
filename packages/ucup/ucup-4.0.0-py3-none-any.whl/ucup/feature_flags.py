...
class FeatureFlagManager:
        ...
        for flag_name, flag_config in self.config.flags.items():
            for dep in flag_config.dependencies:
                if dep not in self.config.flags:
                    print(f"Warning: Feature flag '{flag_name}' depends on unknown flag '{dep}'")
                elif not self.is_enabled(dep):
                    print(f"Warning: Feature flag '{flag_name}' depends on disabled flag '{dep}'")
    ...
    def is_enabled(self, flag_name: str) -> bool:
        ...
        return state in (FeatureFlagState.ENABLED, FeatureFlagState.BETA, FeatureFlagState.EXPERIMENTAL)
    ...
    def disable_flag(self, flag_name: str) -> bool:
        ...
        if dependents:
            print(f"Warning: Disabling '{flag_name}' may affect: {', '.join(dependents)}")
    ...
    def _get_warning_text(self, config: FeatureFlagConfig) -> str:
        ...
        elif config.state == FeatureFlagState.EXPERIMENTAL and config.experimental_warning:
