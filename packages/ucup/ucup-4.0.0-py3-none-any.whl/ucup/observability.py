...
from .validation import (
    validate_types, sanitize_inputs, TypeValidator,
    UCUPValidationError, UCUPValueError, validate_probability,
    validate_positive_number, validate_non_empty_string,
    ...
    def __post_init__(self):
        ...
        if self.confidence_scores:
            for action_name, confidence in self.confidence_scores.items():
                try:
                    validate_probability(confidence, f"confidence_scores['{action_name}']")
                except UCUPValueError as e:
                    raise UCUPValueError(
                        create_error_message(
                            context="DecisionNode Initialization",
...
class DecisionTracer:
    ...
    def record_decision(
        ...
        try:
            ...
            available_action_names = [action.get("action", "") for action in available_actions]
            ...
            for action_name in available_action_names:
                if action_name not in confidence_scores:
                    raise UCUPValueError(f"Missing confidence score for action '{action_name}'")
                try:
                    validate_probability(confidence_scores[action_name], f"confidence_scores['{action_name}']")
                except UCUPValueError as e:
                    raise UCUPValueError(f"Invalid confidence for action '{action_name}': {e}")
            ...
            for action_info in decision.available_actions:
                ...
                if action_name == decision.chosen_action:
                    ...
                    nodes.append({
                        ...
                        "confidence": decision.confidence_scores.get(action_name, 0),
                        "level": i + 1,
                        "type": "chosen"
                    })
                else:
...
class ReasoningVisualizer:
    ...
    def create_thought_process_diff(
        self,
        successful_trace: DecisionTrace,
        failed_trace: DecisionTrace
    ) -> go.Figure:
        ...
        successful_confidences = [
            ...
            for d in successful_trace.decisions if d.chosen_action
        ]
        failed_confidences = [
            d.confidence_scores.get(d.chosen_action, 0)
            for d in failed_trace.decisions if d.chosen_action
        ]
    ...
    def create_attention_heatmap(
        ...
        for step in range(len(trace.decisions)):
            step_attention = [0.1 * (step + 1) * (i + 1) for i in range(len(context_elements))]
    ...
    def analyze_tool_usage_patterns(self, traces: List[DecisionTrace]) -> Dict[str, Any]:
                ...
                if decision.chosen_action:
                    ...
                    for action_info in decision.available_actions:
                        if action_info.get("action") == decision.chosen_action:
                            chosen_tool = action_info.get("tool", decision.chosen_action)
                            break
        ...
        return {
            ...
            "most_effective": max(tool_effectiveness.items(), key=lambda x: x[1]) if tool_effectiveness else None,
            "least_effective": min(tool_effectiveness.items(), key=lambda x: x[1]) if tool_effectiveness else None
        }
...
class LiveAgentMonitor:
    ...
    def _setup_default_conditions(self) -> List[AlertCondition]:
        ...
        return [
            AlertCondition(
                ...
                condition=lambda m: m.get("claimed_confidence", 0) > 0.8 and m.get("actual_success", False) is False,
                severity="error",
            ...
            AlertCondition(
                ...
                condition=lambda m: m.get("confidence_trend", 1) < 0.9 and m.get("steps_count", 0) > 3,
                severity="warning",
    ...
    def _calculate_metrics(
        ...
        metrics = {
            ...
            "session_duration": (trace.end_time or datetime.now() - trace.start_time).total_seconds()
        }
        ...
        if confidences:
            ...
            metrics["confidence_volatility"] = pd.Series(confidences).std() if len(confidences) > 1 else 0
            ...
            if len(confidences) > 2:
                ...
                metrics["confidence_trend"] = sum(recent) / len(recent) / (sum(earlier) / len(earlier))
            else:
