...
class AgentMemoryPool:
    ...
    def __init__(self, config: Optional[MemoryPoolConfig] = None):
        ...
        self.executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="memory_pool")

        # Start background GC if configured
        if self.config.gc_interval_seconds > 0:
            self._start_background_gc()

    def store(self, agent_id: str, agent_state: Any, metadata: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        with self.lock:
            ...
            logger.debug(f"Stored agent {agent_id} ({state_size} bytes, compressed={should_compress})")
    ...
    def retrieve(self, agent_id: str) -> Optional[Any]:
        ...
        with self.lock:
            ...
            if agent_id in self.compressed_pool:
                ...
                future = self.executor.submit(self.compressor.decompress, self.compressed_pool[agent_id])
                try:
    ...
    def remove(self, agent_id: str) -> bool:
        ...
        with self.lock:
            ...
            if agent_id in self.compressed_pool:
                ...
                size_removed = len(self.compressed_pool[agent_id]) * 3  # Assume 3:1 compression ratio
                del self.compressed_pool[agent_id]
                self.stats.compressed_objects -= 1
                removed = True
    ...
    def _perform_gc(self) -> None:
        ...
        logger.debug(f"GC completed: collected {collected} objects, memory pressure {memory_pressure:.2f}")
    ...
    def _compress_old_objects(self) -> None:
        ...
        for agent_id, last_access in self.access_times.items():
            if (current_time - last_access) > max_age and agent_id in self.pool:
                state = self.pool[agent_id]
                if state is not None and not self.metadata.get(agent_id, {}).get('compressed', False):
...
class StateCompressor:
    ...
    def compress(self, data: Any) -> bytes:
        ...
        algorithm_byte = best_algorithm.encode('utf-8')[:1]  # First byte of algorithm name
        result = algorithm_byte + best_compressed

        logger.debug(f"Compressed data: {len(serialized)} -> {len(result)} bytes "
...
class LazyAgentLoader:
    ...
    def __init__(self, storage_path: str = "./agent_storage", memory_pool: Optional[AgentMemoryPool] = None):
        self.storage_path = storage_path
        self.memory_pool = memory_pool or AgentMemoryPool()
        self.loaded_agents: Dict[str, weakref.ReferenceType] = {}
        self.agent_metadata: Dict[str, Dict[str, Any]] = {}
        self.executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="lazy_loader")

        # Ensure storage directory exists
        import os
        os.makedirs(storage_path, exist_ok=True)

    def store_agent(self, agent_id: str, agent: Any, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.loaded_agents[agent_id] = weakref.ref(agent, lambda ref: self._on_agent_garbage_collected(agent_id))
    ...
    def _persist_agent(self, agent_id: str, agent: Any, metadata: Optional[Dict[str, Any]]) -> None:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        loaded_count = sum(1 for ref in self.loaded_agents.values() if ref() is not None)
