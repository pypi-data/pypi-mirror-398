...
class DistributedCoordinatorProtocol(ABC):
    ...
    @abstractmethod
    async def list_workers(self, capability_filter: Optional[str] = None) -> List[WorkerNode]:
        """List all registered workers, optionally filtered by capability."""
        pass

    @abstractmethod
    async def assign_task(self, task_data: Dict[str, Any], preferred_worker: Optional[str] = None) -> Optional[str]:
    ...
    @abstractmethod
    async def broadcast_message(self, message: Dict[str, Any], target_workers: Optional[List[str]] = None) -> None:
...
class ConsistentHashRing:
    ...
    def get_nodes_for_key(self, key: str, count: int = 1) -> List[str]:
        ...
        for i, (ring_hash, node_id) in enumerate(ring_items):
            if ring_hash >= hash_value or not result:  # Include first valid node even if hash < key_hash
                if node_id in self.nodes and self.nodes[node_id].is_alive():
...
class DistributedStateStore(ABC):
    ...
    @abstractmethod
    async def store(self, key: str, value: Any, ttl_seconds: Optional[int] = None) -> bool:
        ...
        async with self.lock:
            ...
            for key in keys:
                ...
                if worker_data:
                    ...
                    if capability_filter is None or capability_filter in worker.capabilities:
    ...
    async def broadcast_message(self, message: Dict[str, Any], target_workers: Optional[List[str]] = None) -> None:
        ...
        for worker_id in target_workers:
            if worker_id in self.workers:
                ...
                await self.state_store.store(message_key, message, ttl_seconds=300)  # 5 minute TTL
