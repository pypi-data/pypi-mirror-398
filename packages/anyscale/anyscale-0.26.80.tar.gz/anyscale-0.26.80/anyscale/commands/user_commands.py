...
def list_permissions(user_id: Optional[str], output: Optional[str]) -> None:
    ...
    try:
        ...
        # if a user isn't in org_owners, treat them as collaborator.
        owners = result.get("org_owners") or []
        owner_user_ids = {
            str(o.get("user_id"))
            for o in owners
            if isinstance(o, dict) and o.get("user_id") is not None
        }
        for u in result.get("users") or []:
            ...
            if uid is None:
                continue
            u["org_role"] = "owner" if str(uid) in owner_user_ids else "collaborator"
