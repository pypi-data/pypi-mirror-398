...
`.env` 文件应该放在**运行 Python 脚本的目录**（当前工作目录）。`load_dotenv()` 会自动从当前工作目录向上查找 `.env` 文件。
...
**注意**：
- `.env` 文件应该放在**你的项目目录**，而不是 CTPLite SDK 的安装目录
...
**注意**：`config.py` 会在初始化时自动尝试加载 `.env` 文件，无需手动调用 `load_dotenv()`。如果 `.env` 文件不在当前工作目录，可以手动调用 `load_dotenv()` 指定路径：
