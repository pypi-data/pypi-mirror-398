...
class FreeLLM:
    """
    Simple client for free access to multiple top-tier AI models via talkai.info public endpoint.
    ...
    def _send(self, user_message: str) -> str:
        ...
        try:
            ...
            for line in response.iter_lines(decode_unicode=True):
                ...
                # Skip model name prefix in first chunk
                if first_chunk:
                ...
                # Streaming with smart spacing
                if self.stream:
                    if data:
                        if full_response and last_char not in " \n\t.!?,:;，。！？；：”)]}" and \
                           data[0] not in " \n\t.!?,:;，。！？；：“([{":
