...
class BarPlot(PlotlyPlot):
    ...
    def _create_figure(
        ...
        for feature_positions, series_name, series_data, _series_color in zip(
            positions,
            self._common_settings.labels or self._common_dataset.index,
            data,
            ...
            fig.add_trace(
                Bar(
                    ...
                    name=series_name,
                    text=[
                        round(float(t), self._specific_settings.n_digits) for t in text
                    ],
                )
            )
        fig.update_layout(
        ...
        fig.update_traces(
            ...
            textfont_size=self._common_settings.font_size,
        )
        fig.update_xaxes(
        ...
        fig.update_yaxes(showgrid=self._common_settings.grid, automargin=True)
        return fig
