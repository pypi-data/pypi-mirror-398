...
class ScatterMatrix(DatasetPlot):
    ...
    def __init__(
        ...
        axis_labels_as_keys: bool = False,
        **options: Any,
    ) -> None:
        """
        Args:
            ...
            coloring_variable: The name of the variable used to color the scatter.
        ...
        super().__init__(
            ...
            coloring_variable=coloring_variable,
            axis_labels_as_keys=axis_labels_as_keys,
            kde=kde,
            size=size,
            marker=marker,
    ...
    def _create_specific_data_from_dataset(self) -> tuple[tuple[str, str, int] | None]:
        ...
        if (
            coloring_variable is not None
            and coloring_variable not in self.dataset.variable_names
        ):
            msg = (
                f"{coloring_variable} cannot be used as a coloring_variable "
                f"because it is not a variable name; "
                f"available ones are: {self.dataset.variable_names}."
            )
        ...
        coloring_label = None
        if classifier:
            classifier_label = self._get_label(classifier)[1]
        if self._specific_settings.coloring_variable is not None:
            coloring_label = self._get_label(self._specific_settings.coloring_variable)[
                1
            ]
