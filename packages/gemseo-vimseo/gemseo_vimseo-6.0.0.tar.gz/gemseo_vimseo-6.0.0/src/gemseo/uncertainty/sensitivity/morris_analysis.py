...
class MorrisAnalysis(BaseSensitivityAnalysis):
    ...
    def plot(
        ...
        ax.grid()
        x_offset = offset * (np_max(x_val) - np_min(x_val)) / 100.0
        y_offset = offset * (np_max(y_val) - np_min(y_val)) / 100.0
        index_memory = 0
        for txt in names:
            dimension = len(self._indices.mu_star[output_name][output_component][txt])
            if dimension > 1:
                ...
                ax.annotate(
                    txt,
                    (x_val[index_memory] + x_offset, y_val[index_memory] + y_offset),
                )
            index_memory += dimension
