...
class DistributionSettings(BaseModel):
    """Settings model to defined distributions."""
...
class InterfacedDistributionSettings(BaseModel):
    """Settings model to defined interfaced distributions."""
...
class BaseDistribution(
    ...
    _STANDARD_PARAMETER_NAMES: ClassVar[tuple[str] | None] = None

    _REMAPPING_PARAMETER_NAMES: ClassVar[Mapping[str, str]] = {}
    ...
    def __init__(
        ...
        **options: Any,
        ...
        self._standard_parameter_names = list(standard_parameters.keys())
        # Not satisfying: we are checking the ``parameters`` tuple to infer the type
        # of interface (interface distribution or not) the user used to define the
        # random variable.
        if not standard_parameters:
            self._settings = InterfacedDistributionSettings(
                name=interfaced_distribution,
                parameters=parameters,
                lower_bound=options.get("lower_bound"),
    ...
    @classmethod
    def get_filtered_settings_dict(
        cls, settings: DistributionSettings
    ) -> Mapping[str, Any]:
        """The dictionary of filtered settings."""
        if isinstance(settings, DistributionSettings):
            standard_parameters = settings.model_dump()
            return {
                name: parameter
                for name, parameter in standard_parameters.items()
                if name
                in [
                    *list(cls._STANDARD_PARAMETER_NAMES),
                    "name",
                    "lower_bound",
                    "upper_bound",
                ]
