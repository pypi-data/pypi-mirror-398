...
class ScatterMatrix(MatplotlibPlot):
    ...
    def _create_figures(
        ...
        if coloring_variable:
            c = dataframe.get_view(variable_names=coloring_variable)
            color = c.to_numpy().flatten()
            handles = [
                plt.plot(
                    [],
                    [],
            ...
            labels = (
                [f"{value.item():.2f}" for value in c.to_numpy()]
                if not self._common_settings.labels
                else self._common_settings.labels
            )
            kwargs["c"] = c
            kwargs["cmap"] = "brg"
            dataframe = dataframe.drop(labels=coloring_column, axis=1)
