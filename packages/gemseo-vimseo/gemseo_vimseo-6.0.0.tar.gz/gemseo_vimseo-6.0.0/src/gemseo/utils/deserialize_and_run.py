...
def run(path: Path):
    """Load the serialized disciplmine and input data, run and save the outputs.

    Args:
        path: the path to the directpry containing the serialized discipline and
        input_data.
    """
    workir_path = Path.cwd()
    remote_path = Path("/home/sebastien.bocquet/test_gemseo_ssh/aa20525e")
    serialized_disc_path = remote_path / "discipline.pckl"
    input_data_path = remote_path / "input_data.pckl"
    outputs_path = remote_path / "output_data.pckl"
