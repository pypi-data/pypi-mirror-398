...
class ParameterSpace(DesignSpace):
    ...
    def add_random_vector(
        ...
        settings: DistributionSettings | InterfacedDistributionSettings | None = None,
        **parameters: list[Any],
    ) -> None:
        ...
        if settings:
            if isinstance(settings, InterfacedDistributionSettings):
                ...
                interfaced_distribution = settings_dict["name"]
                interfaced_distribution_parameters = settings_dict["parameters"]
                interfaced_distribution_parameters = tuple(
                    [v] for v in interfaced_distribution_parameters
                )
                parameters_as_tuple = True
            else:
                parameters_as_tuple = True
                parameters = {
                    k: [v]
                    for k, v in distribution_class.get_filtered_settings_dict(
                        settings
                    ).items()
                }
                del parameters["name"]
        else:
            # Strange, it seems it is always the case.
        ...
        for i in range(size):
            ...
            renamed_kwargs = deepcopy(kwargs)
            if settings and interfaced_distribution:
                renamed_kwargs["lower_bound"] = settings.lower_bound
                renamed_kwargs["upper_bound"] = settings.upper_bound
            else:
                for (
                    standard_name,
                    ot_name,
                ) in distribution_class._REMAPPING_PARAMETER_NAMES.items():
                    if standard_name != ot_name:
                        renamed_kwargs[ot_name] = kwargs[standard_name]
                        del renamed_kwargs[standard_name]
            # print("DBG renamed kwargs", renamed_kwargs)
            marginal = distribution_class(**renamed_kwargs)
    ...
    def add_random_variable(
        ...
        **parameters: Any,
        ...
        #         kwargs["interfaced_distribution"] = settings.name
