...
def test_get_location_edge_cases():
    ...
    # Test with grid crossing the 180 meridian
    lat_grid = np.array([-10.0, 0.0, 10.0])
    lon_grid = np.array([170.0, 180.0, -170.0])
    ...
    # The function handles the 180° meridian correctly
    assert col == 1  # 179.0 is closest to 180.0 (distance=1)

    # Test with grid in 0-360 range
    lon_grid = np.array([350.0, 0.0, 10.0])
    row, col = get_location(0.0, 359.0, lat_grid, lon_grid)
    # The function handles wrap-around, so 359.0 is closest to 0.0 (distance=1)
    assert col == 1
