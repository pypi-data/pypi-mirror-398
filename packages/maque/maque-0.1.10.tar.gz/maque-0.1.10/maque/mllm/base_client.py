...
class LLMClientBase(ABC):
    ...
    async def chat_completions(
        ...
        **kwargs,
        ...
        # 检查缓存（缓存不包含 usage 信息，return_usage 时跳过缓存）
        if use_cache and self._response_cache and not return_raw and not return_usage:
    ...
    async def chat_completions_batch(
        ...
        **kwargs,
        ...
        Args:
            ...
            preprocess_msg: 是否预处理消息
            use_cache: 是否使用缓存（断点续传，return_usage=True 时忽略缓存）
    ...
    async def iter_chat_completions_batch(
        ...
        **kwargs,
        ...
        Args:
            ...
            show_progress: 是否显示进度条
            preprocess_msg: 是否预处理消息
            use_cache: 是否使用缓存（断点续传，return_usage=True 时忽略）
            output_file: 输出文件路径（JSONL 格式），用于持久化保存结果
            flush_interval: 文件刷新间隔（秒），默认 1 秒
