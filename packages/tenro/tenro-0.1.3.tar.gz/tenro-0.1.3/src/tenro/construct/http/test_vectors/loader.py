...
Loads JSON fixtures → Validates with Pydantic → Returns dicts for simulation.
"""
...
from tenro.construct.http.schemas.anthropic.messages import Message
from tenro.construct.http.schemas.gemini.generate_content import GenerateContentResponse
from tenro.construct.http.schemas.openai.chat_completions import ChatCompletion
...
class TemplateLoader:
    ...
    _templates: dict[tuple[str, str], type[Any]] = {
    ...
    def load(
        ...
        **overrides: Any,
        ...
        # Validate with Pydantic template
    ...
    def _load_fixture(cls, provider: str, route: str, feature: str) -> dict[str, Any]:
        ...
        # Build path: {provider}/{route}/{feature}_response.json
    ...
    def _validate(cls, provider: str, route: str, data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Validate with Pydantic (extra="allow" permits new fields)
        validated_model = template_class(**data)

        # Return as dict for simulation (model_dump() returns dict[str, Any])
        result = validated_model.model_dump()
        # Type assertion for mypy (Pydantic model_dump always returns dict)
