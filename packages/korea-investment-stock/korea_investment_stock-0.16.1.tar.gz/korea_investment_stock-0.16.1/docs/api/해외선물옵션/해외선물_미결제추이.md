...
> API 경로: `/uapi/overseas-futureoption/v1/quotations/investor-unpd-trend`

---

# REST해외선물 미결제추이 [해외선물-029]

**해외선물 미결제추이 [해외선물-029] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/overseas-futureoption/v1/quotations/investor-unpd-trend  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 모의투자 미지원  
실전 TR ID  | HHDDB95030000 | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
### Data Class (Python)
    ...
    class ResponseBodyoutput1:
        row_cnt: str    #응답레코드카운트
    
    @dataclass
    class ResponseBodyoutput2:
        prod_iscd: str    #상품
        cftc_iscd: str    #CFTC코드
        bsop_date: str    #일자
        bidp_spec: str    #매수투기
        askp_spec: str    #매도투기
        spread_spec: str    #스프레드투기
        bidp_hedge: str    #매수헤지
        askp_hedge: str    #매도헤지
        hts_otst_smtn: str    #미결제합계
        bidp_missing: str    #매수누락
        askp_missing: str    #매도누락
        bidp_spec_cust: str    #매수투기고객
        askp_spec_cust: str    #매도투기고객
        spread_spec_cust: str    #스프레드투기고객
        bidp_hedge_cust: str    #매수헤지고객
        askp_hedge_cust: str    #매도헤지고객
        cust_smtn: str    #고객합계
