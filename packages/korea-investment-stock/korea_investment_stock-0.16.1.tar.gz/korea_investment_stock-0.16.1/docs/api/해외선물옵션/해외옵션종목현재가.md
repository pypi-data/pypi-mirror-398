...
> API 경로: `/uapi/overseas-futureoption/v1/quotations/opt-price`

---

# REST해외옵션종목현재가 [해외선물-035]

**해외옵션종목현재가 [해외선물-035] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/overseas-futureoption/v1/quotations/opt-price  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 모의투자 미지원  
실전 TR ID  | HHDFO55010000 | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
last_price  | 현재가 | String | Y | 15 | 현재가  
...
### Data Class (Python)
    ...
    class ResponseBodyoutput1:
        proc_date: str    #최종처리일자
        proc_time: str    #최종처리시각
        open_price: str    #시가
        high_price: str    #고가
        low_price: str    #저가
        last_price: str    #현재가
        ...
        remn_cnt: str    #잔존일수
        last_qntt: str    #체결량
        tot_ask_qntt: str    #총매도잔량
        tot_bid_qntt: str    #총매수잔량
        tick_size: str    #틱사이즈
        open_date: str    #장개시일자
        open_time: str    #장개시시각
        close_date: str    #장종료일자
        close_time: str    #장종료시각
        sbsnsdate: str    #영업일자
