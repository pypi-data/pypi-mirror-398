...
> API 경로: `/uapi/domestic-stock/v1/quotations/tradprt-byamt`

---

# REST국내주식 체결금액별 매매비중 [국내주식-192]

**국내주식 체결금액별 매매비중 [국내주식-192] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/domestic-stock/v1/quotations/tradprt-byamt  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 미지원  
실전 TR ID  | FHKST111900C0 | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
### Data Class (Python)
    ...
    class ResponseBodyoutput:
        prpr_name: str    #가격명
        smtn_avrg_prpr: str    #합계 평균가격
        acml_vol: str    #합계 거래량
        whol_ntby_qty_rate: str    #합계 순매수비율
        ntby_cntg_csnu: str    #합계 순매수건수
        seln_cnqn_smtn: str    #매도 거래량
        whol_seln_vol_rate: str    #매도 거래량비율
        seln_cntg_csnu: str    #매도 건수
        shnu_cnqn_smtn: str    #매수 거래량
        whol_shun_vol_rate: str    #매수 거래량비율
        shnu_cntg_csnu: str    #매수 건수
