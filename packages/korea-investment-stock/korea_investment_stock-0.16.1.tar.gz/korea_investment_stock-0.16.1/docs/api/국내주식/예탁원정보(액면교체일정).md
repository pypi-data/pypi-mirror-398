...
> API 경로: `/uapi/domestic-stock/v1/ksdinfo/rev-split`

---

# REST예탁원정보(액면교체일정)[국내주식-148]

**예탁원정보(액면교체일정)[국내주식-148] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/domestic-stock/v1/ksdinfo/rev-split  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 미지원  
실전 TR ID  | HHKDB669105C0 | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
### Data Class (Python)
    ...
    class ResponseBodyoutput1:
        ...
        sht_cd: str    #종목코드
        isin_name: str    #종목명
        inter_bf_face_amt: str    #변경전액면가
        inter_af_face_amt: str    #변경후액면가
