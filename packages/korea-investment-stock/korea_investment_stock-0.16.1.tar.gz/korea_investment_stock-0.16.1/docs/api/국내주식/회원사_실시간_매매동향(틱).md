...
> API 경로: `/uapi/domestic-stock/v1/quotations/frgnmem-trade-trend`

---

# WEBSOCKET회원사 실시간 매매동향(틱) [국내주식-163]

**회원사 실시간 매매동향(틱) [국내주식-163] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/domestic-stock/v1/quotations/frgnmem-trade-trend  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 모의투자 미지원  
실전 TR ID  | FHPST04320000 | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
FID_INPUT_ISCD | 종목코드 | String | Y | 12 | ex. 005930(삼성전자)   
  
※ FID_INPUT_ISCD(종목코드) 혹은 FID_MRKT_CLS_CODE(시장구분코드) 둘 중 하나만 입력  
  
FID_INPUT_ISCD_2 | 회원사코드 | String | Y | 10 | ex. 99999(전체)  
  
※ 회원사코드 (kis developers 포탈 사이트 포럼-> FAQ -> 종목정보 다운로드(국내) 참조)  
  
FID_MRKT_CLS_CODE | 시장구분코드 | String | Y | 2 | A(전체),K(코스피), Q(코스닥), K2(코스피200), W(ELW)  
...
### Data Class (Python)
    ...
    class ResponseBodyoutput2:
        ...
        cntg_vol: str    #체결거래량
        acml_ntby_qty: str    #누적순매수수량
        glob_ntby_qty: str    #외국계순매수수량
        frgn_ntby_qty_icdc: str    #외국인순매수수량증감
