...
> API 경로: `/uapi/domestic-futureoption/v1/quotations/exp-price-trend`

---

# REST선물옵션 일중예상체결추이[국내선물-018]

**선물옵션 일중예상체결추이[국내선물-018] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/domestic-futureoption/v1/quotations/exp-price-trend  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 모의투자 미지원  
실전 TR ID  | FHPIF05110100 | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
### Data Class (Python)
    ...
    class ResponseBodyoutput1:
        hts_kor_isnm: str    #영업 시간
        futs_antc_cnpr: str    #업종 지수 현재가
        antc_cntg_vrss_sign: str    #업종 지수 전일 대비
        futs_antc_cntg_vrss: str    #전일 대비 부호
        antc_cntg_prdy_ctrt: str    #업종 지수 전일 대비율
        futs_sdpr: str    #누적 거래 대금
