...
> API 경로: `/uapi/domestic-futureoption/v1/trading/inquire-psbl-ngt-order`

---

# REST(야간)선물옵션 주문가능 조회 [국내선물-011]

**(야간)선물옵션 주문가능 조회 [국내선물-011] 정보** **Method, URL, 실전 Domain, 모의 Domain, Format, Content-Type** Method | GET | URL | /uapi/domestic-futureoption/v1/trading/inquire-psbl-ngt-order  
---|---|---|---  
실전 Domain  | https://openapi.koreainvestment.com:9443 | 모의 Domain | 모의투자 미지원  
실전 TR ID  | (구) JTCE1004R (신) STTN5105R | 모의 TR ID | 모의투자 미지원  
Format |  | Content-Type |   
...
### Data Class (Python)
    ...
    class ResponseBodyoutput:
        max_ord_psbl_qty: str    #최대주문가능수량
        tot_psbl_qty: str    #최대주문가능수량
        lqd_psbl_qty: str    #청산가능수량
        lqd_psbl_qty_1: str    #청산가능수량
