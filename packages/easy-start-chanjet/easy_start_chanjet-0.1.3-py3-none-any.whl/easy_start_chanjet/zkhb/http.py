...
class Api:
    ...
    def get_actual_payment_item_list(
        ...
        Returns:
            Union[Tuple[list, dict, httpx.Response], list, None]:
                - 如果multiple_return_values为True且请求成功：
                ...
                - 如果multiple_return_values为False且请求成功：
                  仅返回payment_list
                - 如果JSON Schema验证失败：
                  返回None（或包含None的元组，取决于multiple_return_values）
        ...
        # JSON Schema验证：确保响应符合预期结构
        if Draft202012Validator(validate_json_schema).is_valid(response_json):
            # 验证通过：根据multiple_return_values参数决定返回内容
            if multiple_return_values:
                # 返回完整结果信息：支付项目列表、响应JSON、响应对象
                return response_json.get("Getpayment", list()), response_json, response
            else:
                # 仅返回支付项目列表
                return response_json.get("Getpayment", list())
        
        # 验证失败：返回None（或包含None的元组）
        if multiple_return_values:
