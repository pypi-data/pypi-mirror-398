...
def scan1d(problem: ProblemSpec, goal: Optional[GoalSpec], cfg: Scan1DConfig) -> Tuple[np.ndarray, List[Dict[str, Any]]]:
    goal = goal or GoalSpec(objective="trajectory")

    mus = _linspace(cfg.start, cfg.stop, cfg.n)
    ...
    for mu_val in mus:
        p2 = _apply_override(current_problem, cfg.mu, float(mu_val))

        if cfg.continuation and last_y_end is not None:
            p2 = _replace_y0(p2, last_y_end)

        res = solve_best(problem=p2, goal=goal)
        sol = res["sol"]
        ...
        rows.append(
            {
                ...
                "continuation": bool(cfg.continuation),
            }
        )
...
def save_scan_csv(rows: List[Dict[str, Any]], path: str) -> None:
    import csv
