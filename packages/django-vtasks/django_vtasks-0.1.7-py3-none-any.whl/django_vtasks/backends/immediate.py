...
class ImmediateBackend(DjangoImmediateBackend):
    ...
    def flush_batch(self, queue_name: str):
        ...
        return super().enqueue(underlying_task, (pending_tasks,), {})
    ...
    async def aflush_batch(self, queue_name: str):
        ...
        if iscoroutinefunction(underlying_task.func):
            return await super().aenqueue(underlying_task, (pending_tasks,), {})
        else:
            # Note: sync_to_async(super().enqueue) might not work due to super() proxy.
            # We use a helper method on self to access super().enqueue.
            return await sync_to_async(self._super_enqueue)(
                underlying_task, (pending_tasks,), {}
            )
