...
class ValkeyTaskAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        try:
            ...
            for key in processing_keys:
                ...
                for task_data_raw in client.lrange(key, 0, -1):
                    try:
                        ...
                        logger.warning(
                            "Failed to deserialize processing task", exc_info=True
                        )
            ...
            logger.exception(
                "Failed to fetch tasks from Valkey backend '%s'", valkey_backend_alias
            )
            self.message_user(
                request,
        ...
        if sort_key:
            ...
            if hasattr(ValkeyTask, sort_field):
                task_instances.sort(
                    key=lambda x: getattr(x, sort_field) or "", reverse=reverse
                )
