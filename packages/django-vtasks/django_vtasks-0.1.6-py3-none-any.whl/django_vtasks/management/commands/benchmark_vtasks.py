    ...
    async def handle(self, *args, **options):
        ...
        try:
            result = subprocess.run(
                clear_cmd, capture_output=True, text=True, timeout=30
            )
            if result.returncode != 0:
                self.stdout.write(
                    self.style.WARNING(f"Failed to clear queue: {result.stderr}")
                )
        ...
        for i in range(count):
            if task_type == "noop":
                ...
                if payload_size > 0:
                    await benchmark_task.aenqueue(
                        0.1, dummy_payload, queue_name=queue_name
                    )
                else:
        ...
        self.stdout.write(
            f"Enqueued {count} tasks in {enqueue_duration:.2f}s "
            f"({enqueue_rate:.2f} ops/s)"
        )
        self.stdout.write(self.style.SUCCESS("-" * 30))
        ...
        self.stdout.write(
            f"Processed {count} tasks in {process_duration:.2f}s "
            f"({process_rate:.2f} ops/s)"
        )
