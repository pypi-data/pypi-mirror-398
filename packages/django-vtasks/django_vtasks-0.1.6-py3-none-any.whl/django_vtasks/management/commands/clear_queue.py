...
class Command(BaseCommand):
    ...
    async def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(
                f"Backend '{backend_alias}' not found in TASKS configuration"
            )
        ...
        if all_queues and not clear_failed:
            if not force:
                self.stdout.write(
                    self.style.WARNING(
                        (
                            f"This will clear ALL queues for backend '{backend_alias}' "
                            f"({backend_class_name})"
                        )
                    )
                )
    ...
    async def _clear_single_queue(self, backend_instance, queue_name, backend_alias):
        ...
        if hasattr(backend_instance, "_clear_queue"):
            ...
            if backend_instance.__class__.__name__ == "DatabaseTaskBackend":
                ...
                raise CommandError(
                    "Queue clearing not implemented for "
                    f"{backend_instance.__class__.__name__}"
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ Cleared {cleared_count} tasks from queue '{queue_name}' "
                f"on backend '{backend_alias}'"
            )
        )
    ...
    async def _clear_postgres_queue(self, backend_instance, queue_name):
        ...
        @sync_to_async
        def delete_tasks():
            return QueuedTask.objects.filter(
                queue=queue_name, status=TaskStatus.QUEUED
            ).delete()
    ...
    async def _clear_all_queues(self, backend_instance, backend_alias):
        ...
        if backend_instance.__class__.__name__ == "DatabaseTaskBackend":
            ...
            raise CommandError(
                "Clear all queues not implemented for "
                f"{backend_instance.__class__.__name__}"
            )

        self.stdout.write(
            self.style.SUCCESS(f"✅ Cleared all queues for backend '{backend_alias}'")
        )
    ...
    async def _clear_failed_queue(self, backend_instance, backend_alias):
        ...
        if backend_instance.__class__.__name__ == "DatabaseTaskBackend":
            ...
            raise CommandError(
                "Clear failed queue not implemented for "
                f"{backend_instance.__class__.__name__}"
            )

        self.stdout.write(
            self.style.SUCCESS(f"✅ Cleared failed tasks for backend '{backend_alias}'")
        )
