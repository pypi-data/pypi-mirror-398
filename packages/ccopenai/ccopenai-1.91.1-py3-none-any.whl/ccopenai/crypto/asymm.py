...
def load_public_key(data: bytes, format: str = "PEM") -> PublicKeyTypes:
    if format == "PEM":
        ...
        raise NotImplementedError(f"public key format {format} is not supported")
...
class RsaEncryptor(Encryptor):
    ...
    def encrypt(self, data: bytes) -> bytes:
        if self.name == "RSA-OAEP-256":
            ...
            raise NotImplementedError(f"encrypted method {self.name} not support")
...
class RsaDecryptor(Decryptor):
    ...
    def decrypt(self, data: bytes) -> bytes:
        if self.name == "RSA-OAEP-256":
            ...
            raise NotImplementedError(f"decrypt method {self.name} not support")
...
class RsaVerifier(Verifier):
    ...
    def verify(self, signature: bytes) -> None:
        if self.name == "RS256":
            ...
            raise NotImplementedError(f"verifier method {self.name} not support")
...
class RsaSigner(Signer):
    ...
    def sign(self) -> bytes:
        if self.name == "RS256":
            ...
            raise NotImplementedError(f"signer method {self.name} not support")
