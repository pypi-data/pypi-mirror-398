...
class AgentTrace:
    ...
    def start(self, script_name: str, metadata: Optional[Dict] = None) -> str:
        ...
        # Store script path for replay support
        # Try to find the actual running script (not just the display name)
        self._script_path = None
        if script_name and os.path.exists(script_name):
            self._script_path = os.path.abspath(script_name)
        else:
            # Auto-detect the caller's script
            import inspect
            for frame_info in inspect.stack():
                file_path = frame_info.filename
                if file_path and not file_path.startswith('<') and os.path.exists(file_path):
                    if file_path.endswith('.py') and 'agenttrace' not in file_path.lower():
                        self._script_path = os.path.abspath(file_path)
                        break
    ...
    def end(self, status: str = "completed") -> Dict[str, Any]:
        ...
        # Read script content for replay support
        script_content = None
        ...
        # Finalize on server (with script for replay)
        response = requests.post(
            ...
            json={
                "trace_id": self.trace_id,
                "status": status,
                "script_content": script_content  # CRITICAL for replay!
            }
        )
