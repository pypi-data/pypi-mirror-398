...
class HTTPXClient:
    ...
    def __init__(
        ...
        http2: bool = False,  # 是否启用 HTTP/2 协议
        ...
        创建一个功能完整的 HTTP 客户端实例，配置各项功能
            ...
            hooks: 钩子注册字典，键为事件名，值为回调函数或函数列表
                支持的事件：
                - before_request: 请求发送前（可修改RequestConfig）
                - after_response: 收到响应后（可修改ResponseData）
                - on_request_failure: 请求失败时（可修改失败的ResponseData）
                - response_from_cache: 使用缓存响应时（通知型）
                - circuit_breaker_open: 断路器打开时（通知型）
                - request_error: 请求发生错误时（通知型）
                - request_failed: 请求最终失败时（通知型）
                - request_retry: 请求重试时（通知型）
                - http_error: HTTP 错误时（通知型）
                - response_received: 收到响应时（通知型，已弃用）
        """
        # ===== 参数验证 =====
        ...
        if hooks:
            self._register_initial_hooks(hooks)  # 注册初始化钩子

    def _register_initial_hooks(self, hooks: Dict[str, Union[Callable, List[Callable]]]) -> None:
        """
        注册初始化时传入的钩子
        
        Args:
            hooks: 钩子字典，键为事件名，值为单个函数或函数列表
        """
        valid_events = {
            "before_request",
            "after_response", 
        ...
        for event, callbacks in hooks.items():
            ...
            if event not in valid_events:
                logger.warning(
                    f"未知的钩子事件 '{event}'，支持的事件: {', '.join(sorted(valid_events))}"
                )
                continue
            ...
            if callable(callbacks):
                ...
                logger.error(
                    f"钩子 '{event}' 的值必须是可调用对象或可调用对象列表，"
                    f"得到: {type(callbacks).__name__}"
                )
                continue
            ...
            for callback in callbacks:
                if not callable(callback):
                    logger.error(
                        f"钩子 '{event}' 中包含不可调用对象: {type(callback).__name__}"
                    )
                    continue
