...
class DensityFlow(nn.Module):
    ...
    def __init__(self,
        ...
        print(f"   - Perturbation size: {self.perturb_size}")
        print(f"   - Covariate size: {self.cov_size}")
        print(f"   - Latent Dimension: {self.latent_dim}")
        print(f"   - Gene Dimension: {self.input_dim}")
        print(f"   - Hidden Dimensions: {self.hidden_layers}")
        print(f"   - Flow transforms: {self.transforms}")
        print(f"   - Flow Hidden Dimensions: {self.flow_hidden_layers}")
    ...
    def setup_networks(self):
        ...
        self.decoder_dz = MLP(
            [self.code_dim + self.perturb_size] + hidden_sizes + [[latent_dim, latent_dim]],
            activation=activate_fct,
            output_activation=[None, Exp],
            post_layer_fct=post_layer_fct,
            post_act_fct=post_act_fct,
        ...
        if self.perturb_size>0:
            self.perturb_effect = zuko.flows.NSF(features=self.latent_dim, 
                                                 context=self.code_dim+self.perturb_size+self.perturb_classes, 
                                                 transforms=self.transforms, 
                                                 hidden_features=self.flow_hidden_layers)
            self.cell_action = zuko.flows.NSF(features=self.latent_dim, 
                                                 context=0, 
                                                 transforms=self.transforms, 
                                                 hidden_features=self.flow_hidden_layers)
            self.action_predictor = MLP(
                [latent_dim] + hidden_sizes + [[latent_dim, latent_dim]],
                activation=activate_fct,
                output_activation=[None, Exp],
                post_layer_fct=post_layer_fct,
                post_act_fct=post_act_fct,
    ...
    def model(self, xs, us=None, ts=None, cs=None):
        ...
        with pyro.plate('data'):
            prior = torch.ones(batch_size, self.code_dim, **self.options)
            ...
            if self.perturb_size>0:
                dzs = pyro.sample('dz', ZukoToPyro(self.cell_action()))
                zs = zns+dzs
            else:
                zs = zns
            if self.cov_size>0:
                zs = zs + self.covariate_effect(cs)
            ...
            if us is not None:
                us_loc,us_scale = self.action_predictor(dzs)
                pyro.sample('u', dist.LogNormal(loc=us_loc,scale=us_scale).to_event(1), obs=us)
    ...
    def guide(self, xs, us=None, ts=None, cs=None):
        with pyro.plate('data'):
            ...
            if self.perturb_size>0:
                us = torch.cat([us,ts], dim=1)
                #dz_loc,dz_scale = self.encoder_dz([ns, us])
                #dzs = pyro.sample('dz', dist.Normal(dz_loc, dz_scale).to_event(1))
                dzs = pyro.sample('dz', ZukoToPyro(self.perturb_effect(torch.cat([ns, us.unsqueeze(0).expand(ns.shape[0], -1, -1)], dim=2))))
                #dzs = pyro.sample('dz', ZukoToPyro(self.perturb_effect(torch.cat([ns, us], dim=1))))

    def _total_effects(self, zs, us, ts):
        ns = self.encoder_n(zs)
        ns = self.softmax(ns)
        dzs = self.perturb_effect(torch.cat([ns, us, ts], dim=1)).sample((100,)).mean(axis=0)
        return dzs
    ...
    def get_complete_embedding(self, xs, us, batch_size:int=1024, show_progress=True):
        xs = self.preprocess(xs)
        xs = convert_to_tensor(xs, device='cpu')
    ...
    def _soft_assignments(self, xs):
        ...
        alpha = dist.Dirichlet(alpha.exp()).mean
        return alpha
    ...
    def predict(self, xs, perturbs_predict:list, perturbs_reference:list, library_sizes=None, show_progress=True):
        perturbs_reference = np.array(perturbs_reference)
        perturbs_predict = np.unique(perturbs_predict)
        
        # basal embedding
        zs = self.get_basal_embedding(xs)
        ps = np.zeros([xs.shape[0], len(perturbs_reference)])
        
        for pert in perturbs_predict:
            if pert in perturbs_reference:
                ind = int(np.where(perturbs_reference==pert)[0])
                ps[:,ind] = 1

        dzs = self.get_cell_shift(zs, perturb_us=ps, show_progress=show_progress)       
        zs = zs + dzs
    ...
    def _cell_shift(self, zs, perturb, perturb_type, sample_size=10):
        ns = self.encoder_n(zs)
        ns = self.softmax(ns)    
        ms = self.perturb_effect(torch.cat([ns, perturb, perturb_type],dim=1)).sample((sample_size,)).mean(axis=0)
        return ms 
    ...
    def get_cell_shift(self, 
        ...
        pt = convert_to_tensor(perturb_types, device='cpu')
        ...
        with tqdm(total=len(dataloader), disable=not show_progress, desc='', unit='batch') as pbar:
            for Z_batch, idx in dataloader:
                ...
                T_batch = pt[idx].to(self.get_device())
                zns = self._cell_shift(Z_batch, P_batch, T_batch, sample_size)
                Z.append(tensor_to_numpy(zns))
                pbar.update(1)
    ...
    def preprocess(self, xs, threshold=0):
        if self.loss_func == 'bernoulli':
            ...
            xs = ad.X.copy()
        elif self.loss_func in ['poisson','multinomial','negbinomial']:
            xs = np.round(xs)
        ...
        with tqdm(total=num_epochs, disable=not show_progress, desc='Training', unit='epoch') as pbar:
            for epoch in range(num_epochs):
                ...
                for batch_x, idx in dataloader:
                    ...
                    batch_y,batch_u,batch_t,batch_c=None,None,None,None
                    if us is not None:
                    ...
                    for loss_id in range(num_losses):
                        new_loss = losses[loss_id].step(xs=batch_x, us=batch_u, ts=batch_t, cs=batch_c)
    ...
    def load_model(cls, file_path):
        ...
        print(f"   - Perturbation size: {model.perturb_size}")
        print(f"   - Covariate size: {model.cov_size}")
        print(f"   - Latent Dimension: {model.latent_dim}")
        print(f"   - Gene Dimension: {model.input_dim}")
        print(f"   - Hidden Dimensions: {model.hidden_layers}")
        print(f"   - Flow Hidden Dimensions: {model.flow_hidden_layers}")
