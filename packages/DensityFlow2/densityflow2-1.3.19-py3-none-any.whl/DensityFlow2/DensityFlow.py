...
class DensityFlow(nn.Module):
    ...
    def setup_networks(self):
        ...
        self.decoder_dz = MLP(
            [self.code_size + self.perturb_size] + hidden_sizes + [[latent_dim, latent_dim]],
            activation=activate_fct,
            output_activation=[None, Exp],
            post_layer_fct=post_layer_fct,
            post_act_fct=post_act_fct,
        ...
        if self.perturb_size>0:
            self.perturb_effect = zuko.flows.NSF(features=self.latent_dim, 
                                                 context=self.perturb_size+self.code_size, 
    ...
    def model(self, xs, ys=None, us=None, cs=None):
        ...
        with pyro.plate('data'):
            ...
            prior = torch.ones(batch_size, self.code_size, **self.options)
            if ys is not None:
                ns = pyro.sample('n', dist.OneHotCategorical(logits=prior), obs=ys)
            else:
    ...
    def guide(self, xs, ys=None, us=None, cs=None):
        with pyro.plate('data'):
    ...
    def _total_effects(self, zs, us):
        ns = self.encoder_n(zs)
        ns = self.softmax(ns)
        dzs = self.perturb_effect(torch.cat([ns, us], dim=1)).sample((100,)).mean(axis=0)
        return dzs
    ...
    def _cell_shift(self, zs, perturb, sample_size=10):
        ns = self.encoder_n(zs)
        ns = self.softmax(ns)    
        ms = self.perturb_effect(torch.cat([ns, perturb],dim=1)).sample((sample_size,)).mean(axis=0)
        return ms 
    ...
    def get_cell_shift(self, 
        ...
        with tqdm(total=len(dataloader), disable=not show_progress, desc='', unit='batch') as pbar:
            for Z_batch, idx in dataloader:
                ...
                zns = self._cell_shift(Z_batch, P_batch, sample_size)
                Z.append(tensor_to_numpy(zns))
                pbar.update(1)
    ...
    def fit(self, xs, 
        ...
        if ys is not None:
            ys = convert_to_tensor(ys, dtype=self.dtype, device='cpu')
        if us is not None:
        ...
        with tqdm(total=num_epochs, disable=not show_progress, desc='Training', unit='epoch') as pbar:
            for epoch in range(num_epochs):
                ...
                for batch_x, idx in dataloader:
                    ...
                    batch_y = ys[idx].to(self.get_device())
                    batch_u = us[idx].to(self.get_device())
                    batch_c = cs[idx].to(self.get_device())
                    for loss_id in range(num_losses):
                        new_loss = losses[loss_id].step(batch_x, batch_y, batch_u, batch_c)
