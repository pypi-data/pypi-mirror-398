...
def put_stadata_on_station(sta,station):
    ...
    sta_list = meteva_base.split(sta)
    sta1_list = []
...
def smooth(grd,smooth_times = 1,used_coords = "xy"):
    ...
    grd_new = meteva_base.grid_data(grid0)
...
def max_on_id(sta1_0, sta2_0, how="left"):
    if sta1_0 is None:
        ...
        datas[datas == meteva_base.IV] = -meteva_base.IV
...
def reset_value_as_IV(sta,iv_value):
    ...
    for name in data_names:
        ...
        sta1.loc[sta1.loc[:,name] == iv_value,name] = meteva_base.IV
    return sta1


def get_ob_from_combined_data(sta_all):
    # data_names = meteva.base.get_stadata_names(sta_all)
    data_names = meteva_base.get_stadata_names(sta_all)
    # sta_ob = meteva.base.sele_by_para(sta_all,member=[data_names[0]])
    sta_ob = meteva_base.sele_by_para(sta_all,member=[data_names[0]])
