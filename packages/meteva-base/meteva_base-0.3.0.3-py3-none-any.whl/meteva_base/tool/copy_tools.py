...
import meteva_base
import time
from meteva_base.io.GDS_data_service import GDSDataService
from meteva_base.io import DataBlock_pb2
...
def copy_data_one_cup(input_path,output_path,read_method,write_method,read_args,write_args):
    ...
    meteva_base.creat_path(output_path)
...
def copy_data(input_root_dir,output_root_dir,read_method,write_method,read_args ={},write_args ={}, recover = False,tran_extension = None,process_count = 1):
    ...
    if output_root_dir[-1] != '/':
        output_root_dir += "/"
    file_list = meteva_base.tool.path_tools.get_path_list_in_dir(input_root_dir)
    ...
    meteva_base.tool.multi_run(process_count = process_count,method=copy_data_one_cup,
...
def copy_m4_to_nc(input_root_dir,output_root_dir,effectiveNum = 3,recover= False,grid = None):
    ...
    file_list = meteva_base.tool.path_tools.get_path_list_in_dir(gds_dir)
    len_input = len(input_root_dir)
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            meteva_base.tool.path_tools.creat_path(path_output)
            if grid is None:
                grd = meteva_base.io.read_griddata_from_micaps4(path_input)
            else:
                grd = meteva_base.io.read_griddata_from_micaps4(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_nc(grd,path_output,effectiveNum)
                end = time.time()
                copyed_num +=1
...
def copy_gds_to_nc(input_root_dir,output_root_dir,effectiveNum = 3,recover= False,grid = None):
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            meteva_base.tool.path_tools.creat_path(path_output)
            if grid is None:
                grd = meteva_base.io.read_griddata_from_gds_file(path_input)
            else:
                grd = meteva_base.io.read_griddata_from_gds_file(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_nc(grd,path_output,effectiveNum)
                end = time.time()
                copyed_num +=1
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            ...
            if grid is None:
                ...
                grd = meteva_base.io.read_griddata_from_gds_file(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_micaps4(grd,path_output,effectiveNum = effectiveNum)
                end = time.time()
                copyed_num +=1
                left_minutes = int((end - start) * ( file_num - i -1) / (copyed_num * 60)) + 1
                print("剩余" + str(left_minutes) + "分钟")
...
def copy_wind_gds_to_nc(input_root_dir,output_root_dir,effectiveNum = 3,recover= False,grid = None):
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            meteva_base.tool.path_tools.creat_path(path_output)
            if grid is None:
                grd = meteva_base.io.read_gridwind_from_gds_file(path_input)
            else:
                grd = meteva_base.io.read_gridwind_from_gds_file(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_nc(grd,path_output,effectiveNum)
                end = time.time()
                copyed_num +=1
...
def copy_wind_gds_to_m11(input_root_dir,output_root_dir,effectiveNum = 3,recover= False,grid = None):
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            meteva_base.tool.path_tools.creat_path(path_output)
            if grid is None:
                grd = meteva_base.io.read_griddata_wind_from_gds_file(path_input)
            else:
                grd = meteva_base.io.read_gridwind_from_gds_file(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_micaps11(grd,path_output,effectiveNum)
                end = time.time()
                copyed_num +=1
                left_minutes = int((end - start) * ( file_num - i -1) / (copyed_num * 60)) + 1
                print("剩余" + str(left_minutes) + "分钟")
...
def copy_wind_m2_to_m11(input_root_dir,output_root_dir,effectiveNum = 3,recover= False,grid = None):
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            meteva_base.tool.path_tools.creat_path(path_output)
            if grid is None:
                grd = meteva_base.io.read_griddata_wind_from_micaps2(path_input)
            else:
                grd = meteva_base.io.read_griddata_wind_from_micaps2(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_micaps11(grd,path_output,effectiveNum)
                end = time.time()
                copyed_num +=1
...
def copy_wind_m11_to_nc(input_root_dir,output_root_dir,effectiveNum = 3,recover= False,grid = None):
    ...
    for i in range(file_num):
        ...
        if not os.path.exists(path_output) or recover:
            meteva_base.tool.path_tools.creat_path(path_output)
            if grid is None:
                grd = meteva_base.io.read_griddata_wind_from_micap11(path_input)
            else:
                grd = meteva_base.io.read_gridwind_from_micap11(path_input,grid=grid)
            if grd is not None:
                meteva_base.io.write_griddata_to_nc(grd,path_output,effectiveNum)
                end = time.time()
                copyed_num +=1
...
def download_gds_files_to_local(ip, port, gds_dir, local_dir,recover= False):
    print(gds_dir)
    filelist = meteva_base.tool.path_tools.get_gds_file_list_in_one_dir(ip, port, gds_dir)
