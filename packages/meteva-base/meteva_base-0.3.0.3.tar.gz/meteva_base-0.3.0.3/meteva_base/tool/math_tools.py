...
def u_v_to_s_d(u,v):
    ...
    d[index] = ag_n0
    s[u==meteva_base.IV] = meteva_base.IV
    d[u==meteva_base.IV] = meteva_base.IV
    return s,d
...
def ctl_proj(grid0,proj_para,dat):
    ...
    inter_i = meteva_base.ER * 1000 / proj_para["dx"]
    inter_j = meteva_base.ER * 1000 / proj_para["dy"]
    if proj_para["type"] == "lcc":
        ...
        grd = meteva_base.grid_data(grid0,data_re)
        return grd
...
def distance_in_a_straight_line(lon1,lat1,lon2,lat2):
    ...
    :return:
    '''
    xyz1 = lon_lat_to_cartesian(lon1, lat1, R=meteva_base.ER)
    xyz2 = lon_lat_to_cartesian(lon2,lat2,R = meteva_base.ER)
    dxyz = xyz2 - xyz1
...
def distance_on_earth_surface(lon1,lat1,lon2,lat2):
    ...
    dis = meteva_base.ER * np.arccos(np.cos(b1)*np.cos(b2)*np.cos(a1-a2) + np.sin(b1)*np.sin(b2))
