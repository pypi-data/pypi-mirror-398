...
def get_time_str_one_by_one(time1,time0 = None,row = 1):
    if row == 3:
        if time0 is None:
            time2 = meteva_base.tool.time_tools.all_type_time_to_datetime(time1)
            if  time2.minute == 0:
                time_str = time2.strftime('%H{h}\n%d{d}\n%Y{y}%m{m}').format(y='年', m='月', d='日',h = "时")
            else:
                time_str = time2.strftime('%H{h}%M{mi}\n%m{m}%d{d}\n%Y{y}').format(y='年', m='月', d='日',h='时',mi = '分')
        else:
            time00 = meteva_base.tool.time_tools.all_type_time_to_datetime(time0)
            time2 = meteva_base.tool.time_tools.all_type_time_to_datetime(time1)
...
def get_time_str_list(time_list,row = 1):
    ...
    for i in range(1,len(time_list)):
        time0 = meteva_base.time_tools.all_type_time_to_datetime(time_list[i])
        time_1 =  meteva_base.time_tools.all_type_time_to_datetime(time_list[i-1])
    ...
    if title is not None:
        ...
        if s is not None:
            ...
            if "id" in r.keys():
                id0 = r["id"]
                del r["id"]
                if id0 == meteva_base.IV or np.isnan(id0):
                    s_str = ""
                else:
                    if id0 in meteva_base.tool.station_id_name_dict.keys():
                        id_str = "id:"+ str(id0) +"(" + meteva_base.tool.station_id_name_dict[id0]+")"
                    else:
                        id_str = "id:" + str(id0)
                    if len(r.keys()) > 0:
                        s_str = str(r) + ""
...
def get_unique_coods(sta):
    ...
    time0 = meteva_base.time_tools.all_type_time_to_datetime(sta["time"].values[0])
    time_1 =  meteva_base.time_tools.all_type_time_to_datetime(sta["time"].values[-1])
...
def get_x_ticks(ticks,width,row = 2):
    ...
    if isinstance(tick0,datetime.datetime) or isinstance(tick0,np.datetime64):
        ...
        if  isinstance(tick0,datetime.datetime):
            for tick in ticks:
                ticks1.append(meteva_base.time_tools.all_type_time_to_time64(tick))
        else:
            ticks1 = ticks
