...
def cuvis_async_call_get(io_pAsyncResult, timeout_ms):
    ...
    ,out] io_pAsyncResult the async handle obtained by calling a async function. If the call finished, the handle will be invalidated
...
def cuvis_async_capture_get(io_pAsyncResult, timeout_ms, o_pMesu):
    ...
    ,out] io_pAsyncResult the async handle obtained by calling 'cuvis_acq_cont_capture_async'. If the call finished, the handle will be invalidated
    timeout_ms the timeout in ms. Give 0 to wait for ever.
    o_pMesu write the measurement handle to this variable, if the call was successful. Else write CUVIS_HANDLE_NULL
    :rtype: int
    :return: 'status_ok' if the async function finished successfully. 'status_timeout' or 'status_deferred' will be returned,
...
def cuvis_session_file_free(o_pSess):
    ...
    ,out] o_pSess The handle to the measurement to be deleted
    :rtype: int
...
def cuvis_measurement_free(io_pMesu):
    ...
    ,out] io_pMesu The handle to the measurement to be deleted
    :rtype: int
...
def cuvis_measurement_get_data_image(i_mesu, i_key, o_pBuf):
    ...
    i_key The data frame identification key (see 'cuvis_measurement_get_data_info' or 'cuvis_reserved_keys)'
...
def cuvis_measurement_get_data_sensor_info(i_mesu, i_key, o_pValue):
    ...
    i_mesu The measurement handle
    i_key the data frame identification key (see 'cuvis_measurement_get_data_info' or 'cuvis_reserved_keys)'
    o_pValue The string buffer to be filled. The provided array must have the length of 'CUVIS_MAXBUF'
    :rtype: int
...
def cuvis_calib_free(io_pCalibration):
    ...
    ,out] io_pCalibration The handle of the calibration. The handle
...
def cuvis_acq_cont_free(io_pAcqCont):
    ...
    ,out] io_pAcqCont The handle of the processing context. The handle
    number will be invalidated.
    :rtype: int
...
def cuvis_proc_cont_create_from_session_file(i_sess, i_loadReferences, o_pProcCont):
    ...
    i_sess The session_file with a valid reference to the processing context
    o_pProcCont The handle of the processing context.
    :rtype: int
    :return: 'status_ok' if the processing context could be loaded
    """
...
def cuvis_proc_cont_free(io_pProcCont):
    ...
    The internal memory is freed.
    ,out] io_pProcCont The handle of the processing context. The handle
    number will be invalidated.
    :rtype: int
...
def cuvis_exporter_free(io_pExporter):
    ...
    ,out] io_pExporter Exporter to be released. If successfully, handle will be invalidated
    :rtype: int
...
def cuvis_viewer_free(io_pViewer):
    ...
    ,out] io_pViewer Viewer to be released. If successfully, handle will be invalidated
    :rtype: int
    :return: 'status_ok' if the exporter was cleared.
    """
...
def cuvis_view_free(io_pView):
    ...
    ,out] io_pView View to be released. If successfully, handle will be invalidated
    :rtype: int
    :return: 'status_ok' if the exporter was cleared.
    """
...
def cuvis_calib_get_capabilities(i_calibration, i_mode, o_pCapabilities):
    ...
    o_pCapabilities write the capabilites here. See CUVIS_MODE_CAPABILITIES_x flags.
    """
...
def cuvis_worker_is_processing(i_worker, o_pIsProcessing):
    ...
    i_worker The worker handle
    o_pIsProcessing If 1, the worker is allowed process measurements. This does not mean, that it is currently working on a measurement - see cuvis_worker_get_threads_busy
    """
