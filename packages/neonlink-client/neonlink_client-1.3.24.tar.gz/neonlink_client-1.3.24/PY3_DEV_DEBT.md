...
### 2.4 Missing Handler Concurrency Config

**Issue:** No configuration for concurrent message handler workers.

**Go SDK Reference:**
```go
HandlerConcurrency int // Number of concurrent handler workers per subscription (default: 1)
```

**Resolution:** This is primarily a server-side/subscription management concern. For Python async, concurrency is handled via asyncio tasks. Add a configuration option for documentation purposes:

```python
@dataclass
class NeonLinkConfig:
    # ... existing fields ...
    handler_concurrency: int = 1  # For documentation/future use
```
